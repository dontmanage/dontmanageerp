...
class IncomingCallSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.telephony.doctype.incoming_call_handling_schedule.incoming_call_handling_schedule import (
	...
	def validate_call_schedule_timeslot(self, schedule: list):
		...
		if errors:
			dontmanage.throw("<br/>".join(errors))
		...
		for day in week_days:
			...
			for i in range(1, len(timeslots)):
				if self.check_timeslots_overlap(timeslots[i - 1], timeslots[i]):
					dontmanage.throw(_("Please fix overlapping time slots for {0}.").format(day))
