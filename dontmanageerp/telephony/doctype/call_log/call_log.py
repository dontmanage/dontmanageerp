...
from dontmanage.contacts.doctype.contact.contact import get_contact_with_phone_number
from dontmanage.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from dontmanage.model.document import Document

from dontmanageerp.crm.doctype.lead.lead import get_lead_with_phone_number
from dontmanageerp.crm.doctype.utils import get_scheduled_employees_for_popup, strip_number
...
class CallLog(Document):
	...
	def on_update(self):
		...
		if _is_call_missed(doc_before_save, self):
			dontmanage.publish_realtime("call_{id}_missed".format(id=self.id), self)
			self.trigger_call_popup()

		if _is_call_ended(doc_before_save, self):
			dontmanage.publish_realtime("call_{id}_ended".format(id=self.id), self)
	...
	def trigger_call_popup(self):
		if self.is_incoming_call():
			...
			for email in emails:
				dontmanage.publish_realtime("show_call_popup", self, user=email)
...
def add_call_summary_and_call_type(call_log, summary, call_type):
	doc = dontmanage.get_doc("Call Log", call_log)
	doc.type_of_call = call_type
	doc.save()
	doc.add_comment("Comment", dontmanage.bold(_("Call Summary")) + "<br><br>" + summary)
...
def get_employees_with_number(number):
	...
	employee_doc_name_and_emails = dontmanage.cache().hget("employees_with_number", number)
	if employee_doc_name_and_emails:
		return employee_doc_name_and_emails

	employee_doc_name_and_emails = dontmanage.get_all(
		"Employee",
		filters={"cell_number": ["like", f"%{number}%"], "user_id": ["!=", ""]},
		fields=["name", "user_id"],
	)

	dontmanage.cache().hset("employees_with_number", number, employee_doc_name_and_emails)
...
def link_existing_conversations(doc, state):
	...
	try:
		...
		for number in numbers:
			...
			for log in logs:
				call_log = dontmanage.get_doc("Call Log", log)
				call_log.add_link(link_type=doc.doctype, link_name=doc.name)
				call_log.save(ignore_permissions=True)
			dontmanage.db.commit()
	except Exception:
		dontmanage.log_error(title=_("Error during caller information update"))
...
def get_linked_call_logs(doctype, docname):
	...
	logs = dontmanage.get_all("Call Log", fields=["*"], filters={"name": ["in", logs]})
