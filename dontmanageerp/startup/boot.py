...
def boot_session(bootinfo):
	...
	bootinfo.custom_css = dontmanage.db.get_value("Style Settings", None, "custom_css") or ""

	if dontmanage.session["user"] != "Guest":
		update_page_info(bootinfo)

		bootinfo.sysdefaults.territory = dontmanage.db.get_single_value("Selling Settings", "territory")
		bootinfo.sysdefaults.customer_group = dontmanage.db.get_single_value("Selling Settings", "customer_group")
		...
		bootinfo.sysdefaults.allow_sales_order_creation_for_expired_quotation = cint(
			dontmanage.db.get_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation")
		)

		# if no company, show a dialog box to create a new company
		bootinfo.customer_count = dontmanage.db.sql("""SELECT count(*) FROM `tabCustomer`""")[0][0]
		...
		bootinfo.docs += dontmanage.db.sql(
		...
		party_account_types = dontmanage.db.sql(""" select name, ifnull(account_type, '') from `tabParty Type`""")
