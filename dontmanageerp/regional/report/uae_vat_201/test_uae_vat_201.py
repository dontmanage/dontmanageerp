...
from dontmanageerp.regional.report.uae_vat_201.uae_vat_201 import (
	get_exempt_total,
...
from dontmanageerp.stock.doctype.warehouse.test_warehouse import get_warehouse_account
...
class TestUaeVat201(TestCase):
	def setUp(self):
		...
		dontmanage.db.sql("delete from `tabSales Invoice` where company='_Test Company UAE VAT'")
		dontmanage.db.sql("delete from `tabPurchase Invoice` where company='_Test Company UAE VAT'")
...
def make_company(company_name, abbr):
	...
	if not dontmanage.db.get_value("Cost Center", {"is_group": 0, "company": company.name}):
...
def set_vat_accounts():
	if not dontmanage.db.exists("UAE VAT Settings", "_Test Company UAE VAT"):
		vat_accounts = dontmanage.get_all(
			"Account",
			fields=["name"],
...
def make_customer():
	if not dontmanage.db.exists("Customer", "_Test UAE Customer"):
...
def make_supplier():
	if not dontmanage.db.exists("Supplier", "_Test UAE Supplier"):
		dontmanage.get_doc(
			{
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	if not dontmanage.db.exists("Warehouse", warehouse_id):
		warehouse = dontmanage.new_doc("Warehouse")
