...
def append_vat_on_sales(data, filters):
	...
	append_data(
		...
		dontmanage.format((-1) * get_tourist_tax_return_total(filters), "Currency"),
		dontmanage.format((-1) * get_tourist_tax_return_tax(filters), "Currency"),
	)
	...
	append_data(
		...
		dontmanage.format(get_reverse_charge_total(filters), "Currency"),
		dontmanage.format(get_reverse_charge_tax(filters), "Currency"),
	)

	append_data(data, "4", _("Zero Rated"), dontmanage.format(get_zero_rated_total(filters), "Currency"), "-")

	append_data(data, "5", _("Exempt Supplies"), dontmanage.format(get_exempt_total(filters), "Currency"), "-")
...
def standard_rated_expenses_emiratewise(data, filters):
	...
	for emirate, amount, vat in total_emiratewise:
		amounts_by_emirate[emirate] = {
			...
			"raw_amount": amount,
			"raw_vat_amount": vat,
			"amount": dontmanage.format(amount, "Currency"),
			"vat_amount": dontmanage.format(vat, "Currency"),
		}
...
def append_vat_on_expenses(data, filters):
	...
	append_data(
		...
		dontmanage.format(get_standard_rated_expenses_total(filters), "Currency"),
		dontmanage.format(get_standard_rated_expenses_tax(filters), "Currency"),
	)
	append_data(
		data,
		"10",
		_("Supplies subject to the reverse charge provision"),
		dontmanage.format(get_reverse_charge_recoverable_total(filters), "Currency"),
		dontmanage.format(get_reverse_charge_recoverable_tax(filters), "Currency"),
	)
