...
dontmanage.query_reports["Fichier des Ecritures Comptables [FEC]"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
...
let fec_export = function(query_report) {
	...
	dontmanage.db.get_value("Company", company, "siren_number", (value) => {
		const company_data = value.siren_number;
		if (company_data === null || company_data === undefined) {
			dontmanage.msgprint(__("Please register the SIREN number in the company information file"));
		} else {
			dontmanage.db.get_value("Fiscal Year", fiscal_year, "year_end_date", (r) => {
...
let downloadify = function(data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		dontmanage.msgprint(__("Export not allowed. You need {0} role to export.", [dontmanage.utils.comma_or(roles)]));
		return;
	}
