...
def get_data(filters):
	...
	if dontmanage.db.exists("KSA VAT Setting", company) is None:
		url = get_url_to_list("KSA VAT Setting")
		dontmanage.msgprint(_('Create <a href="{}">KSA VAT Setting</a> for this company').format(url))
		return data

	ksa_vat_setting = dontmanage.get_doc("KSA VAT Setting", company)
...
def get_tax_data_for_each_vat_setting(vat_setting, filters, doctype):
	...
	for invoice in invoices:
		invoice_items = dontmanage.get_all(
			f"{doctype} Item",
			filters={
				"docstatus": 1,
				"parent": invoice.name,
...
def get_tax_amount(item_code, account_head, doctype, parent):
	...
	item_wise_tax_detail = dontmanage.get_value(
		tax_doctype,
