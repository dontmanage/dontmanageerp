...
from dontmanageerp.regional.report.vat_audit_report.vat_audit_report import execute
...
class TestVATAuditReport(TestCase):
	...
	def tearDown(self):
		dontmanage.db.sql("delete from `tabSales Invoice` where company='_Test Company SA VAT'")
		dontmanage.db.sql("delete from `tabPurchase Invoice` where company='_Test Company SA VAT'")
...
def set_sa_vat_accounts():
	if not dontmanage.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
...
def make_customer():
	if not dontmanage.db.exists("Customer", "_Test SA Customer"):
		dontmanage.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not dontmanage.db.exists("Supplier", "_Test SA Supplier"):
		dontmanage.get_doc(
			{
				"doctype": "Supplier",
