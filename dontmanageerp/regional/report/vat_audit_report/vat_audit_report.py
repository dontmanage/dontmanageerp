...
from dontmanage.utils import formatdate, get_link_to_form
...
class VATAuditReport(object):
	...
	def get_sa_vat_accounts(self):
		self.sa_vat_accounts = dontmanage.get_all(
			"South Africa VAT Account", filters={"parent": self.filters.company}, pluck="account"
		)
		if not self.sa_vat_accounts and not dontmanage.flags.in_test and not dontmanage.flags.in_migrate:
			link_to_settings = get_link_to_form(
				"South Africa VAT Settings", "", label="South Africa VAT Settings"
			)
			dontmanage.throw(_("Please set VAT Accounts in {0}").format(link_to_settings))

	def get_invoice_data(self, doctype):
		conditions = self.get_conditions()
		self.invoices = dontmanage._dict()

		invoice_data = dontmanage.db.sql(
			"""
			SELECT
				{select_columns}
			FROM
	...
	def get_invoice_items(self, doctype):
		self.invoice_items = dontmanage._dict()
	...
	def get_items_based_on_tax_rate(self, doctype):
		self.items_based_on_tax_rate = dontmanage._dict()
		self.item_tax_rate = dontmanage._dict()
		self.tax_doctype = (
			"Purchase Taxes and Charges" if doctype == "Purchase Invoice" else "Sales Taxes and Charges"
		)

		self.tax_details = dontmanage.db.sql(
			"""
			SELECT
	...
	def get_data(self, doctype):
		...
		for rate, section in consolidated_data.items():
			rate = int(rate)
			label = dontmanage.bold(section_name + "- " + "Rate" + " " + str(rate) + "%")
			...
			total = {
				"posting_date": dontmanage.bold(_("Total")),
