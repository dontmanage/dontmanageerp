...
from dontmanage.utils import cstr, nowdate
from dontmanage.utils.data import fmt_money
...
def execute(filters=None):
	filters = filters if isinstance(filters, dontmanage._dict) else dontmanage._dict(filters)
	if not filters:
		filters.setdefault("fiscal_year", get_fiscal_year(nowdate())[0])
		filters.setdefault("company", dontmanage.db.get_default("company"))

	region = dontmanage.db.get_value("Company", filters={"name": filters.company}, fieldname=["country"])
	...
	if filters.supplier_group:
		conditions += "AND s.supplier_group = %s" % dontmanage.db.escape(filters.get("supplier_group"))
...
def irs_1099_print(filters):
	if not filters:
		dontmanage._dict(
			{
				"company": dontmanage.db.get_default("Company"),
				"fiscal_year": dontmanage.db.get_default("Fiscal Year"),
			}
		)
	else:
	...
	company_tin = dontmanage.db.get_value("Company", filters.company, "tax_id")

	columns, data = execute(filters)
	template = dontmanage.get_doc("Print Format", "IRS 1099 Form").html
	output = PdfWriter()
	...
	dontmanage.local.response.filename = (
...
def get_payer_address_html(company):
	address_list = dontmanage.db.sql(
		"""
		SELECT
			name
		FROM
	...
	if address_list:
		...
		address_display = dontmanage.get_doc("Address", company_address).get_display()
...
def get_street_address_html(party_type, party):
	...
	if address_list:
		...
		doc = dontmanage.get_doc("Address", supplier_address)
