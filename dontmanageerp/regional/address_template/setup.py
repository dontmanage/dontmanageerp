...
def update_address_template(country, html, is_default=0):
	...
	if not dontmanage.db.exists("Country", country):
		dontmanage.log_error("Country {} for regional Address Template does not exist.".format(country))
		return

	if dontmanage.db.exists("Address Template", country):
		dontmanage.db.set_value("Address Template", country, "template", html)
		dontmanage.db.set_value("Address Template", country, "is_default", is_default)
	else:
		dontmanage.get_doc(
