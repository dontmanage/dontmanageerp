...
from dontmanageerp.regional.address_template.setup import get_address_templates, update_address_template


def ensure_country(country):
	if dontmanage.db.exists("Country", country):
		return dontmanage.get_doc("Country", country)
	else:
		c = dontmanage.get_doc({"doctype": "Country", "country_name": country})
		c.insert()
		return c
...
class TestRegionalAddressTemplate(TestCase):
	...
	def test_create_address_template(self):
		...
		doc = dontmanage.get_doc("Address Template", country.name)
		self.assertEqual(doc.template, "TEST")

	def test_update_address_template(self):
		"""Update an existing Address Template."""
		country = ensure_country("Germany")
		if not dontmanage.db.exists("Address Template", country.name):
