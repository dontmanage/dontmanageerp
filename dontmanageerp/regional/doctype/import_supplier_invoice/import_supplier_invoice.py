...
from dontmanage.utils import flt, get_datetime_str, today
...
class ImportSupplierInvoice(Document):
	...
	def import_xml_data(self):
		...
		self.default_uom = dontmanage.db.get_value("Stock Settings", fieldname="stock_uom")
	...
	def prepare_data_for_import(self, file_content, file_name, encoded_content):
		for line in file_content.find_all("DatiGeneraliDocumento"):
			...
			if not invoices_args.get("bill_no", ""):
				dontmanage.throw(_("Numero has not set in the XML file"))
	...
	def process_file_data(self):
		...
		dontmanage.enqueue_doc(self.doctype, self.name, "import_xml_data", queue="long", timeout=3600)
...
def get_file_content(file_name, zip_file_object):
	...
	try:
		...
		try:
			...
			dontmanage.log_error("UTF-16 encoding error for File Name: " + file_name)
...
def get_payment_terms_from_file(file_content):
	...
	mop_options = dontmanage.get_meta("Mode of Payment").fields[4].options
	mop_str = re.sub("\n", ",", mop_options)
...
def create_supplier(supplier_group, args):
	...
	existing_supplier_name = dontmanage.db.get_value(
	...
	if existing_supplier_name:
		...
		if not dontmanage.get_list("Contact", filters):
		...
		new_supplier = dontmanage.new_doc("Supplier")
...
def create_address(supplier_name, args):
	...
	existing_address = dontmanage.get_list("Address", filters)
	...
	if args.address_line1:
		...
		for address in existing_address:
			address_doc = dontmanage.get_doc("Address", address["name"])
			if (
...
def create_purchase_invoice(supplier_name, file_name, args, name):
	args = dontmanage._dict(args)
	pi = dontmanage.get_doc(
		{
			"doctype": "Purchase Invoice",
			"company": args.company,
			"currency": dontmanageerp.get_company_currency(args.company),
	...
	try:
		...
		dontmanage.db.set_value("Import Supplier Invoice", name, "status", "Error")
		pi.log_error("Unable to create Puchase Invoice")
		return None


def get_country(code):
	existing_country_name = dontmanage.db.get_value("Country", filters={"code": code}, fieldname="name")
	if existing_country_name:
		return existing_country_name
	else:
		dontmanage.throw(_("Country Code in File does not match with country code set up in the system"))


def create_uom(uom):
	existing_uom = dontmanage.db.get_value("UOM", filters={"uom_name": uom}, fieldname="uom_name")
	if existing_uom:
		return existing_uom
	else:
		new_uom = dontmanage.new_doc("UOM")
