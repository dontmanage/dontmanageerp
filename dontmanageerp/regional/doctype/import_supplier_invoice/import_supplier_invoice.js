...
dontmanage.ui.form.on('Import Supplier Invoice', {
	onload: function(frm) {
		dontmanage.realtime.on("import_invoice_update", function (data) {
	...
	setup: function(frm) {
		...
		frm.set_query("default_buying_price_list", function(doc) {
			return {
				filters: {
					currency: dontmanage.get_doc(":Company", doc.company).default_currency
				}
			};
		});
	},
