...
from dontmanage.utils import flt, money_in_words, round_based_on_smallest_currency_fraction
...
def update_itemised_tax_data(doc):
	...
	meta = dontmanage.get_meta(doc.items[0].doctype)
...
def get_tax_accounts(company):
	...
	tax_accounts_dict = dontmanage._dict()
	tax_accounts_list = dontmanage.get_all("UAE VAT Account", filters={"parent": company}, fields=["Account"])

	if not tax_accounts_list and not dontmanage.flags.in_test:
		dontmanage.throw(_('Please set Vat Accounts for Company: "{0}" in UAE VAT Settings').format(company))
...
def update_totals(vat_tax, base_vat_tax, doc):
	...
	doc.base_in_words = money_in_words(doc.base_grand_total, dontmanageerp.get_company_currency(doc.company))
