...
from dontmanageerp.regional.report.irs_1099.irs_1099 import execute as execute_1099_report


class TestUnitedStates(unittest.TestCase):
	def test_irs_1099_custom_field(self):

		if not dontmanage.db.exists("Supplier", "_US 1099 Test Supplier"):
			doc = dontmanage.new_doc("Supplier")
			...
			supplier = dontmanage.get_doc("Supplier", "_US 1099 Test Supplier")
			self.assertEqual(supplier.irs_1099, 1)

	def test_irs_1099_report(self):
		make_payment_entry_to_irs_1099_supplier()
		filters = dontmanage._dict({"fiscal_year": "_Test Fiscal Year 2016", "company": "_Test Company 1"})
...
def make_payment_entry_to_irs_1099_supplier():
	...
	dontmanage.db.sql("delete from `tabGL Entry` where party='_US 1099 Test Supplier'")
	dontmanage.db.sql("delete from `tabGL Entry` where against='_US 1099 Test Supplier'")
	dontmanage.db.sql("delete from `tabPayment Entry` where party='_US 1099 Test Supplier'")
