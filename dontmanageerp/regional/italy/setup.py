...
from dontmanageerp.regional.italy import (
	fiscal_regimes,
...
def make_custom_fields(update=True):
	...
	custom_fields = {
		"Company": [
			...
			dict(
				...
				print_hide=1,
				options="\n".join(map(lambda x: dontmanage.safe_decode(x, encoding="utf-8"), fiscal_regimes)),
			),
			dict(
			...
			dict(
				...
				print_hide=1,
				options="\n".join(
					map(lambda x: dontmanage.safe_decode(x, encoding="utf-8"), vat_collectability_options)
				),
			),
			dict(
		...
		"Sales Taxes and Charges": [
			dict(
				...
				options="\n"
				+ "\n".join(map(lambda x: dontmanage.safe_decode(x, encoding="utf-8"), tax_exemption_reasons)),
			),
			dict(
		...
		"Mode of Payment": [
			dict(
				...
				print_hide=1,
				options="\n".join(
					map(lambda x: dontmanage.safe_decode(x, encoding="utf-8"), mode_of_payment_codes)
				),
			)
		],
		"Payment Schedule": [
	...
	create_custom_fields(custom_fields, ignore_validate=dontmanage.flags.in_patch, update=update)


def setup_report():
	report_name = "Electronic Invoice Register"
	dontmanage.db.set_value("Report", report_name, "disabled", 0)
