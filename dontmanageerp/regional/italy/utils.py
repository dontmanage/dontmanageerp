...
from dontmanageerp.controllers.taxes_and_totals import get_itemised_tax
...
def export_invoices(filters=None):
	dontmanage.has_permission("Sales Invoice", throw=True)

	invoices = dontmanage.get_all(
		"Sales Invoice", filters=get_conditions(filters), fields=["name", "company_tax_id"]
	)

	attachments = get_e_invoice_attachments(invoices)

	zip_filename = "{}-einvoices.zip".format(dontmanage.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	company = dontmanage.get_doc("Company", invoice.company)
	...
	company_address = dontmanage.get_doc("Address", invoice.company_address)
	...
	if not invoice.type_of_document:
		if invoice.is_return and invoice.return_against:
			...
			invoice.return_against_unamended = get_unamended_name(
				dontmanage.get_doc("Sales Invoice", invoice.return_against)
			)
		else:
			invoice.type_of_document = "TD01"  # Sales Invoice (Fattura)

	# set customer information
	invoice.customer_data = dontmanage.get_doc("Customer", invoice.customer)
	customer_address = dontmanage.get_doc("Address", invoice.customer_address)
	invoice.customer_address_data = customer_address

	if invoice.shipping_address_name:
		invoice.shipping_address_data = dontmanage.get_doc("Address", invoice.shipping_address_name)
...
def download_zip(files, output_filename):
	...
	with zipfile.ZipFile(zip_stream, "w", zipfile.ZIP_DEFLATED) as zip_file:
		for file in files:
			file_path = dontmanage.utils.get_files_path(file.file_name, is_private=file.is_private)

			zip_file.write(file_path, arcname=file.file_name)

	dontmanage.local.response.filename = output_filename
	dontmanage.local.response.filecontent = zip_stream.getvalue()
	dontmanage.local.response.type = "download"
	zip_stream.close()


def get_invoice_summary(items, taxes):
	summary_data = dontmanage._dict()
	for tax in taxes:
			...
			if reference_row:
				items.append(
					dontmanage._dict(
						...
						stock_uom=dontmanage.db.get_single_value("Stock Settings", "stock_uom") or _("Nos"),
						tax_rate=tax.rate,
...
def sales_invoice_validate(doc):
	...
	company_fiscal_regime = dontmanage.get_cached_value("Company", doc.company, "fiscal_regime")
	...
	doc.company_tax_id = dontmanage.get_cached_value("Company", doc.company, "tax_id")
	doc.company_fiscal_code = dontmanage.get_cached_value("Company", doc.company, "fiscal_code")
	...
	customer = dontmanage.get_doc("Customer", doc.customer)
	...
	if not doc.customer_address:
		dontmanage.throw(_("Please set the Customer Address"), title=_("E-Invoicing Information Missing"))
	else:
	...
	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = dontmanage.get_cached_value(
...
def sales_invoice_on_submit(doc, method):
	...
	if not len(doc.payment_schedule):
		dontmanage.throw(_("Please set the Payment Schedule"), title=_("E-Invoicing Information Missing"))
	else:
		for schedule in doc.payment_schedule:
			...
			elif not dontmanage.db.get_value("Mode of Payment", schedule.mode_of_payment, "mode_of_payment_code"):
				dontmanage.throw(
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	item_meta = dontmanage.get_meta("Sales Invoice Item")

	invoice_xml = dontmanage.render_template(
		"dontmanageerp/regional/italy/e-invoice.xml",
...
def generate_single_invoice(docname):
	doc = dontmanage.get_doc("Sales Invoice", docname)
	dontmanage.has_permission("Sales Invoice", doc=doc, throw=True)
...
def validate_address(address_name):
	...
	data = dontmanage.get_cached_value("Address", address_name, fields, as_dict=1) or {}
...
def get_progressive_name_and_number(doc, replace=False):
	...
	progressive_name = dontmanage.model.naming.make_autoname(company_tax_id + "_.#####")
