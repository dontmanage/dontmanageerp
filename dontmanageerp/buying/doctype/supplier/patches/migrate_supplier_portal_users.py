...
def execute():
	try:
		...
		dontmanage.log_error("Failed to migrate portal users")
...
def add_portal_users(contacts):
	for contact in contacts:
		user = dontmanage.db.get_value("User", {"email": contact.portal_user}, "name")
		if not user:
			continue
		...
		portal_user_doc = dontmanage.new_doc("Portal User")
