...
class Supplier(TransactionBase):
	...
	def autoname(self):
		supp_master_name = dontmanage.defaults.get_global_default("supp_master_name")
	...
	def _add_supplier_role(self, portal_user):
		...
		user_doc = dontmanage.get_doc("User", portal_user.user)
		...
		dontmanage.msgprint(_("Added Supplier Role to User {0}.").format(dontmanage.bold(user_doc.name)), alert=True)

	def validate(self):
		self.flags.is_new_doc = self.is_new()

		# validation for Naming Series mandatory field...
		if dontmanage.defaults.get_global_default("supp_master_name") == "Naming Series":
	...
	def get_supplier_group_details(self):
		doc = dontmanage.get_doc("Supplier Group", self.supplier_group)
		self.payment_terms = ""
		self.accounts = []
	...
	def validate_internal_supplier(self):
		...
		internal_supplier = dontmanage.db.get_value(
			"Supplier",
			{
	...
	def create_primary_contact(self):
		from dontmanageerp.selling.doctype.customer.customer import make_contact
	...
	def create_primary_address(self):
		...
		from dontmanageerp.selling.doctype.customer.customer import make_address
	...
	def after_rename(self, olddn, newdn, merge=False):
		if dontmanage.defaults.get_global_default("supp_master_name") == "Supplier Name":
...
def get_supplier_primary_contact(doctype, txt, searchfield, start, page_len, filters):
	...
	contact = dontmanage.qb.DocType("Contact")
	dynamic_link = dontmanage.qb.DocType("Dynamic Link")

	return (
		dontmanage.qb.from_(contact)
		.join(dynamic_link)
