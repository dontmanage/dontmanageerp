...
dontmanage.ui.form.on("Supplier", {
	setup: function (frm) {
		...
		frm.set_query("supplier_primary_contact", function(doc) {
			return {
				query: "dontmanageerp.buying.doctype.supplier.supplier.get_supplier_primary_contact",
				filters: {
					"supplier": doc.name
				}
			};
	...
	refresh: function (frm) {
		dontmanage.dynamic_link = { doc: frm.doc, fieldname: 'name', doctype: 'Supplier' }

		if (dontmanage.defaults.get_default("supp_master_name") != "Naming Series") {
		...
		if (frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Accounts Payable'), function () {
				dontmanage.set_route('query-report', 'Accounts Payable', { supplier: frm.doc.name });
			}, __("View"));

			frm.add_custom_button(__('Bank Account'), function () {
				dontmanageerp.utils.make_bank_account(frm.doc.doctype, frm.doc.name);
			}, __('Create'));
