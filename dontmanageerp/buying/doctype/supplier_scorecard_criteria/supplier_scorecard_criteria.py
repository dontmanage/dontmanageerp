...
class InvalidFormulaVariable(dontmanage.ValidationError):
	pass
...
class SupplierScorecardCriteria(Document):
	...
	def validate_formula(self):
		...
		try:
			dontmanage.safe_eval(test_formula, None, {"max": max, "min": min})
		except Exception:
			dontmanage.throw(_("Error evaluating the criteria formula"))


@dontmanage.whitelist()
def get_criteria_list():
	criteria = dontmanage.db.sql(
		"""
		SELECT
			scs.name
		FROM
...
def get_variables(criteria_name):
	criteria = dontmanage.get_doc("Supplier Scorecard Criteria", criteria_name)
	...
	for dummy1, match in enumerate(mylist):
		for dummy2 in range(0, len(match.groups())):
			try:
				...
				dontmanage.throw(_("Unable to find variable:") + " " + str(match.group(1)), InvalidFormulaVariable)
