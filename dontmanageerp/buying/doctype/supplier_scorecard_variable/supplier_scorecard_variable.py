...
class VariablePathNotFound(dontmanage.ValidationError):
	pass
	...
	def validate_path_exists(self):
		if "." in self.path:
			try:
				...
				dontmanage.throw(_("Could not find path for " + self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	"""Gets the number of days in this period"""
	supplier = dontmanage.get_doc("Supplier", scorecard.supplier)
	total_item_days = dontmanage.db.sql(
		"""
			SELECT
...
def get_cost_of_on_time_shipments(scorecard):
	...
	total_delivered_on_time_costs = dontmanage.db.sql(
		"""
			SELECT
...
def get_total_days_late(scorecard):
	...
	supplier = dontmanage.get_doc("Supplier", scorecard.supplier)
	total_delivered_late_days = dontmanage.db.sql(
		"""
			SELECT
	...
	total_missed_late_days = dontmanage.db.sql(
		"""
			SELECT
...
def get_on_time_shipments(scorecard):
	...
	# Look up all PO Items with delivery dates between our dates
	total_items_delivered_on_time = dontmanage.db.sql(
		"""
			SELECT
				COUNT(pr_item.qty)
			FROM
...
def get_ordered_qty(scorecard):
	...
	po = dontmanage.qb.DocType("Purchase Order")

	return (
		dontmanage.qb.from_(po)
...
def get_rfq_response_days(scorecard):
	...
	supplier = dontmanage.get_doc("Supplier", scorecard.supplier)
	total_sq_days = dontmanage.db.sql(
		"""
			SELECT
