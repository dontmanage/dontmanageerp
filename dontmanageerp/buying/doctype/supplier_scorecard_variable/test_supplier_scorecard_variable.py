...
from dontmanageerp.buying.doctype.supplier_scorecard_variable.supplier_scorecard_variable import (
	VariablePathNotFound,
)


class TestSupplierScorecardVariable(DontManageTestCase):
	def test_variable_exist(self):
		for d in test_existing_variables:
			my_doc = dontmanage.get_doc("Supplier Scorecard Variable", d.get("name"))
	...
	def test_path_exists(self):
		...
		for d in test_bad_variables:
			self.assertRaises(VariablePathNotFound, dontmanage.get_doc(d).insert)
