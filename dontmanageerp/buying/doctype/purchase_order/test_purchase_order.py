...
from dontmanage.utils import add_days, flt, getdate, nowdate
...
from dontmanageerp.accounts.party import get_due_date_from_template
...
class TestPurchaseOrder(DontManageTestCase):
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			dontmanage.ValidationError, update_child_qty_rate, "Purchase Order", trans_item, po.name
		)
	...
	def test_purchase_order_invoice_receipt_workflow(self):
		from dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice import make_purchase_receipt
	...
	def test_make_purchase_invoice(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_pi_from_po, po.name)
		...
		dontmanage.get_doc(
			{
				...
				"currency": dontmanage.get_cached_value("Company", "_Test Company", "default_currency"),
				"conversion_factor": 1,
	...
	def test_po_for_blocked_supplier_all(self):
		...
		self.assertRaises(dontmanage.ValidationError, create_purchase_order)
	...
	def test_terms_are_not_copied_if_automatically_fetch_payment_terms_is_unchecked(self):
		...
		dontmanage.db.set_value("Company", "_Test Company", "payment_terms", "_Test Payment Term Template 1")
		pi = make_pi_from_po(po.name)
		pi.save()

		self.assertEqual(pi.get("payment_terms_template"), "_Test Payment Term Template 1")
		dontmanage.db.set_value("Company", "_Test Company", "payment_terms", "")
	...
	def test_advance_payment_entry_unlink_against_purchase_order(self):
		...
		po_doc = dontmanage.get_doc("Purchase Order", po_doc.name)
		po_doc.cancel()

		pe_doc = dontmanage.get_doc("Payment Entry", pe.name)
		pe_doc.cancel()

	def create_account(self, account_name, company, currency, parent):
		if not dontmanage.db.get_value("Account", filters={"account_name": account_name, "company": company}):
			...
			account = dontmanage.get_doc("Account", {"account_name": account_name, "company": company})
	...
	def test_po_optional_blanket_order(self):
		...
		po_doc = dontmanage.get_doc("Purchase Order", po.get("name"))
	...
	def test_new_sc_flow(self):
		from dontmanageerp.buying.doctype.purchase_order.purchase_order import make_subcontracting_order
		...
		self.assertRaises(dontmanage.ValidationError, sco.save)
		...
		self.assertRaises(dontmanage.ValidationError, make_subcontracting_order, po.name)
...
def create_purchase_order(**args):
	...
	po.currency = args.currency or dontmanage.get_cached_value("Company", po.company, "default_currency")
...
def get_ordered_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(dontmanage.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "ordered_qty"))


def get_requested_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(dontmanage.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty"))


test_dependencies = ["BOM", "Item Price"]

test_records = dontmanage.get_test_records("Purchase Order")
