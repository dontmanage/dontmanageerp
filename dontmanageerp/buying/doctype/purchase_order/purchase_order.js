...
dontmanageerp.buying.PurchaseOrderController = class PurchaseOrderController extends dontmanageerp.buying.BuyingController {
	...
	get_items_from_open_material_requests() {
		dontmanageerp.utils.map_current_doc({
			...
			get_query_method: "dontmanageerp.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	}
					...
					while (i < item_length) {
						...
						(r.message[0] || []).forEach(function(d) {
							if (d.qty > 0 && qty > 0 && me.frm.doc.items[i].item_code == d.item_code && !me.frm.doc.items[i].material_request_item)
							{
								...
								dontmanage.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + me.frm.doc.items[i].idx + ")");
								if (qty > 0) {
									dontmanage.msgprint("Splitting " + qty + " units of " + d.item_code);
									var new_row = dontmanage.model.add_child(me.frm.doc, me.frm.doc.items[i].doctype, "items");
									item_length++;
