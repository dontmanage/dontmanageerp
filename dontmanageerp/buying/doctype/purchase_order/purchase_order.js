	...
	get_materials_from_supplier: function (frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "dontmanageerp.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
						...
						callback: function (r) {
							if (r && r.message) {
								const doc = dontmanage.model.sync(r.message);
...
dontmanageerp.buying.PurchaseOrderController = class PurchaseOrderController extends (
	...
	get_items_from_open_material_requests() {
		dontmanageerp.utils.map_current_doc({
			method: "dontmanageerp.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method:
				"dontmanageerp.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier",
		});
	}
	...
	make_stock_entry() {
		dontmanage.call({
			method: "dontmanageerp.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_purchase_receipt() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: cur_frm,
	...
	make_subcontracting_order() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: cur_frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				dontmanageerp.utils.map_current_doc({
					method: "dontmanageerp.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
							...
							(r.message[0] || []).forEach(function (d) {
								if (
									...
									if (qty > 0) {
										...
										var new_row = dontmanage.model.add_child(
											me.frm.doc,
...
extend_cscript(cur_frm.cscript, new dontmanageerp.buying.PurchaseOrderController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (label, status) {
	dontmanage.call({
		method: "dontmanageerp.buying.doctype.purchase_order.purchase_order.update_status",
...
dontmanage.ui.form.on("Purchase Order", "is_subcontracted", function (frm) {
	if (frm.doc.is_old_subcontracting_flow) {
