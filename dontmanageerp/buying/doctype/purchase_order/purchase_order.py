...
class PurchaseOrder(BuyingController):
		...
		for d in self.get("items"):
			if d.item_code:
				...
				if last_purchase_details:
					...
					item_last_purchase_rate = dontmanage.get_cached_value("Item", d.item_code, "last_purchase_rate")
	...
	def check_modified_date(self):
		...
		date_diff = dontmanage.db.sql("select '%s' - '%s' " % (mod_db[0][0], cstr(self.modified)))
...
def get_mapped_subcontracting_order(source_name, target_doc=None):
	...
	if target_doc.set_warehouse:
		...
		source_doc = dontmanage.get_doc("Purchase Order", source_name)
...
def is_subcontracting_order_created(po_name) -> bool:
	return (
		True
		if dontmanage.db.exists("Subcontracting Order", {"purchase_order": po_name, "docstatus": ["=", 1]})
		else False
	)
