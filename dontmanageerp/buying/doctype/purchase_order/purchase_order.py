...
from dontmanage.utils import cint, cstr, flt, get_link_to_form
...
from dontmanageerp.accounts.party import get_party_account, get_party_account_currency
from dontmanageerp.buying.utils import check_on_hold_or_closed_status, validate_for_items
...
from dontmanageerp.stock.doctype.item.item import get_item_defaults, get_last_purchase_details
...
from dontmanageerp.subcontracting.doctype.subcontracting_bom.subcontracting_bom import (
...
class PurchaseOrder(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.buying.doctype.purchase_order_item.purchase_order_item import PurchaseOrderItem
		from dontmanageerp.buying.doctype.purchase_order_item_supplied.purchase_order_item_supplied import (
	...
	def validate_with_previous_doc(self):
		...
		if cint(dontmanage.db.get_single_value("Buying Settings", "maintain_same_rate")):
	...
	def validate_supplier(self):
		prevent_po = dontmanage.db.get_value("Supplier", self.supplier, "prevent_pos")
		if prevent_po:
			standing = dontmanage.db.get_value("Supplier Scorecard", self.supplier, "status")
			if standing:
				dontmanage.throw(
		...
		warn_po = dontmanage.db.get_value("Supplier", self.supplier, "warn_pos")
		if warn_po:
			standing = dontmanage.db.get_value("Supplier Scorecard", self.supplier, "status")
			dontmanage.msgprint(
				_(
	...
	def validate_minimum_order_qty(self):
		...
		itemwise_min_order_qty = dontmanage._dict(
			dontmanage.db.sql(
			...
			if not self.is_old_subcontracting_flow:
				for item in self.items:
					if not item.fg_item:
						...
						if not dontmanage.get_value("Item", item.fg_item, "is_sub_contracted_item"):
							dontmanage.throw(
								_("Row #{0}: Finished Good Item {1} must be a sub-contracted item").format(
									item.idx, item.fg_item
								)
							)
						elif not dontmanage.get_value("Item", item.fg_item, "default_bom"):
							dontmanage.throw(
					...
					if not item.fg_item_qty:
						dontmanage.throw(_("Row #{0}: Finished Good Item Qty can not be zero").format(item.idx))
		else:
			for item in self.items:
	...
	def get_schedule_dates(self):
		for d in self.get("items"):
			if d.material_request_item and not d.schedule_date:
				d.schedule_date = dontmanage.db.get_value(
		...
		for d in self.get("items"):
			if d.item_code:
				...
				if last_purchase_details:
					...
					item_last_purchase_rate = dontmanage.get_cached_value(
	...
	def check_modified_date(self):
		mod_db = dontmanage.db.sql("select modified from `tabPurchase Order` where name = %s", self.name)
		date_diff = dontmanage.db.sql(f"select '{mod_db[0][0]}' - '{cstr(self.modified)}' ")
	...
	def update_delivered_qty_in_sales_order(self):
		...
		for so_name in sales_orders_to_update:
			so = dontmanage.get_doc("Sales Order", so_name)
	...
	def update_ordered_qty_in_so_for_removed_items(self, removed_items):
		...
		for item in removed_items:
			prev_ordered_qty = (
				dontmanage.get_cached_value("Sales Order Item", item.get("sales_order_item"), "ordered_qty")
				or 0.0
			)
	...
	def auto_create_subcontracting_order(self):
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			if dontmanage.db.get_single_value("Buying Settings", "auto_create_subcontracting_order"):
	...
	def update_subcontracting_order_status(self):
		...
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			sco = dontmanage.db.get_value("Subcontracting Order", {"purchase_order": self.name, "docstatus": 1})
...
def item_last_purchase_rate(name, conversion_rate, item_code, conversion_factor=1.0):
	...
	if last_purchase_details:
		...
		item_last_purchase_rate = dontmanage.get_cached_value("Item", item_code, "last_purchase_rate")
		if item_last_purchase_rate:
...
def close_or_unclose_purchase_orders(names, status):
	if not dontmanage.has_permission("Purchase Order", "write"):
		dontmanage.throw(_("Not permitted"), dontmanage.PermissionError)

	names = json.loads(names)
	for name in names:
		po = dontmanage.get_doc("Purchase Order", name)
		if po.docstatus == 1:
			if status == "Closed":
...
def make_purchase_invoice_from_portal(purchase_order_name):
	...
	if doc.contact_email != dontmanage.session.user:
		dontmanage.throw(_("Not Permitted"), dontmanage.PermissionError)
	doc.save()
	dontmanage.db.commit()
	dontmanage.response["type"] = "redirect"
	dontmanage.response.location = "/purchase-invoices/" + doc.name
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	def update_item(obj, target, source_parent):
		...
		target.cost_center = (
			obj.cost_center
			or dontmanage.db.get_value("Project", obj.project, "cost_center")
...
def make_subcontracting_order(source_name, target_doc=None, save=False, submit=False, notify=False):
	if not is_po_fully_subcontracted(source_name):
		...
		if (save or submit) and dontmanage.has_permission(target_doc.doctype, "create"):
			target_doc.save()

			if submit and dontmanage.has_permission(target_doc.doctype, "submit", target_doc):
				try:
					target_doc.submit()
		...
		dontmanage.throw(_("This PO has been fully subcontracted."))


def is_po_fully_subcontracted(po_name):
	table = dontmanage.qb.DocType("Purchase Order Item")
	query = (
