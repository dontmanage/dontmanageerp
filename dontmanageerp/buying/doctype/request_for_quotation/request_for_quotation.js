...
dontmanage.ui.form.on("Request for Quotation",{
	setup: function(frm) {
	...
	refresh: function(frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(__("Send Emails to Suppliers"), function() {
				dontmanage.call({
					method: 'dontmanageerp.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails',
					freeze: true,
					args: {
						...
						(data) => {
							var w = window.open(
								dontmanage.urllib.get_full_url(
									"/api/method/dontmanageerp.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
									new URLSearchParams({
										name: frm.doc.name,
										supplier: data.supplier,
										print_format: data.print_format || "Standard",
										language: data.language || dontmanage.boot.lang,
		...
		var dialog = new dontmanage.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return dontmanage.call({
					type: "GET",
					method: "dontmanageerp.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
dontmanage.ui.form.on("Request for Quotation Item", {
	items_add(frm, cdt, cdn) {
		if (frm.doc.schedule_date) {
			dontmanage.model.set_value(cdt, cdn, 'schedule_date', frm.doc.schedule_date);
		}
	}
});
dontmanage.ui.form.on("Request for Quotation Supplier",{
	supplier: function(frm, cdt, cdn) {
		...
		dontmanage.call({
			...
			callback: function(r){
				if(r.message){
					dontmanage.model.set_value(cdt, cdn, 'contact', r.message.contact_person)
					dontmanage.model.set_value(cdt, cdn, 'email_id', r.message.contact_email)
				}
			}
		})
	},
...
dontmanageerp.buying.RequestforQuotationController = class RequestforQuotationController extends dontmanageerp.buying.BuyingController {
	refresh() {
		var me = this;
		super.refresh();
		if (this.frm.doc.docstatus===0) {
			...
			this.frm.add_custom_button(__('Possible Supplier'), function() {
				...
				var dialog = new dontmanage.ui.Dialog({
					...
					primary_action: (args) => {
						...
						dontmanageerp.utils.map_current_doc({
							method: "dontmanageerp.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(__('Link to Material Requests'),
				function() {
					dontmanageerp.buying.link_to_mrs(me.frm);
				}, __("Tools"));
		...
		var dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				{
					...
					onchange() {
						if(dialog.get_value('search_type') == 'Tag'){
							dontmanage.call({
								method: 'dontmanageerp.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag',
							}).then(r => {
			...
			primary_action : (args) => {
				...
				if (args.search_type === "Tag" && args.tag) {
					return dontmanage.call({
						type: "GET",
						method: "dontmanage.desk.doctype.tag.tag.get_tagged_docs",
						args: {
							"doctype": "Supplier",
							"tag": args.tag
						},
...
extend_cscript(cur_frm.cscript, new dontmanageerp.buying.RequestforQuotationController({frm: cur_frm}));
