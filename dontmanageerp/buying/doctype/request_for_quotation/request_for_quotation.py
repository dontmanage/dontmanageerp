...
from dontmanage.utils.print_format import download_pdf
from dontmanage.utils.user import get_user_fullname

from dontmanageerp.accounts.party import get_party_account_currency, get_party_details
from dontmanageerp.buying.utils import validate_for_items
from dontmanageerp.controllers.buying_controller import BuyingController
from dontmanageerp.stock.doctype.material_request.material_request import set_missing_values
...
class RequestforQuotation(BuyingController):
	...
	def validate_duplicate_supplier(self):
		...
		if len(supplier_list) != len(set(supplier_list)):
			dontmanage.throw(_("Same supplier has been entered multiple times"))

	def validate_supplier_list(self):
		for d in self.suppliers:
			prevent_rfqs = dontmanage.db.get_value("Supplier", d.supplier, "prevent_rfqs")
			if prevent_rfqs:
				standing = dontmanage.db.get_value("Supplier Scorecard", d.supplier, "status")
				dontmanage.throw(
					_("RFQs are not allowed for {0} due to a scorecard standing of {1}").format(
						d.supplier, standing
					)
				)
			warn_rfqs = dontmanage.db.get_value("Supplier", d.supplier, "warn_rfqs")
			if warn_rfqs:
				standing = dontmanage.db.get_value("Supplier Scorecard", d.supplier, "status")
				dontmanage.msgprint(
					_(
	...
	def update_email_id(self):
		for rfq_supplier in self.suppliers:
			if not rfq_supplier.email_id:
				rfq_supplier.email_id = dontmanage.db.get_value("Contact", rfq_supplier.contact, "email_id")

	def validate_email_id(self, args):
		if not args.email_id:
			dontmanage.throw(
				_("Row {0}: For Supplier {1}, Email Address is Required to send an email").format(
					args.idx, dontmanage.bold(args.supplier)
				)
			)
	...
	def update_supplier_part_no(self, supplier):
		...
		for item in self.items:
			item.supplier_part_no = dontmanage.db.get_value(
	...
	def update_supplier_contact(self, rfq_supplier, link):
		...
		if dontmanage.db.exists("User", rfq_supplier.email_id):
			user = dontmanage.get_doc("User", rfq_supplier.email_id)
		else:
	...
	def link_supplier_contact(self, rfq_supplier, user):
		...
		if rfq_supplier.contact:
			contact = dontmanage.get_doc("Contact", rfq_supplier.contact)
		else:
	...
	def supplier_rfq_mail(self, data, update_password_link, rfq_link, preview=False):
		full_name = get_user_fullname(dontmanage.session["user"])
		if full_name == "Guest":
	...
	def send_email(self, data, sender, subject, message, attachments):
		...
		dontmanage.msgprint(_("Email Sent to Supplier {0}").format(data.supplier))
...
def send_supplier_emails(rfq_name):
	...
	rfq = dontmanage.get_doc("Request for Quotation", rfq_name)
	if rfq.docstatus == 1:
		rfq.send_to_supplier()


def check_portal_enabled(reference_doctype):
	if not dontmanage.db.get_value("Portal Menu Item", {"reference_doctype": reference_doctype}, "enabled"):
		dontmanage.throw(
			_(
...
def make_supplier_quotation_from_rfq(source_name, target_doc=None, for_supplier=None):
	def postprocess(source, target_doc):
		if for_supplier:
			...
			target_doc.buying_price_list = args.buying_price_list or dontmanage.db.get_value(
...
def create_supplier_quotation(doc):
	...
	try:
		sq_doc = dontmanage.get_doc(
			{
				...
				or dontmanage.db.get_value("Buying Settings", None, "buying_price_list"),
			}
		)
		...
		sq_doc.save()
		dontmanage.msgprint(_("Supplier Quotation {0} Created").format(sq_doc.name))
		return sq_doc.name
	except Exception:
		return None
	...
	args.update(
		{
			...
			"supplier_part_no": dontmanage.db.get_value(
...
def get_pdf(
	...
	doc = dontmanage.get_doc("Request for Quotation", name)
	if supplier:
...
def get_item_from_material_requests_based_on_supplier(source_name, target_doc=None):
	mr_items_list = dontmanage.db.sql(
		"""
		SELECT
...
def get_supplier_tag():
	...
	tags = list(set(tag.tag for tag in dontmanage.get_all("Tag Link", filters=filters, fields=["tag"]) if tag))
