...
from dontmanage.utils import add_days, add_years, get_last_day, getdate, nowdate

from dontmanageerp.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import (
...
class SupplierScorecard(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.buying.doctype.supplier_scorecard_scoring_criteria.supplier_scorecard_scoring_criteria import (
			SupplierScorecardScoringCriteria,
		)
		from dontmanageerp.buying.doctype.supplier_scorecard_scoring_standing.supplier_scorecard_scoring_standing import (
	...
	def calculate_total_score(self):
		...
		for scp in scorecards:
			my_sc = dontmanage.get_doc("Supplier Scorecard Period", scp.name)
		...
		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and (
				...
				for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
					...
					dontmanage.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))


@dontmanage.whitelist()
def get_timeline_data(doctype, name):
	# Get a list of all the associated scorecards
	scs = dontmanage.get_doc(doctype, name)
	out = {}
	timeline_data = {}
	scorecards = dontmanage.db.sql(
		"""
	...
	for sc in scorecards:
		start_date, end_date, total_score = dontmanage.db.get_value(
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			dontmanage.get_doc("Supplier Scorecard", sc.name).save()


@dontmanage.whitelist()
def make_all_scorecards(docname):

	sc = dontmanage.get_doc("Supplier Scorecard", docname)
	supplier = dontmanage.get_doc("Supplier", sc.supplier)
