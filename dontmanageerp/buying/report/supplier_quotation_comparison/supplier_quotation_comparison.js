...
dontmanage.query_reports["Supplier Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = dontmanage.query_report.get_filter_value("supplier_quotation");
				if (quote != "") {
					return {
						query: "dontmanageerp.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
		...
		},
		{
			...
			get_data: function (txt) {
				return dontmanage.db.get_link_options("Supplier", txt);
			},
		},
		{
			fieldtype: "MultiSelectList",
			...
			default: "",
			get_data: function (txt) {
				return dontmanage.db.get_link_options("Supplier Quotation", txt, { docstatus: ["<", 2] });
			},
		},
		{
			fieldtype: "Link",
	...
	formatter: (value, row, column, data, default_formatter) => {
		...
		if (column.fieldname === "valid_till" && data.valid_till) {
			if (dontmanage.datetime.get_diff(data.valid_till, dontmanage.datetime.nowdate()) <= 1) {
				value = `<div style="color:red">${value}</div>`;
			} else if (dontmanage.datetime.get_diff(data.valid_till, dontmanage.datetime.nowdate()) <= 7) {
	...
	onload: (report) => {
		...
		report.page.add_inner_button(
			...
			() => {
				let reporter = dontmanage.query_reports["Supplier Quotation Comparison"];
	...
	make_default_supplier_dialog: (report) => {
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if (values) {
				...
				dontmanage.call({
					method: "dontmanageerp.buying.report.supplier_quotation_comparison.supplier_quotation_comparison.set_default_supplier",
					args: {
					...
					freeze: true,
					callback: (r) => {
						dontmanage.msgprint(__("Successfully Set Supplier"));
						dialog.hide();
					},
				});
			}
