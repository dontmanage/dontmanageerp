...
def get_data(filters):
	sq = dontmanage.qb.DocType("Supplier Quotation")
	sq_item = dontmanage.qb.DocType("Supplier Quotation Item")

	query = (
		dontmanage.qb.from_(sq_item)
		.from_(sq)
		.select(
			sq_item.parent,
			sq_item.item_code,
...
def prepare_data(supplier_quotation_data, filters):
	...
	company_currency = dontmanage.db.get_default("currency")
	...
	for data in supplier_quotation_data:
		...
		supplier_currency = dontmanage.db.get_value("Supplier", data.get("supplier_name"), "default_currency")
...
def prepare_chart_data(suppliers, qty_list, supplier_qty_price_map):
	...
	dataset = []
	currency_symbol = dontmanage.db.get_value("Currency", dontmanage.db.get_default("currency"), "symbol")
	for qty in qty_list:
		datapoints = {
...
def get_columns(filters):
	currency = dontmanage.get_cached_value("Company", filters.get("company"), "default_currency")
