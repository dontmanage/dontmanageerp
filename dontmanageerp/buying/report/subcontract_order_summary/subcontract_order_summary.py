...
def get_subcontracted_orders(report_filters):
	...
	return dontmanage.get_all(report_filters.order_type, fields=fields, filters=filters) or []
...
def get_supplied_items(orders, report_filters):
	...
	for row in dontmanage.get_all(supplied_items_table, fields=fields, filters=filters):
...
def prepare_subcontracted_data(orders, supplied_items):
	...
	for row in orders:
		...
		if key not in order_details:
			order_details.setdefault(key, dontmanage._dict({"order_item": row, "supplied_items": []}))
