...
from dontmanageerp.stock.doctype.item.item import get_last_purchase_details, validate_end_of_life
	...
	for d in doc.get("items"):
		...
		if last_purchase_details and (
			...
			# Conversion factor should not be mandatory for non itemized items
				...
				dontmanage.throw(_("UOM Conversion factor is required in row {0}").format(d.idx))

		# update last purchsae rate
		dontmanage.db.set_value("Item", d.item_code, "last_purchase_rate", flt(last_purchase_rate))
...
def validate_for_items(doc) -> None:
	...
	for d in doc.get("items"):
		if not d.qty:
			if doc.doctype == "Purchase Receipt" and d.rejected_qty:
				continue
			dontmanage.throw(_("Please enter quantity for Item {0}").format(d.item_code))
	...
	if (
		...
		and not cint(dontmanage.db.get_single_value("Buying Settings", "allow_multiple_items") or 0)
	):
		dontmanage.throw(_("Same item cannot be entered multiple times."))
...
def validate_item_and_get_basic_data(row) -> dict:
	...
	if not item:
		dontmanage.throw(_("Row #{0}: Item {1} does not exist").format(row.idx, dontmanage.bold(row.item_code)))
	...
	if item.is_stock_item == 1 and row.qty and not row.warehouse and not row.get("delivered_by_supplier"):
		dontmanage.throw(
			_("Row #{1}: Warehouse is mandatory for stock Item {0}").format(
				dontmanage.bold(row.item_code), row.idx
			)
		)


def check_on_hold_or_closed_status(doctype, docname) -> None:
	status = dontmanage.db.get_value(doctype, docname, "status")

	if status in ("Closed", "On Hold"):
		dontmanage.throw(_("{0} {1} status is {2}").format(doctype, docname, status), dontmanage.InvalidStatusError)
...
def get_linked_material_requests(items):
	...
	mr_list = []
	for item in items:
		material_request = dontmanage.db.sql(
