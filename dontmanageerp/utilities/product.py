...
import dontmanage
from dontmanage.utils import cint, flt, fmt_money, getdate, nowdate

from dontmanageerp.accounts.doctype.pricing_rule.pricing_rule import get_pricing_rule_for_item
from dontmanageerp.stock.doctype.batch.batch import get_batch_qty


def get_web_item_qty_in_stock(item_code, item_warehouse_field, warehouse=None):
	in_stock, stock_qty = 0, ""
	template_item_code, is_stock_item = dontmanage.db.get_value(
		"Item", item_code, ["variant_of", "is_stock_item"]
	)

	if not warehouse:
		warehouse = dontmanage.db.get_value("Website Item", {"item_code": item_code}, item_warehouse_field)

	if not warehouse and template_item_code and template_item_code != item_code:
		warehouse = dontmanage.db.get_value(
			"Website Item", {"item_code": template_item_code}, item_warehouse_field
		)

	if warehouse:
		stock_qty = dontmanage.db.sql(
			"""
...
def adjust_qty_for_expired_items(item_code, stock_qty, warehouse):
	batches = dontmanage.get_all("Batch", filters=[{"item": item_code}], fields=["expiry_date", "name"])
...
def get_price(item_code, price_list, customer_group, company, qty=1):
	from dontmanageerp.e_commerce.shopping_cart.cart import get_party

	template_item_code = dontmanage.db.get_value("Item", item_code, "variant_of")

	if price_list:
		price = dontmanage.get_all(
			"Item Price",
		...
		if price:
			party = get_party()
			pricing_rule_dict = dontmanage._dict(
				{
					"item_code": item_code,
					"qty": qty,
					"stock_qty": qty,
					...
					"currency": dontmanage.db.get_value("Price List", price_list, "currency"),
					"doctype": "Quotation",
				}
			)
			...
			if price_obj:
				...
				price_obj["currency_symbol"] = (
					not cint(dontmanage.db.get_default("hide_currency_symbol"))
					and (
						dontmanage.db.get_value("Currency", price_obj.currency, "symbol", cache=True)
						or price_obj.currency
					)
					or ""
				)

				uom_conversion_factor = dontmanage.db.sql(
...
def get_non_stock_item_status(item_code, item_warehouse_field):
	...
	if dontmanage.db.exists("Product Bundle", item_code):
		items = dontmanage.get_doc("Product Bundle", item_code).get_all_children()
		bundle_warehouse = dontmanage.db.get_value(
