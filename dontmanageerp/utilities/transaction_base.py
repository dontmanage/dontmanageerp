...
from dontmanage.utils import cint, flt, get_time, now_datetime

from dontmanageerp.controllers.status_updater import StatusUpdater


class UOMMustBeIntegerError(dontmanage.ValidationError):
	pass
...
class TransactionBase(StatusUpdater):
	def validate_posting_time(self):
		...
		if dontmanage.flags.in_import and self.posting_date:
		...
		if not getattr(self, "set_posting_time", None):
			...
			try:
				...
				dontmanage.throw(_("Invalid Posting Time"))
				...
				if ref_dn:
					if is_child:
						...
						if ref_dn not in item_ref_dn:
							...
							dontmanage.throw(_("Duplicate row {0} with same {1}").format(d.idx, key))
					elif ref_dn:
		...
		for reference_doctype, ref_dn_list in ref_doc.items():
			...
			for reference_name in ref_dn_list:
				...
				if not prevdoc_values:
					dontmanage.throw(_("Invalid reference {0} {1}").format(reference_doctype, reference_name))
	...
	def validate_rate_with_reference_doc(self, ref_details):
		...
		if self.doctype in buying_doctypes:
			action, role_allowed_to_override = dontmanage.get_cached_value(
			...
			for d in self.get("items"):
				if d.get(ref_link_field):
					...
					if abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
						if action == "Stop":
							if role_allowed_to_override not in dontmanage.get_roles():
								stop_actions.append(
		...
		if stop_actions:
			dontmanage.throw(stop_actions, as_list=True)
	...
	def validate_currency_for_receivable_payable_and_advance_account(self):
		if self.doctype in ["Customer", "Supplier"]:
			...
			for x in self.accounts:
				company_default_currency = dontmanage.get_cached_value("Company", x.company, "default_currency")
				receivable_payable_account_currency = None
				advance_account_currency = None

				if x.account:
					receivable_payable_account_currency = dontmanage.get_cached_value(
				...
				if receivable_payable_account_currency and (
					...
					dontmanage.throw(
						_(
							...
							dontmanage.bold(receivable_payable_account_currency),
							dontmanage.bold(self.default_currency),
							dontmanage.bold(company_default_currency),
						)
					)
...
def delete_events(ref_type, ref_name):
	...
	if events:
		dontmanage.delete_doc("Event", events, for_reload=True)
					...
					if abs(cint(qty) - flt(qty, precision)) > 0.0000001:
						dontmanage.throw(
							_(
								...
								dontmanage.bold(d.get(uom_field)),
							),
