...
class Video(Document):
	...
	def set_youtube_statistics(self):
		api_key = dontmanage.db.get_single_value("Video Settings", "api_key")
...
def is_tracking_enabled():
	return dontmanage.db.get_single_value("Video Settings", "enable_youtube_tracking")
...
def update_youtube_data():
	...
	enable_youtube_tracking, frequency = dontmanage.db.get_value(
	...
	time = datetime.now()
	timezone = pytz.timezone(dontmanage.utils.get_time_zone())
...
def get_id_from_url(url):
	...
	if not isinstance(url, str):
		dontmanage.throw(_("URL can only be a string"), title=_("Invalid URL"))
...
def batch_update_youtube_data():
	def get_youtube_statistics(video_ids):
		...
		try:
			...
			dontmanage.log_error("Unable to update YouTube statistics")
	...
	def set_youtube_data(entries):
		for entry in entries:
			...
			dontmanage.db.set_value("Video", video_id, stats)

	video_list = dontmanage.get_all("Video", fields=["youtube_video_id"])
