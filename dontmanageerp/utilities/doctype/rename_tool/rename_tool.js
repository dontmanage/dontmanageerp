...
dontmanage.ui.form.on("Rename Tool", {
	onload: function(frm) {
		return dontmanage.call({
			method: "dontmanageerp.utilities.doctype.rename_tool.rename_tool.get_doctypes",
			callback: function(r) {
	...
	refresh: function(frm) {
		...
		frm.page.set_primary_action(__("Rename"), function() {
			...
			dontmanage.call({
				method: "dontmanageerp.utilities.doctype.rename_tool.rename_tool.upload",
				args: {
				...
				callback: function(r) {
					...
					if (r.exc) {
						r.exc = dontmanage.utils.parse_json(r.exc);
