...
def get_level():
	...
	for doctype, min_count in doctypes.items():
		count = dontmanage.db.count(doctype)
		if count > min_count:
			activation_level += 1
	...
	communication_number = dontmanage.db.count("Communication", dict(communication_medium="Email"))
	if communication_number > 10:
		activation_level += 1
	sales_data.append({"Communication": communication_number})

	# recent login
	if dontmanage.db.sql("select name from tabUser where last_login > date_sub(now(), interval 2 day) limit 1"):
		activation_level += 1
...
def get_help_messages():
	...
	domain = dontmanage.get_cached_value("Company", dontmanageerp.get_default_company(), "domain")
	messages = []
	...
	for m in message_settings:
		if not m.domain or domain in m.domain:
			m.count = dontmanage.db.count(m.doctype)
			if m.count < m.target:
				messages.append(m)
