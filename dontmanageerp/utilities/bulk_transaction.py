...
from dontmanage.utils import get_link_to_form, today
...
def transaction_processing(data, from_doctype, to_doctype):
	...
	dontmanage.msgprint(_("Started a background job to create {1} {0}").format(to_doctype, length_of_data))
	dontmanage.enqueue(
		job,
...
def retry(date: str | None = None):
	...
	if date:
		failed_docs = dontmanage.db.get_all(
		...
		if not failed_docs:
			dontmanage.msgprint(_("There are no Failed transactions"))
		else:
			job = dontmanage.enqueue(
...
def retry_failed_transactions(failed_docs: list | None):
	if failed_docs:
		for log in failed_docs:
			try:
				dontmanage.db.savepoint("before_creation_state")
				task(log.transaction_name, log.from_doctype, log.to_doctype)
			except Exception:
				dontmanage.db.rollback(save_point="before_creation_state")
				update_log(log.name, "Failed", 1, str(dontmanage.get_traceback(with_context=True)))
			else:
				update_log(log.name, "Success", 1)


def update_log(log_name, status, retried, err=None):
	dontmanage.db.set_value("Bulk Transaction Log Detail", log_name, "transaction_status", status)
	dontmanage.db.set_value("Bulk Transaction Log Detail", log_name, "retried", retried)
	if err:
		dontmanage.db.set_value("Bulk Transaction Log Detail", log_name, "error_description", err)
...
def job(deserialized_data, from_doctype, to_doctype):
	...
	for d in deserialized_data:
		try:
			...
			create_log(
				doc_name,
				str(dontmanage.get_traceback(with_context=True)),
				from_doctype,
				to_doctype,
				status="Failed",
...
def task(doc_name, from_doctype, to_doctype):
	from dontmanageerp.accounts.doctype.payment_entry import payment_entry
	from dontmanageerp.accounts.doctype.purchase_invoice import purchase_invoice
	from dontmanageerp.accounts.doctype.sales_invoice import sales_invoice
	from dontmanageerp.buying.doctype.purchase_order import purchase_order
	from dontmanageerp.buying.doctype.supplier_quotation import supplier_quotation
	from dontmanageerp.selling.doctype.quotation import quotation
	from dontmanageerp.selling.doctype.sales_order import sales_order
	from dontmanageerp.stock.doctype.delivery_note import delivery_note
	from dontmanageerp.stock.doctype.purchase_receipt import purchase_receipt
	...
	hooks = dontmanage.get_hooks("bulk_transaction_task_mapper")
	for hook in hooks:
		mapper.update(dontmanage.get_attr(hook)())

	dontmanage.flags.bulk_transaction = True
	...
	del dontmanage.flags.bulk_transaction


def create_log(doc_name, e, from_doctype, to_doctype, status, log_date=None, restarted=0):
	transaction_log = dontmanage.new_doc("Bulk Transaction Log Detail")
