...
class ProcessLoanInterestAccrual(Document):
	def on_submit(self):
		...
		if self.loan:
			loan_doc = dontmanage.get_doc("Loan", self.loan)
			if loan_doc:
...
def process_loan_interest_accrual_for_demand_loans(
	...
	loan_process = dontmanage.new_doc("Process Loan Interest Accrual")
...
def term_loan_accrual_pending(date, loan=None):
	...
	pending_accrual = dontmanage.db.get_value("Repayment Schedule", filters)
