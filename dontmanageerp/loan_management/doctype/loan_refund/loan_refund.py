...
class LoanRefund(AccountsController):
	...
	def validate_refund_amount(self):
		loan = dontmanage.get_doc("Loan", self.loan)
		pending_amount = get_pending_principal_amount(loan)
		if pending_amount >= 0:
			dontmanage.throw(_("No excess amount to refund."))
		else:
			excess_amount = pending_amount * -1

		if self.refund_amount > excess_amount:
			dontmanage.throw(_("Refund amount cannot be greater than excess amount {0}").format(excess_amount))
	...
	def update_outstanding_amount(self, cancel=0):
		refund_amount = dontmanage.db.get_value("Loan", self.loan, "refund_amount")
		...
		dontmanage.db.set_value("Loan", self.loan, "refund_amount", refund_amount)
