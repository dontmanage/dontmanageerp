...
from dontmanage.utils import add_days, cint, date_diff, flt, get_datetime, getdate
...
class LoanRepayment(AccountsController):
	...
	def set_missing_values(self, amounts):
		precision = cint(dontmanage.db.get_default("currency_precision")) or 2

		if not self.posting_date:
			self.posting_date = get_datetime()

		if not self.cost_center:
			self.cost_center = dontmanageerp.get_default_cost_center(self.company)
	...
	def check_future_entries(self):
		future_repayment_date = dontmanage.db.get_value(
			"Loan Repayment",
		...
		if future_repayment_date:
			dontmanage.throw("Repayment already made till date {0}".format(get_datetime(future_repayment_date)))

	def validate_amount(self):
		precision = cint(dontmanage.db.get_default("currency_precision")) or 2

		if not self.amount_paid:
			dontmanage.throw(_("Amount paid cannot be zero"))
	...
	def mark_as_unpaid(self):
		...
		for payment in self.repayment_details:
			...
			if payment.accrual_type == "Repayment" and payment.idx == no_of_repayments:
				lia_doc = dontmanage.get_doc("Loan Interest Accrual", payment.loan_interest_accrual)
				lia_doc.cancel()
	...
	def check_future_accruals(self):
		future_accrual_date = dontmanage.db.get_value(
			"Loan Interest Accrual",
...
def get_accrued_interest_entries(against_loan, posting_date=None):
	...
	unpaid_accrued_entries = dontmanage.db.sql(
		"""
...
def get_penalty_details(against_loan):
	penalty_details = dontmanage.db.sql(
		"""
...
def get_amounts(amounts, against_loan, posting_date):
	...
	against_loan_doc = dontmanage.get_doc("Loan", against_loan)
	loan_type_details = dontmanage.get_doc("Loan Type", against_loan_doc.loan_type)
