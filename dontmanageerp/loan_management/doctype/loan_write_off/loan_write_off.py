...
class LoanWriteOff(AccountsController):
	...
	def validate_write_off_amount(self):
		...
		total_payment, principal_paid, interest_payable, written_off_amount = dontmanage.get_value(
			"Loan",
			self.loan,
		...
		if self.write_off_amount > pending_principal_amount:
			dontmanage.throw(_("Write off amount cannot be greater than pending principal amount"))
	...
	def update_outstanding_amount(self, cancel=0):
		written_off_amount = dontmanage.db.get_value("Loan", self.loan, "written_off_amount")
		...
		dontmanage.db.set_value("Loan", self.loan, "written_off_amount", written_off_amount)

	def make_gl_entries(self, cancel=0):
		gl_entries = []
		loan_details = dontmanage.get_doc("Loan", self.loan)
