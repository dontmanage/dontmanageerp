...
from dontmanage.utils import add_days, flt, get_datetime, nowdate
...
class LoanDisbursement(AccountsController):
	...
	def validate_disbursal_amount(self):
		...
		if self.disbursed_amount > possible_disbursal_amount:
			dontmanage.throw(_("Disbursed Amount cannot be greater than {0}").format(possible_disbursal_amount))
...
def get_total_pledged_security_value(loan):
	...
	hair_cut_map = dontmanage._dict(
		dontmanage.get_all("Loan Security", fields=["name", "haircut"], as_list=1)
	)
...
def get_disbursal_amount(loan, on_current_security_price=0):
	...
	if loan_details.is_secured_loan and dontmanage.get_all(
...
def get_maximum_amount_as_per_pledged_security(loan):
	return flt(dontmanage.db.get_value("Loan Security Pledge", {"loan": loan}, "sum(maximum_loan_value)"))
