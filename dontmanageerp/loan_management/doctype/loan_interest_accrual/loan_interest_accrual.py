...
from dontmanage.utils import add_days, cint, date_diff, flt, get_datetime, getdate, nowdate
...
class LoanInterestAccrual(AccountsController):
	def validate(self):
		if not self.loan:
			dontmanage.throw(_("Loan is mandatory"))

		if not self.posting_date:
			self.posting_date = nowdate()

		if not self.interest_amount and not self.payable_principal_amount:
			dontmanage.throw(_("Interest Amount or Principal Amount is mandatory"))
	...
	def update_is_accrued(self):
		dontmanage.db.set_value("Repayment Schedule", self.repayment_schedule_name, "is_accrued", 0)

	def make_gl_entries(self, cancel=0, adv_adj=0):
		gle_map = []

		cost_center = dontmanage.db.get_value("Loan", self.loan, "cost_center")
...
def get_term_loans(date, term_loan=None, loan_type=None):
	...
	if term_loan:
		condition += " AND l.name = %s" % dontmanage.db.escape(term_loan)

	if loan_type:
		condition += " AND l.loan_type = %s" % dontmanage.db.escape(loan_type)

	term_loans = dontmanage.db.sql(
...
def make_loan_interest_accrual_entry(args):
	...
	loan_interest_accrual = dontmanage.new_doc("Loan Interest Accrual")
...
def get_last_accrual_date(loan):
	last_posting_date = dontmanage.db.sql(
	...
	if last_posting_date[0][0]:
		...
		return dontmanage.db.get_value("Loan", loan, "disbursement_date")
