...
import dontmanage
from dontmanage.utils import add_to_date, date_diff, flt, get_datetime, get_first_day, nowdate
...
class TestLoanInterestAccrual(unittest.TestCase):
	def setUp(self):
		...
		self.applicant = dontmanage.db.get_value("Customer", {"name": "_Test Loan Customer"}, "name")
	...
	def test_loan_interest_accural(self):
		...
		loan_interest_accural = dontmanage.get_doc("Loan Interest Accrual", {"loan": loan.name})
	...
	def test_accumulated_amounts(self):
		...
		make_loan_disbursement_entry(loan.name, loan.loan_amount, disbursement_date=first_date)
		process_loan_interest_accrual_for_demand_loans(posting_date=last_date)
		loan_interest_accrual = dontmanage.get_doc("Loan Interest Accrual", {"loan": loan.name})
		...
		loan_interest_accrual = dontmanage.get_doc(
