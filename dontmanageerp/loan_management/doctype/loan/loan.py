...
from dontmanageerp.accounts.doctype.journal_entry.journal_entry import get_payment_entry
...
class Loan(AccountsController):
	...
	def validate_cost_center(self):
		if not self.cost_center and self.rate_of_interest != 0.0:
			self.cost_center = dontmanage.db.get_value("Company", self.company, "cost_center")

			if not self.cost_center:
				dontmanage.throw(_("Cost center is mandatory for loans having rate of interest greater than 0"))
	...
	def set_missing_fields(self):
		if not self.company:
			self.company = dontmanageerp.get_default_company()

		if not self.posting_date:
			self.posting_date = nowdate()

		if self.loan_type and not self.rate_of_interest:
			self.rate_of_interest = dontmanage.db.get_value("Loan Type", self.loan_type, "rate_of_interest")
		...
		if sanctioned_amount_limit and flt(self.loan_amount) + flt(total_loan_amount) > flt(
			...
			dontmanage.throw(
				_("Sanctioned Amount limit crossed for {0} {1}").format(
					self.applicant_type, dontmanage.bold(self.applicant)
				)
			)

	def make_repayment_schedule(self):
		if not self.repayment_start_date:
			dontmanage.throw(_("Repayment Start Date is mandatory for term loans"))

		schedule_type_details = dontmanage.db.get_value(
	...
	def set_loan_amount(self):
		if self.loan_application and not self.loan_amount:
			self.loan_amount = dontmanage.db.get_value("Loan Application", self.loan_application, "loan_amount")
	...
	def validate_loan_amount(self):
		...
		if not self.loan_amount:
			dontmanage.throw(_("Loan amount is mandatory"))

	def link_loan_security_pledge(self):
		if self.is_secured_loan and self.loan_application:
			maximum_loan_value = dontmanage.db.get_value(
	...
	def unlink_loan_security_pledge(self):
		pledges = dontmanage.get_all("Loan Security Pledge", fields=["name"], filters={"loan": self.name})
...
def update_total_amount_paid(doc):
	...
	dontmanage.db.set_value("Loan", doc.name, "total_amount_paid", total_amount_paid)


def get_total_loan_amount(applicant_type, applicant, company):
	pending_amount = 0
	loan_details = dontmanage.db.get_all(
		"Loan",
		filters={
...
def validate_repayment_method(
	...
	if is_term_loan and not repayment_method:
		dontmanage.throw(_("Repayment Method is mandatory for term loans"))

	if repayment_method == "Repay Over Number of Periods" and not repayment_periods:
		dontmanage.throw(_("Please enter Repayment Periods"))

	if repayment_method == "Repay Fixed Amount per Period":
		if not monthly_repayment_amount:
			dontmanage.throw(_("Please enter repayment Amount"))
		if monthly_repayment_amount > loan_amount:
			dontmanage.throw(_("Monthly Repayment Amount cannot be greater than Loan Amount"))
...
def request_loan_closure(loan, posting_date=None):
	...
	loan_type = dontmanage.get_value("Loan", loan, "loan_type")
	write_off_limit = dontmanage.get_value("Loan Type", loan_type, "write_off_amount")
	...
	if pending_amount and abs(pending_amount) < write_off_limit:
		...
		dontmanage.throw(_("Cannot close loan as there is an outstanding of {0}").format(pending_amount))

	dontmanage.db.set_value("Loan", loan, "status", "Loan Closure Requested")


@dontmanage.whitelist()
def get_loan_application(loan_application):
	loan = dontmanage.get_doc("Loan Application", loan_application)
	if loan:
		return loan.as_dict()
...
def close_unsecured_term_loan(loan):
	loan_details = dontmanage.db.get_value(
	...
	if (
		...
		dontmanage.db.set_value("Loan", loan, "status", "Closed")
	else:
		dontmanage.throw(_("Cannot close this loan until full repayment"))


def close_loan(loan, total_amount_paid):
	dontmanage.db.set_value("Loan", loan, "total_amount_paid", total_amount_paid)
	dontmanage.db.set_value("Loan", loan, "status", "Closed")


@dontmanage.whitelist()
def make_loan_disbursement(loan, company, applicant_type, applicant, pending_amount=0, as_dict=0):
	disbursement_entry = dontmanage.new_doc("Loan Disbursement")
...
def make_repayment_entry(loan, applicant_type, applicant, loan_type, company, as_dict=0):
	repayment_entry = dontmanage.new_doc("Loan Repayment")
...
def make_loan_write_off(loan, company=None, posting_date=None, amount=0, as_dict=0):
	if not company:
		company = dontmanage.get_value("Loan", loan, "company")
	...
	if amount and (amount > pending_amount):
		dontmanage.throw(_("Write Off amount cannot be greater than pending loan amount"))

	if not amount:
		amount = pending_amount

	# get default write off account from company master
	write_off_account = dontmanage.get_value("Company", company, "write_off_account")

	write_off = dontmanage.new_doc("Loan Write Off")
	write_off.loan = loan
...
def unpledge_security(
	...
	if loan:
		...
		loan_doc = dontmanage.get_doc("Loan", loan)
	...
	# will unpledge qty based on loan security pledge
		...
		security_map = {}
		pledge_doc = dontmanage.get_doc("Loan Security Pledge", loan_security_pledge)
	...
	if approve:
		if unpledge_request.docstatus == 1:
			...
			dontmanage.throw(_("Only submittted unpledge requests can be approved"))
...
def create_loan_security_unpledge(unpledge_map, loan, company, applicant_type, applicant):
	unpledge_request = dontmanage.new_doc("Loan Security Unpledge")
...
def get_shortfall_applicants():
	loans = dontmanage.get_all("Loan Security Shortfall", {"status": "Pending"}, pluck="loan")
	applicants = set(dontmanage.get_all("Loan", {"name": ("in", loans)}, pluck="name"))
...
def make_refund_jv(loan, amount=0, reference_number=None, reference_date=None, submit=0):
	...
	if not amount:
		...
		if amount < 0:
			dontmanage.throw(_("No excess amount pending for refund"))
