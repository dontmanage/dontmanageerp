...
from dontmanageerp.loan_management.doctype.loan.loan import (
	make_loan_write_off,
	request_loan_closure,
	unpledge_security,
)
from dontmanageerp.loan_management.doctype.loan_application.loan_application import create_pledge
from dontmanageerp.loan_management.doctype.loan_disbursement.loan_disbursement import (
	get_disbursal_amount,
)
from dontmanageerp.loan_management.doctype.loan_interest_accrual.loan_interest_accrual import (
	days_in_year,
)
from dontmanageerp.loan_management.doctype.loan_repayment.loan_repayment import calculate_amounts
from dontmanageerp.loan_management.doctype.loan_security_unpledge.loan_security_unpledge import (
...
from dontmanageerp.loan_management.doctype.process_loan_security_shortfall.process_loan_security_shortfall import (
...
class TestLoan(unittest.TestCase):
	def setUp(self):
		...
		if not dontmanage.db.exists("Customer", "_Test Loan Customer"):
			dontmanage.get_doc(get_customer_dict("_Test Loan Customer")).insert(ignore_permissions=True)

		if not dontmanage.db.exists("Customer", "_Test Loan Customer 1"):
			dontmanage.get_doc(get_customer_dict("_Test Loan Customer 1")).insert(ignore_permissions=True)

		self.applicant2 = dontmanage.db.get_value("Customer", {"name": "_Test Loan Customer"}, "name")
		self.applicant3 = dontmanage.db.get_value("Customer", {"name": "_Test Loan Customer 1"}, "name")

		create_loan(self.applicant1, "Personal Loan", 280000, "Repay Over Number of Periods", 20)

	def test_loan(self):
		loan = dontmanage.get_doc("Loan", {"applicant": self.applicant1})
	...
	def test_loan_disbursement(self):
		...
		loan = dontmanage.get_doc("Loan", loan.name)
		gl_entries1 = dontmanage.db.get_all(
			"GL Entry",
			fields=["name"],
			filters={"voucher_type": "Loan Disbursement", "voucher_no": loan_disbursement_entry1.name},
		)

		gl_entries2 = dontmanage.db.get_all(
			"GL Entry",
			fields=["name"],
	...
	def test_sanctioned_amount_limit(self):
		...
		dontmanage.db.sql("DELETE FROM `tabLoan` where applicant = '_Test Loan Customer 1'")
		dontmanage.db.sql("DELETE FROM `tabLoan Application` where applicant = '_Test Loan Customer 1'")
		dontmanage.db.sql("DELETE FROM `tabLoan Security Pledge` where applicant = '_Test Loan Customer 1'")
		...
		self.assertRaises(dontmanage.ValidationError, loan_application.save)
	...
	def test_loan_repayment_for_term_loan(self):
		...
		amounts = dontmanage.db.get_value(
	...
	def test_security_shortfall(self):
		...
		create_process_loan_security_shortfall()
		loan_security_shortfall = dontmanage.get_doc("Loan Security Shortfall", {"loan": loan.name})
	...
	def test_penalty(self):
		...
		calculated_penalty_amount = dontmanage.db.get_value(
...
def create_loan_accounts():
	if not dontmanage.db.exists("Account", "Loans and Advances (Assets) - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
	...
	if not dontmanage.db.exists("Account", "Loan Account - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not dontmanage.db.exists("Account", "Disbursement Account - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not dontmanage.db.exists("Account", "Interest Income Account - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not dontmanage.db.exists("Account", "Penalty Income Account - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_loan_type(
	...
	if not dontmanage.db.exists("Loan Type", loan_name):
		loan_type = dontmanage.get_doc(
			{
				"doctype": "Loan Type",
				"company": "_Test Company",
				"loan_name": loan_name,
...
def create_loan_security_type():
	if not dontmanage.db.exists("Loan Security Type", "Stock"):
		dontmanage.get_doc(
			{
...
def create_loan_security():
	if not dontmanage.db.exists("Loan Security", "Test Security 1"):
		dontmanage.get_doc(
			{
	...
	if not dontmanage.db.exists("Loan Security", "Test Security 2"):
		dontmanage.get_doc(
			{
				"doctype": "Loan Security",
				"loan_security_type": "Stock",
...
def create_loan_security_pledge(applicant, pledges, loan_application=None, loan=None):
	...
	lsp = dontmanage.new_doc("Loan Security Pledge")
...
def make_loan_disbursement_entry(loan, amount, disbursement_date=None):
	...
	loan_disbursement_entry = dontmanage.get_doc(
		{
...
def create_loan_application(
	...
	loan_application = dontmanage.new_doc("Loan Application")
	...
	loan = dontmanage.get_doc(
		{
			...
			"repayment_periods": repayment_periods,
			"repayment_start_date": repayment_start_date or nowdate(),
			"mode_of_payment": dontmanage.db.get_value("Mode of Payment", {"type": "Cash"}, "name"),
			"payment_account": "Payment Account - _TC",
			"loan_account": "Loan Account - _TC",
			"interest_income_account": "Interest Income Account - _TC",
			"penalty_income_account": "Penalty Income Account - _TC",
