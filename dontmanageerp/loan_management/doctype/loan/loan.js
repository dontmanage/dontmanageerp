...
dontmanage.ui.form.on('Loan', {
	...
	make_loan_disbursement: function (frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.loan_management.doctype.loan.loan.make_loan_disbursement",
			callback: function (r) {
				if (r.message)
	...
	make_repayment_entry: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.loan_management.doctype.loan.loan.make_repayment_entry",
			callback: function (r) {
				if (r.message)
					var doc = dontmanage.model.sync(r.message)[0];
				dontmanage.set_route("Form", doc.doctype, doc.name);
	...
	make_loan_write_off_entry: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.loan_management.doctype.loan.loan.make_loan_write_off",
			callback: function (r) {
				if (r.message)
					var doc = dontmanage.model.sync(r.message)[0];
				dontmanage.set_route("Form", doc.doctype, doc.name);
	...
	make_loan_refund: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.loan_management.doctype.loan.loan.make_refund_jv",
			callback: function (r) {
				if (r.message) {
	...
	close_unsecured_term_loan: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.loan_management.doctype.loan.loan.close_unsecured_term_loan",
			callback: function () {
				frm.refresh();
			}
		})
	...
	request_loan_closure: function(frm) {
		dontmanage.confirm(__("Do you really want to close this loan"),
			function() {
				dontmanage.call({
					args: {
						'loan': frm.doc.name
					},
					method: "dontmanageerp.loan_management.doctype.loan.loan.request_loan_closure",
					callback: function() {
						frm.reload_doc();
					}
				});
	...
	create_loan_security_unpledge: function(frm) {
		dontmanage.call({
			method: "dontmanageerp.loan_management.doctype.loan.loan.unpledge_security",
			args : {
				"loan": frm.doc.name,
				"as_dict": 1
			},
	...
	loan_application: function (frm) {
		if(frm.doc.loan_application){
			return dontmanage.call({
				method: "dontmanageerp.loan_management.doctype.loan.loan.get_loan_application",
				args: {
