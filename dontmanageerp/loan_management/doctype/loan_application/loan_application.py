...
from dontmanage.utils import cint, flt, rounded
...
from dontmanageerp.loan_management.doctype.loan_security_price.loan_security_price import (
...
class LoanApplication(Document):
	...
	def validate_loan_type(self):
		company = dontmanage.get_value("Loan Type", self.loan_type, "company")
		if company != self.company:
			dontmanage.throw(_("Please select Loan Type for company {0}").format(dontmanage.bold(self.company)))

	def validate_loan_amount(self):
		if not self.loan_amount:
			dontmanage.throw(_("Loan Amount is mandatory"))

		maximum_loan_limit = dontmanage.db.get_value("Loan Type", self.loan_type, "maximum_loan_amount")
	...
	def set_pledge_amount(self):
		for proposed_pledge in self.proposed_pledges:
			...
			if not proposed_pledge.qty and not proposed_pledge.amount:
				dontmanage.throw(_("Qty or Amount is mandatroy for loan security"))
			...
			if self.repayment_method == "Repay Fixed Amount per Period":
				...
				if monthly_interest_rate:
					...
					if self.repayment_amount - min_repayment_amount <= 0:
						dontmanage.throw(_("Repayment Amount must be greater than " + str(flt(min_repayment_amount, 2))))
	...
	def set_loan_amount(self):
		if self.is_secured_loan and not self.proposed_pledges:
			dontmanage.throw(_("Proposed Pledges are mandatory for secured Loans"))
...
def create_loan(source_name, target_doc=None, submit=0):
	def update_accounts(source_doc, target_doc, source_parent):
		account_details = dontmanage.get_all(
			"Loan Type",
			fields=[
				"mode_of_payment",
				"payment_account",
...
def create_pledge(loan_application, loan=None):
	loan_application_doc = dontmanage.get_doc("Loan Application", loan_application)
...
def get_proposed_pledge(securities):
	...
	for security in securities:
		security = dontmanage._dict(security)
