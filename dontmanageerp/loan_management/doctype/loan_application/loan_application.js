...
dontmanage.ui.form.on('Loan Application', {
	...
	add_toolbar_buttons: function(frm) {
		if (frm.doc.status == "Approved") {
			...
			if (frm.doc.is_secured_loan) {
				dontmanage.db.get_value("Loan Security Pledge", {"loan_application": frm.doc.name, "docstatus": 1}, "name", (r) => {
			...
			dontmanage.db.get_value("Loan", {"loan_application": frm.doc.name, "docstatus": 1}, "name", (r) => {
				if (Object.keys(r).length === 0) {
	...
	create_loan: function(frm) {
		if (frm.doc.status != "Approved") {
			dontmanage.throw(__("Cannot create loan until application is approved"));
		}

		dontmanage.model.open_mapped_doc({
			method: 'dontmanageerp.loan_management.doctype.loan_application.loan_application.create_loan',
			frm: frm
		});
	},
	create_loan_security_pledge: function(frm) {
		...
		if(!frm.doc.is_secured_loan) {
			dontmanage.throw(__("Loan Security Pledge can only be created for secured loans"));
		}

		dontmanage.call({
			method: "dontmanageerp.loan_management.doctype.loan_application.loan_application.create_pledge",
			args: {
				loan_application: frm.doc.name
			},
			callback: function(r) {
				dontmanage.set_route("Form", "Loan Security Pledge", r.message);
			}
		})
	},
	...
	calculate_amounts: function(frm, cdt, cdn) {
		...
		if (row.qty) {
			...
			dontmanage.model.set_value(cdt, cdn, 'qty', cint(row.amount / row.loan_security_price));
			dontmanage.model.set_value(cdt, cdn, 'amount', row.qty * row.loan_security_price);
			dontmanage.model.set_value(cdt, cdn, 'post_haircut_amount', cint(row.amount - (row.amount * row.haircut/100)));
		}
...
dontmanage.ui.form.on("Proposed Pledge", {
