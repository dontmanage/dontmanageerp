...
def update_shortfall_status(loan, security_value, on_cancel=0):
	loan_security_shortfall = dontmanage.db.get_value(
...
def add_security(loan):
	...
	loan_security_pledge = dontmanage.new_doc("Loan Security Pledge")
...
def check_for_ltv_shortfall(process_loan_security_shortfall):
	...
	loan_shortfall_map = dontmanage._dict(
		dontmanage.get_all(
...
def create_loan_security_shortfall(
	...
	existing_shortfall = dontmanage.db.get_value(
		"Loan Security Shortfall", {"loan": loan, "status": "Pending"}, "name"
	)

	if existing_shortfall:
		ltv_shortfall = dontmanage.get_doc("Loan Security Shortfall", existing_shortfall)
	else:
		ltv_shortfall = dontmanage.new_doc("Loan Security Shortfall")
...
def get_ltv_ratio(loan_security):
	loan_security_type = dontmanage.db.get_value("Loan Security", loan_security, "loan_security_type")
	ltv_ratio = dontmanage.db.get_value("Loan Security Type", loan_security_type, "loan_to_value_ratio")
	return ltv_ratio
