...
class LoanBalanceAdjustment(AccountsController):
	...
	def validate(self):
		if self.amount == 0:
			dontmanage.throw(_("Amount cannot be zero"))
		if self.amount < 0:
			dontmanage.throw(_("Amount cannot be negative"))
	...
	def set_status_and_amounts(self, cancel=0):
		...
		dontmanage.db.set_value("Loan", self.loan, {adj_field: adjustment_amount})
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		gle_map = []
		loan_account = dontmanage.db.get_value("Loan", self.loan, "loan_account")
