...
class LoanSecurityPledge(Document):
	...
	def validate_duplicate_securities(self):
		...
		for security in self.securities:
			if security.loan_security not in security_list:
				...
				dontmanage.throw(
					_("Loan Security {0} added multiple times").format(dontmanage.bold(security.loan_security))
				)
	...
	def validate_loan_security_type(self):
		...
		if self.loan:
			existing_pledge = dontmanage.db.get_value(
				"Loan Security Pledge", {"loan": self.loan, "docstatus": 1}, ["name"]
			)

		if existing_pledge:
			loan_security_type = dontmanage.db.get_value(
		...
		for security in self.securities:
			if ltv_ratio_map.get(security.loan_security_type) != ltv_ratio:
				dontmanage.throw(_("Loan Securities with different LTV ratio cannot be pledged against one loan"))
	...
	def set_pledge_amount(self):
		...
		for pledge in self.securities:
			...
			if not pledge.qty and not pledge.amount:
				dontmanage.throw(_("Qty or Amount is mandatory for loan security!"))
...
def update_loan(loan, maximum_value_against_pledge, cancel=0):
	maximum_loan_value = dontmanage.db.get_value("Loan", {"name": loan}, ["maximum_loan_amount"])
