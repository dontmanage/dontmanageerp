...
from dontmanageerp.crm.report.sales_pipeline_analytics.sales_pipeline_analytics import execute


class TestSalesPipelineAnalytics(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Opportunity")
		create_company()
		create_customer()
		create_opportunity()
...
def create_company():
	doc = dontmanage.db.exists("Company", "Best Test")
	if not doc:
		doc = dontmanage.new_doc("Company")
...
def create_customer():
	doc = dontmanage.db.exists("Customer", "_Test NC")
	if not doc:
		doc = dontmanage.new_doc("Customer")
		doc.customer_name = "_Test NC"
		doc.insert()


def create_opportunity():
	doc = dontmanage.db.exists({"doctype": "Opportunity", "party_name": "_Test NC"})
	if not doc:
		doc = dontmanage.new_doc("Opportunity")
		doc.opportunity_from = "Customer"
		customer_name = dontmanage.db.get_value("Customer", {"customer_name": "_Test NC"}, ["customer_name"])
