...
class SalesPipelineAnalytics:
	...
	def validate_filters(self):
		if not self.filters.from_date:
			dontmanage.throw(_("From Date is mandatory"))

		if not self.filters.to_date:
			dontmanage.throw(_("To Date is mandatory"))
	...
	def get_data(self):
		...
		if self.filters.get("based_on") == "Number":
			self.query_result = dontmanage.db.get_list(
				"Opportunity",
	...
	def get_periodic_data(self):
		self.periodic_data = dontmanage._dict()
				...
				if self.filters.get("assigned_to"):
					for user in assigned_to:
						if self.filters.get("assigned_to") == user:
							value = user
							self.periodic_data.setdefault(value, dontmanage._dict()).setdefault(period, 0)
	...
	def get_default_currency(self):
		...
		return dontmanage.db.get_value("Company", company, ["default_currency"])

	def get_currency_rate(self, from_currency, to_currency):
		cacheobj = dontmanage.cache()
