...
from dontmanage.utils import cstr, now, today
...
def link_communications(doctype, docname, doc):
	...
	for communication in communication_list:
		communication_doc = dontmanage.get_doc("Communication", communication)
...
def get_linked_communication_list(doctype, docname):
	...
	communication_links = dontmanage.get_all(
		"Communication Link",
...
def get_linked_prospect(reference_doctype, reference_name):
	...
	if reference_doctype == "Lead":
		prospect = dontmanage.db.get_value("Prospect Lead", {"lead": reference_name}, "parent")

	elif reference_doctype == "Opportunity":
		opportunity_from, party_name = dontmanage.db.get_value(
			"Opportunity", reference_name, ["opportunity_from", "party_name"]
		)
		if opportunity_from == "Lead":
			prospect = dontmanage.db.get_value("Prospect Opportunity", {"opportunity": reference_name}, "parent")
...
def link_open_tasks(ref_doctype, ref_docname, doc):
	...
	for todo in todos:
		todo_doc = dontmanage.get_doc("ToDo", todo.name)
...
def link_open_events(ref_doctype, ref_docname, doc):
	...
	for event in events:
		event_doc = dontmanage.get_doc("Event", event.name)
...
def get_open_events(ref_doctype, ref_docname):
	...
	event_link = dontmanage.qb.DocType("Event Participants")

	query = (
		dontmanage.qb.from_(event)
		.join(event_link)
...
def open_leads_opportunities_based_on_todays_event():
	...
	for d in data:
		dontmanage.db.set_value(d.reference_doctype, d.reference_docname, "status", "Open")


class CRMNote(Document):
	@dontmanage.whitelist()
	def add_note(self, note):
		self.append("notes", {"note": note, "added_by": dontmanage.session.user, "added_on": now()})
		self.save()
