...
dontmanage.ui.form.on('Social Media Post', {
	validate: function(frm) {
		if (frm.doc.twitter === 0 && frm.doc.linkedin === 0) {
			dontmanage.throw(__("Select atleast one Social Media Platform to Share on."));
		}
		if (frm.doc.scheduled_time) {
			...
			if (scheduled_time.getTime() < date_time.getTime()) {
				dontmanage.throw(__("Scheduled Time must be a future time."));
			}
		}
	...
	validate_tweet_length: function(frm) {
		if (frm.doc.text && frm.doc.text.length > 280) {
			dontmanage.throw(__("Tweet length Must be less than 280."));
		}
	},
		...
		if (frm.doc.docstatus === 1) {
			...
			if (frm.doc.post_status !='Deleted') {
				frm.add_custom_button(__('Delete Post'), function() {
					dontmanage.confirm(__('Are you sure want to delete the Post from Social Media platforms?'),
						function() {
							dontmanage.call({
								doc: frm.doc,
								method: 'delete_post',
