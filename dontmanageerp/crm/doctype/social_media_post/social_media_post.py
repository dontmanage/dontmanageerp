...
class SocialMediaPost(Document):
	def validate(self):
		if not self.twitter and not self.linkedin:
			dontmanage.throw(_("Select atleast one Social Media Platform to Share on."))

		if self.scheduled_time:
			current_time = dontmanage.utils.now_datetime()
			scheduled_time = dontmanage.utils.get_datetime(self.scheduled_time)
			if scheduled_time < current_time:
				dontmanage.throw(_("Scheduled Time must be a future time."))

		if self.text and len(self.text) > 280:
			dontmanage.throw(_("Tweet length must be less than 280."))
	...
	def delete_post(self):
		if self.twitter and self.twitter_post_id:
			twitter = dontmanage.get_doc("Twitter Settings")
			twitter.delete_tweet(self.twitter_post_id)

		if self.linkedin and self.linkedin_post_id:
			linkedin = dontmanage.get_doc("LinkedIn Settings")
...
def process_scheduled_social_media_posts():
	posts = dontmanage.get_list(
		"Social Media Post",
		filters={"post_status": "Scheduled", "docstatus": 1},
		fields=["name", "scheduled_time"],
	)
	start = dontmanage.utils.now_datetime()
	end = start + datetime.timedelta(minutes=10)
	for post in posts:
		if post.scheduled_time:
			post_time = dontmanage.utils.get_datetime(post.scheduled_time)
			if post_time > start and post_time <= end:
				sm_post = dontmanage.get_doc("Social Media Post", post.name)
				sm_post.post()
