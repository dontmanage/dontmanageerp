...
def get_last_interaction(contact=None, lead=None):
	...
	if contact:
		...
		if query_condition:
			...
			last_communication = dontmanage.db.sql(
				"""
				SELECT `name`, `content`
	...
	if lead:
		last_communication = dontmanage.get_all(
			"Communication",
...
def get_scheduled_employees_for_popup(communication_medium):
	...
	now_time = dontmanage.utils.nowtime()
	weekday = dontmanage.utils.get_weekday()

	available_employee_groups = dontmanage.get_all(
