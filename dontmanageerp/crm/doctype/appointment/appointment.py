...
from dontmanage.share import add_docshare
from dontmanage.utils import get_url, getdate, now
...
class Appointment(Document):
	...
	def find_customer_by_email(self):
		customer_list = dontmanage.get_list(
	...
	def before_insert(self):
		number_of_appointments_in_same_slot = dontmanage.db.count(
			"Appointment", filters={"scheduled_time": self.scheduled_time}
		)
		number_of_agents = dontmanage.db.get_single_value("Appointment Booking Settings", "number_of_agents")
		if not number_of_agents == 0:
			if number_of_appointments_in_same_slot >= number_of_agents:
				dontmanage.throw(_("Time slot is not available"))
		# Link lead
		if not self.party:
	...
	def send_confirmation_email(self):
		...
		if dontmanage.session.user == "Guest":
			dontmanage.msgprint(_("Please check your email to confirm the appointment"))
		else:
			dontmanage.msgprint(
	...
	def on_change(self):
		...
		if not self.calendar_event:
			return
		cal_event = dontmanage.get_doc("Event", self.calendar_event)
		cal_event.starts_on = self.scheduled_time
		cal_event.save(ignore_permissions=True)

	def set_verified(self, email):
		if not email == self.customer_email:
			dontmanage.throw(_("Email verification failed."))
		# Create new lead
		...
		if self.customer_details:
			lead.append(
				"notes",
				{
					...
					"added_by": dontmanage.session.user,
					"added_on": now(),
				},
			)
	...
	def get_assignee_from_latest_opportunity(self):
		if not self.party:
			return None
		if not dontmanage.db.exists("Lead", self.party):
			return None
		opporutnities = dontmanage.get_list(
			"Opportunity",
			filters={
		...
		if not opporutnities:
			return None
		latest_opportunity = dontmanage.get_doc("Opportunity", opporutnities[0].name)
		assignee = latest_opportunity._assign
		if not assignee:
			return None
		assignee = dontmanage.parse_json(assignee)[0]
		return assignee

	def create_calendar_event(self):
		if self.calendar_event:
			return
		appointment_event = dontmanage.get_doc(
			{
				"doctype": "Event",
				...
				"status": "Open",
				"type": "Public",
				"send_reminder": dontmanage.db.get_single_value(
	...
	def assign_agent(self, agent):
		if not dontmanage.has_permission(doc=self, user=agent):
...
def _get_agents_sorted_by_asc_workload(date):
	appointments = dontmanage.get_all("Appointment", fields="*")
	agent_list = _get_agent_list_as_strings()
	if not appointments:
		return agent_list
	appointment_counter = Counter(agent_list)
	for appointment in appointments:
		assigned_to = dontmanage.parse_json(appointment._assign)
		if not assigned_to:
			continue
...
def _get_agent_list_as_strings():
	...
	agent_list = dontmanage.get_doc("Appointment Booking Settings").agent_list
...
def _check_agent_availability(agent_email, scheduled_time):
	appointemnts_at_scheduled_time = dontmanage.get_all("Appointment", filters={"scheduled_time": scheduled_time})
...
def _get_employee_from_user(user):
	employee_docname = dontmanage.db.get_value("Employee", {"user_id": user})
	if employee_docname:
		return dontmanage.get_doc("Employee", employee_docname)
	return None
