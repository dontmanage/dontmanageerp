...
from dontmanage.utils import flt, get_fullname

from dontmanageerp.crm.utils import (
	CRMNote,
	copy_comments,
	link_communications,
	link_open_events,
...
class Opportunity(TransactionBase, CRMNote):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.crm.doctype.competitor_detail.competitor_detail import CompetitorDetail
		from dontmanageerp.crm.doctype.crm_note.crm_note import CRMNote
		from dontmanageerp.crm.doctype.opportunity_item.opportunity_item import OpportunityItem
		from dontmanageerp.crm.doctype.opportunity_lost_reason_detail.opportunity_lost_reason_detail import (
	...
	def onload(self):
		ref_doc = dontmanage.get_doc(self.opportunity_from, self.party_name)
	...
	def after_insert(self):
		if self.opportunity_from == "Lead":
			dontmanage.get_doc("Lead", self.party_name).set_status(update=True)

			link_open_tasks(self.opportunity_from, self.party_name, self)
			link_open_events(self.opportunity_from, self.party_name, self)
			if dontmanage.db.get_single_value("CRM Settings", "carry_forward_communication_and_comments"):
	...
	def map_fields(self):
		for field in self.meta.get_valid_columns():
			if not self.get(field) and dontmanage.db.field_exists(self.opportunity_from, field):
				try:
					value = dontmanage.db.get_value(self.opportunity_from, self.party_name, field)
					self.set(field, value)
				except Exception:
					continue

	def set_exchange_rate(self):
		company_currency = dontmanage.get_cached_value("Company", self.company, "default_currency")
	...
	def update_prospect(self):
		...
		if self.opportunity_from == "Prospect" and self.party_name:
			...
			prospect_name = dontmanage.db.get_value("Prospect Lead", {"lead": self.party_name}, "parent")

		if prospect_name:
			prospect = dontmanage.get_doc("Prospect", prospect_name)
	...
	def make_new_lead_if_required(self):
		...
		if (not self.get("party_name")) and self.contact_email:
			...
			customer = (
				dontmanage.qb.from_(dynamic_link)
				.join(contact)
				.on(
			...
			lead_name = dontmanage.db.get_value("Lead", {"email_id": self.contact_email})
			if not lead_name:
	...
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not self.has_active_quotation():
			...
			dontmanage.throw(_("Cannot declare as lost, because Quotation has been made."))
	...
	def has_lost_quotation(self):
		lost_quotation = dontmanage.db.sql(
			"""
			select name
			from `tabQuotation`
			where docstatus=1
	...
	def validate_cust_name(self):
		if self.party_name:
			if self.opportunity_from == "Customer":
				self.customer_name = dontmanage.db.get_value("Customer", self.party_name, "customer_name")
			elif self.opportunity_from == "Lead":
				customer_name = dontmanage.db.get_value("Prospect Lead", {"lead": self.party_name}, "parent")
				if not customer_name:
					lead_name, company_name = dontmanage.db.get_value(
	...
	def validate_item_details(self):
		...
		for d in self.items:
			...
			item = dontmanage.db.get_value("Item", d.item_code, item_fields, as_dict=True)
...
def make_quotation(source_name, target_doc=None):
	def set_missing_values(source, target):
		from dontmanageerp.controllers.accounts_controller import get_default_taxes_and_charges

		quotation = dontmanage.get_doc(target)

		company_currency = dontmanage.get_cached_value("Company", quotation.company, "default_currency")
...
def set_multiple_status(names, status):
	...
	for name in names:
		opp = dontmanage.get_doc("Opportunity", name)
		opp.status = status
		opp.save()
...
def auto_close_opportunity():
	...
	auto_close_after_days = dontmanage.db.get_single_value("CRM Settings", "close_opportunity_after_days") or 15

	table = dontmanage.qb.DocType("Opportunity")
	opportunities = (
	...
	for opportunity in opportunities:
		doc = dontmanage.get_doc("Opportunity", opportunity)
		doc.status = "Closed"
...
def make_opportunity_from_communication(communication, company, ignore_communication_links=False):
	from dontmanageerp.crm.doctype.lead.lead import make_lead_from_communication
