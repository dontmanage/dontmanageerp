...
import dontmanage
from dontmanage.utils import now_datetime, random_string, today

from dontmanageerp.crm.doctype.lead.lead import make_customer
from dontmanageerp.crm.doctype.lead.test_lead import make_lead
from dontmanageerp.crm.doctype.opportunity.opportunity import make_quotation
from dontmanageerp.crm.utils import get_linked_communication_list

test_records = dontmanage.get_test_records("Opportunity")
...
class TestOpportunity(unittest.TestCase):
	def test_opportunity_status(self):
		...
		doc = dontmanage.get_doc("Opportunity", doc.name)
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(dontmanage.db.get_value("Lead", opp_doc.party_name, "email_id"), opp_doc.contact_email)
	...
	def test_carry_forward_of_email_and_comments(self):
		dontmanage.db.set_single_value("CRM Settings", "carry_forward_communication_and_comments", 1)
		lead_doc = make_lead()
		...
		opportunity_comment_count = dontmanage.db.count(
...
def make_opportunity_from_lead():
	...
	opp_doc = dontmanage.get_doc(args).insert(ignore_permissions=True)
