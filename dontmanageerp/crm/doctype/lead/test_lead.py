...
import dontmanage
from dontmanage.utils import random_string, today

from dontmanageerp.crm.doctype.lead.lead import make_opportunity
from dontmanageerp.crm.utils import get_linked_prospect

test_records = dontmanage.get_test_records("Lead")


class TestLead(unittest.TestCase):
	def test_make_customer(self):
		from dontmanageerp.crm.doctype.lead.lead import make_customer

		dontmanage.delete_doc_if_exists("Customer", "_Test Lead")
	...
	def test_create_lead_and_unlinking_dynamic_links(self):
		...
		lead_doc.delete()
		address_1.reload()
		self.assertEqual(dontmanage.db.exists("Lead", lead_doc.name), None)
		self.assertEqual(len(address_1.get("links")), 1)

	def test_prospect_creation_from_lead(self):
		dontmanage.db.sql("delete from `tabLead` where lead_name='Rahul Tripathi'")
		dontmanage.db.sql("delete from `tabProspect` where name='Prospect Company'")
	...
	def test_opportunity_from_lead(self):
		dontmanage.db.sql("delete from `tabLead` where lead_name='Rahul Tripathi'")
		dontmanage.db.sql("delete from `tabOpportunity` where party_name='Rahul Tripathi'")
		...
		self.assertTrue(
			dontmanage.db.get_value("ToDo", {"reference_type": "Opportunity", "reference_name": opportunity.name})
		)
