...
from dontmanage.utils import comma_and, get_link_to_form, has_gravatar, validate_email_address
...
from dontmanageerp.crm.utils import CRMNote, copy_comments, link_communications, link_open_events
from dontmanageerp.selling.doctype.customer.customer import parse_full_name
...
class Lead(SellingController, CRMNote):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.crm.doctype.crm_note.crm_note import CRMNote
	...
	def onload(self):
		customer = dontmanage.db.get_value("Customer", {"lead_name": self.name})
	...
	def before_insert(self):
		...
		if dontmanage.db.get_single_value("CRM Settings", "auto_creation_of_contact"):
			if self.source == "Existing Customer" and self.customer:
				...
				if contact:
					self.contact_doc = dontmanage.get_doc("Contact", contact)
					return
	...
	def on_trash(self):
		dontmanage.db.set_value("Issue", {"lead": self.name}, "lead", None)
	...
	def set_lead_name(self):
		if not self.lead_name:
			...
			if not self.company_name and not self.email_id and not self.flags.ignore_mandatory:
				dontmanage.throw(_("A Lead requires either a person's name or an organization's name"))
	...
	def check_email_id_is_unique(self):
		if self.email_id:
			...
			if not dontmanage.db.get_single_value("CRM Settings", "allow_lead_duplication_based_on_emails"):
				duplicate_leads = dontmanage.get_all(
					"Lead", filters={"email_id": self.email_id, "name": ["!=", self.name]}
				)
				duplicate_leads = [
					dontmanage.bold(get_link_to_form("Lead", lead.name)) for lead in duplicate_leads
				]
	...
	def validate_email_id(self):
		if self.email_id:
			...
			if self.email_id == self.lead_owner:
				dontmanage.throw(_("Lead Owner cannot be same as the Lead Email Address"))
	...
	def update_prospect(self):
		lead_row_name = dontmanage.db.get_value("Prospect Lead", filters={"lead": self.name}, fieldname="name")
		if lead_row_name:
			lead_row = dontmanage.get_doc("Prospect Lead", lead_row_name)
			lead_row.update(
				{
	...
	def remove_link_from_prospect(self):
		...
		for d in prospects:
			prospect = dontmanage.get_doc("Prospect", d.parent)
	...
	def has_customer(self):
		return dontmanage.db.get_value("Customer", {"lead_name": self.name})

	def has_opportunity(self):
		return dontmanage.db.get_value("Opportunity", {"party_name": self.name, "status": ["!=", "Lost"]})
	...
	def has_lost_quotation(self):
		return dontmanage.db.get_value("Quotation", {"party_name": self.name, "docstatus": 1, "status": "Lost"})
	...
	def create_prospect(self, company_name):
		try:
			prospect = dontmanage.new_doc("Prospect")
			...
			dontmanage.throw(_("Prospect {0} already exists").format(company_name or self.company_name))
...
def _make_customer(source_name, target_doc=None, ignore_permissions=False):
	def set_missing_values(source, target):
		...
		target.customer_group = dontmanage.db.get_default("Customer Group")
...
def get_lead_details(lead, posting_date=None, company=None):
	...
	from dontmanageerp.accounts.party import set_address_details

	out = dontmanage._dict()

	lead_doc = dontmanage.get_doc("Lead", lead)
	lead = lead_doc
...
def make_lead_from_communication(communication, ignore_communication_links=False):
	...
	lead_name = None
	if doc.sender:
		lead_name = dontmanage.db.get_value("Lead", {"email_id": doc.sender})
	if not lead_name and doc.phone_no:
		lead_name = dontmanage.db.get_value("Lead", {"mobile_no": doc.phone_no})
	if not lead_name:
		lead = dontmanage.get_doc(
			{
				"doctype": "Lead",
...
def add_lead_to_prospect(lead, prospect):
	prospect = dontmanage.get_doc("Prospect", prospect)
	prospect.append("leads", {"lead": lead})
	prospect.save(ignore_permissions=True)

	carry_forward_communication_and_comments = dontmanage.db.get_single_value(
	...
	dontmanage.msgprint(
		_("Lead {0} has been added to prospect {1}.").format(dontmanage.bold(lead), dontmanage.bold(prospect.name)),
