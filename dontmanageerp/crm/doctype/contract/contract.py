...
class Contract(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.crm.doctype.contract_fulfilment_checklist.contract_fulfilment_checklist import (
	...
	def autoname(self):
		...
		if dontmanage.db.exists("Contract", name):
			count = len(dontmanage.get_all("Contract", filters={"name": ["like", "%{}%".format(name)]}))
	...
	def before_submit(self):
		self.signed_by_company = dontmanage.session.user
...
def update_status_for_contracts():
	...
	for contract in contracts:
		...
		dontmanage.db.set_value("Contract", contract.get("name"), "status", status)
