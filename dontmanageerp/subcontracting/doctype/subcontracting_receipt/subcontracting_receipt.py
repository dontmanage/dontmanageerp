	...
	def update_status(self, status=None, update_modified=False):
		if self.docstatus >= 1 and not status:
			if self.docstatus == 1:
				if self.is_return:
					status = "Return"
					return_against = dontmanage.get_doc("Subcontracting Receipt", self.return_against)
		...
		if status:
			dontmanage.db.set_value("Subcontracting Receipt", self.name, "status", status, update_modified)
