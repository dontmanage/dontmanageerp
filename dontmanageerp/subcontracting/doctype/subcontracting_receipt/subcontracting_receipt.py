...
class SubcontractingReceipt(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.subcontracting.doctype.subcontracting_receipt_item.subcontracting_receipt_item import (
			SubcontractingReceiptItem,
		)
		from dontmanageerp.subcontracting.doctype.subcontracting_receipt_supplied_item.subcontracting_receipt_supplied_item import (
	...
	def validate(self):
		...
		if getdate(self.posting_date) > getdate(nowdate()):
			dontmanage.throw(_("Posting Date cannot be future date"))

		super(SubcontractingReceipt, self).validate()

		if self.is_new() and self.get("_action") == "save" and not dontmanage.flags.in_test:
	...
	def set_items_bom(self):
		if self.is_return:
			for item in self.items:
				if not item.bom:
					item.bom = dontmanage.db.get_value(
	...
	def set_available_qty_for_consumption(self):
		...
		sco_supplied_item = dontmanage.qb.DocType("Subcontracting Order Supplied Item")
		for item in self.get("items"):
			supplied_items = (
				dontmanage.qb.from_(sco_supplied_item)
				.select(
	...
	def update_status(self, status=None, update_modified=False):
		if not status:
			...
			if self.is_return:
				dontmanage.get_doc("Subcontracting Receipt", self.return_against).update_status(
	...
	def get_gl_entries(self, warehouse_account=None):
		...
		if not dontmanageerp.is_perpetual_inventory_enabled(self.company):
			return []
	...
	def auto_create_purchase_receipt(self):
		if dontmanage.db.get_single_value("Buying Settings", "auto_create_purchase_receipt"):
...
def make_purchase_receipt(source_name, target_doc=None, save=False, submit=False, notify=False):
	if isinstance(source_name, str):
		source_doc = dontmanage.get_doc("Subcontracting Receipt", source_name)
	else:
		source_doc = source_name

	if not source_doc.is_return:
		if not target_doc:
			target_doc = dontmanage.new_doc("Purchase Receipt")
		...
		target_doc.currency = dontmanage.get_cached_value("Company", target_doc.company, "default_currency")

		po_items_details = {}
		for item in source_doc.items:
			if item.purchase_order and item.purchase_order_item:
				if item.purchase_order not in po_items_details:
					po_doc = dontmanage.get_doc("Purchase Order", item.purchase_order)
