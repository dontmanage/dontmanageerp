...
from dontmanage.utils import add_days, cint, flt, nowtime, today
...
class TestSubcontractingReceipt(DontManageTestCase):
	def setUp(self):
		make_subcontracted_items()
		make_raw_materials()
		make_service_items()
	...
	def test_subcontracting_over_receipt(self):
		...
		"""
		from dontmanageerp.controllers.subcontracting_controller import (
		...
		ste = dontmanage.get_doc(make_subcontract_transfer_entry(sco.name, rm_items))
		...
		scr1.submit()
		self.assertRaises(dontmanage.ValidationError, scr2.submit)
	...
	def test_subcontracting_receipt_over_return(self):
		...
		args = dontmanage._dict(scr_name=scr1.name, qty=-15)
	...
	def test_subcontracting_receipt_gl_entry(self):
		...
		self.assertEqual(cint(dontmanageerp.is_perpetual_inventory_enabled(scr.company)), 1)
		...
		bundle_doc = make_serial_batch_bundle(
			{
				...
				"qty": -1,
				"batches": dontmanage._dict({rm_batch_no: 1}),
	...
	def test_subcontracting_receipt_valuation_with_auto_created_serial_batch_bundle(self):
		...
		for row in scr.supplied_items:
			...
			auto_created_serial_batch = dontmanage.db.get_value(
				"Stock Ledger Entry",
	...
	def test_quality_inspection_for_subcontracting_receipt(self):
		from dontmanageerp.stock.doctype.quality_inspection.test_quality_inspection import (
		...
		dontmanage.db.set_value("Item", fg_item, "inspection_required_before_purchase", 1)

		# ValidationError should be raised as Quality Inspection is not created/linked
		self.assertRaises(dontmanage.ValidationError, scr1.submit)
		...
		dontmanage.db.set_value("Item", fg_item, "inspection_required_before_purchase", 0)
		...
		batch_doc = dontmanage.get_doc(
			{
				"doctype": "Batch",
				"item": fg_item,
				"batch_id": dontmanage.generate_hash(length=10),
			}
		).insert(ignore_permissions=True)

		serial_batch_bundle = dontmanage.get_doc(
			{
	...
	def test_auto_create_purchase_receipt(self):
		...
		pr_qty = dontmanage.db.get_value("Purchase Receipt Item", {"parent": pr_details[0]["name"]}, "qty")
		...
		rm_items = [
			{
				...
				"warehouse": "_Test Warehouse - _TC",
				"rate": 100,
				"stock_uom": dontmanage.get_cached_value("Item", rm_item1, "stock_uom"),
				"use_serial_batch_fields": 1,
			},
		]
		se = dontmanage.get_doc(make_rm_stock_entry(sco.name, rm_items))
