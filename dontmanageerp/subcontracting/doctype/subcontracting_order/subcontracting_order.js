...
dontmanageerp.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
dontmanage.ui.form.on("Subcontracting Order Item", {
	qty(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		dontmanage.model.set_value(cdt, cdn, "amount", row.qty * row.rate);
		...
		dontmanage.model.set_value(service_item.doctype, service_item.name, "fg_item_qty", row.qty);
		dontmanage.model.set_value(
			service_item.doctype,
			service_item.name,
			"amount",
...
dontmanage.ui.form.on("Subcontracting Order", {
	setup: (frm) => {
	...
	set_queries: (frm) => {
		...
		frm.set_query("supplier_address", dontmanageerp.queries.address_query);

		frm.set_query("billing_address", dontmanageerp.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return dontmanageerp.queries.company_address_query(frm.doc);
		});
	},
	...
	refresh: function (frm) {
		dontmanage.dynamic_link = { doc: frm.doc, fieldname: "supplier", doctype: "Supplier" };
	...
	update_subcontracting_order_status(frm, status) {
		dontmanage.call({
			method: "dontmanageerp.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
...
dontmanageerp.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
...
extend_cscript(cur_frm.cscript, new dontmanageerp.buying.SubcontractingOrderController({ frm: cur_frm }));
