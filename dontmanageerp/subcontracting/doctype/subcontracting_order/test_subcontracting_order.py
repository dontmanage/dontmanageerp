...
from dontmanageerp.buying.doctype.purchase_order.purchase_order import get_mapped_subcontracting_order
...
class TestSubcontractingOrder(DontManageTestCase):
	def setUp(self):
		make_subcontracted_items()
		make_raw_materials()
		make_service_items()
	...
	def test_update_status(self):
		...
		scr = make_subcontracting_receipt(sco.name)
		scr.save()
		self.assertRaises(dontmanage.exceptions.ValidationError, scr.submit)
		sco.update_status()
		self.assertEqual(sco.status, "Partially Received")
		scr.cancel()
	...
	def test_update_reserved_qty_for_subcontracting(self):
		...
		bin_before_sco = dontmanage.db.get_value(
			"Bin",
		...
		bin_after_sco = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname=["reserved_qty_for_sub_contract", "projected_qty", "modified"],
			as_dict=1,
		...
		ste = dontmanage.get_doc(make_rm_stock_entry(sco.name, rm_items))
		ste.to_warehouse = "_Test Warehouse 1 - _TC"
		ste.save()
		ste.submit()

		bin_after_rm_transfer = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
		...
		ste.cancel()
		bin_after_cancel_ste = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname="reserved_qty_for_sub_contract",
			as_dict=1,
		...
		sco.reload()
		sco.cancel()
		bin_after_cancel_sco = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname="reserved_qty_for_sub_contract",
			as_dict=1,
	...
	def test_exploded_items(self):
		...
		item_name = dontmanage.db.get_value("BOM", {"item": item_code}, "name")
		bom = dontmanage.get_doc("BOM", item_name)
	...
	def test_backflush_based_on_stock_entry(self):
		...
		transferred_rm_map = dontmanage._dict()
		for item in rm_items:
	...
	def test_ordered_qty_for_subcontracting_order(self):
		...
		ordered_qty = dontmanage.db.get_value(
			"Bin",
		...
		new_ordered_qty = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "Subcontracted Item SA8"},
			fieldname="ordered_qty",
		)
	...
	def test_requested_qty_for_subcontracting_order(self):
		...
		requested_qty = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "Subcontracted Item SA8"},
		...
		new_requested_qty = dontmanage.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "Subcontracted Item SA8"},
			fieldname="indented_qty",
		)
...
def create_subcontracting_order(**args):
	...
	if args.warehouse:
		...
		warehouse = dontmanage.get_value("Purchase Order", args.po_name, "set_warehouse")
		if warehouse:
			for item in sco.items:
				item.warehouse = warehouse
		else:
			po = dontmanage.get_doc("Purchase Order", args.po_name)
			warehouses = []
