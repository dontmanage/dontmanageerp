dontmanage.provide("dontmanageerp.edi");

dontmanageerp.edi.import_genericode = function (listview_or_form) {
	...
	new dontmanage.ui.FileUploader({
		method: "dontmanageerp.edi.doctype.code_list.code_list_import.import_genericode",
		doctype: doctype,
		docname: docname,
...
function show_column_selection_dialog(context) {
	...
	let d = new dontmanage.ui.Dialog({
		...
		primary_action(values) {
			...
			dontmanage
				.xcall("dontmanageerp.edi.doctype.code_list.code_list_import.process_genericode_import", {
				...
				.then((count) => {
					dontmanage.msgprint(__("Import completed. {0} common codes created.", [count]));
				});
			d.hide();
		},
	});
