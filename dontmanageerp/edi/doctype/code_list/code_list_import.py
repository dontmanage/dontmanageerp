...
def import_genericode():
	...
	content = dontmanage.local.uploaded_file

	# recover the content, if it's a link
	if (file_url := dontmanage.local.uploaded_file_url) and file_url.startswith(URL_PREFIXES):
		try:
			# If it's a URL, fetch the content and make it a local file (for durable audit)
			response = requests.get(dontmanage.local.uploaded_file_url)
			response.raise_for_status()
			dontmanage.local.uploaded_file = content = response.content
			dontmanage.local.uploaded_filename = dontmanage.local.uploaded_file_url.split("/")[-1]
			dontmanage.local.uploaded_file_url = None
		except Exception as e:
			dontmanage.throw(f"<pre>{e!s}</pre>", title=_("Fetching Error"))

	if file_url := dontmanage.local.uploaded_file_url:
		file_path = dontmanage.utils.file_manager.get_file_path(file_url)
	...
	try:
		...
		dontmanage.throw(f"<pre>{e!s}</pre>", title=_("Parsing Error"))

	# Extract the name (CanonicalVersionUri) from the parsed XML
	name = root.find(".//CanonicalVersionUri").text
	docname = docname or name

	if dontmanage.db.exists(doctype, docname):
		code_list = dontmanage.get_doc(doctype, docname)
		if code_list.name != name:
			dontmanage.throw(_("The uploaded file does not match the selected Code List."))
	else:
		# Create a new Code List document with the extracted name
		code_list = dontmanage.new_doc(doctype)
		code_list.name = name
	...
	file_doc = dontmanage.get_doc(
		{
			...
			"file_name": dontmanage.local.uploaded_filename,
			"file_url": dontmanage.local.uploaded_file_url,
			"is_private": 1,
			"content": content,
		}
	).save()
...
def process_genericode_import(
	...
	from dontmanageerp.edi.doctype.common_code.common_code import import_genericode
