...
class CodeList(Document):
	...
	def __delete_linked_docs(self):
		...
		for doc in linked_docs:
			dontmanage.delete_doc("Common Code", doc.name)
	...
	def get_default_code(self) -> str | None:
		...
		return (
			dontmanage.db.get_value("Common Code", self.default_common_code, "common_code")
...
def get_codes_for(code_list: str, doctype: str, name: str) -> tuple[str]:
	...
	CommonCode = dontmanage.qb.DocType("Common Code")
	DynamicLink = dontmanage.qb.DocType("Dynamic Link")

	codes = (
		dontmanage.qb.from_(CommonCode)
		.join(DynamicLink)
...
def get_default_code(code_list: str) -> str | None:
	...
	code_id = dontmanage.db.get_value("Code List", code_list, "default_common_code")
	return dontmanage.db.get_value("Common Code", code_id, "common_code") if code_id else None
