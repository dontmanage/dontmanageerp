...
class CommonCode(Document):
	...
	def validate_distinct_references(self):
		...
		for link in self.applies_to:
			existing_links = dontmanage.get_all(
				"Common Code",
				filters=[
					["name", "!=", self.name],
...
def import_genericode(code_list: str, file_name: str, column_map: dict, filters: dict | None = None):
	...
	file_path = dontmanage.utils.file_manager.get_file_path(file_name)
	...
	for i, xml_element in enumerate(elements, start=1):
		common_code: "CommonCode" = dontmanage.new_doc("Common Code")
		common_code.code_list = code_list
		common_code.from_genericode(column_map, xml_element)
		common_code.save()
		dontmanage.publish_progress(i / total_elements * 100, title=_("Importing Common Codes"))

	return total_elements


def on_doctype_update():
	dontmanage.db.add_index("Common Code", ["code_list", "common_code"])
