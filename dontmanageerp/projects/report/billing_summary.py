...
from dontmanage.utils import flt, time_diff_in_hours
...
def get_data(filters):
	...
	if filters.from_date > filters.to_date:
		dontmanage.msgprint(_("From Date can not be greater than To Date"))
		return data

	timesheets = get_timesheets(filters)

	filters.from_date = dontmanage.utils.get_datetime(filters.from_date)
	filters.to_date = dontmanage.utils.add_to_date(dontmanage.utils.get_datetime(filters.to_date), days=1, seconds=-1)
...
def get_timesheets(filters):
	...
	timesheet_map = dontmanage._dict()
	for d in timesheets:
...
def get_timesheet_details(filters, timesheet_list):
	...
	timesheet_details = dontmanage.get_all(
		"Timesheet Detail",
	...
	timesheet_details_map = dontmanage._dict()
...
def get_billable_and_total_duration(activity, start_time, end_time):
	precision = dontmanage.get_precision("Timesheet Detail", "hours")
