...
from dontmanageerp.projects.doctype.task.task import CircularReferenceError
...
class TestTask(unittest.TestCase):
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(
			dontmanage.db.get_value("Task", task2.name, "exp_start_date"), getdate(add_days(nowdate(), 21))
		)
		self.assertEqual(
			dontmanage.db.get_value("Task", task2.name, "exp_end_date"), getdate(add_days(nowdate(), 25))
		)

		self.assertEqual(
			dontmanage.db.get_value("Task", task3.name, "exp_start_date"), getdate(add_days(nowdate(), 26))
		)
		self.assertEqual(
			dontmanage.db.get_value("Task", task3.name, "exp_end_date"), getdate(add_days(nowdate(), 30))
		)

	def test_close_assignment(self):
		if not dontmanage.db.exists("Task", "Test Close Assignment"):
			task = dontmanage.new_doc("Task")
	...
	def test_overdue(self):
		...
		from dontmanageerp.projects.doctype.task.task import set_tasks_as_overdue

		set_tasks_as_overdue()

		self.assertEqual(dontmanage.db.get_value("Task", task.name, "status"), "Overdue")
...
def create_task(
	...
	if not dontmanage.db.exists("Task", subject):
		task = dontmanage.new_doc("Task")
		task.status = "Open"
		...
		task.project = (
			project or None if is_template else dontmanage.get_value("Project", {"project_name": "_Test Project"})
		)
		...
		task = dontmanage.get_doc("Task", subject)
