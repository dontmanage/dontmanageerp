dontmanage.listview_settings['Task'] = {
	add_fields: ["project", "status", "priority", "exp_start_date",
		"exp_end_date", "subject", "progress", "depends_on_tasks"],
	filters: [["status", "=", "Open"]],
	onload: function(listview) {
		var method = "dontmanageerp.projects.doctype.task.task.set_multiple_status";
		...
		if (task._assign) {
			...
			const assignment_wrapper = `
				...
				<span class="text-white">
					${assign_list.map((user) => dontmanage.user_info(user).fullname).join(", ")}
				</span>
			`;
