...
class CircularReferenceError(dontmanage.ValidationError):
	pass
...
class Task(NestedSet):
	...
	def validate_status(self):
		...
		if self.status != self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if dontmanage.db.get_value("Task", d.task, "status") not in ("Completed", "Cancelled"):
					dontmanage.throw(
						_(
							"Cannot complete task {0} as its dependant task {1} are not completed / cancelled."
						).format(dontmanage.bold(self.name), dontmanage.bold(d.task))
					)
	...
	def validate_progress(self):
		if flt(self.progress or 0) > 100:
			dontmanage.throw(_("Progress % for a task cannot be more than 100."))
	...
	def validate_parent_template_task(self):
		if self.parent_task:
			if not dontmanage.db.get_value("Task", self.parent_task, "is_template"):
				parent_task_format = """<a href="#Form/Task/{0}">{0}</a>""".format(self.parent_task)
				dontmanage.throw(_("Parent Task {0} is not a Template Task").format(parent_task_format))

	def validate_depends_on_tasks(self):
		if self.depends_on:
			for task in self.depends_on:
				if not dontmanage.db.get_value("Task", task.task, "is_template"):
					dependent_task_format = """<a href="#Form/Task/{0}">{0}</a>""".format(task.task)
					dontmanage.throw(_("Dependent Task {0} is not a Template Task").format(dependent_task_format))

	def validate_completed_on(self):
		if self.completed_on and getdate(self.completed_on) > getdate():
			dontmanage.throw(_("Completed On cannot be greater than Today"))
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			dontmanage.get_cached_doc("Project", self.project).update_project()
	...
	def check_recursion(self):
		...
		for d in check_list:
			...
			while len(task_list) > count:
				...
				for b in tasks:
					if b[0] == self.name:
						dontmanage.throw(_("Circular Reference Error"), CircularReferenceError)
					if b[0]:
	...
	def reschedule_dependent_tasks(self):
		...
		if end_date:
			for task_name in dontmanage.db.sql(
				"""
			...
			""",
				...
				task = dontmanage.get_doc("Task", task_name.name)
				if (
					task.exp_start_date
	...
	def has_webform_permission(self):
		project_user = dontmanage.db.get_value(
			"Project User", {"parent": self.project, "user": dontmanage.session.user}, "user"
		)
		if project_user:
			return True
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = dontmanage.get_doc("Task", self.parent_task)
...
def check_if_child_exists(name):
	child_tasks = dontmanage.get_all("Task", filters={"parent_task": name})
...
def set_multiple_status(names, status):
	...
	for name in names:
		task = dontmanage.get_doc("Task", name)
		task.status = status
		task.save()


def set_tasks_as_overdue():
	tasks = dontmanage.get_all(
		"Task",
	...
	for task in tasks:
		if task.status == "Pending Review":
			if getdate(task.review_date) > getdate(today()):
				continue
		dontmanage.get_doc("Task", task.name).update_status()
...
def add_multiple_tasks(data, parent):
	...
	new_doc["project"] = dontmanage.db.get_value("Task", {"name": parent}, "project") or ""

	for d in data:
		if not d.get("subject"):
			continue
		new_doc["subject"] = d.get("subject")
		new_task = dontmanage.get_doc(new_doc)
		new_task.insert()
