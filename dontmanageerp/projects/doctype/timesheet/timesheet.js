...
dontmanage.ui.form.on("Timesheet", {
	setup: function(frm) {
		dontmanage.require("/assets/dontmanageerp/js/projects/timer.js");
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus < 1) {
			...
			$.each(frm.doc.time_logs || [], function(i, row) {
				if ((row.from_time <= dontmanage.datetime.now_datetime()) && !row.completed) {
					button = 'Resume Timer';
				}
			});
			...
			frm.add_custom_button(__(button), function() {
				...
				$.each(frm.doc.time_logs || [], function(i, row) {
					...
					if (flag && row.activity_type && !row.from_time){
						dontmanageerp.timesheet.timer(frm, row);
						row.from_time = dontmanage.datetime.now_datetime();
						frm.refresh_fields("time_logs");
						frm.save();
						flag = false;
					}
					// Fetch the row for timer where activity is not completed and from_time is before now_time
					if (flag && row.from_time <= dontmanage.datetime.now_datetime() && !row.completed) {
						let timestamp = moment(dontmanage.datetime.now_datetime()).diff(moment(row.from_time),"seconds");
						dontmanageerp.timesheet.timer(frm, row, timestamp);
						flag = false;
					}
				});
				// If no activities found to start a timer, create new
				if (flag) {
					dontmanageerp.timesheet.timer(frm);
				}
	...
	currency: function(frm) {
		let base_currency = dontmanage.defaults.get_global_default('currency');
	...
	set_dynamic_field_label: function(frm) {
		...
		if (frm.doc.time_logs.length > 0) {
			...
			$.each(["base_billing_rate", "base_billing_amount", "base_costing_rate", "base_costing_amount"], function(i, d) {
				if (dontmanage.meta.get_docfield(time_logs_grid.doctype, d))
	...
	make_invoice: function(frm) {
		...
		dialog.set_primary_action(__('Create Sales Invoice'), () => {
			...
			return dontmanage.call({
				type: "GET",
				method: "dontmanageerp.projects.doctype.timesheet.timesheet.make_sales_invoice",
				args: {
...
dontmanage.ui.form.on("Timesheet Detail", {
	...
	task: (frm, cdt, cdn) => {
		...
		if (row.task) {
			dontmanage.db.get_value("Task", row.task, "project", (r) => {
				dontmanage.model.set_value(cdt, cdn, "project", r.project);
			});
		}
	},
	...
	to_time: function(frm, cdt, cdn) {
		...
		dontmanage.model.set_value(cdt, cdn, "hours", hours);
	},

	time_logs_add: function(frm, cdt, cdn) {
		if(frm.doc.parent_project) {
			dontmanage.model.set_value(cdt, cdn, 'project', frm.doc.parent_project);
		}
	},
	...
	activity_type: function (frm, cdt, cdn) {
		if (!dontmanage.get_doc(cdt, cdn).activity_type) return;

		dontmanage.call({
			method: "dontmanageerp.projects.doctype.timesheet.timesheet.get_activity_cost",
			args: {
			...
			callback: function (r) {
				if (r.message) {
					dontmanage.model.set_value(cdt, cdn, "billing_rate", r.message["billing_rate"]);
					dontmanage.model.set_value(cdt, cdn, "costing_rate", r.message["costing_rate"]);
					calculate_billing_costing_amount(frm, cdt, cdn);
				}
			}
		});
...
var calculate_end_time = function(frm, cdt, cdn) {
	...
	if(!child.from_time) {
		...
		dontmanage.model.set_value(cdt, cdn, "from_time", dontmanage.datetime.get_datetime_as_string());
	}
	...
	if(child.hours) {
		...
		dontmanage.model.set_value(cdt, cdn, "to_time",
			d.format(dontmanage.defaultDatetimeFormat)).then(() => {
...
var update_billing_hours = function(frm, cdt, cdn) {
	let child = dontmanage.get_doc(cdt, cdn);
	if (!child.is_billable) {
		dontmanage.model.set_value(cdt, cdn, 'billing_hours', 0.0);
	} else {
		// bill all hours by default
		dontmanage.model.set_value(cdt, cdn, "billing_hours", child.hours);
	}
};
...
var update_time_rates = function(frm, cdt, cdn) {
	let child = dontmanage.get_doc(cdt, cdn);
	if (!child.is_billable) {
		dontmanage.model.set_value(cdt, cdn, 'billing_rate', 0.0);
	}
};
...
var calculate_billing_costing_amount = function(frm, cdt, cdn) {
	...
	dontmanage.model.set_value(cdt, cdn, 'base_billing_rate', flt(row.billing_rate) * exchange_rate);
	dontmanage.model.set_value(cdt, cdn, 'base_costing_rate', flt(row.costing_rate) * exchange_rate);
	...
	dontmanage.model.set_value(cdt, cdn, 'base_billing_amount', base_billing_amount);
	dontmanage.model.set_value(cdt, cdn, 'base_costing_amount', flt(row.base_costing_rate) * flt(row.hours));
	dontmanage.model.set_value(cdt, cdn, 'billing_amount', billing_amount);
	dontmanage.model.set_value(cdt, cdn, 'costing_amount', flt(row.costing_rate) * flt(row.hours));
};
...
const set_employee_and_company = function(frm) {
	const options = { user_id: dontmanage.session.user };
	const fields = ['name', 'company'];
	dontmanage.db.get_value('Employee', options, fields).then(({ message }) => {
		if (message) {
...
function set_project_in_timelog(frm) {
	if(frm.doc.parent_project) {
		$.each(frm.doc.time_logs || [], function(i, item) {
			dontmanage.model.set_value(item.doctype, item.name, "project", frm.doc.parent_project);
		});
	}
}
