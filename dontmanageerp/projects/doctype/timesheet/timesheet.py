...
from dontmanage.utils import add_to_date, flt, get_datetime, getdate, time_diff_in_hours
...
class OverWorkLoggedError(dontmanage.ValidationError):
	pass
...
class Timesheet(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.projects.doctype.timesheet_detail.timesheet_detail import TimesheetDetail
	...
	def validate_mandatory_fields(self):
		for data in self.time_logs:
			if not data.from_time and not data.to_time:
				dontmanage.throw(_("Row {0}: From Time and To Time is mandatory.").format(data.idx))

			if not data.activity_type and self.employee:
				dontmanage.throw(_("Row {0}: Activity Type is mandatory.").format(data.idx))

			if flt(data.hours) == 0.0:
				dontmanage.throw(_("Row {0}: Hours value must be greater than zero.").format(data.idx))
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = dontmanage.get_doc("Task", data.task)
		...
		for project in projects:
			project_doc = dontmanage.get_doc("Project", project)
	...
	def validate_overlap(self, data):
		settings = dontmanage.get_single("Projects Settings")
		self.validate_overlap_for("user", data, self.user, settings.ignore_user_time_overlap)
		self.validate_overlap_for("employee", data, self.employee, settings.ignore_employee_time_overlap)

	def set_project(self, data):
		data.project = data.project or dontmanage.db.get_value("Task", data.task, "project")
	...
	def get_overlap_for(self, fieldname, args, value):
		...
		timelog = dontmanage.qb.DocType("Timesheet Detail")
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	return dontmanage.db.sql(query, filters, as_dict=1)


@dontmanage.whitelist()
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = dontmanage.db.sql(
...
def make_sales_invoice(source_name, item_code=None, customer=None, currency=None):
	target = dontmanage.new_doc("Sales Invoice")
	timesheet = dontmanage.get_doc("Timesheet", source_name)

	if not timesheet.total_billable_hours:
		dontmanage.throw(_("Invoice can't be made for zero billing hour"))

	if timesheet.total_billable_hours == timesheet.total_billed_hours:
		dontmanage.throw(_("Invoice already created for all billing hours"))
	...
	if customer:
		...
		default_price_list = dontmanage.get_value("Customer", customer, "default_price_list")
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	contact = dontmanage.db.exists("Contact", {"user": user})
	if contact:
		# find customer
	...
	if customer:
		sales_invoices = [
			d.name for d in dontmanage.get_all("Sales Invoice", filters={"customer": customer})
		] or [None]
		projects = [d.name for d in dontmanage.get_all("Project", filters={"customer": customer})]
		# Return timesheet related data to web portal.
		timesheets = dontmanage.db.sql(
			f"""
			SELECT
