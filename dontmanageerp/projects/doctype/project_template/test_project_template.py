...
def make_project_template(project_template_name, project_tasks=[]):
	if not dontmanage.db.exists("Project Template", project_template_name):
		...
		doc = dontmanage.get_doc(dict(doctype="Project Template", name=project_template_name))
		for task in project_tasks:
			doc.append("tasks", {"task": task.name})
		doc.insert()

	return dontmanage.get_doc("Project Template", project_template_name)
