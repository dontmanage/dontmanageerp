...
class Project(Document):
	...
	def copy_from_template(self):  # nosemgrep
		...
		"""
		if self.project_template and not dontmanage.db.get_all("Task", dict(project=self.name), limit=1):
			# has a template, and no loaded tasks, so lets create
			if not self.expected_start_date:
				# project starts today
				self.expected_start_date = today()

			template = dontmanage.get_doc("Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = dontmanage.get_doc("Task", task.task)
	...
	def after_insert(self):
		self.copy_from_template()  # nosemgrep
		if self.sales_order:
			dontmanage.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def on_trash(self):
		dontmanage.db.set_value("Sales Order", {"project": self.name}, "project", "")

	def update_percent_complete(self):
		if self.status == "Completed":
			if (
				len(dontmanage.get_all("Task", dict(project=self.name))) == 0
		...
		total = dontmanage.db.count("Task", dict(project=self.name))
		...
		if not total:
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				weight_sum = dontmanage.db.sql(
					"""select sum(task_weight) from tabTask where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = dontmanage.db.sql(
				...
				for row in weighted_progress:
					pct_complete += row["progress"] * dontmanage.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_sales_amount(self):
		total_sales_amount = dontmanage.db.sql(
	...
	def update_billed_amount(self):
		# nosemgrep
		total_billed_amount = dontmanage.db.sql(
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			dontmanage.db.set_value("Project", new_name, "copied_from", new_name)
...
def get_cost_center_name(project):
	return dontmanage.db.get_value("Project", project, "cost_center")
...
def create_duplicate_project(prev_doc, project_name):
	...
	if project_name == prev_doc.get("name"):
		dontmanage.throw(_("Use a name that is different from previous project name"))

	# change the copied doc name to new project name
	project = dontmanage.copy_doc(prev_doc)
	...
	task_list = dontmanage.get_all("Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = dontmanage.get_doc("Task", task)
		new_task = dontmanage.copy_doc(task)
...
def send_project_update_email_to_users(project):
	doc = dontmanage.get_doc("Project", project)

	if is_holiday(doc.holiday_list) or not doc.users:
		return

	project_update = dontmanage.get_doc(
		{
...
def collect_project_status():
	for data in dontmanage.get_all("Project Update", {"date": today(), "sent": 0}):
		...
		for d in replies:
			doc = dontmanage.get_doc("Project Update", data.name)
			user_data = dontmanage.db.get_values(
			...
			doc.append(
				"users",
				{
					...
					"project_status": dontmanage.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in dontmanage.get_all("Project Update", {"date": yesterday, "sent": 0}):
		doc = dontmanage.get_doc("Project Update", d.name)

		project_doc = dontmanage.get_doc("Project", doc.project)
...
def update_project_sales_billing():
	sales_update_frequency = dontmanage.db.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif sales_update_frequency == "Monthly" and dontmanage.utils.now_datetime().day != 1:
		return

	# Else simply fallback to Daily
	for project in dontmanage.get_all("Project", filters={"status": ["!=", "Cancelled"]}):
		dontmanage.get_doc("Project", project.name).save()


@dontmanage.whitelist()
def create_kanban_board_if_not_exists(project):
	from dontmanage.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	project = dontmanage.get_doc("Project", project)
	if not dontmanage.db.exists("Kanban Board", project.project_name):
...
def set_project_status(project, status):
	...
	if status not in ("Completed", "Cancelled"):
		dontmanage.throw(_("Status must be Cancelled or Completed"))

	project = dontmanage.get_doc("Project", project)
	dontmanage.has_permission(doc=project, throw=True)

	for task in dontmanage.get_all("Task", dict(project=project.name)):
		dontmanage.db.set_value("Task", task.name, "status", status)
...
def get_holiday_list(company=None):
	if not company:
		company = get_default_company() or dontmanage.get_all("Company")[0].name

	holiday_list = dontmanage.get_cached_value("Company", company, "default_holiday_list")
	if not holiday_list:
		dontmanage.throw(
			_("Please set a default Holiday List for Company {0}").format(dontmanage.bold(get_default_company()))
		)
	return holiday_list
