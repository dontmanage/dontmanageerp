...
from dontmanageerp.projects.doctype.project_template.test_project_template import make_project_template
from dontmanageerp.projects.doctype.task.test_task import create_task
from dontmanageerp.selling.doctype.sales_order.sales_order import make_project as make_project_from_so
from dontmanageerp.selling.doctype.sales_order.test_sales_order import make_sales_order

test_records = dontmanage.get_test_records("Project")
test_ignore = ["Sales Order"]


class TestProject(DontManageTestCase):
	def test_project_with_template_having_no_parent_and_depend_tasks(self):
		project_name = "Test Project with Template - No Parent and Dependend Tasks"
		dontmanage.db.sql(""" delete from tabTask where project = %s """, project_name)
		dontmanage.delete_doc("Project", project_name)
	...
	def test_project_template_having_parent_child_tasks(self):
		...
		if dontmanage.db.get_value("Project", {"project_name": project_name}, "name"):
			project_name = dontmanage.db.get_value("Project", {"project_name": project_name}, "name")
	...
	def test_project_with_template_tasks_having_common_name(self):
		...
		project_tasks = dontmanage.get_all(
...
def make_project(args):
	...
	if args.project_name and dontmanage.db.exists("Project", {"project_name": args.project_name}):
		return dontmanage.get_doc("Project", {"project_name": args.project_name})
...
def task_exists(subject):
	result = dontmanage.db.get_list("Task", filters={"subject": subject}, fields=["name"])
	if not len(result):
		return False
	return dontmanage.get_doc("Task", result[0].name)
