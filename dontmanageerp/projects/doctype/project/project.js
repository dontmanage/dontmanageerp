...
dontmanage.ui.form.on("Project", {
			...
			if (dontmanage.model.can_read("Task")) {
				...
				frm.add_custom_button(
					...
					() => {
						dontmanage
							.call(
								"dontmanageerp.projects.doctype.project.project.create_kanban_board_if_not_exists",
								{
									project: frm.doc.name,
								}
							)
	...
	create_duplicate: function (frm) {
		return new Promise((resolve) => {
			dontmanage.prompt("Project Name", (data) => {
				dontmanage
					.xcall("dontmanageerp.projects.doctype.project.project.create_duplicate_project", {
						prev_doc: frm.doc,
	...
	set_status: function (frm, status) {
		dontmanage.confirm(__("Set Project and all Tasks to status {0}?", [status.bold()]), () => {
			dontmanage
				.xcall("dontmanageerp.projects.doctype.project.project.set_project_status", {
					project: frm.doc.name,
					status: status,
				})
				.then(() => {
