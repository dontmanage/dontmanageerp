...
def email_sending(project_name, frequency, date_start, date_end, progress, number_of_drafts, update):
	holiday = dontmanage.db.sql("""SELECT holiday_date FROM `tabHoliday` where holiday_date = CURRENT_DATE;""")
	msg = (
		"<p>Project Name: "
		+ project_name
		+ "</p><p>Frequency: "
	...
	msg += "</table>"
	if len(holiday) == 0:
		email = dontmanage.db.sql("""SELECT user from `tabProject User` WHERE parent = %s;""", project_name)
		for emails in email:
			dontmanage.sendmail(recipients=emails, subject=dontmanage._(project_name + " " + "Summary"), message=msg)
	else:
		pass
