...
def get_context(context):
	is_enabled = dontmanage.db.get_single_value("Appointment Booking Settings", "enable_scheduling")
	if is_enabled:
		return context
	else:
...
def get_appointment_slots(date, timezone):
	...
	# Database queries
	settings = dontmanage.get_doc("Appointment Booking Settings")
	holiday_list = dontmanage.get_doc("Holiday List", settings.holiday_list)
...
def create_appointment(date, time, tz, contact):
	...
	appointment = dontmanage.new_doc("Appointment")
...
def convert_to_guest_timezone(guest_tz, datetimeobject):
	...
	local_timezone = pytz.timezone(dontmanage.utils.get_time_zone())
...
def convert_to_system_timezone(guest_tz, datetimeobject):
	...
	system_tz = pytz.timezone(dontmanage.utils.get_time_zone())
	datetimeobject = datetimeobject.astimezone(system_tz)
	return datetimeobject


def check_availabilty(timeslot, settings):
	return dontmanage.db.count("Appointment", {"scheduled_time": timeslot}) < settings.number_of_agents
