...
async function get_global_variables() {
    ...
    window.appointment_settings = (await dontmanage.call({
        method: 'dontmanageerp.www.book_appointment.index.get_appointment_settings'
    })).message;
    window.timezones = (await dontmanage.call({
        method:'dontmanageerp.www.book_appointment.index.get_timezones'
    })).message;
}
...
function hide_next_button() {
    ...
    next_button.onclick = () => dontmanage.msgprint(__("Please select a date and time"));
}
...
function on_date_or_timezone_select() {
    ...
    if (date_picker.value === '') {
        clear_time_slots();
        hide_next_button();
        dontmanage.throw(__('Please select a date'));
    }
...
async function get_time_slots(date, timezone) {
    let slots = (await dontmanage.call({
        method: 'dontmanageerp.www.book_appointment.index.get_appointment_slots',
        args: {
            date: date,
            timezone: timezone
        }
...
async function submit() {
    ...
    let appointment =  dontmanage.call({
        method: 'dontmanageerp.www.book_appointment.index.create_appointment',
        args: {
        ...
        callback: (response)=>{
            if (response.message.status == "Unverified") {
                dontmanage.show_alert(__("Please check your email to confirm the appointment"))
            } else {
                dontmanage.show_alert(__("Appointment Created Successfully"));
            }
            setTimeout(()=>{
                let redirect_url = "/";
        ...
        error: (err)=>{
            dontmanage.show_alert(__("Something went wrong please try again"));
            button.disabled = false;
        }
    });
}
