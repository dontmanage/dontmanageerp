    ...
    let appointment =  dontmanage.call({
        ...
        callback: (response)=>{
            if (response.message.status == "Unverified") {
                dontmanage.show_alert("Please check your email to confirm the appointment")
            } else {
                dontmanage.show_alert("Appointment Created Successfully");
            }
            setTimeout(()=>{
                let redirect_url = "/";
        ...
        error: (err)=>{
            dontmanage.show_alert("Something went wrong please try again");
            button.disabled = false;
        }
    });
}
