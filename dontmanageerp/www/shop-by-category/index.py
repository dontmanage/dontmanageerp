...
def get_context(context):
	...
	settings = dontmanage.get_cached_doc("E Commerce Settings")
...
def get_slideshow(slideshow):
	...
	slideshow = dontmanage.get_cached_doc("Website Slideshow", slideshow)
...
def get_tabs(categories):
	...
	for index, tab in enumerate(categorical_data, start=1):
		tab_values[f"tab_{index + 1}_title"] = dontmanage.unscrub(tab)
		# pre-render cards for each tab
		tab_values[f"tab_{index + 1}_content"] = dontmanage.render_template(
			"dontmanageerp/www/shop-by-category/category_card_section.html",
...
def get_category_records(categories: list):
	...
	for c in categories:
		if c == "item_group":
			categorical_data["item_group"] = dontmanage.db.get_all(
				"Item Group",
		...
		try:
			...
			data = dontmanage.db.get_all(doctype, fields=fields)
			categorical_data[c] = data
		except BaseException:
			dontmanage.throw(_("DocType {} not found").format(doctype))
			continue
