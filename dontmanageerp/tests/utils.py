...
import dontmanage
from dontmanage.core.doctype.report.report import get_report_module_dotted_path
...
def create_test_contact_and_address():
	dontmanage.db.sql("delete from tabContact")
	dontmanage.db.sql("delete from `tabContact Email`")
	dontmanage.db.sql("delete from `tabContact Phone`")
	dontmanage.db.sql("delete from tabAddress")
	dontmanage.db.sql("delete from `tabDynamic Link`")
	...
	contact_two = dontmanage.get_doc(
		{
			"doctype": "Contact",
...
def execute_script_report(
	...
	test_filters = []
	report_execute_fn = dontmanage.get_attr(get_report_module_dotted_path(module, report_name) + ".execute")
	report_filters = dontmanage._dict(default_filters).copy().update(filters)
...
def if_lending_app_not_installed(function):
	...
	def wrapper(*args, **kwargs):
		if "lending" not in dontmanage.get_installed_apps():
			return function(*args, **kwargs)
		return
