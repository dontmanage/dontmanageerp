import dontmanage
from dontmanage.contacts.doctype.contact.test_contact import create_contact
...
class TestExotel(DontManageAPITestCase):
	@classmethod
	def setUpClass(cls):
		cls.CURRENT_DB_CONNECTION = dontmanage.db
		cls.test_employee_name = make_employee(
			user="test_employee_exotel@company.com", cell_number="9999999999"
		)
		dontmanage.db.set_value("Exotel Settings", "Exotel Settings", "enabled", 1)
	...
	def test_for_successful_call(self):
		...
		call_log = dontmanage.get_doc("Call Log", call_initiation_data.CallSid)
	...
	def test_for_disconnected_call(self):
		...
		call_log = dontmanage.get_doc("Call Log", call_disconnected_data.CallSid)
	...
	def test_for_call_not_answered(self):
		...
		call_log = dontmanage.get_doc("Call Log", call_not_answered_data.CallSid)
	...
	def emulate_api_call_from_exotel(self, api_method, data):
		self.post(
			f"/api/method/dontmanageerp.dontmanageerp_integrations.exotel_integration.{api_method}",
			data=dontmanage.as_json(data),
	...
	def tearDownClass(cls):
		dontmanage.db = cls.CURRENT_DB_CONNECTION
