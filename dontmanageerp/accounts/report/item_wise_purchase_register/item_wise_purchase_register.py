...
def get_items(filters, additional_table_columns):
	doctype = "Purchase Invoice"
	pi = dontmanage.qb.DocType(doctype)
	pii = dontmanage.qb.DocType(f"{doctype} Item")
...
def get_aii_accounts():
	return dict(dontmanage.db.sql("select name, stock_received_but_not_billed from tabCompany"))


def get_purchase_receipts_against_purchase_order(item_list):
	po_pr_map = dontmanage._dict()
	po_item_rows = list(set(d.po_detail for d in item_list))

	if po_item_rows:
		purchase_receipts = dontmanage.db.sql(
			"""
