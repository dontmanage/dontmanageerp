...
def execute(filters=None):
	if filters.get("based_on") == "Accounting Dimension" and not filters.get("accounting_dimension"):
		dontmanage.throw(_("Select Accounting Dimension."))
...
def get_accounts_data(based_on, company):
	if based_on == "Cost Center":
		...
		return dontmanage.get_all("Project", fields=["name"], filters={"company": company}, order_by="name")
	else:
		filters = {}
		doctype = dontmanage.unscrub(based_on)
		has_company = dontmanage.db.has_column(doctype, "company")

		if has_company:
			filters.update({"company": company})

		return dontmanage.get_all(doctype, fields=["name"], filters=filters, order_by="name")
