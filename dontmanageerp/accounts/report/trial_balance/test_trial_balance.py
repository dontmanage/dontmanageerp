...
from dontmanageerp.accounts.report.trial_balance.trial_balance import execute


class TestTrialBalance(DontManageTestCase):
	def setUp(self):
	...
	def test_offsetting_entries_for_accounting_dimensions(self):
		...
		dontmanage.db.sql("delete from `tabSales Invoice` where company='Trial Balance Company'")
		dontmanage.db.sql("delete from `tabGL Entry` where company='Trial Balance Company'")
...
def create_accounting_dimension(**args):
	...
	if dontmanage.db.exists("Accounting Dimension", document_type):
		accounting_dimension = dontmanage.get_doc("Accounting Dimension", document_type)
		accounting_dimension.disabled = 0
	else:
		accounting_dimension = dontmanage.new_doc("Accounting Dimension")
...
def disable_dimension(**args):
	args = dontmanage._dict(args)
	document_type = args.document_type or "Branch"
	dimension = dontmanage.get_doc("Accounting Dimension", document_type)
	dimension.disabled = 1
	dimension.save()


def clear_dimension_defaults(dimension_name):
	accounting_dimension = dontmanage.get_doc("Accounting Dimension", dimension_name)
