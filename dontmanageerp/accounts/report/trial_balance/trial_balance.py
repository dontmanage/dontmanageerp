...
from dontmanage.utils import add_days, cstr, flt, formatdate, getdate
...
def validate_filters(filters):
	if not filters.fiscal_year:
		dontmanage.throw(_("Fiscal Year {0} is required").format(filters.fiscal_year))

	fiscal_year = dontmanage.get_cached_value(
		"Fiscal Year", filters.fiscal_year, ["year_start_date", "year_end_date"], as_dict=True
	)
	if not fiscal_year:
		dontmanage.throw(_("Fiscal Year {0} does not exist").format(filters.fiscal_year))
	else:
...
def get_data(filters):
	...
	company_currency = filters.presentation_currency or dontmanageerp.get_company_currency(filters.company)
...
def get_opening_balance(
	...
	closing_balance = dontmanage.qb.DocType(doctype)
	account = dontmanage.qb.DocType("Account")

	opening_balance = (
		dontmanage.qb.from_(closing_balance)
		.select(
		...
		.where(
			...
			& (
				closing_balance.account.isin(
					dontmanage.qb.from_(account).select("name").where(account.report_type == report_type)
				)
			)
		)
	...
	if filters.cost_center:
		lft, rgt = dontmanage.db.get_value("Cost Center", filters.cost_center, ["lft", "rgt"])
		cost_center = dontmanage.qb.DocType("Cost Center")
		opening_balance = opening_balance.where(
			closing_balance.cost_center.isin(
				dontmanage.qb.from_(cost_center)
				.select("name")
