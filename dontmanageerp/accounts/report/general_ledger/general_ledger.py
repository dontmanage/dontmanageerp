...
from dontmanage.utils import cstr, getdate

from dontmanageerp import get_company_currency, get_default_company
from dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from dontmanageerp.accounts.report.financial_statements import get_cost_centers_with_children
from dontmanageerp.accounts.report.utils import convert_to_presentation_currency, get_currency
...
def execute(filters=None):
	...
	if filters and filters.get("print_in_account_currency") and not filters.get("account"):
		dontmanage.throw(_("Select an account to print in account currency"))

	for acc in dontmanage.db.sql("""select name, is_group from tabAccount""", as_dict=1):
		account_details.setdefault(acc.name, acc)

	if filters.get("party"):
		filters.party = dontmanage.parse_json(filters.get("party"))
...
def validate_filters(filters, account_details):
	...
	if not filters.get("from_date") and not filters.get("to_date"):
		dontmanage.throw(
			_("{0} and {1} are mandatory").format(dontmanage.bold(_("From Date")), dontmanage.bold(_("To Date")))
		)

	if filters.get("account"):
		filters.account = dontmanage.parse_json(filters.get("account"))
		for account in filters.account:
			if not account_details.get(account):
				dontmanage.throw(_("Account {0} does not exists").format(account))

	if filters.get("account") and filters.get("group_by") == "Group by Account":
		filters.account = dontmanage.parse_json(filters.get("account"))
		for account in filters.account:
			if account_details[account].is_group == 0:
				dontmanage.throw(_("Can not filter based on Child Account, if grouped by Account"))

	if filters.get("voucher_no") and filters.get("group_by") in ["Group by Voucher"]:
		dontmanage.throw(_("Can not filter based on Voucher No, if grouped by Voucher"))

	if filters.from_date > filters.to_date:
		dontmanage.throw(_("From Date must be before To Date"))

	if filters.get("project"):
		filters.project = dontmanage.parse_json(filters.get("project"))

	if filters.get("cost_center"):
		filters.cost_center = dontmanage.parse_json(filters.get("cost_center"))
...
def validate_party(filters):
	...
	if party and party_type:
		for d in party:
			if not dontmanage.db.exists(party_type, d):
				dontmanage.throw(_("Invalid {0}: {1}").format(party_type, d))


def set_account_currency(filters):
	if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
		filters["company_currency"] = dontmanage.get_cached_value("Company", filters.company, "default_currency")
		...
		if filters.get("account"):
			...
			gle_currency = dontmanage.db.get_value(
				"GL Entry",
			...
			if gle_currency:
				...
				account_currency = (
					...
					else dontmanage.get_cached_value(filters.party_type, filters.party[0], "default_currency")
				)
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters.get("show_remarks"):
		if remarks_length := dontmanage.db.get_single_value("Accounts Settings", "general_ledger_remarks_length"):
	...
	if filters.get("include_default_book_entries"):
		filters["company_fb"] = dontmanage.get_cached_value(
...
def get_conditions(filters):
	...
	if filters.get("ignore_err"):
		err_journals = dontmanage.db.get_all(
			"Journal Entry",
			filters={
	...
	if filters.get("ignore_cr_dr_notes"):
		system_generated_cr_dr_journals = dontmanage.db.get_all(
			"Journal Entry",
			filters={
				"company": filters.get("company"),
				"docstatus": 1,
		...
		for dimension in accounting_dimensions:
			...
			if not dimension.disabled and dimension.document_type != "Finance Book":
				if filters.get(dimension.fieldname):
					if dontmanage.get_cached_value("DocType", dimension.document_type, "is_tree"):
...
def get_accounts_with_children(accounts):
	...
	doctype = dontmanage.qb.DocType("Account")
	accounts_data = (
	...
	return dontmanage.qb.from_(doctype).select(doctype.name).where(Criterion.any(conditions)).run(pluck=True)
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, totals):
	...
	immutable_ledger = dontmanage.db.get_single_value("Accounts Settings", "enable_immutable_ledger")
...
def get_account_type_map(company):
	account_type_map = dontmanage._dict(
		dontmanage.get_all("Account", fields=["name", "account_type"], filters={"company": company}, as_list=1)
	)
