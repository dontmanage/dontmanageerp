...
def set_account_currency(filters):
	if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
		filters["company_currency"] = dontmanage.get_cached_value(
	...
	if filters.get("include_default_book_entries"):
		if filters.get("finance_book"):
			if filters.get("company_fb") and cstr(filters.get("finance_book")) != cstr(
				...
				dontmanage.throw(_("To use a different finance book, please uncheck 'Include Default FB Entries'"))
			else:
...
def get_accounts_with_children(accounts):
	...
	all_accounts = []
	for d in accounts:
		account = dontmanage.get_cached_doc("Account", d)
		if account:
			children = dontmanage.get_all(
				"Account", filters={"lft": [">=", account.lft], "rgt": ["<=", account.rgt]}
			)
			all_accounts += [c.name for c in children]
		else:
			dontmanage.throw(_("Account: {0} does not exist").format(d))
