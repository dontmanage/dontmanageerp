...
dontmanage.query_reports["General Ledger"] = {
	"filters": [
		{
			"fieldname":"company",
			"label": __("Company"),
		...
		},
		{
			...
			"fieldtype": "Data",
			on_change: function() {
				dontmanage.query_report.set_filter_value('group_by', "Group by Voucher (Consolidated)");
			}
		},
		{
		...
		},
		{
			...
			options: Object.keys(dontmanage.boot.party_account_types),
			on_change: function() {
		...
		},
		{
			...
			on_change: function() {
				...
				var parties = dontmanage.query_report.get_filter_value('party');

				if(!party_type || parties.length === 0 || parties.length > 1) {
					dontmanage.query_report.set_filter_value('party_name', "");
					dontmanage.query_report.set_filter_value('tax_id', "");
					return;
				} else {
					var party = parties[0];
					var fieldname = dontmanageerp.utils.get_party_name(party_type) || "name";
					dontmanage.db.get_value(party_type, party, fieldname, function(value) {
						dontmanage.query_report.set_filter_value('party_name', value[fieldname]);
					});

					if (party_type === "Customer" || party_type === "Supplier") {
						dontmanage.db.get_value(party_type, party, "tax_id", function(value) {
		...
		},
		{
			...
			"fieldtype": "MultiSelectList",
			get_data: function(txt) {
				return dontmanage.db.get_link_options('Cost Center', txt, {
					company: dontmanage.query_report.get_filter_value("company")
				});
			}
		},
...
dontmanageerp.utils.add_dimensions('General Ledger', 15)
