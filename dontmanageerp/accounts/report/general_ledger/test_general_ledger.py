...
from dontmanageerp.accounts.report.general_ledger.general_ledger import execute
from dontmanageerp.controllers.sales_and_purchase_return import make_return_doc


class TestGeneralLedger(DontManageTestCase):
	def setUp(self):
	...
	def test_foreign_account_balance_after_exchange_rate_revaluation(self):
		...
		revaluation = dontmanage.new_doc("Exchange Rate Revaluation")
	...
	def test_ignore_cr_dr_notes_filter(self):
		...
		system_generated_journal = dontmanage.db.get_all(
			"Journal Entry",
			filters={
				"docstatus": 1,
