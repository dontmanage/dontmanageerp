...
class PartyLedgerSummaryReport:
	...
	def get_additional_columns(self):
		...
		if self.filters.party_type == "Customer":
			self.territories = dontmanage._dict({})
			self.customer_group = dontmanage._dict({})

			customer = qb.DocType("Customer")
			result = (
				dontmanage.qb.from_(customer)
				.select(
			...
			self.supplier_group = dontmanage._dict({})
	...
	def get_data(self):
		company_currency = dontmanage.get_cached_value("Company", self.filters.get("company"), "default_currency")
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"

		self.party_data = dontmanage._dict({})
	...
	def get_gl_entries(self):
		...
		self.gl_entries = dontmanage.db.sql(
			f"""
			select
	...
	def prepare_conditions(self):
		...
		if self.filters.party_type == "Customer":
			if self.filters.get("customer_group"):
				lft, rgt = dontmanage.get_cached_value(
			...
			if self.filters.get("territory"):
				lft, rgt = dontmanage.db.get_value("Territory", self.filters.get("territory"), ["lft", "rgt"])
			...
			if self.filters.get("sales_person"):
				lft, rgt = dontmanage.db.get_value(
	...
	def get_party_adjustment_amounts(self):
		...
		income_or_expense_accounts = dontmanage.db.get_all(
			"Account", filters={"account_type": account_type, "company": self.filters.company}, pluck="name"
		)
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"
		round_off_account = dontmanage.get_cached_value("Company", self.filters.company, "round_off_account")
		...
		if not income_or_expense_accounts:
			...
			# ignoring dontmanage.db.escape as it replaces single quotes with double quotes
		...
		for voucher_gl_entries in adjustment_voucher_entries.values():
			...
			for gle in voucher_gl_entries:
				...
				elif dontmanage.get_cached_value("Account", gle.account, "account_type") == account_type:
