...
from dontmanageerp.accounts.report.deferred_revenue_and_expense.deferred_revenue_and_expense import (
...
class TestDeferredRevenueAndExpense(DontManageTestCase, AccountsTestMixin):
	@classmethod
	def setUpClass(self):
		self.maxDiff = None
	...
	def test_deferred_revenue(self):
		...
		# execute report
		fiscal_year = dontmanage.get_doc("Fiscal Year", get_fiscal_year(date="2021-05-01"))
	...
	def test_zero_amount(self):
		...
		pi = make_purchase_invoice(
			...
			is_return=False,
			update_stock=False,
			posting_date=dontmanage.utils.datetime.date(2021, 12, 30),
			parent_cost_center=self.cost_center,
			cost_center=self.cost_center,
			do_not_save=True,
			rate=3910,
		...
		# execute report
		fiscal_year = dontmanage.get_doc("Fiscal Year", get_fiscal_year(date="2022-01-31"))
		self.filters = dontmanage._dict(
			{
				"company": self.company,
				"filter_based_on": "Date Range",
