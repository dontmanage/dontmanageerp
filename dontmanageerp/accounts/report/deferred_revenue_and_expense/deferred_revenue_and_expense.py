...
class Deferred_Item:
	...
	def report_data(self):
		...
		"""
		ret_data = dontmanage._dict({"name": self.item_name})
	...
	def make_dummy_gle(self, name, date, amount):
		"""
		return - dontmanage._dict() of a dummy gle entry
		"""
	...
	def calculate_item_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			...
			self.period_total.append(dontmanage._dict({"key": period.key, "total": period_sum, "actual": actual}))
...
class Deferred_Invoice:
	def __init__(self, invoice, items, filters, period_list):
		...
		items - list : dontmanage._dict() with item details. Refer Deferred_Item for required fields
		"""
		self.name = invoice
	...
	def calculate_invoice_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			self.period_total.append(dontmanage._dict({"key": period.key, "total": 0, "actual": 0}))
	...
	def report_data(self):
		...
		"""
		ret_data = []
		inv_total = dontmanage._dict({"name": self.name})
...
class Deferred_Revenue_and_Expense_Report:
	...
	def generate_report_data(self):
		...
		# add total row
		if self.filters.type == "Revenue":
			total_row = dontmanage._dict({"name": "Total Deferred Income"})
		elif self.filters.type == "Expense":
			total_row = dontmanage._dict({"name": "Total Deferred Expense"})
