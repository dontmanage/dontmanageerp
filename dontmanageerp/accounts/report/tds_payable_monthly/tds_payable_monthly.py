...
def get_result(
	...
	for name, details in gle_map.items():
		...
		if tds_deducted:
			row = {
				"pan"
				if dontmanage.db.has_column("Supplier", "pan")
...
def get_supplier_pan_map():
	supplier_map = dontmanage._dict()
	suppliers = dontmanage.db.get_all(
...
def get_columns(filters):
	pan = "pan" if dontmanage.db.has_column("Supplier", "pan") else "tax_id"
	columns = [
		{"label": _(dontmanage.unscrub(pan)), "fieldname": pan, "fieldtype": "Data", "width": 90},
		{
			"label": _("Supplier"),
			"options": "Supplier",
			"fieldname": "supplier",
...
def get_tds_docs(filters):
	...
	invoice_net_total_map = dontmanage._dict()
	...
	tds_accounts = dontmanage.get_all(
...
def get_doc_info(vouchers, doctype, tax_category_map, invoice_net_total_map=None):
	...
	entries = dontmanage.get_all(doctype, filters={"name": ("in", vouchers)}, fields=fields)
