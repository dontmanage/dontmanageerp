...
def run_report(report_name, data):
	...
	report_config = json.loads(dontmanage.get_doc("Report", report_name).json)
		...
		for component_name, component in section.items():
			...
			if component["type"] == "section":
				if component_name == section_name:
					dontmanage.throw(_("A report component cannot refer to its parent section") + ": " + section_name)
				try:
...
def filter_match(value, string):
	...
	try:
		...
		return dontmanage.safe_eval(f"{value} {operator} {num}")
	except ValueError:
		if operator == "<":
			return True
		return False
...
def get_custom_reports(name=None):
	...
	if name:
		filters["name"] = name
	reports = dontmanage.get_list("Report", filters=filters, fields=["name", "json"], as_list=False)
...
def save_custom_report(reference_report, report_name, data):
	if reference_report != "Tax Detail":
		dontmanage.throw(_("The wrong report is referenced."))
	if report_name == "Tax Detail":
		dontmanage.throw(_("The parent report cannot be overwritten."))
	...
	try:
		newdoc = dontmanage.get_doc(doc)
		newdoc.insert()
		dontmanage.msgprint(_("Report created successfully"))
	except dontmanage.exceptions.DuplicateEntryError:
		dbdoc = dontmanage.get_doc("Report", report_name)
		dbdoc.update(doc)
		dbdoc.save()
