...
dontmanage.query_reports["Tax Detail"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			label: __("From Date"),
			fieldtype: "Date",
			default: dontmanage.datetime.month_start(dontmanage.datetime.get_today()),
			reqd: 1,
			width: "60px"
		},
		{
			...
			label: __("To Date"),
			fieldtype: "Date",
			default: dontmanage.datetime.month_end(dontmanage.datetime.get_today()),
			reqd: 1,
			width: "60px"
		},
		{
			...
			fieldtype: "Read Only",
			default: dontmanage.query_report.report_name,
			hidden: 1,
			reqd: 1
		},
		{
...
function hide_filters(report) {
	report.page.page_form[0].querySelectorAll('.form-group.dontmanage-control').forEach(function setHidden(field) {
...
dontmanageerp.TaxDetail = class TaxDetail {
	constructor() {
		this.patch();
		this.load_report();
	}
	...
	patch() {
		this.qr = dontmanage.query_report;
		this.super = {
	...
	refresh_report() {
		...
		// already run within dontmanage.run_serially
		this.loading = true;
		this.super.refresh_report.apply(this.qr);
		if (this.qr.report_name !== 'Tax Detail') {
			dontmanage.call({
				method: 'dontmanageerp.accounts.report.tax_detail.tax_detail.get_custom_reports',
	...
	save_report() {
		...
		if (this.qr.report_name !== 'Tax Detail') {
			dontmanage.call({
				method:'dontmanageerp.accounts.report.tax_detail.tax_detail.save_custom_report',
				args: {
	...
	check_datatable() {
		if (!this.qr.datatable) {
			dontmanage.throw(__('Please change the date range to load data first'));
		}
	}
	set_section(name) {
	...
	create_controls() {
		...
		controls['delete_section'] = this.qr.page.add_field({
			...
			click: () => {
				...
				if (cur_section) {
					dontmanage.confirm(__('Are you sure you want to delete section') + ' ' + cur_section + '?',
			...
			click: () => {
				...
				if (section_name) {
					...
					if (component_type === 'filter') {
						...
						dontmanage.throw(__('Please select the Component Type first'));
					}
				} else {
					dontmanage.throw(__('Please select the Section first'));
				}
			}
		});
		controls['delete_component'] = this.qr.page.add_field({
			...
			click: () => {
				...
				if (component) {
					dontmanage.confirm(__('Are you sure you want to delete component') + ' ' + component + '?',
...
if (!window.taxdetail) {
	window.taxdetail = new dontmanageerp.TaxDetail();
}
	...
	const dialog = new dontmanage.ui.Dialog({
		...
		primary_action: function new_report_pa(values) {
			dontmanage.call({
				...
				dontmanage.set_route('query-report', values.report_name);
			});
			dialog.hide();
		}
	});
