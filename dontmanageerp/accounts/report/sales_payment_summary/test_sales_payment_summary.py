...
from dontmanageerp.accounts.report.sales_payment_summary.sales_payment_summary import (
...
class TestSalesPaymentSummary(unittest.TestCase):
	...
	def setUpClass(self):
		create_records()
		pes = dontmanage.get_all("Payment Entry")
		jes = dontmanage.get_all("Journal Entry")
		sis = dontmanage.get_all("Sales Invoice")
		for pe in pes:
			dontmanage.db.set_value("Payment Entry", pe.name, "docstatus", 2)
		for je in jes:
			dontmanage.db.set_value("Journal Entry", je.name, "docstatus", 2)
		for si in sis:
			dontmanage.db.set_value("Sales Invoice", si.name, "docstatus", 2)
	...
	def test_get_mode_of_payments(self):
		...
		payment_entries = dontmanage.get_all(
			"Payment Entry",
			filters={"mode_of_payment": "Cash", "docstatus": 1},
			fields=["name", "docstatus"],
		)
		for payment_entry in payment_entries:
			pe = dontmanage.get_doc("Payment Entry", payment_entry.name)
			pe.cancel()
	...
	return dontmanage.get_doc(
		{
			...
			"customer": dontmanage.get_doc("Customer", {"customer_name": "Prestiga-Biz"}).name,
			...
			"items": [
				{
					...
					"item_code": dontmanage.get_doc("Item", {"item_name": "Consulting"}).name,
					"qty": qty,
					"rate": 10000,
...
def create_records():
	if dontmanage.db.exists("Customer", "Prestiga-Biz"):
		return
