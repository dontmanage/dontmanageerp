...
from dontmanage.utils import add_days, add_months, cint, cstr, flt, formatdate, get_first_day, getdate
...
def get_period_list(
	...
	for i in range(cint(math.ceil(months / months_to_add))):
		period = dontmanage._dict({"from_date": start_date})
...
def validate_fiscal_year(fiscal_year, from_fiscal_year, to_fiscal_year):
	if not fiscal_year.get("year_start_date") or not fiscal_year.get("year_end_date"):
		dontmanage.throw(_("Start Year and End Year are mandatory"))

	if getdate(fiscal_year.get("year_end_date")) < getdate(fiscal_year.get("year_start_date")):
		dontmanage.throw(_("End Year cannot be before Start Year"))


def validate_dates(from_date, to_date):
	if not from_date or not to_date:
		dontmanage.throw(_("From Date and To Date are mandatory"))

	if to_date < from_date:
		dontmanage.throw(_("To Date cannot be less than From Date"))
...
def set_gl_entries_by_account(
	...
	# For balance sheet
	ignore_closing_balances = dontmanage.db.get_single_value(
		"Accounts Settings", "ignore_account_closing_balance"
	)
	if not from_date and not ignore_closing_balances:
		last_period_closing_voucher = dontmanage.db.get_all(
...
def get_accounting_entries(
	...
	gl_entry = dontmanage.qb.DocType(doctype)
	query = (
	...
	return dontmanage.db.sql(query, params, as_dict=True)


def get_account_filter_query(root_lft, root_rgt, root_type, gl_entry):
	acc = dontmanage.qb.DocType("Account")
	exists_query = (
		dontmanage.qb.from_(acc).select(acc.name).where(acc.name == gl_entry.account).where(acc.is_group == 0)
	)
...
def apply_additional_conditions(doctype, query, from_date, ignore_closing_entries, filters):
	...
	if filters:
		...
		if filters.get("include_default_book_entries"):
			company_fb = dontmanage.get_cached_value("Company", filters.company, "default_finance_book")
...
def get_cost_centers_with_children(cost_centers):
	...
	for d in cost_centers:
		if dontmanage.db.exists("Cost Center", d):
			lft, rgt = dontmanage.db.get_value("Cost Center", d, ["lft", "rgt"])
			children = dontmanage.get_all("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_cost_centers += [c.name for c in children]
		else:
			dontmanage.throw(_("Cost Center: {0} does not exist").format(d))
