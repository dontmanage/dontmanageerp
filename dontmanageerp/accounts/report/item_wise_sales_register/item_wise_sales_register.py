...
def get_group_by_conditions(filters, doctype):
	if filters.get("group_by") == "Invoice":
		...
		return "ORDER BY `tab{0} Item`.{1}".format(doctype, dontmanage.scrub(filters.get("group_by")))
	elif filters.get("group_by") in ("Customer", "Customer Group", "Territory", "Supplier"):
		return "ORDER BY `tab{0}`.{1}".format(doctype, dontmanage.scrub(filters.get("group_by")))
...
def get_delivery_notes_against_sales_order(item_list):
	...
	if so_item_rows:
		delivery_notes = dontmanage.db.sql(
			"""
...
def get_tax_accounts(
	...
	tax_amount_precision = (
		get_field_precision(
			dontmanage.get_meta(tax_doctype).get_field("tax_amount"), currency=company_currency
		)
		or 2
	)
