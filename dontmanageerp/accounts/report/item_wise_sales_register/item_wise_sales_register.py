...
from dontmanageerp.accounts.report.sales_register.sales_register import get_mode_of_payments
from dontmanageerp.accounts.report.utils import get_query_columns, get_values_for_columns
from dontmanageerp.selling.report.item_wise_sales_history.item_wise_sales_history import (
...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
	...
	company_currency = dontmanage.get_cached_value("Company", filters.get("company"), "default_currency")
	...
	if item_list:
		...
		for tax in tax_columns:
			scrubbed_tax_fields.update(
				{
					tax + " Rate": dontmanage.scrub(tax + " Rate"),
					tax + " Amount": dontmanage.scrub(tax + " Amount"),
				}
			)
...
def apply_conditions(query, si, sii, filters, additional_conditions=None):
	...
	if filters.get("mode_of_payment"):
		sales_invoice = dontmanage.db.get_all(
	...
	if filters.get("warehouse"):
		if dontmanage.db.get_value("Warehouse", filters.get("warehouse"), "is_group"):
			lft, rgt = dontmanage.db.get_all(
				"Warehouse", filters={"name": filters.get("warehouse")}, fields=["lft", "rgt"], as_list=True
			)[0]
			warehouses = dontmanage.db.get_all("Warehouse", {"lft": (">", lft), "rgt": ("<", rgt)}, pluck="name")
...
def apply_group_by_conditions(query, si, ii, filters):
	if filters.get("group_by") == "Invoice":
		...
		query = query.orderby(si[dontmanage.scrub(filters.get("group_by"))])
...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	si = dontmanage.qb.DocType(doctype)
	sii = dontmanage.qb.DocType(f"{doctype} Item")
	item = dontmanage.qb.DocType("Item")

	query = (
		dontmanage.qb.from_(si)
		.join(sii)
	...
	if additional_query_columns:
		for column in additional_query_columns:
			if column.get("_doctype"):
				table = dontmanage.qb.DocType(column.get("_doctype"))
...
def get_tax_accounts(
	...
	tax_amount_precision = (
		get_field_precision(dontmanage.get_meta(tax_doctype).get_field("tax_amount"), currency=company_currency)
		or 2
	)
	...
	account_doctype = dontmanage.qb.DocType("Account")

	query = (
		dontmanage.qb.from_(account_doctype)
		...
		if item_wise_tax_detail:
			try:
				...
				for item_code, tax_data in item_wise_tax_detail.items():
					itemised_tax.setdefault(item_code, dontmanage._dict())
					...
					for d in item_row_map.get(parent, {}).get(item_code, []):
						...
						if item_tax_amount:
							...
							itemised_tax.setdefault(d.name, {})[description] = dontmanage._dict(
								{
									"tax_rate": tax_rate,
	...
	for desc in tax_columns:
		columns.append(
			{
				"label": _(desc + " Rate"),
				"fieldname": dontmanage.scrub(desc + " Rate"),
				"fieldtype": "Float",
				"width": 100,
			}
		)
		...
		columns.append(
			{
				"label": _(desc + " Amount"),
				"fieldname": dontmanage.scrub(desc + " Amount"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 100,
			}
...
def get_display_value(filters, group_by_field, item):
	if filters.get("group_by") == "Item":
		...
		party = dontmanage.scrub(filters.get("group_by"))
...
def get_group_by_and_display_fields(filters):
	if filters.get("group_by") == "Item":
		...
		group_by_field = dontmanage.scrub(filters.get("group_by"))
		subtotal_display_field = "item_code"
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
	...
	for tax in tax_columns:
		total_row.setdefault(dontmanage.scrub(tax + " Amount"), 0.0)
		total_row[dontmanage.scrub(tax + " Amount")] += flt(item[dontmanage.scrub(tax + " Amount")])
