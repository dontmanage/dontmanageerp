...
def validate_filters(filters):
	...
	if filters.get("pos_profile") and filters.get("group_by") == _("POS Profile"):
		dontmanage.throw(_("Can not filter based on POS Profile, if grouped by POS Profile"))

	if filters.get("customer") and filters.get("group_by") == _("Customer"):
		dontmanage.throw(_("Can not filter based on Customer, if grouped by Customer"))

	if filters.get("owner") and filters.get("group_by") == _("Cashier"):
		dontmanage.throw(_("Can not filter based on Cashier, if grouped by Cashier"))

	if filters.get("mode_of_payment") and filters.get("group_by") == _("Payment Method"):
		dontmanage.throw(_("Can not filter based on Payment Method, if grouped by Payment Method"))
