...
from dontmanageerp.accounts.report.utils import (
...
def _execute(filters, additional_table_columns=None):
	...
	if filters.get("include_payments") and not filters.get("customer"):
		dontmanage.throw(_("Please select a customer for fetching payments."))
	...
	for inv in invoice_list:
		...
		for income_acc in income_accounts:
			...
			row.update({dontmanage.scrub(income_acc): income_amount})

		# Add amount in unrealized account
		for account in unrealized_profit_loss_accounts:
			row.update(
				{dontmanage.scrub(account + "_unrealized"): flt(internal_invoice_map.get((inv.name, account)))}
			)
		...
		for tax_acc in tax_accounts:
			if tax_acc not in income_accounts:
				tax_amount_precision = (
					get_field_precision(
						dontmanage.get_meta("Sales Taxes and Charges").get_field("tax_amount"), currency=company_currency
					)
					or 2
				)
				tax_amount = flt(invoice_tax_map.get(inv.name, {}).get(tax_acc), tax_amount_precision)
				total_tax += tax_amount
				row.update({dontmanage.scrub(tax_acc): tax_amount})
...
def get_account_columns(invoice_list, include_payments):
	...
	if invoice_list:
		income_accounts = dontmanage.db.sql_list(
		...
		unrealized_profit_loss_accounts = dontmanage.db.sql_list(
	...
	for account in income_accounts:
		income_columns.append(
			{
				"label": account,
				"fieldname": dontmanage.scrub(account),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
	...
	for account in unrealized_profit_loss_accounts:
		unrealized_profit_loss_account_columns.append(
			{
				"label": account,
				"fieldname": dontmanage.scrub(account + "_unrealized"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
...
def get_conditions(filters, query, doctype):
	...
	if filters.get("mode_of_payment"):
		payment_doc = dontmanage.qb.DocType("Sales Invoice Payment")
...
def get_invoice_income_map(invoice_list):
	income_details = dontmanage.db.sql(
	...
	for d in income_details:
		invoice_income_map.setdefault(d.parent, dontmanage._dict()).setdefault(d.income_account, [])
...
def get_internal_invoice_map(invoice_list):
	unrealized_amount_details = dontmanage.db.sql(
...
def get_invoice_tax_map(invoice_list, invoice_income_map, income_accounts, include_payments=False):
	...
	for d in tax_details:
		if d.account_head in income_accounts:
			...
			invoice_tax_map.setdefault(d.parent, dontmanage._dict()).setdefault(d.account_head, [])
...
def get_invoice_so_dn_map(invoice_list):
	...
	for d in si_items:
		if d.sales_order:
			invoice_so_dn_map.setdefault(d.parent, dontmanage._dict()).setdefault("sales_order", []).append(
				d.sales_order
			)
		...
		if d.delivery_note:
			...
			delivery_note_list = dontmanage.db.sql_list(
		...
		if delivery_note_list:
			invoice_so_dn_map.setdefault(d.parent, dontmanage._dict()).setdefault(
...
def get_invoice_cc_wh_map(invoice_list):
	...
	for d in si_items:
		if d.cost_center:
			invoice_cc_wh_map.setdefault(d.parent, dontmanage._dict()).setdefault("cost_center", []).append(
				d.cost_center
			)

		if d.warehouse:
			invoice_cc_wh_map.setdefault(d.parent, dontmanage._dict()).setdefault("warehouse", []).append(
				d.warehouse
			)
