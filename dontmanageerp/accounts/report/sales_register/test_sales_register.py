...
from dontmanageerp.accounts.report.sales_register.sales_register import execute
from dontmanageerp.accounts.test.accounts_mixin import AccountsTestMixin


class TestItemWiseSalesRegister(AccountsTestMixin, DontManageTestCase):
	def setUp(self):
		self.create_company()
	...
	def create_child_cost_center(self):
		cc_name = "South Wing"
		if dontmanage.db.exists("Cost Center", cc_name):
			cc = dontmanage.get_doc("Cost Center", cc_name)
		else:
			parent = dontmanage.db.get_value("Cost Center", self.cost_center, "parent_cost_center")
	...
	def test_basic_report_output(self):
		...
		filters = dontmanage._dict({"from_date": today(), "to_date": today(), "company": self.company})
