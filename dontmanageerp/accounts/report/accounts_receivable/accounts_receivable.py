...
import dontmanage
from dontmanage import _, qb, query_builder, scrub
from dontmanage.query_builder import Criterion
from dontmanage.query_builder.functions import Date, Substring, Sum
from dontmanage.utils import cint, cstr, flt, getdate, nowdate

from dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from dontmanageerp.accounts.utils import get_currency_precision, get_party_types_from_account_type
...
class ReceivablePayableReport:
	...
	def run(self, args):
		...
		self.set_defaults()
		self.party_naming_by = dontmanage.db.get_value(args.get("naming_by")[0], None, args.get("naming_by")[1])
		self.get_columns()
		self.get_data()
		self.get_chart_data()
	...
	def set_defaults(self):
		if not self.filters.get("company"):
			self.filters.company = dontmanage.db.get_single_value("Global Defaults", "default_company")
		self.company_currency = dontmanage.get_cached_value(
	...
	def build_delivery_note_map(self):
		if self.invoices and self.filters.show_delivery_notes:
			self.delivery_notes = dontmanage._dict()

			# delivery note link inside sales invoice
			# nosemgrep
			si_against_dn = dontmanage.db.sql(
				"""
			...
			# nosemgrep
			dn_against_si = dontmanage.db.sql(
				"""
	...
	def get_invoice_details(self):
		self.invoice_details = dontmanage._dict()
		if self.account_type == "Receivable":
			# nosemgrep
			si_list = dontmanage.db.sql(
				"""
	...
	def get_payment_terms(self, row):
		...
		# nosemgrep
		payment_terms_details = dontmanage.db.sql(
			f"""
			select
		...
		original_row = dontmanage._dict(row)
		row.payment_terms = []
		...
		for d in payment_terms_details:
			term = dontmanage._dict(original_row)
		...
		for key in ("paid", "credit_note"):
			if row[key] > 0:
				if not additional_row:
					additional_row = dontmanage._dict(row)
	...
	def get_future_payments(self):
		if self.filters.show_future_payments:
			self.future_payments = dontmanage._dict()
	...
	def get_future_payments_from_payment_entry(self):
		...
		pe_ref = dontmanage.qb.DocType("Payment Entry Reference")
	...
	def get_return_entries(self):
		...
		self.return_entries = dontmanage._dict(
			dontmanage.get_all(
	...
	def get_ple_entries(self):
		...
		if self.filters.get("show_remarks"):
			if remarks_length := dontmanage.db.get_single_value(
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("sales_person"):
			lft, rgt = dontmanage.db.get_value("Sales Person", self.filters.get("sales_person"), ["lft", "rgt"])
			...
			self.sales_person_records = dontmanage._dict()
			for d in records:
	...
	def get_cost_center_conditions(self):
		lft, rgt = dontmanage.db.get_value("Cost Center", self.filters.cost_center, ["lft", "rgt"])
		cost_center_list = [
			center.name
			for center in dontmanage.get_list("Cost Center", filters={"lft": (">=", lft), "rgt": ("<=", rgt)})
		]
	...
	def get_hierarchical_filters(self, doctype, key):
		lft, rgt = dontmanage.db.get_value(doctype, self.filters.get(key), ["lft", "rgt"])
	...
	def get_party_details(self, party):
		if party not in self.party_details:
			if self.account_type == "Receivable":
				...
				self.party_details[party] = dontmanage.db.get_value(
					"Customer",
					party,
					fields,
					as_dict=True,
...
def get_customer_group_with_children(customer_groups):
	...
	for d in customer_groups:
		if dontmanage.db.exists("Customer Group", d):
			lft, rgt = dontmanage.db.get_value("Customer Group", d, ["lft", "rgt"])
			children = dontmanage.get_all("Customer Group", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_customer_groups += [c.name for c in children]
		else:
			dontmanage.throw(_("Customer Group: {0} does not exist").format(d))
