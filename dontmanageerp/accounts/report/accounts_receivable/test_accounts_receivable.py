...
from dontmanageerp.accounts.report.accounts_receivable.accounts_receivable import execute
from dontmanageerp.selling.doctype.sales_order.test_sales_order import make_sales_order


class TestAccountsReceivable(DontManageTestCase):
	def setUp(self):
		dontmanage.db.sql("delete from `tabSales Invoice` where company='_Test Company 2'")
		dontmanage.db.sql("delete from `tabSales Order` where company='_Test Company 2'")
		dontmanage.db.sql("delete from `tabPayment Entry` where company='_Test Company 2'")
		dontmanage.db.sql("delete from `tabGL Entry` where company='_Test Company 2'")
		dontmanage.db.sql("delete from `tabPayment Ledger Entry` where company='_Test Company 2'")
		dontmanage.db.sql("delete from `tabJournal Entry` where company='_Test Company 2'")
		dontmanage.db.sql("delete from `tabExchange Rate Revaluation` where company='_Test Company 2'")
	...
	def create_usd_account(self):
		name = "Debtors USD"
		exists = dontmanage.db.get_list(
			"Account", filters={"company": "_Test Company 2", "account_name": "Debtors USD"}
		)
		if exists:
			self.debtors_usd = exists[0].name
		else:
			debtors = dontmanage.get_doc(
				"Account",
			...
			debtors_usd = dontmanage.new_doc("Account")
	...
	def test_exchange_revaluation_for_party(self):
		...
		je = dontmanage.get_doc("Journal Entry", err_journals.get("revaluation_jv"))
		je = je.submit()
