...
class TestAccountsReceivable(AccountsTestMixin, DontManageTestCase):
	...
	def test_sales_person(self):
		sales_person = (
			dontmanage.get_doc({"doctype": "Sales Person", "sales_person_name": "John Clark", "enabled": True})
			.insert()
			.submit()
		)
	...
	def test_customer_group_filter(self):
		self.create_sales_invoice()
		cus_group = dontmanage.db.get_value("Customer", self.customer, "customer_group")
		filters = {
			"company": self.company,
			"report_date": today(),
			"range": "30, 60, 90, 120",
	...
	def test_report_output_if_party_is_missing(self):
		...
		if not dontmanage.db.get_value("Account", filters={"account_name": acc_name, "company": self.company}):
			additional_receivable_acc = dontmanage.get_doc(
				{
					"doctype": "Account",
