...
from dontmanage.utils import flt, formatdate

from dontmanageerp.controllers.trends import get_period_date_ranges, get_period_month_ranges
	...
	for year in fiscal_year:
		for from_date, to_date in get_period_date_ranges(filters["period"], year[0]):
			if filters["period"] == "Yearly":
				...
				for label in labels:
					columns.append(
						{"label": label, "fieldtype": "Float", "fieldname": dontmanage.scrub(label), "width": 150}
					)
			else:
				for label in [
...
def get_dimension_target_details(filters):
	budget_against = dontmanage.scrub(filters.get("budget_against"))
	cond = ""
	if filters.get("budget_against_filter"):
...
def get_actual_details(name, filters):
	...
	if filters.get("budget_against") == "Cost Center":
		cc_lft, cc_rgt = dontmanage.db.get_value("Cost Center", name, ["lft", "rgt"])
		cond = """
				and lft >= "{lft}"
				and rgt <= "{rgt}"
			""".format(
	...
	for ccd in dimension_target_details:
		...
		for month_id in range(1, 13):
			...
			cam_map.setdefault(ccd.budget_against, {}).setdefault(ccd.account, {}).setdefault(
				ccd.fiscal_year, {}
			).setdefault(month, dontmanage._dict({"target": 0.0, "actual": 0.0}))
