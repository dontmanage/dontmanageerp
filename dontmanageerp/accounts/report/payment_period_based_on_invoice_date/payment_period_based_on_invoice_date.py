...
def execute(filters=None):
	...
	data = []
	for d in entries:
		invoice = invoice_details.get(d.against_voucher) or dontmanage._dict()
...
def get_invoice_posting_date_map(filters):
	...
	for t in dontmanage.db.sql("select name, posting_date, due_date from `tab{0}`".format(dt), as_dict=1):
