...
from dontmanageerp.accounts.report.gross_profit.gross_profit import execute
...
class TestGrossProfit(DontManageTestCase):
	def setUp(self):
		self.create_company()
		self.create_item()
		self.create_bundle()
	...
	def test_invoice_without_only_delivery_note(self):
		...
		expected_entry_without_dn = {
			...
			"customer": self.customer,
			"posting_date": dontmanage.utils.datetime.date.fromisoformat(nowdate()),
			"item_code": self.item,
			"item_name": self.item,
	...
	def test_crnote_against_invoice_with_multiple_instances_of_same_item(self):
		...
		sinv.append("items", dontmanage.copy_doc(sinv.items[0], ignore_no_copy=False))
		sinv = sinv.save().submit()
