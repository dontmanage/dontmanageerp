...
import dontmanage
from dontmanage import _, qb, scrub
from dontmanage.query_builder import Order
from dontmanage.utils import cint, flt, formatdate
...
from dontmanageerp.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	filters.currency = dontmanage.get_cached_value("Company", filters.company, "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = dontmanage._dict(
		{
			"invoice": [
				"invoice_or_item",
				"customer",
...
class GrossProfitGenerator:
	def __init__(self, filters=None):
		...
		self.filters = dontmanage._dict(filters)
	...
	def process(self):
		...
		self.currency_precision = cint(dontmanage.db.get_default("currency_precision")) or 3
		self.float_precision = cint(dontmanage.db.get_default("float_precision")) or 2
		...
		for row in reversed(self.si_list):
			...
			product_bundles = []
			if row.update_stock:
				product_bundles = self.product_bundles.get(row.parenttype, {}).get(row.parent, dontmanage._dict())
			elif row.dn_detail:
				product_bundles = self.product_bundles.get("Delivery Note", {}).get(
					row.delivery_note, dontmanage._dict()
				)
	...
	def get_returned_invoice_items(self):
		returned_invoices = dontmanage.db.sql(
			"""
			select
		...
		self.returned_invoices = dontmanage._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, dontmanage._dict()).setdefault(
				inv.item_code, []
			).append(inv)
	...
	def get_buying_amount_from_so_dn(self, sales_order, so_detail, item_code):
		...
		delivery_note_item = dontmanage.qb.DocType("Delivery Note Item")

		query = (
			dontmanage.qb.from_(delivery_note_item)
	...
	def get_last_purchase_rate(self, item_code, row):
		...
		query = query.orderby(purchase_invoice.posting_date, order=dontmanage.qb.desc).limit(1)
	...
	def load_invoice_items(self):
		...
		if self.filters.get("cost_center"):
			self.filters.cost_center = dontmanage.parse_json(self.filters.get("cost_center"))
			self.filters.cost_center = get_cost_centers_with_children(self.filters.cost_center)
			conditions += " and `tabSales Invoice Item`.cost_center in %(cost_center)s"

		if self.filters.get("project"):
			self.filters.project = dontmanage.parse_json(self.filters.get("project"))
		...
		self.si_list = dontmanage.db.sql(
			"""
			select
	...
	def load_product_bundle(self):
		...
		for d in pki_query.run(as_dict=True):
			self.product_bundles.setdefault(d.parenttype, dontmanage._dict()).setdefault(
				d.parent, dontmanage._dict()
			).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = dontmanage.db.sql_list(
