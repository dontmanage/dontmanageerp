...
def get_data(filters):
	data = []
	depreciation_accounts = dontmanage.db.sql_list(
	...
	company_fb = dontmanage.get_cached_value("Company", filters.get("company"), "default_finance_book")
	...
	for d in gl_entries:
		...
		if asset_data:
			if not asset_data.get("accumulated_depreciation_amount"):
				...
				query = (
					dontmanage.qb.from_(DepreciationSchedule)
			...
			row = dontmanage._dict(asset_data)
			row.update(
				{
...
def get_assets_details(assets):
	...
	for d in dontmanage.get_all("Asset", fields=fields, filters={"name": ("in", assets)}):
