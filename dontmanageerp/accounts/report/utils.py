...
from dontmanage.utils import flt, formatdate, get_datetime_str, get_table_name
...
from dontmanageerp.accounts.doctype.fiscal_year.fiscal_year import get_from_and_to_date
...
def get_invoiced_item_gross_margin(
	...
	from dontmanageerp.accounts.report.gross_profit.gross_profit import GrossProfitGenerator

	sales_invoice = sales_invoice or dontmanage.form_dict.get("sales_invoice")
	item_code = item_code or dontmanage.form_dict.get("item_code")
	company = company or dontmanage.get_cached_value("Sales Invoice", sales_invoice, "company")
...
def get_party_details(party_type, party_list):
	party_details = {}
	party = dontmanage.qb.DocType(party_type)
	query = dontmanage.qb.from_(party).select(party.name, party.tax_id).where(party.name.isin(party_list))
...
def get_taxes_query(invoice_list, doctype, parenttype):
	taxes = dontmanage.qb.DocType(doctype)

	query = (
		dontmanage.qb.from_(taxes)
...
def get_journal_entries(filters, args):
	...
	journal_account = dontmanage.qb.DocType("Journal Entry Account")
	query = (
...
def apply_common_conditions(filters, query, doctype, child_doctype=None, payments=False):
	...
	if child_doctype:
		child_doc = dontmanage.qb.DocType(child_doctype)
...
def get_advance_taxes_and_charges(invoice_list):
	adv_taxes = dontmanage.qb.DocType("Advance Taxes and Charges")
	return (
		dontmanage.qb.from_(adv_taxes)
		.select(
			adv_taxes.parent,
