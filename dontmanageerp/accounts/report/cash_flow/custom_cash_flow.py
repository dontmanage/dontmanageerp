...
from dontmanage.utils import add_to_date, flt, get_date_str

from dontmanageerp.accounts.report.financial_statements import get_columns, get_data, get_period_list
...
def get_accounts_in_mappers(mapping_names):
	cfm = dontmanage.qb.DocType("Cash Flow Mapping")
	cfma = dontmanage.qb.DocType("Cash Flow Mapping Accounts")
	result = (
		dontmanage.qb.select(
			cfma.name,
			cfm.label,
...
def setup_mappers(mappers):
	...
	for mapping in mappers:
		...
		doc = dontmanage.get_doc("Cash Flow Mapper", mapping["name"])
...
def _get_account_type_based_data(
	...
	from dontmanageerp.accounts.report.cash_flow.cash_flow import get_start_date

	company = filters.company
	data = {}
	total = 0
	GLEntry = dontmanage.qb.DocType("GL Entry")
