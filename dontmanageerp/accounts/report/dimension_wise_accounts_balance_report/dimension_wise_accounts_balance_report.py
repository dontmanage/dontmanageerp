...
from dontmanageerp.accounts.report.trial_balance.trial_balance import validate_filters
...
def get_data(filters, dimension_list):
	company_currency = dontmanageerp.get_company_currency(filters.company)

	acc = dontmanage.db.sql(
		"""
		select
	...
	min_lft, max_rgt = dontmanage.db.sql(
		"""select min(lft), max(rgt) from `tabAccount`
		where company=%s""",
		(filters.company),
	)[0]

	account = dontmanage.db.sql_list(
	...
	format_gl_entries(
		gl_entries_by_account, accounts_by_name, dimension_list, dontmanage.scrub(filters.get("dimension"))
	)
...
def set_gl_entries_by_account(dimension_list, filters, account, gl_entries_by_account):
	...
	if account:
		condition += " and account in ({})".format(", ".join([dontmanage.db.escape(d) for d in account]))
	...
	if filters.get("include_default_book_entries"):
		gl_filters["company_fb"] = dontmanage.get_cached_value("Company", filters.company, "default_finance_book")
	...
	gl_entries = dontmanage.db.sql(
		...
		order by account, posting_date""".format(
			dimension=dontmanage.scrub(filters.get("dimension")), condition=condition
		),
		gl_filters,
		as_dict=True,
	)  # nosec
		...
		for entry in entries:
			...
			for dimension in dimension_list:
				if dimension == entry.get(dimension_type):
					d[dontmanage.scrub(dimension)] = (
						d.get(dontmanage.scrub(dimension), 0.0) + flt(entry.debit) - flt(entry.credit)
					)
...
def prepare_data(accounts, filters, company_currency, dimension_list):
	...
	for d in accounts:
		...
		for dimension in dimension_list:
			row[dontmanage.scrub(dimension)] = flt(d.get(dontmanage.scrub(dimension), 0.0), 3)

			if abs(row[dontmanage.scrub(dimension)]) >= 0.005:
				# ignore zero values
				has_value = True
				total += flt(d.get(dontmanage.scrub(dimension), 0.0), 3)
...
def accumulate_values_into_parents(accounts, accounts_by_name, dimension_list):
	...
	for d in reversed(accounts):
		if d.parent_account:
			for dimension in dimension_list:
				accounts_by_name[d.parent_account][dontmanage.scrub(dimension)] = accounts_by_name[
					d.parent_account
				].get(dontmanage.scrub(dimension), 0.0) + d.get(dontmanage.scrub(dimension), 0.0)


def get_condition(dimension):
	conditions = []

	conditions.append(f"{dontmanage.scrub(dimension)} in %(dimensions)s")

	return " and {}".format(" and ".join(conditions)) if conditions else ""


def get_dimensions(filters):
	meta = dontmanage.get_meta(filters.get("dimension"), cached=False)
	query_filters = {}

	if meta.has_field("company"):
		query_filters = {"company": filters.get("company")}

	return dontmanage.get_all(filters.get("dimension"), filters=query_filters, pluck="name")
...
def get_columns(dimension_list):
	...
	for dimension in dimension_list:
		columns.append(
			{
				"fieldname": dontmanage.scrub(dimension),
				"label": dimension,
				"fieldtype": "Currency",
				"options": "currency",
				"width": 150,
