...
dontmanage.query_reports["Supplier Ledger Summary"] = {
	"filters": [
		{
			"fieldname":"company",
			"label": __("Company"),
		...
		},
		{
			...
			on_change: () => {
				...
				if (party) {
					dontmanage.db.get_value('Supplier', party, ["tax_id", "supplier_name"], function(value) {
						dontmanage.query_report.set_filter_value('tax_id', value["tax_id"]);
						dontmanage.query_report.set_filter_value('supplier_name', value["supplier_name"]);
					});
				} else {
					dontmanage.query_report.set_filter_value('tax_id', "");
					dontmanage.query_report.set_filter_value('supplier_name', "");
				}
			}
		},
		{
