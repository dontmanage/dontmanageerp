...
from dontmanageerp.accounts.report.balance_sheet.balance_sheet import (
	get_chart_data,
...
from dontmanageerp.accounts.report.cash_flow.cash_flow import (
	add_total_row_account,
	get_account_type_based_gl_data,
	get_cash_flow_accounts,
)
from dontmanageerp.accounts.report.cash_flow.cash_flow import get_report_summary as get_cash_flow_summary
from dontmanageerp.accounts.report.financial_statements import (
	filter_out_zero_value_rows,
	get_fiscal_year_data,
	sort_accounts,
)
from dontmanageerp.accounts.report.profit_and_loss_statement.profit_and_loss_statement import (
...
from dontmanageerp.accounts.report.utils import convert, convert_to_presentation_currency
...
def get_company_currency(filters=None):
	return filters.get("presentation_currency") or dontmanage.get_cached_value(
			...
			if d:
				...
				for company in companies:
					...
					if (
						...
						parent_company_currency = dontmanageerp.get_company_currency(d.company)
						child_company_currency = dontmanageerp.get_company_currency(entry.company)
...
def get_subsidiary_companies(company):
	lft, rgt = dontmanage.get_cached_value("Company", company, ["lft", "rgt"])
...
def set_gl_entries_by_account(
	...
	company_lft, company_rgt = dontmanage.get_cached_value("Company", filters.get("company"), ["lft", "rgt"])

	companies = dontmanage.db.sql(
	...
	currency_info = dontmanage._dict(
...
def get_additional_conditions(from_date, ignore_closing_entries, filters, d):
	...
	if filters.get("include_default_book_entries"):
		if company_fb := dontmanage.get_cached_value("Company", d.name, "default_finance_book"):
