...
dontmanage.query_reports["Consolidated Financial Statement"] = {
	filters: [
		...
		},
		{
			...
			reqd: 1,
			on_change: () => {
				dontmanage.model.with_doc(
					"Fiscal Year",
					dontmanage.query_report.get_filter_value("from_fiscal_year"),
					function (r) {
						let year_start_date = dontmanage.model.get_value(
							"Fiscal Year",
							dontmanage.query_report.get_filter_value("from_fiscal_year"),
							"year_start_date"
						);
		...
		},
		{
			...
			on_change: () => {
				dontmanage.model.with_doc(
					"Fiscal Year",
					dontmanage.query_report.get_filter_value("to_fiscal_year"),
					function (r) {
						let year_end_date = dontmanage.model.get_value(
							"Fiscal Year",
							dontmanage.query_report.get_filter_value("to_fiscal_year"),
							"year_end_date"
						);
