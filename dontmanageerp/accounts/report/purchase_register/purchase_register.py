...
def _execute(filters=None, additional_table_columns=None):
	...
	if filters.get("include_payments") and not filters.get("supplier"):
		dontmanage.throw(_("Please select a supplier for fetching payments."))
	...
	company_currency = dontmanage.get_cached_value("Company", filters.company, "default_currency")
	...
	for inv in invoice_list:
		...
		for expense_acc in expense_accounts:
			...
			row.update({dontmanage.scrub(expense_acc): expense_amount})
...
def get_account_columns(invoice_list, include_payments):
	...
	if invoice_list:
		expense_accounts = dontmanage.db.sql_list(
...
def get_invoice_expense_map(invoice_list):
	expense_details = dontmanage.db.sql(
		"""
	...
	for d in expense_details:
		invoice_expense_map.setdefault(d.parent, dontmanage._dict()).setdefault(d.expense_account, [])
...
def get_invoice_po_pr_map(invoice_list):
	...
	for d in pi_items:
		if d.purchase_order:
			invoice_po_pr_map.setdefault(d.parent, dontmanage._dict()).setdefault("purchase_order", []).append(
				d.purchase_order
			)
		...
		if d.purchase_receipt:
			...
			pr_list = dontmanage.db.sql_list(
		...
		if pr_list:
			invoice_po_pr_map.setdefault(d.parent, dontmanage._dict()).setdefault("purchase_receipt", pr_list)

		if d.project:
			invoice_po_pr_map.setdefault(d.parent, dontmanage._dict()).setdefault("project", []).append(d.project)
