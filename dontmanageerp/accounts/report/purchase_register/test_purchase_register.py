...
from dontmanageerp.accounts.report.purchase_register.purchase_register import execute


class TestPurchaseRegister(DontManageTestCase):
	def test_purchase_register(self):
		dontmanage.db.sql("delete from `tabPurchase Invoice` where company='_Test Company 6'")
		dontmanage.db.sql("delete from `tabGL Entry` where company='_Test Company 6'")

		filters = dontmanage._dict(company="_Test Company 6", from_date=add_months(today(), -1), to_date=today())

		pi = make_purchase_invoice()

		report_results = execute(filters)
		first_row = dontmanage._dict(report_results[1][0])
	...
	def test_purchase_register_ledger_view(self):
		...
		report_results = execute(filters)
		first_row = dontmanage._dict(report_results[1][2])
