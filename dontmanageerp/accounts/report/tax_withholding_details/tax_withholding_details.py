...
def execute(filters=None):
	if filters.get("party_type") == "Customer":
		party_naming_by = dontmanage.db.get_single_value("Selling Settings", "cust_master_name")
	else:
		party_naming_by = dontmanage.db.get_single_value("Buying Settings", "supp_master_name")
	...
	for name, details in gle_map.items():
		for entry in details:
			...
			if tax_amount:
				...
				row = {
					"pan" if dontmanage.db.has_column(filters.party_type, "pan") else "tax_id": party_map.get(
						party, {}
					).get("pan"),
...
def get_party_pan_map(party_type):
	...
	if dontmanage.db.has_column(party_type, "pan"):
		fields.append("pan")

	party_details = dontmanage.db.get_all(party_type, fields=fields)
...
def get_columns(filters):
	pan = "pan" if dontmanage.db.has_column(filters.party_type, "pan") else "tax_id"
	columns = [
		{
		...
		{"label": _(dontmanage.unscrub(pan)), "fieldname": pan, "fieldtype": "Data", "width": 60},
	]
...
def get_tds_docs(filters):
	...
	payment_entries = []
	journal_entries = []
	tax_category_map = dontmanage._dict()
	net_total_map = dontmanage._dict()
	journal_entry_party_map = dontmanage._dict()
	bank_accounts = dontmanage.get_all("Account", {"is_group": 0, "account_type": "Bank"}, pluck="name")

	_tds_accounts = dontmanage.get_all(
...
def get_tds_docs_query(filters, bank_accounts, tds_accounts):
	if not tds_accounts:
		dontmanage.throw(
			_("No {0} Accounts found for this company.").format(dontmanage.bold(_("Tax Withholding"))),
	...
	if filters.get("party"):
		...
		party = dontmanage.get_all(filters.get("party_type"), pluck="name")
		jv_condition = gle.against.isin(party) | (
...
def get_tax_rate_map(filters):
	...
	return dontmanage._dict(rate_map)
