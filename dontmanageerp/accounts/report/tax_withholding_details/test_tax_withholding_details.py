...
from dontmanageerp.accounts.doctype.tax_withholding_category.test_tax_withholding_category import (
	create_tax_withholding_category,
)
from dontmanageerp.accounts.report.tax_withholding_details.tax_withholding_details import execute
...
class TestTaxWithholdingDetails(AccountsTestMixin, DontManageTestCase):
	def setUp(self):
		self.create_company()
	...
	def test_tax_withholding_for_customers(self):
		...
		dontmanage.db.set_value("Customer", "_Test Customer", "tax_withholding_category", "TCS")
	...
	def test_single_account_for_multiple_categories(self):
		...
		inv_2.submit()
		result = execute(
			dontmanage._dict(company="_Test Company", party_type="Supplier", from_date=today(), to_date=today())
		)[1]
		expected_values = [
	...
	def check_expected_values(self, result, expected_values):
		for i in range(len(result)):
			voucher = dontmanage._dict(result[i])
