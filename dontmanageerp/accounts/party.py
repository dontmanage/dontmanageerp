...
try:
	from dontmanage.contacts.doctype.address.address import render_address as _render_address
except ImportError:
	# Older dontmanage versions where this function is not available
	from dontmanage.contacts.doctype.address.address import get_address_display as _render_address
...
class DuplicatePartyAccountError(dontmanage.ValidationError):
	pass
...
def get_party_details(
	...
	if not dontmanage.db.exists(party_type, party):
		dontmanage.throw(_("{0}: {1} does not exists").format(party_type, party))
...
def _get_party_details(
	...
	party_details = dontmanage._dict(
	...
	if party_type == "Supplier" and party:
		party_details["supplier_tds"] = dontmanage.get_value(party_type, party.name, "tax_withholding_category")
...
def set_contact_details(party_details, party, party_type):
	...
	if not party_details.contact_person:
		...
		contact_details = dontmanage.db.get_value("Contact", party_details.contact_person, fields, as_dict=True)
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list and len(price_list) == 1:
		...
		customer_price_list = dontmanage.get_value("Customer", party.name, "default_price_list")

		if customer_price_list:
			price_list = customer_price_list
		else:
			pos_price_list = dontmanage.get_value("POS Profile", pos, "selling_price_list")
			price_list = pos_price_list or given_price_list
	else:
		price_list = get_default_price_list(party) or given_price_list

	if price_list:
		party_details.price_list_currency = dontmanage.db.get_value(
...
def get_party_account(party_type, party=None, company=None, include_advance=False):
	...
	if not party and party_type in ["Customer", "Supplier"]:
		...
		return dontmanage.get_cached_value("Company", company, default_account_name)
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		account = dontmanage.get_cached_value("Company", company, default_account_name)
...
def get_party_bank_account(party_type, party):
	return dontmanage.db.get_value("Bank Account", {"party_type": party_type, "party": party, "is_default": 1})
...
def get_party_gle_account(party_type, party, company):
	def generator():
		existing_gle_account = dontmanage.db.sql(
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		dontmanage.throw(
			_(
				...
				dontmanage.bold(party_type),
				dontmanage.bold(party),
				dontmanage.bold(existing_gle_currency),
				dontmanage.bold(company),
			),
...
def validate_party_accounts(doc):
	...
	for account in doc.get("accounts"):
		...
		if dontmanage.db.get_default("Company"):
			...
			company_default_currency = dontmanage.get_cached_value("Company", account.company, "default_currency")
	...
	if (bill_date or posting_date) and party:
		...
		if template_name:
			...
			if party_type == "Supplier":
				supplier_group = dontmanage.get_cached_value(party_type, party, "supplier_group")
				template_name = dontmanage.get_cached_value("Supplier Group", supplier_group, "payment_terms")
				if template_name:
...
def get_due_date_from_template(template_name, posting_date, bill_date):
	...
	template = dontmanage.get_doc("Payment Terms Template", template_name)
	...
	if getdate(due_date) < getdate(posting_date):
		...
		if default_due_date != posting_date and getdate(due_date) > getdate(default_due_date):
			is_credit_controller = (
				dontmanage.db.get_single_value("Accounts Settings", "credit_controller") in dontmanage.get_roles()
			)
...
def get_address_tax_category(tax_category=None, billing_address=None, shipping_address=None):
	addr_tax_category_from = dontmanage.db.get_single_value(
		"Accounts Settings", "determine_address_tax_category_from"
	)
	if addr_tax_category_from == "Shipping Address":
		if shipping_address:
			tax_category = dontmanage.db.get_value("Address", shipping_address, "tax_category") or tax_category
	else:
		if billing_address:
			tax_category = dontmanage.db.get_value("Address", billing_address, "tax_category") or tax_category
...
def set_taxes(
	...
	from dontmanageerp.accounts.doctype.tax_rule.tax_rule import get_party_details, get_tax_template
...
def get_payment_terms_template(party_name, party_type, company=None):
	...
	if party_type == "Customer":
		...
		if not template and customer.customer_group:
			template = dontmanage.get_cached_value("Customer Group", customer.customer_group, "payment_terms")
	else:
		supplier = dontmanage.get_cached_value(
			"Supplier", party_name, fieldname=["payment_terms", "supplier_group"], as_dict=1
		)
		template = supplier.payment_terms
		if not template and supplier.supplier_group:
			template = dontmanage.get_cached_value("Supplier Group", supplier.supplier_group, "payment_terms")

	if not template and company:
		template = dontmanage.get_cached_value("Company", company, fieldname="payment_terms")
	return template


def validate_party_frozen_disabled(party_type, party_name):
	if dontmanage.flags.ignore_party_validation:
		return

	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			party = dontmanage.get_cached_value(party_type, party_name, ["is_frozen", "disabled"], as_dict=True)
			if party.disabled:
				dontmanage.throw(_("{0} {1} is disabled").format(party_type, party_name), PartyDisabled)
			elif party.get("is_frozen"):
				frozen_accounts_modifier = dontmanage.db.get_single_value(
					"Accounts Settings", "frozen_accounts_modifier"
				)
				if frozen_accounts_modifier not in dontmanage.get_roles():
					dontmanage.throw(_("{0} {1} is frozen").format(party_type, party_name), PartyFrozen)

		elif party_type == "Employee":
			if dontmanage.db.get_value("Employee", party_name, "status") != "Active":
				dontmanage.msgprint(_("{0} {1} is not active").format(party_type, party_name), alert=True)


def validate_account_party_type(self):
	if self.party_type and self.party:
		account_type = dontmanage.get_cached_value("Account", self.account, "account_type")
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	company_wise_grand_total = dontmanage.get_all(
		doctype,
		filters={
			"docstatus": 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = dontmanage._dict(
			dontmanage.get_all(
				"Loyalty Point Entry",
				filters={
					"customer": party,
	...
	company_wise_billing_this_year = dontmanage._dict()
	...
	company_wise_total_unpaid = dontmanage._dict(
		dontmanage.db.sql(
			"""
...
def get_partywise_advanced_payment_amount(
	...
	account_type = dontmanage.get_cached_value("Party Type", party_type, "account_type")
...
def add_party_account(party_type, party, company, account):
	doc = dontmanage.get_doc(party_type, party)
...
def render_address(address, check_permissions=True):
	return dontmanage.call(_render_address, address, check_permissions=check_permissions)
