...
from dontmanage import _, qb, throw
from dontmanage.model.meta import get_field_precision
from dontmanage.query_builder import AliasedQuery, Criterion, Table
from dontmanage.query_builder.functions import Count, Sum
...
# imported to enable dontmanageerp.accounts.utils.get_account_currency
from dontmanageerp.accounts.doctype.account.account import get_account_currency
from dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from dontmanageerp.stock import get_warehouse_account_map
from dontmanageerp.stock.utils import get_stock_value_on

if TYPE_CHECKING:
	from dontmanageerp.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation


class FiscalYearError(dontmanage.ValidationError):
	pass


class PaymentEntryUnlinkError(dontmanage.ValidationError):
	pass
...
def get_fiscal_years(
	...
	fiscal_years = dontmanage.cache().hget("fiscal_years", company) or []
	...
	if not fiscal_years:
		...
		query = (
			dontmanage.qb.from_(FY).select(FY.name, FY.year_start_date, FY.year_end_date).where(FY.disabled == 0)
		)
		...
		if company:
			...
			query = query.where(
				ExistsCriterion(dontmanage.qb.from_(FYC).select(FYC.name).where(FYC.parent == FY.name)).negate()
				| ExistsCriterion(
					dontmanage.qb.from_(FYC)
					.select(FYC.company)
		...
		dontmanage.cache().hset("fiscal_years", company, fiscal_years)
	...
	if company:
		error_msg = _("""{0} for {1}""").format(error_msg, dontmanage.bold(company))

	if boolean:
		return False

	if verbose == 1:
		dontmanage.msgprint(error_msg)
...
def get_balance_on(
	...
	if not account and dontmanage.form_dict.get("account"):
		account = dontmanage.form_dict.get("account")
	if not date and dontmanage.form_dict.get("date"):
		date = dontmanage.form_dict.get("date")
	if not party_type and dontmanage.form_dict.get("party_type"):
		party_type = dontmanage.form_dict.get("party_type")
	if not party and dontmanage.form_dict.get("party"):
		party = dontmanage.form_dict.get("party")
	if not cost_center and dontmanage.form_dict.get("cost_center"):
		cost_center = dontmanage.form_dict.get("cost_center")

	cond = ["is_cancelled=0"]
	if start_date:
		cond.append("posting_date >= %s" % dontmanage.db.escape(cstr(start_date)))
	if date:
		cond.append("posting_date <= %s" % dontmanage.db.escape(cstr(date)))
	else:
		# get balance of all entries that exist
		date = nowdate()

	if account:
		acc = dontmanage.get_doc("Account", account)
	...
	if cost_center and report_type == "Profit and Loss":
		cc = dontmanage.get_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(
				f""" exists (
			...
			cond.append(f"""gle.cost_center = {dontmanage.db.escape(cost_center)} """)

	if account:
		if not (dontmanage.flags.ignore_account_permission or ignore_account_permission):
		...
		if acc.is_group:
			...
			if acc.account_currency == dontmanage.get_cached_value("Company", acc.company, "default_currency"):
				in_account_currency = False
		else:
			cond.append(f"""gle.account = {dontmanage.db.escape(account)} """)

	if account_type:
		accounts = dontmanage.db.get_all(
			"Account",
		...
		cond.append(
			...
			gle.account in (%s)
		"""
			% (", ".join([dontmanage.db.escape(account) for account in accounts]))
		)

	if party_type and party:
		cond.append(
			f"""gle.party_type = {dontmanage.db.escape(party_type)} and gle.party = {dontmanage.db.escape(party)} """
		)

	if company:
		cond.append("""gle.company = %s """ % (dontmanage.db.escape(company)))
...
def get_count_on(account, fieldname, date):
	...
	if account:
		...
		if not dontmanage.flags.ignore_account_permission:
			acc.check_permission("read")
		...
		for gle in entries:
			if fieldname not in ("invoiced_amount", "payables"):
				...
				if (
					...
					payment_amount = dontmanage.db.sql(
						f"""
...
def add_cc(args=None):
	...
	if args.parent_cost_center == args.company:
		args.parent_cost_center = "{} - {}".format(
			args.parent_cost_center, dontmanage.get_cached_value("Company", args.company, "abbr")
		)

	cc = dontmanage.new_doc("Cost Center")
	cc.update(args)
...
def reconcile_against_document(
	...
	for key, entries in reconciled_entries.items():
		...
		doc = dontmanage.get_doc(voucher_type, voucher_no)
		dontmanage.flags.ignore_party_validation = True
		...
		doc = dontmanage.get_doc(entry.voucher_type, entry.voucher_no)
		...
		if voucher_type == "Payment Entry" and doc.book_advance_payments_in_separate_party_account:
			...
			from dontmanageerp.accounts.general_ledger import process_debit_credit_difference
		...
		if update_advance_paid:
			for t, n in update_advance_paid:
				dontmanage.get_doc(t, n).set_total_advance_paid()

		dontmanage.flags.ignore_party_validation = False
...
def validate_allocated_amount(args):
	precision = args.get("precision") or dontmanage.db.get_single_value("System Settings", "currency_precision")
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	[
		...
		for field in dontmanage.get_meta("Journal Entry Account").get_fieldnames_with_value()
	]
...
def update_reference_in_payment_entry(
	...
	if not skip_ref_details_update_for_pe:
		reference_exchange_details = dontmanage._dict()
	...
	payment_entry.make_exchange_gain_loss_journal(
		dontmanage._dict({"difference_posting_date": d.difference_posting_date}), dimensions_dict
	)
...
def cancel_exchange_gain_loss_journal(
	...
	if parent_doc.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
		...
		for doc in gain_loss_journals:
			gain_loss_je = dontmanage.get_doc("Journal Entry", doc)
...
def get_linked_exchange_gain_loss_journal(referenced_dt: str, referenced_dn: str, je_docstatus: int) -> list:
	...
	"""
	gain_loss_journals = []
	if journals := dontmanage.db.get_all(
		...
		gain_loss_journals = dontmanage.db.get_all(
			"Journal Entry",
			{
				"name": ["in", journals],
...
def cancel_common_party_journal(self):
	...
	if not dontmanage.db.get_single_value("Accounts Settings", "enable_common_party_accounting"):
		return
	...
	journal_entry = dontmanage.db.get_value(
		"Journal Entry Account",
		filters={
	...
	common_party_journal = dontmanage.db.get_value(
		"Journal Entry",
		filters={
			"name": journal_entry,
	...
	common_party_je = dontmanage.get_doc("Journal Entry", common_party_journal)
...
def update_accounting_ledgers_after_reference_removal(
	...
	gle_update_query = (
		...
		.set(gle.modified_by, dontmanage.session.user)
	...
	ple_update_query = (
		...
		.set(ple.modified_by, dontmanage.session.user)
		.where(
...
def remove_ref_doc_link_from_jv(
	...
	if linked_jv:
		update_query = (
			...
			.set(jea.modified_by, dontmanage.session.user)
		...
		dontmanage.msgprint(_("Journal Entries {0} are un-linked").format("\n".join(linked_jv)))
...
def remove_ref_doc_link_from_pe(
	...
	if linked_pe:
		update_query = (
			...
			.set(per.modified_by, dontmanage.session.user)
		...
		for pe in linked_pe:
			try:
				pe_doc = dontmanage.get_doc("Payment Entry", pe)
				pe_doc.set_amounts()
				...
				dontmanage.throw(msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error"))
			...
			qb.update(pay).set(pay.total_allocated_amount, pe_doc.total_allocated_amount).set(
				...
				pay.modified_by, dontmanage.session.user
			).where(pay.name == pe).run()

		dontmanage.msgprint(_("Payment Entries {0} are un-linked").format("\n".join(linked_pe)))


@dontmanage.whitelist()
def get_company_default(company, fieldname, ignore_validation=False):
	value = dontmanage.get_cached_value("Company", company, fieldname)

	if not ignore_validation and not value:
		throw(
			_("Please set default {0} in Company {1}").format(
				dontmanage.get_meta("Company").get_label(fieldname), company
			)
		)
...
def get_held_invoices(party_type, party):
	...
	if party_type == "Supplier":
		held_invoices = dontmanage.db.sql(
...
def get_outstanding_invoices(
	...
	precision = dontmanage.get_precision("Sales Invoice", "outstanding_amount") or 2

	if account:
		root_type, account_type = dontmanage.get_cached_value(
			"Account", account[0], ["root_type", "account_type"]
		)
		party_account_type = "Receivable" if root_type == "Asset" else "Payable"
		party_account_type = account_type or party_account_type
	else:
		party_account_type = dontmanageerp.get_party_account_type(party_type)
...
def get_account_name(account_type=None, root_type=None, is_group=None, account_currency=None, company=None):
	...
	return dontmanage.db.get_value(
		"Account",
		{
			...
			"account_currency": account_currency or dontmanage.defaults.get_defaults().currency,
			"company": company or dontmanage.defaults.get_defaults().company,
		},
		"name",
	)
...
def get_companies():
	...
	return [d.name for d in dontmanage.get_list("Company", fields=["name"], order_by="name")]


@dontmanage.whitelist()
def get_children(doctype, parent, company, is_root=False):
	from dontmanageerp.accounts.report.financial_statements import sort_accounts

	parent_fieldname = "parent_" + dontmanage.scrub(doctype)
	...
	acc = dontmanage.get_list(doctype, fields=fields, filters=filters)
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	...
	company = dontmanage.get_cached_value("Global Defaults", "Global Defaults", "default_company")
	if not company:
		return

	# NOTE: we translate Payment Gateway account name because that is going to be used by the end user
	bank_account = dontmanage.db.get_value(
		"Account",
	...
	if not bank_account:
		dontmanage.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def update_cost_center(docname, cost_center_name, cost_center_number, company, merge):
	...
	if cost_center_number:
		dontmanage.db.set_value("Cost Center", docname, "cost_center_number", cost_center_number.strip())
	else:
		dontmanage.db.set_value("Cost Center", docname, "cost_center_number", "")

	dontmanage.db.set_value("Cost Center", docname, "cost_center_name", cost_center_name.strip())

	new_name = get_autoname_with_number(cost_center_number, cost_center_name, company)
	if docname != new_name:
		dontmanage.rename_doc("Cost Center", docname, new_name, force=1, merge=merge)
		return new_name
...
def validate_field_number(doctype_name, docname, number_value, company, field_name):
	...
	if number_value:
		...
		doctype_with_same_number = dontmanage.db.get_value(doctype_name, filters)
...
def get_coa(doctype, parent, is_root=None, chart=None):
	from dontmanageerp.accounts.doctype.account.chart_of_accounts.chart_of_accounts import (
		build_tree_from_json,
	)

	# add chart to flags to retrieve when called from expand all function
	chart = chart if chart else dontmanage.flags.chart
	dontmanage.flags.chart = chart
...
def repost_gle_for_stock_vouchers(
	...
	from dontmanageerp.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	precision = get_field_precision(dontmanage.get_meta("GL Entry").get_field("debit")) or 2
	...
	for stock_vouchers_chunk in create_batch(stock_vouchers, GL_REPOSTING_CHUNK):
		...
		for voucher_type, voucher_no in stock_vouchers_chunk:
			...
			voucher_obj = dontmanage.get_doc(voucher_type, voucher_no)
...
def get_future_stock_vouchers(posting_date, posting_time, for_warehouses=None, for_items=None, company=None):
	...
	future_stock_vouchers = dontmanage.db.sql(
...
def get_stock_accounts(company, voucher_type=None, voucher_no=None, accounts=None):
	stock_accounts = [
		d.name
		for d in dontmanage.db.get_all("Account", {"account_type": "Stock", "company": company, "is_group": 0})
	]
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	account_table = dontmanage.qb.DocType("Account")
	query = (
		dontmanage.qb.from_(account_table)
...
def check_and_delete_linked_reports(report):
	...
	icons = dontmanage.get_all("Desktop Icon", fields=["name"], filters={"_report": report})
	if icons:
		for icon in icons:
			dontmanage.delete_doc("Desktop Icon", icon)


def create_err_and_its_journals(companies: list | None = None) -> None:
	if companies:
		for company in companies:
			err = dontmanage.new_doc("Exchange Rate Revaluation")
			...
			if err.accounts:
				...
				if company.submit_err_jv:
					...
					jv and dontmanage.get_doc("Journal Entry", jv).submit()
...
def auto_create_exchange_rate_revaluation_daily() -> None:
	...
	"""
	companies = dontmanage.db.get_all(
		"Company",
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	...
	vouchers = [dontmanage._dict({"voucher_type": voucher_type, "voucher_no": voucher_no})]
	common_filter = []
	if account:
	...
	if (
		...
		ref_doc = dontmanage.get_doc(voucher_type, voucher_no)
...
def create_gain_loss_journal(
	...
	party_account_currency = dontmanage.get_cached_value("Account", party_account, "account_currency")

	if not gain_loss_account:
		dontmanage.throw(_("Please set default Exchange Gain/Loss Account in Company {}").format(company))
	gain_loss_account_currency = get_account_currency(gain_loss_account)
	company_currency = dontmanage.get_cached_value("Company", company, "default_currency")

	if gain_loss_account_currency != company_currency:
		dontmanage.throw(_("Currency for {0} must be {1}").format(gain_loss_account, company_currency))

	journal_account = dontmanage._dict(
		{
			...
			"exchange_rate": 0,
			"cost_center": cost_center or dontmanageerp.get_default_cost_center(company),
...
def get_party_types_from_account_type(account_type):
	return dontmanage.db.get_all("Party Type", {"account_type": account_type}, pluck="name")


def run_ledger_health_checks():
	health_monitor_settings = dontmanage.get_doc("Ledger Health Monitor")
	if health_monitor_settings.enable_health_monitor:
		...
		if health_monitor_settings.debit_credit_mismatch:
			for x in health_monitor_settings.companies:
				...
				voucher_wise = dontmanage.get_doc("Report", "Voucher-wise Balance")
				res = voucher_wise.execute_script_report(filters=filters)
				for x in res[1]:
					doc = dontmanage.new_doc("Ledger Health")
		...
		if health_monitor_settings.general_and_payment_ledger_mismatch:
			for x in health_monitor_settings.companies:
				...
				gl_pl_comparison = dontmanage.get_doc("Report", "General and Payment Ledger Comparison")
