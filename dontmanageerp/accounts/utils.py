...
def get_balance_on(
	...
	if cost_center and report_type == "Profit and Loss":
		...
		if cc.is_group:
			...
			cond.append("""gle.cost_center = %s """ % (dontmanage.db.escape(cost_center, percent=False),))
	...
	if account:
		...
		if acc.is_group:
			...
			cond.append("""gle.account = %s """ % (dontmanage.db.escape(account, percent=False),))

	if party_type and party:
		cond.append(
			"""gle.party_type = %s and gle.party = %s """
			% (dontmanage.db.escape(party_type), dontmanage.db.escape(party, percent=False))
		)

	if company:
		cond.append("""gle.company = %s """ % (dontmanage.db.escape(company, percent=False)))
	...
	for key, entries in reconciled_entries.items():
		...
		if doc.doctype == "Journal Entry":
			try:
				...
				raise dontmanage.ValidationError(_(f"Validation Error for {doc.name}")) from validation_exception
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = (
			"paid_from" if dontmanageerp.get_party_account_type(args.party_type) == "Receivable" else "paid_to"
		)
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	new_row.update((dontmanage.copy_doc(jv_detail)).as_dict())
...
def update_reference_in_payment_entry(d, payment_entry, do_not_save=False):
	...
	if d.difference_amount and d.difference_account:
		account_details = {
			...
			or dontmanage.get_cached_value("Company", payment_entry.company, "cost_center"),
		}
...
def unlink_ref_doc_from_payment_entries(ref_doc):
	...
	dontmanage.db.sql(
		...
		(now(), dontmanage.session.user, ref_doc.doctype, ref_doc.name),
	)
	...
	).set(ple.modified, now()).set(ple.modified_by, dontmanage.session.user).where(
...
def remove_ref_doc_link_from_jv(ref_type, ref_no):
	linked_jv = dontmanage.db.sql_list(
	...
	if linked_jv:
		dontmanage.db.sql(
			...
			and docstatus < 2""",
			(now(), dontmanage.session.user, ref_type, ref_no),
		)
...
def remove_ref_doc_link_from_pe(ref_type, ref_no):
	linked_pe = dontmanage.db.sql_list(
...
def get_stock_rbnb_difference(posting_date, company):
	stock_items = dontmanage.db.sql_list(
		"""select distinct item_code
		from `tabStock Ledger Entry` where company=%s""",
		company,
	)

	pr_valuation_amount = dontmanage.db.sql(
		"""
	...
	pi_valuation_amount = dontmanage.db.sql(
		"""
	...
	stock_rbnb_account = "Stock Received But Not Billed - " + dontmanage.get_cached_value(
