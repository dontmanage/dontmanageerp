...
from dontmanageerp.accounts.doctype.account.account import get_account_currency  # noqa
...
def get_fiscal_year(
	...
	if isinstance(boolean, str):
		boolean = dontmanage.json.loads(boolean)
...
def get_balance_on(
	...
	if cost_center and report_type == "Profit and Loss":
		...
		if cc.is_group:
			...
			cond.append("""gle.cost_center = %s """ % (dontmanage.db.escape(cost_center),))
	...
	if account:
		...
		if acc.is_group:
			...
			cond.append("""gle.account = %s """ % (dontmanage.db.escape(account),))
	...
	if party_type and party:
		cond.append(
			...
			% (dontmanage.db.escape(party_type), dontmanage.db.escape(party))
		)
...
def validate_allocated_amount(args):
	precision = args.get("precision") or dontmanage.db.get_single_value(
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	if jv_detail.get("reference_type") in ("Sales Order", "Purchase Order"):
		dontmanage.get_doc(jv_detail.reference_type, jv_detail.reference_name).set_total_advance_paid()
	...
	if parent_doc.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
		...
		if journals:
			...
			for doc in gain_loss_journals:
				gain_loss_je = dontmanage.get_doc("Journal Entry", doc[0])
