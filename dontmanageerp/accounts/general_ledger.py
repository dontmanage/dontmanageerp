...
from dontmanage.utils import cint, flt, formatdate, get_link_to_form, getdate, now
...
from dontmanageerp.accounts.doctype.accounting_period.accounting_period import ClosedAccountingPeriod
from dontmanageerp.accounts.doctype.budget.budget import validate_expense_against_budget
from dontmanageerp.accounts.utils import create_payment_ledger_entry
...
def get_accounting_dimensions_for_offsetting_entry(gl_map, company):
	acc_dimension = dontmanage.qb.DocType("Accounting Dimension")
	dimension_detail = dontmanage.qb.DocType("Accounting Dimension Detail")

	acc_dimensions = (
		dontmanage.qb.from_(acc_dimension)
...
def validate_disabled_accounts(gl_map):
	...
	disabled_accounts = dontmanage.get_all(
		"Account",
	...
	if used_disabled_accounts:
		account_list = "<br>"
		account_list += ", ".join([dontmanage.bold(d) for d in used_disabled_accounts])
		dontmanage.throw(
...
def validate_accounting_period(gl_map):
	accounting_periods = dontmanage.db.sql(
		""" SELECT
			ap.name as name
		FROM
	...
	if accounting_periods:
		dontmanage.throw(
			...
			).format(dontmanage.bold(accounting_periods[0].name)),
...
def get_cost_center_allocation_data(company, posting_date, cost_center):
	cost_center_allocation = dontmanage.db.get_value(
...
def merge_similar_entries(gl_map, precision=None):
	...
	company = gl_map[0].company if gl_map else dontmanageerp.get_default_company()
	company_currency = dontmanageerp.get_company_currency(company)

	if not precision:
		precision = get_field_precision(dontmanage.get_meta("GL Entry").get_field("debit"), company_currency)
	...
	merged_gl_map = filter(
		...
		or (
			...
			and dontmanage.get_cached_value("Journal Entry", x.voucher_no, "voucher_type")
...
def make_entry(args, adv_adj, update_outstanding, from_repost=False):
	gle = dontmanage.new_doc("GL Entry")
	gle.update(args)
...
def validate_cwip_accounts(gl_map):
	...
	cwip_enabled = any(
		...
		for ac in dontmanage.db.get_all("Asset Category", "enable_cwip_accounting")
	)
	if cwip_enabled:
		cwip_accounts = [
			d[0]
...
def process_debit_credit_difference(gl_map):
	precision = get_field_precision(
		dontmanage.get_meta("GL Entry").get_field("debit"),
		currency=dontmanage.get_cached_value("Company", gl_map[0].company, "default_currency"),
	)
	...
	if abs(debit_credit_diff) > allowance:
		if not (
			...
			and dontmanage.get_cached_value("Journal Entry", voucher_no, "voucher_type")
			== "Exchange Gain Or Loss"
		):
...
def make_round_off_gle(gl_map, debit_credit_diff, precision):
	...
	round_off_gle = dontmanage._dict()
		...
		if not round_off_for_opening:
			dontmanage.throw(
				_("Please set '{0}' in Company: {1}").format(
					dontmanage.bold("Round Off for Opening"), get_link_to_form("Company", gl_map[0].company)
				)
			)
...
def update_accounting_dimensions(round_off_gle):
	...
	meta = dontmanage.get_meta(round_off_gle["voucher_type"])
	...
	if dimensions and has_all_dimensions:
		dimension_values = dontmanage.db.get_value(
...
def get_round_off_account_and_cost_center(company, voucher_type, voucher_no, use_company_default=False):
	round_off_account, round_off_cost_center, round_off_for_opening = dontmanage.get_cached_value(
		"Company", company, ["round_off_account", "round_off_cost_center", "round_off_for_opening"]
	) or [None, None, None]

	# Use expense account as fallback
	if not round_off_account:
		round_off_account = dontmanage.get_cached_value("Company", company, "default_expense_account")

	meta = dontmanage.get_meta(voucher_type)

	# Give first preference to parent cost center for round off GLE
	if not use_company_default and meta.has_field("cost_center"):
		parent_cost_center = dontmanage.db.get_value(voucher_type, voucher_no, "cost_center")
	...
	if not round_off_account:
		dontmanage.throw(
			_("Please mention '{0}' in Company: {1}").format(
				dontmanage.bold("Round Off Account"), get_link_to_form("Company", company)
			)
		)
	...
	if not round_off_cost_center:
		dontmanage.throw(
			_("Please mention '{0}' in Company: {1}").format(
				dontmanage.bold("Round Off Cost Center"), get_link_to_form("Company", company)
			)
		)
	...
	if gl_entries:
		...
		for entry in gl_entries:
			...
			if immutable_ledger_enabled:
				...
				new_gle["posting_date"] = dontmanage.form_dict.get("posting_date") or getdate()
	...
	if not adv_adj:
		...
		if acc_frozen_upto:
			...
			if getdate(posting_date) <= getdate(acc_frozen_upto) and (
				frozen_accounts_modifier not in dontmanage.get_roles() or dontmanage.session.user == "Administrator"
			):
				dontmanage.throw(
...
def validate_against_pcv(is_opening, posting_date, company):
	if is_opening and dontmanage.db.exists("Period Closing Voucher", {"docstatus": 1, "company": company}):
		dontmanage.throw(
			_("Opening Entry can not be created after Period Closing Voucher is created."),
			title=_("Invalid Opening Entry"),
		)

	last_pcv_date = dontmanage.db.get_value(
	...
	if last_pcv_date and getdate(posting_date) <= getdate(last_pcv_date):
		...
		dontmanage.throw(message, title=_("Period Closed"))
			...
			if value["is_mandatory"] and not gl_entry.get(dimension):
				dontmanage.throw(
					_("{0} is mandatory for account {1}").format(
						dontmanage.bold(dontmanage.unscrub(dimension)), dontmanage.bold(gl_entry.account)
					),
				...
				if gl_entry.get(dimension) and gl_entry.get(dimension) not in value["allowed_dimensions"]:
					dontmanage.throw(
						_("Invalid value {0} for {1} against account {2}").format(
							dontmanage.bold(gl_entry.get(dimension)),
							dontmanage.bold(dontmanage.unscrub(dimension)),
							dontmanage.bold(gl_entry.account),
						),
...
def is_immutable_ledger_enabled():
	return dontmanage.db.get_single_value("Accounts Settings", "enable_immutable_ledger")
