...
from dontmanage.utils import cint, cstr, flt, formatdate, getdate, now
...
def validate_disabled_accounts(gl_map):
	...
	Account = dontmanage.qb.DocType("Account")

	disabled_accounts = (
		dontmanage.qb.from_(Account)
	...
	if disabled_accounts:
		account_list = "<br>"
		account_list += ", ".join([dontmanage.bold(d.name) for d in disabled_accounts])
		dontmanage.throw(
...
def get_cost_center_allocation_data(company, posting_date):
	par = dontmanage.qb.DocType("Cost Center Allocation")
	child = dontmanage.qb.DocType("Cost Center Allocation Percentage")
	...
	records = (
		...
		.orderby(par.valid_from, order=dontmanage.qb.desc)
	).run(as_dict=True)

	cc_allocation = dontmanage._dict()
	for d in records:
		cc_allocation.setdefault(d.main_cost_center, dontmanage._dict()).setdefault(
...
def get_round_off_account_and_cost_center(
	...
	round_off_account, round_off_cost_center = dontmanage.get_cached_value(
	...
	if not round_off_account:
		dontmanage.throw(_("Please mention Round Off Account in Company"))

	if not round_off_cost_center:
		dontmanage.throw(_("Please mention Round Off Cost Center in Company"))
...
def validate_against_pcv(is_opening, posting_date, company):
	if is_opening and dontmanage.db.exists(
