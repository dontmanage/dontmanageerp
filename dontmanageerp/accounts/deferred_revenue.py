...
def validate_service_stop_date(doc):
	...
	old_stop_dates = {}
	old_doc = dontmanage.db.get_all(f"{doc.doctype} Item", {"parent": doc.name}, ["name", "service_stop_date"])
	...
	for item in doc.items:
		...
		if item.service_stop_date:
			if date_diff(item.service_stop_date, item.service_start_date) < 0:
				dontmanage.throw(_("Service Stop Date cannot be before Service Start Date"))

			if date_diff(item.service_stop_date, item.service_end_date) > 0:
				dontmanage.throw(_("Service Stop Date cannot be after Service End Date"))
		...
		if (
			...
			dontmanage.throw(_("Cannot change Service Stop Date for item in row {0}").format(item.idx))
...
def build_conditions(process_type, account, company):
	...
	if account:
		conditions += f"AND {deferred_account}={dontmanage.db.escape(account)}"
	elif company:
		conditions += f"AND p.company = {dontmanage.db.escape(company)}"
...
def convert_deferred_expense_to_expense(deferred_process, start_date=None, end_date=None, conditions=""):
	...
	invoices = dontmanage.db.sql_list(
		f"""
	...
	for invoice in invoices:
		doc = dontmanage.get_doc("Purchase Invoice", invoice)
		book_deferred_income_or_expense(doc, deferred_process, end_date)

	if dontmanage.flags.deferred_accounting_error:
...
def convert_deferred_revenue_to_income(deferred_process, start_date=None, end_date=None, conditions=""):
	...
	for invoice in invoices:
		doc = dontmanage.get_doc("Sales Invoice", invoice)
		book_deferred_income_or_expense(doc, deferred_process, end_date)
...
def get_booking_dates(doc, item, posting_date=None, prev_posting_date=None):
	...
	if not prev_posting_date:
		prev_gl_entry = dontmanage.db.sql(
			"""
		...
		prev_gl_via_je = dontmanage.db.sql(
			"""
...
def get_already_booked_amount(doc, item):
	...
	gl_entries_details = dontmanage.db.sql(
		"""
	...
	journal_entry_details = dontmanage.db.sql(
		"""
...
def book_deferred_income_or_expense(doc, deferred_process, posting_date=None):
	...
	accounts_frozen_upto = dontmanage.db.get_single_value("Accounts Settings", "acc_frozen_upto")
	...
	via_journal_entry = cint(
		dontmanage.db.get_singles_value("Accounts Settings", "book_deferred_entries_via_journal_entry")
	)
	submit_journal_entry = cint(dontmanage.db.get_singles_value("Accounts Settings", "submit_journal_entries"))
	book_deferred_entries_based_on = dontmanage.db.get_singles_value(
...
def process_deferred_accounting(posting_date=None):
	...
	if not cint(
		dontmanage.db.get_singles_value("Accounts Settings", "automatically_process_deferred_accounting_entry")
	):
		return
	...
	if gl_entries:
		try:
			...
			if dontmanage.flags.in_test:
				...
				doc.log_error(f"Error while processing deferred accounting for Invoice {doc.name}")
				dontmanage.flags.deferred_accounting_error = True
...
def get_deferred_booking_accounts(doctype, voucher_detail_no, dr_or_cr):
	if doctype == "Sales Invoice":
		credit_account, debit_account = dontmanage.db.get_value(
			"Sales Invoice Item",
