...
import dontmanage
from dontmanage import _, msgprint, scrub, unscrub
from dontmanage.model.document import Document
from dontmanage.utils import get_link_to_form, now
...
class POSProfile(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.pos_customer_group.pos_customer_group import POSCustomerGroup
		from dontmanageerp.accounts.doctype.pos_item_group.pos_item_group import POSItemGroup
		from dontmanageerp.accounts.doctype.pos_payment_method.pos_payment_method import POSPaymentMethod
		from dontmanageerp.accounts.doctype.pos_profile_user.pos_profile_user import POSProfileUser
			...
			if not self.get(acc_dim):
				dontmanage.throw(
					_(
						...
						unscrub(dontmanage.bold(acc_dim)),
					),
	...
	def validate_all_link_fields(self):
		...
		for link_dt, dn_list in accounts.items():
			for link_dn in dn_list:
				if link_dn and not dontmanage.db.exists(
					{"doctype": link_dt, "company": self.company, "name": link_dn}
				):
					dontmanage.throw(_("{0} does not belong to Company {1}").format(link_dn, self.company))
	...
	def validate_payment_methods(self):
		if not self.payments:
			dontmanage.throw(_("Payment methods are mandatory. Please add at least one payment method."))

		default_mode = [d.default for d in self.payments if d.default]
		if not default_mode:
			dontmanage.throw(_("Please select a default mode of payment"))

		if len(default_mode) > 1:
			dontmanage.throw(_("You can only select one mode of payment as default"))
	...
	def set_defaults(self, include_current_pos=True):
		dontmanage.defaults.clear_default("is_pos")
		...
		pos_view_users = dontmanage.db.sql_list(
			"""select pfu.user
		...
		for user in pos_view_users:
			if user:
				dontmanage.defaults.set_user_default("is_pos", 1, user)
			else:
				dontmanage.defaults.set_global_default("is_pos", 1)
...
def get_item_groups(pos_profile):
	...
	if pos_profile.get("item_groups"):
		...
		for data in pos_profile.get("item_groups"):
			item_groups.extend(
				["%s" % dontmanage.db.escape(d.name) for d in get_child_nodes("Item Group", data.item_group)]
			)
...
def get_child_nodes(group_type, root):
	lft, rgt = dontmanage.db.get_value(group_type, root, ["lft", "rgt"])
...
def pos_profile_query(doctype, txt, searchfield, start, page_len, filters):
	...
	company = filters.get("company") or dontmanage.defaults.get_user_default("company")
