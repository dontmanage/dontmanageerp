...
from dontmanageerp.accounts.doctype.pos_profile.pos_profile import get_child_nodes
from dontmanageerp.stock.get_item_details import get_pos_profile
...
class TestPOSProfile(unittest.TestCase):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = dontmanage.get_doc("POS Profile", pos_profile.get("name"))
...
def make_pos_profile(**args):
	dontmanage.db.sql("delete from `tabPOS Payment Method`")
	dontmanage.db.sql("delete from `tabPOS Profile`")

	args = dontmanage._dict(args)

	pos_profile = dontmanage.get_doc(
		{
			...
			"customer_group": dontmanage.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	if not dontmanage.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not dontmanage.db.exists("POS Profile", args.name or "_Test POS Profile"):
		pos_profile.insert()
