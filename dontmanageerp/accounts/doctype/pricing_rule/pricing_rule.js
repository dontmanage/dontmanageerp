...
dontmanage.ui.form.on('Pricing Rule', {
	setup: function(frm) {
		...
		['items', 'item_groups', 'brands'].forEach(d => {
			frm.fields_dict[d].grid.get_field('uom').get_query = function(doc, cdt, cdn){
				...
				return {
					query:"dontmanageerp.accounts.doctype.pricing_rule.pricing_rule.get_item_uoms",
					filters: {'value': row[dontmanage.scrub(doc.apply_on)], apply_on: doc.apply_on}
				}
			};
		})
	},
