...
class PricingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.pricing_rule_item_code.pricing_rule_item_code import PricingRuleItemCode
	...
	def validate_duplicate_apply_on(self):
		if self.apply_on != "Transaction":
			...
			apply_on_field = dontmanage.scrub(self.apply_on)
			values = [d.get(apply_on_field) for d in self.get(apply_on_table) if d.get(apply_on_field)]
			if len(values) != len(set(values)):
				dontmanage.throw(_("Duplicate {0} found in the table").format(self.apply_on))

	def validate_mandatory(self):
		if self.has_priority and not self.priority:
			throw(_("Priority is mandatory"), dontmanage.MandatoryError, _("Please Set Priority"))
		...
		for apply_on, field in apply_on_dict.items():
			if self.apply_on == apply_on and len(self.get(field) or []) < 1:
				throw(_("{0} is not added in the table").format(apply_on), dontmanage.MandatoryError)

		tocheck = dontmanage.scrub(self.get("applicable_for", ""))
		if tocheck and not self.get(tocheck):
			throw(_("{0} is required").format(self.meta.get_label(tocheck)), dontmanage.MandatoryError)

		if self.apply_rule_on_other:
			o_field = "other_" + dontmanage.scrub(self.apply_rule_on_other)
			if not self.get(o_field) and o_field in other_fields:
				dontmanage.throw(
					_("For the 'Apply Rule On Other' condition the field {0} is mandatory").format(
						dontmanage.bold(self.apply_rule_on_other)
					)
				)

		if self.price_or_product_discount == "Price" and not self.rate_or_discount:
			throw(_("Rate or Discount is required for the price discount."), dontmanage.MandatoryError)

		if self.apply_discount_on_rate:
			if not self.priority:
				throw(
					_("As the field {0} is enabled, the field {1} is mandatory.").format(
						dontmanage.bold(_("Apply Discount on Discounted Rate")),
						dontmanage.bold(_("Priority")),
					)
				)
			...
			if self.priority and cint(self.priority) == 1:
				throw(
					...
					).format(dontmanage.bold(_("Apply Discount on Discounted Rate")), dontmanage.bold(_("Priority")))
				)
	...
	def cleanup_fields_value(self):
		for logic_field in ["apply_on", "applicable_for", "rate_or_discount"]:
			fieldname = dontmanage.scrub(self.get(logic_field) or "")
		...
		apply_rule_on_other = dontmanage.scrub(self.apply_rule_on_other or "")
	...
	def validate_max_discount(self):
		if self.rate_or_discount == "Discount Percentage" and self.get("items"):
			for d in self.items:
				max_discount = dontmanage.get_cached_value("Item", d.item_code, "max_discount")
				if max_discount and flt(self.discount_percentage) > flt(max_discount):
					throw(_("Max discount allowed for item: {0} is {1}%").format(d.item_code, max_discount))

	def validate_price_list_with_currency(self):
		if self.currency and self.for_price_list:
			price_list_currency = dontmanage.db.get_value("Price List", self.for_price_list, "currency", True)
	...
	def validate_condition(self):
		if (
			...
			dontmanage.throw(_("Invalid condition expression"))

	def validate_mixed_with_recursion(self):
		if self.mixed_conditions and self.is_recursive:
			dontmanage.throw(_("Recursive Discounts with Mixed condition is not supported by the system"))
...
def update_pricing_rule_uom(pricing_rule, args):
	...
	apply_on_field = dontmanage.scrub(pricing_rule.apply_on)
...
def get_pricing_rule_for_item(args, doc=None, for_validate=False):
	...
	if args.ignore_pricing_rule or not args.item_code:
		if dontmanage.db.exists(args.doctype, args.name) and args.get("pricing_rules"):
	...
	if pricing_rules:
		...
		for pricing_rule in pricing_rules:
			...
			if isinstance(pricing_rule, str):
				pricing_rule = dontmanage.get_cached_doc("Pricing Rule", pricing_rule)
			...
			if pricing_rule.coupon_code_based == 1:
				...
				coupon_code = dontmanage.db.get_value(
				...
				item_details.update(
					{
						...
						"apply_rule_on": (
							dontmanage.scrub(pricing_rule.apply_rule_on_other)
							if pricing_rule.apply_rule_on_other
							else dontmanage.scrub(pricing_rule.get("apply_on"))
						),
					}
				)
		...
		item_details.pricing_rules = dontmanage.as_json([d.pricing_rule for d in rules])
...
def update_args_for_pricing_rule(args):
	if not (args.item_group and args.brand):
		item = dontmanage.get_cached_value("Item", args.item_code, ("item_group", "brand"))
		if not item:
			return
		...
		if not args.item_group:
			dontmanage.throw(_("Item Group not mentioned in item master for item {0}").format(args.item_code))

	if args.transaction_type == "selling":
		if args.customer and not (args.customer_group and args.territory):
			if args.quotation_to and args.quotation_to != "Customer":
				customer = dontmanage._dict()
			else:
				customer = dontmanage.get_cached_value("Customer", args.customer, ["customer_group", "territory"])
		...
		args.supplier_group = dontmanage.get_cached_value("Supplier", args.supplier, "supplier_group")
...
def apply_price_discount_rule(pricing_rule, item_details, args):
	...
	if pricing_rule.rate_or_discount == "Rate":
		...
		# TODO https://github.com/dontmanage/dontmanageerp/pull/23636 solve this in some other way.
	...
	for apply_on in ["Discount Amount", "Discount Percentage"]:
		...
		field = dontmanage.scrub(apply_on)
...
def remove_pricing_rule_for_item(pricing_rules, item_details, item_code=None, rate=None):
	...
	for d in get_applied_pricing_rules(pricing_rules):
		if not d or not dontmanage.db.exists("Pricing Rule", d):
			continue
		pricing_rule = dontmanage.get_cached_doc("Pricing Rule", d)
...
def make_pricing_rule(doctype, docname):
	doc = dontmanage.new_doc("Pricing Rule")
	doc.applicable_for = doctype
	doc.set(dontmanage.scrub(doctype), docname)
...
def get_item_uoms(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters.get("apply_on") != "Item Code":
		field = dontmanage.scrub(filters.get("apply_on"))
		items = [d.name for d in dontmanage.db.get_all("Item", filters={field: filters.get("value")})]
