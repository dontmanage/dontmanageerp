...
class TestPricingRule(DontManageTestCase):
	def setUp(self):
	...
	def test_pricing_rule_for_discount(self):
		from dontmanage import MandatoryError
		...
		dontmanage.get_doc(test_record.copy()).insert()
		...
		prule = dontmanage.get_doc(test_record.copy())
		prule.priority = 1
		...
		dontmanage.db.sql("update `tabPricing Rule` set priority=NULL where campaign='_Test Campaign'")
		from dontmanageerp.accounts.doctype.pricing_rule.utils import MultiplePricingRuleConflict
	...
	def test_pricing_rule_for_variants(self):
		...
		if not dontmanage.db.exists("Item", "Test Variant PRT"):
			dontmanage.get_doc(
				{
					"doctype": "Item",
	...
	def test_pricing_rule_for_stock_qty(self):
		...
		if not dontmanage.db.get_value("UOM Conversion Detail", {"parent": "_Test Item", "uom": "box"}):
		...
		so.submit()
		so = dontmanage.get_doc("Sales Order", so.name)
	...
	def test_pricing_rule_with_margin_and_discount(self):
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule")
		make_pricing_rule(
	...
	def test_cumulative_pricing_rule(self):
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Cumulative Pricing Rule")
		test_record = {
			"doctype": "Pricing Rule",
			...
			"company": "_Test Company",
			"valid_from": dontmanage.utils.nowdate(),
			"valid_upto": dontmanage.utils.nowdate(),
		}
		dontmanage.get_doc(test_record.copy()).insert()
		...
		args = dontmanage._dict(
			{
				...
				"customer": "_Test Customer",
				"name": None,
				"transaction_date": dontmanage.utils.nowdate(),
			}
		)
		details = get_item_details(args)
	...
	def test_multiple_pricing_rules(self):
		...
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 1")
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 2")
	...
	def test_item_price_with_pricing_rule(self):
		...
		rule = dontmanage.get_doc(pricing_rule_record)
		rule.insert()
		...
		si.delete()
		rule.delete()
		dontmanage.get_doc("Item Price", {"item_code": "Water Flask"}).delete()
		item.delete()
	...
	def test_item_price_with_blank_uom_pricing_rule(self):
		...
		si.delete()
		rule.delete()
		dontmanage.get_doc("Item Price", {"item_code": "Item Blank UOM"}).delete()
	...
	def test_item_price_with_selling_uom_pricing_rule(self):
		...
		si.delete()
		rule.delete()
		dontmanage.get_doc("Item Price", {"item_code": "Item UOM other than Stock"}).delete()

		item.delete()

	def test_item_group_price_with_blank_uom_pricing_rule(self):
		group = dontmanage.get_doc(doctype="Item Group", item_group_name="_Test Pricing Rule Item Group")
		group.save()
		properties = {
		...
		si.delete()
		rule.delete()
		dontmanage.get_doc("Item Price", {"item_code": "Item with Group Blank UOM"}).delete()
		item.delete()
		group.delete()

	def test_item_group_price_with_selling_uom_pricing_rule(self):
		group = dontmanage.get_doc(doctype="Item Group", item_group_name="_Test Pricing Rule Item Group UOM")
		group.save()
		properties = {
		...
		si.delete()
		rule.delete()
		dontmanage.get_doc("Item Price", {"item_code": "Item with Group UOM other than Stock"}).delete()
		item.delete()
		group.delete()
	...
	def test_pricing_rule_for_different_currency(self):
		...
		rule1 = dontmanage.get_doc(pricing_rule_record)
		rule1.currency = "USD"
	...
	def test_multiple_pricing_rules_with_min_qty(self):
		...
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule with Min Qty - 1")
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule with Min Qty - 2")
	...
	def test_apply_multiple_pricing_rules_for_discount_percentage_and_amount(self):
		...
		dontmanage.delete_doc_if_exists("Sales Order", so.name)
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 1")
		dontmanage.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 2")
	...
	def test_pricing_rules_with_and_without_apply_multiple(self):
		...
		for item_group_priority, item_code_priority in [(2, 4), (4, 2)]:
			item_group_rule = dontmanage.get_doc(test_records[0].copy())
			item_group_rule.priority = item_group_priority
			item_group_rule.insert()

			item_code_rule = dontmanage.get_doc(test_records[1].copy())
	...
	def test_validation_on_mixed_condition_with_recursion(self):
		...
		self.assertRaises(dontmanage.ValidationError, pricing_rule.save)
...
def setup_pricing_rule_data():
	if not dontmanage.db.exists("Campaign", "_Test Campaign"):
		dontmanage.get_doc(
...
def delete_existing_pricing_rules():
	for doctype in [
		...
		dontmanage.db.sql(f"delete from `tab{doctype}`")
