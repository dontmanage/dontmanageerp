...
from dontmanage.utils import cint, flt, fmt_money, get_link_to_form, getdate, today

from dontmanageerp.setup.doctype.item_group.item_group import get_child_item_groups
...
class MultiplePricingRuleConflict(dontmanage.ValidationError):
	pass
...
def get_pricing_rules(args, doc=None):
	...
	if not dontmanage.db.exists("Pricing Rule", {"disable": 0, args.transaction_type: 1}):
		return
	...
	if doc:
		for pricing_rule in pricing_rules:
			if pricing_rule.condition:
				try:
					if dontmanage.safe_eval(pricing_rule.condition, None, doc.as_dict()):
...
def _get_pricing_rules(apply_on, args, values):
	apply_on_field = dontmanage.scrub(apply_on)
	...
	if apply_on_field in ["item_code", "brand"]:
		...
		if apply_on_field == "item_code":
			...
			if "variant_of" not in args:
				args.variant_of = dontmanage.get_cached_value("Item", args.item_code, "variant_of")
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	field = dontmanage.scrub(parenttype)
	condition = ""
	if args.get(field):
		if not dontmanage.flags.tree_conditions:
			dontmanage.flags.tree_conditions = {}
		key = (parenttype, args.get(field))
		if key in dontmanage.flags.tree_conditions:
			return dontmanage.flags.tree_conditions[key]

		try:
			lft, rgt = dontmanage.db.get_value(parenttype, args.get(field), ["lft", "rgt"])
		except TypeError:
			dontmanage.throw(_("Invalid {0}").format(args.get(field)))

		parent_groups = dontmanage.db.sql_list(
		...
		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = "parent_{0}".format(dontmanage.scrub(parenttype))
			root_name = dontmanage.db.get_list(
				parenttype,
		...
		if parent_groups:
			...
			condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
				table=table, field=field, parent_groups=", ".join(dontmanage.db.escape(d) for d in parent_groups)
			)

			dontmanage.flags.tree_conditions[key] = condition
	return condition
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if pricing_rules:
		...
		pr_doc = dontmanage.get_cached_doc("Pricing Rule", pricing_rules[0].name)
		...
		if pricing_rules[0].mixed_conditions and doc:
			...
			items = [args.get(dontmanage.scrub(pr_doc.get("apply_on")))]
...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	apply_on = dontmanage.scrub(pr_doc.get("apply_on"))
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=None):
	...
	date_field = (
		"transaction_date" if dontmanage.get_meta(doctype).has_field("transaction_date") else "posting_date"
	)
...
def apply_pricing_rule_on_transaction(doc):
	...
	pricing_rules = dontmanage.db.sql(
			...
			if d.price_or_product_discount == "Price":
				...
				for field in ["additional_discount_percentage", "discount_amount"]:
					...
					if (
						...
						dontmanage.msgprint(_("User has not applied rule on the invoice {0}").format(doc.name))
					else:
						if not d.coupon_code_based:
							doc.set(field, d.get(pr_field))
						elif doc.get("coupon_code"):
							# coupon code based pricing rule
							coupon_code_pricing_rule = dontmanage.db.get_value(
				...
				item_details = dontmanage._dict({"parenttype": doc.doctype, "free_item_data": []})
...
def get_pricing_rule_items(pr_doc, other_items=False) -> list:
	...
	if pr_doc.apply_rule_on_other and other_items:
		apply_on = dontmanage.scrub(pr_doc.apply_rule_on_other)
...
def validate_coupon_code(coupon_name):
	coupon = dontmanage.get_doc("Coupon Code", coupon_name)

	if coupon.valid_from:
		if coupon.valid_from > getdate(today()):
			dontmanage.throw(_("Sorry, this coupon code's validity has not started"))
	elif coupon.valid_upto:
		if coupon.valid_upto < getdate(today()):
			dontmanage.throw(_("Sorry, this coupon code's validity has expired"))
	elif coupon.used >= coupon.maximum_use:
		dontmanage.throw(_("Sorry, this coupon code is no longer valid"))
