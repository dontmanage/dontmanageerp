...
dontmanage.ui.form.on("Accounting Dimension Filter", {
	...
	onload: function (frm) {
		...
		dontmanage.db.get_list("Accounting Dimension", { fields: ["document_type"] }).then((res) => {
	...
	setup_filters: function (frm) {
		...
		if (frm.doc.accounting_dimension) {
			dontmanage.model.with_doctype(frm.doc.accounting_dimension, function () {
				if (dontmanage.model.is_tree(frm.doc.accounting_dimension)) {
					filters["is_group"] = 0;
				}

				if (dontmanage.meta.has_field(frm.doc.accounting_dimension, "company")) {
...
dontmanage.ui.form.on("Allowed Dimension", {
