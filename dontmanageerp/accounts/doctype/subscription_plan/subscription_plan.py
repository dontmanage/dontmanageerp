...
from dontmanage.utils import date_diff, flt, get_first_day, get_last_day, getdate
...
class SubscriptionPlan(Document):
	...
	def validate_interval_count(self):
		if self.billing_interval_count < 1:
			dontmanage.throw(_("Billing Interval Count cannot be less than 1"))
...
def get_plan_rate(
	...
	plan = dontmanage.get_doc("Subscription Plan", plan)
	if plan.price_determination == "Fixed Rate":
		return plan.cost * prorate_factor

	elif plan.price_determination == "Based On Price List":
		if customer:
			customer_group = dontmanage.db.get_value("Customer", customer, "customer_group")
		else:
			customer_group = None
