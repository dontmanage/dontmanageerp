...
from dontmanageerp.accounts.utils import run_ledger_health_checks


class TestLedgerHealth(AccountsTestMixin, DontManageTestCase):
	def setUp(self):
		self.create_company()
	...
	def configure_monitoring_tool(self):
		monitor_settings = dontmanage.get_doc("Ledger Health Monitor")
	...
	def test_debit_credit_mismatch(self):
		...
		dontmanage.db.set_value("GL Entry", gle.name, "credit", 8000)
	...
	def test_gl_and_pl_mismatch(self):
		...
		ple = dontmanage.db.get_all("Payment Ledger Entry", filters={"voucher_no": self.je.name})[0]
		dontmanage.db.set_value("Payment Ledger Entry", ple.name, "amount", 11000)
