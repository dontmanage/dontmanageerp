...
def auto_reconcile_vouchers(
	...
	for transaction in bank_transactions:
		...
		transaction = dontmanage.get_doc("Bank Transaction", transaction.name)
		account = dontmanage.db.get_value("Bank Account", transaction.bank_account, "account")
		matched_trans = 0
		for voucher in vouchers:
			gl_entry = dontmanage.db.get_value(
				"GL Entry",
				dict(
	...
	if matched_transaction_len == 0:
		dontmanage.msgprint(_("No matching references found for auto reconciliation"))
	elif matched_transaction_len == 1:
		dontmanage.msgprint(_("{0} transaction is reconcilied").format(matched_transaction_len))
	else:
		dontmanage.msgprint(_("{0} transactions are reconcilied").format(matched_transaction_len))

	dontmanage.flags.auto_reconcile_vouchers = False

	return dontmanage.get_doc("Bank Transaction", transaction.name)
...
def reconcile_vouchers(bank_transaction_name, vouchers):
	...
	return dontmanage.get_doc("Bank Transaction", bank_transaction_name)
...
def get_ld_matching_query(bank_account, exact_match, filters):
	...
	rank = dontmanage.qb.terms.Case().when(matching_reference, 1).else_(0)

	rank1 = dontmanage.qb.terms.Case().when(matching_party, 1).else_(0)
