...
from dontmanageerp.accounts.doctype.bank_transaction.bank_transaction import get_total_allocated_amount
...
from dontmanageerp.accounts.utils import get_account_currency, get_balance_on
...
def get_bank_transactions(bank_account, from_date=None, to_date=None):
	...
	transactions = dontmanage.get_all(
		"Bank Transaction",
		fields=[
			"date",
			"deposit",
...
def get_account_balance(bank_account, till_date):
	...
	account = dontmanage.db.get_value("Bank Account", bank_account, "account")
	filters = dontmanage._dict({"account": account, "report_date": till_date, "include_pos_transactions": 1})
...
def update_bank_transaction(bank_transaction_name, reference_number, party_type=None, party=None):
	...
	bank_transaction = dontmanage.get_doc("Bank Transaction", bank_transaction_name)
...
def create_journal_entry_bts(
	...
	bank_transaction = dontmanage.db.get_values(
		"Bank Transaction",
		bank_transaction_name,
		fieldname=["name", "deposit", "withdrawal", "bank_account", "currency"],
		as_dict=True,
	)[0]
	company_account = dontmanage.get_value("Bank Account", bank_transaction.bank_account, "account")
	account_type = dontmanage.db.get_value("Account", second_account, "account_type")
	...
	company = dontmanage.get_value("Account", company_account, "company")
	company_default_currency = dontmanage.get_cached_value("Company", company, "default_currency")
	company_account_currency = dontmanage.get_cached_value("Account", company_account, "account_currency")
	second_account_currency = dontmanage.get_cached_value("Account", second_account, "account_currency")
...
def create_payment_entry_bts(
	...
	bank_account = dontmanage.get_cached_value("Bank Account", bank_transaction.bank_account, "account")
	company = dontmanage.get_cached_value("Account", bank_account, "company")
	party_account = get_party_account(party_type, party, company)

	bank_currency = bank_transaction.currency
	party_currency = dontmanage.get_cached_value("Account", party_account, "account_currency")
...
def auto_reconcile_vouchers(
	...
	dontmanage.flags.auto_reconcile_vouchers = True
	...
	dontmanage.msgprint(title=_("Auto Reconciliation"), msg=alert_message, indicator=indicator)

	dontmanage.flags.auto_reconcile_vouchers = False
...
def reconcile_vouchers(bank_transaction_name, vouchers):
	...
	transaction = dontmanage.get_doc("Bank Transaction", bank_transaction_name)
...
def get_linked_payments(
	...
	transaction = dontmanage.get_doc("Bank Transaction", bank_transaction_name)
	bank_account = dontmanage.db.get_values(
...
def check_matching(
	...
	common_filters = dontmanage._dict(
		{
...
def get_queries(
	...
	for method_name in dontmanage.get_hooks("get_matching_queries"):
		queries.extend(
...
def get_bt_matching_query(exact_match, transaction):
	...
	bt = dontmanage.qb.DocType("Bank Transaction")

	field = "deposit" if transaction.withdrawal > 0.0 else "withdrawal"
	amount_equality = getattr(bt, field) == transaction.unallocated_amount
	amount_rank = dontmanage.qb.terms.Case().when(amount_equality, 1).else_(0)
	amount_condition = amount_equality if exact_match else getattr(bt, field) > 0.0

	ref_rank = dontmanage.qb.terms.Case().when(bt.reference_number == transaction.reference_number, 1).else_(0)
	unallocated_rank = (
		dontmanage.qb.terms.Case().when(bt.unallocated_amount == transaction.unallocated_amount, 1).else_(0)
	)

	party_condition = (
		(bt.party_type == transaction.party_type) & (bt.party == transaction.party) & bt.party.isnotnull()
	)
	party_rank = dontmanage.qb.terms.Case().when(party_condition, 1).else_(0)

	query = (
		dontmanage.qb.from_(bt)
		.select(
...
def get_pe_matching_query(
	...
	payment_type = "Receive" if transaction.deposit > 0.0 else "Pay"
	pe = dontmanage.qb.DocType("Payment Entry")

	ref_condition = pe.reference_no == transaction.reference_number
	ref_rank = dontmanage.qb.terms.Case().when(ref_condition, 1).else_(0)
	...
	if dontmanage.flags.auto_reconcile_vouchers is True:
...
def get_si_matching_query(exact_match, currency, common_filters):
	...
	sip = dontmanage.qb.DocType("Sales Invoice Payment")
...
def get_pi_matching_query(exact_match, currency, common_filters):
	...
	purchase_invoice = dontmanage.qb.DocType("Purchase Invoice")
	...
	query = (
		dontmanage.qb.from_(purchase_invoice)
		.select(
			(party_rank + amount_rank + 1).as_("rank"),
