...
class OpeningInvoiceCreationTool(Document):
	...
	def get_opening_invoice_summary(self):
		...
		companies = dontmanage.get_all("Company", fields=["name as company", "default_currency as currency"])
		if not companies:
			return None, None
	...
	def validate_company(self):
		if not self.company:
			dontmanage.throw(_("Please select the Company"))
	...
	def validate_mandatory_invoice_fields(self, row):
		if not dontmanage.db.exists(row.party_type, row.party):
			if self.create_missing_party:
				self.add_party(row.party_type, row.party)
			else:
				dontmanage.throw(
					_("Row #{}: {} {} does not exist.").format(
						row.idx, dontmanage.bold(row.party_type), dontmanage.bold(row.party)
					)
				)
		...
		for d in ("Party", "Outstanding Amount", "Temporary Opening Account"):
			if not row.get(scrub(d)):
				dontmanage.throw(mandatory_error_msg.format(row.idx, d, self.invoice_type))
	...
	def get_invoices(self):
		...
		for row in self.invoices:
			...
			default_currency = dontmanage.db.get_value(row.party_type, row.party, "default_currency")
	...
	def add_party(self, party_type, party):
		party_doc = dontmanage.new_doc(party_type)
		if party_type == "Customer":
			party_doc.customer_name = party
		else:
			supplier_group = dontmanage.db.get_single_value("Buying Settings", "supplier_group")
			if not supplier_group:
				dontmanage.throw(_("Please Set Supplier Group in Buying Settings."))
	...
	def get_invoice_dict(self, row=None):
		def get_item_dict():
			cost_center = row.get("cost_center") or dontmanage.get_cached_value(
				"Company", self.company, "cost_center"
			)
			if not cost_center:
				dontmanage.throw(
					_("Please set the Default Cost Center in {0} company.").format(dontmanage.bold(self.company))
				)

			income_expense_account_field = (
				"income_account" if row.party_type == "Customer" else "expense_account"
			)
			default_uom = dontmanage.db.get_single_value("Stock Settings", "stock_uom") or _("Nos")
			rate = flt(row.outstanding_amount) / flt(row.qty)

			item_dict = dontmanage._dict(
				{
					"uom": default_uom,
					"rate": rate or 0.0,
					"qty": row.qty,
		...
		invoice = dontmanage._dict(
			{
				...
				dontmanage.scrub(row.party_type): row.party,
				"is_pos": 0,
				"doctype": "Sales Invoice" if self.invoice_type == "Sales" else "Purchase Invoice",
				"update_stock": 0,  # important: https://github.com/dontmanage/dontmanageerp/pull/23559
...
def get_temporary_opening_account(company=None):
	...
	accounts = dontmanage.get_all("Account", filters={"company": company, "account_type": "Temporary"})
	if not accounts:
		dontmanage.throw(_("Please add a Temporary Opening account in Chart of Accounts"))
