...
dontmanage.ui.form.on("Opening Invoice Creation Tool", {
	setup: function (frm) {
		...
		dontmanage.realtime.on("opening_invoice_creation_progress", (data) => {
				...
				setTimeout(
					() => {
						...
						if (frm.doc.invoice_type == "Sales") {
							dontmanage.msgprint(__("Opening Sales Invoices have been created."));
						} else {
							dontmanage.msgprint(__("Opening Purchase Invoices have been created."));
						}
					},
					1500,
					data.title
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.opening_invoice_creation_tool.opening_invoice_creation_tool.get_temporary_opening_account",
				args: {
	...
	make_dashboard: function (frm) {
		...
		if (!$.isEmptyObject(opening_invoices_summary)) {
			let section = frm.dashboard.add_section(
				dontmanage.render_template("opening_invoice_creation_tool_dashboard", {
			...
			section.on("click", ".invoice-link", function () {
				...
				dontmanage.set_route("List", doctype, { is_opening: "Yes", company: company, docstatus: 1 });
			});
			frm.dashboard.show();
		}
	},
...
dontmanage.ui.form.on("Opening Invoice Creation Tool Item", {
	invoices_add: (frm) => {
		frm.trigger("update_invoice_table");
	},
});
