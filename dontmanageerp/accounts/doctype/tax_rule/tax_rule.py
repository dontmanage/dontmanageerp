...
class TaxRule(Document):
	...
	def validate_filters(self):
		...
		for d in filters:
			if conds:
				conds += " and "
			conds += f"""ifnull({d}, '') = {dontmanage.db.escape(cstr(filters[d]))}"""
...
def get_tax_template(posting_date, args):
	...
	conditions.append(
		"ifnull(tax_category, '') = {}".format(dontmanage.db.escape(cstr(args.get("tax_category")), False))
	)
	...
	for key, value in args.items():
		if key == "use_for_shopping_cart":
			...
			conditions.append(f"ifnull({key}, '') in ('', {dontmanage.db.escape(cstr(value))})")
...
def get_customer_group_condition(customer_group):
	condition = ""
	customer_groups = ["%s" % (dontmanage.db.escape(d.name)) for d in get_parent_customer_groups(customer_group)]
	if customer_groups:
