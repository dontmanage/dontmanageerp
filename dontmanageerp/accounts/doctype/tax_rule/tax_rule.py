...
from dontmanage.contacts.doctype.address.address import get_default_address
from dontmanage.model.document import Document
from dontmanage.utils import cint, cstr
from dontmanage.utils.nestedset import get_root_of

from dontmanageerp.setup.doctype.customer_group.customer_group import get_parent_customer_groups


class IncorrectCustomerGroup(dontmanage.ValidationError):
	pass


class IncorrectSupplierType(dontmanage.ValidationError):
	pass


class ConflictingTaxRule(dontmanage.ValidationError):
	pass
...
class TaxRule(Document):
	...
	def validate_tax_template(self):
		...
		if not (self.sales_tax_template or self.purchase_tax_template):
			dontmanage.throw(_("Tax Template is mandatory."))
	...
	def validate_filters(self):
		...
		for d in filters:
			if conds:
				conds += " and "
			conds += """ifnull({0}, '') = {1}""".format(d, dontmanage.db.escape(cstr(filters[d])))
		...
		if tax_rule:
			if tax_rule[0].priority == self.priority:
				dontmanage.throw(_("Tax Rule Conflicts with {0}").format(tax_rule[0].name), ConflictingTaxRule)

	def validate_use_for_shopping_cart(self):
		"""If shopping cart is enabled and no tax rule exists for shopping cart, enable this one"""
		if (
			not self.use_for_shopping_cart
			and cint(dontmanage.db.get_single_value("E Commerce Settings", "enabled"))
			and not dontmanage.db.get_value("Tax Rule", {"use_for_shopping_cart": 1, "name": ["!=", self.name]})
		):
...
def get_party_details(party, party_type, args=None):
	...
	if args:
		if args.get("billing_address"):
			billing_address = dontmanage.get_doc("Address", args.get("billing_address"))
		if args.get("shipping_address"):
			shipping_address = dontmanage.get_doc("Address", args.get("shipping_address"))
	else:
		...
		if billing_address_name:
			billing_address = dontmanage.get_doc("Address", billing_address_name)
		if shipping_address_name:
			shipping_address = dontmanage.get_doc("Address", shipping_address_name)
...
def get_tax_template(posting_date, args):
	...
	conditions.append(
		"ifnull(tax_category, '') = {0}".format(dontmanage.db.escape(cstr(args.get("tax_category"))))
	)
	...
	for key, value in args.items():
		if key == "use_for_shopping_cart":
			...
			conditions.append("ifnull({0}, '') in ('', {1})".format(key, dontmanage.db.escape(cstr(value))))
	...
	if dontmanage.db.get_value(doctype, tax_template, "disabled") == 1:
		return None
...
def get_customer_group_condition(customer_group):
	condition = ""
	customer_groups = [
		"%s" % (dontmanage.db.escape(d.name)) for d in get_parent_customer_groups(customer_group)
	]
	if customer_groups:
