...
dontmanage.ui.form.on('Subscription', {
	setup: function(frm) {
	...
	cancel_this_subscription: function(frm) {
		...
		dontmanage.confirm(
			...
			function() {
				dontmanage.call({
					method:
					"dontmanageerp.accounts.doctype.subscription.subscription.cancel_subscription",
					args: {name: doc.name},
	...
	renew_this_subscription: function(frm) {
		...
		dontmanage.confirm(
			...
			function() {
				dontmanage.call({
					method:
					"dontmanageerp.accounts.doctype.subscription.subscription.restart_subscription",
					args: {name: doc.name},
					callback: function(data){
						if(!data.exc){
							frm.reload_doc();
	...
	get_subscription_updates: function(frm) {
		...
		dontmanage.call({
			method:
			"dontmanageerp.accounts.doctype.subscription.subscription.get_subscription_updates",
			args: {name: doc.name},
			freeze: true,
			callback: function(data){
				if(!data.exc){
