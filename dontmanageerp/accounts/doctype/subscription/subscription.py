...
class Subscription(Document):
	...
	def set_status_grace_period(self):
		...
		"""
		subscription_settings = dontmanage.get_single("Subscription Settings")
	...
	def get_current_invoice(self):
		...
		if len(self.invoices):
			...
			if dontmanage.db.exists(doctype, current.get("invoice")):
				doc = dontmanage.get_doc(doctype, current.get("invoice"))
				return doc
			else:
				dontmanage.throw(_("Invoice {0} no longer exists").format(current.get("invoice")))
	...
	def validate(self):
		...
		self.cost_center = dontmanageerp.get_default_cost_center(self.get("company"))
	...
	def create_invoice(self, prorate):
		...
		invoice = dontmanage.new_doc(doctype)
	...
	def process_for_active(self):
		...
		if not self.is_current_invoice_generated(
			...
			prorate = dontmanage.db.get_single_value("Subscription Settings", "prorate")
	...
	def process_for_past_due_date(self):
		...
		current_invoice = self.get_current_invoice()
		if not current_invoice:
			dontmanage.throw(_("Current invoice {0} is missing").format(current_invoice.invoice))
		else:
	...
	def has_outstanding_invoice(self):
		...
		outstanding_invoices = dontmanage.get_all(
	...
	def cancel_subscription(self):
		...
		if self.status != "Cancelled":
			...
			to_prorate = dontmanage.db.get_single_value("Subscription Settings", "prorate")
			self.status = "Cancelled"
			self.cancelation_date = nowdate()
	...
	def restart_subscription(self):
		...
		if self.status == "Cancelled":
			...
			dontmanage.throw(_("You cannot restart a Subscription that is not cancelled."))
...
def get_all_subscriptions():
	...
	"""
	return dontmanage.db.get_all("Subscription", {"status": ("!=", "Cancelled")})
...
def process(data):
	...
	if data:
		try:
			subscription = dontmanage.get_doc("Subscription", data["name"])
...
def cancel_subscription(name):
	...
	"""
	subscription = dontmanage.get_doc("Subscription", name)
