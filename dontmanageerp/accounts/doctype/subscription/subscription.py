...
from dontmanageerp import get_default_company, get_default_cost_center
...
class InvoiceCancelled(dontmanage.ValidationError):
	pass


class InvoiceNotCancelled(dontmanage.ValidationError):
	pass
...
class Subscription(Document):
	...
	def validate_plans_billing_cycle(billing_cycle_data: list[dict[str, str]]) -> None:
		...
		if billing_cycle_data and len(billing_cycle_data) != 1:
			dontmanage.throw(_("You can only have Plans with the same billing cycle in a Subscription"))
	...
	def get_billing_cycle_and_interval(self) -> list[dict[str, str]]:
		...
		subscription_plan = dontmanage.qb.DocType("Subscription Plan")
		billing_info = (
			dontmanage.qb.from_(subscription_plan)
	...
	def get_status_for_past_grace_period(self) -> str:
		cancel_after_grace = cint(dontmanage.get_value("Subscription Settings", None, "cancel_after_grace"))
		status = "Unpaid"
	...
	def is_past_grace_period(self, posting_date: DateTimeLikeObject | None = None) -> bool:
		...
		grace_period = cint(dontmanage.get_value("Subscription Settings", None, "grace_period"))
	...
	def validate_party_billing_currency(self):
		...
		"""
		if self.party:
			party_billing_currency = dontmanage.get_cached_value(
				self.party_type, self.party, "default_currency"
			) or dontmanage.get_cached_value("Company", self.company, "default_currency")

			plans = [x.plan for x in self.plans]
			subscription_plan_currencies = dontmanage.db.get_all(
			...
			if unsupported_plans:
				unsupported_plans = [
					...
					).format(dontmanage.bold(party_billing_currency)),
					*unsupported_plans,
				]

				dontmanage.throw(
					unsupported_plans, dontmanage.ValidationError, "Unsupported Subscription Plans", as_list=True
				)
	...
	def validate_trial_period(self) -> None:
		...
		if self.trial_period_start and self.trial_period_end:
			if getdate(self.trial_period_end) < getdate(self.trial_period_start):
				dontmanage.throw(_("Trial Period End Date Cannot be before Trial Period Start Date"))

		if self.trial_period_start and not self.trial_period_end:
			dontmanage.throw(_("Both Trial Period Start Date and Trial Period End Date must be set"))

		if self.trial_period_start and getdate(self.trial_period_start) > getdate(self.start_date):
			dontmanage.throw(_("Trial Period Start date cannot be after Subscription Start Date"))
	...
	def validate_to_follow_calendar_months(self) -> None:
		...
		if not self.end_date:
			dontmanage.throw(_("Subscription End Date is mandatory to follow calendar months"))

		if billing_info[0]["billing_interval"] != "Month":
			dontmanage.throw(_("Billing Interval in Subscription Plan must be Month to follow calendar months"))
	...
	def create_invoice(
		...
		invoice = dontmanage.new_doc(self.invoice_document_type)
		...
		if self.invoice_document_type == "Sales Invoice":
			...
			if dontmanage.db.get_value("Supplier", self.party, "tax_withholding_category"):
				invoice.apply_tds = 1
	...
	def get_items_from_plans(self, plans: list[dict[str, str]], prorate: bool | None = None) -> list[dict]:
		...
		party = self.party
		for plan in plans:
			plan_doc = dontmanage.get_doc("Subscription Plan", plan.plan)
			...
			deferred = dontmanage.db.get_value("Item", item_code, deferred_field)
	...
	def get_current_invoice(self) -> Document | None:
		...
		if invoice:
			return dontmanage.get_doc(self.invoice_document_type, invoice[0])
	...
	def cancel_subscription(self) -> None:
		...
		if self.status == "Cancelled":
			dontmanage.throw(_("subscription is already cancelled."), InvoiceCancelled)
	...
	def restart_subscription(self, posting_date: DateTimeLikeObject | None = None) -> None:
		...
		if not self.status == "Cancelled":
			dontmanage.throw(_("You cannot restart a Subscription that is not cancelled."), InvoiceNotCancelled)
	...
	def force_fetch_subscription_updates(self):
		...
		if nowdate() < self.current_invoice_start:
			dontmanage.msgprint(_("Subscription for Future dates cannot be processed."))
			return
...
def is_prorate() -> int:
	return cint(dontmanage.db.get_single_value("Subscription Settings", "prorate"))
...
def process_all(subscription: str | None = None, posting_date: DateTimeLikeObject | None = None) -> None:
	...
	for subscription in dontmanage.get_all("Subscription", filters, pluck="name"):
		try:
			subscription = dontmanage.get_doc("Subscription", subscription)
