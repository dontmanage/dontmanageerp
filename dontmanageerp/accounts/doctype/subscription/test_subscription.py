...
from dontmanageerp.accounts.doctype.subscription.subscription import get_prorata_factor

test_dependencies = ("UOM", "Item Group", "Item")


class TestSubscription(DontManageTestCase):
	def setUp(self):
		make_plans()
		create_parties()
		reset_settings()
	...
	def test_create_subscription_trial_with_wrong_dates(self):
		...
		self.assertRaises(dontmanage.ValidationError, subscription.save)
	...
	def test_subscription_cancel_after_grace_period(self):
		settings = dontmanage.get_single("Subscription Settings")
	...
	def test_restart_active_subscription(self):
		subscription = create_subscription()
		self.assertRaises(dontmanage.ValidationError, subscription.restart_subscription)
	...
	def test_subscription_with_follow_calendar_months(self):
		subscription = dontmanage.new_doc("Subscription")
	...
	def test_multi_currency_subscription(self):
		...
		dontmanage.db.set_value("Customer", party, "default_currency", "USD")
		subscription = create_subscription(
			start_date="2018-01-01",
			generate_invoice_at="Beginning of the current subscription period",
		...
		currency = dontmanage.db.get_value("Sales Invoice", subscription.invoices[0].name, "currency")
	...
	def test_subscription_recovery(self):
		...
		self.assertEqual(len(subscription.invoices), 2)
		self.assertEqual(
			getdate(dontmanage.db.get_value("Sales Invoice", subscription.invoices[0].name, "from_date")),
			getdate("2021-01-01"),
		)
		self.assertEqual(
			getdate(dontmanage.db.get_value("Sales Invoice", subscription.invoices[1].name, "from_date")),
			getdate("2021-02-01"),
		)
...
def create_plan(**kwargs):
	if not dontmanage.db.exists("Subscription Plan", kwargs.get("plan_name")):
		plan = dontmanage.new_doc("Subscription Plan")
...
def create_parties():
	if not dontmanage.db.exists("Supplier", "_Test Supplier"):
		supplier = dontmanage.new_doc("Supplier")
		supplier.supplier_name = "_Test Supplier"
		supplier.supplier_group = "All Supplier Groups"
		supplier.insert()

	if not dontmanage.db.exists("Customer", "_Test Subscription Customer"):
	...
	if not dontmanage.db.exists("Customer", "_Test Subscription Customer John Doe"):
		customer = dontmanage.new_doc("Customer")
