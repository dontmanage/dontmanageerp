...
dontmanage.ui.form.on('Payment Order', {
	setup: function(frm) {
	...
	get_from_payment_entry: function(frm) {
		...
		dontmanageerp.utils.map_current_doc({
			method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.make_payment_order",
	...
	get_from_payment_request: function(frm) {
		frm.trigger("remove_row_if_empty");
		dontmanageerp.utils.map_current_doc({
			method: "dontmanageerp.accounts.doctype.payment_request.payment_request.make_payment_order",
		...
		var dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				{"fieldtype": "Link", "label": __("Supplier"), "fieldname": "supplier", "options":"Supplier",
					"get_query": function () {
						return {
							query:"dontmanageerp.accounts.doctype.payment_order.payment_order.get_supplier_query",
				...
				{"fieldtype": "Link", "label": __("Mode of Payment"), "fieldname": "mode_of_payment", "options":"Mode of Payment",
					"get_query": function () {
						return {
							query:"dontmanageerp.accounts.doctype.payment_order.payment_order.get_mop_query",
							filters: {'parent': frm.doc.name}
						}
					}
				}
