...
class PaymentOrder(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.payment_order_reference.payment_order_reference import (
	...
	def update_payment_status(self, cancel=False):
		...
		if self.payment_order_type == "Payment Request":
			ref_field = "status"
			ref_doc_field = dontmanage.scrub(self.payment_order_type)
		else:
			ref_field = "payment_order_status"
			ref_doc_field = "reference_name"

		for d in self.references:
			dontmanage.db.set_value(self.payment_order_type, d.get(ref_doc_field), ref_field, status)
...
def make_payment_records(name, supplier, mode_of_payment=None):
	doc = dontmanage.get_doc("Payment Order", name)
...
def make_journal_entry(doc, supplier, mode_of_payment=None):
	...
	mode_of_payment_type = dontmanage._dict(dontmanage.get_all("Mode of Payment", fields=["name", "type"], as_list=1))
	...
	je.save()
	dontmanage.msgprint(_("{0} {1} created").format(je.doctype, je.name))
