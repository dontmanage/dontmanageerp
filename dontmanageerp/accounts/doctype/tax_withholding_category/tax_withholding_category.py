...
class TaxWithholdingCategory(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.tax_withholding_account.tax_withholding_account import (
			TaxWithholdingAccount,
		)
		from dontmanageerp.accounts.doctype.tax_withholding_rate.tax_withholding_rate import TaxWithholdingRate
	...
	def validate_dates(self):
		...
		for d in self.get("rates"):
			if getdate(d.from_date) >= getdate(d.to_date):
				dontmanage.throw(_("Row #{0}: From Date cannot be before To Date").format(d.idx))

			# validate overlapping of dates
			if last_date and getdate(d.to_date) < getdate(last_date):
				dontmanage.throw(_("Row #{0}: Dates overlapping with other row").format(d.idx))

	def validate_accounts(self):
		existing_accounts = []
		for d in self.get("accounts"):
			if d.get("account") in existing_accounts:
				dontmanage.throw(_("Account {0} added multiple times").format(dontmanage.bold(d.get("account"))))
...
def get_party_details(inv):
	...
	if not party:
		dontmanage.throw(_("Please select {0} first").format(party_type))
...
def get_party_tax_withholding_details(inv, tax_withholding_category=None):
	...
	has_pan_field = dontmanage.get_meta(party_type).has_field("pan")
	...
	if not tax_withholding_category:
		...
		tax_withholding_details = dontmanage.db.get_value(party_type, party, fields, as_dict=1)
	...
	if not pan_no and has_pan_field:
		pan_no = dontmanage.db.get_value(party_type, party, "pan")

	# Get others suppliers with the same PAN No
	if pan_no:
		parties = dontmanage.get_all(party_type, filters={"pan": pan_no}, pluck="name")
...
def get_cost_center(inv):
	cost_center = dontmanage.get_cached_value("Company", inv.company, "cost_center")
...
def get_tax_withholding_details(tax_withholding_category, posting_date, company):
	tax_withholding = dontmanage.get_doc("Tax Withholding Category", tax_withholding_category)
...
def get_tax_withholding_rates(tax_withholding, posting_date):
	...
	dontmanage.throw(_("No Tax Withholding data found for the current posting date."))
...
def get_lower_deduction_certificate(company, posting_date, tax_details, pan_no):
	ldc_name = dontmanage.db.get_value(
	...
	if ldc_name:
		return dontmanage.get_doc("Lower Deduction Certificate", ldc_name)
...
def get_invoice_vouchers(parties, tax_details, company, party_type="Supplier"):
	...
	filters = {
		"company": company,
		dontmanage.scrub(party_type): ["in", parties],
	...
	invoices_details = dontmanage.get_all(doctype, filters=filters, fields=["name", field])

	for d in invoices_details:
		vouchers.append(d.name)
		voucher_wise_amount.update({d.name: {"amount": d.base_net_total, "voucher_type": doctype}})

	journal_entries_details = dontmanage.db.sql(
		"""
...
def get_payment_entry_vouchers(parties, tax_details, company, party_type="Supplier"):
	...
	return dontmanage.db.get_all("Payment Entry", filters=payment_entry_filters, pluck="name")
...
def get_taxes_deducted_on_advances_allocated(inv, tax_details):
	...
	if inv.get("advances"):
		...
		if advances:
			pe = dontmanage.qb.DocType("Payment Entry").as_("pe")
			at = dontmanage.qb.DocType("Advance Taxes and Charges").as_("at")

			tax_info = (
				dontmanage.qb.from_(at)
				.inner_join(pe)
...
def get_deducted_tax(taxable_vouchers, tax_details):
	...
	entries = dontmanage.db.get_all("GL Entry", filters, pluck=field)
	return sum(entries)
...
def get_tds_amount(ldc, parties, inv, tax_details, vouchers):
	...
	supp_inv_credit_amt = dontmanage.db.get_value("Purchase Invoice", invoice_filters, field) or 0.0
	...
	payment_entry_amounts = dontmanage.db.get_all(
		"Payment Entry",
	...
	if (threshold and tax_withholding_net_total >= threshold) or (
		...
		net_total = (
			dontmanage.db.get_value("Purchase Invoice", invoice_filters, "sum(tax_withholding_net_total)") or 0.0
		)
...
def get_limit_consumed(ldc, parties):
	limit_consumed = dontmanage.db.get_value(
		"Purchase Invoice",
		{
			"supplier": ("in", parties),
			"apply_tds": 1,
