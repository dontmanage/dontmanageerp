...
class TestTaxWithholdingCategory(DontManageTestCase):
	@classmethod
	def setUpClass(self):
	...
	def test_single_threshold_tds(self):
		...
		gl_entries = dontmanage.db.get_all(
			"GL Entry",
	...
	def test_tax_withholding_category_checks(self):
		invoices = []
		dontmanage.db.set_value("Supplier", "Test TDS Supplier3", "tax_withholding_category", "New TDS Category")
			...
			[
				{
					...
					"item_code": dontmanage.db.get_value("Item", {"item_name": "TDS Item"}, "name"),
					"qty": 1,
					"rate": 20000,
					"cost_center": "Main - _TC",
	...
	def set_previous_fy_and_tax_category(self):
		...
		if prev_fiscal_year := get_fiscal_year(add_days(fiscal_year[1], -10)):
			self.prev_fy = dontmanage.get_doc("Fiscal Year", prev_fiscal_year[0])
			...
			self.prev_fy = dontmanage.get_doc(
				{
					"doctype": "Fiscal Year",
		...
		cat = dontmanage.get_doc("Tax Withholding Category", category)
		cat.append(
			"rates",
			{
...
def cancel_invoices():
	purchase_invoices = dontmanage.get_all(
		"Purchase Invoice",
		{
	...
	sales_invoices = dontmanage.get_all(
		"Sales Invoice", {"customer": "Test TCS Customer", "docstatus": 1}, pluck="name"
	)

	for d in purchase_invoices:
		dontmanage.get_doc("Purchase Invoice", d).cancel()

	for d in sales_invoices:
		dontmanage.get_doc("Sales Invoice", d).cancel()


def create_purchase_invoice(**args):
	# return sales invoice doc object
	item = dontmanage.db.get_value("Item", {"item_name": "TDS Item"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	item = dontmanage.db.get_value("Item", {"item_name": "TCS Item"}, "name")

	args = dontmanage._dict(args)
	si = dontmanage.get_doc(
		{
...
def create_records():
	...
	for name in [
		...
		if dontmanage.db.exists("Supplier", name):
			continue
	...
	# create item
	if not dontmanage.db.exists("Item", "TDS Item"):
		dontmanage.get_doc(
			{
				"doctype": "Item",
				"item_code": "TDS Item",
	...
	if not dontmanage.db.exists("Item", "TCS Item"):
		dontmanage.get_doc(
			{
				"doctype": "Item",
				"item_code": "TCS Item",
	...
	# create tds account
	if not dontmanage.db.exists("Account", "TDS - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	# create tcs account
	if not dontmanage.db.exists("Account", "TCS - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_tax_withholding_category(
	...
	if not dontmanage.db.exists("Tax Withholding Category", category_name):
		dontmanage.get_doc(
			{
...
def create_lower_deduction_certificate(supplier, tax_withholding_category, tax_rate, certificate_no, limit):
	fiscal_year = get_fiscal_year(today(), company="_Test Company")
	if not dontmanage.db.exists("Lower Deduction Certificate", certificate_no):
		dontmanage.get_doc(
			{
