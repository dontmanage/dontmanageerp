...
dontmanage.ui.form.on('POS Settings', {
	onload: function(frm) {
	...
	get_invoice_fields: function(frm) {
		dontmanage.model.with_doctype("POS Invoice", () => {
			var fields = $.map(dontmanage.get_doc("DocType", "POS Invoice").fields, function(d) {
				if (dontmanage.model.no_value_type.indexOf(d.fieldtype) === -1 || ['Button'].includes(d.fieldtype)) {
	...
	add_search_options: function(frm) {
		dontmanage.model.with_doctype("Item", () => {
			var fields = $.map(dontmanage.get_doc("DocType", "Item").fields, function(d) {
...
dontmanage.ui.form.on("POS Search Fields", {
	field: function(frm, doctype, name) {
		var doc = dontmanage.get_doc(doctype, name);
		var df = $.map(dontmanage.get_doc("DocType", "Item").fields, function(d) {
...
dontmanage.ui.form.on("POS Field", {
	fieldname: function(frm, doctype, name) {
		var doc = dontmanage.get_doc(doctype, name);
		var df = $.map(dontmanage.get_doc("DocType", "POS Invoice").fields, function(d) {
