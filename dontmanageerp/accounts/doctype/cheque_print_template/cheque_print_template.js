...
dontmanage.provide("dontmanageerp.cheque_print");

dontmanage.ui.form.on('Cheque Print Template', {
	refresh: function(frm) {
		if(!frm.doc.__islocal) {
			frm.add_custom_button(frm.doc.has_print_format?__("Update Print Format"):__("Create Print Format"),
				function() {
					dontmanageerp.cheque_print.view_cheque_print(frm);
			...
			var template = '<div style="position: relative; overflow-x: scroll;">\
				<div id="cheque_preview" style="width: {{ cheque_width }}cm; \
					...
					<span style="top: {{ date_dist_from_top_edge }}cm;\
						...
						position: absolute;"> {{ dontmanage.datetime.obj_to_user() }} </span>\
			...
			$(dontmanage.render(template, frm.doc)).appendTo(frm.fields_dict.cheque_print_preview.wrapper)
...
dontmanageerp.cheque_print.view_cheque_print = function(frm) {
	dontmanage.call({
		method: "dontmanageerp.accounts.doctype.cheque_print_template.cheque_print_template.create_or_update_cheque_print_format",
		args:{
		...
		callback: function(r) {
			if (!r.exe && !frm.doc.has_print_format) {
				...
				dontmanage.msgprint(__("Print settings updated in respective print format"))
			}
		}
	})
}
