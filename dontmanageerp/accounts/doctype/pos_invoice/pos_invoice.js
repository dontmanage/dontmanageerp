...
extend_cscript(cur_frm.cscript, new dontmanageerp.selling.POSInvoiceController({ frm: cur_frm }))

dontmanage.ui.form.on('POS Invoice', {
	...
	request_for_payment: function (frm) {
		if (!frm.doc.contact_mobile) {
			dontmanage.throw(__('Please enter mobile number first.'));
		}
		frm.dirty();
		frm.save().then(() => {
			dontmanage.dom.freeze(__('Waiting for payment...'));
			dontmanage
				.call({
				...
				.fail(() => {
					...
					dontmanage.msgprint(__('Payment request failed'));
				})
				.then(({ message }) => {
					const payment_request_name = message.name;
					setTimeout(() => {
						dontmanage.db.get_value('Payment Request', payment_request_name, ['status', 'grand_total']).then(({ message }) => {
							...
							} else if (dontmanage.dom.freeze_count != 0) {
								dontmanage.dom.unfreeze();
								cur_frm.reload_doc();
