...
class TestPOSInvoice(unittest.TestCase):
	...
	def tearDown(self):
		...
		if dontmanage.db.get_single_value("Selling Settings", "validate_selling_price"):
			dontmanage.db.set_value("Selling Settings", None, "validate_selling_price", 0)
	...
	def test_change_naming_series(self):
		...
		self.assertRaises(dontmanage.CannotChangeConstantError, inv.save)
	...
	def test_without_payment(self):
		...
		inv.payments = []
		self.assertRaises(dontmanage.ValidationError, inv.insert)
	...
	def test_serialized_item_transaction(self):
		...
		pos2.insert()
		self.assertRaises(dontmanage.ValidationError, pos2.submit)
	...
	def test_invalid_serial_no_validation(self):
		...
		self.assertRaises(dontmanage.ValidationError, pos.submit)
	...
	def test_value_error_on_serial_no_validation(self):
		...
		dontmanage.db.set_value("POS Invoice", pos.name, "docstatus", 2)
	...
	def test_loyalty_points(self):
		...
		from dontmanageerp.accounts.doctype.loyalty_program.test_loyalty_program import create_records
	...
	def test_merging_into_sales_invoice_with_discount(self):
		from dontmanageerp.accounts.doctype.pos_closing_entry.test_pos_closing_entry import (
		...
		dontmanage.db.sql("delete from `tabPOS Invoice`")
		...
		rounded_total = dontmanage.db.get_value(
	...
	def test_merging_with_validate_selling_price(self):
		...
		if not dontmanage.db.get_single_value("Selling Settings", "validate_selling_price"):
			dontmanage.db.set_value("Selling Settings", "Selling Settings", "validate_selling_price", 1)
		...
		self.assertRaises(dontmanage.ValidationError, pos_inv.submit)
	...
	def test_pos_batch_item_qty_validation(self):
		...
		item = dontmanage.get_doc("Item", "_BATCH ITEM")
		batch = dontmanage.get_doc("Batch", "TestBatch 01")
		batch.submit()
		...
		self.assertRaises(dontmanage.ValidationError, pos_inv2.submit)
	...
	def test_ignore_pricing_rule(self):
		from dontmanageerp.accounts.doctype.pricing_rule.test_pricing_rule import make_pricing_rule
	...
	def test_delivered_serial_no_case(self):
		from dontmanageerp.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
			init_user_and_profile,
		)
		from dontmanageerp.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from dontmanageerp.stock.doctype.serial_no.test_serial_no import get_serial_nos
		from dontmanageerp.stock.doctype.stock_entry.test_stock_entry import make_serialized_item

		dontmanage.db.savepoint("before_test_delivered_serial_no_case")
		try:
			se = make_serialized_item()
			...
			delivery_document_no = dontmanage.db.get_value("Serial No", serial_no, "delivery_document_no")
			...
			dontmanage.db.rollback(save_point="before_test_delivered_serial_no_case")
			dontmanage.set_user("Administrator")
	...
	def test_returned_serial_no_case(self):
		...
		from dontmanageerp.stock.doctype.serial_no.serial_no import get_pos_reserved_serial_nos
		from dontmanageerp.stock.doctype.serial_no.test_serial_no import get_serial_nos
		from dontmanageerp.stock.doctype.stock_entry.test_stock_entry import make_serialized_item

		dontmanage.db.savepoint("before_test_returned_serial_no_case")
		try:
			se = make_serialized_item()
			serial_no = get_serial_nos(se.get("items")[0].serial_no)[0]
			...
			dontmanage.db.rollback(save_point="before_test_returned_serial_no_case")
			dontmanage.set_user("Administrator")
...
def create_pos_invoice(**args):
	...
	pos_inv = dontmanage.new_doc("POS Invoice")
	pos_inv.update(args)
	...
	pos_inv.posting_date = args.posting_date or dontmanage.utils.nowdate()
...
def make_batch_item(item_name):
	...
	if not dontmanage.db.exists(item_name):
