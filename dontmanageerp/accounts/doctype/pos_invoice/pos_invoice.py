...
from dontmanageerp.controllers.queries import item_query as _item_query
...
class POSInvoice(SalesInvoice):
	...
	def validate(self):
		if not cint(self.is_pos):
			dontmanage.throw(
				_("POS Invoice should have the field {0} checked.").format(dontmanage.bold(_("Include Payment")))
			)
	...
	def on_submit(self):
		...
		if not self.is_return and self.loyalty_program:
			...
			against_psi_doc = dontmanage.get_doc("POS Invoice", self.return_against)
	...
	def before_cancel(self):
		if (
			...
			and dontmanage.db.get_value("Sales Invoice", self.consolidated_invoice, "docstatus") == 1
		):
	...
	def check_phone_payments(self):
		for pay in self.payments:
			if pay.type == "Phone" and pay.amount >= 0:
				paid_amt = dontmanage.db.get_value(
					"Payment Request",
					filters=dict(
	...
	def validate_stock_availablility(self):
		...
		if self.docstatus.is_draft() and not dontmanage.db.get_value(
		...
		for d in self.get("items"):
			if not d.serial_and_batch_bundle:
				...
				item_code, warehouse, _qty = (
					...
					dontmanage.bold(d.warehouse),
					dontmanage.bold(d.qty),
				)
		...
		for d in self.get("items"):
			...
			if d.get("serial_no"):
				...
				for sr in serial_nos:
					serial_no_exists = dontmanage.db.sql(
						"""
						SELECT name
					...
					if not serial_no_exists:
						bold_return_against = dontmanage.bold(self.return_against)
						bold_serial_no = dontmanage.bold(sr)
						dontmanage.throw(
							_(
	...
	def validate_change_account(self):
		if (
			...
			and dontmanage.get_cached_value("Account", self.account_for_change_amount, "company") != self.company
		):
			dontmanage.throw(
	...
	def validate_change_amount(self):
		...
		if flt(self.change_amount) and not self.account_for_change_amount:
			dontmanage.msgprint(_("Please enter Account for Change Amount"), raise_exception=1)
	...
	def validate_loyalty_transaction(self):
		if self.redeem_loyalty_points and (
			...
			expense_account, cost_center = dontmanage.db.get_value(
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.pos_profile:
			...
			if not pos_profile:
				dontmanage.throw(_("No POS Profile found. Please create a New POS Profile first"))
			...
			for item in self.get("items"):
				if item.get("item_code"):
					profile_details = get_pos_profile_item_details(
						profile.get("company"), dontmanage._dict(item.as_dict()), profile
					)
	...
	def create_payment_request(self):
		for pay in self.payments:
			if pay.type == "Phone":
				if pay.amount <= 0:
					dontmanage.throw(_("Payment amount cannot be less than or equal to 0"))

				if not self.contact_mobile:
					dontmanage.throw(_("Please enter the phone number first"))
	...
	def get_existing_payment_request(self, pay):
		...
		pr = dontmanage.db.get_value("Payment Request", filters=filters)
		if pr:
			return dontmanage.get_doc("Payment Request", pr)


@dontmanage.whitelist()
def get_stock_availability(item_code, warehouse):
	if dontmanage.db.get_value("Item", item_code, "is_stock_item"):
		is_stock_item = True
...
def get_bundle_availability(bundle_item_code, warehouse):
	product_bundle = dontmanage.get_doc("Product Bundle", bundle_item_code)
...
def make_merge_log(invoices):
	...
	if len(invoices) == 0:
		dontmanage.throw(_("Atleast one invoice has to be selected."))

	merge_log = dontmanage.new_doc("POS Invoice Merge Log")
	merge_log.posting_date = getdate(nowdate())
	for inv in invoices:
		inv_data = dontmanage.db.get_values(
...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	if pos_profile := filters.get("pos_profile")[1]:
		pos_profile = dontmanage.get_cached_doc("POS Profile", pos_profile)
