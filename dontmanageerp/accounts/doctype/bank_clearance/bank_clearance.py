...
from dontmanage.utils import flt, fmt_money, get_link_to_form, getdate
...
class BankClearance(Document):
	...
	def get_payment_entries(self):
		...
		default_currency = dontmanageerp.get_default_currency()
		...
		for d in self.get("payment_entries"):
			if d.clearance_date:
				if not d.payment_document:
					dontmanage.throw(_("Row #{0}: Payment document is required to complete the transaction"))
			...
			if d.clearance_date or self.include_reconciled_entries:
				...
				if d.payment_document == "Sales Invoice":
					...
					payment_entry = dontmanage.get_doc(d.payment_document, d.payment_entry)
