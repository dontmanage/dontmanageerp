...
dontmanage.ui.form.on("Exchange Rate Revaluation", {
	setup: function (frm) {
	...
	validate_rounding_loss: function (frm) {
		...
		if (!(allowance >= 0 && allowance < 1)) {
			dontmanage.throw(__("Rounding Loss Allowance should be between 0 and 1"));
		}
	},
			...
			callback: function (r) {
				if (r.message) {
					...
					if (response["revaluation_jv"] || response["zero_balance_jv"]) {
						dontmanage.msgprint(__("Journal entries have been created"));
					}
				}
			},
		});
...
dontmanage.ui.form.on("Exchange Rate Revaluation Account", {
...
var get_account_details = function (frm, cdt, cdn) {
	...
	if (!frm.doc.company || !frm.doc.posting_date) {
		dontmanage.throw(__("Please select Company and Posting Date to getting entries"));
	}
	dontmanage.call({
		method: "dontmanageerp.accounts.doctype.exchange_rate_revaluation.exchange_rate_revaluation.get_account_details",
		args: {
			account: row.account,
