...
from dontmanage.query_builder import Criterion, Order
from dontmanage.query_builder.functions import NullIf, Sum
from dontmanage.utils import flt, get_link_to_form

import dontmanageerp
from dontmanageerp.accounts.doctype.journal_entry.journal_entry import get_balance_on
...
class ExchangeRateRevaluation(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.exchange_rate_revaluation_account.exchange_rate_revaluation_account import (
	...
	def validate_rounding_loss_allowance(self):
		if not (self.rounding_loss_allowance >= 0 and self.rounding_loss_allowance < 1):
			dontmanage.throw(_("Rounding Loss Allowance should be between 0 and 1"))
	...
	def validate_mandatory(self):
		if not (self.company and self.posting_date):
			dontmanage.throw(_("Please select Company and Posting Date to getting entries"))
	...
	def remove_accounts_without_gain_loss(self):
		...
		if not self.accounts:
			dontmanage.throw(_("At least one account with exchange gain or loss is required"))
	...
	def calculate_new_account_balance(company, posting_date, account_details):
		...
		precision = get_field_precision(
			dontmanage.get_meta("Exchange Rate Revaluation Account").get_field("new_balance_in_base_currency"),
			company_currency,
		)
	...
	def get_for_unrealized_gain_loss_account(self):
		unrealized_exchange_gain_loss_account = dontmanage.get_cached_value(
	...
	def make_jv_entries(self):
		...
		if revaluation_jv:
			dontmanage.msgprint(f"Revaluation Journal: {get_link_to_form('Journal Entry', revaluation_jv.name)}")
...
def get_account_details(
	...
	if not (company and posting_date):
		dontmanage.throw(_("Company and Posting Date is mandatory"))

	account_currency, account_type = dontmanage.get_cached_value(
		"Account", account, ["account_currency", "account_type"]
	)

	if account_type in ["Receivable", "Payable"] and not (party_type and party):
		dontmanage.throw(_("Party Type and Party is mandatory for {0} account").format(account_type))

	account_details = {}
	dontmanageerp.get_company_currency(company)
