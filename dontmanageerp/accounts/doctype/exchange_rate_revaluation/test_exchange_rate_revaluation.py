...
from dontmanage.utils import add_days, flt, today
...
class TestExchangeRateRevaluation(AccountsTestMixin, DontManageTestCase):
	def setUp(self):
		self.create_company()
	...
	def set_system_and_company_settings(self):
		...
		company_doc = dontmanage.get_doc("Company", self.company)
	...
	def test_01_revaluation_of_forex_balance(self):
		...
		acc_balance = dontmanage.db.get_all(
			"GL Entry",
	...
	def test_02_accounts_only_with_base_currency_balance(self):
		...
		je = dontmanage.db.get_all("Journal Entry Account", filters={"reference_name": si.name}, pluck="parent")[
			0
		]
		...
		err_journals = err.make_jv_entries()
		je = dontmanage.get_doc("Journal Entry", err_journals.get("zero_balance_jv"))
		je = je.submit()
	...
	def test_03_accounts_only_with_account_currency_balance(self):
		...
		"""
		precision = dontmanage.db.get_single_value("System Settings", "currency_precision")
	...
	def test_04_get_account_details_function(self):
		...
		from dontmanageerp.accounts.doctype.exchange_rate_revaluation.exchange_rate_revaluation import (
			get_account_details,
		)
