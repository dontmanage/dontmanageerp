...
from dontmanageerp.accounts.utils import get_account_currency, get_currency_precision
...
class PaymentRequest(Document):
	...
	def validate_reference_document(self):
		if not self.reference_doctype or not self.reference_name:
			dontmanage.throw(_("To create a Payment Request reference document is required"))
	...
	def validate_subscription_details(self):
		if self.is_a_subscription:
			...
			for subscription_plan in self.subscription_plans:
				payment_gateway = dontmanage.db.get_value(
	...
	def get_request_amount(self):
		data_of_completed_requests = dontmanage.get_all(
			"Integration Request",
			filters={
	...
	def get_payment_url(self):
		if self.reference_doctype != "Fees":
			...
			data.update({"company": dontmanage.defaults.get_defaults().company})
		...
		return controller.get_payment_url(
			**{
				...
				"reference_doctype": "Payment Request",
				"reference_docname": self.name,
				"payer_email": self.email_to or dontmanage.session.user,
	...
	def create_payment_entry(self, submit=True):
		"""create entry"""
		dontmanage.flags.ignore_account_permission = True
	...
	def get_message(self):
		...
		if self.message:
			return dontmanage.render_template(self.message, context)
	...
	def check_if_payment_entry_exists(self):
		if self.status == "Paid":
			if dontmanage.get_all(
				...
				dontmanage.throw(_("Payment Entry already exists"), title=_("Error"))
...
def make_payment_request(**args):
	...
	if args.dt not in ALLOWED_DOCTYPES_FOR_PAYMENT_REQUEST:
		dontmanage.throw(_("Payment Requests cannot be created against: {0}").format(dontmanage.bold(args.dt)))

	ref_doc = dontmanage.get_doc(args.dt, args.dn)
	gateway_account = get_gateway_details(args) or dontmanage._dict()
	...
	if args.loyalty_points and args.dt == "Sales Order":
		...
		dontmanage.db.set_value("Sales Order", args.dn, "loyalty_amount", loyalty_amount, update_modified=False)
	...
	draft_payment_request = dontmanage.db.get_value(
		"Payment Request",
	...
	def validate_and_calculate_grand_total(grand_total, existing_payment_request_amount):
		...
		if not grand_total:
			dontmanage.throw(_("Payment Request is already created"))
		return grand_total
	...
	if draft_payment_request:
		...
		pr = dontmanage.get_doc("Payment Request", draft_payment_request)
	else:
		pr = dontmanage.new_doc("Payment Request")
		...
		if dontmanage.db.get_single_value("Accounts Settings", "create_pr_in_draft_status", cache=True):
	...
	if args.order_type == "Shopping Cart":
		...
		dontmanage.local.response["location"] = pr.get_payment_url()
...
def get_amount(ref_doc, payment_account=None):
	...
	if grand_total > 0:
		...
		dontmanage.throw(_("Payment Entry is already created"))


def get_irequest_status(payment_requests: None | list = None) -> list:
	IR = dontmanage.qb.DocType("Integration Request")
	res = []
	if payment_requests:
		res = (
			dontmanage.qb.from_(IR)
			.select(IR.name)
...
def cancel_old_payment_requests(ref_dt, ref_dn):
	...
	if res := (
		...
		if get_irequest_status([x.name for x in res]):
			dontmanage.throw(_("Another Payment Request is already processed"))
		else:
			for x in res:
				doc = dontmanage.get_doc("Payment Request", x.name)
				doc.flags.ignore_permissions = True
				doc.cancel()

				if ireqs := get_irequests_of_payment_request(doc.name):
					for ireq in ireqs:
						dontmanage.db.set_value("Integration Request", ireq.name, "status", "Cancelled")
...
def get_existing_paid_amount(doctype, name):
	PL = dontmanage.qb.DocType("Payment Ledger Entry")
	PER = dontmanage.qb.DocType("Payment Entry Reference")

	query = (
		dontmanage.qb.from_(PL)
		.left_join(PER)
		.on(
...
def get_print_format_list(ref_doctype):
	...
	print_format_list.extend(
		[p.name for p in dontmanage.get_all("Print Format", filters={"doc_type": ref_doctype})]
	)
...
def resend_payment_email(docname):
	return dontmanage.get_doc("Payment Request", docname).send_email()


@dontmanage.whitelist()
def make_payment_entry(docname):
	doc = dontmanage.get_doc("Payment Request", docname)
...
def update_payment_requests_as_per_pe_references(references=None, cancel=False):
	...
	referenced_payment_requests = dontmanage.get_all(
		"Payment Request",
...
def get_subscription_details(reference_doctype, reference_name):
	if reference_doctype == "Sales Invoice":
		subscriptions = dontmanage.db.sql(
		...
		for subscription in subscriptions:
			plans = dontmanage.get_doc("Subscription", subscription.sub_name).plans
			for plan in plans:
...
def validate_payment(doc, method=None):
	if doc.reference_doctype != "Payment Request" or (
		dontmanage.db.get_value(doc.reference_doctype, doc.reference_docname, "status") != "Paid"
	):
		return
...
def get_open_payment_requests_query(doctype, txt, searchfield, start, page_len, filters):
	...
	open_payment_requests = dontmanage.get_list(
		"Payment Request",
		filters=filters,
