...
from dontmanageerp.accounts.doctype.payment_entry.test_payment_entry import create_payment_terms_template
...
class TestPaymentRequest(DontManageTestCase):
	def setUp(self):
		if not dontmanage.db.get_value("Payment Gateway", payment_gateway["gateway"], "name"):
			dontmanage.get_doc(payment_gateway).insert(ignore_permissions=True)
		...
		for method in payment_method:
			if not dontmanage.db.get_value(
				...
				dontmanage.get_doc(method).insert(ignore_permissions=True)
	...
	def test_payment_request_linkings(self):
		...
		pr = make_payment_request(
			dt="Sales Order",
			dn=so_inr.name,
			recipient_id="saurabh@dontmanageerp.com",
	...
	def test_payment_entry(self):
		...
		dontmanage.db.set_value("Company", "_Test Company", "write_off_account", "_Test Write Off - _TC")
		dontmanage.db.set_value("Company", "_Test Company", "cost_center", "_Test Cost Center - _TC")
		...
		so_inr = dontmanage.get_doc("Sales Order", so_inr.name)
	...
	def test_multiple_payment_entries_against_sales_order(self):
		...
		pr1 = make_payment_request(
			dt="Sales Order", dn=so.name, recipient_id="nabin@dontmanageerp.com", return_doc=1
		)
		pr1.grand_total = 200
		pr1.submit()
		...
		pr2.grand_total = 900
		self.assertRaises(dontmanage.ValidationError, pr2.save)
	...
	def test_conversion_on_foreign_currency_accounts(self):
		...
		pr = make_payment_request(dt=po_doc.doctype, dn=po_doc.name, recipient_id="nabin@dontmanageerp.com")
		pr = dontmanage.get_doc(pr).save().submit()
