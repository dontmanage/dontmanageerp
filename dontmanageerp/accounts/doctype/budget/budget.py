		...
		for d in self.get("accounts"):
			if d.account:
				...
				if account_details.is_group:
					...
					dontmanage.throw(_("Account {0} does not belongs to company {1}").format(d.account, self.company))
	...
	for dimension in default_dimensions + get_accounting_dimensions(as_list=False):
		...
		if (
			...
			if dontmanage.get_cached_value("DocType", doctype, "is_tree"):
				...
				condition = "and b.%s=%s" % (budget_against, dontmanage.db.escape(args.get(budget_against)))
				args.is_tree = False
...
def compare_expense_with_budget(args, budget_amount, action_for, action, budget_against, amount=0):
	...
	if total_expense > budget_amount:
		...
		if (
			dontmanage.flags.exception_approver_role
			and dontmanage.flags.exception_approver_role in dontmanage.get_roles(dontmanage.session.user)
		):
			action = "Warn"
