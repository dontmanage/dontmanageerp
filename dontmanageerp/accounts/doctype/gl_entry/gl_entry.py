...
class GLEntry(Document):
	...
	def validate_cost_center(self):
		...
		is_group, company = dontmanage.get_cached_value(
...
def update_outstanding_amt(
	...
	if against_voucher_type == "Sales Invoice":
		party_account = dontmanage.db.get_value(against_voucher_type, against_voucher, "debit_to")
		account_condition = "and account in ({0}, {1})".format(
			dontmanage.db.escape(account), dontmanage.db.escape(party_account)
		)
	else:
		account_condition = " and account = {0}".format(dontmanage.db.escape(account))
...
def on_doctype_update():
	dontmanage.db.add_index("GL Entry", ["against_voucher_type", "against_voucher"])
