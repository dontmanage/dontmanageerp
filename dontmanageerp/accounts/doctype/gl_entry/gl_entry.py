...
from dontmanage.utils import flt, fmt_money
...
class GLEntry(Document):
	...
	def check_mandatory(self):
		...
		for k in mandatory:
			if not self.get(k):
				dontmanage.throw(_("{0} is required").format(_(self.meta.get_label(k))))
	...
	def pl_must_have_cost_center(self):
		...
		if dontmanage.get_cached_value("Account", self.account, "report_type") == "Profit and Loss":
	...
	def validate_cost_center(self):
		...
		is_group, company = dontmanage.get_cached_value("Cost Center", self.cost_center, ["is_group", "company"])
		...
		if self.voucher_type != "Period Closing Voucher" and is_group:
			dontmanage.throw(
				...
				).format(self.voucher_type, self.voucher_no, dontmanage.bold(self.cost_center))
			)
...
def update_outstanding_amt(
	...
	if against_voucher_type == "Sales Invoice":
		party_account = dontmanage.get_cached_value(against_voucher_type, against_voucher, "debit_to")
		account_condition = f"and account in ({dontmanage.db.escape(account)}, {dontmanage.db.escape(party_account)})"
	else:
		account_condition = f" and account = {dontmanage.db.escape(account)}"
	...
	if against_voucher_type in ["Sales Invoice", "Purchase Invoice", "Fees"]:
		ref_doc = dontmanage.get_doc(against_voucher_type, against_voucher)

		# Didn't use db_set for optimization purpose
		ref_doc.outstanding_amount = bal
		dontmanage.db.set_value(against_voucher_type, against_voucher, "outstanding_amount", bal)

		ref_doc.set_status(update=True)


def validate_frozen_account(account, adv_adj=None):
	frozen_account = dontmanage.get_cached_value("Account", account, "freeze_account")
	if frozen_account == "Yes" and not adv_adj:
		...
		if not frozen_accounts_modifier:
			dontmanage.throw(_("Account {0} is frozen").format(account))
		elif frozen_accounts_modifier not in dontmanage.get_roles():
			dontmanage.throw(_("Not authorized to edit frozen Account {0}").format(account))


def update_against_account(voucher_type, voucher_no):
	entries = dontmanage.db.get_all(
		"GL Entry",
	...
	if not entries:
		return
	company_currency = dontmanageerp.get_company_currency(entries[0].company)
	...
	for d in entries:
		...
		if d.against != new_against:
			dontmanage.db.set_value("GL Entry", d.name, "against", new_against)


def on_doctype_update():
	dontmanage.db.add_index("GL Entry", ["voucher_type", "voucher_no"])
	dontmanage.db.add_index("GL Entry", ["posting_date", "company"])
	dontmanage.db.add_index("GL Entry", ["party_type", "party"])
...
def rename_temporarily_named_docs(doctype):
	...
	docs_to_rename = dontmanage.get_all(doctype, {"to_rename": "1"}, order_by="creation", limit=50000)
	for doc in docs_to_rename:
		oldname = doc.name
		set_name_from_naming_options(dontmanage.get_meta(doctype).autoname, doc)
		newname = doc.name
		dontmanage.db.sql(
