...
class UnreconcilePayment(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.unreconcile_payment_entries.unreconcile_payment_entries import (
	...
	def validate(self):
		...
		if not self.voucher_type in self.supported_types:
			dontmanage.throw(_("Only {0} are supported").format(comma_and(self.supported_types)))
	...
	def on_submit(self):
		...
		for alloc in self.allocations:
			doc = dontmanage.get_doc(alloc.reference_doctype, alloc.reference_name)
			unlink_ref_doc_from_payment_entries(doc, self.voucher_no)
			cancel_exchange_gain_loss_journal(doc, self.voucher_type, self.voucher_no)
			update_voucher_outstanding(
				alloc.reference_doctype, alloc.reference_name, alloc.account, alloc.party_type, alloc.party
			)
			if doc.doctype in dontmanage.get_hooks("advance_payment_doctypes"):
				doc.set_total_advance_paid()

			dontmanage.db.set_value("Unreconcile Payment Entries", alloc.name, "unlinked", True)
...
def create_unreconcile_doc_for_selection(selections=None):
	if selections:
		selections = dontmanage.json.loads(selections)
		# assuming each row is a unique voucher
		for row in selections:
			unrecon = dontmanage.new_doc("Unreconcile Payment")
