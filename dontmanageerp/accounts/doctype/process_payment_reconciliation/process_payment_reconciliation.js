...
dontmanage.ui.form.on("Process Payment Reconciliation", {
	onload: function(frm) {
		// set queries
		...
		frm.set_query('receivable_payable_account',  function(doc) {
			return {
				filters: {
					"company": doc.company,
					"is_group": 0,
					"account_type": dontmanage.boot.party_account_types[doc.party_type]
				}
			};
		});
	...
	refresh: function(frm) {
		if (frm.doc.docstatus==1 && ['Queued', 'Paused'].find(x => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: 'dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.trigger_job_for_doc',
					args: {
						docname: frm.doc.name
					}
				}).then(r => {
					if(!r.exc) {
						dontmanage.show_alert(__("Job Started"));
						frm.reload_doc();
					}
				});
			});
		...
		if (frm.doc.docstatus==1 && ['Completed', 'Running', 'Paused', 'Partially Reconciled'].find(x => x == frm.doc.status)) {
			frm.call({
				'method': "dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.get_reconciled_count",
				args: {
					"docname": frm.docname,
				}
			}).then(r => {
		...
		if (frm.doc.docstatus==1 && frm.doc.status == 'Running') {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					'method': "dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.pause_job_for_doc",
					args: {
						"docname": frm.docname,
					}
				}).then(r => {
					if (!r.exc) {
						dontmanage.show_alert(__("Job Paused"));
						frm.reload_doc()
					}
				});
