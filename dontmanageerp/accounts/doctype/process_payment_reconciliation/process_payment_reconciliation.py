...
class ProcessPaymentReconciliation(Document):
	...
	def validate_receivable_payable_account(self):
		if self.receivable_payable_account:
			if self.company != dontmanage.db.get_value("Account", self.receivable_payable_account, "company"):
				dontmanage.throw(
					_("Receivable/Payable Account: {0} doesn't belong to company {1}").format(
						dontmanage.bold(self.receivable_payable_account), dontmanage.bold(self.company)
					)
				)

	def validate_bank_cash_account(self):
		if self.bank_cash_account:
			if self.company != dontmanage.db.get_value("Account", self.bank_cash_account, "company"):
				dontmanage.throw(
					_("Bank/Cash Account {0} doesn't belong to company {1}").format(
						dontmanage.bold(self.bank_cash_account), dontmanage.bold(self.company)
					)
				)
	...
	def on_cancel(self):
		...
		log = dontmanage.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": self.name})
		if log:
			dontmanage.db.set_value("Process Payment Reconciliation Log", log, "status", "Cancelled")
...
def get_reconciled_count(docname: str | None = None) -> float:
	current_status = {}
	if docname:
		reconcile_log = dontmanage.db.get_value(
...
def get_pr_instance(doc: str):
	process_payment_reconciliation = dontmanage.get_doc("Process Payment Reconciliation", doc)
...
def is_job_running(job_name: str) -> bool:
	jobs = dontmanage.db.get_all("RQ Job", filters={"status": ["in", ["started", "queued"]]})
	for x in jobs:
...
def pause_job_for_doc(docname: str | None = None):
	if docname:
		dontmanage.db.set_value("Process Payment Reconciliation", docname, "status", "Paused")
		log = dontmanage.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": docname})
		if log:
			dontmanage.db.set_value("Process Payment Reconciliation Log", log, "status", "Paused")
...
def trigger_job_for_doc(docname: str | None = None):
	...
	if not dontmanage.db.get_single_value("Accounts Settings", "auto_reconcile_payments"):
		dontmanage.throw(
	...
	if not is_scheduler_inactive():
		if dontmanage.db.get_value("Process Payment Reconciliation", docname, "status") == "Queued":
			dontmanage.db.set_value("Process Payment Reconciliation", docname, "status", "Running")
			job_name = f"start_processing_{docname}"
			if not is_job_running(job_name):
				dontmanage.enqueue(
					method="dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.reconcile_based_on_filters",
					queue="long",
					is_async=True,
					job_name=job_name,
		...
		elif dontmanage.db.get_value("Process Payment Reconciliation", docname, "status") == "Paused":
			dontmanage.db.set_value("Process Payment Reconciliation", docname, "status", "Running")
			log = dontmanage.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": docname})
			if log:
				dontmanage.db.set_value("Process Payment Reconciliation Log", log, "status", "Running")
		...
		dontmanage.msgprint(_("Scheduler is Inactive. Can't trigger job now."))
...
def trigger_reconciliation_for_queued_docs():
	...
	if not is_scheduler_inactive():
		...
		all_queued = dontmanage.db.get_all(
		...
		for x in all_queued:
			doc = dontmanage.get_doc("Process Payment Reconciliation", x)
		...
		dontmanage.msgprint(_("Scheduler is Inactive. Can't trigger jobs now."))
...
def reconcile_based_on_filters(doc: None | str = None) -> None:
	...
	"""
	if doc:
		log = dontmanage.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": doc})
		if not log:
			log = dontmanage.new_doc("Process Payment Reconciliation Log")
			log.process_pr = doc
			log.status = "Running"
			log = log.save()
			...
			if not is_job_running(job_name):
				dontmanage.enqueue(
					method="dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.fetch_and_allocate",
					queue="long",
					timeout="3600",
					is_async=True,
					job_name=job_name,
			...
			if not allocated:
				...
				if not is_job_running(reconcile_job_name):
					dontmanage.enqueue(
						method="dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.reconcile",
						queue="long",
						timeout="3600",
						is_async=True,
				...
				dontmanage.db.set_value("Process Payment Reconciliation", doc, "status", "Completed")


def get_next_allocation(log: str) -> list:
	if log:
		allocations = []
		next = dontmanage.db.get_all(
		...
		if next:
			allocations = dontmanage.db.get_all(
				"Process Payment Reconciliation Log Allocations",
				filters={
					"parent": log,
					"reconciled": 0,
...
def fetch_and_allocate(doc: str) -> None:
	...
	if doc:
		log = dontmanage.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": doc})
		if log:
			if not dontmanage.db.get_value("Process Payment Reconciliation Log", log, "allocated"):
				reconcile_log = dontmanage.get_doc("Process Payment Reconciliation Log", log)
			...
			if reconciled_entries != total_allocations:
				try:
					...
					if allocations[0].reference_type == "Payment Entry":
						...
						pe = dontmanage.get_doc(allocations[0].reference_type, allocations[0].reference_name)
					...
					reconciled_count = dontmanage.db.count(
						"Process Payment Reconciliation Log Allocations",
					...
					if traceback:
						...
						dontmanage.db.set_value("Process Payment Reconciliation Log", log, "error_log", message)
						dontmanage.db.set_value(
							"Process Payment Reconciliation",
							doc,
							"error_log",
					...
					if reconciled_entries and total_allocations and reconciled_entries < total_allocations:
						...
						dontmanage.db.set_value("Process Payment Reconciliation Log", log, "status", "Failed")
						dontmanage.db.set_value(
							"Process Payment Reconciliation",
							doc,
							"status",
					...
					if reconciled_entries == total_allocations:
						dontmanage.db.set_value("Process Payment Reconciliation Log", log, "status", "Reconciled")
						dontmanage.db.set_value("Process Payment Reconciliation Log", log, "reconciled", True)
						dontmanage.db.set_value("Process Payment Reconciliation", doc, "status", "Completed")
					else:
						if not (
							dontmanage.db.get_value("Process Payment Reconciliation", doc, "status") == "Paused"
						):
...
def is_any_doc_running(for_filter: str | dict | None = None) -> str | None:
	...
	if for_filter:
		...
		running_doc = dontmanage.db.get_value(
			"Process Payment Reconciliation",
			filters={
				"docstatus": 1,
