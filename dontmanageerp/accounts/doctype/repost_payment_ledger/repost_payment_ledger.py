...
from dontmanageerp.accounts.utils import _delete_pl_entries, create_payment_ledger_entry
...
def start_payment_ledger_repost(docname=None):
	...
	"""
	if docname:
		repost_doc = dontmanage.get_doc("Repost Payment Ledger", docname)
		if repost_doc.docstatus.is_submitted() and repost_doc.repost_status in ["Queued", "Failed"]:
			try:
				...
				dontmanage.db.set_value(repost_doc.doctype, repost_doc.name, "repost_error_log", "")
				dontmanage.db.set_value(repost_doc.doctype, repost_doc.name, "repost_status", "Completed")
				...
				dontmanage.db.set_value(repost_doc.doctype, repost_doc.name, "repost_status", "Failed")
...
class RepostPaymentLedger(Document):
	...
	def on_submit(self):
		...
		dontmanage.msgprint(_("Repost started in the background"))
...
def execute_repost_payment_ledger(docname):
	...
	dontmanage.enqueue(
		method="dontmanageerp.accounts.doctype.repost_payment_ledger.repost_payment_ledger.start_payment_ledger_repost",
		docname=docname,
		is_async=True,
		job_name=job_name,
	)
