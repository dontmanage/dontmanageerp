...
dontmanage.ui.form.on('Repost Payment Ledger', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus==1 && ['Queued', 'Failed'].find(x => x == frm.doc.repost_status)) {
			...
			frm.add_custom_button(btn_label, () => {
				dontmanage.call({
					method: 'dontmanageerp.accounts.doctype.repost_payment_ledger.repost_payment_ledger.execute_repost_payment_ledger',
					args: {
						docname: frm.doc.name,
					}
				});
				dontmanage.msgprint(__('Reposting in the background.'));
			});
		}
