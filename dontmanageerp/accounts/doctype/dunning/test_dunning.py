...
class TestDunning(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_dunning_with_payment_entry(self):
		...
		for overdue_payment in dunning.overdue_payments:
			outstanding_amount = dontmanage.get_value(
...
def create_dunning(overdue_days, dunning_type_name=None):
	...
	if dunning_type_name:
		dunning_type = dontmanage.get_doc("Dunning Type", dunning_type_name)
...
def create_dunning_type(title, fee, interest, is_default):
	...
	if dontmanage.db.exists("Dunning Type", f"{title} - _TC"):
		return

	dunning_type = dontmanage.new_doc("Dunning Type")
...
def get_income_account(company):
	return (
		dontmanage.get_value("Company", company, "default_income_account")
		or dontmanage.get_all(
			"Account",
...
def create_payment_terms_template_for_dunning():
	...
	if not dontmanage.db.exists("Payment Terms Template", "_Test 50-50 for Dunning"):
		dontmanage.get_doc(
			{
