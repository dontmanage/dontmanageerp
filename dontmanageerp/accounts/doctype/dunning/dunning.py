...
class Dunning(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.overdue_payment.overdue_payment import OverduePayment
	...
	def validate_same_currency(self):
		...
		for row in self.overdue_payments:
			invoice_currency = dontmanage.get_value("Sales Invoice", row.sales_invoice, "currency")
	...
	def set_dunning_level(self):
		for row in self.overdue_payments:
			past_dunnings = dontmanage.get_all(
				"Overdue Payment",
				filters={
	...
	for reference in doc.references:
		...
		if reference.reference_doctype == "Sales Invoice" and (
			...
			for dunning in dunnings:
				resolve = True
				dunning = dontmanage.get_doc("Dunning", dunning.get("name"))
				for overdue_payment in dunning.overdue_payments:
					outstanding_inv = dontmanage.get_value(
						"Sales Invoice", overdue_payment.sales_invoice, "outstanding_amount"
					)
					outstanding_ps = dontmanage.get_value(
...
def get_linked_dunnings_as_per_state(sales_invoice, state):
	dunning = dontmanage.qb.DocType("Dunning")
	overdue_payment = dontmanage.qb.DocType("Overdue Payment")

	return (
		dontmanage.qb.from_(dunning)
...
def get_dunning_letter_text(dunning_type, doc, language=None):
	...
	letter_text = dontmanage.db.get_value(
		"Dunning Letter Text", filters, ["body_text", "closing_text", "language"], as_dict=1
	)
	if letter_text:
		return {
			"body_text": dontmanage.render_template(letter_text.body_text, doc),
			"closing_text": dontmanage.render_template(letter_text.closing_text, doc),
