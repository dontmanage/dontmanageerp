...
dontmanage.ui.form.on("Dunning", {
	setup: function (frm) {
		...
		frm.set_query("company_address", dontmanageerp.queries.company_address_query);
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			if (frm.fields_dict.currency) {
				const company_currency = dontmanageerp.get_currency(frm.doc.company);
			...
			const company_doc = dontmanage.get_doc(":Company", frm.doc.company);
	...
	conversion_rate: function (frm) {
		if (frm.doc.currency === dontmanageerp.get_currency(frm.doc.company)) {
			frm.set_value("conversion_rate", 1.0);
		}

		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("conversion_rate", "read_only", dontmanageerp.stale_rate_allowed() ? 0 : 1);
	},
	customer_address: function (frm) {
		dontmanageerp.utils.get_address_display(frm, "customer_address");
	},
	company_address: function (frm) {
		dontmanageerp.utils.get_address_display(frm, "company_address");
	},
	...
	get_dunning_letter_text: function (frm) {
		if (frm.doc.dunning_type) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.dunning.dunning.get_dunning_letter_text",
				args: {
	...
	calculate_overdue_days: function (frm) {
		frm.doc.overdue_payments.forEach((row) => {
			if (frm.doc.posting_date && row.due_date) {
				...
				dontmanage.model.set_value(row.doctype, row.name, "overdue_days", overdue_days);
			}
		});
	},
	calculate_interest: function (frm) {
		frm.doc.overdue_payments.forEach((row) => {
			...
			dontmanage.model.set_value(row.doctype, row.name, "interest", interest);
		});
	},
	...
	make_payment_entry: function (frm) {
		return dontmanage.call({
			method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_payment_entry",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},
...
dontmanage.ui.form.on("Overdue Payment", {
