...
from dontmanage.utils import add_months, flt
...
class MonthlyDistribution(Document):
	...
	def validate(self):
		...
		if flt(total, 2) != 100.0:
			dontmanage.throw(_("Percentage Allocation should be equal to 100%") + f" ({flt(total, 2)!s}%)")


def get_periodwise_distribution_data(distribution_id, period_list, periodicity):
	doc = dontmanage.get_doc("Monthly Distribution", distribution_id)
