...
dontmanage.ui.form.on('Cost Center', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		...
		if(!frm.doc.__islocal) {
			frm.add_custom_button(__('Chart of Cost Centers'),
				function() { dontmanage.set_route("Tree", "Cost Center"); });

			frm.add_custom_button(__('Budget'),
				function() { dontmanage.set_route("List", "Budget", {'cost_center': frm.doc.name}); });
		}
	},
	update_cost_center_number: function(frm) {
		var d = new dontmanage.ui.Dialog({
			...
			primary_action: function() {
				...
				dontmanage.call({
					method: "dontmanageerp.accounts.utils.update_cost_center",
					args: {
					...
					callback: function(r) {
						...
						if(!r.exc) {
							if(r.message) {
								dontmanage.set_route("Form", "Cost Center", r.message);
							} else {
	...
	parent_cost_center(frm) {
		if(!frm.doc.company) {
			dontmanage.msgprint(__('Please enter company name first'));
		}
	},
