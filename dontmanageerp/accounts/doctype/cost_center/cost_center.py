...
class CostCenter(NestedSet):
	...
	def validate_mandatory(self):
		if self.cost_center_name != self.company and not self.parent_cost_center:
			dontmanage.throw(_("Please enter parent cost center"))
		elif self.cost_center_name == self.company and self.parent_cost_center:
			dontmanage.throw(_("Root cannot have a parent cost center"))

	def validate_parent_cost_center(self):
		if self.parent_cost_center:
			if not dontmanage.db.get_value("Cost Center", self.parent_cost_center, "is_group"):
				dontmanage.throw(
					_("{0} is not a group node. Please select a group node as parent cost center").format(
						dontmanage.bold(self.parent_cost_center)
					)
				)
	...
	def convert_group_to_ledger(self):
		if self.check_if_child_exists():
			dontmanage.throw(_("Cannot convert Cost Center to ledger as it has child nodes"))
		elif self.check_gle_exists():
			dontmanage.throw(_("Cost Center with existing transactions can not be converted to ledger"))
		else:
			self.is_group = 0
			self.save()
			return 1
	...
	def convert_ledger_to_group(self):
		if self.if_allocation_exists_against_cost_center():
			dontmanage.throw(_("Cost Center with Allocation records can not be converted to a group"))
		...
		if self.check_gle_exists():
			dontmanage.throw(_("Cost Center with existing transactions can not be converted to group"))
		self.is_group = 1
		self.save()
		return 1

	def check_gle_exists(self):
		return dontmanage.db.get_value("GL Entry", {"cost_center": self.name})
	...
	def before_rename(self, olddn, newdn, merge=False):
		...
		from dontmanageerp.setup.doctype.company.company import get_name_with_abbr
	...
	def after_rename(self, olddn, newdn, merge=False):
		...
		if not merge:
			new_cost_center = dontmanage.db.get_value(
...
def on_doctype_update():
	dontmanage.db.add_index("Cost Center", ["lft", "rgt"])
