...
dontmanage.ui.form.on("POS Opening Entry", {
	setup(frm) {
		if (frm.doc.docstatus == 0) {
			frm.trigger("set_posting_date_read_only");
			frm.set_value("period_start_date", dontmanage.datetime.now_datetime());
	...
	pos_profile: (frm) => {
		if (frm.doc.pos_profile) {
			dontmanage.db.get_doc("POS Profile", frm.doc.pos_profile).then(({ payments }) => {
				if (payments.length) {
