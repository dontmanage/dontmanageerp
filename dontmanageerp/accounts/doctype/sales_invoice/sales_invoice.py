...
from dontmanageerp.stock.doctype.batch.batch import set_batch_nos
...
class SalesInvoice(SellingController):
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.sales_order and dontmanage.db.get_value("Sales Order", d.sales_order, "docstatus") != 1:
				dontmanage.throw(_("Sales Order {0} is not submitted").format(d.sales_order))

			if d.delivery_note and dontmanage.db.get_value("Delivery Note", d.delivery_note, "docstatus") != 1:
	...
	def update_serial_no(self, in_cancel=False):
		...
		for item in self.items:
			...
			for serial_no in get_serial_nos(item.serial_no):
				if serial_no and dontmanage.db.get_value("Serial No", serial_no, "item_code") == item.item_code:
					dontmanage.db.set_value("Serial No", serial_no, "sales_invoice", invoice)
	...
	def get_returned_amount(self):
		from dontmanage.query_builder.functions import Coalesce, Sum
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if inter_company_reference:
		...
		if not dontmanage.db.get_value(partytype, {"represents_company": doc.company}, "name") == party:
			dontmanage.throw(_("Invalid {0} for Inter Company Transaction.").format(partytype))
...
def on_doctype_update():
	dontmanage.db.add_index("Sales Invoice", ["customer", "is_return", "return_against"])
...
def create_dunning(source_name, target_doc=None):
	...
	from dontmanageerp.accounts.doctype.dunning.dunning import (
	...
	def set_missing_values(source, target):
		...
		if dontmanage.db.exists(
			...
			dunning_type = dontmanage.get_doc(
				"Dunning Type", {"start_day": ["<", overdue_days], "end_day": [">=", overdue_days]}
			)
