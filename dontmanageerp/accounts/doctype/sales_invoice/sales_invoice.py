...
from dontmanage.utils import add_days, cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
...
from dontmanageerp.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from dontmanageerp.accounts.general_ledger import get_round_off_account_and_cost_center
from dontmanageerp.accounts.party import get_due_date, get_party_account, get_party_details
from dontmanageerp.accounts.utils import cancel_exchange_gain_loss_journal, get_account_currency
from dontmanageerp.assets.doctype.asset.depreciation import (
	depreciate_asset,
...
from dontmanageerp.controllers.selling_controller import SellingController
from dontmanageerp.projects.doctype.timesheet.timesheet import get_projectwise_timesheet_data
from dontmanageerp.setup.doctype.company.company import update_company_current_month_sales
from dontmanageerp.stock.doctype.delivery_note.delivery_note import update_billed_amount_based_on_so
from dontmanageerp.stock.doctype.serial_no.serial_no import get_delivery_note_serial_no, get_serial_nos
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from dontmanageerp.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
		from dontmanageerp.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
	...
	def validate(self):
		...
		if self.redeem_loyalty_points:
			lp = dontmanage.get_doc("Loyalty Program", self.loyalty_program)
	...
	def validate_fixed_asset(self):
		for d in self.get("items"):
			if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
				asset = dontmanage.get_doc("Asset", d.asset)
				if self.doctype == "Sales Invoice" and self.docstatus == 1:
					if self.update_stock:
						dontmanage.throw(_("'Update Stock' cannot be checked for fixed asset sale"))
	...
	def validate_item_cost_centers(self):
		for item in self.items:
			cost_center_company = dontmanage.get_cached_value("Cost Center", item.cost_center, "company")
			if cost_center_company != self.company:
				dontmanage.throw(
					_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
						dontmanage.bold(item.idx), dontmanage.bold(item.cost_center), dontmanage.bold(self.company)
					)
				)
	...
	def on_submit(self):
		...
		if dontmanage.db.get_single_value("Selling Settings", "sales_update_frequency") == "Each Transaction":
		...
		if not self.is_return and not self.is_consolidated and self.loyalty_program:
			...
			against_si_doc = dontmanage.get_doc("Sales Invoice", self.return_against)
	...
	def validate_pos_paid_amount(self):
		if len(self.payments) == 0 and self.is_pos and flt(self.grand_total) > 0:
			dontmanage.throw(_("At least one mode of payment is required for POS invoice."))
	...
	def check_if_consolidated_invoice(self):
		...
		if self.doctype == "Sales Invoice" and self.is_consolidated:
			...
			pos_closing_entry = dontmanage.get_all(
			...
			if pos_closing_entry and pos_closing_entry[0]:
				msg = _("To cancel a {} you need to cancel the POS Closing Entry {}.").format(
					dontmanage.bold(_("Consolidated Sales Invoice")),
	...
	def check_credit_limit(self):
		from dontmanageerp.selling.doctype.customer.customer import check_credit_limit

		validate_against_credit_limit = False
		bypass_credit_limit_check_at_sales_order = dontmanage.db.get_value(
	...
	def unlink_sales_invoice_from_timesheets(self):
		for row in self.timesheets:
			timesheet = dontmanage.get_doc("Timesheet", row.time_sheet)
	...
	def set_missing_values(self, for_validate=False):
		...
		if not print_format and not cint(dontmanage.db.get_value("Print Format", "POS Invoice", "disabled")):
	...
	def update_time_sheet(self, sales_invoice):
		for d in self.timesheets:
			if d.time_sheet:
				timesheet = dontmanage.get_doc("Timesheet", d.time_sheet)
	...
	def validate_time_sheets_are_submitted(self):
		for data in self.timesheets:
			if data.time_sheet:
				status = dontmanage.db.get_value("Timesheet", data.time_sheet, "status")
				if status not in ["Submitted", "Payslip"]:
					dontmanage.throw(_("Timesheet {0} is already completed or cancelled").format(data.time_sheet))
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.account_for_change_amount:
			self.account_for_change_amount = dontmanage.get_cached_value(
				"Company", self.company, "default_cash_account"
			)

		from dontmanageerp.stock.get_item_details import get_pos_profile, get_pos_profile_item_details
		...
		pos = {}
		if self.pos_profile:
			pos = dontmanage.get_doc("POS Profile", self.pos_profile)
		...
		if pos:
			...
			if self.customer:
				customer_price_list, customer_group = dontmanage.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group"]
				)
				customer_group_price_list = dontmanage.get_value(
			...
			for item in self.get("items"):
				if item.get("item_code"):
					profile_details = get_pos_profile_item_details(
						pos, dontmanage._dict(item.as_dict()), pos, update_data=True
					)
			...
			if self.tc_name and not self.terms:
				self.terms = dontmanage.db.get_value("Terms and Conditions", self.tc_name, "terms")
	...
	def get_company_abbr(self):
		return dontmanage.db.sql("select abbr from tabCompany where name=%s", self.company)[0][0]
	...
	def validate_debit_to_acc(self):
		...
		if not account:
			dontmanage.throw(_("Debit To is required"), title=_("Account Missing"))

		if account.report_type != "Balance Sheet":
			msg = (
				_("Please ensure {} account is a Balance Sheet account.").format(dontmanage.bold(_("Debit To")))
				+ " "
			)
			msg += _(
				"You can change the parent account to a Balance Sheet account or select a different account."
			)
			dontmanage.throw(msg, title=_("Invalid Account"))

		if self.customer and account.account_type != "Receivable":
			msg = (
				_("Please ensure {} account {} is a Receivable account.").format(
					dontmanage.bold(_("Debit To")), dontmanage.bold(self.debit_to)
				)
				+ " "
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(dontmanage.db.get_single_value("Selling Settings", "maintain_same_sales_rate"))
	...
	def force_set_against_income_account(self):
		self.set_against_income_account()
		dontmanage.db.set_value(self.doctype, self.name, "against_income_account", self.against_income_account)
	...
	def so_dn_required(self):
		...
		for key, value in prev_doc_field_map.items():
			if dontmanage.db.get_single_value("Selling Settings", value[0]) == "Yes":
				if dontmanage.get_value("Customer", self.customer, value[0]):
					continue
	...
	def validate_pos(self):
		if self.is_return:
			...
			if abs(flt(self.paid_amount)) + abs(flt(self.write_off_amount)) - abs(
				...
				dontmanage.throw(_("Paid amount + Write Off Amount can not be greater than Grand Total"))
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if (
				not d.warehouse
				and d.item_code
				and dontmanage.get_cached_value("Item", d.item_code, "is_stock_item")
			):
				dontmanage.throw(_("Warehouse required for stock Item {0}").format(d.item_code))
	...
	def validate_write_off_account(self):
		if flt(self.write_off_amount) and not self.write_off_account:
			self.write_off_account = dontmanage.get_cached_value("Company", self.company, "write_off_account")
	...
	def validate_dropship_item(self):
		for item in self.items:
			if item.sales_order:
				if dontmanage.db.get_value("Sales Order Item", item.so_detail, "delivered_by_supplier"):
					dontmanage.throw(_("Could not update stock, invoice contains drop shipping item."))

	def update_current_stock(self):
		for d in self.get("items"):
			if d.item_code and d.warehouse:
				bin = dontmanage.db.sql(
	...
	def set_billing_hours_and_amount(self):
		if not self.project:
			for timesheet in self.timesheets:
				ts_doc = dontmanage.get_doc("Timesheet", timesheet.time_sheet)
	...
	def get_warehouse(self):
		user_pos_profile = dontmanage.db.sql(
			"""select name, warehouse from `tabPOS Profile`
			where ifnull(user,'') = %s and company = %s""",
			(dontmanage.session["user"], self.company),
		)
		warehouse = user_pos_profile[0][1] if user_pos_profile else None

		if not warehouse:
			global_pos_profile = dontmanage.db.sql(
				"""select name, warehouse from `tabPOS Profile`
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if (
				d.sales_order
				and dontmanage.db.get_value("Sales Order", d.sales_order, "docstatus", cache=True) != 1
			):
				dontmanage.throw(_("Sales Order {0} is not submitted").format(d.sales_order))

			if (
				d.delivery_note
				and dontmanage.db.get_value("Delivery Note", d.delivery_note, "docstatus", cache=True) != 1
			):
				throw(_("Delivery Note {0} is not submitted").format(d.delivery_note))

	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from dontmanageerp.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = dontmanageerp.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
	...
	def get_gl_entries(self, warehouse_account=None):
		from dontmanageerp.accounts.general_ledger import merge_similar_entries
	...
	def make_item_gl_entries(self, gl_entries):
				...
				if item.is_fixed_asset:
					...
					if self.is_return:
						...
						if asset.calculate_depreciation:
							posting_date = dontmanage.db.get_value(
		...
		if cint(self.update_stock) and dontmanageerp.is_perpetual_inventory_enabled(self.company):
			gl_entries += super().get_gl_entries()

	def get_asset(self, item):
		if item.get("asset"):
			asset = dontmanage.get_doc("Asset", item.asset)
		else:
			dontmanage.throw(
	...
	def make_pos_gl_entries(self, gl_entries):
		if cint(self.is_pos):
			skip_change_gl_entries = not cint(
				dontmanage.db.get_single_value("Accounts Settings", "post_change_gl_entries")
			)
	...
	def make_gle_for_change_amount(self, gl_entries):
		if self.change_amount:
			if self.account_for_change_amount:
				...
				dontmanage.throw(_("Select change amount account"), title=_("Mandatory Field"))
	...
	def make_write_off_gl_entry(self, gl_entries):
		...
		if (
			...
			default_cost_center = dontmanage.get_cached_value("Company", self.company, "cost_center")
				...
				if not round_off_for_opening:
					dontmanage.throw(
						_(
							...
							dontmanage.bold(self.rounding_adjustment),
							dontmanage.bold("Round Off for Opening"),
							get_link_to_form("Company", self.company),
							dontmanage.bold("Disable Rounded Total"),
						)
					)
				else:
	...
	def update_billing_status_in_dn(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dontmanage.get_doc("Delivery Note", dn).update_billing_percentage(update_modified=update_modified)
	...
	def validate_serial_against_delivery_note(self):
		...
		for item in self.items:
			...
			serial_nos = dontmanage.db.get_value("Delivery Note Item", item.dn_detail, "serial_no") or ""
			...
			if serial_no_diff:
				dn_link = dontmanage.utils.get_link_to_form("Delivery Note", item.delivery_note)
				serial_no_msg = ", ".join(dontmanage.bold(d) for d in serial_no_diff)
				...
				dontmanage.throw(msg=msg, title=_("Serial Nos Mismatch"))
	...
	def update_project(self):
		...
		for p in unique_projects:
			project = dontmanage.get_doc("Project", p)
	...
	def verify_payment_amount_is_positive(self):
		for entry in self.payments:
			if entry.amount < 0:
				dontmanage.throw(_("Row #{0} (Payment Table): Amount must be positive").format(entry.idx))

	def verify_payment_amount_is_negative(self):
		for entry in self.payments:
			if entry.amount > 0:
				dontmanage.throw(_("Row #{0} (Payment Table): Amount must be negative").format(entry.idx))
	...
	def delete_loyalty_point_entry(self):
		...
		if against_lp_entry:
			...
			dontmanage.db.sql("""delete from `tabLoyalty Point Entry` where invoice=%s""", (self.name))
			# Set loyalty program
			self.set_loyalty_program_tier()
	...
	def set_loyalty_program_tier(self):
		...
		dontmanage.db.set_value("Customer", self.customer, "loyalty_program_tier", lp_details.tier_name)

	def get_returned_amount(self):
		from dontmanage.query_builder.functions import Sum

		doc = dontmanage.qb.DocType(self.doctype)
		returned_amount = (
			dontmanage.qb.from_(doc)
	...
	def apply_loyalty_points(self):
		from dontmanageerp.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
...
def get_discounting_status(sales_invoice):
	...
	invoice_discounting_list = dontmanage.db.sql(
		"""
		select status
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if inter_company_reference:
		doc = dontmanage.get_doc(ref_doc, inter_company_reference)
		ref_party = doc.supplier if doctype in ["Sales Invoice", "Sales Order"] else doc.customer
		if not dontmanage.db.get_value(partytype, {"represents_company": doc.company}, "name") == party:
			dontmanage.throw(_("Invalid {0} for Inter Company Transaction.").format(_(partytype)))
		if not dontmanage.get_cached_value(ref_partytype, ref_party, "represents_company") == company:
			dontmanage.throw(_("Invalid Company for Inter Company Transaction."))

	elif dontmanage.db.get_value(partytype, {"name": party, internal: 1}, "name") == party:
		companies = dontmanage.get_all(
...
def update_linked_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		dontmanage.db.set_value(doctype, inter_company_reference, ref_field, name)
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		dontmanage.db.set_value(doctype, name, ref_field, "")
		dontmanage.db.set_value(ref_doc, inter_company_reference, ref_field, "")
...
def get_inter_company_details(doc, doctype):
	if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
		...
		company = dontmanage.get_cached_value("Supplier", doc.supplier, "represents_company")
...
def validate_inter_company_transaction(doc, doctype):
	...
	valid_price_list = dontmanage.db.get_value("Price List", {"name": price_list, "buying": 1, "selling": 1})
	if not valid_price_list and not doc.is_internal_transfer():
		dontmanage.throw(_("Selected Price List should have buying and selling fields checked."))

	party = details.get("party")
	if not party:
		partytype = "Supplier" if doctype in ["Sales Invoice", "Sales Order"] else "Customer"
		dontmanage.throw(_("No {0} found for Inter Company Transactions.").format(partytype))

	company = details.get("company")
	default_currency = dontmanage.get_cached_value("Company", company, "default_currency")
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice", "Sales Order"]:
		source_doc = dontmanage.get_doc(doctype, source_name)
	...
	def update_details(source_doc, target_doc, source_parent):
		...
		if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
			currency = dontmanage.db.get_value("Supplier", details.get("party"), "default_currency")
			...
			currency = dontmanage.db.get_value("Customer", details.get("party"), "default_currency")
			target_doc.company = details.get("company")
...
def get_received_items(reference_name, doctype, reference_fieldname):
	...
	target_doctypes = dontmanage.get_all(
		doctype,
		filters=filters,
		as_list=True,
	)
	...
	received_items_map = dontmanage._dict(
		dontmanage.get_all(
			doctype + " Item",
	...
	for item in doc.get("items"):
		...
		if doc.update_stock:
			...
			if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
				item.warehouse = dontmanage.db.get_value(
...
def update_pr_items(doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map):
	for item in doc.get("items"):
		...
		if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
			item.warehouse = dontmanage.db.get_value("Purchase Order Item", item.purchase_order_item, "warehouse")


def get_delivery_note_details(internal_reference):
	si_item_details = dontmanage.get_all(
...
def get_pd_details(doctype, sd_detail_map, sd_detail_field):
	...
	pd_item_details = dontmanage.get_all(
		doctype,
...
def get_loyalty_programs(customer):
	...
	from dontmanageerp.selling.doctype.customer.customer import get_loyalty_programs

	customer = dontmanage.get_doc("Customer", customer)
...
def create_invoice_discounting(source_name, target_doc=None):
	invoice = dontmanage.get_doc("Sales Invoice", source_name)
	invoice_discounting = dontmanage.new_doc("Invoice Discounting")
...
def create_dunning(source_name, target_doc=None, ignore_permissions=False):
	...
	def postprocess_dunning(source, target):
		from dontmanageerp.accounts.doctype.dunning.dunning import get_dunning_letter_text

		dunning_type = dontmanage.db.exists("Dunning Type", {"is_default": 1, "company": source.company})
		if dunning_type:
			dunning_type = dontmanage.get_doc("Dunning Type", dunning_type)
...
def check_if_return_invoice_linked_with_payment_entry(self):
	...
	if not dontmanage.db.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
		return
	...
	payment_entries = dontmanage.db.sql_list(
		"""
		SELECT
			t1.name
		FROM
	...
	if payment_entries:
		for payment in payment_entries:
			payment_entry = dontmanage.get_doc("Payment Entry", payment)
