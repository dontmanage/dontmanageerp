...
class SalesInvoice(SellingController):
	...
	def on_submit(self):
		...
		if (
			dontmanage.db.get_single_value("Selling Settings", "sales_update_frequency") == "Each Transaction"
		):
		...
		if self.doctype == "Sales Invoice" and self.is_consolidated:
			...
			if pos_closing_entry and pos_closing_entry[0]:
				msg = _("To cancel a {} you need to cancel the POS Closing Entry {}.").format(
					dontmanage.bold("Consolidated Sales Invoice"),
	...
	def validate_debit_to_acc(self):
		...
		if account.report_type != "Balance Sheet":
			msg = (
				_("Please ensure {} account is a Balance Sheet account.").format(dontmanage.bold("Debit To")) + " "
			)
			msg += _(
		...
		if self.customer and account.account_type != "Receivable":
			msg = (
				_("Please ensure {} account {} is a Receivable account.").format(
					dontmanage.bold("Debit To"), dontmanage.bold(self.debit_to)
				)
				+ " "
			)
				...
				if item.is_fixed_asset:
					...
					if self.is_return:
						...
						if asset.calculate_depreciation:
							posting_date = dontmanage.db.get_value("Sales Invoice", self.return_against, "posting_date")
	...
	def update_project(self):
		if self.project:
			project = dontmanage.get_doc("Project", self.project)
...
def validate_inter_company_transaction(doc, doctype):
	...
	valid_price_list = dontmanage.db.get_value(
