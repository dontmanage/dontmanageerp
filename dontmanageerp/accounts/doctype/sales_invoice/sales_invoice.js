...
dontmanageerp.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	dontmanageerp.selling.SellingController
) {
	setup(doc) {
	...
	onload() {
		...
		dontmanageerp.queries.setup_warehouse_query(this.frm);
	}
	...
	make_maintenance_schedule() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: cur_frm,
		});
	}
	...
	on_submit(doc, dt, dn) {
		...
		doc.items.forEach((row) => {
			if (row.delivery_note) dontmanage.model.clear_doc("Delivery Note", row.delivery_note);
		});
	}
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				dontmanageerp.utils.map_current_doc({
					method: "dontmanageerp.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				dontmanageerp.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "dontmanageerp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
				});
	...
	customer() {
		...
		dontmanageerp.utils.get_party_details(
			this.frm,
			"dontmanageerp.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	write_off_outstanding_amount_automatically() {
		if (cint(this.frm.doc.write_off_outstanding_amount_automatically)) {
			dontmanage.model.round_floats_in(this.frm.doc, ["grand_total", "paid_amount"]);
	...
	make_sales_return() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			dontmanage.call({
				method: dontmanageerp.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
					company: frm.doc.company,
				},
				callback: function (r, rt) {
					dontmanage.model.set_value(cdt, cdn, "income_account", r.message[0]);
					dontmanage.model.set_value(cdt, cdn, "cost_center", r.message[1]);
				},
			});
		}
	}
	...
	set_pos_data() {
		if (this.frm.doc.is_pos) {
			...
			if (!this.frm.doc.company) {
				...
				dontmanage.msgprint(__("Please specify Company to proceed"));
			} else {
				var me = this;
				...
				return this.frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							dontmanage.model.set_default_values(me.frm.doc);
							me.set_dynamic_labels();
	...
	currency() {
		...
		if (this.frm.doc.timesheets) {
			this.frm.doc.timesheets.forEach((d) => {
				let row = dontmanage.get_doc(d.doctype, d.name);
...
extend_cscript(cur_frm.cscript, new dontmanageerp.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	dontmanage.model.open_mapped_doc({
		method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};
...
cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	dontmanageerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	dontmanageerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};

cur_frm.cscript.cost_center = function (doc, cdt, cdn) {
	dontmanageerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
};
...
dontmanage.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "dontmanageerp.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	hide_fields: function (frm) {
		...
		if (cint(doc.is_pos) == 1) {
			...
			for (var i in parent_fields) {
				var docfield = dontmanage.meta.docfield_map[doc.doctype][parent_fields[i]];
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
					customer: frm.doc.customer,
					loyalty_program: frm.doc.loyalty_program,
	...
	set_loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			if (frm.doc.grand_total && remaining_amount < loyalty_amount) {
				...
				dontmanage.throw(__("You can only redeem max {0} points in this order.", [redeemable_points]));
			}
	...
	async get_timesheet_data(frm, kwargs) {
		return dontmanage
			.call({
				method: "dontmanageerp.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
	...
	create_invoice_discounting: function (frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm,
		});
	},
	...
	create_dunning: function (frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm,
		});
	},
});

dontmanage.ui.form.on("Sales Invoice Timesheet", {
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	dontmanage.call({
		method: "dontmanageerp.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
		callback: function (r) {
			if (!r.exc && r.message) {
				dontmanage.model.set_value(cdt, cdn, "billing_amount", r.message);
			}
		},
	});
};
