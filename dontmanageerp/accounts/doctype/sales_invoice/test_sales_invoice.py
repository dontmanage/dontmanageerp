...
from dontmanage.utils import add_days, flt, format_date, getdate, nowdate, today

import dontmanageerp
from dontmanageerp.accounts.doctype.account.test_account import create_account, get_inventory_account
from dontmanageerp.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice import WarehouseMissingError
from dontmanageerp.accounts.doctype.purchase_invoice.test_purchase_invoice import (
	unlink_payment_on_cancel_of_invoice,
)
from dontmanageerp.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_transaction
from dontmanageerp.accounts.utils import PaymentEntryUnlinkError
from dontmanageerp.assets.doctype.asset.depreciation import post_depreciation_entries
...
from dontmanageerp.controllers.accounts_controller import update_invoice_status
from dontmanageerp.controllers.taxes_and_totals import get_itemised_tax_breakup_data
from dontmanageerp.exceptions import InvalidAccountCurrency, InvalidCurrency
from dontmanageerp.selling.doctype.customer.test_customer import get_customer_dict
from dontmanageerp.stock.doctype.delivery_note.delivery_note import make_sales_invoice
...
from dontmanageerp.stock.get_item_details import get_item_tax_map
from dontmanageerp.stock.utils import get_incoming_rate, get_stock_balance


class TestSalesInvoice(DontManageTestCase):
	def setUp(self):
		from dontmanageerp.stock.doctype.stock_ledger_entry.test_stock_ledger_entry import create_items

		create_items(["_Test Internal Transfer Item"], uoms=[{"uom": "Box", "conversion_factor": 10}])
		create_internal_parties()
		setup_accounts()
		dontmanage.db.set_single_value("Accounts Settings", "acc_frozen_upto", None)

	def tearDown(self):
		dontmanage.db.rollback()

	def make(self):
		w = dontmanage.copy_doc(test_records[0])
		w.is_pos = 0
		w.insert()
		w.submit()
		return w
	...
	def test_timestamp_change(self):
		...
		w2 = dontmanage.get_doc(w.doctype, w.name)
		...
		time.sleep(1)
		self.assertRaises(dontmanage.TimestampMismatchError, w2.save)

	def test_sales_invoice_change_naming_series(self):
		si = dontmanage.copy_doc(test_records[2])
		si.insert()
		si.naming_series = "TEST-"

		self.assertRaises(dontmanage.CannotChangeConstantError, si.save)

		si = dontmanage.copy_doc(test_records[1])
		si.insert()
		si.naming_series = "TEST-"
	...
	def test_payment_entry_unlink_against_invoice(self):
		...
		si = dontmanage.copy_doc(test_records[0])
		si.is_pos = 0
		si.insert()
		si.submit()
		...
		unlink_payment_on_cancel_of_invoice(0)
		si = dontmanage.get_doc("Sales Invoice", si.name)
		self.assertRaises(dontmanage.LinkExistsError, si.cancel)
		unlink_payment_on_cancel_of_invoice()
	...
	def test_sales_invoice_discount_amount(self):
		si = dontmanage.copy_doc(test_records[3])
	...
	def test_discount_amount_gl_entry(self):
		dontmanage.db.set_value("Company", "_Test Company", "round_off_account", "Round Off - _TC")
		si = dontmanage.copy_doc(test_records[3])
		si.discount_amount = 104.94
		si.append(
			"taxes",
	...
	def test_inclusive_rate_validations(self):
		...
		self.assertRaises(dontmanage.ValidationError, si.insert)
	...
	def test_sales_invoice_calculation_export_currency_with_tax_inclusive_price(self):
		...
		if dontmanage.db.get_value("Currency", "USD", "smallest_currency_fraction_value") < 0.01:
	...
	def test_payment(self):
		...
		jv = dontmanage.get_doc(dontmanage.copy_doc(jv_test_records[0]))
		jv.get("accounts")[0].reference_type = w.doctype
		jv.get("accounts")[0].reference_name = w.name
		jv.insert()
		jv.submit()

		self.assertEqual(dontmanage.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 162.0)
		...
		jv.cancel()
		self.assertEqual(dontmanage.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 562.0)
	...
	def test_outstanding_on_cost_center_allocation(self):
		...
		from dontmanageerp.accounts.doctype.cost_center_allocation.test_cost_center_allocation import (
	...
	def test_pos_returns_with_repayment(self):
		from dontmanageerp.accounts.doctype.sales_invoice.sales_invoice import make_sales_return
	...
	def test_pos_with_no_gl_entry_for_change_amount(self):
		dontmanage.db.set_single_value("Accounts Settings", "post_change_gl_entries", 0)
		...
		dontmanage.db.set_single_value("Accounts Settings", "post_change_gl_entries", 1)
	...
	def test_bin_details_of_packed_item(self):
		...
		bin_details = dontmanage.db.get_value(
			"Bin",
	...
	def test_pos_si_without_payment(self):
		...
		self.assertRaises(dontmanage.ValidationError, si.submit)
	...
	def _insert_purchase_receipt(self):
		from dontmanageerp.stock.doctype.purchase_receipt.test_purchase_receipt import (
			test_records as pr_test_records,
		)

		pr = dontmanage.copy_doc(pr_test_records[0])
		pr.naming_series = "_T-Purchase Receipt-"
		pr.insert()
		pr.submit()

	def _insert_delivery_note(self):
		from dontmanageerp.stock.doctype.delivery_note.test_delivery_note import (
			test_records as dn_test_records,
		)

		dn = dontmanage.copy_doc(dn_test_records[0])
	...
	def test_sales_invoice_with_advance(self):
		...
		jv = dontmanage.copy_doc(jv_test_records[0])
		jv.insert()
		jv.submit()
	...
	def test_serialized(self):
		from dontmanageerp.stock.doctype.stock_entry.test_stock_entry import make_serialized_item
		...
		self.assertFalse(dontmanage.db.get_value("Serial No", serial_nos[0], "warehouse"))
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(
			dontmanage.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC"
		)
	...
	def test_return_sales_invoice(self):
		...
		incoming_rate, stock_value_difference = dontmanage.db.get_value(
			"Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = dontmanage.db.get_value(
			"GL Entry",
		...
		party_credited = dontmanage.db.get_value(
			"GL Entry",
			{
		...
		self.assertEqual(dontmanage.db.get_value("Sales Invoice", si1.name, "outstanding_amount"), -1000)
		self.assertEqual(dontmanage.db.get_value("Sales Invoice", si.name, "outstanding_amount"), 2500)
	...
	def test_zero_qty_return_invoice_with_stock_effect(self):
		...
		self.assertRaises(dontmanage.ValidationError, cr_note.save)
	...
	def test_gle_made_when_asset_is_returned(self):
		...
		disposal_account = dontmanage.get_cached_value("Company", "_Test Company", "disposal_account")

		# Asset value is 100,000 but it was sold for 90,000, so there should be a loss of 10,000
		loss_for_si = dontmanage.get_all(
			"GL Entry",
			filters={"voucher_no": si.name, "account": disposal_account},
			fields=["credit", "debit"],
		)[0]

		loss_for_return_si = dontmanage.get_all(
			"GL Entry",
	...
	def test_incoming_rate_for_stand_alone_credit_note(self):
		...
		incoming_rate = dontmanage.db.get_value(
			"Stock Ledger Entry", {"voucher_no": return_si.name}, "incoming_rate"
		)
		debit_amount = dontmanage.db.get_value(
	...
	def test_invoice_exchange_rate(self):
		...
		self.assertRaises(dontmanage.ValidationError, si.save)
	...
	def test_outstanding_amount_after_advance_jv_cancellation(self):
		...
		jv = dontmanage.get_doc("Journal Entry", jv.name)
		jv.cancel()
	...
	def test_multiple_uom_in_selling(self):
		...
		item_price = dontmanage.new_doc("Item Price")
	...
	def test_item_wise_tax_breakup(self):
		dontmanage.flags.country = "United States"
	...
	def test_company_monthly_sales(self):
		existing_current_month_sales = dontmanage.get_cached_value(
			"Company", "_Test Company", "total_monthly_sales"
		)

		si = create_sales_invoice()
		current_month_sales = dontmanage.get_cached_value("Company", "_Test Company", "total_monthly_sales")
	...
	def test_rounding_adjustment_2(self):
		...
		round_off_account = dontmanage.get_cached_value("Company", "_Test Company", "round_off_account")
		expected_values = {
	...
	def test_rounding_adjustment_3(self):
		...
		round_off_gle = dontmanage.db.get_value(
			"GL Entry",
	...
	def test_sales_invoice_with_shipping_rule(self):
		from dontmanageerp.accounts.doctype.shipping_rule.test_shipping_rule import create_shipping_rule
	...
	def test_credit_note(self):
		...
		si_doc = dontmanage.get_doc("Sales Invoice", si.name)
	...
	def test_fixed_deferred_revenue(self):
		...
		acc_settings = dontmanage.get_doc("Accounts Settings", "Accounts Settings")
	...
	def test_inter_company_transaction_without_default_warehouse(self):
		...
		# setup
		old_negative_stock = dontmanage.db.get_single_value("Stock Settings", "allow_negative_stock")
		dontmanage.db.set_single_value("Stock Settings", "allow_negative_stock", 1)

		old_perpetual_inventory = dontmanageerp.is_perpetual_inventory_enabled("_Test Company 1")
		dontmanage.local.enable_perpetual_inventory["_Test Company 1"] = 1
		...
		# tear down
		dontmanage.local.enable_perpetual_inventory["_Test Company 1"] = old_perpetual_inventory
		dontmanage.db.set_single_value("Stock Settings", "allow_negative_stock", old_negative_stock)
	...
	def test_internal_transfer_gl_precision_issues(self):
		...
		if get_stock_balance("_Test Internal Transfer Item", "Stores - TCP1", "2022-04-10"):
			create_stock_reconciliation(
				...
				if dontmanage.get_all("Stock Ledger Entry")
	...
	def test_additional_discount_for_sales_invoice_with_discount_accounting_enabled(self):
		from dontmanageerp.accounts.doctype.repost_accounting_ledger.test_repost_accounting_ledger import (
	...
	def test_sales_invoice_against_supplier(self):
		from dontmanageerp.accounts.doctype.opening_invoice_creation_tool.test_opening_invoice_creation_tool import (
			make_customer,
		)
		from dontmanageerp.accounts.doctype.party_link.party_link import create_party_link
		from dontmanageerp.buying.doctype.supplier.test_supplier import create_supplier
		...
		dontmanage.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 1)
		...
		party_link.delete()
		dontmanage.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 0)
	...
	def test_sales_invoice_against_supplier_usd_with_dimensions(self):
		...
		cust_doc = dontmanage.get_doc("Customer", customer)
		cust_doc.default_currency = "USD"
		cust_doc.save()
		# create a supplier
		supplier = create_supplier(supplier_name="_Test Common Supplier USD").name
		supp_doc = dontmanage.get_doc("Supplier", supplier)
		...
		if not dontmanage.get_all("Accounting Dimension", filters={"document_type": "Department"}):
			dim = dontmanage.get_doc(
				{
			...
			dim = dontmanage.get_doc(
				...
				dontmanage.get_all("Accounting Dimension", filters={"document_type": "Department"})[0],
			)
			dim.disabled = False
	...
	def test_sales_invoice_cancel_with_common_party_advance_jv(self):
		...
		jv_status = dontmanage.db.get_value("Journal Entry", jv, "docstatus")
	...
	def test_sales_commission(self):
		...
		dontmanage.db.set_value("Item", si.get("items")[0].item_code, "grant_commission", 1)
		dontmanage.db.set_value("Item", si.get("items")[1].item_code, "grant_commission", 0)
	...
	def test_over_billing_case_against_delivery_note(self):
		...
		over_billing_allowance = dontmanage.db.get_single_value("Accounts Settings", "over_billing_allowance")
		...
		item_copy = dontmanage.copy_doc(si.items[0])
		si.append("items", item_copy)
		si.save()

		with self.assertRaises(dontmanage.ValidationError) as err:
			si.submit()

		self.assertTrue("cannot overbill" in str(err.exception).lower())

		dontmanage.db.set_single_value("Accounts Settings", "over_billing_allowance", over_billing_allowance)
	...
	def test_multi_currency_deferred_revenue_via_journal_entry(self):
		...
		dontmanage.db.set_single_value("Accounts Settings", "acc_frozen_upto", getdate("2019-01-31"))
	...
	def test_sales_invoice_with_disabled_account(self):
		try:
			...
			account = dontmanage.get_doc("Account", account_name)
			account.disabled = 1
			account.save()
	...
	def test_gain_loss_with_advance_entry(self):
		from dontmanageerp.accounts.doctype.journal_entry.test_journal_entry import make_journal_entry
		...
		je_type = dontmanage.get_cached_value("Journal Entry", journals[0], "voucher_type")
	...
	def test_sales_invoice_without_customer_group_and_territory(self):
		# create a customer
		if not dontmanage.db.exists("Customer", "_Test Simple Customer"):
			customer_dict = get_customer_dict("_Test Simple Customer")
			customer_dict.pop("customer_group")
			customer_dict.pop("territory")
			customer = dontmanage.get_doc(customer_dict).insert(ignore_permissions=True)
	...
	def test_taxes_merging_from_delivery_note(self):
		...
		from dontmanage.model.mapper import map_docs

		map_docs(
			method="dontmanageerp.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
	...
	def _create_opening_roundoff_account(self, company_name):
		liability_root = dontmanage.db.get_all(
			"Account",
		...
		if acc := dontmanage.db.exists(
			...
			dontmanage.db.set_value("Company", company_name, "round_off_for_opening", acc)
		else:
			...
			acc.save()
			dontmanage.db.set_value("Company", company_name, "round_off_for_opening", acc.name)
	...
	def test_total_billed_amount(self):
		...
		project = dontmanage.new_doc("Project")
		...
		doc = dontmanage.get_doc("Project", project.name)
...
def create_sales_invoice(**args):
	si = dontmanage.new_doc("Sales Invoice")
...
test_records = dontmanage.get_test_records("Sales Invoice")
...
def create_internal_parties():
	...
	dontmanage.db.set_value("Company", "_Test Company", "unrealized_profit_loss_account", account)
...
def create_internal_supplier(supplier_name, represents_company, allowed_to_interact_with):
	if not dontmanage.db.exists("Supplier", supplier_name):
		...
		supplier_name = dontmanage.db.exists("Supplier", supplier_name)
