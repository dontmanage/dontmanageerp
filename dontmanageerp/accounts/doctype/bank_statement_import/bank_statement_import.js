...
dontmanage.ui.form.on("Bank Statement Import", {
	setup(frm) {
		dontmanage.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			dontmanage.model.clear_doc("Bank Statement Import", frm.doc.name);
			dontmanage.model.with_doc("Bank Statement Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
	...
	show_import_status(frm) {
		...
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_statement_import.bank_statement_import.get_import_status",
			args: {
	...
	// method: 'dontmanage.core.doctype.data_import.data_import.get_preview_from_template',
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/dontmanageerp.accounts.doctype.bank_statement_import.bank_statement_import.download_errored_template",
			{
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/dontmanageerp.accounts.doctype.bank_statement_import.bank_statement_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_statement_import.bank_statement_import.get_import_logs",
			args: {
				docname: frm.doc.name,
			},
			callback: function (r) {
