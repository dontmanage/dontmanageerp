...
from dontmanage.core.doctype.data_import.data_import import DataImport
from dontmanage.core.doctype.data_import.importer import Importer, ImportFile
from dontmanage.utils.background_jobs import enqueue
from dontmanage.utils.xlsxutils import ILLEGAL_CHARACTERS_RE, handle_html
...
class BankStatementImport(DataImport):
	...
	def validate(self):
		...
		if (
			...
			column_to_field_map = {}
			bank = dontmanage.get_doc("Bank", self.bank)
	...
	def start_import(self):
		preview = dontmanage.get_doc("Bank Statement Import", self.name).get_preview_from_template(
			self.import_file, self.google_sheets_url
		)

		if "Bank Account" not in json.dumps(preview["columns"]):
			dontmanage.throw(_("Please add the Bank Account column"))
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return dontmanage.get_doc("Bank Statement Import", data_import).get_preview_from_template(
...
def form_start_import(data_import):
	return dontmanage.get_doc("Bank Statement Import", data_import).start_import()


@dontmanage.whitelist()
def download_errored_template(data_import_name):
	data_import = dontmanage.get_doc("Bank Statement Import", data_import_name)
	data_import.export_errored_rows()


@dontmanage.whitelist()
def download_import_log(data_import_name):
	return dontmanage.get_doc("Bank Statement Import", data_import_name).download_import_log()
...
def start_import(data_import, bank_account, import_file_path, google_sheets_url, bank, template_options):
	...
	data_import = dontmanage.get_doc("Bank Statement Import", data_import)
...
def update_mapping_db(bank, template_options):
	bank = dontmanage.get_doc("Bank", bank)
...
def get_import_status(docname):
	...
	data_import = dontmanage.get_doc("Bank Statement Import", docname)
...
def get_import_logs(docname: str):
	dontmanage.has_permission("Bank Statement Import")
...
def upload_bank_statement(**args):
	...
	bsi = dontmanage.new_doc("Bank Statement Import")
