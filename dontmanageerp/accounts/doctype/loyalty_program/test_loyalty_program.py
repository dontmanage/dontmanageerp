...
import dontmanage
from dontmanage.utils import cint, flt, getdate, today

from dontmanageerp.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_program_details_with_points,
)
from dontmanageerp.accounts.party import get_dashboard_info
...
class TestLoyaltyProgram(unittest.TestCase):
	...
	def test_loyalty_points_earned_single_tier(self):
		...
		customer = dontmanage.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		dontmanage.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Multiple Loyalty")
		# assign multiple tier program to the customer
		customer = dontmanage.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = dontmanage.get_doc(
	...
	def test_cancel_sales_invoice(self):
		...
		si.cancel()
		lpe = dontmanage.db.exists("Loyalty Point Entry", lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		earned_points = get_points_earned(si_original)
		lpe_original = dontmanage.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		si_original = dontmanage.get_doc("Sales Invoice", lpe_original.invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = dontmanage.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		for d in [si_return, si_original]:
			try:
				...
				dontmanage.get_doc("Sales Invoice", d.name).cancel()

	def test_loyalty_points_for_dashboard(self):
		doc = dontmanage.get_doc("Customer", "Test Loyalty Customer")
...
def get_points_earned(self):
	def get_returned_amount():
		returned_amount = dontmanage.db.sql(
			"""
	...
	return dontmanage.get_doc(
		{
			...
			"customer": dontmanage.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name,
			...
			"items": [
				{
					...
					"item_code": dontmanage.get_doc("Item", {"item_name": "Loyal Item"}).name,
					"qty": qty,
					"rate": 10000,
...
def create_records():
	...
	if not dontmanage.db.exists("Account", "Loyalty - _TC"):
		dontmanage.get_doc(
			{
				"doctype": "Account",
	...
	if not dontmanage.db.exists("Loyalty Program", "Test Single Loyalty"):
		dontmanage.get_doc(
			{
	...
	if not dontmanage.db.exists("Customer", "Test Loyalty Customer"):
		dontmanage.get_doc(
			{
	...
	if not dontmanage.db.exists("Loyalty Program", "Test Multiple Loyalty"):
		dontmanage.get_doc(
			{
				"doctype": "Loyalty Program",
	...
	# create an item
	if not dontmanage.db.exists("Item", "Loyal Item"):
		dontmanage.get_doc(
			{
				"doctype": "Item",
	...
	# create item price
	if not dontmanage.db.exists("Item Price", {"price_list": "Standard Selling", "item_code": "Loyal Item"}):
		dontmanage.get_doc(
			{
				"doctype": "Item Price",
