...
class LoyaltyProgram(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.loyalty_program_collection.loyalty_program_collection import (
...
def get_loyalty_details(
	...
	condition = ""
	if company:
		condition = " and company=%s " % dontmanage.db.escape(company)
	if not include_expired_entry:
		condition += " and expiry_date>='%s' " % expiry_date

	loyalty_point_details = dontmanage.db.sql(
...
def get_loyalty_program_details_with_points(
	...
	loyalty_program = dontmanage.get_doc("Loyalty Program", loyalty_program)
	lp_details.update(
...
def get_loyalty_program_details(
	...
	lp_details = dontmanage._dict()

	if not loyalty_program:
		loyalty_program = dontmanage.db.get_value("Customer", customer, "loyalty_program")

		if not loyalty_program and not silent:
			dontmanage.throw(_("Customer isn't enrolled in any Loyalty Program"))
		elif silent and not loyalty_program:
			return dontmanage._dict({"loyalty_programs": None})

	if not company:
		company = dontmanage.db.get_default("company") or dontmanage.get_all("Company")[0].name
...
def get_redeemption_factor(loyalty_program=None, customer=None):
	...
	if not loyalty_program:
		customer_loyalty_program = dontmanage.db.get_value("Customer", customer, "loyalty_program")
		loyalty_program = customer_loyalty_program
	if loyalty_program:
		return dontmanage.db.get_value("Loyalty Program", loyalty_program, "conversion_factor")
	else:
		dontmanage.throw(_("Customer isn't enrolled in any Loyalty Program"))
...
def validate_loyalty_points(ref_doc, points_to_redeem):
	...
	if hasattr(ref_doc, "loyalty_program") and ref_doc.loyalty_program:
		...
		loyalty_program = dontmanage.db.get_value("Customer", ref_doc.customer, ["loyalty_program"])

	if (
		loyalty_program
		and dontmanage.db.get_value("Loyalty Program", loyalty_program, ["company"]) != ref_doc.company
	):
		dontmanage.throw(_("The Loyalty Program isn't valid for the selected company"))
	...
	if loyalty_program and points_to_redeem:
		...
		if points_to_redeem > loyalty_program_details.loyalty_points:
			dontmanage.throw(_("You don't have enough Loyalty Points to redeem"))

		loyalty_amount = flt(points_to_redeem * loyalty_program_details.conversion_factor)

		if loyalty_amount > ref_doc.rounded_total:
			dontmanage.throw(_("You can't redeem Loyalty Points having more value than the Rounded Total."))
