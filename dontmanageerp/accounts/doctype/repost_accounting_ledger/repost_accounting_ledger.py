...
class RepostAccountingLedger(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.repost_accounting_ledger_items.repost_accounting_ledger_items import (
		...
		if self.vouchers:
			...
			for vtype in self._allowed_types:
				if names := [x.voucher_no for x in self.vouchers if x.voucher_type == vtype]:
					latest_voucher = dontmanage.db.get_all(
						vtype,
					...
					if latest_voucher and latest_pcv[0] >= latest_voucher:
						dontmanage.throw(_("Cannot Resubmit Ledger entries for vouchers in Closed fiscal year."))
	...
	def get_existing_ledger_entries(self):
		...
		self.gles = dontmanage._dict({})

		for gle in existing_gles:
			self.gles.setdefault((gle.voucher_type, gle.voucher_no), dontmanage._dict({})).setdefault(
				"existing", []
			).append(gle.update({"old": True}))

	def generate_preview_data(self):
		dontmanage.flags.through_repost_accounting_ledger = True
		self.gl_entries = []
		self.get_existing_ledger_entries()
		for x in self.vouchers:
			doc = dontmanage.get_doc(x.voucher_type, x.voucher_no)
	...
	def generate_preview(self):
		...
		rendered_page = dontmanage.render_template(
			"dontmanageerp/accounts/doctype/repost_accounting_ledger/repost_accounting_ledger.html",
	...
	def on_submit(self):
		if len(self.vouchers) > 5:
			...
			dontmanage.enqueue(
				method="dontmanageerp.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger.start_repost",
				account_repost_doc=self.name,
				is_async=True,
				job_name=job_name,
			)
			dontmanage.msgprint(_("Repost has started in the background"))
		else:
...
def start_repost(account_repost_doc=str) -> None:
	...
	if account_repost_doc:
		repost_doc = dontmanage.get_doc("Repost Accounting Ledger", account_repost_doc)
		...
		if repost_doc.docstatus == 1:
			...
			for x in repost_doc.vouchers:
				...
				if repost_doc.delete_cancelled_entries:
					dontmanage.db.delete("GL Entry", filters={"voucher_type": doc.doctype, "voucher_no": doc.name})
...
def validate_docs_for_deferred_accounting(sales_docs, purchase_docs):
	docs_with_deferred_revenue = dontmanage.db.get_all(
		"Sales Invoice Item",
	...
	docs_with_deferred_expense = dontmanage.db.get_all(
	...
	if docs_with_deferred_revenue or docs_with_deferred_expense:
		dontmanage.throw(
			_("Documents: {0} have deferred revenue/expense enabled for them. Cannot repost.").format(
				dontmanage.bold(comma_and([x[0] for x in docs_with_deferred_expense + docs_with_deferred_revenue]))
			)
		)
	...
	if disallowed_types := voucher_types.difference(allowed_types):
		...
		dontmanage.throw(
			_("{0} {1} not allowed to be reposted. Modify {2} to enable reposting.").format(
				dontmanage.bold(comma_and(list(disallowed_types))),
				message,
				dontmanage.bold(
					dontmanage.utils.get_link_to_form(
...
def get_repost_allowed_types(doctype, txt, searchfield, start, page_len, filters):
	...
	if allowed_types := dontmanage.db.get_all(
