...
from dontmanage.www.printview import get_print_style
...
from dontmanageerp.accounts.report.general_ledger.general_ledger import execute as get_soa
...
class ProcessStatementOfAccounts(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.process_statement_of_accounts_cc.process_statement_of_accounts_cc import (
	...
	def validate(self):
		...
		if not self.customers:
			dontmanage.throw(_("Customers not selected."))
...
def get_customers_based_on_territory_or_customer_group(customer_collection, collection_name):
	...
	collection = dontmanage.get_doc(customer_collection, collection_name)
	selected = [
		customer.name
		for customer in dontmanage.get_list(
			customer_collection,
...
def get_customers_based_on_sales_person(sales_person):
	lft, rgt = dontmanage.db.get_value("Sales Person", sales_person, ["lft", "rgt"])
	...
	sales_person_records = dontmanage._dict()
	for d in records:
...
def get_recipients_and_cc(customer, doc):
	...
	if doc.cc_to != "":
		try:
			cc = [dontmanage.get_value("User", user.cc, "email") for user in doc.cc_to]
		except Exception:
			pass
...
def fetch_customers(customer_collection, collection_name, primary_mandatory):
	...
	if customer_collection == "Sales Person":
		...
		if not bool(customers):
			dontmanage.throw(_("No Customers found with selected options."))
	else:
		if customer_collection == "Sales Partner":
			customers = dontmanage.get_list(
				"Customer",
				fields=["name", "customer_name", "email_id"],
...
def get_customer_emails(customer_name, primary_mandatory, billing_and_primary=True):
	...
	billing_email = dontmanage.db.sql(
		"""
		SELECT
			email.email_id
		FROM
	...
	if len(billing_email) == 0 or (billing_email[0][0] is None):
		if billing_and_primary:
			dontmanage.throw(_("No billing email found for customer: {0}").format(customer_name))
		else:
			return ""

	if billing_and_primary:
		primary_email = dontmanage.get_value("Customer", customer_name, "email_id")
		if primary_email is None and int(primary_mandatory):
			dontmanage.throw(_("No primary email found for customer: {0}").format(customer_name))
...
def download_statements(document_name):
	...
	if report:
		dontmanage.local.response.filename = doc.name + ".pdf"
		dontmanage.local.response.filecontent = report
...
def send_emails(document_name, from_scheduler=False, posting_date=None):
	...
	if report:
		for customer, report_pdf in report.items():
			...
			if doc.sender:
				sender_email = dontmanage.db.get_value("Email Account", doc.sender, "email_id")
			else:
				sender_email = dontmanage.session.user
		...
		if doc.enable_auto_email and from_scheduler:
			...
			doc.add_comment("Comment", "Emails sent on: " + dontmanage.utils.format_datetime(dontmanage.utils.now()))
			if doc.report == "General Ledger":
