...
from dontmanageerp.accounts.doctype.process_statement_of_accounts.process_statement_of_accounts import (
	get_statement_dict,
	send_emails,
)
...
class TestProcessStatementOfAccounts(AccountsTestMixin, DontManageTestCase):
	def setUp(self):
		self.create_company()
	...
	def test_process_soa_for_ar(self):
		...
		expected_summary = dontmanage._dict(
			range1=100,
			range2=0,
			range3=0,
			range4=0,
...
def create_process_soa(**args):
	...
	dontmanage.delete_doc_if_exists("Process Statement Of Accounts", args.name)
	process_soa = dontmanage.new_doc("Process Statement Of Accounts")
	soa_dict = dontmanage._dict(
		name=args.name,
