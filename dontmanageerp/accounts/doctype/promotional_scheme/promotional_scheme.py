...
class TransactionExists(dontmanage.ValidationError):
	pass
...
class PromotionalScheme(Document):
	...
	def validate(self):
		...
		if not (self.price_discount_slabs or self.product_discount_slabs):
			dontmanage.throw(_("Price or product discount slabs are required"))
	...
	def validate_applicable_for(self):
		if self.applicable_for:
			applicable_for = dontmanage.scrub(self.applicable_for)

			if not self.get(applicable_for):
				msg = f"The field {dontmanage.bold(self.applicable_for)} is required"
	...
	def validate_pricing_rules(self):
		...
		for doc in invalid_pricing_rule:
			dontmanage.delete_doc("Pricing Rule", doc)
	...
	def get_invalid_pricing_rules(self):
		pr = dontmanage.qb.DocType("Pricing Rule")
		conditions = []
		...
		return dontmanage.qb.from_(pr).select(pr.name).where(Criterion.all(conditions)).run(pluck=True)
	...
	def update_pricing_rules(self, pricing_rules):
		...
		for doc in docs:
			...
			if doc.get("__islocal"):
				...
				doc.save()
				dontmanage.msgprint(_("Pricing Rule {0} is updated").format(doc.name))

		if count:
			dontmanage.msgprint(_("New {0} pricing rules are created").format(count))

	def on_trash(self):
		for rule in dontmanage.get_all("Pricing Rule", {"promotional_scheme": self.name}):
			dontmanage.delete_doc("Pricing Rule", rule.name)


def raise_for_transaction_exists(name):
	msg = f"""You can't change the {dontmanage.bold(_('Applicable For'))}
		because transactions are present against the Promotional Scheme {dontmanage.bold(name)}. """
	msg += "Kindly disable this Promotional Scheme and create new for new Applicable For."

	dontmanage.throw(_(msg), TransactionExists)
...
def _get_pricing_rules(doc, child_doc, discount_fields, rules=None):
	...
	applicable_for = dontmanage.scrub(doc.get("applicable_for"))
...
def get_pricing_rule_docname(
	...
	docname = dontmanage.get_all("Pricing Rule", fields=fields, filters=filters)
...
def prepare_pricing_rule(
	...
	if docname:
		pr = dontmanage.get_doc("Pricing Rule", docname)
	else:
		pr = dontmanage.new_doc("Pricing Rule")
...
def set_args(args, pr, doc, child_doc, discount_fields, child_doc_fields):
	...
	for field in ["items", "item_groups", "brands"]:
		...
		apply_on = dontmanage.scrub(doc.get("apply_on"))
