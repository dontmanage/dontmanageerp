...
from dontmanageerp.accounts.doctype.promotional_scheme.promotional_scheme import TransactionExists
from dontmanageerp.selling.doctype.sales_order.test_sales_order import make_sales_order


class TestPromotionalScheme(unittest.TestCase):
	def setUp(self):
		if dontmanage.db.exists("Promotional Scheme", "_Test Scheme"):
			dontmanage.delete_doc("Promotional Scheme", "_Test Scheme")

	def test_promotional_scheme(self):
		ps = make_promotional_scheme(applicable_for="Customer", customer="_Test Customer")
		price_rules = dontmanage.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name", "creation"],
			filters={"promotional_scheme": ps.name},
		)
		self.assertTrue(len(price_rules), 1)
		price_doc_details = dontmanage.db.get_value(
		...
		dontmanage.delete_doc("Promotional Scheme", ps.name)
		price_rules = dontmanage.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name"],
			filters={"promotional_scheme": ps.name},
	...
	def test_promotional_scheme_without_applicable_for(self):
		...
		price_rules = dontmanage.get_all("Pricing Rule", filters={"promotional_scheme": ps.name})
	...
	def test_min_max_amount_configuration(self):
		...
		price_rules_data = dontmanage.db.get_value(
	...
	def test_pricing_rule_for_product_discount_slabs(self):
		...
		ps.save()
		pr = dontmanage.get_doc("Pricing Rule", {"promotional_scheme_id": ps.product_discount_slabs[0].name})
	...
	def test_validation_on_recurse_with_mixed_condition(self):
		...
		self.assertRaises(dontmanage.ValidationError, ps.save)


def make_promotional_scheme(**args):
	args = dontmanage._dict(args)

	ps = dontmanage.new_doc("Promotional Scheme")
	...
	if args.applicable_for:
		...
		ps.append(
			dontmanage.scrub(args.applicable_for),
			{dontmanage.scrub(args.applicable_for): args.get(dontmanage.scrub(args.applicable_for))},
		)
