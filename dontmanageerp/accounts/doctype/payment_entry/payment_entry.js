...
dontmanageerp.accounts.taxes.setup_tax_validations("Payment Entry");
dontmanageerp.accounts.taxes.setup_tax_filters("Advance Taxes and Charges");
...
dontmanage.ui.form.on('Payment Entry', {
	...
	setup: function(frm) {
		frm.set_query("paid_from", function() {
			...
			var account_types = in_list(["Pay", "Internal Transfer"], frm.doc.payment_type) ?
				["Bank", "Cash"] : [dontmanage.boot.party_account_types[frm.doc.party_type]];
			return {
				filters: {
		...
		frm.set_query("party_type", function() {
			...
			return{
				filters: {
					"name": ["in", Object.keys(dontmanage.boot.party_account_types)],
				}
			}
		});
		...
		frm.set_query('payment_term', 'references', function(frm, cdt, cdn) {
			...
			if (in_list(['Purchase Invoice', 'Sales Invoice'], child.reference_doctype) && child.reference_name) {
				return {
					query: "dontmanageerp.controllers.queries.get_payment_terms_for_references",
					filters: {
	...
	refresh: function(frm) {
		...
		dontmanageerp.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if((frm.doc.references) && (frm.doc.references.find((elem) => {return elem.exchange_gain_loss != 0}))) {
			frm.add_custom_button(__("View Exchange Gain/Loss Journals"), function() {
				dontmanage.set_route("List", "Journal Entry", {"voucher_type": "Exchange Gain Or Loss", "reference_name": frm.doc.name});
			}, __('Actions'));
	...
	hide_unhide_fields: function(frm) {
		var company_currency = frm.doc.company? dontmanage.get_doc(":Company", frm.doc.company).default_currency: "";
	...
	mode_of_payment: function(frm) {
		dontmanageerp.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function(account){
	...
	party_type: function(frm) {
		...
		let party_types = Object.keys(dontmanage.boot.party_account_types);
		if(frm.doc.party_type && !party_types.includes(frm.doc.party_type)){
			frm.set_value("party_type", "");
			dontmanage.throw(__("Party can only be one of {0}", [party_types.join(", ")]));
		}
		...
		frm.set_query("party", function() {
			if(frm.doc.party_type == 'Employee'){
				...
				return {
					query: "dontmanageerp.controllers.queries.customer_query"
				}
			}
		});
	...
	party: function(frm) {
		...
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			if(!frm.doc.posting_date) {
				dontmanage.msgprint(__("Please select Posting Date before selecting Party"))
				frm.set_value("party", "");
				return ;
			}
			frm.set_party_account_based_on_party = true;

			let company_currency = dontmanage.get_doc(":Company", frm.doc.company).default_currency;

			return dontmanage.call({
				method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	apply_tax_withholding_amount: function(frm) {
		if (!frm.doc.apply_tax_withholding_amount) {
			...
			dontmanage.db.get_value('Supplier', frm.doc.party, 'tax_withholding_category', (values) => {
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		if (frm.doc.posting_date && account) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					"account": account,
					"date": frm.doc.posting_date,
					"cost_center": frm.doc.cost_center
	...
	source_exchange_rate: function(frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", dontmanageerp.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function(frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", dontmanageerp.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_invoices_or_orders: function(frm, get_outstanding_invoices, get_orders_to_be_billed) {
		const today = dontmanage.datetime.get_today();
		let fields = [
			{fieldtype:"Section Break", label: __("Posting Date")},
			{fieldtype:"Date", label: __("From Date"),
				fieldname:"from_posting_date", default:dontmanage.datetime.add_days(today, -30)},
		...
		dontmanage.prompt(fields, function(filters){
			dontmanage.flags.allocate_payment_amount = true;
	...
	get_outstanding_documents: function(frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		dontmanage.flags.allocate_payment_amount = filters['allocate_payment_amount'];

		return  dontmanage.call({
			method: 'dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents',
			args: {
				args:args
			},
			callback: function(r, rt) {
	...
	allocate_party_amount_against_ref_docs: function(frm, paid_amount, paid_amount_change) {
		...
		var total_negative_outstanding = 0;
		var total_deductions = dontmanage.utils.sum($.map(frm.doc.deductions || [],
		...
		$.each(frm.doc.references || [], function(i, row) {
			if (dontmanage.flags.allocate_payment_amount == 0) {
				//If allocate payment amount checkbox is unchecked, set zero to allocate amount
				row.allocated_amount = 0;

			} else if (dontmanage.flags.allocate_payment_amount != 0 && (!row.allocated_amount || paid_amount_change)) {
	...
	check_mandatory_to_fetch: function(frm) {
		$.each(["Company", "Party Type", "Party", "payment_type"], function(i, field) {
			if(!frm.doc[dontmanage.model.scrub(field)]) {
				dontmanage.msgprint(__("Please select {0} first", [field]));
				return false;
			}
	...
	validate_reference_document: function(frm, row) {
		var _validate = function(i, row) {
			...
			if(frm.doc.party_type=="Customer" &&
				...
				dontmanage.model.set_value(row.doctype, row.name, "reference_doctype", null);
				dontmanage.msgprint(__("Row #{0}: Reference Document Type must be one of Sales Order, Sales Invoice, Journal Entry or Dunning", [row.idx]));
				return false;
			}
			...
			if(frm.doc.party_type=="Supplier" &&
				...
				dontmanage.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				dontmanage.msgprint(__("Row #{0}: Reference Document Type must be one of Purchase Order, Purchase Invoice or Journal Entry", [row.idx]));
				return false;
			}
		}
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
					if(r.message) {
						...
						const add_deductions = (details) => {
							...
							if (row) {
								...
								dontmanage.msgprint(__("No gain or loss in the exchange rate"))
							}
	...
	bank_account: function(frm) {
		...
		if (frm.doc.bank_account && in_list(['Pay', 'Receive'], frm.doc.payment_type)) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
...
dontmanage.ui.form.on('Payment Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return dontmanage.call({
				method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
...
dontmanage.ui.form.on('Advance Taxes and Charges', {
	rate: function(frm) {
		frm.events.apply_taxes(frm);
		frm.events.set_unallocated_amount(frm);
	},
...
dontmanage.ui.form.on('Payment Entry Deduction', {
	amount: function(frm) {
		frm.events.set_unallocated_amount(frm);
	},
...
dontmanage.ui.form.on('Payment Entry', {
	cost_center: function(frm){
		if (frm.doc.posting_date && (frm.doc.paid_from||frm.doc.paid_to)) {
			return dontmanage.call({
				method: "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
