...
dontmanage.ui.form.on("Payment Entry", {
	...
	setup: function (frm) {
		frm.set_query("paid_from", function () {
			...
			var account_types = ["Pay", "Internal Transfer"].includes(frm.doc.payment_type)
				? ["Bank", "Cash"]
				: [dontmanage.boot.party_account_types[frm.doc.party_type]];
		...
		frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
			const row = dontmanage.get_doc(cdt, cdn);
			return {
				query: "dontmanageerp.accounts.doctype.payment_request.payment_request.get_open_payment_requests_query",
				filters: {
	...
	refresh: function (frm) {
		...
		if (
			...
			frm.add_custom_button(
				...
				function () {
					dontmanage.set_route("List", "Journal Entry", {
	...
	hide_unhide_fields: function (frm) {
		var company_currency = frm.doc.company
			? dontmanage.get_doc(":Company", frm.doc.company)?.default_currency
			: "";
	...
	get_outstanding_invoices_or_orders: function (frm, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		let fields = [
			{ fieldtype: "Section Break", label: __("Posting Date") },
			{
				...
				default: dontmanage.datetime.add_days(today, -30),
			},
	...
	validate_filters_data: function (frm, filters) {
		...
		for (let key in fields) {
			...
			if (filters[from_field] && !filters[to_field]) {
				dontmanage.throw(__("Error: {0} is mandatory field", [to_field.replace(/_/g, " ")]));
	...
	allocate_party_amount_against_ref_docs: async function (frm, paid_amount, paid_amount_change) {
		await frm.call("allocate_amount_to_references", {
			...
			allocate_payment_amount: dontmanage.flags.allocate_payment_amount ?? false,
		});
	...
	set_difference_amount: function (frm) {
		...
		var total_deductions = dontmanage.utils.sum(
	...
	after_save: function (frm) {
		const { matched_payment_requests } = dontmanage.last_response;
...
dontmanage.ui.form.on("Payment Entry Deduction", {
	before_deductions_remove: function (doc, cdt, cdn) {
		...
		if (row.is_exchange_gain_loss && row.amount) {
			dontmanage.throw(__("Cannot delete Exchange Gain/Loss row"));
		}
	},
...
function prompt_for_missing_account(frm, account) {
	return new Promise((resolve) => {
		const dialog = dontmanage.prompt(
			{
				label: __(dontmanage.unscrub(account)),
				fieldname: account,
				fieldtype: "Link",
				options: "Account",
				get_query: () => ({
...
function get_deduction_amount_precision() {
	return dontmanage.meta.get_field_precision(dontmanage.meta.get_field("Payment Entry Deduction", "amount"));
}
