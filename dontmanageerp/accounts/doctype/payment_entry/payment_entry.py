...
import dontmanage
from dontmanage import ValidationError, _, qb, scrub, throw
from dontmanage.query_builder import Tuple
from dontmanage.query_builder.functions import Count
from dontmanage.utils import cint, comma_or, flt, getdate, nowdate
from dontmanage.utils.data import comma_and, fmt_money, get_link_to_form
...
from dontmanageerp.accounts.doctype.bank_account.bank_account import (
	get_bank_account_details,
	get_default_company_bank_account,
	get_party_bank_account,
)
from dontmanageerp.accounts.doctype.invoice_discounting.invoice_discounting import (
...
from dontmanageerp.accounts.general_ledger import (
	make_gl_entries,
...
class PaymentEntry(AccountsController):
	...
	def on_submit(self):
		if self.difference_amount:
			dontmanage.throw(_("Difference Amount must be zero"))
	...
	def set_liability_account(self):
		...
		account_type = dontmanage.get_value(
		...
		if not liability_account:
			throw(
				_("Please set default {0} in Company {1}").format(
					dontmanage.bold(dontmanage.get_meta("Company").get_label(fieldname)), dontmanage.bold(self.company)
				)
			)
		...
		dontmanage.msgprint(
			_(
				...
				dontmanage.bold(self.party_account),
				dontmanage.bold(liability_account),
			),
			alert=True,
		)
	...
	def update_payment_requests(self, cancel=False):
		from dontmanageerp.accounts.doctype.payment_request.payment_request import (
		...
		if self.party_type in ("Customer", "Supplier"):
			...
			for d in self.get("references"):
				if (flt(d.allocated_amount)) > 0 and flt(d.allocated_amount) > flt(d.outstanding_amount):
					dontmanage.throw(fail_message.format(d.idx))
	...
	def term_based_allocation_enabled_for_reference(
		...
		if (
			...
			if template := dontmanage.db.get_value(reference_doctype, reference_name, "payment_terms_template"):
	...
	def validate_allocated_amount_with_latest_data(self):
		if self.references:
			...
			vouchers = [dontmanage._dict({"voucher_type": x[0], "voucher_no": x[1]}) for x in uniq_vouchers]
			...
			for d in latest_references:
				...
				latest_lookup.setdefault((d.voucher_type, d.voucher_no), dontmanage._dict())[d.payment_term] = d

			for idx, d in enumerate(self.get("references"), start=1):
				latest = latest_lookup.get((d.reference_doctype, d.reference_name)) or dontmanage._dict()
				...
				if (
					...
					dontmanage.throw(
						...
						).format(dontmanage.bold(d.reference_name), dontmanage.bold(idx))
					)
	...
	def delink_advance_entry_references(self):
		for reference in self.references:
			if reference.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				doc = dontmanage.get_doc(reference.reference_doctype, reference.reference_name)
	...
	def set_missing_values(self):
		if self.payment_type == "Internal Transfer":
			...
			if not self.party_type:
				dontmanage.throw(_("Party Type is mandatory"))

			if not self.party:
				dontmanage.throw(_("Party is mandatory"))

			_party_name = "title" if self.party_type == "Shareholder" else self.party_type.lower() + "_name"

			if dontmanage.db.has_column(self.party_type, _party_name):
				self.party_name = dontmanage.db.get_value(self.party_type, self.party, _party_name)
			else:
				self.party_name = dontmanage.db.get_value(self.party_type, self.party, "name")
	...
	def validate_payment_type(self):
		if self.payment_type not in ("Receive", "Pay", "Internal Transfer"):
			dontmanage.throw(_("Payment Type must be one of Receive, Pay and Internal Transfer"))

	def validate_party_details(self):
		if self.party:
			if not dontmanage.db.exists(self.party_type, self.party):
				dontmanage.throw(_("{0} {1} does not exist").format(_(self.party_type), self.party))
	...
	def validate_mandatory(self):
		for field in ("paid_amount", "received_amount", "source_exchange_rate", "target_exchange_rate"):
			if not self.get(field):
				dontmanage.throw(_("{0} is mandatory").format(self.meta.get_label(field)))
	...
	def validate_reference_documents(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not dontmanage.db.exists(d.reference_doctype, d.reference_name):
					dontmanage.throw(_("{0} {1} does not exist").format(d.reference_doctype, d.reference_name))
				else:
					ref_doc = dontmanage.get_doc(d.reference_doctype, d.reference_name)
					...
					if d.reference_doctype in dontmanage.get_hooks("invoice_doctypes"):
						if self.party_type == "Customer":
							ref_party_account = (
	...
	def validate_paid_invoices(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				outstanding_amount, is_return = dontmanage.get_cached_value(
		...
		for reference_doctype, references in no_oustanding_refs.items():
			dontmanage.msgprint(
				_(
					...
					dontmanage.bold(comma_and([d.reference_name for d in references])),
					_(reference_doctype),
				)
				+ "<br><br>"
	...
	def update_payment_schedule(self, cancel=0):
		...
		for ref in self.get("references"):
			if ref.payment_term and ref.reference_name:
				...
				if not invoice_paid_amount_map.get(key):
					payment_schedule = dontmanage.get_all(
						"Payment Schedule",
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
			if not invoice_paid_amount_map.get(key):
				dontmanage.throw(_("Payment term {0} not used in {1}").format(key[0], key[1]))
	...
	def get_allocated_amount_in_transaction_currency(
		...
		* PE in INR (accounting done in base currency)
		"""
		ref_currency, ref_exchange_rate = dontmanage.db.get_value(
	...
	def set_total_in_words(self):
		from dontmanage.utils import money_in_words
	...
	def validate_received_amount(self):
		if self.paid_from_account_currency == self.paid_to_account_currency:
			if self.paid_amount < self.received_amount:
				dontmanage.throw(_("Received Amount cannot be greater than Paid Amount"))
	...
	def calculate_base_allocated_amount_for_reference(self, d) -> float:
		...
		if d.reference_doctype in dontmanage.get_hooks("advance_payment_doctypes"):
	...
	def set_exchange_gain_loss(self):
		...
		if not exchange_gain_loss_row:
			values = dontmanage.get_cached_value(
			...
			for fieldname, value in values.items():
				...
				label = _(dontmanage.get_meta("Company").get_label(fieldname))
	...
	def set_title(self):
		if dontmanage.flags.in_import and self.title:
	...
	def validate_transaction_reference(self):
		...
		bank_account_type = dontmanage.get_cached_value("Account", bank_account, "account_type")

		if bank_account_type == "Bank":
			if not self.reference_no or not self.reference_date:
				dontmanage.throw(_("Reference No and Reference Date is mandatory for Bank transaction"))
	...
	def add_party_gl_entries(self, gl_entries):
		...
		party_account_type = dontmanage.db.get_value("Party Type", self.party_type, "account_type")
		...
		for d in self.get("references"):
			...
			if d.reference_doctype == "Sales Invoice" and not cost_center:
				cost_center = dontmanage.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
	...
	def add_advance_gl_for_reference(self, gl_entries, invoice):
		...
		if self.reconcile_on_advance_payment_date:
			...
			posting_date = dontmanage.db.get_value(invoice.reference_doctype, invoice.reference_name, date_field)
	...
	def add_tax_gl_entries(self, gl_entries):
		for d in self.get("taxes"):
			...
			if account_currency != self.company_currency:
				dontmanage.throw(_("Currency for {0} must be {1}").format(d.account_head, self.company_currency))
	...
	def add_deductions_gl_entries(self, gl_entries):
		for d in self.get("deductions"):
			if d.amount:
				...
				if account_currency != self.company_currency:
					dontmanage.throw(_("Currency for {0} must be {1}").format(d.account, self.company_currency))
	...
	def update_advance_paid(self):
		if self.payment_type in ("Receive", "Pay") and self.party:
			for d in self.get("references"):
				if d.allocated_amount and d.reference_doctype in dontmanage.get_hooks("advance_payment_doctypes"):
					dontmanage.get_doc(
	...
	def calculate_deductions(self, tax_details):
		return {
			...
			"cost_center": dontmanage.get_cached_value("Company", self.company, "cost_center"),
	...
	def set_matched_unset_payment_requests_to_response(self):
		...
		And set to `dontmanage.response` to show in the frontend for allocation.
		"""
		if not self.references:
			return
		...
		dontmanage.response["matched_payment_requests"] = matched_payment_requests
...
def get_outstanding_of_references_with_payment_term(references=None):
	...
	PS = dontmanage.qb.DocType("Payment Schedule")
...
def get_outstanding_reference_documents(args, validate=False):
	...
	company_currency = dontmanage.get_cached_value("Company", args.get("company"), "default_currency")

	# Get positive outstanding sales /purchase invoices
	condition = ""
	if args.get("voucher_type") and args.get("voucher_no"):
		condition = " and voucher_type={} and voucher_no={}".format(
			dontmanage.db.escape(args["voucher_type"]), dontmanage.db.escape(args["voucher_no"])
		)
	...
	if args.get("company"):
		condition += " and company = {}".format(dontmanage.db.escape(args.get("company")))
	...
	if args.get("get_outstanding_invoices"):
		...
		for d in outstanding_invoices:
			...
			if party_account_currency != company_currency:
				if d.voucher_type in dontmanage.get_hooks("invoice_doctypes"):
					d["exchange_rate"] = dontmanage.db.get_value(d.voucher_type, d.voucher_no, "conversion_rate")
				elif d.voucher_type == "Journal Entry":
					d["exchange_rate"] = get_exchange_rate(
						party_account_currency, company_currency, d.posting_date
					)
			if d.voucher_type in ("Purchase Invoice"):
				d["bill_no"] = dontmanage.db.get_value(d.voucher_type, d.voucher_no, "bill_no")
		...
		if not validate:
			dontmanage.msgprint(
				_(
					...
					_(ref_document_type), _(args.get("party_type")).lower(), dontmanage.bold(args.get("party"))
				)
			)
...
def split_invoices_based_on_payment_terms(outstanding_invoices, company) -> list:
	...
	for entry in outstanding_invoices:
		if entry.voucher_type in ["Sales Invoice", "Purchase Invoice"]:
			if payment_term_template := dontmanage.db.get_value(
				...
				if len(split_rows) > 1:
					dontmanage.msgprint(
						_("Splitting {0} {1} into {2} rows as per Payment Terms").format(
							_(entry.voucher_type), dontmanage.bold(entry.voucher_no), len(split_rows)
						),
						alert=True,
					)
...
def get_currency_data(outstanding_invoices: list, company: str | None = None) -> dict:
	...
	exc_rates = dontmanage._dict()
	company_currency = dontmanage.db.get_value("Company", company, "default_currency") if company else None
	...
	for doctype in ["Sales Invoice", "Purchase Invoice"]:
		...
		for x in dontmanage.db.get_all(
			...
			exc_rates[x.name] = dontmanage._dict(
...
def get_orders_to_be_billed(
	...
	# Add cost center condition
	doc = dontmanage.get_doc({"doctype": voucher_type})
	condition = ""
...
def get_party_details(company, party_type, party, date, cost_center=None):
	...
	if not dontmanage.db.exists(party_type, party):
		dontmanage.throw(_("{0} {1} does not exist").format(_(party_type), party))

	party_account = get_party_account(party_type, party, company)
	account_currency = get_account_currency(party_account)
	account_balance = get_balance_on(party_account, date, cost_center=cost_center)
	_party_name = "title" if party_type == "Shareholder" else party_type.lower() + "_name"
	party_name = dontmanage.db.get_value(party_type, party, _party_name)
...
def get_account_details(account, date, cost_center=None):
	dontmanage.has_permission("Payment Entry", throw=True)

	# to check if the passed account is accessible under reference doctype Payment Entry
	account_list = dontmanage.get_list("Account", {"name": account}, reference_doctype="Payment Entry", limit=1)

	# There might be some user permissions which will allow account under certain doctypes
	# except for Payment Entry, only in such case we should throw permission error
	if not account_list:
		dontmanage.throw(_("Account: {0} is not permitted under Payment Entry").format(account))
	...
	return dontmanage._dict(
		{
			...
			"account_balance": account_balance,
			"account_type": dontmanage.get_cached_value("Account", account, "account_type"),
		}
	)
...
def get_company_defaults(company):
	...
	return dontmanage.get_cached_value("Company", company, fields, as_dict=1)
...
def get_reference_details(
	...
	company_currency = ref_doc.get("company_currency") or dontmanageerp.get_company_currency(ref_doc.company)
...
def get_payment_entry(
	...
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) >= (100.0 + over_billing_allowance):
		dontmanage.throw(_("Can only make payment against unbilled {0}").format(_(dt)))
	...
	if party_type in ["Customer", "Supplier"] and not bank:
		...
		if party_bank_account:
			account = dontmanage.db.get_value("Bank Account", party_bank_account, "account")
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		dontmanage.msgprint(_("{0} is on hold till {1}").format(doc.name, doc.release_date))
	else:
		if doc.doctype in (
			"Sales Invoice",
			"Purchase Invoice",
	...
	if party_account and bank:
		if discount_amount:
			...
			if dontmanage.db.get_single_value("Accounts Settings", "book_tax_discount_loss"):
...
def get_open_payment_requests_for_references(references=None):
	...
	response = (
		...
		.orderby(Coalesce(PR.transaction_date, PR.creation), order=dontmanage.qb.asc)
	).run(as_dict=True)
	...
	while row_number <= len(references):
		...
		if pr_outstanding_amount == allocated_amount:
			...
			while allocated_amount:
				...
				new_row = dontmanage.copy_doc(row)
...
def set_paid_amount_and_received_amount(
	...
	if party_account_currency == bank.account_currency:
		...
		company_currency = dontmanage.get_cached_value("Company", doc.get("company"), "default_currency")
...
def apply_early_payment_discount(paid_amount, received_amount, doc, party_account_currency, reference_date):
	...
	if doc.doctype in eligible_for_payments and has_payment_schedule:
		...
		if total_discount:
			...
			money = dontmanage.utils.fmt_money(total_discount, currency=currency)
			dontmanage.msgprint(_("Discount of {} applied as per Payment Term").format(money), alert=1)
...
def set_pending_discount_loss(pe, doc, discount_amount, base_total_discount_loss, party_account_currency):
	...
	if discount_amount > 0.0:
		...
		book_tax_loss = dontmanage.db.get_single_value("Accounts Settings", "book_tax_discount_loss")
		account_type = "round_off_account" if book_tax_loss else "default_discount_account"

		pe.append(
			"deductions",
			{
				"account": dontmanage.get_cached_value("Company", pe.company, account_type),
				"cost_center": pe.cost_center
				or dontmanage.get_cached_value("Company", pe.company, "cost_center"),
...
def add_income_discount_loss(pe, doc, total_discount_percent) -> float:
	...
	pe.append(
		"deductions",
		{
			"account": dontmanage.get_cached_value("Company", pe.company, "default_discount_account"),
			"cost_center": pe.cost_center or dontmanage.get_cached_value("Company", pe.company, "cost_center"),
