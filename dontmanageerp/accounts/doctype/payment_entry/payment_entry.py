...
from dontmanageerp.accounts.general_ledger import make_gl_entries, process_gl_map
from dontmanageerp.accounts.party import get_party_account
from dontmanageerp.accounts.utils import get_account_currency, get_balance_on, get_outstanding_invoices
...
class PaymentEntry(AccountsController):
	...
	def validate_party_details(self):
		if self.party:
			if not dontmanage.db.exists(self.party_type, self.party):
				dontmanage.throw(_("Invalid {0}: {1}").format(self.party_type, self.party))
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not dontmanage.db.exists(d.reference_doctype, d.reference_name):
					...
					if ref_doc.docstatus != 1:
						dontmanage.throw(_("{0} {1} must be submitted").format(d.reference_doctype, d.reference_name))
		...
		for k, v in no_oustanding_refs.items():
			dontmanage.msgprint(
				_(
					...
					_(k),
					dontmanage.bold(", ".join(d.reference_name for d in v)),
					dontmanage.bold(_("negative outstanding amount")),
				)
				+ "<br><br>"
	...
	def validate_payment_against_negative_invoice(self):
		...
		if not total_negative_outstanding:
			...
			dontmanage.throw(msg, InvalidPaymentEntry)
	...
	def validate_transaction_reference(self):
		...
		bank_account_type = dontmanage.db.get_value("Account", bank_account, "account_type")
	...
	def add_party_gl_entries(self, gl_entries):
		if self.party_account:
			...
			dr_or_cr = (
				"credit" if dontmanageerp.get_party_account_type(self.party_type) == "Receivable" else "debit"
			)
...
def get_outstanding_reference_documents(args):
	...
	if not data:
		dontmanage.msgprint(
			...
			).format(_(args.get("party_type")).lower(), dontmanage.bold(args.get("party")))
		)
...
def split_invoices_based_on_payment_terms(outstanding_invoices):
	...
	for idx, d in enumerate(outstanding_invoices):
		if d.voucher_type in ["Sales Invoice", "Purchase Invoice"]:
			payment_term_template = dontmanage.db.get_value(
...
def get_party_details(company, party_type, party, date, cost_center=None):
	...
	if not dontmanage.db.exists(party_type, party):
		dontmanage.throw(_("Invalid {0}: {1}").format(party_type, party))
	...
	return dontmanage._dict(
		{
			...
			"account_balance": account_balance,
			"account_type": dontmanage.db.get_value("Account", account, "account_type"),
		}
	)
...
def get_company_defaults(company):
	...
	ret = dontmanage.get_cached_value("Company", company, fields, as_dict=1)
...
def get_payment_entry(
	...
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) >= 99.99:
		dontmanage.throw(_("Can only make payment against unbilled {0}").format(dt))
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		...
		if doc.doctype in ("Sales Invoice", "Purchase Invoice") and dontmanage.get_value(
...
def apply_early_payment_discount(paid_amount, received_amount, doc):
	...
	if doc.doctype in eligible_for_payments and has_payment_schedule:
		...
		if total_discount:
			money = dontmanage.utils.fmt_money(total_discount, currency=doc.get("currency"))
