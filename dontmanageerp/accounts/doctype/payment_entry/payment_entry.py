...
from dontmanage.utils.data import comma_and, fmt_money
...
class PaymentEntry(AccountsController):
	...
	def set_payment_req_status(self):
		from dontmanageerp.accounts.doctype.payment_request.payment_request import update_payment_req_status
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not dontmanage.db.exists(d.reference_doctype, d.reference_name):
					...
					if ref_doc.docstatus != 1:
						dontmanage.throw(_("{0} {1} must be submitted").format(_(d.reference_doctype), d.reference_name))
	...
	def get_order_net_total(self):
		...
		tax_withholding_net_total = dontmanage.db.get_value(
		...
		if self.party_account:
			...
			if self.book_advance_payments_in_separate_party_account:
				...
				for d in self.get("references"):
					...
					if d.reference_doctype in ["Sales Invoice", "Purchase Invoice"]:
						is_return = dontmanage.db.get_value(d.reference_doctype, d.reference_name, "is_return")
...
def get_outstanding_reference_documents(args, validate=False):
	...
	if args.get("company"):
		condition += " and company = {0}".format(dontmanage.db.escape(args.get("company")))
...
def get_currency_data(outstanding_invoices: list, company: str = None) -> dict:
	...
	company_currency = (
		dontmanage.db.get_value("Company", company, "default_currency") if company else None
	)
...
def get_account_details(account, date, cost_center=None):
	...
	account_list = dontmanage.get_list(
