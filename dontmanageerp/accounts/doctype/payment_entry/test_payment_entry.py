...
from dontmanage.utils import add_days, flt, nowdate
...
from dontmanageerp.accounts.doctype.sales_invoice.test_sales_invoice import (
...
class TestPaymentEntry(DontManageTestCase):
	def tearDown(self):
	...
	def test_payment_entry_against_order(self):
		...
		so_advance_paid = dontmanage.db.get_value("Sales Order", so.name, "advance_paid")
	...
	def test_payment_entry_against_si_usd_to_usd(self):
		...
		outstanding_amount = flt(dontmanage.db.get_value("Sales Invoice", si.name, "outstanding_amount"))
	...
	def test_payment_entry_against_pi(self):
		...
		outstanding_amount = flt(dontmanage.db.get_value("Sales Invoice", pi.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_payment_against_sales_invoice_to_check_status(self):
		...
		outstanding_amount, status = dontmanage.db.get_value(
	...
	def test_payment_entry_against_payment_terms_with_discount(self):
		...
		dontmanage.db.set_value("Company", si.company, "default_discount_account", "Write Off - _TC")
		...
		dontmanage.db.set_single_value("Accounts Settings", "book_tax_discount_loss", 1)
		...
		dontmanage.db.set_single_value("Accounts Settings", "book_tax_discount_loss", 0)
		pe = get_payment_entry("Sales Invoice", si.name, bank_account="_Test Cash - _TC")
	...
	def test_payment_entry_against_payment_terms_with_discount_amount(self):
		...
		pe_1 = get_payment_entry(
			...
			reference_date=dontmanage.utils.add_days(si.posting_date, 2),
		)
	...
	def test_payment_entry_retrieves_last_exchange_rate(self):
		from dontmanageerp.setup.doctype.currency_exchange.test_currency_exchange import (
			save_new_records,
			test_records,
		)
	...
	def test_payment_against_negative_sales_invoice(self):
		...
		si1_outstanding = dontmanage.db.get_value("Sales Invoice", si1.name, "outstanding_amount")

		# create JE(credit note) manually against si1 and cr_note
		je = dontmanage.get_doc(
			{
		...
		outstanding_amount = flt(dontmanage.db.get_value("Sales Invoice", si1.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_duplicate_payment_entry_allocate_amount(self):
		...
		self.assertRaises(dontmanage.ValidationError, pe_draft.submit)
	...
	def test_overallocation_validation_shouldnt_misfire(self):
		...
		template = dontmanage.get_doc("Payment Terms Template", "Test Receivable Template")
	...
	def check_gl_entries(self):
		...
		gl_entries = (
			...
			.orderby(gle.account, gle.debit, gle.credit, order=dontmanage.qb.desc)
		).run(as_dict=True)
	...
	def test_reverse_payment_reconciliation(self):
		customer = create_customer(dontmanage.generate_hash(length=10), "INR")
		pe = create_payment_entry(
			party_type="Customer",
			party=customer,
			payment_type="Receive",
	...
	def test_opening_flag_for_advance_as_liability(self):
		...
		gl_with_opening_set = dontmanage.db.get_all(
	...
	def test_delete_linked_exchange_gain_loss_journal(self):
		...
		pe = get_payment_entry("Sales Invoice", si.name)
		pe.reference_no = "1"
		pe.reference_date = dontmanage.utils.nowdate()
		pe.paid_amount = 100
		...
		self.assertTrue(dontmanage.db.get_value("Journal Entry", {"name": jv[0]}, "docstatus") == 2)

		# check deletion of payment entry and journal entry
		pe.delete()
		self.assertRaises(dontmanage.DoesNotExistError, dontmanage.get_doc, pe.doctype, pe.name)
		self.assertRaises(dontmanage.DoesNotExistError, dontmanage.get_doc, "Journal Entry", jv[0])


def create_payment_entry(**args):
	payment_entry = dontmanage.new_doc("Payment Entry")
...
def create_payment_terms_template():
	...
	if not dontmanage.db.exists("Payment Terms Template", "Test Receivable Template"):
		dontmanage.get_doc(
			{
...
def create_payment_terms_template_with_discount(
	...
	if not dontmanage.db.exists("Payment Terms Template", template_name):
		dontmanage.get_doc(
			{
				"doctype": "Payment Terms Template",
...
def create_payment_term(name):
	if not dontmanage.db.exists("Payment Term", name):
		dontmanage.get_doc({"doctype": "Payment Term", "payment_term_name": name}).insert()


def create_customer(name="_Test Customer 2 USD", currency="USD"):
	customer = None
	if dontmanage.db.exists("Customer", name):
		customer = name
	else:
