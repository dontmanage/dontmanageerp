		...
		def _import_accounts(children, parent, root_type, root_account=False):
			for account_name, child in children.items():
				...
				if account_name not in [
					...
					if root_account or dontmanage.local.flags.allow_unverified_charts:
...
def get_chart(chart_template, existing_company=None):
	...
	if existing_company:
		...
		from dontmanageerp.accounts.doctype.account.chart_of_accounts.verified import (
		...
		for folder in folders:
			...
			for fname in os.listdir(path):
				fname = dontmanage.as_unicode(fname)
...
def get_account_tree_from_existing_company(existing_company):
	all_accounts = dontmanage.get_all(
		"Account",
