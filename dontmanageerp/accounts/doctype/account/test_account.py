...
class TestAccount(unittest.TestCase):
	def test_rename_account(self):
		if not dontmanage.db.exists("Account", "1210 - Debtors - _TC"):
		...
		account_number, account_name = dontmanage.db.get_value(
		...
		new_acc = dontmanage.db.get_value(
			"Account",
	...
	def test_account_sync(self):
		...
		acc_tc_4 = dontmanage.db.get_value(
			"Account", {"account_name": "Test Sync Account", "company": "_Test Company 4"}
		)
		acc_tc_5 = dontmanage.db.get_value(
	...
	def test_add_account_to_a_group(self):
		dontmanage.db.set_value("Account", "Office Rent - _TC3", "is_group", 1)

		acc = dontmanage.new_doc("Account")
		acc.account_name = "Test Group Account"
		acc.parent_account = "Office Rent - _TC3"
		acc.company = "_Test Company 3"
		self.assertRaises(dontmanage.ValidationError, acc.insert)

		dontmanage.db.set_value("Account", "Office Rent - _TC3", "is_group", 0)
	...
	def test_child_company_account_rename_sync(self):
		...
		self.assertTrue(
			dontmanage.db.exists("Account", {"account_name": "Test Group Account", "company": "_Test Company 4"})
		)
		self.assertTrue(
			dontmanage.db.exists("Account", {"account_name": "Test Group Account", "company": "_Test Company 5"})
		)
		...
		self.assertRaises(dontmanage.ValidationError, update_account_number, acc_tc_5, "Test Modified Account")

		# Rename child company account with allow_account_creation_against_child_company enabled
		dontmanage.db.set_value("Company", "_Test Company 5", "allow_account_creation_against_child_company", 1)
		...
		dontmanage.db.set_value("Company", "_Test Company 5", "allow_account_creation_against_child_company", 0)
		...
		for doc in to_delete:
			dontmanage.delete_doc("Account", doc)
	...
	def test_validate_account_currency(self):
		...
		if not dontmanage.db.get_value("Account", "Test Currency Account - _TC"):
			acc = dontmanage.new_doc("Account")
			...
			acc = dontmanage.get_doc("Account", "Test Currency Account - _TC")
...
def get_inventory_account(company, warehouse=None):
	account = None
	if warehouse:
		account = get_warehouse_account(dontmanage.get_doc("Warehouse", warehouse))
	else:
