...
from dontmanage.utils.nestedset import NestedSet, get_ancestors_of, get_descendants_of

import dontmanageerp


class RootNotEditable(dontmanage.ValidationError):
	pass


class BalanceMismatchError(dontmanage.ValidationError):
	pass
...
class Account(NestedSet):
	...
	def on_update(self):
		if dontmanage.local.flags.ignore_update_nsm:
			return
		else:
			super().on_update()
	...
	def onload(self):
		...
		if not frozen_accounts_modifier or frozen_accounts_modifier in dontmanage.get_roles():
			self.set_onload("can_freeze_account", True)

	def autoname(self):
		from dontmanageerp.accounts.utils import get_autoname_with_number
		...
		if (
			...
			if dontmanage.db.get_all("GL Entry", filters={"account": self.name, "is_cancelled": 0}, limit=1):
				msg = _(
					...
					dontmanage.bold(_("Account Type")), doc_before_save.account_type, doc_before_save.account_type
				)
	...
	def validate_root_company_and_sync_account_to_children(self):
		...
		if dontmanage.local.flags.ignore_root_company_validation or self.flags.ignore_root_company_validation:
			return
		...
		if ancestors:
			...
			if not dontmanage.db.get_value(
				...
				dontmanage.throw(_("Please add the account to root level Company - {}").format(ancestors[0]))
			...
			for d in dontmanage.db.get_values(
	...
	def validate_account_currency(self):
		...
		if gl_currency and self.account_currency != gl_currency:
			if dontmanage.db.get_value("GL Entry", {"account": self.name}):
				dontmanage.throw(_("Currency can not be changed after making entries using some other currency"))
	...
	def validate_account_number(self, account_number=None):
		...
		if account_number:
			account_with_same_number = dontmanage.db.get_value(
				"Account",
	...
	def create_account_for_child_company(self, parent_acc_name_map, descendants, parent_acc_name):
		for company in descendants:
			company_bold = dontmanage.bold(company)
			parent_acc_name_bold = dontmanage.bold(parent_acc_name)
			...
			if dontmanage.get_cached_value(
				...
				dontmanage.throw(msg, title=_("Invalid Parent Account"))
			...
			child_account = dontmanage.db.get_value("Account", filters, "name")
			if not child_account:
				doc = dontmanage.copy_doc(self)
				...
				doc.save()
				dontmanage.msgprint(_("Account {0} is added in the child company {1}").format(doc.name, company))
			elif child_account:
				# update the parent company's value in child companies
				doc = dontmanage.get_doc("Account", child_account)
	...
	def check_gle_exists(self):
		return dontmanage.db.get_value("GL Entry", {"account": self.name})
...
def get_account_currency(account):
	...
	def generator():
		account_currency, company = dontmanage.get_cached_value(
			"Account", account, ["account_currency", "company"]
		)
		if not account_currency:
			account_currency = dontmanage.get_cached_value("Company", company, "default_currency")

		return account_currency

	return dontmanage.local_cache("account_currency", account, generator)


def on_doctype_update():
	dontmanage.db.add_index("Account", ["lft", "rgt"])


def get_account_autoname(account_number, account_name, company):
	# first validate if company exists
	company = dontmanage.get_cached_value("Company", company, ["abbr", "name"], as_dict=True)
	if not company:
		dontmanage.throw(_("Company {0} does not exist").format(company))
...
def update_account_number(name, account_name, account_number=None, from_descendant=False):
	...
	if ancestors and not allow_independent_account_creation:
		for ancestor in ancestors:
			old_name = dontmanage.db.get_value(
				"Account",
			...
			if old_name:
				...
				message = _("Account {0} exists in parent company {1}.").format(
					dontmanage.bold(old_acc_name), dontmanage.bold(ancestor)
				)
				message += "<br>"
				message += _("Renaming it is only allowed via parent company {0}, to avoid mismatch.").format(
					dontmanage.bold(ancestor)
				)
				message += "<br><br>"
				message += _("To overrule this, enable '{0}' in company {1}").format(
					allow_child_account_creation, dontmanage.bold(account.company)
				)

				dontmanage.throw(message, title=_("Rename Not Allowed"))

	account.validate_account_number(account_number)
	if account_number:
		dontmanage.db.set_value("Account", name, "account_number", account_number.strip())
	else:
		dontmanage.db.set_value("Account", name, "account_number", "")
	dontmanage.db.set_value("Account", name, "account_name", account_name.strip())
	...
	if name != new_name:
		dontmanage.rename_doc("Account", name, new_name, force=1)
		return new_name
...
def sync_update_account_number_in_child(
	...
	for d in dontmanage.db.get_values("Account", filters=filters, fieldname=["company", "name"], as_dict=True):
