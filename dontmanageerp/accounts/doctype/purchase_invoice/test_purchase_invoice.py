...
from dontmanageerp.buying.doctype.purchase_order.purchase_order import get_mapped_purchase_invoice
from dontmanageerp.buying.doctype.purchase_order.purchase_order import make_purchase_invoice as make_pi_from_po
...
from dontmanageerp.controllers.accounts_controller import get_payment_terms
...
class TestPurchaseInvoice(DontManageTestCase, StockTestMixin):
	...
	def test_gl_entries_without_perpetual_inventory(self):
		...
		pi = dontmanage.copy_doc(test_records[0])
		self.assertTrue(not cint(dontmanageerp.is_perpetual_inventory_enabled(pi.company)))
		pi.insert()
		pi.submit()
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		self.assertTrue(cint(dontmanageerp.is_perpetual_inventory_enabled(pi.company)), 1)

		self.check_gle_for_pi(pi.name)

	def test_terms_added_after_save(self):
		pi = dontmanage.copy_doc(test_records[1])
		pi.insert()
	...
	def test_payment_entry_unlink_against_purchase_invoice(self):
		...
		pi_doc = dontmanage.get_doc("Purchase Invoice", pi_doc.name)
		pi_doc.load_from_db()
		self.assertTrue(pi_doc.status, "Paid")

		self.assertRaises(dontmanage.LinkExistsError, pi_doc.cancel)
		unlink_payment_on_cancel_of_invoice()

	def test_purchase_invoice_for_blocked_supplier(self):
		supplier = dontmanage.get_doc("Supplier", "_Test Supplier")
		supplier.on_hold = 1
		supplier.save()

		self.assertRaises(dontmanage.ValidationError, make_purchase_invoice)
	...
	def test_purchase_invoice_blocked_invoice_must_be_in_future(self):
		...
		self.assertRaises(dontmanage.ValidationError, pi.save)
		pi.release_date = ""
		pi.save()
	...
	def test_purchase_invoice_temporary_blocked(self):
		...
		self.assertRaises(dontmanage.ValidationError, pe.save)
	...
	def test_purchase_invoice_with_exchange_rate_difference(self):
		...
		exchange_gain_loss_account = dontmanage.db.get_value("Company", pi.company, "exchange_gain_loss_account")
	...
	def test_purchase_invoice_change_naming_series(self):
		...
		self.assertRaises(dontmanage.CannotChangeConstantError, pi.save)
	...
	def test_purchase_invoice_with_advance(self):
		...
		jv = dontmanage.copy_doc(jv_test_records[1])
		jv.insert()
		jv.submit()
	...
	def test_total_purchase_cost_for_project(self):
		if not dontmanage.db.exists("Project", {"project_name": "_Test Project for Purchase"}):
			project = make_project({"project_name": "_Test Project for Purchase"})
		else:
			project = dontmanage.get_doc("Project", {"project_name": "_Test Project for Purchase"})

		existing_purchase_cost = dontmanage.db.sql(
		...
		self.assertEqual(
			dontmanage.db.get_value("Project", project.name, "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(
			dontmanage.db.get_value("Project", project.name, "total_purchase_cost"), existing_purchase_cost
		)
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		pi = make_purchase_invoice(
			update_stock=1,
			posting_date=dontmanage.utils.nowdate(),
			posting_time=dontmanage.utils.nowtime(),
	...
	def test_auto_batch(self):
		item_code = dontmanage.db.get_value("Item", {"has_batch_no": 1, "create_new_batch": 1}, "name")
		...
		self.assertTrue(dontmanage.db.get_value("Batch", {"item": item_code, "reference_name": pi.name}))

	def test_update_stock_and_purchase_return(self):
		actual_qty_0 = get_qty_after_transaction()

		pi = make_purchase_invoice(
			update_stock=1, posting_date=dontmanage.utils.nowdate(), posting_time=dontmanage.utils.nowtime()
		)
	...
	def test_duplicate_due_date_in_terms(self):
		...
		self.assertRaises(dontmanage.ValidationError, pi.insert)

	def test_debit_note(self):
		from dontmanageerp.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from dontmanageerp.accounts.doctype.sales_invoice.test_sales_invoice import get_outstanding_amount
		...
		pi_doc = dontmanage.get_doc("Purchase Invoice", pi.name)
	...
	def test_deferred_expense_via_journal_entry(self):
		...
		gl_entries = gl_entries = dontmanage.db.sql(
	...
	def test_provisional_accounting_entry_for_over_billing(self):
		...
		dontmanage.db.set_single_value("Buying Settings", "maintain_same_rate", 0)
	...
	def test_adjust_incoming_rate(self):
		...
		dontmanage.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 1)
		...
		dontmanage.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 0)
	...
	def test_item_less_defaults(self):
		pi = dontmanage.new_doc("Purchase Invoice")
	...
	def test_make_pr_and_pi_from_po(self):
		from dontmanageerp.assets.doctype.asset.test_asset import create_asset_category

		if not dontmanage.db.exists("Asset Category", "Computers"):
		...
		pr_gl_entries = dontmanage.db.sql(
			"""select account, debit, credit
		...
		pi_gl_entries = dontmanage.db.sql(
			"""select account, debit, credit
			from `tabGL Entry` where voucher_type='Purchase Invoice' and voucher_no=%s
			order by account asc""",
			pi.name,
	...
	def test_opening_invoice_rounding_adjustment_validation(self):
		...
		pi.is_opening = "Yes"
		pi.save()
		self.assertRaises(dontmanage.ValidationError, pi.submit)
...
def toggle_provisional_accounting_setting(**args):
	args = dontmanage._dict(args)
	company = dontmanage.get_doc("Company", args.company or "_Test Company")
...
test_records = dontmanage.get_test_records("Purchase Invoice")
