...
from dontmanage.utils import cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
...
from dontmanageerp.controllers.buying_controller import BuyingController
...
class WarehouseMissingError(dontmanage.ValidationError):
	pass
...
class PurchaseInvoice(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.purchase_invoice_item.purchase_invoice_item import PurchaseInvoiceItem
		from dontmanageerp.accounts.doctype.purchase_taxes_and_charges.purchase_taxes_and_charges import (
			PurchaseTaxesandCharges,
		)
		from dontmanageerp.accounts.doctype.tax_withheld_vouchers.tax_withheld_vouchers import TaxWithheldVouchers
	...
	def onload(self):
		super().onload()
		supplier_tds = dontmanage.db.get_value("Supplier", self.supplier, "tax_withholding_category")
	...
	def validate_release_date(self):
		if self.release_date and getdate(nowdate()) >= getdate(self.release_date):
			dontmanage.throw(_("Release date must be in the future"))

	def validate_cash(self):
		if not self.cash_bank_account and flt(self.paid_amount):
			dontmanage.throw(_("Cash or Bank Account is mandatory for making payment entry"))
	...
	def set_missing_values(self, for_validate=False):
		...
		tds_category = dontmanage.db.get_value("Supplier", self.supplier, "tax_withholding_category")
	...
	def validate_credit_to_acc(self):
		...
		if account.report_type != "Balance Sheet":
			dontmanage.throw(
				...
				).format(dontmanage.bold(_("Credit To"))),
		...
		if self.supplier and account.account_type != "Payable":
			dontmanage.throw(
				...
				).format(dontmanage.bold(_("Credit To")), dontmanage.bold(self.credit_to)),
				title=_("Invalid Account"),
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(dontmanage.db.get_single_value("Buying Settings", "maintain_same_rate"))
	...
	def validate_item_code(self):
		for d in self.get("items"):
			if not d.item_code:
				dontmanage.msgprint(_("Item Code required at Row No {0}").format(d.idx), raise_exception=True)
	...
	def set_expense_account(self, for_validate=False):
		...
		for item in self.get("items"):
			...
			if (
				...
				and (
					not item.po_detail
					or not dontmanage.db.get_value("Purchase Order Item", item.po_detail, "delivered_by_supplier")
				)
			):
				if self.update_stock and item.warehouse and (not item.from_warehouse):
					if (
						...
						msg = _(
							...
							item.idx,
							dontmanage.bold(warehouse_account[item.warehouse]["account"]),
							dontmanage.bold(item.expense_account),
							dontmanage.bold(item.warehouse),
						)
					...
					if item.purchase_receipt:
						negative_expense_booked_in_pr = dontmanage.db.sql(
						...
						if negative_expense_booked_in_pr:
							if (
								...
								msg = _(
									...
									item.idx,
									dontmanage.bold(stock_not_billed_account),
									dontmanage.bold(item.purchase_receipt),
								)
								dontmanage.msgprint(msg, title=_("Expense Head Changed"))
						...
						if (
							...
							msg = _(
								...
								item.idx, dontmanage.bold(stock_not_billed_account), dontmanage.bold(item.item_code)
							)
							msg += "<br>"
							msg += _(
				...
				if not item.pr_detail and item.po_detail:
					receipt_item = dontmanage.get_cached_value(
						"Purchase Receipt Item",
						{
				...
				if item.pr_detail:
					...
					arbnb_booked_in_pr = dontmanage.db.get_value(
						"GL Entry",
						{
	...
	def force_set_against_expense_account(self):
		self.set_against_expense_account()
		dontmanage.db.set_value(self.doctype, self.name, "against_expense_account", self.against_expense_account)

	def po_required(self):
		if dontmanage.db.get_value("Buying Settings", None, "po_required") == "Yes":
			if dontmanage.get_value(
			...
			for d in self.get("items"):
				if not d.purchase_order:
					msg = _("Purchase Order Required for item {}").format(dontmanage.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase order please set {0} as {1} in {2}"
					).format(
						dontmanage.bold(_("Purchase Order Required")),
						dontmanage.bold(_("No")),
	...
	def pr_required(self):
		stock_items = self.get_stock_items()
		if dontmanage.db.get_value("Buying Settings", None, "pr_required") == "Yes":
			if dontmanage.get_value(
			...
			for d in self.get("items"):
				if not d.purchase_receipt and d.item_code in stock_items:
					msg = _("Purchase Receipt Required for item {}").format(dontmanage.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase receipt please set {0} as {1} in {2}"
					).format(
						dontmanage.bold(_("Purchase Receipt Required")),
						dontmanage.bold(_("No")),
						get_link_to_form("Buying Settings", "Buying Settings", "Buying Settings"),
					)
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.purchase_order:
				submitted = dontmanage.db.sql(
					"select name from `tabPurchase Order` where docstatus = 1 and name = %s", d.purchase_order
				)
				if not submitted:
					dontmanage.throw(_("Purchase Order {0} is not submitted").format(d.purchase_order))
			if d.purchase_receipt:
				...
				if not submitted:
					dontmanage.throw(_("Purchase Receipt {0} is not submitted").format(d.purchase_receipt))
	...
	def on_submit(self):
		...
		if dontmanage.db.get_single_value("Buying Settings", "project_update_frequency") == "Each Transaction":
			self.update_project()
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if self.update_stock:
			stock_ledger_entries = dontmanage.get_all(
				"Stock Ledger Entry",
		...
		for item in self.get("items"):
			if flt(item.base_net_amount):
				...
				if item.item_code:
					dontmanage.get_cached_value("Item", item.item_code, "asset_category")
	...
	def get_provisional_accounts(self):
		...
		provisional_gl_entries = dontmanage.get_all(
			"GL Entry",
			filters={
				"voucher_type": "Purchase Receipt",
	...
	def make_provisional_gl_entry(self, gl_entries, item):
		if item.purchase_receipt:
			...
			if pr_item.get("has_provisional_entry"):
				purchase_receipt_doc = dontmanage.get_cached_doc("Purchase Receipt", item.purchase_receipt)
	...
	def update_project(self):
		...
		for proj, value in projects.items():
			res = dontmanage.qb.from_(pj).select(pj.total_purchase_cost).where(pj.name == proj).for_update().run()
			current_purchase_cost = res and res[0][0] or 0
			# dontmanage.db.set_value("Project", proj, "total_purchase_cost", current_purchase_cost + value)
			project_doc = dontmanage.get_doc("Project", proj)
	...
	def validate_supplier_invoice(self):
		if self.bill_date:
			if getdate(self.bill_date) > getdate(self.posting_date):
				dontmanage.throw(_("Supplier Invoice Date cannot be greater than Posting Date"))

		if self.bill_no:
			if cint(dontmanage.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				fiscal_year = get_fiscal_year(self.posting_date, company=self.company, as_dict=True)

				pi = dontmanage.db.sql(
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		adjust_incoming_rate = dontmanage.db.get_single_value(
			"Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate"
		)

		for pr in set(updated_pr):
			from dontmanageerp.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage

			pr_doc = dontmanage.get_doc("Purchase Receipt", pr)
	...
	def get_pr_details_billed_amt(self):
		...
		if pr_details:
			doctype = dontmanage.qb.DocType("Purchase Invoice Item")
			query = (
			...
			pr_details_billed_amt = dontmanage._dict(query.run(as_list=1))
	...
	def set_tax_withholding(self):
		...
		if self.apply_tds and not self.get("tax_withholding_category"):
			self.tax_withholding_category = dontmanage.db.get_value(
	...
	def update_advance_tax_references(self, cancel=0):
		for tax in self.get("advance_tax"):
			at = dontmanage.qb.DocType("Advance Taxes and Charges").as_("at")

			if cancel:
				dontmanage.qb.update(at).set(
...
def get_purchase_document_details(doc):
	...
	exchange_rate_map = dontmanage._dict(
		dontmanage.get_all(
			parent_doctype,
	...
	net_rate_map = dontmanage._dict(
		dontmanage.get_all(child_doctype, filters={"name": ("in", items)}, fields=["name", "net_rate"], as_list=1)
	)
...
def change_release_date(name, release_date=None):
	if dontmanage.db.exists("Purchase Invoice", name):
		pi = dontmanage.get_doc("Purchase Invoice", name)
