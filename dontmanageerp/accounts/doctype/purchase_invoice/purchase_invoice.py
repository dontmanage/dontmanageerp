	...
	def validate_credit_to_acc(self):
		...
		if account.report_type != "Balance Sheet":
			dontmanage.throw(
				...
				).format(dontmanage.bold("Credit To")),
		...
		if self.supplier and account.account_type != "Payable":
			dontmanage.throw(
				...
				).format(dontmanage.bold("Credit To"), dontmanage.bold(self.credit_to)),
				title=_("Invalid Account"),
			)
						...
						if negative_expense_booked_in_pr:
							if (
								...
								msg = _(
									...
									item.idx, dontmanage.bold(stock_not_billed_account), dontmanage.bold(item.purchase_receipt)
								)
								dontmanage.msgprint(msg, title=_("Expense Head Changed"))
				...
				if self.update_stock and self.auto_accounting_for_stock and item.item_code in stock_items:
					...
					if flt(item.rm_supp_cost):
						...
						if not supplier_warehouse_account:
							dontmanage.throw(_("Please set account in Warehouse {0}").format(self.supplier_warehouse))
						gl_entries.append(
							self.get_gl_dict(
								{
					...
					if provisional_accounting_for_non_stock_items:
						if item.purchase_receipt:
							provisional_account = dontmanage.db.get_value(
							...
							if not purchase_receipt_doc:
								purchase_receipt_doc = dontmanage.get_doc("Purchase Receipt", item.purchase_receipt)
								purchase_receipt_doc_map[item.purchase_receipt] = purchase_receipt_doc

							# Post reverse entry for Stock-Received-But-Not-Billed if it is booked in Purchase Receipt
							expense_booked_in_pr = dontmanage.db.get_value(
								"GL Entry",
								{
									"is_cancelled": 0,
