...
dontmanageerp.accounts.payment_triggers.setup("Purchase Invoice");
dontmanageerp.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
dontmanageerp.accounts.taxes.setup_tax_validations("Purchase Invoice");
dontmanageerp.buying.setup_buying_controller();

dontmanageerp.accounts.PurchaseInvoice = class PurchaseInvoice extends dontmanageerp.buying.BuyingController {
	setup(doc) {
	...
	refresh(doc) {
		...
		if(doc.docstatus===0) {
			this.frm.add_custom_button(__('Purchase Order'), function() {
				dontmanageerp.utils.map_current_doc({
					method: "dontmanageerp.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			...
			this.frm.add_custom_button(__('Purchase Receipt'), function() {
				dontmanageerp.utils.map_current_doc({
					method: "dontmanageerp.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_invoice",
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			dontmanage.model.with_doc("Supplier", me.frm.doc.supplier, function() {
				var supplier = dontmanage.model.get_doc("Supplier", me.frm.doc.supplier);
	...
	unblock_invoice() {
		const me = this;
		dontmanage.call({
			'method': 'dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice',
	...
	can_change_release_date(date) {
		const diff = dontmanage.datetime.get_diff(date, dontmanage.datetime.nowdate());
		if (diff < 0) {
			dontmanage.throw(__('New release date should be in the future'));
			return false;
		} else {
			return true;
		}
	...
	make_comment_dialog_and_block_invoice(){
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			dontmanage.call({
				'method': 'dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice',
				'args': {
	...
	set_release_date(data) {
		return dontmanage.call({
			'method': 'dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date',
			'args': data,
	...
	supplier() {
		...
		dontmanageerp.utils.get_party_details(this.frm, "dontmanageerp.accounts.party.get_party_details",
			{
	...
	make_inter_company_invoice(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm
		});
	}
	...
	on_submit() {
		...
		$.each(this.frm.doc["items"] || [], function(i, row) {
			if(row.purchase_receipt) dontmanage.model.clear_doc("Purchase Receipt", row.purchase_receipt)
		})
	}

	make_debit_note() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	}
};

cur_frm.script_manager.make(dontmanageerp.accounts.PurchaseInvoice);
...
dontmanage.ui.form.on("Purchase Invoice", {
	setup: function(frm) {
	...
	create_landed_cost_voucher: function (frm) {
		let lcv = dontmanage.model.get_new_doc('Landed Cost Voucher');
		lcv.company = frm.doc.company;

		let lcv_receipt = dontmanage.model.get_new_doc('Landed Cost Purchase Invoice');
		...
		dontmanage.set_route("Form", lcv.doctype, lcv.name);
	},
	...
	add_custom_buttons: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.per_received > 0) {
			frm.add_custom_button(__('Purchase Receipt'), () => {
				...
				dontmanage.set_route("List", "Purchase Receipt", "List")
			}, __('View'));
		}
	},
	...
	is_subcontracted: function(frm) {
		if (frm.doc.is_old_subcontracting_flow) {
			dontmanageerp.buying.get_default_bom(frm);
		}
	...
	make_purchase_receipt: function(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice.make_purchase_receipt",
			frm: frm,
