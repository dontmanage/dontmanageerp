...
from dontmanageerp.accounts.doctype.shipping_rule.shipping_rule import (
	FromGreaterThanToError,
	ManyBlankToValuesError,
	OverlappingConditionError,
)

test_records = dontmanage.get_test_records("Shipping Rule")


class TestShippingRule(unittest.TestCase):
	def test_from_greater_than_to(self):
		shipping_rule = dontmanage.copy_doc(test_records[0])
...
def create_shipping_rule(shipping_rule_type, shipping_rule_name):
	...
	if dontmanage.db.exists("Shipping Rule", shipping_rule_name):
		return dontmanage.get_doc("Shipping Rule", shipping_rule_name)

	sr = dontmanage.new_doc("Shipping Rule")
