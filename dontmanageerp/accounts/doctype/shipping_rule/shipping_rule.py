...
class OverlappingConditionError(dontmanage.ValidationError):
	pass


class FromGreaterThanToError(dontmanage.ValidationError):
	pass


class ManyBlankToValuesError(dontmanage.ValidationError):
	pass
...
class ShippingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.shipping_rule_condition.shipping_rule_condition import (
			ShippingRuleCondition,
		)
		from dontmanageerp.accounts.doctype.shipping_rule_country.shipping_rule_country import (
			ShippingRuleCountry,
		)
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if not doc.meta.get_field("taxes").options == "Sales Taxes and Charges":
				dontmanage.throw(_("Shipping rule only applicable for Selling"))
			...
			if not doc.meta.get_field("taxes").options == "Purchase Taxes and Charges":
				dontmanage.throw(_("Shipping rule only applicable for Buying"))
