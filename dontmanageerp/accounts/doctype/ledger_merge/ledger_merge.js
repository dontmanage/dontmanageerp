...
dontmanage.ui.form.on('Ledger Merge', {
	setup: function(frm) {
		dontmanage.realtime.on('ledger_merge_refresh', ({ ledger_merge }) => {
			if (ledger_merge !== frm.doc.name) return;
			dontmanage.model.clear_doc(frm.doc.doctype, frm.doc.name);
			dontmanage.model.with_doc(frm.doc.doctype, frm.doc.name).then(() => {
				frm.refresh();
			});
		});

		dontmanage.realtime.on('ledger_merge_progress', data => {
		...
		frm.set_query("account", function(doc) {
			if (!doc.company) dontmanage.throw(__('Please set Company'));
			if (!doc.root_type) dontmanage.throw(__('Please set Root Type'));
			return {
				filters: {
		...
		frm.set_query('account', 'merge_accounts', function(doc) {
			if (!doc.company) dontmanage.throw(__('Please set Company'));
			if (!doc.root_type) dontmanage.throw(__('Please set Root Type'));
			if (!doc.account) dontmanage.throw(__('Please set Account'));
			let acc = [doc.account];
...
dontmanage.ui.form.on('Ledger Merge Accounts', {
