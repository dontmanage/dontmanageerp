...
class ModeofPayment(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.mode_of_payment_account.mode_of_payment_account import (
	...
	def validate_accounts(self):
		for entry in self.accounts:
			...
			if dontmanage.get_cached_value("Account", entry.default_account, "company") != entry.company:
				dontmanage.throw(
	...
	def validate_pos_mode_of_payment(self):
		if not self.enabled:
			pos_profiles = dontmanage.db.sql(
			...
			if pos_profiles:
				...
				).format(dontmanage.bold(", ".join(pos_profiles)), dontmanage.bold(str(self.name)))
				dontmanage.throw(message, title=_("Not Allowed"))
