...
from dontmanage.utils import comma_and, cstr, flt, fmt_money, formatdate, get_link_to_form, nowdate
...
class StockAccountInvalidTransaction(dontmanage.ValidationError):
	pass
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate(self):
		...
		if not dontmanage.flags.is_reverse_depr_entry:
	...
	def validate_advance_accounts(self):
		...
		advance_accounts.add(
			dontmanage.get_cached_value("Company", self.company, "default_advance_received_account")
		)
		advance_accounts.add(dontmanage.get_cached_value("Company", self.company, "default_advance_paid_account"))
		if advance_accounts_used := journal_accounts & advance_accounts:
			dontmanage.msgprint(
				...
				).format(dontmanage.bold(comma_and(advance_accounts_used)))
			)
	...
	def update_advance_paid(self):
		advance_paid = dontmanage._dict()
		for d in self.get("accounts"):
			if d.is_advance:
				if d.reference_type in dontmanage.get_hooks("advance_payment_doctypes"):
					advance_paid.setdefault(d.reference_type, []).append(d.reference_name)

		for voucher_type, order_list in advance_paid.items():
			for voucher_no in list(set(order_list)):
				dontmanage.get_doc(voucher_type, voucher_no).set_total_advance_paid()

	def validate_inter_company_accounts(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			doc = dontmanage.get_doc("Journal Entry", self.inter_company_journal_entry_reference)
			account_currency = dontmanage.get_cached_value("Company", self.company, "default_currency")
			previous_account_currency = dontmanage.get_cached_value("Company", doc.company, "default_currency")
	...
	def apply_tax_withholding(self):
		from dontmanageerp.accounts.report.general_ledger.general_ledger import get_account_type_map
		...
		if len(parties) > 1:
			dontmanage.throw(_("Cannot apply TDS against multiple parties in one entry"))
	...
	def update_invoice_discounting(self):
		...
		for inv_disc in invoice_discounting_list:
			inv_disc_doc = dontmanage.get_doc("Invoice Discounting", inv_disc)
			status = None
	...
	def update_booked_depreciation(self, cancel=0):
		for d in self.get("accounts"):
			if (
				...
				and d.reference_name
				and dontmanage.get_cached_value("Account", d.account, "root_type") == "Expense"
				and d.debit
			):
				asset = dontmanage.get_doc("Asset", d.reference_name)
				for fb_row in asset.get("finance_books"):
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in ("Sales Invoice", "Purchase Invoice"):
				doc = dontmanage.get_doc(d.reference_type, d.reference_name)
	...
	def unlink_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			...
			dontmanage.db.set_value("Journal Entry", self.name, "inter_company_journal_entry_reference", "")
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				...
				if d.is_advance == "Yes":
					if d.party_type == "Customer" and flt(d.debit) > 0:
						dontmanage.throw(_("Row {0}: Advance against Customer must be credit").format(d.idx))
					elif d.party_type == "Supplier" and flt(d.credit) > 0:
						dontmanage.throw(_("Row {0}: Advance against Supplier must be debit").format(d.idx))
		...
		for d in self.get("accounts"):
			if d.reference_type == "Journal Entry":
				...
				if d.reference_name == self.name:
					dontmanage.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))

				against_entries = dontmanage.db.sql(
	...
	def validate_reference_doc(self):
		...
		for d in self.get("accounts"):
			...
			if d.reference_type and d.reference_name and (d.reference_type in list(field_dict)):
				...
				against_voucher = dontmanage.db.get_value(
					d.reference_type, d.reference_name, [scrub(dt) for dt in field_dict.get(d.reference_type)]
				)

				if not against_voucher:
					dontmanage.throw(_("Row {0}: Invalid reference {1}").format(d.idx, d.reference_name))
	...
	def validate_orders(self):
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				order = dontmanage.get_doc(reference_type, reference_name)

				if order.docstatus != 1:
					dontmanage.throw(_("{0} {1} is not submitted").format(reference_type, reference_name))

				if flt(order.per_billed) >= 100:
					dontmanage.throw(_("{0} {1} is fully billed").format(reference_type, reference_name))

				if cstr(order.status) == "Closed":
					dontmanage.throw(_("{0} {1} is closed").format(reference_type, reference_name))
	...
	def validate_invoices(self):
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Invoice", "Purchase Invoice") and self.voucher_type not in [
				...
				invoice = dontmanage.db.get_value(
	...
	def set_against_account(self):
		...
		if self.voucher_type in ("Deferred Revenue", "Deferred Expense"):
			for d in self.get("accounts"):
				...
				d.against_account = dontmanage.db.get_value(d.reference_type, d.reference_name, field)
		else:
			for d in self.get("accounts"):
				if flt(d.debit) > 0:
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			if d.debit and d.credit:
				dontmanage.throw(_("You cannot credit and debit same account at the same time"))
	...
	def validate_multi_currency(self):
		...
		if alternate_currency:
			if not self.multi_currency:
				dontmanage.throw(_("Please check Multi Currency option to allow accounts with other currency"))
	...
	def create_remarks(self):
		...
		if self.cheque_no:
			if self.cheque_date:
				...
				msgprint(_("Please enter Reference date"), raise_exception=dontmanage.MandatoryError)
	...
	def set_print_format_fields(self):
		...
		if party_type and pay_to_recd_from:
			self.pay_to_recd_from = dontmanage.db.get_value(
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": dontmanageerp.get_default_cost_center(self.company),
						},
					)
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			if dontmanage.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				dontmanage.throw(_("Stock Entry {0} is not submitted").format(self.stock_entry))
	...
	def validate_empty_accounts_table(self):
		if not self.get("accounts"):
			dontmanage.throw(_("Accounts table cannot be blank."))
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	...
	if not account:
		...
		if account_type == "Bank":
			account = dontmanage.get_cached_value("Company", company, "default_bank_account")
			if not account:
				account_list = dontmanage.get_all(
			...
			account = dontmanage.get_cached_value("Company", company, "default_cash_account")
			if not account:
				account_list = dontmanage.get_all(
...
def get_payment_entry_against_order(
	...
	ref_doc = dontmanage.get_doc(dt, dn)
...
def get_payment_entry(ref_doc, args):
	cost_center = ref_doc.get("cost_center") or dontmanage.get_cached_value(
...
def get_against_jv(doctype, txt, searchfield, start, page_len, filters):
	if not dontmanage.db.has_column("Journal Entry", searchfield):
		return []
...
def get_outstanding(args):
	if not dontmanage.has_permission("Account"):
		dontmanage.msgprint(_("No Permission"), raise_exception=1)

	if isinstance(args, str):
		args = json.loads(args)

	company_currency = dontmanageerp.get_company_currency(args.get("company"))
	due_date = None

	if args.get("doctype") == "Journal Entry":
		condition = " and party=%(party)s" if args.get("party") else ""

		against_jv_amount = dontmanage.db.sql(
			f"""
...
def get_exchange_rate(
	...
	if not account_details:
		dontmanage.throw(_("Please select correct account"))
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = dontmanage.db.get_value(reference_type, reference_name, "conversion_rate")
