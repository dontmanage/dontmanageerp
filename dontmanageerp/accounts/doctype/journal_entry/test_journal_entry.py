...
from dontmanageerp.accounts.doctype.journal_entry.journal_entry import StockAccountInvalidTransaction
from dontmanageerp.exceptions import InvalidAccountCurrency


class TestJournalEntry(unittest.TestCase):
	def test_journal_entry_with_against_jv(self):
		jv_invoice = dontmanage.copy_doc(test_records[2])
		base_jv = dontmanage.copy_doc(test_records[0])
	...
	def test_jv_against_purchase_order(self):
		...
		base_jv = dontmanage.copy_doc(test_records[1])
	...
	def jv_against_voucher_testcase(self, base_jv, test_voucher):
		...
		submitted_voucher = dontmanage.get_doc(test_voucher.doctype, test_voucher.name)
	...
	def advance_paid_testcase(self, base_jv, test_voucher, dr_or_cr):
		...
		advance_paid = dontmanage.db.sql(
	...
	def cancel_against_voucher_testcase(self, test_voucher):
		if test_voucher.doctype == "Journal Entry":
			...
			submitted_voucher = dontmanage.get_doc(test_voucher.doctype, test_voucher.name)
			self.assertRaises(dontmanage.LinkExistsError, submitted_voucher.cancel)
	...
	def test_inter_company_jv(self):
		dontmanage.db.set_value("Account", "Sales Expenses - _TC", "inter_company_account", 1)
		dontmanage.db.set_value("Account", "Buildings - _TC", "inter_company_account", 1)
		dontmanage.db.set_value("Account", "Sales Expenses - _TC1", "inter_company_account", 1)
		dontmanage.db.set_value("Account", "Buildings - _TC1", "inter_company_account", 1)
		jv = make_journal_entry(
			"Sales Expenses - _TC",
			"Buildings - _TC",
			100,
	...
	def test_jv_with_project(self):
		...
		if not dontmanage.db.exists("Project", {"project_name": "Journal Entry Project"}):
			...
			project_name = dontmanage.get_value("Project", {"project_name": "_Test Project"})
...
test_records = dontmanage.get_test_records("Journal Entry")
