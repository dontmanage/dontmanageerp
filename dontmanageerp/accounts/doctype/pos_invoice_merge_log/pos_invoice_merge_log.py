...
class POSInvoiceMergeLog(Document):
	...
	def validate_duplicate_pos_invoices(self):
		...
		for key, value in pos_occurences.items():
			if len(value) > 1:
				error_list.append(
					_("{0} is added multiple times on rows: {1}").format(dontmanage.bold(key), dontmanage.bold(value))
				)
	...
	def on_submit(self):
		pos_invoice_docs = [dontmanage.get_cached_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices]
	...
	def serial_and_batch_bundle_reference_for_pos_invoice(self):
		for d in self.pos_invoices:
			pos_invoice = dontmanage.get_doc("POS Invoice", d.pos_invoice)
