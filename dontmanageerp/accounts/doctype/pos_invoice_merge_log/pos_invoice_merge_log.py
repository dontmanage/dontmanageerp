...
from dontmanage.model.mapper import map_child_doc, map_doc
from dontmanage.utils import cint, flt, get_time, getdate, nowdate, nowtime
...
from dontmanageerp.accounts.doctype.pos_profile.pos_profile import required_accounting_dimensions
...
class POSInvoiceMergeLog(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.pos_invoice_reference.pos_invoice_reference import (
			POSInvoiceReference,
		)
	...
	def validate_duplicate_pos_invoices(self):
		...
		for key, value in pos_occurences.items():
			if len(value) > 1:
				error_list.append(
					_("{} is added multiple times on rows: {}".format(dontmanage.bold(key), dontmanage.bold(value)))
				)

		if error_list:
			dontmanage.throw(error_list, title=_("Duplicate POS Invoices found"), as_list=True)
	...
	def validate_pos_invoice_status(self):
		for d in self.pos_invoices:
			status, docstatus, is_return, return_against = dontmanage.db.get_value(
				"POS Invoice", d.pos_invoice, ["status", "docstatus", "is_return", "return_against"]
			)

			bold_pos_invoice = dontmanage.bold(d.pos_invoice)
			bold_status = dontmanage.bold(status)
			if docstatus != 1:
				dontmanage.throw(_("Row #{}: POS Invoice {} is not submitted yet").format(d.idx, bold_pos_invoice))
			...
			if (
				...
				bold_return_against = dontmanage.bold(return_against)
				return_against_status = dontmanage.db.get_value("POS Invoice", return_against, "status")
				if return_against_status != "Consolidated":
					# if return entry is not getting merged in the current pos closing and if it is not consolidated
					bold_unconsolidated = dontmanage.bold("not Consolidated")
	...
	def on_submit(self):
		pos_invoice_docs = [
			dontmanage.get_cached_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices
		]
	...
	def merge_pos_invoice_into(self, invoice, data):
		...
		invoice.disable_rounded_total = cint(
			dontmanage.db.get_value("POS Profile", invoice.pos_profile, "disable_rounded_total")
		)
			...
			if not dimension_value:
				dontmanage.throw(
					_("Please set Accounting Dimension {} in {}").format(
						...
						dontmanage.get_desk_link("POS Profile", invoice.pos_profile),
					)
				)
	...
	def get_new_sales_invoice(self):
		sales_invoice = dontmanage.new_doc("Sales Invoice")
	...
	def cancel_linked_invoices(self):
		for si_name in [self.consolidated_invoice, self.consolidated_credit_note]:
			if not si_name:
				continue
			si = dontmanage.get_doc("Sales Invoice", si_name)
...
def get_all_unconsolidated_invoices():
	...
	pos_invoices = dontmanage.db.get_all(
		"POS Invoice",
		filters=filters,
		fields=[
			"name as pos_invoice",
...
def consolidate_pos_invoices(pos_invoices=None, closing_entry=None):
	...
	if dontmanage.flags.in_test and not invoices:
...
def split_invoices(invoices):
	...
	special_invoices = []
	pos_return_docs = [
		dontmanage.get_cached_doc("POS Invoice", d.pos_invoice)
		for d in invoices
	...
	for pos_invoice in pos_return_docs:
		for item in pos_invoice.items:
			...
			return_against_is_consolidated = (
				dontmanage.db.get_value("POS Invoice", pos_invoice.return_against, "status", cache=True)
				== "Consolidated"
			)
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	try:
		...
		message_log = dontmanage.message_log.pop() if dontmanage.message_log else str(e)
		error_message = get_error_message(message_log)

		if closing_entry:
			closing_entry.set_status(update=True, status="Failed")
			if type(error_message) == list:
				error_message = dontmanage.json.dumps(error_message)
			closing_entry.db_set("error_message", error_message)
		raise

	finally:
		dontmanage.db.commit()
		dontmanage.publish_realtime("closing_process_complete", user=dontmanage.session.user)


def cancel_merge_logs(merge_logs, closing_entry=None):
	try:
		for log in merge_logs:
			merge_log = dontmanage.get_doc("POS Invoice Merge Log", log)
...
def enqueue_job(job, **kwargs):
	...
	if not is_job_enqueued(job_id):
		...
		dontmanage.msgprint(msg, alert=1)


def check_scheduler_status():
	if is_scheduler_inactive() and not dontmanage.flags.in_test:
		dontmanage.throw(_("Scheduler is inactive. Cannot enqueue job."), title=_("Scheduler Inactive"))
