...
from dontmanage.utils.data import guess_date_format
...
class BisectAccountingStatements(Document):
		...
		if getdate(self.from_date) > getdate(self.to_date):
			dontmanage.throw(
				_("From Date: {0} cannot be greater than To date: {1}").format(
					dontmanage.bold(self.from_date), dontmanage.bold(self.to_date)
				)
			)

	def bfs(self, from_date: datetime, to_date: datetime):
		# Make Root node
		node = dontmanage.new_doc("Bisect Nodes")
		node.root = None
		...
		while period_queue:
			...
			if delta.days == 0:
				...
				left_node = dontmanage.new_doc("Bisect Nodes")
				...
				right_node = dontmanage.new_doc("Bisect Nodes")
	...
	def build_tree(self):
		dontmanage.db.delete("Bisect Nodes")
		...
		root = dontmanage.db.get_all("Bisect Nodes", filters={"root": ["is", "not set"]})[0]
	...
	def get_report_summary(self):
		...
		pl_summary = dontmanage.get_doc("Report", "Profit and Loss Statement")
		self.p_l_summary = pl_summary.execute_script_report(filters=filters)[5]
		bs_summary = dontmanage.get_doc("Report", "Balance Sheet")
	...
	def update_node(self):
		current_node = dontmanage.get_doc("Bisect Nodes", self.current_node)
	...
	def current_node_has_summary_info(self):
		"Assertion method"
		return dontmanage.db.get_value("Bisect Nodes", self.current_node, "generated")
	...
	def bisect_left(self):
		if self.current_node is not None:
			cur_node = dontmanage.get_doc("Bisect Nodes", self.current_node)
			if cur_node.left_child is not None:
				lft_node = dontmanage.get_doc("Bisect Nodes", cur_node.left_child)
				...
				dontmanage.msgprint(_("No more children on Left"))
	...
	def bisect_right(self):
		if self.current_node is not None:
			...
			if cur_node.right_child is not None:
				rgt_node = dontmanage.get_doc("Bisect Nodes", cur_node.right_child)
				...
				dontmanage.msgprint(_("No more children on Right"))
	...
	def move_up(self):
		if self.current_node is not None:
			...
			if cur_node.root is not None:
				root = dontmanage.get_doc("Bisect Nodes", cur_node.root)
				...
				dontmanage.msgprint(_("Reached Root"))
