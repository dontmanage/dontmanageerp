...
from dontmanage.utils import add_days, flt, formatdate, getdate

from dontmanageerp.accounts.doctype.account_closing_balance.account_closing_balance import (
...
class PeriodClosingVoucher(AccountsController):
	...
	def validate_start_and_end_date(self):
		self.fy_start_date, self.fy_end_date = dontmanage.db.get_value(
		...
		if getdate(self.period_start_date) != getdate(valid_start_date):
			dontmanage.throw(_("Period Start Date must be {0}").format(formatdate(valid_start_date)))

		if getdate(self.period_start_date) > getdate(self.period_end_date):
			dontmanage.throw(_("Period Start Date cannot be greater than Period End Date"))

		if getdate(self.period_end_date) > getdate(self.fy_end_date):
			dontmanage.throw(_("Period End Date cannot be greater than Fiscal Year End Date"))
	...
	def check_if_previous_year_closed(self):
		...
		gle_exists_in_previous_year = dontmanage.db.exists(
			"GL Entry",
			{
		...
		previous_fiscal_year_closed = dontmanage.db.exists(
		...
		if not previous_fiscal_year_closed:
			dontmanage.throw(_("Previous Year is not closed, please close it first"))
	...
	def check_closing_account_type(self):
		closing_account_type = dontmanage.get_cached_value("Account", self.closing_account_head, "root_type")
	...
	def check_closing_account_currency(self):
		...
		if account_currency != company_currency:
			dontmanage.throw(_("Currency of the Closing Account must be {0}").format(company_currency))
		...
		gl_entry = dontmanage._dict(
			{
				...
				"account_currency": dontmanage.db.get_value(
	...
	def set_account_balance_dict(self, gle, acc_bal_dict):
		...
		acc_bal_dict.setdefault(key, dontmanage._dict()).setdefault(
			gle.account,
			dontmanage._dict(
				{
	...
	def get_closing_entry(self, account, balances, dimensions):
		closing_entry = dontmanage._dict(
			{
				"company": self.company,
	...
	def is_first_period_closing_voucher(self):
		first_pcv = dontmanage.db.get_value(
			"Period Closing Voucher",
...
def process_gl_and_closing_entries(doc):
	...
	try:
		...
		dontmanage.db.set_value(doc.doctype, doc.name, "gle_processing_status", "Completed")
	except Exception as e:
		dontmanage.db.rollback()
		dontmanage.log_error(e)
		dontmanage.db.set_value(doc.doctype, doc.name, "gle_processing_status", "Failed")
...
def process_cancellation(voucher_type, voucher_no):
	...
	try:
		...
		dontmanage.db.set_value("Period Closing Voucher", voucher_no, "gle_processing_status", "Completed")
	except Exception as e:
		dontmanage.db.rollback()
		dontmanage.log_error(e)
		dontmanage.db.set_value("Period Closing Voucher", voucher_no, "gle_processing_status", "Failed")


def delete_closing_entries(voucher_no):
	closing_balance = dontmanage.qb.DocType("Account Closing Balance")
	dontmanage.qb.from_(closing_balance).delete().where(
...
def get_period_start_end_date(fiscal_year, company):
	fy_start_date, fy_end_date = dontmanage.db.get_value(
...
def get_previous_closed_period_in_current_year(fiscal_year, company):
	prev_closed_period_end_date = dontmanage.db.get_value(
		"Period Closing Voucher",
		filters={
			"company": company,
