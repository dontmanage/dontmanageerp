...
from dontmanage.utils import add_days, flt

from dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
)
from dontmanageerp.accounts.utils import get_account_currency, get_fiscal_year, validate_fiscal_year
...
class PeriodClosingVoucher(AccountsController):
	...
	def validate_posting_date(self):
		...
		pcv = dontmanage.qb.DocType("Period Closing Voucher")
		existing_entry = (
			dontmanage.qb.from_(pcv)
			.select(pcv.name)
			.where(
	...
	def check_if_previous_year_closed(self):
		...
		if previous_fiscal_year and not dontmanage.db.exists(
	...
	def get_balances_based_on_dimensions(
		...
		accounts = dontmanage.get_all("Account", filters=account_filters, pluck="name")

		gl_entry = dontmanage.qb.DocType("GL Entry")
		query = dontmanage.qb.from_(gl_entry).select(gl_entry.account, gl_entry.account_currency)
...
def process_gl_entries(gl_entries, voucher_name):
	...
	try:
		...
		dontmanage.db.set_value("Period Closing Voucher", voucher_name, "gle_processing_status", "Completed")
	except Exception as e:
		dontmanage.db.rollback()
		dontmanage.log_error(e)
		dontmanage.db.set_value("Period Closing Voucher", voucher_name, "gle_processing_status", "Failed")
