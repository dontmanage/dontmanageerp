...
dontmanage.ui.form.on('Invoice Discounting', {
	setup: (frm) => {
	...
	set_end_date: (frm) => {
		if (frm.doc.loan_start_date && frm.doc.loan_period) {
			let end_date = dontmanage.datetime.add_days(frm.doc.loan_start_date, frm.doc.loan_period);
	...
	get_invoices: (frm) => {
		var d = new dontmanage.ui.Dialog({
			...
			primary_action: function() {
				...
				dontmanage.call({
					method: "dontmanageerp.accounts.doctype.invoice_discounting.invoice_discounting.get_invoices",
					args: {
						filters: data
					},
					callback: function(r) {
...
dontmanage.ui.form.on('Discounted Invoice', {
	sales_invoice: (frm) => {
		frm.events.calculate_total_amount(frm);
		frm.events.refresh_filters(frm);
	},
