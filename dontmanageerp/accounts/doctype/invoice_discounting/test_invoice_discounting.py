...
from dontmanageerp.accounts.doctype.journal_entry.journal_entry import get_payment_entry_against_invoice
...
class TestInvoiceDiscounting(unittest.TestCase):
	def setUp(self):
		...
		dontmanage.db.set_value("Company", "_Test Company", "default_bank_account", self.bank_account)
	...
	def test_on_close_after_loan_period_after_inv_payment(self):
		...
		je_on_payment = dontmanage.get_doc(get_payment_entry_against_invoice("Sales Invoice", inv.name))
...
def create_invoice_discounting(invoices, **args):
	...
	inv_disc = dontmanage.new_doc("Invoice Discounting")
