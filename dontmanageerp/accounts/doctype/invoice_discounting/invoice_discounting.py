...
class InvoiceDiscounting(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.discounted_invoice.discounted_invoice import DiscountedInvoice
	...
	def validate_mandatory(self):
		if self.docstatus == 1 and not (self.loan_start_date and self.loan_period):
			dontmanage.throw(_("Loan Start Date and Loan Period are mandatory to save the Invoice Discounting"))

	def validate_invoices(self):
		discounted_invoices = [
			record.sales_invoice
			for record in dontmanage.get_all(
			...
			if record.sales_invoice in discounted_invoices:
				dontmanage.throw(
					_("Row({0}): {1} is already discounted in {2}").format(
						record.idx, dontmanage.bold(record.sales_invoice), dontmanage.bold(record.parent)
					)
				)

			actual_outstanding = dontmanage.db.get_value(
			...
			if record.outstanding_amount > actual_outstanding:
				dontmanage.throw(
					_(
						...
						record.idx, dontmanage.bold(actual_outstanding), dontmanage.bold(record.sales_invoice)
					)
				)
	...
	def set_status(self, status=None, cancel=0):
		if status:
			...
			for d in self.invoices:
				dontmanage.get_doc("Sales Invoice", d.sales_invoice).set_status(update=True, update_modified=False)
		else:
			self.status = "Draft"
			if self.docstatus == 1:
	...
	def update_sales_invoice(self):
		for d in self.invoices:
			if self.docstatus == 1:
				...
				discounted_invoice = dontmanage.db.exists(
					{"doctype": "Discounted Invoice", "sales_invoice": d.sales_invoice, "docstatus": 1}
				)
				is_discounted = 1 if discounted_invoice else 0
			dontmanage.db.set_value("Sales Invoice", d.sales_invoice, "is_discounted", is_discounted)
	...
	def make_gl_entries(self):
		...
		for d in self.invoices:
			inv = dontmanage.db.get_value("Sales Invoice", d.sales_invoice, invoice_fields, as_dict=1)

			if d.outstanding_amount:
				outstanding_in_company_currency = flt(
					d.outstanding_amount * inv.conversion_rate, d.precision("outstanding_amount")
				)
				ar_credit_account_currency = dontmanage.get_cached_value(
	...
	def close_loan(self):
		...
		if getdate(self.loan_end_date) > getdate(nowdate()):
			for d in self.invoices:
				outstanding_amount = dontmanage.db.get_value(
...
def get_invoices(filters):
	filters = dontmanage._dict(json.loads(filters))
	cond = []
	if filters.customer:
...
def get_party_account_based_on_invoice_discounting(sales_invoice):
	party_account = None
	invoice_discounting = dontmanage.db.sql(
		"""
