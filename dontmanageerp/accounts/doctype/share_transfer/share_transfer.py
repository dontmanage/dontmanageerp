...
class ShareTransfer(Document):
	...
	def validate(self):
		...
		if self.transfer_type == "Issue":
			...
			if ret_val in ("Complete", "Partial"):
				dontmanage.throw(_("The shares already exist"), dontmanage.DuplicateEntryError)
		else:
	...
	def basic_validations(self):
		if self.transfer_type == "Purchase":
			...
			if not self.from_shareholder:
				dontmanage.throw(_("The field From Shareholder cannot be blank"))
			if not self.from_folio_no:
				self.to_folio_no = self.autoname_folio(self.to_shareholder)
			if not self.asset_account:
				dontmanage.throw(_("The field Asset Account cannot be blank"))
		elif self.transfer_type == "Issue":
			self.from_shareholder = ""
			if not self.to_shareholder:
				dontmanage.throw(_("The field To Shareholder cannot be blank"))
			...
			if not self.from_shareholder or not self.to_shareholder:
				dontmanage.throw(_("The fields From Shareholder and To Shareholder cannot be blank"))
			if not self.to_folio_no:
				self.to_folio_no = self.autoname_folio(self.to_shareholder)
		if not self.equity_or_liability_account:
			dontmanage.throw(_("The field Equity/Liability Account cannot be blank"))
		if self.from_shareholder == self.to_shareholder:
			dontmanage.throw(_("The seller and the buyer cannot be the same"))
		if self.no_of_shares != self.to_no - self.from_no + 1:
			dontmanage.throw(_("The number of shares and the share numbers are inconsistent"))
		if not self.amount:
	...
	def folio_no_validation(self):
		...
		for shareholder_field in shareholder_fields:
			...
			if doc.company != self.company:
				dontmanage.throw(_("The shareholder does not belong to this company"))
			if not doc.folio_no:
				doc.folio_no = (
				...
				if doc.folio_no and doc.folio_no != (
					...
					dontmanage.throw(_("The folio numbers are not matching"))
	...
	def remove_shares(self, shareholder):
		...
		doc = dontmanage.get_doc("Shareholder", shareholder)
	...
	def get_shareholder_doc(self, shareholder):
		...
		name = dontmanage.db.get_value("Shareholder", {"name": shareholder}, "name")

		return dontmanage.get_doc("Shareholder", name)

	def get_company_shareholder(self):
		# Get company doc or create one if not present
		company_shareholder = dontmanage.db.get_value(
			"Shareholder", {"company": self.company, "is_company": 1}, "name"
		)

		if company_shareholder:
			return dontmanage.get_doc("Shareholder", company_shareholder)
		else:
			shareholder = dontmanage.get_doc(
