...
dontmanage.provide("dontmanageerp.share_transfer");

dontmanage.ui.form.on('Share Transfer', {
	refresh: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.equity_or_liability_account && frm.doc.asset_account) {
			frm.add_custom_button(__('Create Journal Entry'), function () {
				dontmanageerp.share_transfer.make_jv(frm);
			});
		}
	...
	no_of_shares: (frm) => {
		if (frm.doc.rate != undefined || frm.doc.rate != null){
			dontmanageerp.share_transfer.update_amount(frm);
		}
	},
	rate: (frm) => {
	...
	company: async function(frm) {
		if (frm.doc.company) {
			let currency = (await dontmanage.db.get_value("Company", frm.doc.company, "default_currency")).message.default_currency;
...
dontmanageerp.share_transfer.update_amount = function(frm) {
	frm.doc.amount = frm.doc.no_of_shares * frm.doc.rate;
	frm.refresh_field("amount");
};

dontmanageerp.share_transfer.make_jv = function (frm) {
	...
	dontmanage.call({
		...
		method: "dontmanageerp.accounts.doctype.share_transfer.share_transfer.make_jv_entry",
		callback: function (r) {
