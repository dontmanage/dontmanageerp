...
class TestBankTransaction(DontManageTestCase):
	def setUp(self):
		for dt in [
			"Bank Transaction",
			"Payment Entry",
		...
		uniq_identifier = dontmanage.generate_hash(length=10)
	...
	# This test checks if DontManageErp is able to provide a linked payment for a bank transaction based on the amount of the bank transaction.
	def test_linked_payments(self):
		bank_transaction = dontmanage.get_doc(
			"Bank Transaction",
	...
	def test_reconcile(self):
		...
		payment = dontmanage.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1700))
		vouchers = json.dumps(
			[
				{
		...
		unallocated_amount = dontmanage.db.get_value(
			"Bank Transaction", bank_transaction.name, "unallocated_amount"
		)
		self.assertTrue(unallocated_amount == 0)

		clearance_date = dontmanage.db.get_value("Payment Entry", payment.name, "clearance_date")
	...
	# Check if DontManageErp can correctly filter a linked payments based on the debit/credit amount
	...
	def test_already_reconciled(self):
		...
		payment = dontmanage.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1200))
		vouchers = json.dumps(
			[
				{
					"payment_doctype": "Payment Entry",
	...
	def test_clear_sales_invoice(self):
		...
		payment = dontmanage.get_doc("Sales Invoice", dict(customer="Fayva", status=["=", "Paid"]))
		vouchers = json.dumps(
			[
				{
		...
		self.assertEqual(
			dontmanage.db.get_value("Bank Transaction", bank_transaction.name, "unallocated_amount"), 0
		)
		self.assertTrue(
			dontmanage.db.get_value("Sales Invoice Payment", dict(parent=payment.name), "clearance_date")
			is not None
		)
...
def clear_loan_transactions():
	dontmanage.db.delete("Loan Repayment")
...
def add_vouchers(gl_account="_Test Bank - _TC"):
	...
	mode_of_payment = dontmanage.get_doc({"doctype": "Mode of Payment", "name": "Cash"})

	if not dontmanage.db.get_value("Mode of Payment Account", {"company": "_Test Company", "parent": "Cash"}):
