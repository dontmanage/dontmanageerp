...
from dontmanageerp.accounts.doctype.bank_transaction.test_bank_transaction import create_bank_account


class TestAutoMatchParty(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		create_bank_account()
		dontmanage.db.set_single_value("Accounts Settings", "enable_party_matching", 1)
		dontmanage.db.set_single_value("Accounts Settings", "enable_fuzzy_matching", 1)
		return super().setUpClass()

	@classmethod
	def tearDownClass(cls):
		dontmanage.db.set_single_value("Accounts Settings", "enable_party_matching", 0)
		dontmanage.db.set_single_value("Accounts Settings", "enable_fuzzy_matching", 0)
...
def create_supplier_for_match(supplier_name="John Doe & Co.", iban=None, account_no=None):
	if dontmanage.db.exists("Supplier", {"supplier_name": supplier_name}):
	...
	if not dontmanage.db.exists("Bank", "TestBank"):
		bank = dontmanage.new_doc("Bank")
		bank.bank_name = "TestBank"
		bank.insert(ignore_if_duplicate=True)

	if not dontmanage.db.exists("Bank Account", supplier.name + " - " + "TestBank"):
		bank_account = dontmanage.new_doc("Bank Account")
...
def create_bank_transaction(
	...
	doc = dontmanage.new_doc("Bank Transaction")
	doc.update(
		{
