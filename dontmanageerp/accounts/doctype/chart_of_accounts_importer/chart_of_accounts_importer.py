...
from dontmanage.desk.form.linked_with import get_linked_fields
...
def validate_columns(data):
	if not data:
		dontmanage.throw(_("No data found. Seems like you uploaded a blank file"))
...
def validate_company(company):
	parent_company, allow_account_creation_against_child_company = dontmanage.get_cached_value(
		"Company", company, ["parent_company", "allow_account_creation_against_child_company"]
	)

	if parent_company and (not allow_account_creation_against_child_company):
		msg = _("{} is a child company.").format(dontmanage.bold(company)) + " "
		msg += _("Please import accounts against parent company or enable {} in company master.").format(
			dontmanage.bold(_("Allow Account Creation Against Child Company"))
		)
		dontmanage.throw(msg, title=_("Wrong Company"))

	if dontmanage.db.get_all("GL Entry", {"company": company}, "name", limit=1):
		return False
...
def get_file(file_name):
	file_doc = dontmanage.get_doc("File", {"file_url": file_name})
...
def generate_data_from_csv(file_doc, as_dict=False):
	...
	with open(file_path, "r") as in_file:
		...
		for row in csv_reader:
			if as_dict:
				data.append({dontmanage.scrub(header): row[index] for index, header in enumerate(headers)})
			else:
...
def build_forest(data):
				...
				if not parent_account_list and parent_account:
					dontmanage.throw(
						_("The parent account {0} does not exists in the uploaded template").format(
							dontmanage.bold(parent_account)
						)
					)
	...
	if error_messages:
		dontmanage.throw("<br>".join(error_messages))
...
def build_response_as_excel(writer):
	filename = dontmanage.generate_hash("", 10)
	...
	dontmanage.response["filename"] = "coa_importer_template.xlsx"
...
def download_template(file_type, template_type, company):
	...
	if file_type == "CSV":
		# download csv file
		dontmanage.response["result"] = cstr(writer.getvalue())
		dontmanage.response["type"] = "csv"
		dontmanage.response["doctype"] = "Chart of Accounts Importer"
	else:
...
def get_sample_template(writer, company):
	currency = dontmanage.db.get_value("Company", company, "default_currency")
...
def validate_accounts(file_doc, extension):
	...
	for account in accounts:
		...
		if "parent_account" not in account:
			...
			dontmanage.throw(msg, title=_("Parent Account Missing"))
...
def validate_missing_roots(roots):
	...
	if missing:
		dontmanage.throw(_("Please add Root Account for - {0}").format(" , ".join(missing)))
...
def unset_existing_data(company):
	...
	dontmanage.db.set_value("Company", company, update_values, update_values)
...
def set_default_accounts(company):
	...
	company.update(
		{
			"default_receivable_account": dontmanage.db.get_value(
				"Account", {"company": company.name, "account_type": "Receivable", "is_group": 0}
			),
			"default_payable_account": dontmanage.db.get_value(
				"Account", {"company": company.name, "account_type": "Payable", "is_group": 0}
			),
			"default_provisional_account": dontmanage.db.get_value(
				"Account",
