...
class TestPaymentLedgerEntry(DontManageTestCase):
	def setUp(self):
	...
	def create_company(self):
		...
		# create bank account
		if dontmanage.db.exists("Account", "HDFC - _PL"):
			self.bank = "HDFC - _PL"
		else:
	...
	def test_multi_payment_unlink_on_invoice_cancellation(self):
		...
		entries = dontmanage.db.get_list(
			"Payment Ledger Entry",
		...
		si.delete()
		self.assertRaises(dontmanage.DoesNotExistError, dontmanage.get_doc, si.doctype, si.name)
	...
	def test_advance_payment_unlink_on_order_cancellation(self):
		...
		# with references removed, deletion should be possible
		so.delete()
		self.assertRaises(dontmanage.DoesNotExistError, dontmanage.get_doc, so.doctype, so.name)
