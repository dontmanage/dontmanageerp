...
from dontmanage.utils import add_days, add_years, flt, getdate, nowdate, today
...
class TestPaymentReconciliation(DontManageTestCase):
	...
	def create_company(self):
		company = None
		if dontmanage.db.exists("Company", "_Test Payment Reconciliation"):
			company = dontmanage.get_doc("Company", "_Test Payment Reconciliation")
		else:
		...
		# create bank account
		if dontmanage.db.exists("Account", "HDFC - _PR"):
			self.bank = "HDFC - _PR"
		else:
			bank_acc = dontmanage.get_doc(
				{
					"doctype": "Account",
					"account_name": "HDFC",
	...
	def create_cost_center(self):
		...
		self.main_cc = dontmanage.get_doc("Cost Center", get_default_cost_center(self.company))

		cc_exists = dontmanage.db.get_list("Cost Center", filters={"cost_center_name": cc_name})
		if cc_exists:
			self.sub_cc = dontmanage.get_doc("Cost Center", cc_exists[0].name)
		else:
			sub_cc = dontmanage.new_doc("Cost Center")
	...
	def test_no_difference_amount_for_base_currency_accounts(self):
		...
		exc_gain_loss_journals = dontmanage.db.get_all(
			"Journal Entry Account",
	...
	def test_advance_reverse_payment_against_payment_for_supplier(self):
		...
		pl_entries = dontmanage.db.get_all(
			"Payment Ledger Entry",
			filters={"voucher_no": pe.name},
	...
	def test_reconciliation_on_closed_period_payment(self):
		...
		first_fy_start_date = dontmanage.db.get_value("Fiscal Year", {"disabled": 0}, "min(year_start_date)")
		...
		je_1 = dontmanage.new_doc("Journal Entry")
...
def create_fiscal_year(company, year_start_date, year_end_date):
	...
	if not fy_docname:
		...
		fy_doc = dontmanage.get_doc("Fiscal Year", fy_docname)
		if not dontmanage.db.exists("Fiscal Year Company", {"parent": fy_docname, "company": company}):
