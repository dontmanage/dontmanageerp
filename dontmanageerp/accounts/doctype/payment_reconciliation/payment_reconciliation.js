...
dontmanageerp.accounts.PaymentReconciliationController = class PaymentReconciliationController extends dontmanage.ui.form.Controller {
	onload() {
		const default_company = dontmanage.defaults.get_default('company');
		...
		this.frm.set_query('receivable_payable_account', () => {
			return {
				filters: {
					...
					"is_group": 0,
					"account_type": dontmanage.boot.party_account_types[this.frm.doc.party_type],
		...
		if (this.frm.doc.receivable_payable_account) {
			this.frm.call({
				...
				callback: (r) => {
					if (r.message) {
						this.frm.call({
							'method': "dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.is_any_doc_running",
							"args": {
								for_filter: {
									company: this.frm.doc.company,
							...
							if (r.message) {
								let doc_link = dontmanage.utils.get_form_link("Process Payment Reconciliation", r.message, true);
	...
	set_query_for_dimension_filters() {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.payment_reconciliation.payment_reconciliation.get_queries_for_dimension_filters",
			args: {
				company: this.frm.doc.company,
			},
			callback: (r) => {
		...
		return this.frm.call({
			...
			callback: () => {
				if (!(this.frm.doc.payments.length || this.frm.doc.invoices.length)) {
					dontmanage.throw({message: __("No Unreconciled Invoices and Payments found for this party and account")});
				} else if (!(this.frm.doc.invoices.length)) {
					dontmanage.throw({message: __("No Outstanding Invoices found for this party")});
				} else if (!(this.frm.doc.payments.length)) {
					dontmanage.throw({message: __("No Unreconciled Payments found for this party")});
				}
				this.frm.refresh();
			}
		});
		...
		if (show_dialog && show_dialog.length) {
			...
			const dialog = new dontmanage.ui.Dialog({
				...
				primary_action: () => {
					...
					args.forEach(d => {
						dontmanage.model.set_value("Payment Reconciliation Allocation", d.docname,
...
dontmanage.ui.form.on('Payment Reconciliation Allocation', {
...
extend_cscript(cur_frm.cscript, new dontmanageerp.accounts.PaymentReconciliationController({frm: cur_frm}));
