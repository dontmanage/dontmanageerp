...
from dontmanage.utils import flt, getdate, nowdate, today
...
from dontmanageerp.controllers.accounts_controller import get_advance_payment_entries
...
class PaymentReconciliation(Document):
	...
	def get_dr_or_cr_notes(self):
		...
		return_invoices = (
			...
			.where(
				(doc.docstatus == 1)
				& (doc[dontmanage.scrub(self.party_type)] == self.party)
				& (doc.is_return == 1)
	...
	def make_difference_entry(self, row):
		...
		party_account_currency = dontmanage.get_cached_value(
			"Account", self.receivable_payable_account, "account_currency"
		)
		difference_account_currency = dontmanage.get_cached_value(
	...
	def get_conditions(self, get_payments=False):
		...
		condition += (
			" and posting_date >= {0}".format(dontmanage.db.escape(self.from_payment_date))
			if self.from_payment_date
			else ""
		)
		condition += (
			" and posting_date <= {0}".format(dontmanage.db.escape(self.to_payment_date))
			if self.to_payment_date
			else ""
		)
