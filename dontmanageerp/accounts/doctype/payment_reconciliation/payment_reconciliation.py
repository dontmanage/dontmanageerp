...
from dontmanage.utils import flt, fmt_money, get_link_to_form, getdate, nowdate, today

import dontmanageerp
from dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from dontmanageerp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation import (
	is_any_doc_running,
)
...
from dontmanageerp.controllers.accounts_controller import get_advance_payment_entries_for_regional
...
class PaymentReconciliation(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.accounts.doctype.payment_reconciliation_allocation.payment_reconciliation_allocation import (
			PaymentReconciliationAllocation,
		)
		from dontmanageerp.accounts.doctype.payment_reconciliation_invoice.payment_reconciliation_invoice import (
			PaymentReconciliationInvoice,
		)
		from dontmanageerp.accounts.doctype.payment_reconciliation_payment.payment_reconciliation_payment import (
	...
	def get_jv_entries(self):
		...
		account_type = dontmanageerp.get_party_account_type(self.party_type)
	...
	def get_return_invoices(self):
		...
		conditions.append(doc[dontmanage.scrub(self.party_type)] == self.party)
	...
	def get_dr_or_cr_notes(self):
		...
		if dontmanageerp.get_party_account_type(self.party_type) == "Receivable":
	...
	def get_difference_amount(self, payment_entry, invoice, allocated_amount):
		...
		) != dontmanage.get_cached_value("Company", self.company, "default_currency"):
	...
	def is_auto_process_enabled(self):
		return dontmanage.db.get_single_value("Accounts Settings", "auto_reconcile_payments")
	...
	def allocate_entries(self, args):
		...
		default_exchange_gain_loss_account = dontmanage.get_cached_value(
	...
	@dontmanage.whitelist()
	def reconcile(self):
		if dontmanage.db.get_single_value("Accounts Settings", "auto_reconcile_payments"):
	...
	def get_payment_details(self, row, dr_or_cr):
		payment_details = dontmanage._dict(
			{
	...
	def check_mandatory_to_fetch(self):
		for fieldname in ["company", "party_type", "party", "receivable_payable_account"]:
			if not self.get(fieldname):
				dontmanage.throw(_("Please select {0} first").format(self.meta.get_label(fieldname)))

	def validate_entries(self):
		if not self.get("invoices"):
			dontmanage.throw(_("No records found in the Invoices table"))

		if not self.get("payments"):
			dontmanage.throw(_("No records found in the Payments table"))
	...
	def get_invoice_exchange_map(self, invoices, payments):
		...
		invoice_exchange_map = dontmanage._dict()

		if sales_invoices:
			sales_invoice_map = dontmanage._dict(
		...
		if purchase_invoices:
			purchase_invoice_map = dontmanage._dict(
				dontmanage.db.get_all(
					"Purchase Invoice",
	...
	def validate_allocation(self):
		unreconciled_invoices = dontmanage._dict()
		...
		if not invoices_to_reconcile:
			dontmanage.throw(_("No records found in Allocation table"))
				...
				"accounts": [
					{
						...
						"cost_center": inv.cost_center or dontmanageerp.get_default_cost_center(company),
...
def get_queries_for_dimension_filters(company: str | None = None):
	...
	for d in get_dimensions()[0]:
		filters = {}
		meta = dontmanage.get_meta(d.document_type)
