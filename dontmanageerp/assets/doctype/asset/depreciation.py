...
from dontmanage.query_builder.functions import Max, Min
...
from dontmanageerp.accounts.doctype.journal_entry.journal_entry import make_reverse_journal_entry
...
def post_depreciation_entries(date=None):
	...
	if not cint(
		dontmanage.db.get_value("Accounts Settings", None, "book_asset_depreciation_entry_automatically")
	):
		return
	...
	for asset_depr_schedule_data in depreciable_asset_depr_schedules_data:
		...
		try:
			...
			error_log = dontmanage.log_error(e)
...
def get_depreciable_asset_depr_schedules_data(date):
	a = dontmanage.qb.DocType("Asset")
	ads = dontmanage.qb.DocType("Asset Depreciation Schedule")
	ds = dontmanage.qb.DocType("Depreciation Schedule")

	res = (
		dontmanage.qb.from_(ads)
		.join(a)
		.on(ads.asset == a.name)
		.join(ds)
...
def get_acc_frozen_upto():
	acc_frozen_upto = dontmanage.db.get_single_value("Accounts Settings", "acc_frozen_upto")

	if not acc_frozen_upto:
		return

	frozen_accounts_modifier = dontmanage.db.get_single_value("Accounts Settings", "frozen_accounts_modifier")

	if frozen_accounts_modifier not in dontmanage.get_roles() or dontmanage.session.user == "Administrator":
...
def get_depreciation_cost_center_and_depreciation_series_for_company():
	company_names = dontmanage.db.get_all("Company", pluck="name")
...
def make_depreciation_entry(
	...
	dontmanage.has_permission("Journal Entry", throw=True)

	if not date:
		date = today()

	asset_depr_schedule_doc = dontmanage.get_doc("Asset Depreciation Schedule", asset_depr_schedule_name)

	asset = dontmanage.get_doc("Asset", asset_depr_schedule_doc.asset)
...
def get_depreciation_accounts(asset_category, company):
	...
	accounts = dontmanage.db.get_value(
	...
	if not accumulated_depreciation_account or not depreciation_expense_account:
		accounts = dontmanage.get_cached_value(
...
def get_credit_and_debit_accounts(accumulated_depreciation_account, depreciation_expense_account):
	root_type = dontmanage.get_value("Account", depreciation_expense_account, "root_type")
	...
	if root_type == "Expense":
		...
		dontmanage.throw(_("Depreciation Expense Account should be an Income or Expense Account."))
...
def set_depr_entry_posting_status_for_failed_assets(failed_asset_names):
	for asset_name in failed_asset_names:
		dontmanage.db.set_value("Asset", asset_name, "depr_entry_posting_status", "Failed")
...
def scrap_asset(asset_name):
	...
	if asset.docstatus != 1:
		dontmanage.throw(_("Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped", "Capitalized", "Decapitalized"):
		dontmanage.throw(_("Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status))
	...
	depreciation_series = dontmanage.get_cached_value("Company", asset.company, "series_for_depreciation_entry")
	...
	dontmanage.db.set_value("Asset", asset_name, "disposal_date", date)
	dontmanage.db.set_value("Asset", asset_name, "journal_entry_for_scrap", je.name)
	asset.set_status("Scrapped")

	add_asset_activity(asset_name, _("Asset scrapped"))

	dontmanage.msgprint(_("Asset scrapped via Journal Entry {0}").format(je.name))
...
def restore_asset(asset_name):
	...
	dontmanage.get_doc("Journal Entry", je).cancel()
...
def modify_depreciation_schedule_for_asset_repairs(asset, notes):
	asset_repairs = dontmanage.get_all(
		"Asset Repair", filters={"asset": asset.name}, fields=["name", "increase_in_asset_life"]
	)

	for repair in asset_repairs:
		if repair.increase_in_asset_life:
			asset_repair = dontmanage.get_doc("Asset Repair", repair.name)
		...
		for schedule_idx, schedule in enumerate(asset_depr_schedule_doc.get("depreciation_schedule")):
			if schedule.schedule_date == date and schedule.journal_entry:
				if not disposal_was_made_on_original_schedule_date(
					...
					dontmanage.flags.is_reverse_depr_entry = True
					reverse_journal_entry.submit()

					dontmanage.flags.is_reverse_depr_entry = False
...
def get_disposal_account_and_cost_center(company):
	disposal_account, depreciation_cost_center = dontmanage.get_cached_value(
		"Company", company, ["disposal_account", "depreciation_cost_center"]
	)

	if not disposal_account:
		dontmanage.throw(_("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(company))
	if not depreciation_cost_center:
		dontmanage.throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
