...
from dontmanageerp.accounts.general_ledger import make_reverse_gl_entries
...
class Asset(AccountsController):
	...
	def validate(self):
		...
		if not self.split_from:
			...
			if self.calculate_depreciation:
				...
				if dontmanage.db.exists("Asset", self.name):
	...
	def validate_asset_and_reference(self):
		if self.purchase_invoice or self.purchase_receipt:
			...
			reference_doc = dontmanage.get_doc(reference_doc, reference_name)
		...
		if self.is_existing_asset and self.purchase_invoice:
			dontmanage.throw(_("Purchase Invoice cannot be made against an existing asset {0}").format(self.name))
	...
	def validate_item(self):
		...
		if not item:
			dontmanage.throw(_("Item {0} does not exist").format(self.item_code))
		elif item.disabled:
			dontmanage.throw(_("Item {0} has been disabled").format(self.item_code))
		elif not item.is_fixed_asset:
			dontmanage.throw(_("Item {0} must be a Fixed Asset Item").format(self.item_code))
		elif item.is_stock_item:
			dontmanage.throw(_("Item {0} must be a non-stock item").format(self.item_code))

	def validate_cost_center(self):
		if self.cost_center:
			cost_center_company, cost_center_is_group = dontmanage.db.get_value(
				"Cost Center", self.cost_center, ["company", "is_group"]
			)
			if cost_center_company != self.company:
				dontmanage.throw(
					_("Cost Center {} doesn't belong to Company {}").format(
						dontmanage.bold(self.cost_center), dontmanage.bold(self.company)
					),
			...
			if cost_center_is_group:
				dontmanage.throw(
					...
					).format(dontmanage.bold(self.cost_center)),
					title=_("Invalid Cost Center"),
				)

		else:
			if not dontmanage.get_cached_value("Company", self.company, "depreciation_cost_center"):
				dontmanage.throw(
					_(
	...
	def validate_in_use_date(self):
		if not self.available_for_use_date:
			dontmanage.throw(_("Available for use date is required"))
	...
	def set_missing_values(self):
		if not self.asset_category:
			self.asset_category = dontmanage.get_cached_value("Item", self.item_code, "asset_category")
	...
	def validate_asset_values(self):
		...
		if not flt(self.gross_purchase_amount) and not self.is_composite_asset:
			dontmanage.throw(_("Gross Purchase Amount is mandatory"), dontmanage.MandatoryError)
		...
		if is_cwip_accounting_enabled(self.asset_category):
			...
			if (
				...
				and not dontmanage.db.get_value("Purchase Invoice", self.purchase_invoice, "update_stock")
			):
				dontmanage.throw(
		...
		if not self.calculate_depreciation:
			...
			if not self.finance_books:
				dontmanage.throw(_("Enter depreciation details"))
			if self.is_fully_depreciated:
				dontmanage.throw(_("Depreciation cannot be calculated for fully depreciated assets"))

		if self.is_existing_asset:
			return

		if self.available_for_use_date and getdate(self.available_for_use_date) < getdate(self.purchase_date):
			dontmanage.throw(_("Available-for-use Date should be after purchase date"))
	...
	def validate_gross_and_purchase_amount(self):
		...
		if self.gross_purchase_amount and self.gross_purchase_amount != self.purchase_amount:
			...
			dontmanage.throw(error_message, title=_("Invalid Gross Purchase Amount"))
	...
	def make_asset_movement(self):
		...
		if reference_docname:
			posting_date, posting_time = dontmanage.db.get_value(
		...
		asset_movement = dontmanage.get_doc(
			{
		...
		if not self.is_existing_asset:
			...
			if self.opening_accumulated_depreciation:
				if not self.opening_number_of_booked_depreciations:
					dontmanage.throw(_("Please set Opening Number of Booked Depreciations"))
			else:
				self.opening_number_of_booked_depreciations = 0
	...
	def validate_cancellation(self):
		...
		if self.status not in ("Submitted", "Partially Depreciated", "Fully Depreciated"):
			dontmanage.throw(_("Asset cannot be cancelled, as it is already {0}").format(self.status))

	def cancel_movement_entries(self):
		movements = dontmanage.db.sql(
		...
		for movement in movements:
			movement = dontmanage.get_doc("Asset Movement", movement.get("name"))
			movement.cancel()
		...
		if self.calculate_depreciation:
			for row in self.get("finance_books"):
				...
				for d in depr_schedule or []:
					if d.journal_entry:
						dontmanage.get_doc("Journal Entry", d.journal_entry).cancel()
		else:
			depr_entries = self.get_manual_depreciation_entries()

			for depr_entry in depr_entries or []:
				dontmanage.get_doc("Journal Entry", depr_entry.name).cancel()
	...
	def get_default_finance_book_idx(self):
		if not self.get("default_finance_book") and self.company:
			self.default_finance_book = dontmanageerp.get_default_finance_book(self.company)
	...
	def validate_make_gl_entry(self):
		...
		if asset_bought_with_invoice:
			...
			expense_booked = dontmanage.db.sql(query, (purchase_document, fixed_asset_account), as_dict=1)
			if expense_booked:
				# if expense is already booked from invoice then do not make gl entries regardless of cwip enabled/disabled
				return False

			cwip_booked = dontmanage.db.sql(query, (purchase_document, cwip_account), as_dict=1)
			if cwip_booked:
	...
	def get_purchase_document(self):
		asset_bought_with_invoice = self.purchase_invoice and dontmanage.db.get_value(
		...
		if not fixed_asset_account:
			dontmanage.throw(
				_("Set {0} in asset category {1} for company {2}").format(
					dontmanage.bold(_("Fixed Asset Account")),
					dontmanage.bold(self.asset_category),
	...
	def get_depreciation_rate(self, args, on_validate=False):
		...
		float_precision = cint(dontmanage.db.get_default("float_precision")) or 2
...
def has_gl_entries(doctype, docname, target_account):
	gl_entry = dontmanage.qb.DocType("GL Entry")
	gl_entries = (
		dontmanage.qb.from_(gl_entry)
...
def update_maintenance_status():
	assets = dontmanage.get_all(
		"Asset", filters={"docstatus": 1, "maintenance_required": 1, "disposal_date": ("is", "not set")}
	)

	for asset in assets:
		asset = dontmanage.get_doc("Asset", asset.name)
		if dontmanage.db.exists("Asset Repair", {"asset_name": asset.name, "repair_status": "Pending"}):
			asset.set_status("Out of Order")
		elif dontmanage.db.exists("Asset Maintenance Task", {"parent": asset.name, "next_due_date": today()}):
...
def make_post_gl_entry():
	asset_categories = dontmanage.db.get_all("Asset Category", fields=["name", "enable_cwip_accounting"])

	for asset_category in asset_categories:
		if cint(asset_category.enable_cwip_accounting):
			assets = dontmanage.db.sql_list(
			...
			for asset in assets:
				doc = dontmanage.get_doc("Asset", asset)
				doc.make_gl_entries()


def get_asset_naming_series():
	meta = dontmanage.get_meta("Asset")
	return meta.get_field("naming_series").options


@dontmanage.whitelist()
def make_sales_invoice(asset, item_code, company, serial_no=None):
	asset_doc = dontmanage.get_doc("Asset", asset)
	si = dontmanage.new_doc("Sales Invoice")
	si.company = company
	si.currency = dontmanage.get_cached_value("Company", company, "default_currency")
...
def create_asset_maintenance(asset, item_code, item_name, asset_category, company):
	asset_maintenance = dontmanage.new_doc("Asset Maintenance")
...
def create_asset_repair(company, asset, asset_name):
	asset_repair = dontmanage.new_doc("Asset Repair")
...
def create_asset_capitalization(company, asset, asset_name, item_code):
	asset_capitalization = dontmanage.new_doc("Asset Capitalization")
...
def create_asset_value_adjustment(asset, asset_category, company):
	asset_value_adjustment = dontmanage.new_doc("Asset Value Adjustment")
...
def transfer_asset(args):
	...
	movement_entry = dontmanage.new_doc("Asset Movement")
...
def get_item_details(item_code, asset_category, gross_purchase_amount):
	asset_category_doc = dontmanage.get_cached_doc("Asset Category", asset_category)
	books = []
...
def get_asset_account(account_name, asset=None, asset_category=None, company=None):
	...
	if not account:
		if not asset_category:
			dontmanage.throw(_("Set {0} in company {1}").format(account_name.replace("_", " ").title(), company))
		else:
			dontmanage.throw(
...
def make_asset_movement(assets, purpose=None):
	...
	if len(assets) == 0:
		dontmanage.throw(_("Atleast one asset has to be selected."))

	asset_movement = dontmanage.new_doc("Asset Movement")
	asset_movement.quantity = len(assets)
	for asset in assets:
		asset = dontmanage.get_doc("Asset", asset.get("name"))
...
def is_cwip_accounting_enabled(asset_category):
	return cint(dontmanage.db.get_value("Asset Category", asset_category, "enable_cwip_accounting"))
...
def has_active_capitalization(asset):
	active_capitalizations = dontmanage.db.count(
...
def split_asset(asset_name, split_qty):
	...
	if split_qty >= asset.asset_quantity:
		dontmanage.throw(_("Split qty cannot be grater than or equal to asset qty"))
...
def update_existing_asset(asset, remaining_qty, new_asset_name):
	...
	for row in asset.get("finance_books"):
		...
		new_asset_depr_schedule_doc = dontmanage.copy_doc(current_asset_depr_schedule_doc)
...
def create_new_asset_after_split(asset, split_qty):
	new_asset = dontmanage.copy_doc(asset)
