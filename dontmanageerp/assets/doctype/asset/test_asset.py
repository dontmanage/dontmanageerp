...
class AssetSetup(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		dontmanage.db.sql("delete from `tabTax Rule`")
...
class TestAsset(AssetSetup):
	...
	def test_gross_purchase_amount_is_mandatory(self):
		...
		self.assertRaises(dontmanage.MandatoryError, asset.save)
	...
	def test_pr_or_pi_mandatory_if_not_existing_asset(self):
		...
		self.assertRaises(dontmanage.ValidationError, asset.save)
	...
	def test_validate_item(self):
		asset = create_asset(item_code="MacBook Pro", do_not_save=1)
		item = dontmanage.get_doc("Item", "MacBook Pro")
	...
	def test_is_fixed_asset_set(self):
		...
		doc = dontmanage.new_doc("Purchase Invoice")
	...
	def test_gle_made_by_asset_sale(self):
		...
		self.assertEqual(dontmanage.db.get_value("Asset", asset.name, "status"), "Sold")
		...
		si.cancel()
		self.assertEqual(dontmanage.db.get_value("Asset", asset.name, "status"), "Partially Depreciated")
	...
	def test_asset_splitting(self):
		...
		jv = dontmanage.get_doc("Journal Entry", journal_entry)
	...
	def test_cwip_accounting(self):
		...
		asset = dontmanage.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
	...
	def test_asset_cwip_toggling_cases(self):
		cwip = dontmanage.db.get_value("Asset Category", "Computers", "enable_cwip_accounting")
		name = dontmanage.db.get_value(
			"Asset Category Account", filters={"parent": "Computers"}, fieldname=["name"]
		)
		cwip_acc = "CWIP Account - _TC"

		dontmanage.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 0)
		dontmanage.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", "")
		dontmanage.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", "")
		...
		asset = dontmanage.db.get_value("Asset", {"purchase_invoice": pi.name, "docstatus": 0}, "name")
		asset_doc = dontmanage.get_doc("Asset", asset)
		...
		dontmanage.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 1)
		dontmanage.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		asset = dontmanage.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
		asset_doc = dontmanage.get_doc("Asset", asset)
		asset_doc.available_for_use_date = nowdate()
		asset_doc.calculate_depreciation = 0
		...
		dontmanage.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", cwip)
		dontmanage.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		dontmanage.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", cwip_acc)
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_number_of_depreciations(self):
		...
		self.assertRaises(dontmanage.ValidationError, asset_2.save)
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_expense_account(self):
		...
		je = dontmanage.get_doc("Journal Entry", get_depr_schedule(asset.name, "Active")[0].journal_entry)
		accounting_entries = [
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_income_account(self):
		...
		depr_expense_account = dontmanage.get_doc("Account", "_Test Depreciations - _TC")
	...
	def test_depreciation_entry_cancellation(self):
		...
		dontmanage.get_doc("Journal Entry", depr_entry).cancel()
	...
	def test_gle_made_by_depreciation_entries(self):
		...
		dontmanage.db.set_value("Company", "_Test Company", "series_for_depreciation_entry", "DEPR-")
		post_depreciation_entries(date="2021-01-01")
		asset.load_from_db()
	...
	def test_asset_cost_center(self):
		...
		self.assertRaises(dontmanage.ValidationError, asset.submit)
	...
	def test_manual_depreciation_with_incorrect_jv_voucher_type(self):
		...
		self.assertRaises(dontmanage.ValidationError, jv.insert)
...
def create_asset_data():
	...
	if not dontmanage.db.exists("Item", "Macbook Pro"):
	...
	if not dontmanage.db.exists("Finance Book", "Test Finance Book 1"):
		dontmanage.get_doc({"doctype": "Finance Book", "finance_book_name": "Test Finance Book 1"}).insert()

	if not dontmanage.db.exists("Finance Book", "Test Finance Book 2"):
		dontmanage.get_doc({"doctype": "Finance Book", "finance_book_name": "Test Finance Book 2"}).insert()

	if not dontmanage.db.exists("Finance Book", "Test Finance Book 3"):
		dontmanage.get_doc({"doctype": "Finance Book", "finance_book_name": "Test Finance Book 3"}).insert()
...
def enable_cwip_accounting(asset_category, enable=1):
	dontmanage.db.set_value("Asset Category", asset_category, "enable_cwip_accounting", enable)
