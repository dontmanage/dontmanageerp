...
dontmanage.provide("dontmanageerp.asset");
dontmanage.provide("dontmanageerp.accounts.dimensions");

dontmanage.ui.form.on('Asset', {
	onload: function(frm) {
	...
	setup: function(frm) {
		...
		frm.make_methods = {
			'Asset Movement': () => {
				dontmanage.call({
				method: "dontmanageerp.assets.doctype.asset.asset.make_asset_movement",
				freeze: true,
				args:{
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "dontmanageerp.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code }
			}
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "dontmanageerp.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code }
			}
		});
	},
		...
		if (frm.doc.docstatus==1) {
			if (in_list(["Submitted", "Partially Depreciated", "Fully Depreciated"], frm.doc.status)) {
				...
				frm.add_custom_button(__("Restore Asset"), function() {
					dontmanageerp.asset.restore_asset(frm);
				}, __("Manage"));
			}
	...
	make_journal_entry: function(frm) {
		dontmanage.call({
			method: "dontmanageerp.assets.doctype.asset.asset.make_journal_entry",
			args: {
	...
	render_depreciation_schedule_view: function(frm, asset_depr_schedule_doc) {
		...
		asset_depr_schedule_doc.depreciation_schedule.forEach((sch) => {
			const row = [
				sch['idx'],
				dontmanage.format(sch['schedule_date'], { fieldtype: 'Date' }),
				dontmanage.format(sch['depreciation_amount'], { fieldtype: 'Currency' }),
				dontmanage.format(sch['accumulated_depreciation_amount'], { fieldtype: 'Currency' }),
				sch['journal_entry'] || '',
			];
		...
		let datatable = new dontmanage.DataTable(wrapper.get(0), {
			columns: columns,
			data: data,
			layout: "fluid",
	...
	setup_chart_and_depr_schedule_view: async function(frm) {
		...
		var x_intervals = [dontmanage.format(frm.doc.purchase_date, { fieldtype: 'Date' })];
		var asset_values = [frm.doc.gross_purchase_amount];

		if(frm.doc.calculate_depreciation) {
			if(frm.doc.opening_accumulated_depreciation) {
				var depreciation_date = dontmanage.datetime.add_months(
					frm.doc.finance_books[0].depreciation_start_date,
					-1 * frm.doc.finance_books[0].frequency_of_depreciation
				);
				x_intervals.push(dontmanage.format(depreciation_date, { fieldtype: 'Date' }));
			...
			let asset_depr_schedule_doc = (await dontmanage.call(
				"dontmanageerp.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule.get_asset_depr_schedule_doc",
				{
					asset_name: frm.doc.name,
					status: "Active",
			...
			$.each(asset_depr_schedule_doc.depreciation_schedule || [], function(i, v) {
				x_intervals.push(dontmanage.format(v.schedule_date, { fieldtype: 'Date' }));
			...
			if(frm.doc.opening_accumulated_depreciation) {
				x_intervals.push(dontmanage.format(frm.doc.creation.split(" ")[0], { fieldtype: 'Date' }));
				asset_values.push(flt(frm.doc.gross_purchase_amount - frm.doc.opening_accumulated_depreciation, precision('gross_purchase_amount')));
			}

			let depr_entries = (await dontmanage.call({
				method: "get_manual_depreciation_entries",
				doc: frm.doc,
			})).message;

			$.each(depr_entries || [], function(i, v) {
				x_intervals.push(dontmanage.format(v.posting_date, { fieldtype: 'Date' }));
		...
		if(in_list(["Scrapped", "Sold"], frm.doc.status)) {
			x_intervals.push(dontmanage.format(frm.doc.disposal_date, { fieldtype: 'Date' }));
			asset_values.push(0);
		}
	...
	set_finance_book: function(frm) {
		dontmanage.call({
			method: "dontmanageerp.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	make_sales_invoice: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.assets.doctype.asset.asset.make_sales_invoice",
			callback: function(r) {
				var doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_maintenance: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function(r) {
				var doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_repair: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.assets.doctype.asset.asset.create_asset_repair",
			callback: function(r) {
				var doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_capitalization: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.assets.doctype.asset.asset.create_asset_capitalization",
			callback: function(r) {
				var doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	split_asset: function(frm) {
		...
		dialog.set_primary_action(__('Split'), function() {
			...
			dontmanage.call({
				...
				method: "dontmanageerp.assets.doctype.asset.asset.split_asset",
				callback: function(r) {
					let doclist = dontmanage.model.sync(r.message);
					dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
				}
			});
	...
	create_asset_value_adjustment: function(frm) {
		dontmanage.call({
			...
			method: "dontmanageerp.assets.doctype.asset.asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function(r) {
				var doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	purchase_receipt: (frm) => {
		...
		if (frm.doc.purchase_receipt) {
			if (frm.doc.item_code) {
				dontmanage.db.get_doc('Purchase Receipt', frm.doc.purchase_receipt).then(pr_doc => {
	...
	purchase_invoice: (frm) => {
		...
		if (frm.doc.purchase_invoice) {
			if (frm.doc.item_code) {
				dontmanage.db.get_doc('Purchase Invoice', frm.doc.purchase_invoice).then(pi_doc => {
	...
	set_values_from_purchase_doc: function(frm, doctype, purchase_doc) {
		...
		if (!item) {
			let doctype_field = dontmanage.scrub(doctype)
		...
		dontmanage.db.get_value('Item', item.item_code, 'is_grouped_asset', (r) => {
	...
	set_depreciation_rate: function(frm, row) {
		if (row.total_number_of_depreciations && row.frequency_of_depreciation
			...
			dontmanage.call({
				...
				callback: function(r) {
					if (r.message) {
						dontmanage.flags.dont_change_rate = true;
						dontmanage.model.set_value(row.doctype, row.name,
	...
	set_salvage_value_percentage_or_expected_value_after_useful_life: function(frm, row, salvage_value_percentage_changed, expected_value_after_useful_life_changed) {
		if (expected_value_after_useful_life_changed) {
			dontmanage.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = true;
			...
			dontmanage.model.set_value(row.doctype, row.name, "salvage_value_percentage", new_salvage_value_percentage);
			dontmanage.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = false;
			...
			dontmanage.model.set_value(row.doctype, row.name, "expected_value_after_useful_life", new_expected_value_after_useful_life);
			dontmanage.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = false;
		}
	},
});

dontmanage.ui.form.on('Asset Finance Book', {
	...
	expected_value_after_useful_life: function(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (!dontmanage.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life) {
	...
	rate_of_depreciation: function(frm, cdt, cdn) {
		if(!dontmanage.flags.dont_change_rate) {
			dontmanage.model.set_value(cdt, cdn, "expected_value_after_useful_life", 0);
		}

		dontmanage.flags.dont_change_rate = false;
	},

	depreciation_start_date: function(frm, cdt, cdn) {
		const book = locals[cdt][cdn];
		if (frm.doc.available_for_use_date && book.depreciation_start_date == frm.doc.available_for_use_date) {
			dontmanage.msgprint(__("Depreciation Posting Date should not be equal to Available for Use Date."));
...
dontmanageerp.asset.scrap_asset = function(frm) {
	dontmanage.confirm(__("Do you really want to scrap this asset?"), function () {
		dontmanage.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "dontmanageerp.assets.doctype.asset.depreciation.scrap_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
dontmanageerp.asset.restore_asset = function(frm) {
	dontmanage.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		dontmanage.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "dontmanageerp.assets.doctype.asset.depreciation.restore_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
dontmanageerp.asset.transfer_asset = function() {
	dontmanage.call({
		method: "dontmanageerp.assets.doctype.asset.asset.make_asset_movement",
		freeze: true,
		args:{
