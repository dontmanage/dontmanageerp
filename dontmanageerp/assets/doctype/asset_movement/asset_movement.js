...
dontmanage.ui.form.on('Asset Movement', {
	setup: (frm) => {
...
dontmanage.ui.form.on('Asset Movement Item', {
	asset: function(frm, cdt, cdn) {
		// on manual entry of an asset auto sets their source location / employee
		const asset_name = locals[cdt][cdn].asset;
		if (asset_name){
			dontmanage.db.get_doc('Asset', asset_name).then((asset_doc) => {
				if(asset_doc.location) dontmanage.model.set_value(cdt, cdn, 'source_location', asset_doc.location);
				if(asset_doc.custodian) dontmanage.model.set_value(cdt, cdn, 'from_employee', asset_doc.custodian);
			}).catch((err) => {
