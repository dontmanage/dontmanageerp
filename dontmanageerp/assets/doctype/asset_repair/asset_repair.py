...
class AssetRepair(AccountsController):
	...
	def check_repair_status(self):
		if self.repair_status == "Pending":
			dontmanage.throw(_("Please update Repair Status."))

	def check_for_stock_items_and_warehouse(self):
		if not self.get("stock_items"):
			dontmanage.throw(_("Please enter Stock Items consumed during the Repair."), title=_("Missing Items"))
	...
	def get_gl_entries_for_repair_cost(self, gl_entries, fixed_asset_account):
		...
		pi_expense_account = (
			dontmanage.get_doc("Purchase Invoice", self.purchase_invoice).items[0].expense_account
		)
	...
	def get_gl_entries_for_consumed_items(self, gl_entries, fixed_asset_account):
		...
		stock_entry = dontmanage.get_doc("Stock Entry", {"asset_repair": self.name})
		...
		if not dontmanageerp.is_perpetual_inventory_enabled(self.company):
			...
			if not default_expense_account:
				dontmanage.throw(_("Please set default Expense Account in Company {0}").format(self.company))
