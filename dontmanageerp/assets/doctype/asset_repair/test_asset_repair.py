...
import dontmanage
from dontmanage.utils import flt, nowdate, nowtime, today

from dontmanageerp.assets.doctype.asset.asset import (
	get_asset_account,
	get_asset_value_after_depreciation,
)
from dontmanageerp.assets.doctype.asset.test_asset import (
	create_asset,
	create_asset_data,
...
class TestAssetRepair(unittest.TestCase):
	...
	def test_update_status(self):
		...
		asset_repair.save()
		asset_status = dontmanage.db.get_value("Asset", asset_repair.asset, "status")
	...
	def test_decrease_stock_quantity(self):
		...
		stock_entry = dontmanage.get_last_doc("Stock Entry")
	...
	def test_serialized_item_consumption(self):
		...
		asset_repair.repair_status = "Completed"
		self.assertRaises(dontmanage.ValidationError, asset_repair.submit)
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		asset_category = dontmanage.get_doc("Asset Category", "Computers")
		...
		pi_expense_account = (
			dontmanage.get_doc("Purchase Invoice", asset_repair.purchase_invoice).items[0].expense_account
		)
		stock_entry_expense_account = (
			dontmanage.get_doc("Stock Entry", {"asset_repair": asset_repair.name}).get("items")[0].expense_account
		)
	...
	def test_gl_entries_with_periodical_inventory(self):
		dontmanage.db.set_value("Company", "_Test Company", "default_expense_account", "Cost of Goods Sold - _TC")
		asset_repair = create_asset_repair(
			capitalize_repair_cost=1,
			stock_consumption=1,
			submit=1,
		...
		default_expense_account = dontmanage.get_cached_value(
...
def create_asset_repair(**args):
	...
	if args.submit:
		asset_repair.repair_status = "Completed"
		asset_repair.cost_center = dontmanage.db.get_value("Company", asset.company, "cost_center")
		...
		if args.capitalize_repair_cost:
			...
			pi = make_purchase_invoice(
				company=asset.company,
				expense_account=dontmanage.db.get_value("Company", asset.company, "default_expense_account"),
				cost_center=asset_repair.cost_center,
				warehouse=args.warehouse or create_warehouse("Test Warehouse", company=asset.company),
			)
