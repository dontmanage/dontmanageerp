...
dontmanage.ui.form.on("Asset Repair", {
	setup: function (frm) {
	...
	repair_status: (frm) => {
		if (frm.doc.completion_date && frm.doc.repair_status == "Completed") {
			dontmanage.call({
				method: "dontmanageerp.assets.doctype.asset_repair.asset_repair.get_downtime",
				args: {
		...
		if (frm.doc.repair_status == "Completed" && !frm.doc.completion_date) {
			frm.set_value("completion_date", dontmanage.datetime.now_datetime());
		}
	},
...
dontmanage.ui.form.on("Asset Repair Consumed Item", {
	warehouse: function (frm, cdt, cdn) {
		var item = locals[cdt][cdn];

		if (!item.item_code) {
			dontmanage.msgprint(__("Please select an item code before setting the warehouse."));
			dontmanage.model.set_value(cdt, cdn, "warehouse", "");
			return;
		}
		...
		dontmanage.call({
			...
			callback: function (r) {
				dontmanage.model.set_value(cdt, cdn, "valuation_rate", r.message);
			},
		});
	},
	...
	consumed_quantity: function (frm, cdt, cdn) {
		...
		dontmanage.model.set_value(cdt, cdn, "total_value", row.consumed_quantity * row.valuation_rate);
	},
});
