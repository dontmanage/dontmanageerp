...
from dontmanage.utils.nestedset import NestedSet, update_nsm
...
class Location(NestedSet):
	...
	def update_ancestor_location_features(self):
		...
		for ancestor in self.get_ancestors():
			ancestor_doc = dontmanage.get_doc("Location", ancestor)
...
def get_children(doctype, parent=None, location=None, is_root=False):
	...
	return dontmanage.db.sql(
		...
		from
			`tabLocation` comp
		where
			ifnull(parent_location, "")={dontmanage.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def on_doctype_update():
	dontmanage.db.add_index("Location", ["lft", "rgt"])
