...
class AssetShiftAllocation(Document):
			...
			for i, schedule in reversed(list(enumerate(self.depreciation_schedule))):
				...
				if shift_factor <= diff:
					...
					try:
						...
						dontmanage.throw(_("Could not auto update shifts. Shift with shift factor {0} needed.")).format(
							shift_factor - diff
						)
		elif diff < 0:
	...
	def create_new_asset_depr_schedule(self):
		new_asset_depr_schedule_doc = dontmanage.copy_doc(self.asset_depr_schedule_doc)
