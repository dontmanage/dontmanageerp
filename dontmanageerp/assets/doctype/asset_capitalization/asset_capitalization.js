...
dontmanage.provide("dontmanageerp.assets");
...
dontmanageerp.assets.AssetCapitalization = class AssetCapitalization extends dontmanageerp.stock.StockController {
	...
	setup_queries() {
		...
		me.frm.set_query("target_item_code", function () {
			if (me.frm.doc.entry_type == "Capitalization") {
				return dontmanageerp.queries.item({ is_stock_item: 0, is_fixed_asset: 1 });
			} else {
				return dontmanageerp.queries.item({ is_stock_item: 1, is_fixed_asset: 0 });
			}
		});
		...
		me.frm.set_query("item_code", "service_items", function () {
			return dontmanageerp.queries.item({ is_stock_item: 0, is_fixed_asset: 0 });
		});
	...
	company() {
		...
		if (me.frm.doc.company) {
			dontmanage.model.set_value(me.frm.doc.doctype, me.frm.doc.name, "cost_center", null);
			$.each(me.frm.doc.stock_items || [], function (i, d) {
				dontmanage.model.set_value(d.doctype, d.name, "cost_center", null);
			});
	...
	stock_items_add(doc, cdt, cdn) {
		dontmanageerp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "stock_items");
	}

	asset_items_add(doc, cdt, cdn) {
		dontmanageerp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "asset_items");
	}

	serivce_items_add(doc, cdt, cdn) {
		dontmanageerp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "service_items");
	}
	...
	get_target_item_details() {
		...
		if (me.frm.doc.target_item_code) {
			return me.frm.call({
				method: "dontmanageerp.assets.doctype.asset_capitalization.asset_capitalization.get_target_item_details",
				child: me.frm.doc,
				args: {
	...
	get_target_asset_details() {
		...
		if (me.frm.doc.target_asset) {
			return me.frm.call({
				method: "dontmanageerp.assets.doctype.asset_capitalization.asset_capitalization.get_target_asset_details",
				child: me.frm.doc,
				args: {
	...
	get_consumed_stock_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "dontmanageerp.assets.doctype.asset_capitalization.asset_capitalization.get_consumed_stock_item_details",
				child: row,
				args: {
					args: {
						item_code: row.item_code,
	...
	get_consumed_asset_details(row) {
		...
		if (row && row.asset) {
			return me.frm.call({
				method: "dontmanageerp.assets.doctype.asset_capitalization.asset_capitalization.get_consumed_asset_details",
				child: row,
				args: {
					args: {
						asset: row.asset,
	...
	get_service_item_details(row) {
		...
		if (row && row.item_code) {
			return me.frm.call({
				method: "dontmanageerp.assets.doctype.asset_capitalization.asset_capitalization.get_service_item_details",
				child: row,
				args: {
					args: {
						item_code: row.item_code,
	...
	get_warehouse_details(item) {
		...
		if (item.item_code && item.warehouse) {
			me.frm.call({
				method: "dontmanageerp.assets.doctype.asset_capitalization.asset_capitalization.get_warehouse_details",
				child: item,
				args: {
					args: {
						item_code: item.item_code,
...
cur_frm.cscript = new dontmanageerp.assets.AssetCapitalization({ frm: cur_frm });
