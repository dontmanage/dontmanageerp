...
class AssetCapitalization(StockController):
	...
	def validate_asset(self, asset):
		if asset.status in ("Draft", "Scrapped", "Sold", "Capitalized", "Decapitalized"):
			dontmanage.throw(_("Asset {0} is {1}").format(asset.name, asset.status))

		if asset.docstatus == 0:
			dontmanage.throw(_("Asset {0} is Draft").format(asset.name))
		if asset.docstatus == 2:
			dontmanage.throw(_("Asset {0} is cancelled").format(asset.name))

		if asset.company != self.company:
			dontmanage.throw(_("Asset {0} does not belong to company {1}").format(asset.name, self.company))
