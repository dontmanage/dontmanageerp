...
import dontmanageerp
from dontmanageerp.assets.doctype.asset.asset import get_asset_value_after_depreciation
...
class AssetCapitalization(StockController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.assets.doctype.asset_capitalization_asset_item.asset_capitalization_asset_item import (
			AssetCapitalizationAssetItem,
		)
		from dontmanageerp.assets.doctype.asset_capitalization_service_item.asset_capitalization_service_item import (
			AssetCapitalizationServiceItem,
		)
		from dontmanageerp.assets.doctype.asset_capitalization_stock_item.asset_capitalization_stock_item import (
	...
	def cancel_target_asset(self):
		if self.entry_type == "Capitalization" and self.target_asset:
			asset_doc = dontmanage.get_doc("Asset", self.target_asset)
	...
	def validate_target_item(self):
		target_item = dontmanage.get_cached_doc("Item", self.target_item_code)
		...
		if self.entry_type == "Capitalization" and not target_item.is_fixed_asset:
			dontmanage.throw(_("Target Item {0} must be a Fixed Asset item").format(target_item.name))
		elif self.entry_type == "Decapitalization" and not target_item.is_stock_item:
			dontmanage.throw(_("Target Item {0} must be a Stock Item").format(target_item.name))

		if target_item.is_fixed_asset:
			self.target_qty = 1
		if flt(self.target_qty) <= 0:
			dontmanage.throw(_("Target Qty must be a positive number"))
		...
		if target_item.is_stock_item and not self.target_warehouse:
			dontmanage.throw(_("Target Warehouse is mandatory for Decapitalization"))
	...
	def validate_target_asset(self):
		if self.target_asset:
			...
			if not target_asset.is_composite_asset:
				dontmanage.throw(_("Target Asset {0} needs to be composite asset").format(target_asset.name))
			...
			if target_asset.docstatus == 1:
				dontmanage.throw(_("Target Asset {0} cannot be submitted").format(target_asset.name))
			elif target_asset.docstatus == 2:
				dontmanage.throw(_("Target Asset {0} cannot be cancelled").format(target_asset.name))
	...
	def validate_consumed_stock_item(self):
		for d in self.stock_items:
			if d.item_code:
				item = dontmanage.get_cached_doc("Item", d.item_code)

				if not item.is_stock_item:
					dontmanage.throw(_("Row #{0}: Item {1} is not a stock item").format(d.idx, d.item_code))

				if flt(d.stock_qty) <= 0:
					dontmanage.throw(_("Row #{0}: Qty must be a positive number").format(d.idx))
		...
		for d in self.asset_items:
			if d.asset:
				...
				if asset.docstatus == 0:
					dontmanage.throw(_("Row #{0}: Consumed Asset {1} cannot be Draft").format(d.idx, asset.name))
				elif asset.docstatus == 2:
					dontmanage.throw(_("Row #{0}: Consumed Asset {1} cannot be cancelled").format(d.idx, asset.name))
		...
		for d in self.service_items:
			if d.item_code:
				...
				if item.is_stock_item or item.is_fixed_asset:
					dontmanage.throw(_("Row #{0}: Item {1} is not a service item").format(d.idx, d.item_code))

				if flt(d.qty) <= 0:
					dontmanage.throw(_("Row #{0}: Qty must be a positive number").format(d.idx))

				if flt(d.rate) <= 0:
					dontmanage.throw(_("Row #{0}: Amount must be a positive number").format(d.idx))

				self.validate_item(item)

			if not d.cost_center:
				d.cost_center = dontmanage.get_cached_value("Company", self.company, "cost_center")

	def validate_source_mandatory(self):
		if not self.target_is_fixed_asset and not self.get("asset_items"):
			dontmanage.throw(_("Consumed Asset Items is mandatory for Decapitalization"))

		if not self.get("stock_items") and not self.get("asset_items"):
			dontmanage.throw(_("Consumed Stock Items or Consumed Asset Items is mandatory for Capitalization"))
		...
		for item_row in self.stock_items:
			...
			if sle_list:
				for sle in sle_list:
					...
					if dontmanageerp.is_perpetual_inventory_enabled(self.company):
	...
	def create_target_asset(self):
		...
		asset_doc = dontmanage.new_doc("Asset")
...
def get_target_item_details(item_code=None, company=None):
	...
	out.cost_center = get_default_cost_center(
		dontmanage._dict({"item_code": item.name, "company": company}),
		item_defaults,
		item_group_defaults,
		brand_defaults,
	)
...
def get_target_asset_details(asset=None, company=None):
	...
	# Get Asset Details
	asset_details = dontmanage._dict()
	if asset:
		asset_details = dontmanage.db.get_value("Asset", asset, ["asset_name", "item_code"], as_dict=1)
		if not asset_details:
			dontmanage.throw(_("Asset {0} does not exist").format(asset))
...
def get_consumed_stock_item_details(args):
	...
	if args.item_code and out.warehouse:
		incoming_rate_args = dontmanage._dict(
			{
...
def get_consumed_asset_details(args):
	...
	asset_details = dontmanage._dict()
	if args.asset:
		asset_details = dontmanage.db.get_value(
			"Asset", args.asset, ["asset_name", "item_code", "item_name"], as_dict=1
		)
		if not asset_details:
			dontmanage.throw(_("Asset {0} does not exist").format(args.asset))
	...
	# Cost Center
	if asset_details.item_code:
		item = dontmanage.get_cached_doc("Item", asset_details.item_code)
		item_defaults = get_item_defaults(item.name, args.company)
		item_group_defaults = get_item_group_defaults(item.name, args.company)
		brand_defaults = get_brand_defaults(item.name, args.company)
		out.cost_center = get_default_cost_center(
...
def get_items_tagged_to_wip_composite_asset(asset):
	...
	for d in pr_items:
		if not d.is_fixed_asset:
			stock_items.append(dontmanage._dict(d))
		else:
			asset_details = dontmanage.db.get_value(
				"Asset",
			...
			asset_items.append(dontmanage._dict(d))
