...
dontmanage.ui.form.on('Asset Maintenance', {
	setup: (frm) => {
		frm.set_query("assign_to", "asset_maintenance_tasks", function(doc) {
			return {
				query: "dontmanageerp.assets.doctype.asset_maintenance.asset_maintenance.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if(!frm.is_new()) {
			dontmanage.call({
				method: 'dontmanageerp.assets.doctype.asset_maintenance.asset_maintenance.get_maintenance_log',
					...
					(r.message || []).forEach(function(d) {
						$(`<div class='row' style='margin-bottom: 10px;'>
							<div class='col-sm-3 small'>
								<a onclick="dontmanage.set_route('List', 'Asset Maintenance Log',
...
dontmanage.ui.form.on('Asset Maintenance Task', {
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return dontmanage.call({
			method: 'dontmanageerp.assets.doctype.asset_maintenance.asset_maintenance.calculate_next_due_date',
			args: {
			...
			callback: function(r) {
				if (r.message) {
					dontmanage.model.set_value(cdt, cdn, "next_due_date", r.message);
				}
				else {
					dontmanage.model.set_value(cdt, cdn, "next_due_date", "");
				}
			}
		});
	}
