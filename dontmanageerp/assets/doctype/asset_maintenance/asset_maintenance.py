...
from dontmanage.utils import add_days, add_months, add_years, getdate, nowdate
...
class AssetMaintenance(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.assets.doctype.asset_maintenance_task.asset_maintenance_task import AssetMaintenanceTask
	...
	def after_delete(self):
		asset = dontmanage.get_doc("Asset", self.asset_name)
	...
	def sync_maintenance_tasks(self):
		...
		asset_maintenance_logs = dontmanage.get_all(
		...
		if asset_maintenance_logs:
			for asset_maintenance_log in asset_maintenance_logs:
				maintenance_log = dontmanage.get_doc("Asset Maintenance Log", asset_maintenance_log.name)
				maintenance_log.db_set("maintenance_status", "Cancelled")


def assign_tasks(asset_maintenance_name, assign_to_member, maintenance_task, next_due_date):
	team_member = dontmanage.db.get_value("User", assign_to_member, "email")
	args = {
...
def calculate_next_due_date(
	...
	if not start_date and not last_completion_date:
		start_date = dontmanage.utils.now()
...
def update_maintenance_log(asset_maintenance, item_code, item_name, task):
	asset_maintenance_log = dontmanage.get_value(
		"Asset Maintenance Log",
		{
	...
	if not asset_maintenance_log:
		...
		maintenance_log = dontmanage.get_doc("Asset Maintenance Log", asset_maintenance_log)
...
def get_team_members(doctype, txt, searchfield, start, page_len, filters):
	return dontmanage.db.get_values(
