...
import dontmanage
from dontmanage.utils import add_days, get_last_day, nowdate
...
class TestAssetMaintenance(unittest.TestCase):
	...
	def test_create_asset_maintenance(self):
		...
		if not dontmanage.db.exists("Asset Maintenance", "Photocopier"):
			asset_maintenance = dontmanage.get_doc(
				{
	...
	def test_create_asset_maintenance_log(self):
		if not dontmanage.db.exists("Asset Maintenance Log", "Photocopier"):
			asset_maintenance_log = dontmanage.get_doc(
				{
		...
		asset_maintenance = dontmanage.get_doc("Asset Maintenance", "Photocopier")
		next_due_date = calculate_next_due_date(asset_maintenance_log.completion_date, "Monthly")
		self.assertEqual(asset_maintenance.asset_maintenance_tasks[0].next_due_date, next_due_date)


def create_asset_data():
	if not dontmanage.db.exists("Asset Category", "Equipment"):
	...
	if not dontmanage.db.exists("Item", "Photocopier"):
...
def create_maintenance_team():
	...
	if not dontmanage.db.exists("Role", "Technician"):
		dontmanage.get_doc({"doctype": "Role", "role_name": "Technician"}).insert()
	...
	if not dontmanage.db.exists("Asset Maintenance Team", "Team Awesome"):
		dontmanage.get_doc(
			{
