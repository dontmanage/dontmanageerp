...
class AssetDepreciationSchedule(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.assets.doctype.depreciation_schedule.depreciation_schedule import (
	...
	def validate_another_asset_depr_schedule_does_not_exist(self):
		...
		asset_depr_schedule = dontmanage.db.exists(
	...
	def update_shift_depr_schedule(self):
		...
		asset_doc = dontmanage.get_doc("Asset", self.asset)
	...
	def prepare_draft_asset_depr_schedule_data_from_asset_name_and_fb_name(self, asset_name, fb_name):
		asset_doc = dontmanage.get_doc("Asset", asset_name)

		finance_book_filter = ["finance_book", "is", "not set"]
		if fb_name:
			finance_book_filter = ["finance_book", "=", fb_name]

		asset_finance_book_name = dontmanage.db.get_value(
			doctype="Asset Finance Book",
			filters=[["parent", "=", asset_name], finance_book_filter],
		)
		asset_finance_book_doc = dontmanage.get_doc("Asset Finance Book", asset_finance_book_name)
				...
				if flt(depreciation_amount, asset_doc.precision("gross_purchase_amount")) <= 0:
					dontmanage.throw(
						_(
							...
							dontmanage.bold(asset_doc.gross_purchase_amount),
							dontmanage.bold(row.total_number_of_depreciations),
							dontmanage.bold(row.frequency_of_depreciation),
						)
					)
	...
	def add_depr_schedule_row(self, schedule_date, depreciation_amount, schedule_idx):
		if self.shift_based:
			shift = (
				...
				else dontmanage.get_cached_value("Asset Shift Factor", {"default": 1}, "shift_name")
			)
		else:
			shift = None
...
def get_daily_depr_amount(asset, row, schedule_idx, amount):
	if cint(dontmanage.db.get_single_value("Accounts Settings", "calculate_depr_using_total_days")):
		total_days = (
			date_diff(
				get_last_day(
					add_months(
...
def get_asset_shift_factors_map():
	return dict(dontmanage.db.get_all("Asset Shift Factor", ["shift_name", "shift_factor"], as_list=True))
...
def make_draft_asset_depr_schedule(asset_doc, row):
	asset_depr_schedule_doc = dontmanage.new_doc("Asset Depreciation Schedule")
...
def get_temp_asset_depr_schedule_doc(
	...
	temp_asset_depr_schedule_doc = dontmanage.copy_doc(current_asset_depr_schedule_doc)
...
def get_asset_depr_schedule_doc(asset_name, status, finance_book=None):
	...
	asset_depr_schedule_doc = dontmanage.get_doc("Asset Depreciation Schedule", asset_depr_schedule[0].name)
