...
from dontmanage.utils import add_months, cstr, flt, formatdate, getdate, nowdate, today
...
def get_conditions(filters):
	...
	date_field = dontmanage.scrub(filters.date_based_on or "Purchase Date")
...
def get_data(filters):
	...
	group_by = dontmanage.scrub(filters.get("group_by"))
	...
	assets_record = dontmanage.db.get_all("Asset", filters=conditions, fields=fields)
...
def prepare_chart_data(data, filters):
	...
	if filters.filter_based_on not in ("Date Range", "Fiscal Year"):
		...
		date_field = dontmanage.scrub(filters.date_based_on)
	...
	for d in period_list:
		labels_values_map.setdefault(
			d.get("label"), dontmanage._dict({"asset_value": 0, "depreciated_amount": 0})
		)
...
def get_assets_linked_to_fb(filters):
	afb = dontmanage.qb.DocType("Asset Finance Book")

	query = dontmanage.qb.from_(afb).select(
		afb.parent,
	)
	...
	if filters.include_default_book_assets:
		...
		if filters.finance_book and company_fb and cstr(filters.finance_book) != cstr(company_fb):
			dontmanage.throw(_("To use a different finance book, please uncheck 'Include Default FB Assets'"))
...
def get_asset_depreciation_amount_map(filters, finance_book):
	...
	aca = dontmanage.qb.DocType("Asset Category Account")
	company = dontmanage.qb.DocType("Company")
...
def get_group_by_data(group_by, conditions, assets_linked_to_fb, depreciation_amount_map):
	...
	assets = dontmanage.db.get_all("Asset", filters=conditions, fields=fields)
		...
		return [
			{
				...
				"fieldtype": "Link",
				"fieldname": dontmanage.scrub(filters.get("group_by")),
