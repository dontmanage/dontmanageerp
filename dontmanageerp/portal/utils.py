...
from dontmanageerp.e_commerce.shopping_cart.cart import get_debtors_account


def set_default_role(doc, method):
	"""Set customer, supplier, student, guardian based on email"""
	if dontmanage.flags.setting_role or dontmanage.flags.in_migrate:
		return

	roles = dontmanage.get_roles(doc.name)

	contact_name = dontmanage.get_value("Contact", dict(email_id=doc.email))
	if contact_name:
		contact = dontmanage.get_doc("Contact", contact_name)
		for link in contact.links:
			dontmanage.flags.setting_role = True
...
def create_customer_or_supplier():
	...
	if dontmanage.db.get_value("User", user, "user_type") != "Website User":
		return

	user_roles = dontmanage.get_roles()
	portal_settings = dontmanage.get_single("Portal Settings")
	...
	party = dontmanage.new_doc(doctype)
	fullname = dontmanage.utils.get_fullname(user)
...
def party_exists(doctype, user):
	...
	contact_name = dontmanage.db.get_value("Contact", {"email_id": user})
	if contact_name:
		contact = dontmanage.get_doc("Contact", contact_name)
