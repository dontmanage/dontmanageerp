...
dontmanage.ui.form.on('Maintenance Schedule', {
	setup: function (frm) {
	...
	generate_schedule: function (frm) {
		if (frm.is_new()) {
			dontmanage.msgprint(__('Please save first'));
		} else {
...
dontmanageerp.maintenance.MaintenanceSchedule = class MaintenanceSchedule extends dontmanage.ui.form.Controller {
	refresh() {
...
extend_cscript(cur_frm.cscript, new dontmanageerp.maintenance.MaintenanceSchedule({frm: cur_frm}));
