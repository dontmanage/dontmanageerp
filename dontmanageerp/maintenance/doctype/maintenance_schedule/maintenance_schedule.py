...
from dontmanage.utils import add_days, cint, cstr, date_diff, formatdate, getdate
...
class MaintenanceSchedule(TransactionBase):
	...
	def on_submit(self):
		...
		for d in self.get("items"):
			...
			for key in scheduled_date:
				description = dontmanage._("Reference: {0}, Item Code: {1} and Customer: {2}").format(
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = dontmanage.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = dontmanage.get_cached_value("Company", self.company, "default_holiday_list")

		holidays = dontmanage.db.sql_list(
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			...
			if not sr_details:
				dontmanage.throw(_("Serial No {0} not found").format(serial_no))

			if sr_details.get("item_code") != item_code:
				dontmanage.throw(
					_("Serial No {0} does not belong to Item {1}").format(
						dontmanage.bold(serial_no), dontmanage.bold(item_code)
					),
					title=_("Invalid"),
				)
