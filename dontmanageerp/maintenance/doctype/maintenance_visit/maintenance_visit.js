...
dontmanage.provide("dontmanageerp.maintenance");
dontmanage.ui.form.on('Maintenance Visit', {
	setup: function (frm) {
	...
	onload: function (frm) {
		...
		if (frm.doc.maintenance_type === "Scheduled") {
			...
			dontmanage.call({
				method: "dontmanageerp.maintenance.doctype.maintenance_schedule.maintenance_schedule.get_serial_nos_from_schedule",
				args: {
		...
		if (frm.doc.__islocal) {
			frm.set_value({ mntc_date: dontmanage.datetime.get_today() });
		}
	},
	...
	customer_address: function (frm) {
		dontmanageerp.utils.get_address_display(frm, 'customer_address', 'address_display');
	},
...
dontmanageerp.maintenance.MaintenanceVisit = class MaintenanceVisit extends dontmanage.ui.form.Controller {
	refresh() {
			...
			this.frm.add_custom_button(__('Maintenance Schedule'),
				function () {
					if (!me.frm.doc.customer) {
						dontmanage.msgprint(__('Please select Customer first'));
						return;
					}
					dontmanageerp.utils.map_current_doc({
						method: "dontmanageerp.maintenance.doctype.maintenance_schedule.maintenance_schedule.make_maintenance_visit",
...
extend_cscript(cur_frm.cscript, new dontmanageerp.maintenance.MaintenanceVisit({frm: cur_frm}));
