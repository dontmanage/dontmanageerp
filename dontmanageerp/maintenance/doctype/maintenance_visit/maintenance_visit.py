...
from dontmanage.utils import format_date, get_datetime
...
class MaintenanceVisit(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.maintenance.doctype.maintenance_visit_purpose.maintenance_visit_purpose import (
	...
	def validate_serial_no(self):
		for d in self.get("purposes"):
			if d.serial_no and not dontmanage.db.exists("Serial No", d.serial_no):
				dontmanage.throw(_("Serial No {0} does not exist").format(d.serial_no))

	def validate_purpose_table(self):
		if not self.purposes:
			dontmanage.throw(_("Add Items in the Purpose Table"), title=_("Purposes Required"))

	def validate_maintenance_date(self):
		if self.maintenance_type == "Scheduled":
			if self.maintenance_schedule_detail:
				item_ref = dontmanage.db.get_value(
		...
		if not self.maintenance_schedule:
			for d in self.get("purposes"):
				if d.prevdoc_docname and d.prevdoc_doctype == "Warranty Claim":
					...
					wc_doc = dontmanage.get_doc("Warranty Claim", d.prevdoc_docname)
					wc_doc.update(
						{
