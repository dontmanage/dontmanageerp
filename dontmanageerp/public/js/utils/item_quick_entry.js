...
dontmanage.ui.form.ItemQuickEntryForm = class ItemQuickEntryForm extends dontmanage.ui.form.QuickEntryForm {
	...
	check_naming_series_based_on() {
		if (dontmanage.defaults.get_default("item_naming_by") === "Naming Series") {
			this.mandatory = this.mandatory.filter(d => d.fieldname !== "item_code");
		}
	}

	init_post_render_dialog_operations() {
		this.dialog.fields_dict.attribute_html.$wrapper.append(dontmanage.render_template("item_quick_entry"));
	...
	open_doc() {
		...
		if (this.dialog.fields_dict.create_variant.$input.prop("checked")) {
			...
			if (template)
				dontmanage.set_route("Form", this.doctype, template);
		} else {
		...
		$.each(attributes, function(index, row) {
			...
			if (!row.numeric_values) {
				...
				me[row.attribute].$input.on("change", function() {
					...
					$(this).closest(".dontmanage-control").toggleClass("has-error", $(this).val() ? false : true);
				});
			}
		});
	}
	...
	init_make_control(fieldtype, row) {
		this[row.attribute] = dontmanage.ui.form.make_control({
			df: {
				"fieldtype": fieldtype,
				"label": row.attribute,
	...
	init_awesomplete_for_attribute(row) {
		...
		this[row.attribute].$input.on('input', function(e) {
			...
			$(e.target).closest(".dontmanage-control").toggleClass("has-error", e.target.value ? false : true);
		});
	}
	...
	get_variant_doc() {
		...
		if (Object.keys(attribute).length) {
			dontmanage.call({
				method: "dontmanageerp.controllers.item_variant.create_variant_doc_for_quick_entry",
				args: {
				...
				callback: function(r) {
					if (Object.prototype.toString.call(r.message) == "[object Object]") {
						...
						var msgprint_dialog = dontmanage.msgprint(__("Item Variant {0} already exists with same attributes", [repl('<a class="strong variant-click" data-item-code="%(item)s" \
								>%(item)s</a>', {
								item: r.message
							})]));
						...
						msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							...
							me.dialog.hide();
							if (dontmanage._from_link) {
								dontmanage._from_link.set_value($(this).attr("data-item-code"));
							} else {
								dontmanage.set_route('Form', "Item", $(this).attr("data-item-code"));
							}
						});
					}
				}
