...
dontmanageerp.landed_cost_taxes_and_charges = {
		...
		dontmanage.ui.form.on(doctype, {
			...
			set_account_currency: function(frm, cdt, cdn) {
				...
				if (row.expense_account) {
					dontmanage.db.get_value('Account', row.expense_account, 'account_currency', function(value) {
						dontmanage.model.set_value(cdt, cdn, "account_currency", value.account_currency);
			...
			set_exchange_rate: function(frm, cdt, cdn) {
				...
				if (row.account_currency == company_currency) {
					...
					dontmanage.call({
						...
						callback: function(r) {
							if (r.message) {
								dontmanage.model.set_value(cdt, cdn, "exchange_rate", r.message);
							}
						}
					});
				}
			...
			set_base_amount: function(frm, cdt, cdn) {
				let row = locals[cdt][cdn];
				dontmanage.model.set_value(cdt, cdn, "base_amount",
