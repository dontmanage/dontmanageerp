dontmanageerp.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
	...
	download_csv_file() {
		...
		const method = `/api/method/dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(JSON.stringify(csvFileData))}`;
	...
	edit_full_form() {
		...
		if (!bundle_id) {
			let _new = dontmanage.model.get_new_doc(
