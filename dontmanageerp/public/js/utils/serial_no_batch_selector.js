dontmanageerp.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
		...
		if (qty > 0) {
			this.dialog.set_value("qty", qty).then(() => {
				if (this.item.serial_no && !this.item.serial_and_batch_bundle) {
					...
					dontmanage.model.set_value(this.item.doctype, this.item.name, "serial_no", "");
				} else if (this.item.batch_no && !this.item.serial_and_batch_bundle) {
					this.dialog.set_value("scan_batch_no", this.item.batch_no);
					dontmanage.model.set_value(this.item.doctype, this.item.name, "batch_no", "");
				}
	...
	set_serial_nos_from_range() {
		...
		const serial_nos = dontmanageerp.stock.utils.get_serial_range(serial_no_range, "::");
	...
	create_serial_nos() {
		...
		if (!upload_serial_nos) {
			dontmanage.throw(__("Please enter Serial Nos"));
		}

		dontmanage.call({
			method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.create_serial_nos",
			args: {
				item_code: this.item.item_code,
	...
	download_csv_file() {
		...
		const method = `/api/method/dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(
			JSON.stringify(csvFileData)
		)}`;
		const w = window.open(dontmanage.urllib.get_full_url(method));
		if (!w) {
	...
	upload_csv_file() {
		...
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file",
			args: {
				item_code: this.item.item_code,
				file_path: file_path,
			},
	...
	get_auto_data() {
		...
		if (qty) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data",
				args: {
					item_code: this.item.item_code,
					warehouse: warehouse,
	...
	scan_barcode_data() {
		...
		if (scan_serial_no || scan_batch_no) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.is_serial_batch_no_exists",
				args: {
					item_code: this.item.item_code,
	...
	update_serial_batch_no() {
		...
		if (scan_serial_no) {
			...
			if (existing_row?.length) {
				dontmanage.throw(__("Serial No {0} already exists", [scan_serial_no]));
			}
			...
			if (!this.item.has_batch_no) {
				...
				dontmanage.call({
					method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_batch_no_from_serial_no",
					args: {
						serial_no: scan_serial_no,
					},
					callback: (r) => {
	...
	update_bundle_entries() {
		...
		if ((entries && !entries.length) || !entries) {
			dontmanage.throw(__("Please add atleast one Serial No / Batch No"));
		}

		if (!warehouse) {
			dontmanage.throw(__("Please select a Warehouse"));
		}

		if (this.item?.is_rejected && this.item.rejected_warehouse === this.item.warehouse) {
			dontmanage.throw(__("Rejected Warehouse and Accepted Warehouse cannot be same."));
		}

		dontmanage
			.call({
				method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.add_serial_batch_ledgers",
				args: {
					entries: entries,
					child_row: this.item,
					doc: this.frm.doc,
	...
	edit_full_form() {
		...
		if (!bundle_id) {
			let _new = dontmanage.model.get_new_doc("Serial and Batch Bundle", null, null, true);
		...
		dontmanage.set_route("Form", "Serial and Batch Bundle", bundle_id);
		this.dialog.hide();
	}
	...
	render_data() {
		if (this.bundle || this.frm.doc.is_return) {
			dontmanage
				.call({
					method: "dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_serial_batch_ledgers",
					args: {
						item_code: this.item.item_code,
						name: this.bundle,
