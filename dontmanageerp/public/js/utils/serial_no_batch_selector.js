...
dontmanageerp.SerialNoBatchSelector = class SerialNoBatchSelector {
	...
	make_dialog() {
		...
		let fields = [
			...
			},
			{
				...
				get_query: function() {
					return {
						query: "dontmanageerp.controllers.queries.warehouse_query",
						filters: [
			...
			},
			{
				...
				click: () => {
					...
					let numbers = dontmanage.call({
						method: "dontmanageerp.stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
						args: {
							qty: qty,
					...
					numbers.then((data) => {
						...
						if (records_length < qty) {
							dontmanage.msgprint(__('Fetched only {0} available serial numbers.', [records_length]));
						}
	...
	validate() {
		...
		if(this.has_batch && !this.has_serial_no) {
			if(values.batches.length === 0 || !values.batches) {
				dontmanage.throw(__("Please select batches for batched item {0}", [values.item_code]));
			}
			values.batches.map((batch, i) => {
				if(!batch.selected_qty || batch.selected_qty === 0 ) {
					if (!this.show_dialog) {
						dontmanage.throw(__("Please select quantity on row {0}", [i+1]));
					}
				}
			});
			return true;
			...
			if (!serial_nos || !serial_nos.replace(/\s/g, '').length) {
				dontmanage.throw(__("Please enter serial numbers for serialized item {0}", [values.item_code]));
			}
			return true;
		}
	}
	...
	update_batch_serial_no_items() {
		...
		if(this.has_batch && this.has_serial_no) {
			...
			return dontmanage.db.get_list("Serial No", {
				...
				fields: [
					{
						...
						change: function () {
							...
							if (selected_batches.includes(batch_no)) {
								this.set_value("");
								dontmanage.throw(__('Batch {0} already selected.', [batch_no]));
							}
							...
							if (me.warehouse_details.name) {
								...
								this.set_value("");
								dontmanage.throw(__('Please select a warehouse to get available quantities'));
							}
					...
					},
					{
						...
						change: function () {
							...
							if (batch_no.length === 0 && parseInt(selected_qty) !== 0) {
								dontmanage.throw(__("Please select a batch"));
							}
							if (me.warehouse_details.type === 'Source Warehouse' &&
								parseFloat(available_qty) < parseFloat(selected_qty)) {

								this.set_value('0');
								dontmanage.throw(__('For transfer from source, selected quantity cannot be greater than available quantity'));
							} else {
								this.grid.refresh();
							}
	...
	get_serial_no_fields() {
		...
		if (me.frm.doc.doctype === 'POS Invoice' && !this.showing_reserved_serial_nos_error) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos",
				args: {
					filters: {
						item_code: me.item_code,
						warehouse: typeof me.warehouse_details.name == "string" ? me.warehouse_details.name : '',
...
function get_pending_qty_fields(me) {
	...
	const { qty_consumed_per_unit } = dontmanageerp.stock.bom.items[item_code];
...
function check_can_calculate_pending_qty(me) {
	...
	const docChecks = doc.bom_no
		...
		&& dontmanageerp.stock.bom
		&& dontmanageerp.stock.bom.name === doc.bom_no;
	const itemChecks = !!item
		&& !item.original_item
		&& dontmanageerp.stock.bom && dontmanageerp.stock.bom.items
		&& (item.item_code in dontmanageerp.stock.bom.items);
