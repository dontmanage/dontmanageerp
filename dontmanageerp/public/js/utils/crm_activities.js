dontmanageerp.utils.CRMActivities = class CRMActivities {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		if (!$(this.all_activities_wrapper).find(".form-footer").length) {
			...
			// remove dontmanage-control class to avoid absolute position for action-btn
			$(this.all_activities_wrapper).removeClass("dontmanage-control");
		...
		// open activities
		dontmanage.call({
			method: "dontmanageerp.crm.utils.get_open_activities",
			args: {
			...
			callback: (r) => {
				if (!r.exc) {
					var activities_html = dontmanage.render_template("crm_activities", {
	...
	create_task() {
		...
		let _create_task = () => {
			...
			let composer = new dontmanage.views.InteractionComposer(args);
	...
	async update_status(input_field, doctype) {
		...
		if (completed) {
			await dontmanage.db.set_value(doctype, docname, "status", "Closed");
			this.refresh();
		}
	}
};

dontmanageerp.utils.CRMNotes = class CRMNotes {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		let notes_html = dontmanage.render_template("crm_notes", {
			notes: notes,
		});
