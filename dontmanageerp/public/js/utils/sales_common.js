		...
		dontmanageerp.selling.SellingController = class SellingController extends dontmanageerp.TransactionController {
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], dontmanageerp.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", dontmanageerp.queries.contact_query);
				me.frm.set_query("company_contact_person", dontmanageerp.queries.company_contact_query);
				...
				me.frm.set_query("company_address", dontmanageerp.queries.company_address_query);
			...
			customer_address() {
				...
				dontmanageerp.utils.set_taxes_from_address(
					this.frm,
					"customer_address",
					"customer_address",
			...
			set_actual_qty(doc, cdt, cdn) {
				...
				if (row.item_code && row.warehouse && sales_doctypes.includes(doc.doctype)) {
					dontmanage.call({
						...
						callback(r) {
							if (r.message) {
								dontmanage.model.set_value(cdt, cdn, "actual_qty", r.message.actual_qty);
							}
						},
					});
				}
			...
			toggle_editable_price_list_rate() {
				...
				if (df && editable_price_list_rate) {
					const parent_field = dontmanage.meta.get_parentfield(
			...
			calculate_commission() {
				...
				if (this.frm.doc.commission_rate > 100) {
					...
					dontmanage.throw(
						`${__(
							dontmanage.meta.get_label(this.frm.doc.doctype, "commission_rate", this.frm.doc.name)
			...
			pick_serial_and_batch(doc, cdt, cdn) {
				...
				dontmanage.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new dontmanageerp.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
