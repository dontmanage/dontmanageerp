dontmanageerp.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		...
		// see https://dontmanageframework.com/docs/v14/user/en/python-api/hooks#sounds
		...
		// }
		this.scan_api = opts.scan_api || "dontmanageerp.stock.utils.scan_barcode";
	}
	...
	update_table(data) {
		return new Promise((resolve, reject) => {
			...
			if (!row) {
				...
				row = dontmanage.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
	...
	set_item(row, item_code, barcode, batch_no, serial_no) {
		return new Promise(resolve => {
			...
			if (this.prompt_qty) {
				dontmanage.prompt(__("Please enter quantity for item {0}", [item_code]), ({value}) => {
	...
	prepare_item_for_scan(row, item_code, barcode, batch_no, serial_no) {
		...
		this.dialog.set_primary_action(__("Update"), () => {
			...
			dontmanage.model.set_value(row.doctype, row.name, item_data);
	...
	validate_duplicate_serial_no(serial_no) {
		...
		if (in_list(serial_nos, serial_no)) {
			dontmanage.throw(__("Serial No {0} already scanned", [serial_no]));
		}
	}
	...
	add_child_for_remaining_qty(prev_row) {
		if (this.remaining_qty && this.remaining_qty >0) {
			let cur_grid = this.frm.fields_dict[this.items_table_name].grid;
			let row = dontmanage.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
			...
			if (this.qty_field == "qty" && dontmanage.meta.has_field(row.doctype, "stock_qty")) {
	...
	async set_serial_no(row, serial_no) {
		if (serial_no && dontmanage.meta.has_field(row.doctype, this.serial_no_field)) {
			...
			await dontmanage.model.set_value(row.doctype, row.name, this.serial_no_field, new_serial_nos);
		}
	}

	async set_barcode_uom(row, uom) {
		if (uom && dontmanage.meta.has_field(row.doctype, this.uom_field)) {
			await dontmanage.model.set_value(row.doctype, row.name, this.uom_field, uom);
		}
	}

	async set_batch_no(row, batch_no) {
		if (batch_no && dontmanage.meta.has_field(row.doctype, this.batch_no_field)) {
			await dontmanage.model.set_value(row.doctype, row.name, this.batch_no_field, batch_no);
		}
	}

	async set_barcode(row, barcode) {
		if (barcode && dontmanage.meta.has_field(row.doctype, this.barcode_field)) {
			await dontmanage.model.set_value(row.doctype, row.name, this.barcode_field, barcode);
		}
	}
	...
	get_row_to_modify_on_scan(item_code, batch_no, uom, barcode) {
		...
		let is_batch_no_scan = batch_no && dontmanage.meta.has_field(cur_grid.doctype, this.batch_no_field);
		let check_max_qty = this.max_qty_field && dontmanage.meta.has_field(cur_grid.doctype, this.max_qty_field);
	...
	play_success_sound() {
		this.success_sound && dontmanage.utils.play_sound(this.success_sound);
	}

	play_fail_sound() {
		this.fail_sound && dontmanage.utils.play_sound(this.fail_sound);
	}
	...
	show_alert(msg, indicator, duration=3) {
		dontmanage.show_alert({message: msg, indicator: indicator}, duration);
	}
};