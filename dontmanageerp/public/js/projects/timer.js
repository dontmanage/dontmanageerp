dontmanage.provide("dontmanageerp.timesheet");

dontmanageerp.timesheet.timer = function(frm, row, timestamp=0) {
	...
	dontmanageerp.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

dontmanageerp.timesheet.control_timer = function(frm, dialog, row, timestamp=0) {
	...
	$btn_start.click(function(e) {
		if (!initialized) {
			...
			row = dontmanage.model.add_child(frm.doc, "Timesheet Detail", "time_logs");
			row.activity_type = args.activity_type;
			row.from_time = dontmanage.datetime.get_datetime_as_string();
			...
			if(row.expected_hours) {
				...
				row.to_time = d.format(dontmanage.defaultDatetimeFormat);
			}
	...
	$btn_complete.click(function() {
		...
		grid_row.doc.to_time = dontmanage.datetime.now_datetime();
		grid_row.refresh();
		frm.dirty();
		frm.save();
		reset();
	...
	function updateStopwatch(increment) {
		...
		if(cur_dialog && cur_dialog.get_value('expected_hours') > 0) {
			if(flag && (currentIncrement >= (cur_dialog.get_value('expected_hours') * 3600))) {
				dontmanage.utils.play_sound("alert");
				dontmanage.msgprint(__("Timer exceeded the given hours."));
				flag = false;
			}
		}
