...
dontmanageerp.AccountTreeGrid = class AccountTreeGrid extends dontmanage.views.TreeGridReport {
		...
		super({
			...
			tree_grid: {
				...
				formatter: function(item) {
					return repl("<a \
						onclick='dontmanage.cur_grid_report.show_general_ledger(\"%(value)s\")'>\
						%(value)s</a>", {
							value: item.name,
						});
				}
	...
	setup_filters() {
		...
		this.filter_inputs.fiscal_year.change(function() {
			var fy = $(this).val();
			$.each(dontmanage.report_dump.data["Fiscal Year"], function(i, v) {
				if (v.name==fy) {
					me.filter_inputs.from_date.val(dontmanage.datetime.str_to_user(v.year_start_date));
					me.filter_inputs.to_date.val(dontmanage.datetime.str_to_user(v.year_end_date));
				}
			});
			me.refresh();
		});
	...
	prepare_data() {
		...
		if(!this.primary_data) {
			...
			$.each(dontmanage.report_dump.data["Account"], function(i, v) {
				var d = copy_dict(v);
	...
	prepare_balances() {
		var gl = dontmanage.report_dump.data['GL Entry'];
		var me = this;

		this.opening_date = dontmanage.datetime.user_to_obj(this.filter_inputs.from_date.val());
		this.closing_date = dontmanage.datetime.user_to_obj(this.filter_inputs.to_date.val());
		...
		$.each(gl, function(i, v) {
			var posting_date = dontmanage.datetime.str_to_obj(v.posting_date);
	...
	update_balances(account, posting_date, v) {
		...
		if (posting_date < this.opening_date || v.is_opening === "Yes") {
			if (account.report_type === "Profit and Loss" &&
				posting_date <= dontmanage.datetime.str_to_obj(this.fiscal_year[1])) {
	...
	set_fiscal_year() {
		if (this.opening_date > this.closing_date) {
			dontmanage.msgprint(__("Opening Date should be before Closing Date"));
			return;
		}
		...
		var me = this;
		$.each(dontmanage.report_dump.data["Fiscal Year"], function(i, v) {
			if (me.opening_date >= dontmanage.datetime.str_to_obj(v.year_start_date) &&
				me.closing_date <= dontmanage.datetime.str_to_obj(v.year_end_date)) {
				me.fiscal_year = v;
			}
		});

		if (!this.fiscal_year) {
			dontmanage.msgprint(__("Opening Date and Closing Date should be within same Fiscal Year"));
			return;
		}
	}
