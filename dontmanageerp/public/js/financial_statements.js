dontmanage.provide("dontmanageerp.financial_statements");
...
dontmanageerp.financial_statements = {
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		if (
			dontmanage.query_report.get_filter_value("selected_view") == "Growth" &&
			data &&
			column.colIndex >= 3
		) {
	...
	open_general_ledger: function (data) {
		...
		let project = $.grep(dontmanage.query_report.filters, function (e) {
		...
		if (["Payable", "Receivable"].includes(data.account_type)) {
			...
			dontmanage.route_options["party_account"] = data.account;
			dontmanage.route_options["report_date"] = data.year_end_date;
		}
	...
	let filters = [
		...
		},
		{
			...
			on_change: function () {
				...
				dontmanage.query_report.toggle_filter_display(
					"from_fiscal_year",
