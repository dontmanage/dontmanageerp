class CallPopup {
	...
	make() {
		dontmanage.utils.play_sound('incoming-call');
		...
		dontmanage.utils.bind_actions_with_object(this.dialog.$body, this);
	...
	close_modal() {
		this.dialog.hide();
		delete dontmanageerp.call_popup;
	}

	call_ended(call_log, missed) {
		dontmanage.utils.play_sound('call-disconnect');
	...
	setup_listener() {
		dontmanage.realtime.on(`call_${this.call_log.id}_ended`, call_log => {
			this.call_ended(call_log);
		});

		dontmanage.realtime.on(`call_${this.call_log.id}_missed`, call_log => {
			this.call_ended(call_log, true);
		});
	}

	clear_listeners() {
		dontmanage.realtime.off(`call_${this.call_log.id}_ended`);
		dontmanage.realtime.off(`call_${this.call_log.id}_missed`);
	}

	setup_call_details() {
		this.caller_info = $(`<div></div>`);
		this.call_details = new dontmanage.ui.FieldGroup({
			fields: [{
				'fieldname': 'name',
				'label': 'Name',
				...
				'click': () => dontmanage.set_route('Form', 'Contact', this.get_contact_link().link_name),
				...
				'click': () => dontmanage.new_doc('Lead', { 'mobile_no': this.caller_number }),
				'depends_on': () => !this.get_caller_name()
			}, {
				...
				'click': () => {
					...
					dontmanage.xcall('dontmanageerp.telephony.doctype.call_log.call_log.add_call_summary_and_call_type', {
	...
	create_new_customer() {
		...
		const new_customer = dontmanage.model.get_new_doc('Customer');
		new_customer.mobile_no = this.caller_number;
		dontmanage.set_route('Form', new_customer.doctype, new_customer.name);
	}

	create_new_contact() {
		// TODO: fix new_doc, it should accept child table values
		const new_contact = dontmanage.model.get_new_doc('Contact');
		const phone_no = dontmanage.model.add_child(new_contact, 'Contact Phone', 'phone_nos');
		phone_no.phone = this.caller_number;
		phone_no.is_primary_mobile_no = 1;
		dontmanage.set_route('Form', new_contact.doctype, new_contact.name);
	}
}

$(document).on('app_ready', function () {
	dontmanage.realtime.on('show_call_popup', call_log => {
		let call_popup = dontmanageerp.call_popup;
		if (call_popup && call_log.name === call_popup.call_log.name) {
			call_popup.update_call_log(call_log);
			call_popup.dialog.show();
		} else {
			dontmanageerp.call_popup = new CallPopup(call_log);
		}
	});
});
