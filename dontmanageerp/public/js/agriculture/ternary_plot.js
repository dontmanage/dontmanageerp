dontmanage.provide('agriculture');

agriculture.TernaryPlot = class TernaryPlot {
	constructor(opts) {
		Object.assign(this, opts);

		dontmanage.require('assets/dontmanage/js/lib/snap.svg-min.js', () => {
			this.make_svg();
			this.init_snap();
			this.init_config();
			this.make_plot();
	...
	init_config() {
		this.config = {
			...
			spacing: 50,
			strokeWidth: 1,
			stroke: dontmanage.ui.color.get('black')
		};
		...
		this.coords = {
			sand: {
				...
				color: dontmanage.ui.color.get('peach')
			},
			loamy_sand: {
				points: [
				...
				color: dontmanage.ui.color.get('pink')
			},
			sandy_loam: {
				points: [
				...
				color: dontmanage.ui.color.get('pink', 'light')
			},
			loam: {
				points: [
				...
				color: dontmanage.ui.color.get('brown')
			},
			silt_loam: {
				points: [
				...
				color: dontmanage.ui.color.get('green', 'dark')
			},
			silt: {
				points: [
					s + t - 12.5 * p * Snap.cos(60), s + (100 - 12.5) * p * Snap.cos(30),
				...
				color: dontmanage.ui.color.get('green')
			},
			silty_clay_loam: {
				points: [
				...
				color: dontmanage.ui.color.get('cyan', 'dark')
			},
			silty_clay: {
				points: [
				...
				color: dontmanage.ui.color.get('cyan')
			},
			clay_loam: {
				points: [
					s + t - 40 * p * Snap.cos(60) - 20 * p, s + 60 * p * Snap.cos(30),
				...
				color: dontmanage.ui.color.get('green', 'light')
			},
			sandy_clay_loam: {
				points: [
				...
				color: dontmanage.ui.color.get('pink', 'dark')
			},
			sandy_clay: {
				points: [
				...
				color: dontmanage.ui.color.get('red')
			},
			clay: {
				points: [
					s + t * Snap.cos(60), s,
				...
				color: dontmanage.ui.color.get('yellow')
			},
		};
	}
	...
	make_plot_marking() {
		...
		let clay = this.paper.text(t * Snap.cos(60) / 2, s + t * Snap.cos(30) / 2, __("Clay")).attr({
			fill: dontmanage.ui.color.get('black')
		});
		clay.transform("r300");
	...
	mark_blip({clay, sand, silt} = this) {
		if (clay + sand + silt != 0){
			...
			this.blip = this.paper.circle(x_blip, y_blip, 4).attr({
				fill: dontmanage.ui.color.get("orange"),
				stroke: dontmanage.ui.color.get("orange"),
				strokeWidth: 2
			});
		}
	}
