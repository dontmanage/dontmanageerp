dontmanage.provide("dontmanageerp.e_commerce.wishlist");
var wishlist = dontmanageerp.e_commerce.wishlist;
...
$.extend(wishlist, {
	set_wishlist_count: function(animate=false) {
		...
		var wish_count = dontmanage.get_cookie("wish_count");
	...
	bind_remove_action: function() {
		...
		$('.page_content').on("click", ".remove-wish", (e) => {
			...
			let success_action = function() {
				...
				$card_wrapper.addClass("wish-removed");
				if (dontmanage.get_cookie("wish_count") == 0) {
					$(".page_content").empty();
	...
	wishlist_action(btn) {
		...
		let success_action = function() {
			dontmanageerp.e_commerce.wishlist.set_wishlist_count(true);
		};
	...
	add_remove_from_wishlist(action, args, success_action, failure_action, async=false) {
		...
		if (dontmanage.session.user==="Guest") {
			...
			let method = "dontmanageerp.e_commerce.doctype.wishlist.wishlist.add_to_wishlist";
			if (action === "remove") {
				method = "dontmanageerp.e_commerce.doctype.wishlist.wishlist.remove_from_wishlist";
			}
	...
	render_empty_state() {
		$(".page_content").append(`
			<div class="cart-empty dontmanage-card">
				<div class="cart-empty-state">
					<img src="/assets/dontmanageerp/images/ui-states/cart-empty-state.png" alt="Empty Cart">
				</div>
