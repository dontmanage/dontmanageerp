dontmanage.ui.form.ControlData = class ControlData extends dontmanage.ui.form.ControlData  {
	make_input() {
		super.make_input();
	...
	setup_phone() {
		if (dontmanage.phone_call.handler) {
			let control = this.df.read_only ? '.control-value' : '.control-input';
			this.$wrapper.find(control)
				.append(`
					<span class="phone-btn">
						<a class="btn-open no-decoration" title="${__('Make a call')}">
							${dontmanage.utils.icon('call')}
					</span>
				`)
				.find('.phone-btn')
				.click(() => {
					dontmanage.phone_call.handler(this.get_value(), this.frm);
				});
		}
	}
};
