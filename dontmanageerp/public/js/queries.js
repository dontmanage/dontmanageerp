...
$.extend(dontmanageerp.queries, {
	...
	customer_filter: function (doc) {
		if (!doc.customer) {
			dontmanage.throw(
				__("Please set {0}", [__(dontmanage.meta.get_label(doc.doctype, "customer", doc.name))])
			);
		}
	...
	company_contact_query: function (doc) {
		if (!doc.company) {
			dontmanage.throw(__("Please set {0}", [__(dontmanage.meta.get_label(doc.doctype, "company", doc.name))]));
		}
	...
	supplier_filter: function (doc) {
		if (!doc.supplier) {
			dontmanage.throw(
				__("Please set {0}", [__(dontmanage.meta.get_label(doc.doctype, "supplier", doc.name))])
			);
		}
	...
	lead_filter: function (doc) {
		if (!doc.lead) {
			dontmanage.throw(
				__("Please specify a {0}", [__(dontmanage.meta.get_label(doc.doctype, "lead", doc.name))])
			);
		}
...
dontmanageerp.queries.setup_queries = function (frm, options, query_fn) {
	...
	$.each(
		dontmanage.meta.get_docfields(frm.doc.doctype, frm.doc.name, { fieldtype: "Table" }),
		function (i, df) {
