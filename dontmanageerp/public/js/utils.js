...
$.extend(dontmanageerp, {
	...
	setup_serial_or_batch_no: function() {
		...
		dontmanage.model.get_value('Item', {'name': grid_row.doc.item_code},
	...
	**/
	...
	check_payments_app: () => {
		if (dontmanage.boot.versions && !dontmanage.boot.versions.payments) {
			const marketplace_link = '<a href="https://dontmanagecloud.com/marketplace/apps/payments">Marketplace</a>'
		...
		if($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			if(already_set) {
				opts.source_name.forEach(function(src) {
					...
					if(already_set) {
						dontmanage.msgprint(__("You have already selected items from {0} {1}",
