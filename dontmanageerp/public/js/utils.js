...
dontmanage.provide("dontmanageerp.stock.utils");

$.extend(dontmanageerp, {
	get_currency: function (company) {
		...
		else return dontmanage.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = dontmanage.boot.docs;
	...
	stale_rate_allowed: () => {
		return cint(dontmanage.boot.sysdefaults.allow_stale);
	},
	...
	route_to_adjustment_jv: (args) => {
		dontmanage.model.with_doctype("Journal Entry", () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = dontmanage.model.get_new_doc("Journal Entry");

			args.accounts.forEach((je_account) => {
				let child_row = dontmanage.model.add_child(journal_entry, "accounts");
			...
			dontmanage.set_route("Form", "Journal Entry", journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		dontmanage.set_route("List", "Repost Item Valuation", args);
	},
});

$.extend(dontmanageerp.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					dontmanageerp.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return dontmanage.call({
				method: "dontmanageerp.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	add_dimensions: function (report_name, index) {
		...
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
					...
					if (!found) {
						filters.splice(index, 0, {
							...
							get_data: function (txt) {
								return dontmanage.db.get_link_options(dimension["document_type"], txt);
							},
						});
					}
				});
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
					...
					r.message.forEach((dimension) => {
						...
						if (!existing_filter.length) {
							filters.splice(index, 0, {
								...
								fieldtype: "MultiSelectList",
								get_data: function (txt) {
									return dontmanage.db.get_link_options(dimension["doctype"], txt);
								},
							});
						} else {
	...
	make_pricing_rule: function (doctype, docname) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_tree_options: function (option) {
		...
		let unscrub_option = dontmanage.model.unscrub(option);
		let user_permission = dontmanage.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function (option) {
		...
		if (options.includes(dontmanage.defaults.get_default(option))) {
			return dontmanage.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function (doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			dontmanage.refresh_field(table_fieldname);
		}
	},
	create_new_doc: function (doctype, update_fields) {
		dontmanage.model.with_doctype(doctype, function () {
			var new_doc = dontmanage.model.get_new_doc(doctype);
	...
	check_payments_app: () => {
		if (dontmanage.boot.versions && !dontmanage.boot.versions.payments) {
			const marketplace_link =
				'<a href="https://dontmanagecloud.com/marketplace/apps/payments">Marketplace</a>';
			const github_link = '<a href="https://github.com/dontmanage/payments/">GitHub</a>';
	...
	pick_serial_and_batch_bundle(frm, cdt, cdn, type_of_transaction, warehouse_field) {
		...
		dontmanage.db.get_value("Item", item_row.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
	...
	const dialog = new dontmanage.ui.Dialog({
					...
					},
					{
						...
						onchange: function () {
							...
							if (item_code && warehouse) {
								dontmanage.call({
									method: "dontmanageerp.stock.utils.get_latest_stock_qty",
									args: {
										item_code: item_code,
										warehouse: warehouse,
									},
						...
						get_query: (e) => {
							return {
								query: "dontmanageerp.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
		...
		primary_action: function () {
			...
			alternative_items.forEach((d) => {
				let row = dontmanage.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				dontmanage.model.set_value(row.doctype, row.name, "qty", qty);
				dontmanage.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
dontmanageerp.utils.update_child_items = function (opts) {
	const frm = opts.frm;
...
dontmanageerp.utils.map_current_doc = function (opts) {
	function _map() {
		if ($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = dontmanage.meta.get_docfield(cur_frm.doctype, "items").options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			dontmanage.get_meta(items_doctype).fields.forEach(function (d) {
			...
			if (already_set) {
				opts.source_name.forEach(function (src) {
					dontmanage.model.with_doc(opts.source_doctype, src, function (r) {
						var source_doc = dontmanage.model.get_doc(opts.source_doctype, src);
	...
	if (opts.source_doctype) {
		...
		if (["Purchase Receipt", "Delivery Note"].includes(opts.source_doctype)) {
			let target_meta = dontmanage.get_meta(cur_frm.doc.doctype);
		...
		const d = new dontmanage.ui.form.MultiSelectDialog({
			...
			action: function (selections, args) {
				...
				if (values.length === 0) {
					dontmanage.msgprint(__("Please select {0}", [opts.source_doctype]));
					return;
				}
...
dontmanage.form.link_formatters["Item"] = function (value, doc) {
...
dontmanage.form.link_formatters["Employee"] = function (value, doc) {
...
dontmanage.form.link_formatters["Project"] = function (value, doc) {
...
$(document).on("app_ready", function () {
	if (!dontmanage.datetime.is_timezone_same()) {
		$.each(
			[
			...
			function (i, d) {
				dontmanage.ui.form.on(d, "onload", function (frm) {
					cur_frm.set_df_property("posting_time", "description", dontmanage.sys_defaults.time_zone);
				});
			}
		);
	}
...
$.extend(dontmanageerp.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new dontmanageerp.utils.BarcodeScanner({ frm: frm });
