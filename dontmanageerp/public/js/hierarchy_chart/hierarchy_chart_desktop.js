...
dontmanageerp.HierarchyChart = class {
	/* Options:
		- doctype
	...
	*/
	...
	make_node_element(node) {
		let node_card = dontmanage.render_template('node_card', {
			id: node.id,
			name: node.name,
			title: node.title,
			image: node.image,
		...
		let company = this.page.add_field({
			...
			change: () => {
				...
				if (company.get_value()) {
					...
					dontmanage.throw(__('Please select a company first.'));
				}
			}
		});
	...
	export_chart() {
		dontmanage.dom.freeze(__('Exporting...'));
		this.page.main.css({
			'min-height': '',
			'max-height': '',
			'overflow': 'visible',
	...
	get_all_nodes() {
		...
		return new Promise(resolve => {
			dontmanage.call({
				method: 'dontmanageerp.utilities.hierarchy_chart.get_all_nodes',
				args: {
					method: me.method,
					company: me.company
				},
	...
	setup_edit_node_action(node) {
		...
		node_element.find('.btn-edit-node').click(function() {
			dontmanage.set_route('Form', me.doctype, node.id);
		});
	}
