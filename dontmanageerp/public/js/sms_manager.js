...
dontmanageerp.SMSManager = function SMSManager(doc) {
	var me = this;
	this.setup = function() {
		var default_msg = {
			'Lead'				: '',
	...
	this.get_contact_number = function(contact, ref_doctype, ref_name) {
		dontmanage.call({
			method: "dontmanage.core.doctype.sms_settings.sms_settings.get_contact_number",
			args: {
			...
			callback: function(r) {
				if(r.exc) { dontmanage.msgprint(r.exc); return; }
				me.number = r.message;
				me.show_dialog();
			}
		});
	...
	this.make_dialog = function() {
		...
		d.fields_dict.send.input.onclick = function() {
			...
			if(v) {
				$(btn).set_working();
				dontmanage.call({
					method: "dontmanage.core.doctype.sms_settings.sms_settings.send_sms",
					args: {
