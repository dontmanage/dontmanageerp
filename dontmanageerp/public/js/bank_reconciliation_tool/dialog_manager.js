dontmanage.provide("dontmanageerp.accounts.bank_reconciliation");

dontmanageerp.accounts.bank_reconciliation.DialogManager = class DialogManager {
	constructor(
		company,
		bank_account,
	...
	get_linked_vouchers(document_types) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_linked_payments",
			args: {
	...
	get_dt_columns() {
		this.columns = [
			...
			},
			{
				...
				width: 1,
				format: (value, row) => {
					return dontmanage.form.formatters.Link(value, { options: row[2].content });
				},
			},
			{
	...
	get_datatable(proposals_wrapper) {
		if (!this.datatable) {
			...
			this.datatable = new dontmanage.DataTable(proposals_wrapper.get(0), datatable_options);
		} else {
	...
	make_dialog() {
		...
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_transaction.bank_transaction.get_doctypes_for_bank_reconciliation",
			callback: (r) => {
				$.each(r.message, (_i, entry) => {
					...
					fields.push({
						fieldtype: "Check",
						label: entry,
						fieldname: dontmanage.scrub(entry),
	...
	match() {
		...
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.reconcile_vouchers",
			args: {
			...
			callback: (response) => {
				...
				dontmanage.show_alert(alert_string);
	...
	add_payment_entry(values) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_payment_entry_bts",
			args: {
				bank_transaction_name: this.bank_transaction.name,
	...
	add_journal_entry(values) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_journal_entry_bts",
			args: {
				bank_transaction_name: this.bank_transaction.name,
				reference_number: values.reference_number,
				reference_date: values.reference_date,
	...
	update_transaction(values) {
		dontmanage.call({
			method: "dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.update_bank_transaction",
			args: {
				bank_transaction_name: this.bank_transaction.name,
				reference_number: values.reference_number,
				party_type: values.party_type,
