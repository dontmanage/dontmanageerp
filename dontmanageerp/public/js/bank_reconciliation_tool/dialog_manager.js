...
dontmanageerp.accounts.bank_reconciliation.DialogManager = class DialogManager {
	...
	get_linked_vouchers(document_types) {
		dontmanage.call({
			method:
				"dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_linked_payments",
			args: {
	...
	get_datatable(proposals_wrapper) {
		if (!this.datatable) {
			...
			this.datatable = new dontmanage.DataTable(
				...
				get_query: function () {
					return {
						filters: {
							name: [
								"in",
								Object.keys(dontmanage.boot.party_account_types),
							],
						},
					};
				},
	...
	match() {
		...
		dontmanage.call({
			method:
				"dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.reconcile_vouchers",
			args: {
	...
	add_payment_entry(values) {
		dontmanage.call({
			method:
				"dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_payment_entry_bts",
			args: {
				bank_transaction_name: this.bank_transaction.name,
	...
	add_journal_entry(values) {
		dontmanage.call({
			method:
				"dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_journal_entry_bts",
			args: {
				bank_transaction_name: this.bank_transaction.name,
				reference_number: values.reference_number,
				reference_date: values.reference_date,
	...
	update_transaction(values) {
		dontmanage.call({
			method:
				"dontmanageerp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.update_bank_transaction",
			args: {
				bank_transaction_name: this.bank_transaction.name,
				reference_number: values.reference_number,
				party_type: values.party_type,
