...
dontmanageerp.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new dontmanageerp.accounts.NumberCard(summary);
...
dontmanageerp.accounts.NumberCard = class NumberCard {
	constructor(options) {
		this.$card = dontmanage.utils.build_summary_item(options);
	}
