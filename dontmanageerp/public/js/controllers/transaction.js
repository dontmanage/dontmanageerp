...
dontmanageerp.TransactionController = class TransactionController extends dontmanageerp.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
		...
		dontmanage.flags.hide_serial_batch_dialog = true;
		dontmanage.ui.form.on(this.frm.doctype + " Item", "rate", function(frm, cdt, cdn) {
			var item = dontmanage.get_doc(cdt, cdn);
			var has_margin_field = dontmanage.meta.has_field(cdt, 'margin_type');

			dontmanage.model.round_floats_in(item, ["rate", "price_list_rate"]);
		...
		dontmanage.ui.form.on(this.frm.cscript.tax_table, "rate", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		dontmanage.ui.form.on(this.frm.cscript.tax_table, "tax_amount", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		dontmanage.ui.form.on(this.frm.cscript.tax_table, "row_id", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		dontmanage.ui.form.on(this.frm.cscript.tax_table, "included_in_print_rate", function(frm, cdt, cdn) {
			cur_frm.cscript.set_dynamic_labels();
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		dontmanage.ui.form.on(this.frm.doctype, "apply_discount_on", function(frm) {
		...
		dontmanage.ui.form.on(this.frm.doctype, "additional_discount_percentage", function(frm) {
			if(!frm.doc.apply_discount_on) {
				dontmanage.msgprint(__("Please set 'Apply Additional Discount On'"));
				return;
			}
			...
			var total = flt(frm.doc[dontmanage.model.scrub(frm.doc.apply_discount_on)]);
		...
		dontmanage.ui.form.on(this.frm.doctype, "discount_amount", function(frm) {
		...
		dontmanage.ui.form.on(this.frm.doctype + " Item", {
			items_add: function(frm, cdt, cdn) {
				...
				if (item.docstatus === 0
					&& dontmanage.meta.has_field(item.doctype, "use_serial_batch_fields")
				...
				dontmanageerp.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, 'items');
			}
		});
		...
		if(dontmanage.meta.get_docfield(this.frm.doc.doctype, "pricing_rules")) {
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "dontmanageerp.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	toggle_enable_for_stock_uom(field) {
		dontmanage.call({
			method: 'dontmanageerp.stock.doctype.stock_settings.stock_settings.get_enable_stock_uom_editing',
			callback: (r) => {
				if (r.message) {
	...
	onload() {
		...
		if(this.frm.doc.__islocal) {
			var currency = dontmanage.defaults.get_user_default("currency");
	...
	setup_quality_inspection() {
		...
		const me = this;
		if (!this.frm.is_new() && this.frm.doc.docstatus === 0 && dontmanage.model.can_create("Quality Inspection")) {
	...
	make_payment_request() {
		...
		dontmanage.call({
			method:"dontmanageerp.accounts.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	onload_post_render() {
		if(this.frm.doc.__islocal && !(this.frm.doc.taxes || []).length
			...
			dontmanage.after_ajax(() => this.apply_default_taxes());
		} else if(this.frm.doc.__islocal && this.frm.doc.company && this.frm.doc["items"]
			&& !this.frm.doc.is_pos) {
			dontmanage.after_ajax(() => this.calculate_taxes_and_totals());
		}
		if(dontmanage.meta.get_docfield(this.frm.doc.doctype + " Item", "item_code")) {
	...
	barcode(doc, cdt, cdn)  {
		...
		if (row.barcode) {
			dontmanageerp.stock.utils.set_item_details_using_barcode(this.frm, row, (r) => {
				dontmanage.model.set_value(cdt, cdn, {
	...
	apply_default_taxes() {
		var me = this;
		var taxes_and_charges_field = dontmanage.meta.get_docfield(me.frm.doc.doctype, "taxes_and_charges",
			me.frm.doc.name);
		...
		if (taxes_and_charges_field) {
			return dontmanage.call({
				method: "dontmanageerp.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	setup_sms() {
		...
		if(dontmanage.boot.sms_gateway_enabled && this.frm.doc.docstatus===1 && !["Lost", "Stopped", "Closed"].includes(this.frm.doc.status)
	...
	send_sms() {
		var sms_man = new dontmanageerp.SMSManager(this.frm.doc);
	}
		...
		if(item.item_code || item.serial_no) {
			if(!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "dontmanageerp.stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						args: {
					...
					callback: function(r) {
						if(!r.exc) {
							dontmanage.run_serially([
								...
								() => {
									if (show_batch_dialog && !dontmanage.flags.trigger_from_barcode_scanner)
										return dontmanage.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"])
											.then((r) => {
												if (r.message &&
								...
								() => {
									...
									if (show_batch_dialog && !dontmanage.flags.hide_serial_batch_dialog)
										return dontmanage.db.get_single_value('Stock Settings', 'disable_serial_no_and_batch_selector')
											.then((value) => {
												if (value) {
													dontmanage.flags.hide_serial_batch_dialog = true;
												}
								...
								() => {
									if(show_batch_dialog && !dontmanage.flags.hide_serial_batch_dialog && !dontmanage.flags.dialog_set) {
										var d = locals[cdt][cdn];
										...
										dontmanageerp.show_serial_batch_selector(me.frm, d, (item) => {
											me.frm.script_manager.trigger('qty', item.doctype, item.name);
											if (!me.frm.doc.set_warehouse)
												me.frm.script_manager.trigger('warehouse', item.doctype, item.name);
											me.apply_price_list(item, true);
										}, undefined, !dontmanage.flags.hide_serial_batch_dialog);
									} else {
										dontmanage.flags.dialog_set = false;
									}
								},
	...
	price_list_rate(doc, cdt, cdn) {
		var item = dontmanage.get_doc(cdt, cdn);
		dontmanage.model.round_floats_in(item, ["price_list_rate", "discount_percentage"]);
	...
	margin_type(doc, cdt, cdn) {
		...
		let item = dontmanage.get_doc(cdt, cdn);
		if (!item.margin_type) {
			dontmanage.model.set_value(cdt, cdn, "margin_rate_or_amount", 0);
		} else {
	...
	get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
		...
		dontmanage.call({
			method: 'dontmanageerp.stock.utils.get_incoming_rate',
			args: {
				args: item_args
			},
			callback: function(r) {
				dontmanage.model.set_value(item.doctype, item.name, 'rate', r.message * item.conversion_factor);
			}
		});
	}
	...
	add_taxes_from_item_tax_template(item_tax_map) {
		...
		if(item_tax_map && cint(dontmanage.defaults.get_default("add_taxes_from_item_tax_template"))) {
			...
			$.each(item_tax_map, function(tax, rate) {
				...
				if(!found) {
					let child = dontmanage.model.add_child(me.frm.doc, "taxes");
	...
	refresh_serial_batch_bundle_field() {
		dontmanage.route_hooks.after_submit = (frm_obj) => {
			frm_obj.reload_doc();
		}
	}
	...
	update_qty(cdt, cdn) {
		...
		dontmanage.model.set_value(item.doctype, item.name, "stock_qty", valid_serial_nos.length);
	}
	...
	set_default_internal_warehouse() {
		...
		if ((this.frm.doc.doctype === 'Sales Invoice' && me.frm.doc.update_stock)
			...
			if (this.frm.doc.is_internal_customer && this.frm.doc.company === this.frm.doc.represents_company) {
				dontmanage.db.get_value('Company', this.frm.doc.company, 'default_in_transit_warehouse', function(value) {
	...
	company() {
		...
		var get_party_currency = function() {
			...
			var party_type = dontmanage.meta.has_field(me.frm.doc.doctype, "customer") ? "Customer" : "Supplier";
		...
		var set_terms = function() {
			if (dontmanage.meta.has_field(me.frm.doc.doctype, "tc_name") && !me.frm.doc.tc_name) {
				var company_doc = dontmanage.get_doc(":Company", me.frm.doc.company);
		...
		var set_party_account = function(set_pricing) {
			if (["Sales Invoice", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
				...
				var party = me.frm.doc[dontmanage.model.scrub(party_type)];
				if(party && me.frm.doc.company) {
					return dontmanage.call({
						method: "dontmanageerp.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
		...
		if (dontmanage.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			['Purchase Order', 'Purchase Receipt', 'Purchase Invoice'].includes(this.frm.doctype)) {
				let is_drop_ship = me.frm.doc.items.some(item => item.delivered_by_supplier);

				if (!is_drop_ship) {
					dontmanageerp.utils.get_shipping_address(this.frm, function() {
		...
		if(this.frm.doc.company) {
			dontmanageerp.last_selected_company = this.frm.doc.company;
		}
	}
	...
	transaction_date() {
		...
		if (this.frm.doc.transaction_date) {
			...
			dontmanage.ui.form.trigger(this.frm.doc.doctype, "currency");
		}
	}
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if ((this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer) ||
				...
				return dontmanage.call({
					method: "dontmanageerp.accounts.party.get_due_date",
					args: {
					...
					callback: function(r, rt) {
						if(r.message) {
							...
							dontmanage.ui.form.trigger(me.frm.doc.doctype, "currency");
	...
	due_date() {
		...
		if (this.frm.doc.due_date && !this.frm.updating_party_details && !this.frm.doc.is_pos) {
			if (this.frm.doc.payment_terms_template ||
				...
				dontmanage.msgprint(final_message);
			}
		}
	}
	...
	get_company_currency() {
		return dontmanageerp.get_currency(this.frm.doc.company);
	}

	contact_person() {
		dontmanageerp.utils.get_contact_details(this.frm);
	}
	...
	conversion_rate() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", dontmanageerp.stale_rate_allowed() ? 0 : 1);
	}
		...
		if (in_list(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "Purchase Invoice", "Purchase Order", "Purchase Receipt"]), this.frm.doc.doctype) {
			...
			$.each(this.frm.doc.items || [], function(i, d) {
				if(d.margin_type == "Amount") {
					dontmanage.model.set_value(d.doctype, d.name, "margin_rate_or_amount",
	...
	set_actual_charges_based_on_currency(exchange_rate) {
		...
		$.each(this.frm.doc.taxes || [], function(i, d) {
			if(d.charge_type == "Actual") {
				dontmanage.model.set_value(d.doctype, d.name, "tax_amount",
	...
	uom(doc, cdt, cdn) {
		...
		if(item.item_code && item.uom) {
			return this.frm.call({
				method: "dontmanageerp.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom
				},
				callback: function(r) {
					if(!r.exc) {
						dontmanage.model.set_value(cdt, cdn, 'conversion_factor', r.message.conversion_factor);
						me.apply_price_list(item, true);
					}
				}
			});
	...
	conversion_factor(doc, cdt, cdn, dont_fetch_price_list_rate) {
		if(dontmanage.meta.get_docfield(cdt, "stock_qty", cdn)) {
			...
			if(dontmanage.flags.dont_fetch_price_list_rate) {
				return
			}

			if (!dont_fetch_price_list_rate &&
				dontmanage.meta.has_field(doc.doctype, "price_list_currency")) {
	...
	toggle_conversion_factor(item) {
		...
		if(this.frm.get_field('items').grid.fields_map.conversion_factor) {
			this.frm.fields_dict.items.grid.toggle_enable("conversion_factor",
				((item.uom != item.stock_uom) && !dontmanage.meta.get_docfield(cur_frm.fields_dict.items.grid.doctype, "conversion_factor").read_only)? true: false);
		}
	}
	...
	service_stop_date(frm, cdt, cdn) {
		...
		if(child.service_stop_date) {
			...
			if(stop_date < start_date) {
				dontmanage.model.set_value(cdt, cdn, "service_stop_date", "");
				dontmanage.throw(__("Service Stop Date cannot be before Service Start Date"));
			} else if (stop_date > end_date) {
				dontmanage.model.set_value(cdt, cdn, "service_stop_date", "");
				dontmanage.throw(__("Service Stop Date cannot be after Service End Date"));
			}
		}
	}
	...
	service_start_date(frm, cdt, cdn) {
		...
		if(child.service_start_date) {
			dontmanage.call({
				"method": "dontmanageerp.stock.get_item_details.calculate_service_end_date",
				args: {"args": child},
				callback: function(r) {
					dontmanage.model.set_value(cdt, cdn, "service_end_date", r.message.service_end_date);
				}
			})
		}
	}
	...
	change_form_labels(company_currency) {
		...
		if(this.frm.doc.doctype && dontmanage.meta.get_docfield(this.frm.doc.doctype, "net_total")) {
			this.frm.toggle_display("net_total", show);
		}

		if(this.frm.doc.doctype && dontmanage.meta.get_docfield(this.frm.doc.doctype, "base_net_total")) {
	...
	change_grid_labels(company_currency) {
		...
		if (this.frm.doc.operations && this.frm.doc.operations.length > 0) {
			...
			$.each(["base_operating_cost", "base_hour_rate"], function(i, fname) {
				if(dontmanage.meta.get_docfield(item_grid.doctype, fname))
	...
	update_payment_schedule_grid_labels(company_currency) {
		...
		if (this.frm.doc.payment_schedule && this.frm.doc.payment_schedule.length > 0) {
			...
			$.each(["base_payment_amount", "base_outstanding", "base_paid_amount"], function(i, fname) {
				if (dontmanage.meta.get_docfield(schedule_grid.doctype, fname))
	...
	batch_no(frm, cdt, cdn) {
		...
		if (row.use_serial_batch_fields && row.batch_no) {
			...
			dontmanage.call({
				method: "dontmanageerp.stock.get_item_details.get_batch_based_item_price",
				args: {
					params: params,
	...
	ignore_pricing_rule() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "dontmanageerp.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	remove_pricing_rule_for_item(item) {
		...
		if (item.pricing_rules){
			let me = this;
			return this.frm.call({
				method: "dontmanageerp.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rule_for_item",
				args: {
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "dontmanageerp.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	_set_values_for_item_list(children) {
		...
		for (const child of children) {
			const existing_pricing_rule = dontmanage.model.get_value(child.doctype, child.name, "pricing_rules");

			for (const [key, value] of Object.entries(child)) {
				if (!["doctype", "name"].includes(key)) {
					if (key === "price_list_rate") {
						dontmanage.model.set_value(child.doctype, child.name, "rate", value);
					}

					if (key === "pricing_rules") {
						dontmanage.model.set_value(child.doctype, child.name, key, value);
					}
			...
			dontmanage.model.round_floats_in(
				dontmanage.get_doc(child.doctype, child.name),
			...
			if (!this.frm.doc.ignore_pricing_rule && existing_pricing_rule && !child.pricing_rules) {
				this.apply_price_list(dontmanage.get_doc(child.doctype, child.name));
			} else if (!child.pricing_rules) {
				this.remove_pricing_rule(dontmanage.get_doc(child.doctype, child.name));
			}
					...
					if (in_list(JSON.parse(data.apply_rule_on_other_items), d[data.apply_rule_on]) && d.item_code === data.item_code) {
						for(var k in data) {
							...
							if (in_list(fields, k) && data[k] && (data.price_or_product_discount === 'Price' || k === 'pricing_rules')) {
								dontmanage.model.set_value(d.doctype, d.name, k, data[k]);
							}
						}
					}
				});
	...
	apply_product_discount(args) {
		...
		args.free_item_data.forEach(pr_row => {
			...
			if (!items || !exist_items.filter(e_row => {
				...
				row_to_modify = dontmanage.model.add_child(this.frm.doc,
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `dontmanageerp.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "dontmanageerp.stock.get_item_details.apply_price_list",
			args: {	args: args, doc: me.frm.doc },
			callback: function(r) {
				if (!r.exc) {
					dontmanage.run_serially([
	...
	validate_company_and_party() {
		...
		if (dontmanage.flags.ignore_company_party_validation) {
			return valid;
		}

		$.each(["company", "customer"], function(i, fieldname) {
			if(dontmanage.meta.has_field(me.frm.doc.doctype, fieldname) &&  !["Purchase Order","Purchase Invoice"].includes(me.frm.doc.doctype)) {
				if (!me.frm.doc[fieldname]) {
					dontmanage.msgprint(__("Please specify") + ": " +
						dontmanage.meta.get_label(me.frm.doc.doctype, fieldname, me.frm.doc.name) +
	...
	get_terms() {
		...
		dontmanageerp.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
			if(!r.exc) {
	...
	taxes_and_charges() {
		...
		if(this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "dontmanageerp.controllers.accounts_controller.get_taxes_and_charges",
				args: {
					"master_doctype": dontmanage.meta.get_docfield(this.frm.doc.doctype, "taxes_and_charges",
	...
	tax_category() {
		...
		dontmanage.run_serially([
			...
			() => dontmanageerp.utils.set_taxes(this.frm, "tax_category"),
		]);
	}
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "dontmanageerp.stock.get_item_details.get_item_tax_info",
				args: {
					company: me.frm.doc.company,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "dontmanageerp.stock.get_item_details.get_item_tax_map",
				args: {
					company: me.frm.doc.company,
					item_tax_template: item.item_tax_template,
					as_json: true
	...
	is_recurring() {
		...
		if(this.frm.doc.is_recurring && this.frm.doc.__islocal) {
			dontmanage.msgprint(__("Please set recurring after saving"));
		...
		if(this.frm.doc.is_recurring) {
			...
			var owner_email = this.frm.doc.owner=="Administrator"
				? dontmanage.user_info("Administrator").email
				: this.frm.doc.owner;
			...
			this.frm.doc.repeat_on_day_of_month = dontmanage.datetime.str_to_obj(this.frm.doc.posting_date).getDate();
		}
	...
	from_date() {
		...
		if(this.frm.doc.from_date) {
			...
			if(months) {
				var to_date = dontmanage.datetime.add_months(this.frm.doc.from_date,
					months);
				this.frm.doc.to_date = dontmanage.datetime.add_days(to_date, -1);
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new dontmanageerp.ItemSelector({frm: this.frm});
		// }
	}
	...
	make_quality_inspection() {
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			primary_action: function () {
				...
				dontmanage.call({
					method: "dontmanageerp.controllers.stock_controller.make_quality_inspections",
					args: {
						doctype: me.frm.doc.doctype,
					...
					callback: function (r) {
						if (r.message.length > 0) {
							if (r.message.length === 1) {
								dontmanage.set_route("Form", "Quality Inspection", r.message[0]);
							} else {
								dontmanage.route_options = {
									"reference_type": me.frm.doc.doctype,
									"reference_name": me.frm.doc.name
								};
								dontmanage.set_route("List", "Quality Inspection");
							}
						}
						dialog.hide();
					}
		...
		if (!data.length) {
			dontmanage.msgprint(__("All items in this document already have a linked Quality Inspection."));
		} else {
			dialog.show();
		}
	}
	...
	get_method_for_payment() {
		var method = "dontmanageerp.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if(cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry){
			if(['Sales Invoice', 'Purchase Invoice'].includes( cur_frm.doc.doctype)){
				method = "dontmanageerp.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "dontmanageerp.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			dontmanage.throw(__("Please enter Item Code to get batch no"));
			...
			let filters = {
				'item_code': item.item_code,
				'posting_date': me.frm.doc.posting_date || dontmanage.datetime.nowdate(),
			}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "dontmanageerp.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	}
	...
	payment_terms_template() {
		...
		if(doc.payment_terms_template && doc.doctype !== 'Delivery Note' && !doc.is_return) {
			...
			dontmanage.call({
				method: "dontmanageerp.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if(row.payment_term) {
			dontmanage.call({
				method: "dontmanageerp.controllers.accounts_controller.get_payment_term_details",
				args: {
				...
				callback: function(r) {
					if(r.message && !r.exc) {
						for (var d in r.message) {
							dontmanage.model.set_value(cdt, cdn, d, r.message[d]);
							const company_currency = me.get_company_currency();
							me.update_payment_schedule_grid_labels(company_currency);
						}
					}
	...
	against_blanket_order(doc, cdt, cdn) {
		...
		if(!item.against_blanket_order) {
			dontmanage.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order", null);
			dontmanage.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order_rate", 0.00);
		}
	}
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype=="Sales Order" || item.parenttype=="Purchase Order")) {
			dontmanage.call({
				method: "dontmanageerp.stock.get_item_details.get_blanket_order_details",
				args: {
					args:{
						item_code: item.item_code,
				...
				callback: function(r) {
					if (!r.message) {
						dontmanage.throw(__("Invalid Blanket Order for the selected Customer and Item"));
					} else {
						dontmanage.run_serially([
							() => dontmanage.model.set_value(cdt, cdn, "blanket_order_rate", r.message.blanket_order_rate),
							() => me.frm.script_manager.trigger("price_list_rate", cdt, cdn)
						]);
					}
				}
	...
	autofill_warehouse(child_table, warehouse_field, warehouse) {
		if (warehouse && child_table && child_table.length) {
			...
			$.each(child_table || [], function(i, item) {
				dontmanage.model.set_value(doctype, item.name, warehouse_field, warehouse);
			});
		}
	}
...
dontmanageerp.show_serial_batch_selector = function (frm, item_row, callback, on_close, show_dialog) {
...
dontmanageerp.apply_putaway_rule = (frm, purpose=null) => {
	if (!frm.doc.company) {
		dontmanage.throw({message: __("Please select a Company first."), title: __("Mandatory")});
	}
	if (!frm.doc.items.length) return;

	dontmanage.call({
		method: "dontmanageerp.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
