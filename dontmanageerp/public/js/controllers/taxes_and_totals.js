...
dontmanageerp.taxes_and_totals = class TaxesAndTotals extends dontmanageerp.payments {
	setup() {
	...
	apply_pricing_rule_on_item(item) {
		...
		dontmanage.model.set_value(item.doctype, item.name, "rate", item_rate);
	}
	...
	calculate_discount_amount() {
		if (dontmanage.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate() {
		...
		var conversion_rate_label = dontmanage.meta.get_label(this.frm.doc.doctype, "conversion_rate",
			this.frm.doc.name);
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			if (!this.discount_amount_applied) {
				...
				dontmanageerp.accounts.taxes.validate_inclusive_tax(tax, this.frm);
			}
	...
	fetch_round_off_accounts() {
		...
		if (me.frm.doc.company) {
			dontmanage.call({
				...
				"args": {
					...
					"account_list": dontmanage.flags.round_off_applicable_accounts
				},
				callback(r) {
					if (r.message) {
						dontmanage.flags.round_off_applicable_accounts.push(...r.message);
					}
				}
			});
		}
		...
		dontmanage.call({
			method: "dontmanageerp.controllers.taxes_and_totals.get_rounding_tax_settings",
			callback: function(r) {
				dontmanage.flags.round_off_settings = r.message;
			}
		});
	}
	...
	calculate_net_total() {
		...
		dontmanage.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
	}
	...
	calculate_shipping_charges() {
		...
		dontmanage.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
		if (dontmanage.meta.get_docfield(this.frm.doc.doctype, "shipping_rule", this.frm.doc.name)) {
	...
	round_off_totals(tax) {
		if (dontmanage.flags.round_off_applicable_accounts.includes(tax.account_head)) {
	...
	*/
	...
	adjust_grand_total_for_inclusive_tax() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = dontmanage.utils.sum($.map(this.frm.doc.taxes || [],
					function(d) {
	...
	calculate_totals() {
		...
		if(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "POS Invoice"].includes(this.frm.doc.doctype)) {
			...
			if(tax_count) {
				...
				dontmanage.model.round_floats_in(this.frm.doc,
		...
		dontmanage.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total() {
		...
		if(dontmanage.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (dontmanage.sys_defaults.disable_rounded_total) {
			disable_rounded_total = dontmanage.sys_defaults.disable_rounded_total;
		}
		...
		if(dontmanage.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup() {
		...
		if(this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if(!dontmanage.meta.get_docfield(this.frm.doc["taxes"][0].doctype, "tax_amount_after_discount_amount", this.frm.doctype)) {
	...
	set_discount_amount() {
		if(this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(flt(this.frm.doc[dontmanage.scrub(this.frm.doc.apply_discount_on)])
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			if(!this.frm.doc.apply_discount_on)
				dontmanage.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance(update_paid_amount) {
		var total_allocated_amount = dontmanage.utils.sum($.map(this.frm.doc["advances"] || [], function(adv) {
	...
	calculate_outstanding_amount(update_paid_amount) {
		...
		dontmanage.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if(["Sales Invoice", "POS Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
			...
			dontmanage.model.round_floats_in(this.frm.doc, ["paid_amount"]);
		...
		if(this.frm.doc.is_pos && (update_paid_amount===undefined || update_paid_amount)) {
			$.each(this.frm.doc['payments'] || [], function(index, data) {
				if(data.default && payment_status && total_amount_to_pay > 0) {
					...
					dontmanage.model.set_value(data.doctype, data.name, "base_amount", base_amount);
					dontmanage.model.set_value(data.doctype, data.name, "amount", amount);
					payment_status = false;

				} else if(me.frm.doc.paid_amount) {
					dontmanage.model.set_value(data.doctype, data.name, "amount", 0.0);
				}
			});
		}
	}
