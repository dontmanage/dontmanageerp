...
dontmanageerp.StockAnalytics = class StockAnalytics extends dontmanageerp.StockGridReport {
		...
		var args = {
			...
			tree_grid: {
				...
				formatter: function(item) {
					if(!item.is_group) {
						return repl("<a \
							onclick='dontmanage.cur_grid_report.show_stock_ledger(\"%(value)s\")'>\
							%(value)s</a>", {
								value: item.name,
							});
					} else {
	...
	prepare_balances() {
		var me = this;
		var from_date = dontmanage.datetime.str_to_obj(this.from_date);
		var to_date = dontmanage.datetime.str_to_obj(this.to_date);
		var data = dontmanage.report_dump.data["Stock Ledger Entry"];
		...
		for(var i=0, j=data.length; i<j; i++) {
			...
			var posting_datetime = dontmanage.datetime.str_to_obj(sl.posting_datetime);
			...
			if(me.is_default("warehouse") ? true : me.warehouse == sl.warehouse) {
				...
				if(me.value_or_qty!="Quantity") {
					...
					var valuation_method = item.valuation_method ?
						item.valuation_method : dontmanage.sys_defaults.valuation_method;
