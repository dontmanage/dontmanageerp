class BOMConfigurator {
	...
	tree_methods() {
		...
		return {
			...
			toolbar: this.frm?.doc.docstatus === 0 ? [
				{
					label:__(dontmanage.utils.icon('edit', 'sm') + " Qty"),
					click: function(node) {
						let view = dontmanage.views.trees["BOM Configurator"];
				...
				{
					label:__(dontmanage.utils.icon('add', 'sm') + " Raw Material"),
					click: function(node) {
						let view = dontmanage.views.trees["BOM Configurator"];
				...
				{
					label:__(dontmanage.utils.icon('add', 'sm') + " Sub Assembly"),
					click: function(node) {
						let view = dontmanage.views.trees["BOM Configurator"];
				...
				{
					label:__(dontmanage.utils.icon('move', 'sm') + " Sub Assembly"),
					click: function(node) {
						let view = dontmanage.views.trees["BOM Configurator"];
				...
				{
					label:__(dontmanage.utils.icon('delete', 'sm') + __(" Item")),
					click: function(node) {
						let view = dontmanage.views.trees["BOM Configurator"];
	...
	load_tree(response, node) {
		...
		while (node) {
			...
			total_amount = dontmanage.format(
				total_amount, {
					fieldtype: "Currency",
