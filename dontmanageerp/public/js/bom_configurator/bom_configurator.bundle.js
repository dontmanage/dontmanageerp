class BOMConfigurator {
	...
	make() {
		...
		dontmanage.views.trees["BOM Configurator"] = new dontmanage.views.TreeView(options);
		let node = dontmanage.views.trees["BOM Configurator"].tree.root_node;
		dontmanage.views.trees["BOM Configurator"].tree.show_toolbar(node);
		dontmanage.views.trees["BOM Configurator"].tree.load_children(node, true);
		this.tree_view = dontmanage.views.trees["BOM Configurator"];
	}

	bind_events() {
		dontmanage.views.trees["BOM Configurator"].events = {
			frm: this.frm,
	...
	tree_options() {
		return {
			...
			get_tree_nodes: "dontmanageerp.manufacturing.doctype.bom_creator.bom_creator.get_children",
	...
	tree_methods() {
		let frm_obj = this;
		let view = dontmanage.views.trees["BOM Configurator"];
		...
		return {
			...
			onrender(node) {
				...
				amount = dontmanage.format(amount, { fieldtype: "Currency", currency: frm_obj.frm.doc.currency });
			...
			toolbar:
				this.frm?.doc.docstatus === 0
					? [
							{
								label: `${dontmanage.utils.icon("edit", "sm")} ${__("Qty")}`,
								click: function (node) {
									let view = dontmanage.views.trees["BOM Configurator"];
							...
							{
								label: `${dontmanage.utils.icon("add", "sm")} ${__("Raw Material")}`,
								click: function (node) {
									let view = dontmanage.views.trees["BOM Configurator"];
							...
							{
								label: `${dontmanage.utils.icon("add", "sm")} ${__("Sub Assembly")}`,
								click: function (node) {
									let view = dontmanage.views.trees["BOM Configurator"];
							...
							{
								label: `${dontmanage.utils.icon("move", "sm")} ${__("Sub Assembly")}`,
								click: function (node) {
									let view = dontmanage.views.trees["BOM Configurator"];
							...
							{
								label: `${dontmanage.utils.icon("delete", "sm")} ${__("Item")}`,
								click: function (node) {
									let view = dontmanage.views.trees["BOM Configurator"];
	...
	add_item(node, view) {
		dontmanage.prompt(
			...
			(data) => {
				...
				dontmanage.call({
					method: "dontmanageerp.manufacturing.doctype.bom_creator.bom_creator.add_item",
					args: {
	...
	add_sub_assembly(node, view) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.bom_creator.bom_creator.add_sub_assembly",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
					fg_reference_id: node.data.name || this.frm.doc.name,
	...
	delete_node(node, view) {
		dontmanage.confirm(__("Are you sure you want to delete this Item?"), () => {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.bom_creator.bom_creator.delete_node",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
	...
	edit_qty(node, view) {
		...
		dontmanage.prompt(
			...
			(data) => {
				...
				dontmanage.call({
					method: "dontmanageerp.manufacturing.doctype.bom_creator.bom_creator.edit_qty",
					args: {
						doctype: doctype,
						docname: docname,
						qty: data.qty,
	...
	load_tree(response, node) {
		...
		dontmanage.views.trees["BOM Configurator"].tree.load_children(node);
		...
		while (node) {
			...
			total_amount = dontmanage.format(total_amount, {
				fieldtype: "Currency",
...
dontmanage.ui.BOMConfigurator = BOMConfigurator;
