...
dontmanage.pages['setup-wizard'].on_page_load = function(wrapper) {
	if(dontmanage.sys_defaults.company) {
		dontmanage.set_route("desk");
		return;
	}
};

dontmanage.setup.on("before_load", function () {
	dontmanageerp.setup.slides_settings.map(dontmanage.setup.add_slide);
});

dontmanageerp.setup.slides_settings = [
	{
		// Brand
		name: 'brand',
		icon: "fa fa-bookmark",
		...
		bind_events: function (slide) {
			...
			}).val(dontmanage.boot.sysdefaults.company_name || "").trigger("change");

			slide.get_input("company_abbr").on("change", function () {
				if (slide.get_input("company_abbr").val().length > 10) {
					dontmanage.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
		...
		validate: function() {
			if ((this.values.company_name || "").toLowerCase() == "company") {
				dontmanage.msgprint(__("Company Name cannot be Company"));
				return false;
			}
	...
	},
	{
		...
		validate: function () {
			...
			if(me.values.bank_account) {
				dontmanage.call({
					async: false,
					method: "dontmanageerp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
		...
		validate_fy_dates: function() {
			...
			if (invalid || start_greater_than_end) {
				dontmanage.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = dontmanage.wizard.values.country;

			if (country) {
				var fy = dontmanageerp.setup.fiscal_years[country];
				...
				if (year_start_date > dontmanage.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date =
					dontmanage.datetime.add_days(dontmanage.datetime.add_months(start_date, 12), -1);
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new dontmanage.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: 'dontmanageerp.accounts.utils.get_coa',
				args: {
			...
			const buttons = $(form_container).find('.dontmanage-control');
...
dontmanageerp.setup.fiscal_years = {
