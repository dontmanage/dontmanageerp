dontmanageerp.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new dontmanage.ui.Dialog({
			title: 'Email Receipt',
			fields: [
		...
		const print_dialog = new dontmanage.ui.Dialog({
			title: 'Print Receipt',
			fields: [
	...
	print_receipt() {
		...
		dontmanage.utils.print(
			this.doc.doctype,
			this.doc.name,
			frm.pos_print_format,
			this.doc.letter_head,
			this.doc.language || dontmanage.boot.lang
		);
	}
	...
	send_email() {
		...
		dontmanage.call({
			...
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: dontmanage.user.full_name(),
				_lang: doc.language
			},
			callback: r => {
				if (!r.exc) {
					...
					dontmanage.msgprint(__("There were errors while sending email. Please try again."));
				}
			}
		});
	}
	...
	attach_document_info(doc) {
		dontmanage.db.get_value('Customer', this.doc.customer, 'email_id').then(({ message }) => {
