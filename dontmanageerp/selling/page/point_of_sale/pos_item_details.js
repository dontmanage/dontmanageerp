dontmanageerp.PointOfSale.ItemDetails = class {
	...
	async toggle_item_details_section(item) {
		...
		if (item && current_item_changed) {
			...
			this.item_meta = dontmanage.get_meta(this.doctype);
			this.name = item.name;
			this.item_row = item;
		...
		if (!this.hide_images && image) {
			this.$item_image.html(
				`<img
					...
					alt="${dontmanage.get_abbr(item_name)}"
					style="object-fit: cover;">`
			);
		} else {
			this.$item_image.html(`<div class="item-abbr">${dontmanage.get_abbr(item_name)}</div>`);
		}
	...
	render_form(item) {
		...
		fields_to_display.forEach((fieldname, idx) => {
			...
			this[`${fieldname}_control`] = dontmanage.ui.form.make_control({
				df: {
					...field_meta,
					onchange: function() {
	...
	bind_custom_control_change_event() {
			...
			this.rate_control.df.onchange = function() {
				if (this.value || flt(this.value) === 0) {
					me.events.form_updated(me.current_item, 'rate', this.value).then(() => {
						const item_row = dontmanage.get_doc(me.doctype, me.name);
		...
		dontmanage.model.on("POS Invoice Item", "*", (fieldname, value, item_row) => {
	...
	async auto_update_batch_no() {
		if (this.serial_no_control && this.batch_no_control) {
			...
			const serials_with_batch_no = await dontmanage.db.get_list("Serial No", {
	...
	bind_auto_serial_fetch_event() {
		this.$form_container.on('click', '.auto-fetch-btn', () => {
			dontmanage.require("assets/dontmanageerp/js/utils/serial_no_batch_selector.js", () => {
				...
				new dontmanageerp.SerialBatchPackageSelector(frm, item_row, (r) => {
					if (r) {
						dontmanage.model.set_value(item_row.doctype, item_row.name, {
