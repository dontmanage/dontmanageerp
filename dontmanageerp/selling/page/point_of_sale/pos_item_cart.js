dontmanageerp.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new dontmanageerp.PointOfSale.NumberPad({
	...
	bind_events() {
		...
		dontmanage.ui.form.on("POS Invoice", "paid_amount", frm => {
	...
	attach_shortcuts() {
		for (let row of this.number_pad.keys) {
			for (let btn of row) {
				...
				let shortcut_key = `ctrl+${dontmanage.scrub(String(btn))[0]}`;
				...
				const fieldname = this.number_pad.fieldnames[btn] ? this.number_pad.fieldnames[btn] :
					typeof btn === 'string' ? dontmanage.scrub(btn) : btn;

				let shortcut_label = shortcut_key.split('+').map(dontmanage.utils.to_title_case).join('+');
				shortcut_label = dontmanage.utils.is_mac() ? shortcut_label.replace('Ctrl', '⌘') : shortcut_label;
				this.$numpad_section.find(`.numpad-btn[data-button-value="${fieldname}"]`).attr("title", shortcut_label);

				dontmanage.ui.keys.on(`${shortcut_key}`, () => {
		...
		const ctrl_label = dontmanage.utils.is_mac() ? '⌘' : 'Ctrl';
	...
	make_customer_selector() {
		...
		`);
		const me = this;
		const query = { query: 'dontmanageerp.controllers.queries.customer_query' };
		...
		this.customer_field = dontmanage.ui.form.make_control({
			df: {
				label: __('Customer'),
				fieldtype: 'Link',
				options: 'Customer',
				...
				onchange: function() {
					if (this.value) {
						...
						dontmanage.model.set_value(frm.doc.doctype, frm.doc.name, 'customer', this.value);
	...
	fetch_customer_details(customer) {
		if (customer) {
			return new Promise((resolve) => {
				dontmanage.db.get_value('Customer', customer, ["email_id", "mobile_no", "image", "loyalty_program"]).then(({ message }) => {
					const { loyalty_program } = message;
					// if loyalty program then fetch loyalty points too
					if (loyalty_program) {
						dontmanage.call({
							method: "dontmanageerp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details_with_points",
	...
	show_discount_control() {
		...
		this.discount_field = dontmanage.ui.form.make_control({
			df: {
				label: __('Discount'),
				fieldtype: 'Data',
				...
				onchange: function() {
					if (flt(this.value) != 0) {
						dontmanage.model.set_value(frm.doc.doctype, frm.doc.name, 'additional_discount_percentage', flt(this.value));
						me.hide_discount_control(this.value);
					} else {
						dontmanage.model.set_value(frm.doc.doctype, frm.doc.name, 'additional_discount_percentage', 0);
	...
	get_customer_image() {
		...
		if (image) {
			...
			return `<div class="customer-image customer-abbr">${dontmanage.get_abbr(customer)}</div>`;
		}
	}
	...
	update_totals_section(frm) {
		...
		const grand_total = cint(dontmanage.sys_defaults.disable_rounded_total) ? frm.doc.grand_total : frm.doc.rounded_total;
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_description_html() {
			if (item_data.description) {
				...
				item_data.description = dontmanage.ellipsis(item_data.description, 45);
				...
				return `
					<div class="item-image">
						<img
							...
							src="${image}" alt="${dontmanage.get_abbr(item_name)}"">
					</div>`;
			} else {
				return `<div class="item-image item-abbr">${dontmanage.get_abbr(item_name)}</div>`;
			}
		}
	}
	...
	render_customer_fields() {
		...
		const me = this;
		dfs.forEach(df => {
			this[`customer_${df.fieldname}_field`] = dontmanage.ui.form.make_control({
				df: { ...df,
		...
		function handle_customer_field_change() {
			...
			if (this.value && current_value != this.value && this.df.fieldname != 'loyalty_points') {
				dontmanage.call({
					method: 'dontmanageerp.selling.page.point_of_sale.point_of_sale.set_customer_info',
					args: {
	...
	fetch_customer_transactions() {
		dontmanage.db.get_list('POS Invoice', {
