...
from dontmanageerp.accounts.doctype.pos_invoice.pos_invoice import get_stock_availability
from dontmanageerp.accounts.doctype.pos_profile.pos_profile import get_child_nodes, get_item_groups
from dontmanageerp.stock.utils import scan_barcode
...
def filter_result_items(result, pos_profile):
	if result and result.get("items"):
		pos_item_groups = dontmanage.db.get_all("POS Item Group", {"parent": pos_profile}, pluck="item_group")
...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term=""):
	warehouse, hide_unavailable_items = dontmanage.db.get_value(
	...
	if not dontmanage.db.exists("Item Group", item_group):
	...
	current_date = dontmanage.utils.today()

	for item in items_data:
		uoms = dontmanage.get_doc("Item", item.item_code).get("uoms", [])

		item.actual_qty, _ = get_stock_availability(item.item_code, warehouse)
		item.uom = item.stock_uom

		item_price = dontmanage.get_all(
			"Item Price",
...
def get_conditions(search_term):
	...
	condition += """item.name like {search_term}
		or item.item_name like {search_term}""".format(search_term=dontmanage.db.escape("%" + search_term + "%"))
...
def add_search_fields_condition(search_term):
	condition = ""
	search_fields = dontmanage.get_all("POS Search Fields", fields=["fieldname"])
	if search_fields:
		for field in search_fields:
			condition += " or item.`{}` like {}".format(
				field["fieldname"], dontmanage.db.escape("%" + search_term + "%")
			)
	return condition
...
def check_opening_entry(user):
	open_vouchers = dontmanage.db.get_all(
		"POS Opening Entry",
...
def create_opening_voucher(pos_profile, company, balance_details):
	...
	new_pos_opening = dontmanage.get_doc(
		{
			"doctype": "POS Opening Entry",
			"period_start_date": dontmanage.utils.get_datetime(),
			"posting_date": dontmanage.utils.getdate(),
...
def get_past_order_list(search_term, status, limit=20):
	...
	if search_term and status:
		invoices_by_customer = dontmanage.db.get_all(
			"POS Invoice",
			filters={"customer": ["like", f"%{search_term}%"], "status": status},
			fields=fields,
			page_length=limit,
		)
		invoices_by_name = dontmanage.db.get_all(
			"POS Invoice",
		...
		invoice_list = dontmanage.db.get_all(
...
def set_customer_info(fieldname, customer, value=""):
	if fieldname == "loyalty_program":
		dontmanage.db.set_value("Customer", customer, "loyalty_program", value)

	contact = dontmanage.get_cached_value("Customer", customer, "customer_primary_contact")
	if not contact:
		contact = dontmanage.db.sql(
			"""
	...
	if not contact:
		new_contact = dontmanage.new_doc("Contact")
		new_contact.is_primary_contact = 1
		new_contact.first_name = customer
		new_contact.set("links", [{"link_doctype": "Customer", "link_name": customer}])
		new_contact.save()
		contact = new_contact.name
		dontmanage.db.set_value("Customer", customer, "customer_primary_contact", contact)

	contact_doc = dontmanage.get_doc("Contact", contact)
	if fieldname == "email_id":
		contact_doc.set("email_ids", [{"email_id": value, "is_primary": 1}])
		dontmanage.db.set_value("Customer", customer, "email_id", value)
	elif fieldname == "mobile_no":
		contact_doc.set("phone_nos", [{"phone": value, "is_primary_mobile_no": 1}])
		dontmanage.db.set_value("Customer", customer, "mobile_no", value)
	contact_doc.save()


@dontmanage.whitelist()
def get_pos_profile_data(pos_profile):
	pos_profile = dontmanage.get_doc("POS Profile", pos_profile)
