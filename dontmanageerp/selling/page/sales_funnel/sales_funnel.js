...
dontmanage.pages['sales-funnel'].on_page_load = function(wrapper) {
	...
	wrapper.sales_funnel = new dontmanageerp.SalesFunnel(wrapper);

	dontmanage.breadcrumbs.add("Selling");
}

dontmanageerp.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		this.company_field = wrapper.page.add_field({"fieldtype": "Link", "fieldname": "company", "options": "Company",
			"label": __("Company"), "reqd": 1, "default": dontmanage.defaults.get_user_default('company'),
			change: function() {
				me.company = this.value || dontmanage.defaults.get_user_default('company');
				me.get_data();
			}
		}),
		...
		this.company = dontmanage.defaults.get_user_default('company');
		this.options = {
			from_date: dontmanage.datetime.add_months(dontmanage.datetime.get_today(), -1),
			to_date: dontmanage.datetime.get_today(),
			chart: 'sales_funnel'
		};
		...
		$.each(this.options, function(k, v) {
			if (['from_date', 'to_date'].includes(k)) {
				me.elements[k].val(dontmanage.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function() {
				if (['from_date', 'to_date'].includes(k)) {
					me.options[k] = dontmanage.datetime.user_to_str($(this).val()) != 'Invalid date' ? dontmanage.datetime.user_to_str($(this).val()) : dontmanage.datetime.get_today();
				} else {
	...
	get_data(btn) {
		var me = this;
		if (!this.company) {
			dontmanage.throw(__("Please Select a Company."));
		}

		const method_map = {
			"sales_funnel": "dontmanageerp.selling.page.sales_funnel.sales_funnel.get_funnel_data",
			"opp_by_lead_source": "dontmanageerp.selling.page.sales_funnel.sales_funnel.get_opp_by_lead_source",
			"sales_pipeline": "dontmanageerp.selling.page.sales_funnel.sales_funnel.get_pipeline_data"
		};
		dontmanage.call({
