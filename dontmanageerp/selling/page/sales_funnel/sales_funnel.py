...
from dontmanageerp.accounts.report.utils import convert


def validate_filters(from_date, to_date, company):
	if from_date and to_date and (from_date >= to_date):
		dontmanage.throw(_("To Date must be greater than From Date"))
...
def get_funnel_data(from_date, to_date, company):
	...
	active_leads = dontmanage.db.sql(
	...
	opportunities = dontmanage.db.sql(
...
def get_opp_by_lead_source(from_date, to_date, company):
	...
	opportunities = dontmanage.get_all(
		"Opportunity",
		filters=[
	...
	if opportunities:
		default_currency = dontmanage.get_cached_value("Global Defaults", "None", "default_currency")
