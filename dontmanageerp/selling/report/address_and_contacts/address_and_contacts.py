...
def get_columns(filters):
	...
	return [
		...
		"{party_value_type}::150".format(party_value_type=dontmanage.unscrub(str(party_type_value))),
		"Address Line 1",
		"Address Line 2",
		"City",
		"State",
...
def get_party_addresses_and_contact(party_type, party, party_group):
	...
	fetch_party_list = dontmanage.get_list(
	...
	for d in party_list:
		party_details.setdefault(d, dontmanage._dict())
...
def get_party_details(party_type, party_list, doctype, party_details):
	...
	for d in records:
		...
		details.setdefault(dontmanage.scrub(doctype), []).append(d[1:])
