...
def get_data(filters):
	...
	for record in sales_order_records:
		...
		row["currency"] = dontmanage.get_cached_value("Company", row["company"], "default_currency")
		data.append(row)
...
def get_customer_details():
	details = dontmanage.get_all("Customer", fields=["name", "customer_name", "customer_group"])
	customer_details = {}
	for d in details:
		customer_details.setdefault(
			d.name, dontmanage._dict({"customer_name": d.customer_name, "customer_group": d.customer_group})
		)
	return customer_details


def get_item_details():
	details = dontmanage.db.get_all("Item", fields=["name", "item_name", "item_group"])
	item_details = {}
	for d in details:
		item_details.setdefault(d.name, dontmanage._dict({"item_name": d.item_name, "item_group": d.item_group}))
	return item_details


def get_sales_order_details(company_list, filters):
	db_so = dontmanage.qb.DocType("Sales Order")
	db_so_item = dontmanage.qb.DocType("Sales Order Item")

	query = (
		dontmanage.qb.from_(db_so)
