...
from dontmanageerp.accounts.doctype.monthly_distribution.monthly_distribution import (
...
def get_data_column(filters, partner_doctype):
	...
	for key, value in rows.items():
		value.update({dontmanage.scrub(partner_doctype): key[0], "item_group": key[1]})
...
def get_data(filters, period_list, partner_doctype):
	sales_field = dontmanage.scrub(partner_doctype)
...
def get_columns(filters, period_list, partner_doctype):
	...
	columns = [
		{
			"fieldname": dontmanage.scrub(partner_doctype),
...
def get_item_group_parent_child_map():
	...
	item_groups = dontmanage.get_all(
...
def get_actual_data(filters, sales_users_or_territory_data, date_field, sales_field):
	...
	parent_doc = dontmanage.qb.DocType(filters.get("doctype"))
	child_doc = dontmanage.qb.DocType(filters.get("doctype") + " Item")

	query = dontmanage.qb.from_(parent_doc).inner_join(child_doc).on(child_doc.parent == parent_doc.name)
