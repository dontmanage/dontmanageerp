...
from dontmanageerp.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order import (
	execute,
)
...
class TestPaymentTermsStatusForSalesOrder(DontManageTestCase):
	def setUp(self):
	...
	def cleanup_old_entries(self):
		dontmanage.db.delete("Sales Invoice", filters={"company": "_Test Company"})
		dontmanage.db.delete("Sales Order", filters={"company": "_Test Company"})

	def create_payment_terms_template(self):
		# create template for 50-50 payments
		template = None
		if dontmanage.db.exists("Payment Terms Template", "_Test 50-50"):
			template = dontmanage.get_doc("Payment Terms Template", "_Test 50-50")
		else:
			...
			doc = dontmanage.get_doc(
				{
					...
					"date": date,
					"from_currency": "USD",
					"to_currency": dontmanage.get_cached_value("Company", "_Test Company", "default_currency"),
					"exchange_rate": 70,
					"for_buying": True,
					"for_selling": True,
				}
	...
	def test_03_group_filters(self):
		...
		for idx, g in enumerate(group_filters, 0):
			# build filter
			filters = dontmanage._dict({}).update(base_filters).update(g)
