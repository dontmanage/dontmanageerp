...
import dontmanage
from dontmanage import _, qb, query_builder
from dontmanage.query_builder import Criterion, functions
from dontmanage.utils.dateutils import getdate
...
def get_conditions(filters):
	...
	"""
	filters = dontmanage._dict(filters) if filters else dontmanage._dict({})
	conditions = dontmanage._dict({})

	conditions.company = filters.company or dontmanage.defaults.get_user_default("company")
	conditions.end_date = filters.period_end_date or dontmanage.utils.today()
	conditions.start_date = filters.period_start_date or dontmanage.utils.add_months(conditions.end_date, -1)
...
def set_payment_terms_statuses(sales_orders, invoices, filters):
	...
	for so in sales_orders:
		so.currency = dontmanage.get_cached_value("Company", filters.get("company"), "default_currency")
		so.invoices = ""
