...
function get_filters() {
	let filters = [
		...
		},
		{
			...
			"options": "Customer",
			"get_query": () => {
				var customer_group = dontmanage.query_report.get_filter_value('customer_group');
				return{
					"query": "dontmanageerp.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order.get_customers_or_items",
					"filters": [
						['Customer', 'disabled', '=', '0'],
		...
		},
		{
			...
			"options": "Item",
			"get_query": () => {
				var item_group = dontmanage.query_report.get_filter_value('item_group');
				return{
					"query": "dontmanageerp.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order.get_customers_or_items",
					"filters": [
						['Item', 'disabled', '=', '0'],
...
dontmanage.query_reports["Payment Terms Status for Sales Order"] = {
	"filters": get_filters(),
