...
class Analytics:
	...
	def update_company_list_for_parent_company(self):
		...
		if (
			...
			and dontmanage.db.get_value("Company", selected_company, "is_group")
		):
			lft, rgt = dontmanage.db.get_value("Company", selected_company, ["lft", "rgt"])
			child_companies = dontmanage.db.get_list(
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		self.entries = dontmanage.get_all(
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = (
			dontmanage.qb.from_(doctype_item)
			.join(doctype)
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				...
				if d.parent and (self.filters.tree_type != "Order Type" or d.parent == "Order Types"):
					self.entity_periodic_data.setdefault(d.parent, dontmanage._dict()).setdefault(period, 0.0)
	...
	def get_periodic_data(self):
		self.entity_periodic_data = dontmanage._dict()

		for d in self.entries:
			if self.filters.tree_type == "Supplier Group":
				d.entity = self.parent_child_map.get(d.entity)
			period = self.get_period(d.get(self.date_field))
			self.entity_periodic_data.setdefault(d.entity, dontmanage._dict()).setdefault(period, 0.0)
	...
	def get_groups(self):
		...
		self.depth_map = dontmanage._dict()

		self.group_entries = dontmanage.db.sql(
	...
	def get_supplier_parent_child_map(self):
		self.parent_child_map = dontmanage._dict(
			dontmanage.db.sql(""" select name, supplier_group from `tabSupplier`""")
		)
