...
from dontmanageerp.selling.doctype.sales_order.sales_order import make_delivery_note, make_sales_invoice
from dontmanageerp.selling.doctype.sales_order.test_sales_order import make_sales_order
from dontmanageerp.selling.report.sales_order_analysis.sales_order_analysis import execute
...
class TestSalesOrderAnalysis(DontManageTestCase):
	...
	def test_01_so_to_deliver_and_bill(self):
		...
		expected_value = {
			"status": "To Deliver and Bill",
			"sales_order": so.name,
			"delay_days": dontmanage.utils.date_diff(dontmanage.utils.datetime.date.today(), so.delivery_date),
			"qty": 10,
			"delivered_qty": 0,
			"pending_qty": 10,
			"qty_to_bill": 10,
