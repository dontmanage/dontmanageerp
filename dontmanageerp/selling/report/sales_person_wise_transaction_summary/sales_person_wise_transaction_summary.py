	...
	columns = [
		{
			...
			"fieldname": dontmanage.scrub(filters["doc_type"]),
			"fieldtype": "Link",
			"width": 140,
		},
		{
...
def get_conditions(filters, date_field):
	...
	if filters.get("sales_person"):
		lft, rgt = dontmanage.get_value("Sales Person", filters.get("sales_person"), ["lft", "rgt"])
		conditions.append(
...
def get_item_details():
	item_details = {}
	for d in dontmanage.db.sql("""SELECT `name`, `item_group`, `brand` FROM `tabItem`""", as_dict=1):
