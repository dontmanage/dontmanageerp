...
from dontmanage.utils import flt, getdate, nowdate
...
class Quotation(SellingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.selling.doctype.quotation_item.quotation_item import QuotationItem
		from dontmanageerp.setup.doctype.quotation_lost_reason_detail.quotation_lost_reason_detail import (
	...
	def validate_valid_till(self):
		if self.valid_till and getdate(self.valid_till) < getdate(self.transaction_date):
			dontmanage.throw(_("Valid till date cannot be before transaction date"))
	...
	def get_ordered_status(self):
		status = "Open"
		ordered_items = dontmanage._dict(
	...
	def set_customer_name(self):
		if self.party_name and self.quotation_to == "Customer":
			...
			self.customer_name = dontmanage.db.get_value("CRM Deal", self.party_name, "organization")
	...
	def update_opportunity_status(self, status, opportunity=None):
		...
		opp = dontmanage.get_doc("Opportunity", opportunity)
		opp.set_status(status=status, update=True)

	@dontmanage.whitelist()
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not (self.is_fully_ordered() or self.is_partially_ordered()):
			get_lost_reasons = dontmanage.get_list("Quotation Lost Reason", fields=["name"])
				...
				if reason.get("lost_reason") in lost_reasons_lst:
					...
					dontmanage.throw(
						_("Invalid lost reason {0}, please create a new lost reason").format(
							dontmanage.bold(reason.get("lost_reason"))
						)
					)
			...
			dontmanage.throw(_("Cannot set as Lost as Sales Order is made."))
...
def make_sales_order(source_name: str, target_doc=None):
	if not dontmanage.db.get_singles_value(
		"Selling Settings", "allow_sales_order_creation_for_expired_quotation"
	):
		quotation = dontmanage.db.get_value(
			"Quotation", source_name, ["transaction_date", "valid_till"], as_dict=1
		)
		if quotation.valid_till and (
			quotation.valid_till < quotation.transaction_date or quotation.valid_till < getdate(nowdate())
		):
			dontmanage.throw(_("Validity period of this quotation has ended."))
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False):
	...
	selected_rows = [x.get("name") for x in dontmanage.flags.get("args", {}).get("selected_items", [])]
	...
	def set_missing_values(source, target):
		...
		if source.referral_sales_partner:
			...
			target.commission_rate = dontmanage.get_value(
...
def _make_customer(source_name, ignore_permissions=False):
	...
	if quotation.quotation_to == "Customer":
		return dontmanage.get_doc("Customer", quotation.party_name)

	# Check if a Customer already exists for the Lead or Prospect.
	existing_customer = None
	if quotation.quotation_to == "Lead":
		existing_customer = dontmanage.db.get_value("Customer", {"lead_name": quotation.party_name})
	elif quotation.quotation_to == "Prospect":
		existing_customer = dontmanage.db.get_value("Customer", {"prospect_name": quotation.party_name})

	if existing_customer:
		return dontmanage.get_doc("Customer", existing_customer)
...
def create_customer_from_lead(lead_name, ignore_permissions=False):
	from dontmanageerp.crm.doctype.lead.lead import _make_customer
	...
	try:
		customer.insert()
		return customer
	except dontmanage.MandatoryError as e:
		handle_mandatory_error(e, customer, lead_name)


def create_customer_from_prospect(prospect_name, ignore_permissions=False):
	from dontmanageerp.crm.doctype.prospect.prospect import make_customer as make_customer_from_prospect
...
def handle_mandatory_error(e, customer, lead_name):
	...
	dontmanage.throw(message, title=_("Mandatory Missing"))
