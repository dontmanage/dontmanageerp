...
class Quotation(SellingController):
	...
	def validate_shopping_cart_items(self):
		...
		for item in self.items:
			has_web_item = dontmanage.db.exists("Website Item", {"item_code": item.item_code})

			# If variant is unpublished but template is published: valid
			template = dontmanage.get_cached_value("Item", item.item_code, "variant_of")
			if template and not has_web_item:
				has_web_item = dontmanage.db.exists("Website Item", {"item_code": template})
...
def _make_customer(source_name, ignore_permissions=False):
	...
	if quotation and quotation.get("party_name"):
		if not dontmanage.db.exists("Customer", quotation.get("party_name")):
			lead_name = quotation.get("party_name")
			customer_name = dontmanage.db.get_value(
			...
			if not customer_name:
				...
				customer = dontmanage.get_doc(customer_doclist)
				customer.flags.ignore_permissions = ignore_permissions
				if quotation.get("party_name") == "Shopping Cart":
					customer.customer_group = dontmanage.db.get_value(
				...
				try:
					...
					lead_link = dontmanage.utils.get_link_to_form("Lead", lead_name)
					message = (
			...
			return dontmanage.get_doc("Customer", quotation.get("party_name"))
