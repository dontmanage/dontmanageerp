...
dontmanageerp.selling.QuotationController = class QuotationController extends dontmanageerp.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (doc.__islocal && !doc.valid_till) {
			if(dontmanage.boot.sysdefaults.quotation_valid_till){
				this.frm.set_value('valid_till', dontmanage.datetime.add_days(doc.transaction_date, dontmanage.boot.sysdefaults.quotation_valid_till));
			} else {
				this.frm.set_value('valid_till', dontmanage.datetime.add_months(doc.transaction_date, 1));
			}
		}

		if (doc.docstatus == 1 && !["Lost", "Ordered"].includes(doc.status)) {
			if (dontmanage.boot.sysdefaults.allow_sales_order_creation_for_expired_quotation
				|| (!doc.valid_till)
				|| dontmanage.datetime.get_diff(doc.valid_till, dontmanage.datetime.get_today()) >= 0) {
	...
	validate_company_and_party(party_field) {
		if(!this.frm.doc.quotation_to) {
			dontmanage.msgprint(__("Please select a value for {0} quotation_to {1}", [this.frm.doc.doctype, this.frm.doc.name]));
			return false;
		} else if (this.frm.doc.quotation_to == "Lead") {
			return true;
		} else {
	...
	get_lead_details() {
		...
		dontmanage.call({
			method: "dontmanageerp.crm.doctype.lead.lead.get_lead_details",
			args: {
...
cur_frm.script_manager.make(dontmanageerp.selling.QuotationController);

dontmanage.ui.form.on("Quotation Item", "items_on_form_rendered", "packed_items_on_form_rendered", function(frm, cdt, cdn) {
	// enable tax_amount field if Actual
})

dontmanage.ui.form.on("Quotation Item", "stock_balance", function(frm, cdt, cdn) {
	var d = dontmanage.model.get_doc(cdt, cdn);
	dontmanage.route_options = {"item_code": d.item_code};
