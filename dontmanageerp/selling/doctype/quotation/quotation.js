	...
	refresh(doc, dt, dn) {
		...
		if (doc.__islocal && !doc.valid_till) {
			if (dontmanage.boot.sysdefaults.quotation_valid_till) {
				this.frm.set_value(
					"valid_till",
					dontmanage.datetime.add_days(
						doc.transaction_date,
						dontmanage.boot.sysdefaults.quotation_valid_till
					)
				);
			} else {
		...
		if (doc.docstatus == 1 && !["Lost", "Ordered"].includes(doc.status)) {
			if (
				dontmanage.boot.sysdefaults.allow_sales_order_creation_for_expired_quotation ||
				!doc.valid_till ||
