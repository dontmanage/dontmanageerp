...
from dontmanage.utils import add_days, add_months, flt, getdate, nowdate

test_dependencies = ["Product Bundle"]


class TestQuotation(DontManageTestCase):
	...
	def test_make_sales_order_terms_copied(self):
		from dontmanageerp.selling.doctype.quotation.quotation import make_sales_order

		quotation = dontmanage.copy_doc(test_records[0])
	...
	def test_gross_profit(self):
		...
		from dontmanageerp.stock.get_item_details import insert_item_price
		...
		selling_price_list = dontmanage.get_all("Price List", filters={"selling": 1}, limit=1)[0].name
			...
			dontmanage._dict(
				{
					...
					"conversion_factor": 1,
					"discount_amount": 0.0,
					"currency": dontmanage.db.get_value("Price List", selling_price_list, "currency"),
	...
	def test_maintain_rate_in_sales_cycle_is_enforced(self):
		...
		maintain_rate = dontmanage.db.get_single_value("Selling Settings", "maintain_same_sales_rate")
		...
		self.assertRaises(dontmanage.ValidationError, sales_order.save)

		dontmanage.db.set_single_value("Selling Settings", "maintain_same_sales_rate", maintain_rate)
	...
	def test_make_sales_order(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_sales_order, quotation.name)
		quotation.submit()
	...
	def test_valid_till_before_transaction_date(self):
		...
		self.assertRaises(dontmanage.ValidationError, quotation.validate)

	def test_so_from_expired_quotation(self):
		from dontmanageerp.selling.doctype.quotation.quotation import make_sales_order

		dontmanage.db.set_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation", 0)
		...
		dontmanage.db.set_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation", 1)
	...
	def test_quotation_expiry(self):
		from dontmanageerp.selling.doctype.quotation.quotation import set_expired_status
	...
	def test_alternative_items_sales_order_mapping_with_stock_items(self):
		...
		dontmanage.flags.args = dontmanage._dict()
		item_list = []
		stock_items = {
			"_Test Simple Item 1": 100,
			"_Test Alt 1": 120,
		...
		dontmanage.flags.args.selected_items = [quotation.items[2]]
		sales_order = make_sales_order(quotation.name)
	...
	def test_uom_validation(self):
		...
		if not dontmanage.db.exists("UOM", "lbs"):
			dontmanage.get_doc({"doctype": "UOM", "uom_name": "lbs", "must_be_whole_number": 1}).insert()
		else:
			dontmanage.db.set_value("UOM", "lbs", "must_be_whole_number", 1)

		quotation = make_quotation(item_code=item, qty=1, rate=100, do_not_submit=1)
		quotation.items[0].uom = "lbs"
		quotation.items[0].conversion_factor = 2.23
		self.assertRaises(dontmanage.ValidationError, quotation.save)

	def test_item_tax_template_for_quotation(self):
		from dontmanageerp.stock.doctype.item.test_item import make_item

		if not dontmanage.db.exists("Account", {"account_name": "_Test Vat", "company": "_Test Company"}):
			dontmanage.get_doc(
				{
					"doctype": "Account",
		...
		if not dontmanage.db.exists("Item Tax Template", "Vat Template - _TC"):
			dontmanage.get_doc(
				{
...
test_records = dontmanage.get_test_records("Quotation")


def enable_calculate_bundle_price(enable=1):
	selling_settings = dontmanage.get_doc("Selling Settings")
...
def make_quotation(**args):
	qo = dontmanage.new_doc("Quotation")
