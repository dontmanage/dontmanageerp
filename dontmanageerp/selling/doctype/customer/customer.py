...
from dontmanage import _, msgprint, qb
from dontmanage.contacts.address_and_contact import (
	delete_contact_and_address,
	load_address_and_contact,
)
from dontmanage.model.mapper import get_mapped_doc
from dontmanage.model.naming import set_name_by_naming_series, set_name_from_naming_options
from dontmanage.model.utils.rename_doc import update_linked_doctypes
from dontmanage.utils import cint, cstr, flt, get_formatted_email, today
from dontmanage.utils.deprecations import deprecated
from dontmanage.utils.user import get_users_with_role

from dontmanageerp.accounts.party import get_dashboard_info, validate_party_accounts
from dontmanageerp.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	def autoname(self):
		cust_master_name = dontmanage.defaults.get_global_default("cust_master_name")
		if cust_master_name == "Customer Name":
			self.name = self.get_customer_name()
		elif cust_master_name == "Naming Series":
			set_name_by_naming_series(self)
		else:
			set_name_from_naming_options(dontmanage.get_meta(self.doctype).autoname, self)

	def get_customer_name(self):
		if dontmanage.db.get_value("Customer", self.customer_name) and not dontmanage.flags.in_import:
	...
	def validate(self):
		...
		if dontmanage.db.exists("Customer", self.name):
			customer = dontmanage.get_doc("Customer", self.name)
			if self.loyalty_program == customer.loyalty_program and not self.loyalty_program_tier:
				self.loyalty_program_tier = customer.loyalty_program_tier

		if self.sales_team:
			if sum(member.allocated_percentage or 0 for member in self.sales_team) != 100:
				dontmanage.throw(_("Total contribution percentage should be equal to 100"))

	@dontmanage.whitelist()
	def get_customer_group_details(self):
		doc = dontmanage.get_doc("Customer Group", self.customer_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
		dontmanage.flags.customer_group_changed = False

		if not self.get("__islocal"):
			if self.customer_group != dontmanage.db.get_value("Customer", self.name, "customer_group"):
				dontmanage.flags.customer_group_changed = True

	def validate_default_bank_account(self):
		if self.default_bank_account:
			is_company_account = dontmanage.db.get_value(
				"Bank Account", self.default_bank_account, "is_company_account"
			)
			if not is_company_account:
				dontmanage.throw(
					_("{0} is not a company bank account").format(dontmanage.bold(self.default_bank_account))
				)
	...
	def validate_internal_customer(self):
		...
		internal_customer = dontmanage.db.get_value(
			"Customer",
			{
		...
		if internal_customer:
			dontmanage.throw(
				_("Internal Customer for company {0} already exists").format(
					dontmanage.bold(self.represents_company)
				)
			)
	...
	def update_customer_groups(self):
		...
		if dontmanage.flags.customer_group_changed:
	...
	def update_lead_status(self):
		...
		if self.lead_name:
			dontmanage.db.set_value("Lead", self.lead_name, "status", "Converted")

	def link_lead_address_and_contact(self):
		if self.lead_name:
			# assign lead address and contact to customer (if already not set)
			linked_contacts_and_addresses = dontmanage.get_all(
				"Dynamic Link",
				filters=[
			...
			for row in linked_contacts_and_addresses:
				linked_doc = dontmanage.get_doc(row.doctype, row.name)
	...
	def copy_communication(self):
		if not self.lead_name or not dontmanage.db.get_single_value(
			"CRM Settings", "carry_forward_communication_and_comments"
		):
			return

		from dontmanageerp.crm.utils import copy_comments, link_communications

		copy_comments("Lead", self.lead_name, self)
		link_communications("Lead", self.lead_name, self)

	def validate_name_with_customer_group(self):
		if dontmanage.db.exists("Customer Group", self.name):
			dontmanage.throw(
				_(
	...
	def on_trash(self):
		...
		if self.lead_name:
			dontmanage.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)

	def after_rename(self, olddn, newdn, merge=False):
		if dontmanage.defaults.get_global_default("cust_master_name") == "Customer Name":
		...
		if len(loyalty_program) == 1:
			...
			dontmanage.msgprint(
				_("Multiple Loyalty Programs found for Customer {}. Please select manually.").format(
					dontmanage.bold(self.customer_name)
				)
			)
...
def make_quotation(source_name, target_doc=None):
	...
	price_list, currency = dontmanage.db.get_value(
...
def get_loyalty_programs(doc):
	...
	lp_details = []
	loyalty_programs = dontmanage.get_all(
		"Loyalty Program",
...
def get_nested_links(link_doctype, link_name, ignore_permissions=False):
	from dontmanage.desk.treeview import _get_children
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		credit_controller_role = dontmanage.db.get_single_value("Accounts Settings", "credit_controller")
		if not credit_controller_role or credit_controller_role not in dontmanage.get_roles():
			...
			dontmanage.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "dontmanageerp.selling.doctype.customer.customer.send_emails",
...
def send_emails(args):
	...
	dontmanage.sendmail(recipients=args.get("credit_controller_users_list"), subject=subject, message=message)
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
	...
	cond = ""
	if cost_center:
		lft, rgt = dontmanage.get_cached_value("Cost Center", cost_center, ["lft", "rgt"])

		cond = f""" and cost_center in (select name from `tabCost Center` where
			lft >= {lft} and rgt <= {rgt})"""

	outstanding_based_on_gle = dontmanage.db.sql(
		f"""
	...
	if not ignore_outstanding_sales_order:
		outstanding_based_on_so = dontmanage.db.sql(
			"""
	...
	unmarked_delivery_note_items = dontmanage.db.sql(
		"""select
	...
	si_amounts = dontmanage.db.sql(
		...
		GROUP BY dn_detail""".format(
			", ".join(dontmanage.db.escape(dn_item.name) for dn_item in unmarked_delivery_note_items)
		)
	)
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = dontmanage.db.get_value(
			"Customer Credit Limit",
		...
		if not credit_limit:
			customer_group = dontmanage.get_cached_value("Customer", customer, "customer_group")
	...
	if not credit_limit:
		credit_limit = dontmanage.get_cached_value("Company", company, "credit_limit")
...
def make_contact(args, is_primary_contact=1):
	...
	contact = dontmanage.get_doc(values)
