...
test_records = dontmanage.get_test_records("Customer")


class TestCustomer(DontManageTestCase):
	def setUp(self):
		if not dontmanage.get_value("Item", "_Test Item"):
	...
	def test_get_customer_group_details(self):
		doc = dontmanage.new_doc("Customer Group")
		...
		c_doc = dontmanage.new_doc("Customer")
	...
	def test_party_details_tax_category(self):
		...
		dontmanage.delete_doc_if_exists("Address", "_Test Address With Tax Category-Billing")
		dontmanage.delete_doc_if_exists("Address", "_Test Address With Tax Category-Shipping")

		# Tax Category without Address
		details = get_party_details("_Test Customer With Tax Category")
		self.assertEqual(details.tax_category, "_Test Tax Category 1")

		billing_address = dontmanage.get_doc(
			dict(
				doctype="Address",
		...
		shipping_address = dontmanage.get_doc(
			dict(
				doctype="Address",
				address_title="_Test Address With Tax Category",
	...
	def test_rename(self):
		...
		# add comments
		comment = dontmanage.get_doc("Customer", "_Test Customer 1").add_comment(
			"Comment", "Test Comment for Rename"
		)

		# rename
		dontmanage.rename_doc("Customer", "_Test Customer 1", new_name)

		# check if customer renamed
		self.assertTrue(dontmanage.db.exists("Customer", new_name))
		self.assertFalse(dontmanage.db.exists("Customer", "_Test Customer 1"))
		...
		dontmanage.rename_doc("Customer", new_name, "_Test Customer 1")
	...
	def test_freezed_customer(self):
		...
		dontmanage.db.set_value("Customer", "_Test Customer", "is_frozen", 1)
		...
		dontmanage.db.set_value("Customer", "_Test Customer", "is_frozen", 0)

		so.save()

	def test_delete_customer_contact(self):
		customer = dontmanage.get_doc(get_customer_dict("_Test Customer for delete")).insert(
		...
		dontmanage.delete_doc("Customer", customer.name)

	def test_disabled_customer(self):
		make_test_records("Item")

		dontmanage.db.set_value("Customer", "_Test Customer", "disabled", 1)
		...
		dontmanage.db.set_value("Customer", "_Test Customer", "disabled", 0)

		so.save()

	def test_duplicate_customer(self):
		dontmanage.db.sql("delete from `tabCustomer` where customer_name='_Test Customer 1'")

		if not dontmanage.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = dontmanage.get_doc(get_customer_dict("_Test Customer 1")).insert(
				ignore_permissions=True
			)
		else:
			test_customer_1 = dontmanage.get_doc("Customer", "_Test Customer 1")

		duplicate_customer = dontmanage.get_doc(get_customer_dict("_Test Customer 1")).insert(
			ignore_permissions=True
		)
	...
	def test_customer_credit_limit(self):
		...
		self.assertRaises(dontmanage.ValidationError, so.submit)
	...
	def test_customer_credit_limit_on_change(self):
		outstanding_amt = self.get_customer_outstanding_amount()
		customer = dontmanage.get_doc("Customer", "_Test Customer")
		customer.append(
		...
		self.assertRaises(dontmanage.ValidationError, customer.save)
	...
	def test_customer_payment_terms(self):
		...
		dontmanage.db.set_value("Customer", "_Test Customer With Template", "payment_terms", "")
...
def create_internal_customer(customer_name=None, represents_company=None, allowed_to_interact_with=None):
	...
	exisiting_representative = dontmanage.db.get_value("Customer", {"represents_company": represents_company})
	if exisiting_representative:
		return exisiting_representative

	if not dontmanage.db.exists("Customer", customer_name):
		...
		customer_name = dontmanage.db.get_value("Customer", customer_name)
