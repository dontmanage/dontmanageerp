...
class InstallationNote(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.selling.doctype.installation_note_item.installation_note_item import (
	...
	def validate(self):
		...
		from dontmanageerp.controllers.selling_controller import set_default_income_account_for_item

		set_default_income_account_for_item(self)

	def is_serial_no_added(self, item_code, serial_no):
		has_serial_no = dontmanage.db.get_value("Item", item_code, "has_serial_no")
		if has_serial_no == 1 and not serial_no:
			dontmanage.throw(_("Serial No is mandatory for Item {0}").format(item_code))
		elif has_serial_no != 1 and cstr(serial_no).strip():
			dontmanage.throw(_("Item {0} is not a serialized Item").format(item_code))

	def is_serial_no_exist(self, item_code, serial_no):
		for x in serial_no:
			if not dontmanage.db.exists("Serial No", x):
				dontmanage.throw(_("Serial No {0} does not exist").format(x))

	def get_prevdoc_serial_no(self, prevdoc_detail_docname):
		serial_nos = dontmanage.db.get_value("Delivery Note Item", prevdoc_detail_docname, "serial_no")
	...
	def validate_installation_date(self):
		for d in self.get("items"):
			if d.prevdoc_docname:
				d_date = dontmanage.db.get_value("Delivery Note", d.prevdoc_docname, "posting_date")
	...
	def check_item_table(self):
		if not (self.get("items")):
			dontmanage.throw(_("Please pull items from Delivery Note"))
