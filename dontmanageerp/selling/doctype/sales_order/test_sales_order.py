...
from dontmanageerp.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
...
from dontmanageerp.stock.get_item_details import get_bin_details


class TestSalesOrder(AccountsTestMixin, DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_make_material_request(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_material_request, so.name)
	...
	def test_make_delivery_note(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_delivery_note, so.name)
	...
	def test_make_sales_invoice(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_sales_invoice, so.name)
	...
	def test_reserved_qty_for_over_delivery(self):
		...
		dontmanage.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
	...
	def test_reserved_qty_for_over_delivery_via_sales_invoice(self):
		...
		# set over-delivery allowance
		dontmanage.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
		dontmanage.db.set_value("Item", "_Test Item", "over_billing_allowance", 20)
	...
	def test_sales_order_on_hold(self):
		...
		si = make_sales_invoice(so.name)
		self.assertRaises(dontmanage.ValidationError, create_dn_against_so, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery allowance
		dontmanage.db.set_value("Item", "_Test Product Bundle Item", "over_delivery_receipt_allowance", 50)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(dontmanage.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(dontmanage.get_meta("Sales Order Item").get_field("rate"))
	...
	def test_update_child_qty_rate_with_workflow(self):
		from dontmanage.model.workflow import apply_workflow
		...
		test_user2 = dontmanage.get_doc("User", user2)
		test_user2.add_roles("Sales User", "Test Approver")
		dontmanage.set_user(user2)
	...
	def test_update_child_product_bundle(self):
		# test Update Items with product bundle
		if not dontmanage.db.exists("Item", "_Product Bundle Item"):
	...
	def test_update_child_with_tax_template(self):
		...
		"""
		if not dontmanage.db.exists("Item", "Test Item with Tax"):
			make_item(
				"Test Item with Tax",
				{
					"is_stock_item": 1,
		...
		if not dontmanage.db.exists("Item Tax Template", {"title": "Test Update Items Template"}):
			dontmanage.get_doc(
				{
		...
		new_item_with_tax = dontmanage.get_doc("Item", "Test Item with Tax")
		...
		if not dontmanage.db.exists("Item Tax", {"parent": item, "item_tax_template": tax_template}):
			item_doc = dontmanage.get_doc("Item", item)
	...
	def test_warehouse_user(self):
		...
		test_user_2 = dontmanage.get_doc("User", "test2@example.com")
		test_user_2.add_roles("Sales User", "Stock User")
		test_user_2.remove_roles("Sales Manager")

		dontmanage.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		dontmanage.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		dontmanage.permissions.add_user_permission("Company", "_Test Company 1", test_user_2.name)
		...
		self.assertRaises(dontmanage.PermissionError, so.insert)

		dontmanage.set_user(test_user_2.name)
		so.insert()

		dontmanage.set_user("Administrator")
		dontmanage.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		dontmanage.permissions.remove_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		dontmanage.permissions.remove_user_permission("Company", "_Test Company 1", test_user_2.name)
	...
	def test_block_delivery_note_against_cancelled_sales_order(self):
		...
		self.assertRaises(dontmanage.CancelledLinkError, dn.submit)
	...
	def test_auto_insert_price(self):
		...
		if item_price:
			dontmanage.delete_doc("Item Price", item_price)
	...
	def test_drop_shipping(self):
		...
		from dontmanageerp.selling.doctype.sales_order.sales_order import update_status as so_update_status
		...
		if dontmanage.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
		...
		bin_po_item = dontmanage.get_all(
			"Bin",
		...
		self.assertEqual(flt(dontmanage.db.get_value("Sales Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_drop_shipping_full_for_default_suppliers(self):
		...
		if not dontmanage.db.exists("Item", "_Test Item for Drop Shipping 1"):
			make_item("_Test Item for Drop Shipping 1", {"is_stock_item": 1, "delivered_by_supplier": 1})

		if not dontmanage.db.exists("Item", "_Test Item for Drop Shipping 2"):
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		...
		"""
		from dontmanageerp.selling.doctype.sales_order.sales_order import make_purchase_order
	...
	def test_make_work_order(self):
		from dontmanageerp.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		dontmanage.db.set_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order", 0)
		...
		so_doc = dontmanage.get_doc("Sales Order", so.name)

		self.assertRaises(dontmanage.LinkExistsError, so_doc.cancel)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		po_doc = dontmanage.get_doc("Payment Entry", pe.name)
		po_doc.cancel()
	...
	def test_work_order_pop_up_from_sales_order(self):
		...
		from dontmanageerp.controllers.item_variant import create_variant
	...
	def test_so_optional_blanket_order(self):
		...
		so_doc = dontmanage.get_doc("Sales Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		...
		self.assertRaises(dontmanage.ValidationError, so.cancel)
	...
	def test_so_cancellation_after_si_submission(self):
		...
		so.load_from_db()
		self.assertRaises(dontmanage.LinkExistsError, so.cancel)
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		wo = dontmanage.get_doc("Work Order", wo_name)
		...
		se = dontmanage.get_doc(make_se_from_wo(wo.name, "Manufacture", 2))
		se.submit()  # Finish WO
	...
	def test_sales_order_partial_advance_payment(self):
		...
		dontmanage.get_doc(customer).insert()
	...
	def test_auto_update_price_list(self):
		...
		item_price = dontmanage.db.get_value("Item Price", {"item_code": item.name}, "price_list_rate")
		...
		dontmanage.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 1)
		so = make_sales_order(
		...
		dontmanage.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 0)
		dontmanage.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 0)

	def test_credit_limit_on_so_reopning(self):
		# set credit limit
		company = "_Test Company"
		customer = dontmanage.get_doc("Customer", self.customer)
		...
		self.assertRaises(dontmanage.ValidationError, so1.update_status, "Draft")
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(dontmanage.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "reserved_qty"))
...
def make_sales_order_workflow():
	if dontmanage.db.exists("Workflow", "SO Test Workflow"):
		doc = dontmanage.get_doc("Workflow", "SO Test Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc

	dontmanage.get_doc(dict(doctype="Role", role_name="Test Junior Approver")).insert(ignore_if_duplicate=True)
	dontmanage.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	dontmanage.cache().hdel("roles", dontmanage.session.user)

	workflow = dontmanage.get_doc(
		{
			"doctype": "Workflow",
