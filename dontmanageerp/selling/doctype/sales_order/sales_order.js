...
dontmanage.ui.form.on("Sales Order", {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0) {
				dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation").then((value) => {
					if (!value) {
...
dontmanageerp.selling.SalesOrderController = class SalesOrderController extends dontmanageerp.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if (doc.docstatus==1) {
			...
			if(doc.status !== 'Closed') {
				...
				// payment request
				if(flt(doc.per_billed, precision('per_billed', doc)) < 100 + dontmanage.boot.sysdefaults.over_billing_allowance) {
		...
		var d = new dontmanage.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					...
					callback: function(r) {
						if(r.message) {
							dontmanage.msgprint(__('Material Request {0} submitted.',
	...
	make_purchase_order(){
		...
		if(!pending_items){
			dontmanage.throw({message: __("Purchase Order already created for all Sales Order items"), title: __("Note")});
		}
		...
		var dialog = new dontmanage.ui.Dialog({
			...
			primary_action (args) {
				...
				if(selected_items.length == 0) {
					dontmanage.throw({message: 'Please select Items from the Table', title: __('Items Required'), indicator:'blue'})
				}
