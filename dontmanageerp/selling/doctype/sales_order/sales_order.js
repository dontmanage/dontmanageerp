...
dontmanageerp.accounts.taxes.setup_tax_validations("Sales Order");
dontmanageerp.sales_common.setup_selling_controller();

dontmanage.ui.form.on("Sales Order", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		if (frm.doc.docstatus === 1) {
			...
			if (
				...
				dontmanage.model.can_cancel("Stock Reservation Entry")
			) {
				frm.add_custom_button(
					__("Unreserve"),
			...
			frm.doc.items.forEach((item) => {
				if (flt(item.stock_reserved_qty) > 0 && dontmanage.model.can_read("Stock Reservation Entry")) {
					frm.add_custom_button(
						__("Reserved Stock"),
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0) {
				dontmanage.call({
					method: "dontmanageerp.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
	...
	get_items_from_internal_purchase_order(frm) {
		if (!dontmanage.model.can_read("Purchase Order")) {
			return;
		}
		...
		frm.add_custom_button(
			...
			() => {
				dontmanageerp.utils.map_current_doc({
					method: "dontmanageerp.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
	...
	company: function (frm) {
		if (frm.doc.company) {
			dontmanage.call({
				method: "dontmanageerp.setup.doctype.company.company.get_default_company_address",
				args: {
	...
	onload: function (frm) {
		if (!frm.doc.transaction_date) {
			frm.set_value("transaction_date", dontmanage.datetime.get_today());
		}
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "dontmanageerp.controllers.queries.warehouse_query";
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "dontmanageerp.controllers.queries.get_filtered_child_rows",
							filters: {
			...
			primary_action: () => {
				...
				if (data.items && data.items.length > 0) {
					...
					dontmanage.msgprint(__("Please select items to reserve."));
				}
			},
		});
	...
	cancel_stock_reservation_entries(frm) {
		const dialog = new dontmanage.ui.Dialog({
			...
			primary_action: () => {
				...
				if (data.sr_entries && data.sr_entries.length > 0) {
					...
					dontmanage.msgprint(__("Please select items to unreserve."));
				}
			},
		});
		...
		dontmanage
			.call({
				method: "dontmanageerp.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
...
dontmanage.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			dontmanageerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
});

dontmanageerp.selling.SalesOrderController = class SalesOrderController extends dontmanageerp.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
			...
			if (doc.status !== "Closed") {
				if (doc.status !== "On Hold") {
					...
					if (
						...
						dontmanage.model.can_create("Pick List")
					) {
						this.frm.add_custom_button(
							__("Pick List"),
					...
					if (
						...
						if (dontmanage.model.can_create("Delivery Note")) {
							this.frm.add_custom_button(
								__("Delivery Note"),
						...
						if (dontmanage.model.can_create("Work Order")) {
							this.frm.add_custom_button(
								__("Work Order"),
					...
					// sales invoice
					if (flt(doc.per_billed) < 100 && dontmanage.model.can_create("Sales Invoice")) {
						this.frm.add_custom_button(
							__("Sales Invoice"),
					...
					if (
						...
						dontmanage.model.can_create("Material Request")
					) {
						this.frm.add_custom_button(
							__("Material Request"),
					...
					// Make Purchase Order
					if (!this.frm.doc.is_internal_customer && dontmanage.model.can_create("Purchase Order")) {
						this.frm.add_custom_button(
							__("Purchase Order"),
					...
					if (flt(doc.per_delivered) < 100 && (order_is_maintenance || order_is_a_custom_sale)) {
						if (dontmanage.model.can_create("Maintenance Visit")) {
							this.frm.add_custom_button(
						...
						if (dontmanage.model.can_create("Maintenance Schedule")) {
							this.frm.add_custom_button(
					...
					// project
					if (flt(doc.per_delivered) < 100 && dontmanage.model.can_create("Project")) {
					...
					if (
						...
						dontmanage.model.can_create("Purchase Order")
					) {
						let me = this;
				...
				// payment request
				if (flt(doc.per_billed) < 100 + dontmanage.boot.sysdefaults.over_billing_allowance) {
					this.frm.add_custom_button(
						__("Payment Request"),
					...
					if (dontmanage.model.can_create("Payment Entry")) {
						this.frm.add_custom_button(
							__("Payment"),
		...
		if (this.frm.doc.docstatus === 0 && dontmanage.model.can_read("Quotation")) {
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = dontmanageerp.utils.map_current_doc({
						method: "dontmanageerp.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
				...
				if (!r.message) {
					...
					var d = new dontmanage.ui.Dialog({
						...
						primary_action: function () {
							...
							if (!data) {
								dontmanage.throw(__("Please select items"));
							}
							me.frm.call({
	...
	make_material_request() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new dontmanage.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
					...
					${delivery_dates
						...
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								${dontmanage.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`
	...
	make_sales_invoice() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_sales_invoice",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm,
		});
	}
		...
		var dialog = new dontmanage.ui.Dialog({
			...
			primary_action(args) {
				...
				return dontmanage.call({
					method: "dontmanageerp.selling.doctype.sales_order.sales_order." + method,
					...
					callback: function (r) {
						if (!r.exc) {
							if (!args.against_default_supplier) {
								...
								dontmanage.set_route("List", "Purchase Order");
							}
						}
					},
				});
	...
	hold_sales_order() {
		...
		var d = new dontmanage.ui.Dialog({
			...
			primary_action: function () {
				var data = d.get_values();
				dontmanage.call({
					method: "dontmanage.desk.form.utils.add_comment",
					args: {
	...
	update_status(label, status) {
		...
		dontmanage.call({
			method: "dontmanageerp.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new dontmanageerp.selling.SalesOrderController({ frm: cur_frm }));
