...
from dontmanageerp.stock.get_item_details import get_default_bom, get_price_list_rate
...
class SalesOrder(SellingController):
				...
				if cint(
					...
					dontmanage.msgprint(
						_("Warning: Sales Order {0} already exists against Customer's Purchase Order {1}").format(
							dontmanage.bold(so[0][0]), dontmanage.bold(self.po_no)
						)
					)
				else:
					dontmanage.throw(
						_(
							...
							dontmanage.bold(_("'Allow Multiple Sales Orders Against a Customer's Purchase Order'")),
	...
	def validate_sales_mntc_quotation(self):
		for d in self.get("items"):
			if d.prevdoc_docname:
				...
				if not res:
					dontmanage.msgprint(_("Quotation {0} not of type {1}").format(d.prevdoc_docname, self.order_type))
	...
	def check_modified_date(self):
		...
		date_diff = dontmanage.db.sql("select TIMEDIFF('%s', '%s')" % (mod_db, cstr(self.modified)))
	...
	def validate_reserved_stock(self):
		...
		enable_stock_reservation = dontmanage.db.get_single_value(
