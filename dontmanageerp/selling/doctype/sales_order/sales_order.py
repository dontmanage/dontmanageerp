...
from dontmanage.contacts.doctype.address.address import get_company_address
...
from dontmanage.utils import add_days, cint, cstr, flt, get_link_to_form, getdate, nowdate, strip_html
...
from dontmanageerp.manufacturing.doctype.blanket_order.blanket_order import (
...
from dontmanageerp.stock.get_item_details import get_bin_details, get_default_bom, get_price_list_rate
from dontmanageerp.stock.stock_balance import get_reserved_qty, update_bin_qty

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class WarehouseRequired(dontmanage.ValidationError):
	pass
...
class SalesOrder(SellingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.selling.doctype.sales_order_item.sales_order_item import SalesOrderItem
	...
	def onload(self) -> None:
		...
		if dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation"):
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			...
			if so and so[0][0]:
				if cint(
					dontmanage.db.get_single_value("Selling Settings", "allow_against_multiple_purchase_orders")
				):
					dontmanage.msgprint(
						_(
							"Warning: Sales Order {0} already exists against Customer's Purchase Order {1}"
						).format(dontmanage.bold(so[0][0]), dontmanage.bold(self.po_no)),
						alert=True,
					)
				else:
					dontmanage.throw(
						_(
							...
							dontmanage.bold(so[0][0]),
							dontmanage.bold(self.po_no),
							dontmanage.bold(
	...
	def validate_for_items(self):
		for d in self.get("items"):
			...
			tot_avail_qty = dontmanage.db.sql(
	...
	def validate_delivery_date(self):
		if self.order_type == "Sales" and not self.skip_delivery_note:
			...
			if self.delivery_date:
				...
				dontmanage.throw(_("Please enter Delivery Date"))
	...
	def validate_warehouse(self):
		...
		for d in self.get("items"):
			if (
				(
					dontmanage.get_cached_value("Item", d.item_code, "is_stock_item") == 1
					or (
	...
	def validate_drop_ship(self):
		for d in self.get("items"):
			if d.delivered_by_supplier and not d.supplier:
				dontmanage.throw(_("Row #{0}: Set Supplier for item {1}").format(d.idx, d.item_code))
	...
	def on_cancel(self):
		...
		if self.status == "Closed":
			dontmanage.throw(_("Closed order cannot be cancelled. Unclose to cancel."))
	...
	def check_nextdoc_docstatus(self):
		linked_invoices = dontmanage.db.sql_list(
	...
	def check_modified_date(self):
		mod_db = dontmanage.db.get_value("Sales Order", self.name, "modified")
		date_diff = dontmanage.db.sql(f"select TIMEDIFF('{mod_db}', '{cstr(self.modified)}')")
		if date_diff and date_diff[0][0]:
			dontmanage.throw(_("{0} {1} has been modified. Please refresh.").format(self.doctype, self.name))
	...
	def update_reserved_qty(self, so_item_rows=None):
		...
		def _valid_for_reserve(item_code, warehouse):
			if (
				...
				and dontmanage.get_cached_value("Item", item_code, "is_stock_item")
			):
	...
	def validate_supplier_after_submit(self):
		...
		for item in self.items:
			if item.supplier:
				supplier = dontmanage.db.get_value(
	...
	def update_delivery_status(self):
		...
		for item in self.items:
			if item.delivered_by_supplier:
				item_delivered_qty = dontmanage.db.sql(
					"""select sum(qty)
	...
	def update_picking_status(self):
		...
		for so_item in self.items:
			if cint(
				dontmanage.get_cached_value("Item", so_item.item_code, "is_stock_item")
		...
		if total_picked_qty and total_qty:
			...
			pick_percentage = dontmanage.db.get_single_value("Stock Settings", "over_picking_allowance")
			if pick_percentage:
	...
	def on_recurring(self, reference_doc, auto_repeat_doc):
		def _get_delivery_date(ref_doc_delivery_date, red_doc_transaction_date, transaction_date):
			...
			if delivery_date <= transaction_date:
				delivery_date_diff = dontmanage.utils.date_diff(ref_doc_delivery_date, red_doc_transaction_date)
				delivery_date = dontmanage.utils.add_days(transaction_date, delivery_date_diff)
		...
		for d in self.get("items"):
			reference_delivery_date = dontmanage.db.get_value(
				"Sales Order Item",
		...
		for item in self.items:
			if item.ensure_delivery_based_on_produced_serial_no:
				...
				if item.item_code not in reserved_items:
					...
					if not dontmanage.db.exists("BOM", {"item": item.item_code, "is_active": 1}):
						dontmanage.throw(
							_(
	...
	def validate_reserved_stock(self):
		...
		enable_stock_reservation = dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation")
...
def close_or_unclose_sales_orders(names, status):
	if not dontmanage.has_permission("Sales Order", "write"):
		dontmanage.throw(_("Not permitted"), dontmanage.PermissionError)

	names = json.loads(names)
	for name in names:
		so = dontmanage.get_doc("Sales Order", name)
		if so.docstatus == 1:
			if status == "Closed":
...
def get_requested_item_qty(sales_order):
	...
	for d in dontmanage.db.get_all(
		...
		result[d.sales_order_item] = dontmanage._dict({"qty": d.qty, "received_qty": d.received_qty})
...
def make_material_request(source_name, target_doc=None):
	...
	def postprocess(source, target):
		if source.tc_name and dontmanage.db.get_value("Terms and Conditions", source.tc_name, "buying") != 1:
			target.tc_name = None
			target.terms = None
	...
	def update_item(source, target, source_parent):
		...
		args.update(
			{
				...
				"price_list": dontmanage.db.get_single_value("Buying Settings", "buying_price_list"),
		...
		target.rate = flt(
			get_price_list_rate(args=args, item_doc=dontmanage.get_cached_doc("Item", target.item_code)).get(
				"price_list_rate"
			)
		)
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Sales Order Item": {
				...
				"condition": lambda item: not dontmanage.db.exists(
...
def make_delivery_note(source_name, target_doc=None, kwargs=None):
	...
	if not kwargs:
		kwargs = {
			"for_reserved_stock": dontmanage.flags.args and dontmanage.flags.args.for_reserved_stock,
			"skip_item_mapping": dontmanage.flags.args and dontmanage.flags.args.skip_item_mapping,
		}
	...
	def set_missing_values(source, target):
		...
		if dontmanage.flags.bulk_transaction:
	...
	def condition(doc):
		...
		# make_mapped_doc sets js `args` into `dontmanage.flags.args`
		if dontmanage.flags.args and dontmanage.flags.args.delivery_dates:
			if cstr(doc.delivery_date) not in dontmanage.flags.args.delivery_dates:
				return False
	...
	def update_item(source, target, source_parent):
		...
		if item:
			target.cost_center = (
				dontmanage.db.get_value("Project", source_parent.project, "cost_center")
	...
	so = dontmanage.get_doc("Sales Order", source_name)
...
def make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	def update_item(source, target, source_parent):
		...
		if source_parent.project:
			target.cost_center = dontmanage.db.get_value("Project", source_parent.project, "cost_center")
		if target.item_code:
			item = get_item_defaults(target.item_code, source_parent.company)
			item_group = get_item_group_defaults(target.item_code, source_parent.company)
...
def make_maintenance_schedule(source_name, target_doc=None):
	maint_schedule = dontmanage.db.sql(
		"""select t1.name
...
def make_purchase_order_for_default_supplier(source_name, selected_items=None, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		target.currency = dontmanage.db.get_value(
			"Supplier", filters={"name": supplier}, fieldname=["default_currency"]
		)
		company_currency = dontmanage.db.get_value(
		...
		default_price_list = dontmanage.get_value("Supplier", supplier, "default_price_list")
		if default_price_list:
			target.buying_price_list = default_price_list

		default_payment_terms = dontmanage.get_value("Supplier", supplier, "payment_terms")
	...
	if not suppliers:
		dontmanage.throw(_("Please set a Supplier against the Items to be considered in the Purchase Order."))
...
def make_purchase_order(source_name, selected_items=None, target_doc=None):
	...
	# po = dontmanage.get_list("Purchase Order", filters={"sales_order":source_name, "supplier":supplier, "docstatus": ("<", "2")})
	doc = get_mapped_doc(
		"Sales Order",
		source_name,
		{
...
def is_product_bundle(item_code):
	return dontmanage.db.exists("Product Bundle", {"name": item_code, "disabled": 0})
...
def make_work_orders(items, sales_order, company, project=None):
	...
	for i in items:
		if not i.get("bom"):
			dontmanage.throw(_("Please select BOM against item {0}").format(i.get("item_code")))
		if not i.get("pending_qty"):
			dontmanage.throw(_("Please select Qty against item {0}").format(i.get("item_code")))

		work_order = dontmanage.get_doc(
			dict(
				doctype="Work Order",
...
def make_raw_material_request(items, company, sales_order, project=None):
	...
	if isinstance(items, str):
		items = dontmanage._dict(json.loads(items))
	...
	if not raw_materials:
		dontmanage.msgprint(_("Material Request not created, as quantity for Raw Materials already available."))
		return

	material_request = dontmanage.new_doc("Material Request")
	...
	for item in raw_materials:
		item_doc = dontmanage.get_cached_doc("Item", item.get("item_code"))
...
def create_pick_list(source_name, target_doc=None):
	from dontmanageerp.stock.doctype.packed_item.packed_item import is_product_bundle
		...
		for item in so.items:
			if item.stock_reserved_qty > 0:
				dontmanage.throw(
					...
					).format(dontmanage.bold(source_name))
				)
...
def update_produced_qty_in_so_item(sales_order, sales_order_item):
	...
	linked_wo_with_so_item = dontmanage.db.get_all(
		"Work Order",
		["produced_qty"],
	...
	if not total_produced_qty and dontmanage.flags.in_patch:
		return

	dontmanage.db.set_value("Sales Order Item", sales_order_item, "produced_qty", total_produced_qty)
...
def get_work_order_items(sales_order, for_raw_material_request=0):
	...
	if sales_order:
		so = dontmanage.get_doc("Sales Order", sales_order)
...
def get_stock_reservation_status():
	return dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation")
