...
import dontmanage
from dontmanage import ValidationError, _, msgprint
from dontmanage.contacts.doctype.address.address import render_address
from dontmanage.utils import cint, flt, getdate
from dontmanage.utils.data import nowtime

import dontmanageerp
from dontmanageerp.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from dontmanageerp.accounts.doctype.budget.budget import validate_expense_against_budget
from dontmanageerp.accounts.party import get_party_details
from dontmanageerp.buying.utils import update_last_purchase_rate, validate_for_items
...
class BuyingController(SubcontractingController):
	...
	def validate(self):
		...
		if getattr(self, "supplier", None) and not self.supplier_name:
			self.supplier_name = dontmanage.db.get_value("Supplier", self.supplier, "supplier_name")
	...
	def onload(self):
		...
		self.set_onload(
			"backflush_based_on",
			dontmanage.db.get_single_value("Buying Settings", "backflush_raw_materials_of_subcontract_based_on"),
		)
	...
	def create_package_for_transfer(self) -> None:
		...
		if self.is_internal_transfer() and (
			...
			bundle_ids = {}
			if ids:
				for bundle in dontmanage.get_all(
		...
		if self.meta.get_field("supplier") and not self.supplier:
			for d in self.get("items"):
				...
				if supplier:
					...
					item_group = dontmanage.db.get_value("Item", d.item_code, "item_group")
					supplier = dontmanage.db.get_value(
	...
	def validate_asset_return(self):
		...
		if self.return_against:
			not_cancelled_asset = [
				d.name
				for d in dontmanage.db.get_all("Asset", {purchase_doc_field: self.return_against, "docstatus": 1})
			]
	...
	def set_landed_cost_voucher_amount(self):
		for d in self.get("items"):
			lc_voucher_data = dontmanage.db.sql(
	...
	def set_incoming_rate(self):
		...
		allow_at_arms_length_price = dontmanage.get_cached_value(
	...
	def set_sales_incoming_rate_for_internal_transfer(self):
		...
		for d in self.get("items"):
			if not d.get(dontmanage.scrub(ref_doctype)):
				posting_time = self.get("posting_time")
				if not posting_time:
				...
				d.sales_incoming_rate = flt(
					dontmanage.db.get_value(ref_doctype, d.get(dontmanage.scrub(ref_doctype)), field)
	...
	def validate_for_subcontracting(self):
		if self.is_subcontracted and self.get("is_old_subcontracting_flow"):
			if self.doctype in ["Purchase Receipt", "Purchase Invoice"] and not self.supplier_warehouse:
				dontmanage.throw(_("Supplier Warehouse mandatory for sub-contracted {0}").format(self.doctype))

			for item in self.get("items"):
				if item in self.sub_contracted_items and not item.bom:
					dontmanage.throw(_("Please select BOM in BOM field for Item {0}").format(item.item_code))
			if self.doctype != "Purchase Order":
				return
			for row in self.get("supplied_items"):
				if not row.reserve_warehouse:
					msg = f"Reserved Warehouse is mandatory for the Item {dontmanage.bold(row.rm_item_code)} in Raw Materials supplied"
	...
	def validate_purchase_return(self):
		for d in self.get("items"):
			if self.is_return and flt(d.rejected_qty) != 0:
				dontmanage.throw(_("Row #{0}: Rejected Qty can not be entered in Purchase Return").format(d.idx))
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			...
			if flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty")):
				...
				dontmanage.throw(msg=message, title=_("Mismatch"), exc=QtyMismatchError)
			...
			if flt(item_row[fieldname]) < 0:
				dontmanage.throw(
					_("Row #{0}: {1} can not be negative for item {2}").format(
						item_row["idx"],
						dontmanage.get_meta(item_row.doctype).get_label(fieldname),
						item_row["item_code"],
					)
				)
	...
	def check_for_on_hold_or_closed_status(self, ref_doctype, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = dontmanage.db.get_value(ref_doctype, d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold"):
					dontmanage.throw(_("{0} {1} is {2}").format(ref_doctype, d.get(ref_fieldname), status))
				...
				if pr_qty:
					if d.from_warehouse and (
						...
						if self.is_internal_transfer() and self.is_return and self.docstatus == 2:
							serial_and_batch_bundle = dontmanage.db.get_value(
								"Stock Ledger Entry",
	...
	def update_ordered_and_reserved_qty(self):
		...
		for po, po_item_rows in po_map.items():
			if po and po_item_rows:
				po_obj = dontmanage.get_doc("Purchase Order", po)

				if po_obj.status in ["Closed", "Cancelled"]:
					dontmanage.throw(
						_("{0} {1} is cancelled or closed").format(_("Purchase Order"), po),
						dontmanage.InvalidStatusError,
					)
	...
	def on_submit(self):
		...
		if self.doctype in [
			...
			"Purchase Receipt",
			"Purchase Invoice",
		] and not dontmanage.db.get_single_value("Buying Settings", "disable_last_purchase_rate"):
	...
	def auto_make_assets(self, asset_items):
						...
						if len(created_assets) > 5:
							...
							messages.append(
								_("{} Assets created for {}").format(
									len(created_assets), dontmanage.bold(d.item_code)
								)
							)
						else:
							assets_link = list(
								map(lambda d: dontmanage.utils.get_link_to_form("Asset", d), created_assets)
							)
							assets_link = dontmanage.bold(",".join(assets_link))

							is_plural = "s" if len(created_assets) != 1 else ""
							messages.append(
								_("Asset{} {assets_link} created for {}").format(
									is_plural, dontmanage.bold(d.item_code), assets_link=assets_link
								)
							)
					else:
						dontmanage.throw(
							...
							).format(d.idx, dontmanage.bold(d.item_code))
						)
				else:
					messages.append(
		...
		for message in messages:
			dontmanage.msgprint(message, title="Success", indicator="green")

	def make_asset(self, row, accounting_dimensions, is_grouped_asset=False):
		if not row.asset_location:
			dontmanage.throw(_("Row {0}: Enter location for the asset item {1}").format(row.idx, row.item_code))

		item_data = dontmanage.get_cached_value(
			"Item", row.item_code, ["asset_naming_series", "asset_category"], as_dict=1
		)
		asset_quantity = row.qty if is_grouped_asset else 1
		purchase_amount = flt(row.valuation_rate) * asset_quantity

		asset = dontmanage.get_doc(
			{
				"doctype": "Asset",
				"item_code": row.item_code,
	...
	def update_fixed_asset(self, field, delete_asset=False):
		for d in self.get("items"):
			if d.is_fixed_asset:
				is_auto_create_enabled = dontmanage.db.get_value("Item", d.item_code, "auto_create_assets")
				assets = dontmanage.db.get_all("Asset", filters={field: self.name, "item_code": d.item_code})
				...
				for asset in assets:
					...
					if delete_asset and is_auto_create_enabled:
						...
						for movement in movements:
							dontmanage.delete_doc("Asset Movement", movement.name, force=1)
						dontmanage.delete_doc("Asset", asset.name, force=1)
						continue
					...
					if self.docstatus == 2:
						...
						if asset.docstatus == 1 and delete_asset:
							dontmanage.throw(
								...
								).format(dontmanage.utils.get_link_to_form("Asset", asset.name))
							)
	...
	def delete_linked_asset(self):
		...
		asset_movement = dontmanage.db.get_value("Asset Movement", {"reference_name": self.name}, "name")
		dontmanage.delete_doc("Asset Movement", asset_movement, force=1)
		...
		if self.schedule_date:
			for d in self.get("items"):
				...
				if (
					...
					dontmanage.throw(_("Row #{0}: Reqd by Date cannot be before Transaction Date").format(d.idx))
		else:
			dontmanage.throw(_("Please enter Reqd by Date"))
...
def validate_item_type(doc, fieldname, message):
	...
	item_list = ", ".join(["%s" % dontmanage.db.escape(d) for d in items])
