...
class BuyingController(SubcontractingController):
	...
	def on_submit(self):
		...
		if self.doctype in ["Purchase Order", "Purchase Receipt"] and not dontmanage.db.get_single_value(
				...
				if item_data.get("auto_create_assets"):
					...
					if item_data.get("asset_naming_series"):
						...
						if len(created_assets) > 5:
							...
							messages.append(
								_("{} Assets created for {}").format(len(created_assets), dontmanage.bold(d.item_code))
							)
						else:
							assets_link = list(map(lambda d: dontmanage.utils.get_link_to_form("Asset", d), created_assets))
						...
						dontmanage.throw(
							_("Row {}: Asset Naming Series is mandatory for the auto creation for item {}").format(
								d.idx, dontmanage.bold(d.item_code)
							)
						)
				else:
					messages.append(
	...
	def make_asset(self, row, is_grouped_asset=False):
		...
		item_data = dontmanage.db.get_value(
