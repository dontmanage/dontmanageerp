...
from dontmanage.utils import cint, flt, format_datetime, get_datetime

import dontmanageerp
from dontmanageerp.stock.serial_batch_bundle import get_batches_from_bundle
...
class StockOverReturnError(dontmanage.ValidationError):
	pass
...
def validate_return_against(doc):
	if not dontmanage.db.exists(doc.doctype, doc.return_against):
		dontmanage.throw(_("Invalid {0}: {1}").format(doc.meta.get_label("return_against"), doc.return_against))
	else:
		ref_doc = dontmanage.get_doc(doc.doctype, doc.return_against)
...
def validate_returned_items(doc):
	...
	for d in doc.get("items"):
		...
		if doc.doctype in ["Purchase Receipt", "Purchase Invoice", "Sales Invoice"]:
			field = dontmanage.scrub(doc.doctype) + "_item"
			if d.get(field):
		...
		if d.item_code and (flt(d.qty) < 0 or flt(d.get("received_qty")) < 0):
			if key not in valid_items:
				...
				ref = valid_items.get(key, dontmanage._dict())
				...
				if (
					...
					and dontmanage.db.get_value("Item", d.item_code, "is_stock_item")
				):
					dontmanage.throw(_("Warehouse is mandatory"))
	...
	if not items_returned:
		dontmanage.throw(_("Atleast one item should be entered with negative quantity in return document"))
...
def validate_quantity(doc, key, args, ref, valid_items, already_returned_items):
	...
	stock_qty_precision = get_field_precision(
		dontmanage.get_meta(doc.doctype + " Item").get_field("stock_qty"), company_currency
	)
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				dontmanage.throw(_("{0} must be negative in return document").format(label))
...
def get_already_returned_items(doc):
	...
	field = (
		dontmanage.scrub(doc.doctype) + "_item"
...
def get_returned_qty_map_for_row(return_against, party, row_name, doctype):
	...
	reference_field = "dn_detail" if doctype == "Delivery Note" else dontmanage.scrub(child_doctype)
...
def make_return_doc(doctype: str, source_name: str, target_doc=None, return_against_rejected_qty=False):
	...
	company = dontmanage.db.get_value("Delivery Note", source_name, "company")
	default_warehouse_for_sales_return = dontmanage.get_cached_value(
	...
	def set_missing_values(source, target):
		...
		if doctype == "Sales Invoice" or doctype == "POS Invoice":
			...
			if not doc.select_print_heading:
				doc.select_print_heading = dontmanage.get_cached_value("Print Heading", _("Credit Note"))

		elif doctype == "Purchase Invoice":
			# look for Print Heading "Debit Note"
			doc.select_print_heading = dontmanage.get_cached_value("Print Heading", _("Debit Note"))
	...
	def update_serial_batch_no(source_doc, target_doc, source_parent, item_details, qty_field):
		...
		returned_serial_nos = []
		returned_batches = dontmanage._dict()
		...
		if (
			...
			if source_doc.get(old_batch_no_field):
				batches = dontmanage._dict({source_doc.batch_no: source_doc.get(qty_field)})
	...
	def get_returned_non_bundled_serial_nos(child_doc, parent_doc, serial_no_field="serial_no"):
		...
		return_ref_field = dontmanage.scrub(child_doc.doctype)
		...
		for row in dontmanage.get_all(parent_doc.doctype, fields=fields, filters=filters):
...
def get_rate_for_return(
	...
	if not return_against:
		return_against = dontmanage.get_cached_value(voucher_type, voucher_no, "return_against")
	...
	rate = flt(dontmanage.db.get_value("Stock Ledger Entry", filters, select_field))
	if not (rate and return_against) and voucher_type in ["Sales Invoice", "Delivery Note"]:
		rate = dontmanage.db.get_value(f"{voucher_type} Item", voucher_detail_no, "incoming_rate")
...
def get_filters(
	...
	if item_row:
		...
		reference_voucher_detail_no = dontmanage.db.get_value(
...
def available_serial_batch_for_return(field, doctype, reference_ids, is_rejected=False):
	...
	if not available_dict:
		dontmanage.throw(_("No Serial / Batches are available for return"))
...
def get_serial_batches_based_on_bundle(field, _bundle_ids):
	available_dict = dontmanage._dict({})
	batch_serial_nos = dontmanage.get_all(
	...
	for row in batch_serial_nos:
		...
		if dontmanage.get_cached_value(row.voucher_type, row.voucher_no, "is_return"):
			key = dontmanage.get_cached_value(row.voucher_type + " Item", row.voucher_detail_no, field)

		if row.voucher_type in ["Sales Invoice", "Delivery Note"]:
			row.qty = -1 * row.qty

		if key not in available_dict:
			available_dict[key] = dontmanage._dict(
				{
					"qty": 0.0,
...
def get_serial_and_batch_bundle(field, doctype, reference_ids, is_rejected=False):
	...
	_bundle_ids = dontmanage.get_all(
		doctype,
		filters=filters,
		pluck=pluck_field,
	)
...
def filter_serial_batches(parent_doc, data, row, warehouse_field=None, qty_field=None):
	...
	filterd_serial_batch = dontmanage._dict(
		{
			"serial_nos": [],
			"batches": defaultdict(float),
	...
	if not child_doc.get(qty_field):
		dontmanage.throw(
			_("For the {0}, the quantity is required to make the return entry").format(
				dontmanage.bold(child_doc.item_code)
			)
		)
