...
class SellingController(StockController):
	...
	def set_missing_lead_customer_details(self, for_validate=False):
		...
		if customer:
			from dontmanageerp.accounts.party import _get_party_details
			...
			from dontmanageerp.crm.doctype.lead.lead import get_lead_details
	...
	def validate_sales_team(self, sales_team):
		...
		sales_person_status = dontmanage.db.get_all(
			"Sales Person", filters={"name": ["in", sales_persons]}, fields=["name", "enabled"]
		)

		for row in sales_person_status:
			if not row.enabled:
				dontmanage.throw(_("Sales Person <b>{0}</b> is disabled.").format(row.name))

	def validate_max_discount(self):
		for d in self.get("items"):
			if d.item_code:
				discount = flt(dontmanage.get_cached_value("Item", d.item_code, "max_discount"))
	...
	def validate_selling_price(self):
		...
		if self.get("is_return") or not dontmanage.db.get_single_value(
		...
		for item in self.items:
			...
			last_purchase_rate, is_stock_item = dontmanage.get_cached_value(
		...
		or_conditions = (
			f"""(item_code = {dontmanage.db.escape(valuation_rate[0])}
			and warehouse = {dontmanage.db.escape(valuation_rate[1])})"""
			for valuation_rate in valuation_rate_map
		)

		valuation_rates = dontmanage.db.sql(
			f"""
			select
	...
	def get_item_list(self):
		...
		for d in self.get("items"):
			if d.qty is None:
				dontmanage.throw(_("Row {0}: Qty is mandatory").format(d.idx))
	...
	def get_already_delivered_qty(self, current_docname, so, so_detail):
		...
		delivered_via_si = dontmanage.db.sql(
	...
	def update_stock_ledger(self):
		...
		for d in self.get_item_list():
			if dontmanage.get_cached_value("Item", d.item_code, "is_stock_item") == 1 and flt(d.qty):
	...
	def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
		...
		if doc_list:
			po_nos += [
				d.po_no
				for d in dontmanage.get_all(ref_doctype, "po_no", filters={"name": ("in", doc_list)})
				if d.get("po_no")
			]
	...
	def validate_for_duplicate_items(self):
		...
		if cint(dontmanage.db.get_single_value("Selling Settings", "allow_multiple_items")):
			return
		...
		for d in self.get("items"):
			...
			duplicate_items_msg = _("Item {0} entered multiple times.").format(dontmanage.bold(d.item_code))
			duplicate_items_msg += "<br><br>"
			duplicate_items_msg += _("Please enable {} in {} to allow same item in multiple rows").format(
				dontmanage.bold(_("Allow Item to Be Added Multiple Times in a Transaction")),
				get_link_to_form("Selling Settings", "Selling Settings"),
			)
			if dontmanage.db.get_value("Item", d.item_code, "is_stock_item") == 1:
				if stock_items in check_list:
					dontmanage.throw(duplicate_items_msg)
				else:
	...
	def validate_target_warehouse(self):
		...
		for d in items:
			if d.get("target_warehouse") and d.get("warehouse") == d.get("target_warehouse"):
				warehouse = dontmanage.bold(d.get("target_warehouse"))
				dontmanage.throw(
					_(
		...
		if not self.get("is_internal_customer") and any(d.get("target_warehouse") for d in items):
			...
			dontmanage.msgprint(msg, title="Internal Transfer", alert=True)

	def validate_items(self):
		# validate items to see if they have is_sales_item enabled
		from dontmanageerp.controllers.buying_controller import validate_item_type
...
def get_serial_and_batch_bundle(child, parent):
	...
	if not dontmanage.db.get_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"):
		return

	item_details = dontmanage.db.get_value("Item", child.item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
