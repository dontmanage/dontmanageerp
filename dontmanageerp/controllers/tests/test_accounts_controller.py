	...
	def test_42_cost_center_from_cr_note(self):
		...
		for x in exc_je_for_si + exc_je_for_cr_note:
			with self.subTest(x=x):
				self.assertEqual(
					[self.cost_center, self.cost_center],
					dontmanage.db.get_all("Journal Entry Account", filters={"parent": x.parent}, pluck="cost_center"),
				)
		...
		for x in exc_je_for_si + exc_je_for_cr_note:
			with self.subTest(x=x):
				self.assertEqual(
					...
					dontmanage.db.get_all("Journal Entry Account", filters={"parent": x.parent}, pluck="department"),
				)
