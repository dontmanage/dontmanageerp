...
from dontmanageerp.buying.doctype.purchase_order.test_purchase_order import prepare_data_for_internal_transfer
...
def make_supplier(supplier_name, currency=None):
	if not dontmanage.db.exists("Supplier", supplier_name):
		supplier = dontmanage.new_doc("Supplier")
...
class TestAccountsController(DontManageTestCase):
	"""
	...
	def create_company(self):
		...
		if dontmanage.db.exists("Company", company_name):
			company = dontmanage.get_doc("Company", company_name)
		else:
	...
	def create_account(self):
		...
		for x in accounts:
			if not dontmanage.db.get_value("Account", filters={"account_name": x.name, "company": self.company}):
	...
	def setup_advance_accounts_in_party_master(self):
		company = dontmanage.get_doc("Company", self.company)
		...
		supplier = dontmanage.get_doc("Supplier", self.supplier)
		supplier.append(
			"accounts",
			{
				"company": self.company,
	...
	def test_15_gain_loss_on_different_posting_date(self):
		...
		self.assertEqual(
			dontmanage.db.get_value("Journal Entry", exc_je_for_si[0].parent, "posting_date"),
	...
	def test_16_internal_transfer_at_arms_length_price(self):
		from dontmanageerp.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_purchase_invoice
		...
		dontmanage.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 0)
		si.items[0].rate = arms_length_price
		si.save()
		...
		dontmanage.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 1)
		pi = make_inter_company_purchase_invoice(si.name)
		pi.update_stock = 1
		pi.items[0].rate = arms_length_price
		pi.items[0].warehouse = target_warehouse
	...
	def test_40_cost_center_from_payment_entry(self):
		...
		cc = dontmanage.db.get_value("Company", self.company, "cost_center")
		dontmanage.db.set_value("Company", self.company, "cost_center", None)
		...
		dontmanage.db.set_value("Company", self.company, "cost_center", cc)
	...
	def setup_dimensions(self):
		...
		loc = dontmanage.get_doc("Accounting Dimension", "Location")
	...
	def test_difference_posting_date_in_pi_and_si(self):
		...
		jv = dontmanage.get_doc("Journal Entry", exc_je_for_si[0].parent)
		sales_invoice = dontmanage.get_doc("Sales Invoice", si.name)
		...
		journal_voucher = dontmanage.get_doc("Journal Entry", exc_je_for_pi[0].parent)
		purchase_invoice = dontmanage.get_doc("Purchase Invoice", pi.name)
