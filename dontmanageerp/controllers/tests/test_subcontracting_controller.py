...
class TestSubcontractingController(DontManageTestCase):
	def setUp(self):
	...
	def test_item_with_batch_based_on_bom(self):
		...
		dontmanage.flags.add_debugger = True
		scr2 = make_subcontracting_receipt(sco.name)
		scr2.items[0].qty = 2
	...
	def test_incorrect_serial_no_components_based_on_material_transfer(self):
		...
		scr1 = make_subcontracting_receipt(sco.name)
		scr1.save()
		bundle = dontmanage.get_doc("Serial and Batch Bundle", scr1.supplied_items[0].serial_and_batch_bundle)
		original_serial_no = ""
		...
		self.assertRaises(dontmanage.ValidationError, scr1.save)
		bundle.load_from_db()
		for row in bundle.entries:
			if row.idx == 1:
	...
	for row in scr_doc.get("supplied_items"):
		if row.rm_item_code not in supplied_items:
			supplied_items.setdefault(
				row.rm_item_code, dontmanage._dict({"qty": 0, "serial_no": [], "batch_no": defaultdict(float)})
			)
...
def make_stock_in_entry(**args):
	...
	for row in args.rm_items:
		...
		if row.item_code not in items:
			items.setdefault(
				row.item_code, dontmanage._dict({"qty": 0, "serial_no": [], "batch_no": defaultdict(float)})
			)
...
def make_stock_transfer_entry(**args):
	...
	doc = dontmanage.get_doc(ste_dict)
	doc.insert()
	doc.submit()
...
def make_bom_for_subcontracted_items():
	...
	for item_code, raw_materials in boms.items():
		if not dontmanage.db.exists("BOM", {"item": item_code}):
			make_bom(item=item_code, raw_materials=raw_materials, rate=100, currency="INR")


def set_backflush_based_on(based_on):
	dontmanage.db.set_single_value("Buying Settings", "backflush_raw_materials_of_subcontract_based_on", based_on)
...
def get_subcontracting_order(**args):
	...
	if args.get("po_name"):
		po = dontmanage.get_doc("Purchase Order", args.get("po_name"))
...
def make_subcontracted_item(**args):
	...
	if not dontmanage.db.exists("Item", args.item_code):
		make_item(
			args.item_code,
			{
				"is_stock_item": 1,
	...
	if not dontmanage.db.get_value("BOM", {"item": args.item_code}, "name"):
