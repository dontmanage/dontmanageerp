...
def validate_filters(filters):
	for f in ["Fiscal Year", "Based On", "Period", "Company"]:
		if not filters.get(f.lower().replace(" ", "_")):
			dontmanage.throw(_("{0} is mandatory").format(f))

	if not dontmanage.db.exists("Fiscal Year", filters.get("fiscal_year")):
		dontmanage.throw(_("Fiscal Year {0} Does Not Exist").format(filters.get("fiscal_year")))

	if filters.get("based_on") == filters.get("group_by"):
		dontmanage.throw(_("'Based On' and 'Group By' can not be same"))
...
def get_data(filters, conditions):
	...
	year_start_date, year_end_date = dontmanage.get_cached_value(
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Item":
		...
		if trans in ["Sales Invoice", "Delivery Note", "Sales Order"]:
			...
			dontmanage.throw(_("Project-wise data is not available for Quotation"))
