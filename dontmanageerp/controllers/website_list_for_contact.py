...
from dontmanage.utils import flt, has_common
from dontmanage.utils.user import is_website_user


def get_list_context(context=None):
	return {
		"global_number_format": dontmanage.db.get_default("number_format") or "#,###.##",
...
def get_webform_list_context(module):
	if get_module_app(module) != "dontmanageerp":
		return
	...
	if txt:
		if meta.get_field("items"):
			if meta.get_field("items").options:
				...
				for item in dontmanage.get_all(child_doctype, {"item_name": ["like", "%" + txt + "%"]}):
					child = dontmanage.get_doc(child_doctype, item.name)
					or_filters.append([doctype, "name", "=", child.parent])

	if or_filters:
		for r in dontmanage.get_list(
			doctype,
			fields=fields,
			filters=filters,
...
def get_customers_suppliers(doctype, user):
	...
	if has_common(["Supplier", "Customer"], dontmanage.get_roles(user)):
		suppliers = get_parents_for_user("Supplier")
		customers = get_parents_for_user("Customer")
	elif dontmanage.has_permission(doctype, "read", user=user):
		customer_list = dontmanage.get_list("Customer")
...
def get_parents_for_user(parenttype: str) -> list[str]:
	portal_user = dontmanage.qb.DocType("Portal User")

	return (
		dontmanage.qb.from_(portal_user)
		.select(portal_user.parent)
		.where(portal_user.user == dontmanage.session.user)
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	if customers:
		return dontmanage.db.exists(doctype, get_customer_filter(doc, customers))
	elif suppliers:
		fieldname = "suppliers" if doctype == "Request for Quotation" else "supplier"
		return dontmanage.db.exists(doctype, {"name": doc.name, fieldname: ["in", suppliers]})
	else:
		return False
...
def add_role_for_portal_user(portal_user, role):
	...
	dontmanage.msgprint(_("Added {1} Role to User {0}.").format(dontmanage.bold(user_doc.name), role), alert=True)
