...
def customer_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	doctype = "Customer"
	conditions = []
	cust_master_name = dontmanage.defaults.get_user_default("cust_master_name")
...
def supplier_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	doctype = "Supplier"
	supp_master_name = dontmanage.defaults.get_user_default("supp_master_name")
...
def warehouse_query(doctype, txt, searchfield, start, page_len, filters):
	...
	query = """select `tabWarehouse`.`{warehouse_field}`,
		...
		start=start,
		page_len=page_len,
		txt=dontmanage.db.escape("%{0}%".format(txt)),
	)
