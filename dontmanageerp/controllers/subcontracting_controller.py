...
class SubcontractingController(StockController):
		...
		for item in self.items:
			...
			if not item.get("is_scrap_item"):
				...
				if item.bom:
					...
					dontmanage.throw(_("Row {0}: Please select a BOM for Item {1}.").format(item.idx, item.item_name))
			else:
				item.bom = None
	...
	def __get_materials_from_bom(self, item_code, bom_no, exploded_item=0):
		...
		return (
			dontmanage.get_all("BOM", fields=fields, filters=filters, order_by=f"`tab{doctype}`.`idx`") or []
		)
