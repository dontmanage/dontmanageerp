...
from dontmanageerp.stock.serial_batch_bundle import SerialBatchCreation, get_serial_nos_from_bundle
...
class SubcontractingController(StockController):
	...
	def validate_items(self):
		for item in self.items:
			is_stock_item, is_sub_contracted_item = dontmanage.get_value(
				"Item", item.item_code, ["is_stock_item", "is_sub_contracted_item"]
			)

			if not is_stock_item:
				dontmanage.throw(_("Row {0}: Item {1} must be a stock item.").format(item.idx, item.item_name))
			...
			if not item.get("is_scrap_item"):
				...
				if item.bom:
					is_active, bom_item = dontmanage.get_value("BOM", item.bom, ["is_active", "item"])
	...
	def __get_backflush_based_on(self):
		self.backflush_based_on = dontmanage.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

	def initialized_fields(self):
		self.available_materials = dontmanage._dict()
		self.__transferred_items = dontmanage._dict()
		self.alternative_item_details = dontmanage._dict()
	...
	def __get_received_items(self, doctype):
		...
		return dontmanage.get_all(f"{doctype}", fields=fields, filters=filters)
	...
	def __update_consumed_materials(self, doctype, return_consumed_items=False):
		...
		for row in consumed_materials:
			...
			consumed_bundles = voucher_bundle_data.get(bundle_key, dontmanage._dict())
	...
	def get_available_materials(self):
		...
		for row in transferred_items:
			...
			if row.serial_no:
				...
				bundle_data = voucher_bundle_data.get(bundle_key, dontmanage._dict())
	...
	def __remove_serial_and_batch_bundle(self, item):
		if item.serial_and_batch_bundle:
			dontmanage.delete_doc("Serial and Batch Bundle", item.serial_and_batch_bundle, force=True)
	...
	def __get_materials_from_bom(self, item_code, bom_no, exploded_item=0):
		...
		return dontmanage.get_all("BOM", fields=fields, filters=filters, order_by=f"`tab{doctype}`.`idx`") or []
	...
	def __get_qty_based_on_material_transfer(self, item_row, transfer_item):
		...
		if self.qty_to_be_received:
			...
			if transfer_item.serial_no or dontmanage.get_cached_value(
				"UOM", transfer_item.item_details.stock_uom, "must_be_whole_number"
			):
				return dontmanage.utils.ceil(qty)
	...
	def __set_rate_for_serial_and_batch_bundle(self):
		...
		for row in self.get(self.raw_material_table):
			...
			row.rate = dontmanage.get_cached_value(
	...
	def __validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.__transferred_items.get(key).get(
			...
			msg = f'The Batch No {dontmanage.bold(row.get("batch_no"))} has not supplied against the {self.subcontract_data.order_doctype} {link}'
			dontmanage.throw(_(msg), title=_("Incorrect Batch Consumed"))
	...
	def __validate_serial_no(self, row, key):
		if row.get("serial_and_batch_bundle") and self.__transferred_items.get(key).get("serial_no"):
			...
			if incorrect_sn:
				...
				dontmanage.throw(_(msg), title=_("Incorrect Serial Number Consumed"))
	...
	def update_ordered_and_reserved_qty(self):
		...
		for sco, sco_item_rows in sco_map.items():
			if sco and sco_item_rows:
				sco_doc = dontmanage.get_doc("Subcontracting Order", sco)
		...
		for item in self.get("supplied_items"):
			if item.reference_name == item_row_id:
				if (
					...
					and dontmanage.get_cached_value("Item", item.rm_item_code, "is_stock_item")
				):
...
def get_pending_sco_qty(po_name):
	...
	query = dontmanage.qb.from_(table).select(table.name, table.qty, table.sco_qty).where(table.parent == po_name)
...
def make_rm_stock_entry(
	...
	if subcontract_order:
		subcontract_order = dontmanage.get_doc(order_doctype, subcontract_order)

		if not rm_items:
			if not subcontract_order.supplied_items:
				dontmanage.throw(_("No item available for transfer."))
		...
		if fg_item_code_list:
			...
			dontmanage.throw(_("No Items selected for transfer."))
...
def get_materials_from_supplier(subcontract_order, rm_details, order_doctype="Subcontracting Order"):
	...
	doc = dontmanage.get_cached_doc(order_doctype, subcontract_order)
