...
from dontmanage.utils import cint, flt, round_based_on_smallest_currency_fraction
from dontmanage.utils.deprecations import deprecated

import dontmanageerp
from dontmanageerp.accounts.doctype.journal_entry.journal_entry import get_exchange_rate
from dontmanageerp.accounts.doctype.pricing_rule.utils import get_applied_pricing_rules
from dontmanageerp.controllers.accounts_controller import (
	validate_conversion_rate,
	validate_inclusive_tax,
	validate_taxes_and_charges,
)
from dontmanageerp.stock.get_item_details import _get_item_tax_template, get_item_tax_map
...
class calculate_taxes_and_totals:
	def __init__(self, doc: Document):
		...
		dontmanage.flags.round_row_wise_tax = dontmanage.db.get_single_value(
			"Accounts Settings", "round_row_wise_tax"
		)

		self._items = self.filter_rows() if self.doc.doctype == "Quotation" else self.doc.get("items")

		get_round_off_applicable_accounts(self.doc.company, dontmanage.flags.round_off_applicable_accounts)
		self.calculate()
	...
	def validate_conversion_rate(self):
		...
		company_currency = dontmanageerp.get_company_currency(self.doc.company)
	...
	def calculate_shipping_charges(self):
		...
		if hasattr(self.doc, "shipping_rule") and self.doc.shipping_rule:
			shipping_rule = dontmanage.get_doc("Shipping Rule", self.doc.shipping_rule)
	...
	def round_off_totals(self, tax):
		if tax.account_head in dontmanage.flags.round_off_applicable_accounts:
	...
	def apply_discount_amount(self):
		if self.doc.discount_amount:
			if not self.doc.apply_discount_on:
				dontmanage.throw(_("Please select Apply Discount On"))
	...
	def calculate_outstanding_amount(self):
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			if (
				...
				write_off_limit = flt(
					dontmanage.db.get_value("POS Profile", self.doc.pos_profile, "write_off_limit")
				)
	...
	def set_total_amount_to_default_mop(self, total_amount_to_pay):
		...
		if pending_amount > 0:
			default_mode_of_payment = dontmanage.db.get_value(
				"POS Payment Method",
	...
	for tax in taxes:
		...
		if item_tax_map:
			for item_code, tax_data in item_tax_map.items():
				...
				itemised_tax[item_code][tax.description] = dontmanage._dict(
...
def get_itemised_taxable_amount(items):
	itemised_taxable_amount = dontmanage._dict()
	for item in items:
...
def get_rounding_tax_settings():
	return dontmanage.db.get_single_value("Accounts Settings", "round_row_wise_tax")
	...
	def set_account_currency(self):
		...
		for d in self.doc.get(self.tax_field):
			if not d.account_currency:
				account_currency = dontmanage.get_cached_value("Account", d.expense_account, "account_currency")
