...
class AccountsController(TransactionBase):
			...
			if invalid_advances := [
				...
				dontmanage.throw(
					_(
						...
						dontmanage.bold(comma_and([x.idx for x in invalid_advances])), dontmanage.bold(_("Advance Payments"))
					)
				)
	...
	def validate_return_against_account(self):
		if (
			...
			if dontmanage.get_value(self.doctype, self.return_against, cr_dr_account_field) != cr_dr_account:
				dontmanage.throw(
					_("'{0}' account: '{1}' should match the Return Against Invoice").format(
						dontmanage.bold(cr_dr_account_label), dontmanage.bold(cr_dr_account)
					)
				)
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center", self.get("cost_center") or dontmanageerp.get_default_cost_center(self.company)
						)
	...
	def is_inclusive_tax(self):
		is_inclusive = cint(
			dontmanage.db.get_single_value("Accounts Settings", "show_inclusive_tax_in_print")
		)
					...
					for arg in args:
						...
						if (
							...
							if not self.gain_loss_journal_already_booked(
								...
								posting_date = dontmanage.db.get_value(arg.voucher_type, arg.voucher_no, "posting_date")
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		...
		role_allowed_to_over_bill = dontmanage.db.get_single_value(
	...
	def set_total_advance_paid(self):
		...
		if advance:
			...
			dontmanage.db.set_value(self.doctype, self.name, "party_account_currency", advance.account_currency)
			...
			dontmanage.db.set_value(self.doctype, self.name, "advance_paid", advance_paid)
	...
	def raise_missing_debit_credit_account_error(self, party_type, party):
		...
		db_or_cr = (
			dontmanage.bold("Debit To") if self.doctype == "Sales Invoice" else dontmanage.bold("Credit To")
		)
		...
		message = _("{0} Account not found against Customer {1}.").format(
			db_or_cr, dontmanage.bold(party) or ""
		)
	...
	def create_advance_and_reconcile(self, party_link):
		...
		reconcilation_entry.cost_center = self.cost_center or dontmanageerp.get_default_cost_center(
			self.company
		)
	...
	def repost_accounting_entries(self):
		if self.repost_required:
			...
			dontmanage.throw(_("No updates pending for reposting"))
		...
		if not child_item.warehouse:
			dontmanage.throw(
				_("Cannot find {} for item {}. Please set the same in Item Master or Stock Settings.").format(
					dontmanage.bold("default warehouse"), dontmanage.bold(item.item_code)
				)
			)
