...
import dontmanage
from dontmanage import _, bold, qb, throw
from dontmanage.model.workflow import get_workflow_name, is_transition_condition_satisfied
from dontmanage.query_builder import Criterion
from dontmanage.query_builder.custom import ConstantColumn
from dontmanage.query_builder.functions import Abs, Sum
...
from dontmanageerp.accounts.general_ledger import get_round_off_account_and_cost_center
from dontmanageerp.accounts.party import (
	get_party_account,
...
from dontmanageerp.buying.utils import update_last_purchase_rate
from dontmanageerp.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from dontmanageerp.controllers.sales_and_purchase_return import validate_return
from dontmanageerp.exceptions import InvalidCurrency
from dontmanageerp.setup.utils import get_exchange_rate
from dontmanageerp.stock.doctype.item.item import get_uom_conv_factor
from dontmanageerp.stock.doctype.packed_item.packed_item import make_packing_list
from dontmanageerp.stock.get_item_details import (
...
class AccountMissingError(dontmanage.ValidationError):
	pass


class InvalidQtyError(dontmanage.ValidationError):
	pass
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = dontmanageerp.get_company_currency(self.company)
	...
	def onload(self):
		self.set_onload(
			...
			dontmanage.db.get_single_value("Accounts Settings", "make_payment_via_journal_entry"),
		)
	...
	def ensure_supplier_is_not_blocked(self):
		...
		if is_buying_invoice or is_supplier_payment:
			...
			supplier = dontmanage.get_doc("Supplier", supplier_name)
			...
			if invalid_advances := [x for x in self.advances if not x.reference_type or not x.reference_name]:
				dontmanage.throw(
					_(
						...
						dontmanage.bold(comma_and([x.idx for x in invalid_advances])),
						dontmanage.bold(_("Advance Payments")),
					)
				)
				...
				if self.get("update_outstanding_for_self"):
					...
					dontmanage.msgprint(
						_(
							...
							dontmanage.bold(document_type),
							get_link_to_form(self.doctype, self.get("return_against")),
							dontmanage.bold(_("Update Outstanding for Self")),
	...
	def set_default_letter_head(self):
		if hasattr(self, "letter_head") and not self.letter_head:
			self.letter_head = dontmanage.db.get_value("Company", self.company, "default_letter_head")
	...
	def _remove_references_in_unreconcile(self):
		upe = dontmanage.qb.DocType("Unreconcile Payment Entries")
		rows = (
			dontmanage.qb.from_(upe)
		...
		if rows:
			references_map = dontmanage._dict()
			for x in rows:
				references_map.setdefault(x.parent, []).append(x.name)

			for doc, rows in references_map.items():
				unreconcile_doc = dontmanage.get_doc("Unreconcile Payment", doc)
				for row in rows:
		...
		unreconcile_docs = dontmanage.db.get_all("Unreconcile Payment", filters={"voucher_no": self.name})
		for x in unreconcile_docs:
			_doc = dontmanage.get_doc("Unreconcile Payment", x.name)
			if _doc.docstatus == 1:
				_doc.cancel()
			_doc.delete()
	...
	def _remove_references_in_repost_doctypes(self):
		...
		for _doctype in repost_doctypes:
			dt = dontmanage.qb.DocType(_doctype)
			rows = (
			...
			if rows:
				...
				for doc, rows in references_map.items():
					repost_doc = dontmanage.get_doc(doc[0], doc[1])
	...
	def _remove_advance_payment_ledger_entries(self):
		...
		advance_payment_doctypes = dontmanage.get_hooks("advance_payment_doctypes")
	...
	def on_trash(self):
		from dontmanageerp.accounts.utils import delete_exchange_gain_loss_journal
		...
		if dontmanage.db.get_single_value("Accounts Settings", "delete_linked_ledger_entries"):
			...
			dontmanage.qb.from_(ple).delete().where(
	...
	def validate_return_against_account(self):
		if self.doctype in ["Sales Invoice", "Purchase Invoice"] and self.is_return and self.return_against:
			...
			original_account = dontmanage.get_value(self.doctype, self.return_against, cr_dr_account_field)
			if original_account != self.get(cr_dr_account_field):
				dontmanage.throw(
					_(
						...
						dontmanage.bold(_(self.meta.get_label(cr_dr_account_field), context=self.doctype)),
						dontmanage.bold(original_account),
						dontmanage.bold(self.return_against),
					)
				)
		...
		for item in self.get("items"):
			if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
				if not item.get(field_map.get(self.doctype)):
					default_deferred_account = dontmanage.get_cached_value(
	...
	def validate_auto_repeat_subscription_dates(self):
		if self.get("from_date") and self.get("to_date") and getdate(self.from_date) > getdate(self.to_date):
			dontmanage.throw(_("To Date cannot be before From Date"), title=_("Invalid Auto Repeat Date"))
	...
	def calculate_taxes_and_totals(self):
		from dontmanageerp.controllers.taxes_and_totals import calculate_taxes_and_totals
			...
			if item.get(item_field) == self.get(party_account_field):
				dontmanage.throw(
					_("Row {0}: {1} {2} cannot be same as {3} (Party Account) {4}").format(
						item.idx,
						dontmanage.bold(dontmanage.unscrub(item_field)),
						item.get(item_field),
						dontmanage.bold(dontmanage.unscrub(party_account_field)),
	...
	def validate_due_date(self):
		...
		from dontmanageerp.accounts.party import validate_due_date
		...
		if dontmanage.flags.in_import and getdate(self.due_date) < getdate(posting_date):
			self.due_date = posting_date

		elif self.doctype == "Sales Invoice":
			if not self.due_date:
				dontmanage.throw(_("Due Date is mandatory"))
	...
	def set_price_list_currency(self, buying_or_selling):
		...
		if self.meta.get_field("currency"):
			...
			if self.meta.get_field(fieldname) and self.get(fieldname):
				self.price_list_currency = dontmanage.db.get_value("Price List", self.get(fieldname), "currency")
			...
			if (
				...
				and dontmanage.db.get_single_value("Buying Settings", "use_transaction_date_exchange_rate")
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or dontmanageerp.get_default_cost_center(self.company),
						)
	...
	def apply_pricing_rule_on_items(self, item, pricing_rule_args):
		if not pricing_rule_args.get("validate_applied_rule", 0):
			...
			for pricing_rule in get_applied_pricing_rules(item.get("pricing_rules")):
				pricing_rule_doc = dontmanage.get_cached_doc("Pricing Rule", pricing_rule)
					...
					if item.get(field) < pricing_rule_doc.get(field):
						...
						dontmanage.msgprint(
							_("Row {0}: user has not applied the rule {1} on the item {2}").format(
								item.idx, dontmanage.bold(title), dontmanage.bold(item.item_code)
							)
						)
	...
	def set_taxes(self):
		...
		if (self.is_new() or self.is_pos_profile_changed()) and not self.get("taxes"):
			if self.company and not self.get("taxes_and_charges"):
				...
				self.taxes_and_charges = dontmanage.db.get_value(
	...
	def is_pos_profile_changed(self):
		if (
			...
			and self.is_pos
			and self.pos_profile != dontmanage.db.get_value("Sales Invoice", self.name, "pos_profile")
		):
			return True
	...
	def append_taxes_from_item_tax_template(self):
		if not dontmanage.db.get_single_value("Accounts Settings", "add_taxes_from_item_tax_template"):
			return
	...
	def validate_enabled_taxes_and_charges(self):
		...
		if self.taxes_and_charges and dontmanage.get_cached_value(
			taxes_and_charges_doctype, self.taxes_and_charges, "disabled"
		):
			dontmanage.throw(_("{0} '{1}' is disabled").format(taxes_and_charges_doctype, self.taxes_and_charges))

	def validate_tax_account_company(self):
		for d in self.get("taxes"):
			if d.account_head:
				tax_account_company = dontmanage.get_cached_value("Account", d.account_head, "company")
	...
	def get_gl_dict(self, args, account_currency=None, item=None):
		...
		dimension_dict = dontmanage._dict()
	...
	def get_voucher_subtype(self):
		...
		for method_name in dontmanage.get_hooks("voucher_subtypes"):
			voucher_subtype = dontmanage.get_attr(method_name)(self)
	...
	def validate_zero_qty_for_return_invoices_with_stock(self):
		...
		if rows:
			dontmanage.throw(
				...
				).format(dontmanage.bold(comma_and(["#" + str(x.idx) for x in rows])))
			)
	...
	def clear_unallocated_advances(self, childtype, parentfield):
		...
		doctype = dontmanage.qb.DocType(childtype)
		dontmanage.qb.from_(doctype).delete().where(
	...
	def apply_shipping_rule(self):
		if self.shipping_rule:
			shipping_rule = dontmanage.get_doc("Shipping Rule", self.shipping_rule)
		...
		for fieldname in ("shipping_address_name", "shipping_address"):
			...
			if shipping_field and shipping_field.fieldtype == "Link":
				if self.get(fieldname):
					return dontmanage.get_doc("Address", self.get(fieldname))
	...
	def is_inclusive_tax(self):
		is_inclusive = cint(dontmanage.db.get_single_value("Accounts Settings", "show_inclusive_tax_in_print"))
	...
	def should_show_taxes_as_table_in_print(self):
		return cint(dontmanage.db.get_single_value("Accounts Settings", "show_taxes_as_table_in_print"))
	...
	def gain_loss_journal_already_booked(
		...
		if res := dontmanage.db.get_all(
			...
			if exc_vouchers := dontmanage.db.get_all(
				"Journal Entry",
	...
	def make_exchange_gain_loss_journal(
		...
		if self.docstatus == 1:
			if dimensions_dict is None:
				dimensions_dict = dontmanage._dict()
					...
					for arg in args:
						...
						if (
							...
							if not self.gain_loss_journal_already_booked(
								...
								posting_date = arg.get("difference_posting_date") or dontmanage.db.get_value(
			...
			if self.get("doctype") == "Payment Entry":
				...
				for d in gain_loss_to_book:
					...
					if d.exchange_gain_loss and (
						...
						gain_loss_account = dontmanage.get_cached_value(
	...
	def is_payable_account(self, reference_doctype, account):
		if reference_doctype == "Purchase Invoice" or (
			...
			and dontmanage.get_cached_value("Account", account, "account_type") == "Payable"
		):
			return True
		return False
	...
	def update_against_document_in_jv(self):
				...
				args = dontmanage._dict(
					{
						...
						"difference_account": dontmanage.get_cached_value(
							"Company", self.company, "exchange_gain_loss_account"
						),
		...
		if lst:
			from dontmanageerp.accounts.utils import reconcile_against_document
	...
	def cancel_system_generated_credit_debit_notes(self):
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			for x in journals:
				dontmanage.get_doc("Journal Entry", x).cancel()

	def on_cancel(self):
		from dontmanageerp.accounts.doctype.bank_transaction.bank_transaction import (
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
			...
			if dontmanage.db.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
	...
	def unlink_ref_doc_from_po(self):
		...
		if linked_po:
			...
			dontmanage.msgprint(_("Purchase Orders {0} are un-linked").format("\n".join(linked_po)))
	...
	def make_discount_gl_entries(self, gl_entries):
		if self.doctype == "Purchase Invoice":
			enable_discount_accounting = cint(
				dontmanage.db.get_single_value("Buying Settings", "enable_discount_accounting")
			)
		elif self.doctype == "Sales Invoice":
			enable_discount_accounting = cint(
				dontmanage.db.get_single_value("Selling Settings", "enable_discount_accounting")
			)
				...
				self.get_gl_dict(
					{
						...
						"cost_center": self.cost_center or dontmanageerp.get_default_cost_center(self.company),
					},
					item=self,
				)
			)
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from dontmanageerp.controllers.status_updater import get_allowance_for

		item_allowance = {}
		global_qty_allowance, global_amount_allowance = None, None

		role_allowed_to_over_bill = dontmanage.get_cached_value(
	...
	def get_billed_amount_for_item(self, item, item_ref_dn, based_on):
		...
		item_doctype = dontmanage.qb.DocType(item.doctype)
		based_on_field = dontmanage.qb.Field(based_on)
		join_field = dontmanage.qb.Field(item_ref_dn)

		result = (
			dontmanage.qb.from_(item_doctype)
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from dontmanageerp.accounts.utils import get_company_default
	...
	def get_stock_items(self):
		...
		if item_codes:
			stock_items = dontmanage.db.get_values(
	...
	def calculate_total_advance_from_ledger(self):
		adv = dontmanage.qb.DocType("Advance Payment Ledger Entry")
		advance = (
			dontmanage.qb.from_(adv)
	...
	def company_abbr(self):
		if not hasattr(self, "_abbr"):
			self._abbr = dontmanage.get_cached_value("Company", self.company, "abbr")
	...
	def raise_missing_debit_credit_account_error(self, party_type, party):
		...
		db_or_cr = (
			dontmanage.bold(_("Debit To")) if self.doctype == "Sales Invoice" else dontmanage.bold(_("Credit To"))
		)
		rec_or_pay = "Receivable" if self.doctype == "Sales Invoice" else "Payable"

		link_to_party = dontmanage.utils.get_link_to_form(party_type, party)
		link_to_company = dontmanage.utils.get_link_to_form("Company", self.company)

		message = _("{0} Account not found against Customer {1}.").format(db_or_cr, dontmanage.bold(party) or "")
		...
		dontmanage.throw(message, title=_("Account Missing"), exc=AccountMissingError)
	...
	def validate_party_account_currency(self):
		...
		allow_multi_currency_invoices_against_single_party_account = dontmanage.db.get_singles_value(
		...
		if (
			...
			dontmanage.throw(
				_("Party Account {0} currency ({1}) and document currency ({2}) should be same").format(
					dontmanage.bold(party_account), party_account_currency, self.currency
				)
			)
	...
	def delink_advance_entries(self, linked_doc_name):
		...
		for adv in self.advances:
			...
			if adv.reference_name == linked_doc_name:
				doctype = dontmanage.qb.DocType(self.doctype + " Advance")
				dontmanage.qb.from_(doctype).delete().where(doctype.name == adv.name).run()
	...
	def group_similar_items(self):
		...
		fields_to_group = dontmanage.get_hooks("fields_for_group_similar_items")
	...
	def set_payment_schedule(self):
		...
		if self.doctype in ("Sales Invoice", "Purchase Invoice"):
			...
			automatically_fetch_payment_terms = cint(
				dontmanage.db.get_single_value("Accounts Settings", "automatically_fetch_payment_terms")
			)
		...
		allocate_payment_based_on_payment_terms = dontmanage.db.get_value(
	...
	def linked_order_has_payment_terms_template(self, po_or_so, doctype):
		return dontmanage.get_value(doctype, po_or_so, "payment_terms_template")

	def linked_order_has_payment_schedule(self, po_or_so):
		return dontmanage.get_all("Payment Schedule", filters={"parent": po_or_so})

	def fetch_payment_terms_from_order(self, po_or_so, po_or_so_doctype):
		"""
		Fetch Payment Terms from Purchase/Sales Order on creating a new Purchase/Sales Invoice.
		"""
		po_or_so = dontmanage.get_cached_doc(po_or_so_doctype, po_or_so)
	...
	def validate_payment_schedule_dates(self):
		...
		if li:
			...
			dontmanage.throw(_("Rows with duplicate due dates in other rows were found: {0}").format(duplicates))
	...
	def is_rounded_total_disabled(self):
		if self.meta.get_field("disable_rounded_total"):
			...
			return dontmanage.db.get_single_value("Global Defaults", "disable_rounded_total")
	...
	def set_inter_company_account(self):
		...
		if self.is_internal_transfer() and not self.unrealized_profit_loss_account:
			unrealized_profit_loss_account = dontmanage.get_cached_value(
	...
	def process_common_party_accounting(self):
		...
		if dontmanage.db.get_single_value("Accounts Settings", "enable_common_party_accounting"):
	...
	def create_advance_and_reconcile(self, party_link):
		...
		default_currency = dontmanageerp.get_company_currency(self.company)
		...
		reconcilation_entry = dontmanage._dict()
		advance_entry = dontmanage._dict()
		...
		reconcilation_entry.cost_center = self.cost_center or dontmanageerp.get_default_cost_center(self.company)

		advance_entry.account = primary_account
		advance_entry.party_type = primary_party_type
		advance_entry.party = primary_party
		advance_entry.cost_center = self.cost_center or dontmanageerp.get_default_cost_center(self.company)
	...
	def repost_accounting_entries(self):
		repost_ledger = dontmanage.new_doc("Repost Accounting Ledger")
	...
	def get_advance_payment_doctypes(self) -> list:
		return dontmanage.get_hooks("advance_payment_doctypes")
	...
	def make_advance_payment_ledger_for_journal(self):
		...
		for x in advance_doctype_references:
			...
			if amount > 0:
				doc = dontmanage.new_doc("Advance Payment Ledger Entry")
...
def get_tax_rate(account_head):
	return dontmanage.get_cached_value("Account", account_head, ["tax_rate", "account_name"], as_dict=True)
...
def get_default_taxes_and_charges(master_doctype, tax_template=None, company=None):
	...
	if tax_template and company:
		tax_template_company = dontmanage.get_cached_value(master_doctype, tax_template, "company")
		if tax_template_company == company:
			return

	default_tax = dontmanage.db.get_value(master_doctype, {"is_default": 1, "company": company})
...
def get_taxes_and_charges(master_doctype, master_name):
	...
	tax_master = dontmanage.get_doc(master_doctype, master_name)
...
def validate_conversion_rate(currency, conversion_rate, conversion_rate_label, company):
	...
	company_currency = dontmanage.get_cached_value("Company", company, "default_currency")
...
def validate_account_head(idx, account, company, context=""):
	account_company = dontmanage.get_cached_value("Account", account, "company")
	is_group = dontmanage.get_cached_value("Account", account, "is_group")

	if account_company != company:
		dontmanage.throw(
			_("Row {0}: {3} Account {1} does not belong to Company {2}").format(
				idx, dontmanage.bold(account), dontmanage.bold(company), context
			),
	...
	if is_group:
		dontmanage.throw(
			_("Row {0}: Account {1} is a Group Account").format(idx, dontmanage.bold(account)),
			title=_("Invalid Account"),
		)
...
def validate_cost_center(tax, doc):
	...
	company = dontmanage.get_cached_value("Cost Center", tax.cost_center, "company")

	if company != doc.company:
		dontmanage.throw(
			_("Row {0}: Cost Center {1} does not belong to Company {2}").format(
				tax.idx, dontmanage.bold(tax.cost_center), dontmanage.bold(doc.company)
			),
...
def validate_inclusive_tax(tax, doc):
	...
	if cint(getattr(tax, "included_in_print_rate", None)):
		if tax.charge_type == "Actual":
			...
			dontmanage.throw(_("Valuation type charges can not be marked as Inclusive"))
...
def get_advance_journal_entries(
	...
	journal_entry = dontmanage.qb.DocType("Journal Entry")
	journal_acc = dontmanage.qb.DocType("Journal Entry Account")
	q = (
		dontmanage.qb.from_(journal_entry)
...
def get_advance_payment_entries(
	...
	payment_entries = []
	payment_entry = dontmanage.qb.DocType("Payment Entry")
	...
	if order_list or against_all_orders:
		...
		payment_ref = dontmanage.qb.DocType("Payment Entry Reference")
...
def get_common_query(
	...
	account_type = dontmanage.db.get_value("Party Type", party_type, "account_type")
	payment_type = "Receive" if account_type == "Receivable" else "Pay"
	payment_entry = dontmanage.qb.DocType("Payment Entry")

	q = (
		dontmanage.qb.from_(payment_entry)
		.select(
...
def update_invoice_status():
	...
	today = getdate()
	payment_schedule = dontmanage.qb.DocType("Payment Schedule")
	for doctype in ("Sales Invoice", "Purchase Invoice"):
		invoice = dontmanage.qb.DocType(doctype)
		...
		payable_amount = (
			dontmanage.qb.from_(payment_schedule)
		...
		total_amount = dontmanage.qb.terms.Case().when(consider_base_amount, base_total).else_(total)
		...
		dontmanage.qb.update(invoice).set("status", status).where(conditions).run()
...
def get_payment_terms(
	...
	terms_doc = dontmanage.get_doc("Payment Terms Template", terms_template)
...
def get_payment_term_details(
	...
	term_details = dontmanage._dict()
	if isinstance(term, str):
		term = dontmanage.get_doc("Payment Term", term)
	else:
...
def get_supplier_block_status(party_name):
	...
	a `Supplier`
	"""
	supplier = dontmanage.get_doc("Supplier", party_name)
	info = {
...
def add_taxes_from_tax_template(child_item, parent_doc, db_insert=True):
	add_taxes_from_item_tax_template = dontmanage.db.get_single_value(
...
def set_order_defaults(parent_doctype, parent_doctype_name, child_doctype, child_docname, trans_item):
	...
	"""
	p_doc = dontmanage.get_doc(parent_doctype, parent_doctype_name)
	child_item = dontmanage.new_doc(child_doctype, parent_doc=p_doc, parentfield=child_docname)
	item = dontmanage.get_doc("Item", trans_item.get("item_code"))
	...
	if child_doctype == "Sales Order Item":
		...
		if not child_item.warehouse:
			dontmanage.throw(
				...
				).format(dontmanage.bold(item.item_code))
			)
...
def update_bin_on_delete(row, doctype):
	...
	from dontmanageerp.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	def validate_workflow_conditions(doc):
		...
		workflow_doc = dontmanage.get_doc("Workflow", workflow)
		...
		if not flt(new_data.get("qty")):
			dontmanage.throw(
				_("Row # {0}: Quantity for Item {1} cannot be zero").format(
					new_data.get("idx"), dontmanage.bold(new_data.get("item_code"))
				),
				title=_("Invalid Qty"),
			)

		if parent_doctype == "Sales Order" and flt(new_data.get("qty")) < flt(child_item.delivered_qty):
			dontmanage.throw(_("Cannot set quantity less than delivered quantity"))

		if parent_doctype == "Purchase Order" and flt(new_data.get("qty")) < flt(child_item.received_qty):
			dontmanage.throw(_("Cannot set quantity less than received quantity"))
	...
	def should_update_supplied_items(doc) -> bool:
		...
		if update_supplied_items and supplied_items_processed:
			dontmanage.throw(_("Item qty can not be updated as raw materials are already processed."))
	...
	def validate_fg_item_for_subcontracting(new_data, is_new):
		if is_new:
			if not new_data.get("fg_item"):
				...
				is_sub_contracted_item, default_bom = dontmanage.db.get_value(
				...
				if not is_sub_contracted_item:
					...
					dontmanage.throw(_("Default BOM not found for FG Item {0}").format(new_data["fg_item"]))

		if not new_data.get("fg_item_qty"):
			dontmanage.throw(_("Finished Good Item {0} Qty can not be zero").format(new_data["fg_item"]))
	...
	parent = dontmanage.get_doc(parent_doctype, parent_doctype_name)
	...
	for d in data:
		...
		if not d.get("docname"):
			...
			check_doc_permissions(parent, "write")
			child_item = dontmanage.get_doc(parent_doctype + " Item", d.get("docname"))
	...
	dontmanage.get_doc("Authorization Control").validate_approving_authority(
			...
			if parent.is_old_subcontracting_flow:
				...
				if not parent.can_update_items():
					dontmanage.throw(
						...
						).format(dontmanage.bold(parent.name))
					)
	else:  # Sales Order
...
def merge_taxes(source_taxes, target_doc):
	from dontmanageerp.accounts.doctype.pos_invoice_merge_log.pos_invoice_merge_log import (
