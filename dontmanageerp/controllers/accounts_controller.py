...
from dontmanageerp.accounts.utils import get_account_currency, get_fiscal_years, validate_fiscal_year
...
class AccountsController(TransactionBase):
	...
	def on_trash(self):
		...
		rpi = dontmanage.qb.DocType("Repost Payment Ledger Items")
		dontmanage.qb.from_(rpi).delete().where(
	...
	def validate_enabled_taxes_and_charges(self):
		...
		if dontmanage.get_cached_value(taxes_and_charges_doctype, self.taxes_and_charges, "disabled"):
			dontmanage.throw(
	...
	def validate_qty_is_not_zero(self):
		if self.doctype != "Purchase Receipt":
			for item in self.items:
				if not item.qty:
					dontmanage.throw(_("Item quantity can not be zero"))
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on, parentfield):
		...
		for item in self.get("items"):
			...
			ref_amt = flt(
				dontmanage.db.get_value(ref_dt + " Item", item.get(item_ref_dn), based_on),
...
def get_advance_payment_entries(
	...
	if order_list or against_all_orders:
		...
		payment_entries_against_order = dontmanage.db.sql(
			"""
			select
	...
	if include_unallocated:
		unallocated_payment_entries = dontmanage.db.sql(
			"""
...
def set_order_defaults(
	...
	child_item = dontmanage.new_doc(child_doctype, p_doc, child_docname)
