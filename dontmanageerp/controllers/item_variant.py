...
from dontmanageerp.utilities.product import get_item_codes_by_attributes


class ItemVariantExistsError(dontmanage.ValidationError):
	pass


class InvalidItemAttributeValueError(dontmanage.ValidationError):
	pass


class ItemTemplateCannotHaveStock(dontmanage.ValidationError):
	pass
...
def get_variant(template, args=None, variant=None, manufacturer=None, manufacturer_part_no=None):
	...
	"""
	item_template = dontmanage.get_doc("Item", template)
	...
	if not attribute_args:
		dontmanage.throw(_("Please specify at least one attribute in the Attributes table"))
...
def make_variant_based_on_manufacturer(template, manufacturer, manufacturer_part_no):
	...
	variant = dontmanage.new_doc("Item")
	...
	if not dontmanage.db.exists("Item Manufacturer", {"item_code": variant.name, "manufacturer": manufacturer}):
		manufacturer_doc = dontmanage.new_doc("Item Manufacturer")
...
def validate_item_variant_attributes(item, args=None):
	if isinstance(item, str):
		item = dontmanage.get_doc("Item", item)
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if increment == 0:
		...
		dontmanage.throw(_("Increment for Attribute {0} cannot be 0").format(attribute))
...
def validate_item_attribute_value(attributes_list, attribute, attribute_value, item, from_variant=True):
	allow_rename_attribute_value = dontmanage.db.get_single_value(
		...
		if from_variant:
			dontmanage.throw(
				_("{0} is not a valid Value for Attribute {1} of Item {2}.").format(
					dontmanage.bold(attribute_value), dontmanage.bold(attribute), dontmanage.bold(item)
				),
				InvalidItemAttributeValueError,
			...
			msg = _("The value {0} is already assigned to an existing Item {1}.").format(
				dontmanage.bold(attribute_value), dontmanage.bold(item)
			)
			msg += "<br>" + _(
				"To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings."
			).format(dontmanage.bold(_("Allow Rename Attribute Value")))

			dontmanage.throw(msg, InvalidItemAttributeValueError, title=_("Edit Not Allowed"))


def get_attribute_values(item):
	if not dontmanage.flags.attribute_values:
		attribute_values = {}
		numeric_values = {}
		for t in dontmanage.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
			attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

		for t in dontmanage.get_all(
		...
		dontmanage.flags.attribute_values = attribute_values
		dontmanage.flags.numeric_values = numeric_values

	return dontmanage.flags.attribute_values, dontmanage.flags.numeric_values
...
def find_variant(template, args, variant_item_code=None):
	...
	for variant in possible_variants:
		variant = dontmanage.get_doc("Item", variant)
...
def create_variant(item, args, use_template_image=False):
	use_template_image = dontmanage.parse_json(use_template_image)
	if isinstance(args, str):
		args = json.loads(args)

	template = dontmanage.get_doc("Item", item)
	variant = dontmanage.new_doc("Item")
...
def enqueue_multiple_variant_creation(item, args, use_template_image=False):
	...
	if total_variants >= 600:
		dontmanage.throw(_("Please do not create more than 500 items at a time"))
		return
	if total_variants < 10:
		return create_multiple_variants(item, args, use_template_image)
	else:
		dontmanage.enqueue(
			"dontmanageerp.controllers.item_variant.create_multiple_variants",
			item=item,
			args=args,
...
def create_multiple_variants(item, args, use_template_image=False):
	...
	template_item = dontmanage.get_doc("Item", item)
...
def copy_attributes_to_variant(item, variant):
	...
	allow_fields = [d.field_name for d in dontmanage.get_all("Variant Field", fields=["field_name"])]
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		item_attribute = dontmanage.db.sql(
		...
		if not item_attribute:
			continue
			# dontmanage.throw(_('Invalid attribute {0} {1}').format(dontmanage.bold(attr.attribute),
			# 	dontmanage.bold(attr.attribute_value)), title=_('Invalid Attribute'),
...
def create_variant_doc_for_quick_entry(template, args):
	variant_based_on = dontmanage.db.get_value("Item", template, "variant_based_on")
	args = json.loads(args)
