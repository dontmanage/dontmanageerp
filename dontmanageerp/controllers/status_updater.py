...
def validate_status(status, options):
	if status not in options:
		dontmanage.throw(_("Status must be one of {0}").format(comma_or(options)))
...
class StatusUpdater(Document):
		...
		for args in self.status_updater:
			...
			for d in self.get_all_children():
				if hasattr(d, "qty") and d.qty < 0 and not self.get("is_return"):
					dontmanage.throw(_("For an item {0}, quantity must be positive number").format(d.item_code))
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role_allowed_to_over_deliver_receive = dontmanage.db.get_single_value(
		...
		if overflow_percent - allowance > 0.01:
			...
			if role not in dontmanage.get_roles():
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		dontmanage.throw(
			_(
				...
				dontmanage.bold(_(item["target_ref_field"].title())),
				dontmanage.bold(item["reduce_by"]),
				dontmanage.bold(_(args.get("target_dt"))),
				dontmanage.bold(_(self.doctype)),
				dontmanage.bold(item.get("item_code")),
			)
			+ "<br><br>"
			+ action_msg,
			OverAllowanceError,
	...
	def _update_children(self, args, update_modified):
		...
		for d in self.get_all_children():
			...
			if (
				...
				args["second_source_condition"] = dontmanage.db.sql(
	...
	def _update_percent_field(self, args, update_modified=True):
		...
		if args.get("target_parent_field"):
			...
			if update_modified:
				target = dontmanage.get_doc(args["target_parent_dt"], args["name"])
	...
	def _update_modified(self, args, update_modified):
		...
		args["update_modified"] = ", modified = {}, modified_by = {}".format(
			dontmanage.db.escape(now()), dontmanage.db.escape(dontmanage.session.user)
		)

	def update_billing_status_for_zero_amount_refdoc(self, ref_dt):
		ref_fieldname = dontmanage.scrub(ref_dt)

		ref_docs = [item.get(ref_fieldname) for item in (self.get("items") or []) if item.get(ref_fieldname)]
		if not ref_docs:
			return

		zero_amount_refdocs = dontmanage.db.sql_list(
			f"""
			SELECT
				name
			from
	...
	def update_billing_status(self, zero_amount_refdoc, ref_dt, ref_fieldname):
		for ref_dn in zero_amount_refdoc:
			...
			ref_doc = dontmanage.get_doc(ref_dt, ref_dn)
...
def get_allowance_for(
	...
	if item_allowance is None:
		item_allowance = {}
	if qty_or_amount == "qty":
		if item_allowance.get(item_code, dontmanage._dict()).get("qty"):
			return (
		...
		if item_allowance.get(item_code, dontmanage._dict()).get("amount"):
			return (
	...
	qty_allowance, over_billing_allowance = dontmanage.get_cached_value(
	...
	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance is None:
			global_qty_allowance = flt(
				dontmanage.get_cached_value("Stock Settings", None, "over_delivery_receipt_allowance")
			)
		qty_allowance = global_qty_allowance
	elif qty_or_amount == "amount" and not over_billing_allowance:
		if global_amount_allowance is None:
			global_amount_allowance = flt(
				dontmanage.get_cached_value("Accounts Settings", None, "over_billing_allowance")
			)
		over_billing_allowance = global_amount_allowance

	if qty_or_amount == "qty":
		allowance = qty_allowance
		item_allowance.setdefault(item_code, dontmanage._dict()).setdefault("qty", qty_allowance)
	else:
		allowance = over_billing_allowance
		item_allowance.setdefault(item_code, dontmanage._dict()).setdefault("amount", over_billing_allowance)
