...
class QualityInspectionRequiredError(dontmanage.ValidationError):
	pass


class QualityInspectionRejectedError(dontmanage.ValidationError):
	pass


class QualityInspectionNotSubmittedError(dontmanage.ValidationError):
	pass


class BatchExpiredError(dontmanage.ValidationError):
	pass
...
class StockController(AccountsController):
	...
	def validate_items_exist(self):
		...
		exists_items = dontmanage.get_all("Item", filters={"name": ("in", items)}, pluck="name")
		non_exists_items = set(items) - set(exists_items)

		if non_exists_items:
			dontmanage.throw(_("Items {0} do not exist in the Item master.").format(", ".join(non_exists_items)))
	...
	def validate_serialized_batch(self):
		...
		for d in self.get("items"):
			...
			if flt(d.qty) > 0.0 and d.get("batch_no") and self.get("posting_date") and self.docstatus < 2:
				expiry_date = dontmanage.get_cached_value("Batch", d.get("batch_no"), "expiry_date")
	...
	def clean_serial_nos(self):
		from dontmanageerp.stock.doctype.serial_no.serial_no import clean_serial_no_string
				...
				if not data.get("qty"):
					dontmanage.throw(
						_("For the {0}, no stock is available for the return in the warehouse {1}.").format(
							dontmanage.bold(row.item_code), row.get(warehouse_field)
						)
					)
		...
		for row in self.get(table_name):
			if data := available_dict.get(row.get(field)):
				...
				if self.doctype in ["Sales Invoice", "Delivery Note"]:
					row.db_set(
						"incoming_rate", dontmanage.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
					)
	...
	def is_serial_batch_item(self, item_code) -> bool:
		if not dontmanage.db.exists("Item", item_code):
			dontmanage.throw(_("Item {0} does not exist.").format(bold(item_code)))

		item_details = dontmanage.db.get_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
	...
	def get_gl_entries(self, warehouse_account=None, default_expense_account=None, default_cost_center=None):
		...
		if warehouse_with_no_account:
			for wh in warehouse_with_no_account:
				if dontmanage.get_cached_value("Warehouse", wh, "company"):
					dontmanage.throw(
						_(
	...
	def get_debit_field_precision(self):
		if not dontmanage.flags.debit_field_precision:
			dontmanage.flags.debit_field_precision = dontmanage.get_precision("GL Entry", "debit_in_account_currency")

		return dontmanage.flags.debit_field_precision

	def get_voucher_details(self, default_expense_account, default_cost_center, sle_map):
		if self.doctype == "Stock Reconciliation":
			reconciliation_purpose = dontmanage.db.get_value(self.doctype, self.name, "purpose")
		...
		if not item.get("expense_account"):
			...
			dontmanage.throw(
				_("Row #{0}: Expense Account not set for the Item {1}. {2}").format(
					item.idx, dontmanage.bold(item.item_code), msg
				),
			...
			is_expense_account = (
				dontmanage.get_cached_value("Account", item.get("expense_account"), "report_type")
				== "Profit and Loss"
			)
			if (
				self.doctype
			...
			if not sl_dict.get(dimension.target_fieldname) and dimension.fetch_from_parent:
				...
				if not sl_dict.get(dimension.target_fieldname):
					fieldname = next(
						(
							field.fieldname
							for field in dontmanage.get_meta(self.doctype).fields
	...
	def make_sl_entries(self, sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
		from dontmanageerp.stock.stock_ledger import make_sl_entries
	...
	def get_serialized_items(self):
		...
		if item_codes:
			serialized_items = dontmanage.db.sql_list(
	...
	def validate_warehouse(self):
		from dontmanageerp.stock.utils import validate_disabled_warehouse, validate_warehouse_company
	...
	def validate_inspection(self):
		...
		for row in self.get("items"):
			qi_required = False
			if inspection_required_fieldname and dontmanage.db.get_value(
	...
	def validate_qi_presence(self, row):
		...
		if not row.quality_inspection:
			...
			if self.docstatus == 1:
				dontmanage.throw(msg, title=_("Inspection Required"), exc=QualityInspectionRequiredError)
			else:
				dontmanage.msgprint(msg, title=_("Inspection Required"), indicator="blue")

	def validate_qi_submission(self, row):
		"""Check if QI is submitted on row level, during submission"""
		action = dontmanage.db.get_single_value("Stock Settings", "action_if_quality_inspection_is_not_submitted")
		qa_docstatus = dontmanage.db.get_value("Quality Inspection", row.quality_inspection, "docstatus")

		if not qa_docstatus == 1:
			link = dontmanage.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = _("Row #{0}: Quality Inspection {1} is not submitted for the item: {2}").format(
				row.idx, link, row.item_code
			)
			if action == "Stop":
				dontmanage.throw(msg, title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
			else:
				dontmanage.msgprint(msg, alert=True, indicator="orange")

	def validate_qi_rejection(self, row):
		"""Check if QI is rejected on row level, during submission"""
		action = dontmanage.db.get_single_value("Stock Settings", "action_if_quality_inspection_is_rejected")
		qa_status = dontmanage.db.get_value("Quality Inspection", row.quality_inspection, "status")
		...
		if qa_status == "Rejected":
			...
			if action == "Stop":
				dontmanage.throw(msg, title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
			else:
				dontmanage.msgprint(msg, alert=True, indicator="orange")
	...
	def update_blanket_order(self):
		...
		for blanket_order in blanket_orders:
			dontmanage.get_doc("Blanket Order", blanket_order).update_ordered_qty()
	...
	def validate_multi_currency(self):
		if self.currency != self.company_currency:
			dontmanage.throw(_("Internal transfers can only be done in company's default currency"))
	...
	def validate_putaway_capacity(self):
		...
		from dontmanageerp.stock.doctype.putaway_rule.putaway_rule import get_available_putaway_capacity
		...
		if valid_doctype:
			...
			for rule, values in rule_map.items():
				if flt(values["qty_put"]) > flt(values["capacity"]):
					...
					dontmanage.throw(msg=message, title=_("Over Receipt"))

	def prepare_over_receipt_message(self, rule, values):
		message = _("{0} qty of Item {1} is being received into Warehouse {2} with capacity {3}.").format(
			dontmanage.bold(values["qty_put"]),
			dontmanage.bold(values["item"]),
			dontmanage.bold(values["warehouse"]),
			dontmanage.bold(values["capacity"]),
		)
		message += "<br><br>"
		rule_link = dontmanage.utils.get_link_to_form("Putaway Rule", rule)
...
def get_gl_entries_for_preview(doctype, docname, fields):
	return dontmanage.get_all("GL Entry", filters={"voucher_type": doctype, "voucher_no": docname}, fields=fields)
...
def repost_required_for_queue(doc: StockController) -> bool:
	...
	consuming_sles = dontmanage.db.get_all(
		"Stock Ledger Entry",
		filters={
...
def make_quality_inspections(doctype, docname, items):
	...
	for item in items:
		...
		quality_inspection = dontmanage.get_doc(
			{
				"doctype": "Quality Inspection",
				"inspection_type": "Incoming",
				"inspected_by": dontmanage.session.user,
...
def future_sle_exists(args, sl_entries=None, allow_force_reposting=True):
	if allow_force_reposting and dontmanage.db.get_single_value(
	...
	for d in data:
		dontmanage.local.future_sle[key][(d.item_code, d.warehouse)] = d.total_row
...
def get_cached_data(args, key):
	...
	if args.get("item_code"):
		item_key = (args.get("item_code"), args.get("warehouse"))
		count = dontmanage.local.future_sle[key].get(item_key)

		return True if (count or count == 0) else False
	else:
		return dontmanage.local.future_sle[key]
...
def get_conditions_to_validate_future_sle(sl_entries):
	...
	for warehouse, items in warehouse_items_map.items():
		or_conditions.append(
			f"""warehouse = {dontmanage.db.escape(warehouse)}
				and item_code in ({', '.join(dontmanage.db.escape(item) for item in items)})"""
		)
...
def make_bundle_for_material_transfer(**kwargs):
	...
	bundle_doc = dontmanage.get_doc("Serial and Batch Bundle", kwargs.serial_and_batch_bundle)
