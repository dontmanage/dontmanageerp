...
class StockController(AccountsController):
	...
	def get_debit_field_precision(self):
		if not dontmanage.flags.debit_field_precision:
			dontmanage.flags.debit_field_precision = dontmanage.get_precision(
	...
	def validate_qi_presence(self, row):
		...
		if not row.quality_inspection:
			msg = f"Row #{row.idx}: Quality Inspection is required for Item {dontmanage.bold(row.item_code)}"
			if self.docstatus == 1:
				dontmanage.throw(_(msg), title=_("Inspection Required"), exc=QualityInspectionRequiredError)
			else:
				dontmanage.msgprint(_(msg), title=_("Inspection Required"), indicator="blue")

	def validate_qi_submission(self, row):
		"""Check if QI is submitted on row level, during submission"""
		action = dontmanage.db.get_single_value(
		...
		if not qa_docstatus == 1:
			...
			if action == "Stop":
				dontmanage.throw(_(msg), title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
			else:
				dontmanage.msgprint(_(msg), alert=True, indicator="orange")
	...
	def validate_qi_rejection(self, row):
		...
		if qa_status == "Rejected":
			...
			if action == "Stop":
				dontmanage.throw(_(msg), title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
			else:
				dontmanage.msgprint(_(msg), alert=True, indicator="orange")
