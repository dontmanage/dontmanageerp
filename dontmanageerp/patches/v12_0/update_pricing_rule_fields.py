...
def execute():
	...
	if not dontmanage.get_all("Pricing Rule", limit=1):
		return

	dontmanage.reload_doc("accounts", "doctype", "pricing_rule_detail")
	doctypes = {
	...
	for doctype, module in doctypes.items():
		...
		child_doc = dontmanage.scrub(doctype) + "_item"
		dontmanage.reload_doc(module, "doctype", child_doc, force=True)
	...
	for doctype, apply_on in {
		...
		dontmanage.reload_doc("accounts", "doctype", dontmanage.scrub(doctype))
