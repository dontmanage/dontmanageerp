...
def execute():
	...
	dontmanage.reload_doc("stock", "doctype", "delivery_note_item", force=True)
	dontmanage.reload_doc("stock", "doctype", "purchase_receipt_item", force=True)
	...
	def make_return_document_map(doctype, return_document_map):
		...
		return_against_documents = dontmanage.db.sql(
			"""
			SELECT
	...
	def set_document_detail_in_return_document(doctype):
		...
		child_doc = dontmanage.scrub("{0} Item".format(doctype))
		dontmanage.reload_doc("stock", "doctype", child_doc)
		...
		for docname in return_document_map:
			doc_items = dontmanage.get_cached_doc(doctype, docname).get("items")
			for return_doc in return_document_map[docname]:
				return_doc_items = dontmanage.get_cached_doc(doctype, return_doc).get("items")
