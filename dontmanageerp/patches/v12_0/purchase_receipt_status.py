...
logger = dontmanage.logger("patch", allow_site=True, file_count=50)


def execute():
	affected_purchase_receipts = dontmanage.db.sql(
	...
	dontmanage.reload_doc("stock", "doctype", "Purchase Receipt")
	dontmanage.reload_doc("stock", "doctype", "Purchase Receipt Item")

	for pr in affected_purchase_receipts:
		pr_name = pr[0]
		logger.info("purchase_receipt_status: patching PR - {}".format(pr_name))

		pr_doc = dontmanage.get_doc("Purchase Receipt", pr_name)
