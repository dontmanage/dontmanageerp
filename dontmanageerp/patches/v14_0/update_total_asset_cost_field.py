...
def execute():
	...
	dontmanage.qb.update(asset).set(asset.total_asset_cost, asset.gross_purchase_amount).run()

	asset_repair_list = dontmanage.db.get_all(
		"Asset Repair",
	...
	for asset_repair in asset_repair_list:
		dontmanage.qb.update(asset).set(
