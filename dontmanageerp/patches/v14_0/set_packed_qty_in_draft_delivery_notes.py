...
def execute():
	...
	ps_item = dontmanage.qb.DocType("Packing Slip Item")
	...
	if ps_details:
		...
		dn_details = dontmanage._dict()
		for r in dn_item_query.run(as_dict=True):
			dn_details.setdefault((r.delivery_note, r.item_code), dontmanage._dict()).setdefault(r.name, r.qty)
			...
			if dn_items:
				...
				for name, qty in dn_items.items():
					if remaining_qty > 0:
						...
						dontmanage.db.set_value("Delivery Note Item", name, "packed_qty", row_packed_qty)
