...
def execute():
	try:
		...
		dontmanage.log_error("Failed to migrate deferred accounts in Item Defaults.")
...
def add_company_wise_item_default(item, account_type):
	company = dontmanage.get_cached_value("Account", item[account_type], "company")
	if company and item[account_type]:
		item_defaults = dontmanage.get_cached_value("Item", item["name"], "item_defaults")
		for item_row in item_defaults:
			if item_row.company == company:
				dontmanage.set_value("Item Default", item_row.name, account_type, item[account_type])
				break
		else:
			item_defaults.append({"company": company, account_type: item[account_type]})
			dontmanage.set_value("Item", item["name"], "item_defaults", item_defaults)
