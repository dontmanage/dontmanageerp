...
def execute():
	dontmanage.delete_doc("Module Def", "Non Profit", ignore_missing=True, force=True)

	dontmanage.delete_doc("Workspace", "Non Profit", ignore_missing=True, force=True)

	print_formats = dontmanage.get_all("Print Format", {"module": "Non Profit", "standard": "Yes"}, pluck="name")
	...
	reports = dontmanage.get_all("Report", {"module": "Non Profit", "is_standard": "Yes"}, pluck="name")
	for report in reports:
		dontmanage.delete_doc("Report", report, ignore_missing=True, force=True)

	dashboards = dontmanage.get_all("Dashboard", {"module": "Non Profit", "is_standard": 1}, pluck="name")
	for dashboard in dashboards:
		dontmanage.delete_doc("Dashboard", dashboard, ignore_missing=True, force=True)

	doctypes = dontmanage.get_all("DocType", {"module": "Non Profit", "custom": 0}, pluck="name")
	...
	for record in custom_records:
		try:
			dontmanage.delete_doc(record["doctype"], record["name"], ignore_missing=True)
