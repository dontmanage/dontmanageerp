...
def execute():
	dontmanage.db.truncate("Account Closing Balance")

	for company in dontmanage.get_all("Company", pluck="name"):
		i = 0
		company_wise_order = {}
		for pcv in dontmanage.db.get_all(
			...
			if pcv.posting_date not in company_wise_order[pcv.company]:
				pcv_doc = dontmanage.get_doc("Period Closing Voucher", pcv.name)
				...
				if pcv.posting_date not in company_wise_order[pcv.company]:
					...
					closing_entries = dontmanage.db.get_all(
						"GL Entry",
						filters={
							"is_cancelled": 0,
							"voucher_no": ["!=", pcv.name],
