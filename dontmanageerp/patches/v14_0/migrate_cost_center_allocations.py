...
def get_existing_cost_center_allocations():
	if not dontmanage.db.exists("DocType", "Distributed Cost Center"):
		return

	par = dontmanage.qb.DocType("Cost Center")
	child = dontmanage.qb.DocType("Distributed Cost Center")
	...
	cc_allocations = dontmanage._dict()
	for d in records:
		cc_allocations.setdefault(d.name, dontmanage._dict()).setdefault(d.cost_center, d.percentage_allocation)
