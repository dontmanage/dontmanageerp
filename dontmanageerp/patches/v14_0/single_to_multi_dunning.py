...
def execute():
	dontmanage.reload_doc("accounts", "doctype", "overdue_payment")
	dontmanage.reload_doc("accounts", "doctype", "dunning")
	...
	all_dunnings = dontmanage.get_all("Dunning", filters=filters, pluck="name")

	for dunning_name in all_dunnings:
		dunning = dontmanage.get_doc("Dunning", dunning_name)
		...
		payment_schedules = dontmanage.get_all(
			"Payment Schedule",
...
def get_accounts_closing_date():
	...
	accounts_frozen_till = dontmanage.db.get_single_value(
		"Accounts Settings", "acc_frozen_upto"
	)  # always returns datetime.date

	period_closing_date = dontmanage.db.get_value(
