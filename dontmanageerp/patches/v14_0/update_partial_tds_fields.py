...
from dontmanageerp.accounts.utils import FiscalYearError, get_fiscal_year
...
def execute():
	...
	for company in dontmanage.get_all("Company"):
		try:
			...
			dontmanage.qb.update(purchase_invoice).set(
			...
			purchase_order = dontmanage.qb.DocType("Purchase Order")

			dontmanage.qb.update(purchase_order).set(
