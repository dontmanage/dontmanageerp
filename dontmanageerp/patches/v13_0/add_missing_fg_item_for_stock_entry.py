...
def execute():
	if not dontmanage.db.has_column("Work Order", "has_batch_no"):
		return

	dontmanage.reload_doc("manufacturing", "doctype", "manufacturing_settings")
	if cint(dontmanage.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")):
		return
	...
	work_orders = [d.name for d in dontmanage.get_all("Work Order", filters=filters, fields=fields)]
	...
	stock_entries = dontmanage.db.sql_list(
		"""
		SELECT
			se.name
		FROM
	...
	for stock_entry in stock_entries:
		doc = dontmanage.get_doc("Stock Entry", stock_entry)
...
def set_expense_account(doc):
	for row in doc.items:
		if row.is_finished_item and not row.expense_account:
			row.expense_account = dontmanage.get_cached_value("Company", doc.company, "stock_adjustment_account")
