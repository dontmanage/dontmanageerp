...
def delete_broken_bins():
	# delete useless bins
	dontmanage.db.sql("delete from `tabBin` where item_code is null or warehouse is null")


def delete_and_patch_duplicate_bins():

	duplicate_bins = dontmanage.db.sql(
		"""
		SELECT
	...
	for duplicate_bin in duplicate_bins:
		...
		existing_bins = dontmanage.get_list(
			"Bin",
		...
		for broken_bin in existing_bins:
			dontmanage.delete_doc("Bin", broken_bin.name)
