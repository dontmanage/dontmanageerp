...
def execute():
	...
	if dontmanage.db.exists("DocType", "Service Level Agreement"):
		sla_details = dontmanage.db.get_all("Service Level Agreement", fields=["name", "service_level"])
		priorities = dontmanage.db.get_all(
		...
		dontmanage.reload_doc("support", "doctype", "pause_sla_on_status")
		dontmanage.reload_doc("support", "doctype", "service_level_priority")
		dontmanage.reload_doc("support", "doctype", "service_day")
		...
		if dontmanage.db.exists("DocType", "Service Level"):
			service_levels = dontmanage.db.get_all(
				"Service Level", filters={"service_level": ("not in", sl)}, fields=["*"]
			)
			for entry in service_levels:
				sla = dontmanage.new_doc("Service Level Agreement")
				...
				if priority_list:
					sla = dontmanage.get_doc("Service Level Agreement", sla.name)
	...
	dontmanage.delete_doc_if_exists("DocType", "Service Level")
