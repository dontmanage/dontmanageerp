...
def execute():
	...
	for (p,) in dontmanage.db.get_all("Quality Inspection Reading", fields=["specification"], as_list=True):
		params.add(p.strip())

	# get all parameters from QI Template as some may be unused in QI
	for (p,) in dontmanage.db.get_all(
	...
	for parameter in params:
		if dontmanage.db.exists("Quality Inspection Parameter", parameter):
			continue
