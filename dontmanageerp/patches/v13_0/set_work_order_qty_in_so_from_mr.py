...
def execute():
	...
	for row in results:
		so_item = dontmanage.get_value("Material Request Item", row.material_request_item, "sales_order_item")
		dontmanage.db.set_value("Work Order", row.name, "sales_order_item", so_item)

		if so_item:
			wo = dontmanage.get_doc("Work Order", row.name)
