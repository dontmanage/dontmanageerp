...
def execute():
	...
	dontmanage.db.MAX_WRITES_PER_TRANSACTION += len(broken_sles)
	...
	for sle in broken_sles:
		...
		dontmanage.db.set_value("Stock Ledger Entry", sle.name, "serial_no", correct_sr_no, update_modified=False)
	...
	broken_sr_no_records = dontmanage.get_list(
		"Serial No",
		filters={
			"status": "Active",
	...
	dontmanage.db.MAX_WRITES_PER_TRANSACTION += len(broken_sr_no_records)

	patch_savepoint = "serial_no_patch"
	for serial_no in broken_sr_no_records:
		try:
			dontmanage.db.savepoint(patch_savepoint)
			sn = dontmanage.get_doc("Serial No", serial_no)
			sn.update_serial_no_reference()
			sn.db_update()
		except Exception:
			dontmanage.db.rollback(save_point=patch_savepoint)
