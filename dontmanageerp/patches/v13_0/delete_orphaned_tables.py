...
def execute():
	dontmanage.reload_doc("setup", "doctype", "transaction_deletion_record")

	if has_deleted_company_transactions():
		child_doctypes = get_child_doctypes_whose_parent_doctypes_were_affected()

		for doctype in child_doctypes:
			docs = dontmanage.get_all(doctype, fields=["name", "parent", "parenttype", "creation"])

			for doc in docs:
				if not dontmanage.db.exists(doc["parenttype"], doc["parent"]):
					dontmanage.db.delete(doctype, {"name": doc["name"]})
...
def has_deleted_company_transactions():
	return dontmanage.get_all("Transaction Deletion Record")
...
def get_affected_doctypes():
	affected_doctypes = []
	tdr_docs = dontmanage.get_all("Transaction Deletion Record", pluck="name")

	for tdr in tdr_docs:
		tdr_doc = dontmanage.get_doc("Transaction Deletion Record", tdr)
...
def is_not_child_table(doctype):
	return not bool(dontmanage.get_value("DocType", doctype, "istable"))
...
def check_for_new_doc_with_same_name_as_deleted_parent(doc):
	...
	parent_creation_time = dontmanage.db.get_value(doc["parenttype"], doc["parent"], "creation")
