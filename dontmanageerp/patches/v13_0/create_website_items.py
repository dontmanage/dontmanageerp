...
def execute():
	dontmanage.reload_doc("e_commerce", "doctype", "website_item")
	dontmanage.reload_doc("e_commerce", "doctype", "website_item_tabbed_section")
	dontmanage.reload_doc("e_commerce", "doctype", "website_offer")
	dontmanage.reload_doc("e_commerce", "doctype", "recommended_items")
	dontmanage.reload_doc("e_commerce", "doctype", "e_commerce_settings")
	...
	item_table_fields = dontmanage.db.sql("desc `tabItem`", as_dict=1)  # nosemgrep
	...
	items = dontmanage.db.get_all("Item", fields=item_fields, or_filters=or_filters)
	total_count = len(items)

	for count, item in enumerate(items, start=1):
		if dontmanage.db.exists("Website Item", {"item_code": item.item_code}):
			continue
		...
		dontmanage.utils.update_progress_bar("Creating Website Items", count, total_count)
