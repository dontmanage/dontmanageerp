...
def execute():
	...
	dontmanage.reload_doc("loan_management", "doctype", "repayment_schedule")
	dontmanage.reload_doc("loan_management", "doctype", "process_loan_interest_accrual")
	dontmanage.reload_doc("loan_management", "doctype", "loan_repayment")
	dontmanage.reload_doc("loan_management", "doctype", "loan_repayment_detail")
	dontmanage.reload_doc("loan_management", "doctype", "loan_interest_accrual")
	dontmanage.reload_doc("accounts", "doctype", "gl_entry")
	dontmanage.reload_doc("accounts", "doctype", "journal_entry_account")

	updated_loan_types = []
	loans_to_close = []

	# Update old loan status as closed
	if dontmanage.db.has_column("Repayment Schedule", "paid"):
		loans_list = dontmanage.db.sql(
	...
	for loan in loans:
		...
		loan_type_company = dontmanage.db.get_value("Loan Type", loan.loan_type, "company")
		loan_type = loan.loan_type

		group_income_account = dontmanage.get_value(
			"Account",
			{
		...
		if loan_type_company and loan_type_company != loan.company:
			...
			loan_type_name = dontmanage.get_value(
				"Loan Type",
				{
					"company": loan.company,
			...
			loan_type_doc = dontmanage.get_doc("Loan Type", loan.loan_type)
		...
		if loan_type in updated_loan_types:
			...
			if dontmanage.db.has_column("Repayment Schedule", "paid"):
				total_principal, total_interest = dontmanage.db.get_value(
					"Repayment Schedule",
...
def create_loan_type(loan, loan_type_name, penalty_account):
	loan_type_doc = dontmanage.new_doc("Loan Type")
