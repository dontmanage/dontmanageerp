...
def execute():
	...
	def move_table_multiselect_data(docfield):
		...
		for row in table_multiselect_data:
			...
			web_item = dontmanage.db.get_value("Website Item", {"item_code": row.parent})
			web_item_doc = dontmanage.get_doc("Website Item", web_item)

			child_doc = dontmanage.new_doc(docfield.options, web_item_doc, field)
	...
	def get_table_multiselect_data(docfield):
		child_table = dontmanage.qb.DocType(docfield.options)
		item = dontmanage.qb.DocType("Item")

		table_multiselect_data = (  # query table data for field
			dontmanage.qb.from_(child_table)
			.join(item)
