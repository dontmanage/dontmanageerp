...
def execute():
	dontmanage.reload_doc("accounts", "doctype", "pos_payment_method")
	pos_profiles = dontmanage.get_all("POS Profile")

	for pos_profile in pos_profiles:
		payments = dontmanage.db.sql(
			"""
		...
		if payments:
			for payment_mode in payments:
				pos_payment_method = dontmanage.new_doc("POS Payment Method")
		...
		dontmanage.db.sql("""delete from `tabSales Invoice Payment` where parent=%s""", pos_profile.name)
