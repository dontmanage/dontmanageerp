...
def execute():
	dontmanage.reload_doc("accounts", "doctype", "Tax Withholding Rate")

	if dontmanage.db.has_column("Tax Withholding Rate", "fiscal_year"):
		tds_category_rates = dontmanage.get_all("Tax Withholding Rate", fields=["name", "fiscal_year"])

		fiscal_year_map = {}
		fiscal_year_details = dontmanage.get_all(
