...
def execute():
	...
	if not dontmanage.db.exists("DocType", "Party Specific Item"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		invoices_to_update = dontmanage.get_all(
			doctype,
			fields=fields,
			filters={
				"docstatus": 1,
		...
		payment_schedule_items = dontmanage.get_all(
			"Payment Schedule",
		...
		for invoice in invoices_to_update.values():
			...
			doc = dontmanage.get_doc(invoice)
		...
		for status, docs in status_map.items():
			dontmanage.db.set_value(doctype, {"name": ("in", docs)}, "status", status, update_modified=False)
