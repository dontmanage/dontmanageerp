...
def execute():
	...
	"""
	dontmanage.reload_doc("e_commerce", "web_template", "item_card_group")

	blocks = dontmanage.db.get_all(
		"Web Page Block",
	...
	for block in blocks:
		...
		for field in fields:
			...
			if dontmanage.db.exists("Website Item", {"item_code": item}):
				website_item = dontmanage.db.get_value("Website Item", {"item_code": item})
			else:
...
def make_new_website_item(item: str) -> Union[str, None]:
	try:
		doc = dontmanage.get_doc("Item", item)
