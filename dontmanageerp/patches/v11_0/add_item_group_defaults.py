...
def execute():
	...
	dontmanage.reload_doc("setup", "doctype", "item_group")

	companies = dontmanage.get_all("Company")
	item_groups = dontmanage.db.sql(
	...
	if len(companies) == 1:
		for item_group in item_groups:
			doc = dontmanage.get_doc("Item Group", item_group.get("name"))
			item_group_defaults = []
		...
		for item_group in item_groups:
			...
			for d in [
				...
				if item_group.get(d[0]):
					company = dontmanage.get_value(d[1], item_group.get(d[0]), "company", cache=True)
