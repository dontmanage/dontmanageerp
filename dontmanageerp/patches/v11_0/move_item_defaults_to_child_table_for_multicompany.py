...
def execute():
	...
	"""
	if not dontmanage.db.has_column("Item", "default_warehouse"):
		return
	...
	if len(companies) == 1 and not dontmanage.get_all("Item Default", limit=1):
		try:
			dontmanage.db.sql(
				"""
		...
		item_details = dontmanage.db.sql(
		...
		for item_data in item_details:
			for d in [
				...
				if item_data.get(d[0]):
					company = dontmanage.get_value(d[1], item_data.get(d[0]), "company", cache=True)
