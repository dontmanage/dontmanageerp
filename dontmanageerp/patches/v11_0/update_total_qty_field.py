...
def execute():
	dontmanage.reload_doc("buying", "doctype", "purchase_order")
	dontmanage.reload_doc("buying", "doctype", "supplier_quotation")
	dontmanage.reload_doc("selling", "doctype", "sales_order")
	dontmanage.reload_doc("selling", "doctype", "quotation")
	dontmanage.reload_doc("stock", "doctype", "delivery_note")
	dontmanage.reload_doc("stock", "doctype", "purchase_receipt")
	dontmanage.reload_doc("accounts", "doctype", "sales_invoice")
	dontmanage.reload_doc("accounts", "doctype", "purchase_invoice")
	...
	for doctype in doctypes:
		...
		for i in range(0, len(total_qty), batch_size):
			...
			for d in batch_transactions:
				values.append("({0}, {1})".format(dontmanage.db.escape(d.parent), d.qty))
