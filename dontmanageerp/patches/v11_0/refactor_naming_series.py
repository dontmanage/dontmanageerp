...
def set_series(doctype, options, default):
	def _make_property_setter(property_name, value):
		property_setter = dontmanage.db.exists(
			"Property Setter",
			{"doc_type": doctype, "field_name": "naming_series", "property": property_name},
		)
		if property_setter:
			dontmanage.db.set_value("Property Setter", property_setter, "value", value)
		else:
...
def get_series():
	...
	for doctype in doctype_series_map:
		if not dontmanage.db.exists("DocType", doctype):
			continue
		if not dontmanage.db.a_row_exists(doctype):
			continue
		if not dontmanage.db.has_column(doctype, "naming_series"):
			continue
		if not dontmanage.get_meta(doctype).has_field("naming_series"):
			continue
		...
		if not series_to_preserve:
			continue
		existing_series = (dontmanage.get_meta(doctype).get_field("naming_series").options or "").split("\n")
...
def get_series_to_preserve(doctype):
	series_to_preserve = dontmanage.db.sql_list(
...
def get_default_series(doctype):
	field = dontmanage.get_meta(doctype).get_field("naming_series")
