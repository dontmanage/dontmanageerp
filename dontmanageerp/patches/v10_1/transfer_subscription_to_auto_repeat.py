...
def execute():
	dontmanage.reload_doc("automation", "doctype", "auto_repeat")
	...
	for module, doctypes in doctypes_to_rename.items():
		for doctype in doctypes:
			...
			if dontmanage.db.has_column(doctype, "subscription"):
				rename_field(doctype, "subscription", "auto_repeat")

	subscriptions = dontmanage.db.sql("select * from `tabSubscription`", as_dict=1)

	for doc in subscriptions:
		doc["doctype"] = "Auto Repeat"
		auto_repeat = dontmanage.get_doc(doc)
		auto_repeat.db_insert()

	dontmanage.db.sql("delete from `tabSubscription`")
...
def drop_columns_from_subscription():
	...
	for field in [
		...
		if field in dontmanage.db.get_table_columns("Subscription"):
			fields_to_drop["Subscription"].append(field)

	dontmanage.model.delete_fields(fields_to_drop, delete=1)
