...
from dontmanageerp.support.doctype.issue_priority.test_issue_priority import make_priorities
from dontmanageerp.support.doctype.service_level_agreement.service_level_agreement import (
...
class TestServiceLevelAgreement(unittest.TestCase):
	...
	def create_company(self):
		...
		if dontmanage.db.exists("Company", name):
			company = dontmanage.get_doc("Company", name)
		else:
	...
	def test_custom_field_creation_for_sla_on_standard_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				dontmanage.db.exists("Custom Field", {"dt": doctype, "fieldname": field.get("fieldname")})
			)
	...
	def test_docfield_creation_for_sla_on_custom_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				dontmanage.db.exists("DocField", {"fieldname": field.get("fieldname"), "parent": doctype.name})
			)
	...
	def test_sla_application(self):
		...
		dontmanage.flags.current_time = datetime.datetime(2019, 3, 4, 15, 0)
		lead.reload()
	...
	def test_hold_time(self):
		...
		dontmanage.flags.current_time = datetime.datetime(2020, 3, 4, 4, 15)
		lead.reload()
		lead.status = "Replied"
		lead.save()

		lead.reload()
		self.assertEqual(lead.on_hold_since, dontmanage.flags.current_time)

		dontmanage.flags.current_time = datetime.datetime(2020, 3, 4, 5, 5)
		lead.reload()
		lead.status = "Converted"
		lead.save()
	...
	def test_failed_sla_for_response_only(self):
		...
		dontmanage.flags.current_time = datetime.datetime(2019, 3, 4, 16, 5)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_fulfilled_sla_for_response_only(self):
		...
		dontmanage.flags.current_time = datetime.datetime(2019, 3, 4, 15, 30)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_service_level_agreement_filters(self):
		...
		applied_sla = dontmanage.db.get_value("Lead", lead.name, "service_level_agreement")
		self.assertFalse(applied_sla)

		source = dontmanage.get_doc(doctype="Lead Source", source_name="Test Source")
	...
	def tearDown(self):
		for d in dontmanage.get_all("Service Level Agreement"):
			dontmanage.delete_doc("Service Level Agreement", d.name, force=1)
...
def get_service_level_agreement(
	...
	service_level_agreement = dontmanage.get_doc("Service Level Agreement", filters)
...
def create_service_level_agreement(
	...
	service_level_agreement = dontmanage._dict(
		{
			...
			"entity": entity,
			"start_date": dontmanage.utils.getdate(),
			"end_date": dontmanage.utils.add_to_date(dontmanage.utils.getdate(), days=100),
	...
	sla = dontmanage.db.exists("Service Level Agreement", filters)
	if sla:
		dontmanage.delete_doc("Service Level Agreement", sla, force=1)

	return dontmanage.get_doc(service_level_agreement).insert(ignore_permissions=True, ignore_if_duplicate=True)
...
def create_customer():
	...
	if not dontmanage.db.exists("Customer", "_Test Customer"):
		customer.insert(ignore_permissions=True)
		return customer.name
	else:
		return dontmanage.db.exists("Customer", "_Test Customer")


def create_customer_group():
	customer_group = dontmanage.get_doc(
		{"doctype": "Customer Group", "customer_group_name": "_Test SLA Customer Group"}
	)

	if not dontmanage.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"}):
		customer_group.insert()
		return customer_group.name
	else:
		return dontmanage.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"})


def create_territory():
	territory = dontmanage.get_doc(
		{
			"doctype": "Territory",
	...
	if not dontmanage.db.exists("Territory", {"territory_name": "_Test SLA Territory"}):
		territory.insert()
		return territory.name
	else:
		return dontmanage.db.exists("Territory", {"territory_name": "_Test SLA Territory"})
...
def make_holiday_list():
	holiday_list = dontmanage.db.exists("Holiday List", "__Test Holiday List")
	if not holiday_list:
...
def create_custom_doctype():
	if not dontmanage.db.exists("DocType", "Test SLA on Custom Dt"):
		...
		return dontmanage.get_doc("DocType", "Test SLA on Custom Dt")
