...
from dontmanageerp.support.doctype.issue.issue import get_holidays
...
class ServiceLevelAgreement(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.support.doctype.pause_sla_on_status.pause_sla_on_status import PauseSLAOnStatus
		from dontmanageerp.support.doctype.service_day.service_day import ServiceDay
		from dontmanageerp.support.doctype.service_level_priority.service_level_priority import (
			ServiceLevelPriority,
		)
		from dontmanageerp.support.doctype.sla_fulfilled_on_status.sla_fulfilled_on_status import (
	...
	def validate_doc(self):
		if (
			...
			and not dontmanage.db.get_single_value("Support Settings", "track_service_level_agreement")
		):
			dontmanage.throw(
				_("{0} is not enabled in {1}").format(
					dontmanage.bold(_("Track Service Level Agreement")),
					get_link_to_form("Support Settings", "Support Settings"),
				)
			)

		if self.default_service_level_agreement and dontmanage.db.exists(
	...
	def validate_selected_doctype(self):
		invalid_doctypes = list(dontmanage.model.core_doctypes_list)
		invalid_doctypes.extend(["Cost Center", "Company"])
		valid_document_types = dontmanage.get_all(
			"DocType",
			{
				"issingle": 0,
				"istable": 0,
		...
		if self.document_type not in valid_document_types:
			dontmanage.throw(msg=_("Please select valid document type."), title=_("Invalid Document Type"))
	...
	def validate_status_field(self):
		...
		if not meta.get_field("status"):
			dontmanage.throw(
				...
				).format(dontmanage.bold(self.document_type))
			)
	...
	def get_service_level_agreement_priority(self, priority):
		priority = dontmanage.get_doc("Service Level Priority", {"priority": priority, "parent": self.name})
...
def check_agreement_status():
	service_level_agreements = dontmanage.get_all(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		...
		if doc.end_date and getdate(doc.end_date) < getdate(dontmanage.utils.getdate()):
			dontmanage.db.set_value("Service Level Agreement", service_level_agreement.name, "enabled", 0)


def get_active_service_level_agreement_for(doc):
	if not dontmanage.db.get_single_value("Support Settings", "track_service_level_agreement"):
		return
	...
	for agreement in agreements:
		...
		if not condition or (condition and dontmanage.safe_eval(condition, None, get_context(doc))):
...
def get_customer_group(customer):
	customer_groups = []
	customer_group = dontmanage.db.get_value("Customer", customer, "customer_group") if customer else None
	if customer_group:
...
def get_customer_territory(customer):
	...
	customer_territory = dontmanage.db.get_value("Customer", customer, "territory") if customer else None
...
def get_service_level_agreement_filters(doctype, name, customer=None):
	...
	return {
		...
		"service_level_agreements": [
			d.name for d in dontmanage.get_all("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_service_level_agreement():
	sla_doctypes = dontmanage.cache().hget("service_level_agreement", "active")
...
def set_documents_with_active_service_level_agreement():
	...
	dontmanage.cache().hset("service_level_agreement", "active", active)
	return active
...
def change_service_level_agreement_and_priority(self):
	if (
		...
		and dontmanage.db.exists("Issue", self.name)
		and dontmanage.db.get_single_value("Support Settings", "track_service_level_agreement")
	):
		if not self.priority == dontmanage.db.get_value("Issue", self.name, "priority"):
		...
		if not self.service_level_agreement == dontmanage.db.get_value(
...
def get_response_and_resolution_duration(doc):
	sla = dontmanage.get_doc("Service Level Agreement", doc.service_level_agreement)
...
def reset_service_level_agreement(doctype: str, docname: str, reason, user):
	if not dontmanage.db.get_single_value("Support Settings", "allow_resetting_service_level_agreement"):
		dontmanage.throw(_("Allow Resetting Service Level Agreement from Support Settings."))
...
def on_communication_update(doc, status):
	...
	for_resolution = dontmanage.db.get_value(
...
def get_sla_doctypes():
	doctypes = []
	data = dontmanage.get_all("Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
