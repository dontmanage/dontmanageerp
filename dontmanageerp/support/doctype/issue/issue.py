...
class Issue(Document):
	...
	def set_lead_contact(self, email_id):
		...
		if email_id:
			if not self.lead:
				self.lead = dontmanage.db.get_value("Lead", {"email_id": email_id})

			if not self.contact and not self.customer:
				self.contact = dontmanage.db.get_value("Contact", {"email_id": email_id})

				if self.contact:
					contact = dontmanage.get_doc("Contact", self.contact)
					self.customer = contact.get_link_for("Customer")

			if not self.company:
				self.company = dontmanage.db.get_value("Lead", self.lead, "company") or dontmanage.db.get_default(
					"Company"
				)
	...
	def split_issue(self, subject, communication_id):
		...
		dontmanage.get_doc(replicated_issue).insert()
...
def set_multiple_status(names, status):
	...
	for name in json.loads(names):
		dontmanage.db.set_value("Issue", name, "status", status)
...
def auto_close_tickets():
	...
	auto_close_after_days = (
		dontmanage.db.get_value("Support Settings", "Support Settings", "close_issue_after_days") or 7
	)

	table = dontmanage.qb.DocType("Issue")
	issues = (
	...
	for issue in issues:
		doc = dontmanage.get_doc("Issue", issue)
		doc.status = "Closed"
...
def update_issue(contact, method):
	...
	dontmanage.db.sql("""UPDATE `tabIssue` set contact='' where contact=%s""", contact.name)
...
def calculate_first_response_time(issue, first_responded_on):
	...
	support_hours = dontmanage.get_cached_doc(
...
def get_holidays(holiday_list_name):
	holiday_list = dontmanage.get_cached_doc("Holiday List", holiday_list_name)
