dontmanage.ui.form.on("Issue", {
			...
			frm.add_custom_button(
				...
				function () {
					dontmanage.model.open_mapped_doc({
						method: "dontmanageerp.support.doctype.issue.issue.make_task",
						frm: frm,
					});
				},
				__("Create")
	...
	reset_service_level_agreement: function (frm) {
		let reset_sla = new dontmanage.ui.Dialog({
			...
			primary_action: (values) => {
				...
				dontmanage.call(
					"dontmanageerp.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement",
					{
					...
					() => {
						...
						frm.refresh();
						dontmanage.msgprint(__("Service Level Agreement was reset."));
					}
				);
			},
		});
	...
	timeline_refresh: function (frm) {
		if (!frm.timeline.wrapper.find(".btn-split-issue").length) {
			let split_issue_btn = $(`
				<a class="action-btn btn-split-issue" title="${__("Split Issue")}">
					${dontmanage.utils.icon("branch", "sm")}
				</a>
			`);
		...
		// if (dontmanage.model.can_create("Help Article")) {
		...
		// 			${dontmanage.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = dontmanage.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			dontmanage.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
