...
from dontmanage.utils import flt, get_datetime

from dontmanageerp.support.doctype.service_level_agreement.test_service_level_agreement import (
...
class TestSetUp(unittest.TestCase):
	def setUp(self):
		dontmanage.db.sql("delete from `tabService Level Agreement`")
		dontmanage.db.sql("delete from `tabService Level Priority`")
...
class TestIssue(TestSetUp):
	def test_response_time_and_resolution_time_based_on_different_sla(self):
		...
		dontmanage.flags.current_time = get_datetime("2019-03-04 15:00")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_hold_time_on_replied(self):
		...
		dontmanage.flags.current_time = get_datetime("2020-03-04 4:15")
		issue.reload()
		issue.status = "Replied"
		issue.save()

		self.assertEqual(issue.on_hold_since, dontmanage.flags.current_time)
		self.assertFalse(issue.resolution_by)

		creation = get_datetime("2020-03-04 5:00")
		dontmanage.flags.current_time = get_datetime("2020-03-04 5:00")
		create_communication(issue.name, "test@example.com", "Received", creation)
		...
		dontmanage.flags.current_time = get_datetime("2020-03-04 5:05")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_issue_close_after_on_hold(self):
		dontmanage.flags.current_time = get_datetime("2021-11-01 19:00")

		issue = make_issue(dontmanage.flags.current_time, index=1)
		create_communication(issue.name, "test@example.com", "Received", dontmanage.flags.current_time)

		# send a reply within SLA
		dontmanage.flags.current_time = get_datetime("2021-11-02 11:00")
		create_communication(issue.name, "test@admin.com", "Sent", dontmanage.flags.current_time)
		...
		dontmanage.flags.current_time = get_datetime("2021-11-22 01:00")
		issue.status = "Closed"
		issue.save()
	...
	def test_issue_open_after_closed(self):
		...
		dontmanage.flags.current_time = get_datetime("2021-11-01 13:00")
		issue = make_issue(
			dontmanage.flags.current_time, index=1, issue_type="Critical"
		...
		# Replied on → 2 pm
		dontmanage.flags.current_time = get_datetime("2021-11-01 14:00")
		create_communication(issue.name, "test@admin.com", "Sent", dontmanage.flags.current_time)
		issue.reload()
		issue.status = "Replied"
		issue.save()
		self.assertEquals(issue.agreement_status, "Resolution Due")
		self.assertEquals(issue.on_hold_since, dontmanage.flags.current_time)
		self.assertEquals(issue.first_responded_on, dontmanage.flags.current_time)

		# Customer Replied → 3 pm
		dontmanage.flags.current_time = get_datetime("2021-11-01 15:00")
		create_communication(issue.name, "test@example.com", "Received", dontmanage.flags.current_time)
		issue.reload()
		...
		dontmanage.flags.current_time = get_datetime("2021-11-01 16:00")
		create_communication(issue.name, "test@admin.com", "Sent", dontmanage.flags.current_time)
		issue.reload()
		issue.status = "Replied"
		issue.save()
		...
		dontmanage.flags.current_time = get_datetime("2021-11-01 22:00")
		issue.status = "Closed"
		issue.save()
		# Hold Time + 6 Hrs
		...
		self.assertEquals(issue.resolution_date, dontmanage.flags.current_time)

		# Customer Open → 3 am i.e after resolution by is crossed
		dontmanage.flags.current_time = get_datetime("2021-11-02 03:00")
		create_communication(issue.name, "test@example.com", "Received", dontmanage.flags.current_time)
		issue.reload()
		...
		dontmanage.flags.current_time = get_datetime("2021-11-02 04:00")
		issue.status = "Closed"
		issue.save()
		self.assertEquals(issue.resolution_by, get_datetime("2021-11-02 07:00"))
		self.assertEquals(issue.agreement_status, "Fulfilled")
	...
	def test_recording_of_assignment_on_first_reponse_failure(self):
		from dontmanage.desk.form.assign_to import add as add_assignment
		...
		dontmanage.flags.current_time = get_datetime("2021-11-02 15:00")
		create_communication(issue.name, "test@admin.com", "Sent", dontmanage.flags.current_time)

		# assert if a new timeline item has been added
		# to record the assignment
		comment = dontmanage.db.exists(
			"Comment",
			{
...
def create_issue_and_communication(issue_creation, first_responded_on):
	...
	dontmanage.flags.current_time = first_responded_on
...
def make_issue(creation=None, customer=None, index=0, priority=None, issue_type=None):
	if issue_type and not dontmanage.db.exists("Issue Type", issue_type):
		doc = dontmanage.new_doc("Issue Type")
		doc.name = issue_type
		doc.insert()

	issue = dontmanage.get_doc(
		{
			"doctype": "Issue",
...
def create_customer(name, customer_group, territory):
	...
	if not dontmanage.db.exists("Customer", {"customer_name": name}):
		dontmanage.get_doc(
			{
				"doctype": "Customer",
				"customer_name": name,
...
def create_customer_group(customer_group):
	...
	if not dontmanage.db.exists("Customer Group", {"customer_group_name": customer_group}):
		dontmanage.get_doc({"doctype": "Customer Group", "customer_group_name": customer_group}).insert(
...
def create_territory(territory):
	...
	if not dontmanage.db.exists("Territory", {"territory_name": territory}):
		dontmanage.get_doc(
			{
				"doctype": "Territory",
