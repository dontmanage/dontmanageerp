...
from dontmanageerp.support.doctype.issue.test_issue import create_customer, make_issue
from dontmanageerp.support.doctype.service_level_agreement.test_service_level_agreement import (
	create_service_level_agreements_for_issues,
)
from dontmanageerp.support.report.issue_analytics.issue_analytics import execute
...
class TestIssueAnalytics(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		dontmanage.db.sql("delete from `tabIssue` where company='_Test Company'")
...
def create_issue_types():
	for entry in ["Bug", "Service Request", "Discomfort"]:
		if not dontmanage.db.exists("Issue Type", entry):
			dontmanage.get_doc({"doctype": "Issue Type", "__newname": entry}).insert()
