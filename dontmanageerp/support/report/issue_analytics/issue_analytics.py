...
class IssueAnalytics(object):
	...
	def get_periodic_data(self):
		self.issue_periodic_data = dontmanage._dict()
			...
			if self.filters.based_on == "Assigned To":
				if d._assign:
					for entry in json.loads(d._assign):
						self.issue_periodic_data.setdefault(entry, dontmanage._dict()).setdefault(period, 0.0)
				...
				self.issue_periodic_data.setdefault(value, dontmanage._dict()).setdefault(period, 0.0)
