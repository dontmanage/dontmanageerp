...
class IssueSummary(object):
	...
	def get_summary_data(self):
		self.issue_summary_data = dontmanage._dict()
			...
			if self.filters.based_on == "Assigned To":
				if d._assign:
					for entry in json.loads(d._assign):
						self.issue_summary_data.setdefault(entry, dontmanage._dict()).setdefault(status, 0.0)
						self.issue_summary_data.setdefault(entry, dontmanage._dict()).setdefault(agreement_status, 0.0)
						self.issue_summary_data.setdefault(entry, dontmanage._dict()).setdefault("total_issues", 0.0)
				...
				self.issue_summary_data.setdefault(value, dontmanage._dict()).setdefault(status, 0.0)
				self.issue_summary_data.setdefault(value, dontmanage._dict()).setdefault(agreement_status, 0.0)
				self.issue_summary_data.setdefault(value, dontmanage._dict()).setdefault("total_issues", 0.0)
		...
		if issues:
			if self.filters.based_on == "Assigned To":
					...
					if d._assign:
						for entry in json.loads(d._assign):
							for metric in metrics_list:
								self.issue_summary_data.setdefault(entry, dontmanage._dict()).setdefault(metric, 0.0)
				...
				for entry in data:
					...
					for metric in metrics_list:
						self.issue_summary_data.setdefault(value, dontmanage._dict()).setdefault(metric, 0.0)
