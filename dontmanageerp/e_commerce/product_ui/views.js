dontmanageerp.ProductView =  class {
	/* Options:
		- View Type
	...
	*/
	...
	prepare_toolbar() {
		...
		new dontmanageerp.ProductSearch();
	}
	...
	get_item_filter_data(from_filters=false) {
		...
		dontmanage.call({
			method: "dontmanageerp.e_commerce.api.get_product_filter_data",
			args: {
				query_args: args
			},
	...
	get_query_filters() {
		const filters = dontmanage.utils.get_query_params();
	...
	bind_paging_action() {
		...
		$('.btn-prev, .btn-next').click((e) => {
			...
			let path = window.location.pathname + '?' + dontmanage.utils.get_url_from_dict(query_params);
	...
	bind_filters() {
		...
		$('.filter-lookup-input').on('keydown', dontmanage.utils.debounce((e) => {
	...
	change_route_with_filters() {
		let route_params = dontmanage.utils.get_query_params();
	...
	render_no_products_section(error=false) {
		...
		let no_results_section = `
			<div class="cart-empty dontmanage-card mt-4">
