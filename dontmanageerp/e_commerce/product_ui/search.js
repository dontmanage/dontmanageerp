dontmanageerp.ProductSearch = class {
	constructor(opts) {
	...
	bindSearchAction() {
		...
		this.searchBox.on("input", (e) => {
			...
			dontmanage.call({
				method: "dontmanageerp.templates.pages.product_search.search",
				args: {
					query: query
				},
				callback: (data) => {
	...
	populateResults(product_results) {
		...
		product_results.forEach((res) => {
			let thumbnail = res.thumbnail || '/assets/dontmanageerp/images/ui-states/cart-empty-state.png';
			html += `
