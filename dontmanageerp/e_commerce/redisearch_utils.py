...
def get_indexable_web_fields():
	...
	web_item_meta = dontmanage.get_meta("Website Item", cached=True)
	valid_fields = filter(
...
def is_redisearch_enabled():
	...
	is_redisearch_enabled = dontmanage.db.get_single_value("E Commerce Settings", "is_redisearch_enabled")
...
def is_search_module_loaded():
	try:
		...
		parsed_output = " ".join(
			(" ".join([dontmanage.as_unicode(s) for s in o if not isinstance(s, int)]) for o in out)
		)
...
def make_key(key):
	return "{0}|{1}".format(dontmanage.conf.db_name, key).encode("utf-8")
...
def create_website_items_index():
	...
	# CREATE index
	client = Client(make_key(WEBSITE_ITEM_INDEX), conn=dontmanage.cache())
	...
	idx_fields = dontmanage.db.get_single_value("E Commerce Settings", "search_index_fields")
...
def insert_to_name_ac(web_name, doc_name):
	ac = AutoCompleter(make_key(WEBSITE_ITEM_NAME_AUTOCOMPLETE), conn=dontmanage.cache())
...
def create_item_groups_autocomplete_dict(autocompleter):
	...
	published_item_groups = dontmanage.get_all(
	...
	for item_group in published_item_groups:
		...
		autocompleter.add_suggestions(
			Suggestion(
				...
				score=dontmanage.utils.flt(item_group.weightage) or 1.0,
...
def reindex_all_web_items():
	items = dontmanage.get_all("Website Item", fields=get_fields_indexed(), filters={"published": True})
...
def get_cache_key(name):
	name = dontmanage.scrub(name)
	return f"{WEBSITE_ITEM_KEY_PREFIX}{name}"


def get_fields_indexed():
	fields_to_index = dontmanage.db.get_single_value("E Commerce Settings", "search_index_fields")
...
def raise_redisearch_error():
	...
	log = dontmanage.log_error("Redisearch Error")
	log_link = dontmanage.utils.get_link_to_form("Error Log", log.name)
