...
from dontmanage.utils import comma_and, flt, unique
...
class ShoppingCartSetupError(dontmanage.ValidationError):
	pass


class ECommerceSettings(Document):
	def onload(self):
		self.get("__onload").quotation_series = dontmanage.get_meta("Quotation").get_options("naming_series")
	...
	def validate(self):
		...
		dontmanage.clear_document_cache("E Commerce Settings", "E Commerce Settings")

		self.is_redisearch_enabled_pre_save = dontmanage.db.get_single_value(
	...
	def validate_field_filters(filter_fields, enable_field_filters):
		...
		web_item_meta = dontmanage.get_meta("Website Item")
		valid_fields = [
		...
		for row in filter_fields:
			if row.fieldname not in valid_fields:
				dontmanage.throw(
					...
					).format(row.idx, dontmanage.bold(row.fieldname))
				)
	...
	def validate_search_index_fields(self):
		...
		if not (set(fields).issubset(allowed_indexable_fields)):
			...
			if num_invalid_fields > 1:
				dontmanage.throw(
					_("{0} are not valid options for Search Index Field.").format(dontmanage.bold(invalid_fields))
				)
			else:
				dontmanage.throw(
					_("{0} is not a valid option for Search Index Field.").format(dontmanage.bold(invalid_fields))
				)
	...
	def validate_price_list_exchange_rate(self):
		...
		price_list_currency = dontmanage.db.get_value("Price List", self.price_list, "currency")

		if not company_currency:
			msg = f"Please specify currency in Company {self.company}"
			dontmanage.throw(_(msg), title=_("Missing Currency"), exc=ShoppingCartSetupError)

		if not price_list_currency:
			msg = f"Please specify currency in Price List {dontmanage.bold(self.price_list)}"
			dontmanage.throw(_(msg), title=_("Missing Currency"), exc=ShoppingCartSetupError)
		...
		if price_list_currency != company_currency:
			...
			if not flt(exchange_rate):
				...
				dontmanage.throw(_(msg), title=_("Missing"), exc=ShoppingCartSetupError)

	def validate_tax_rule(self):
		if not dontmanage.db.get_value("Tax Rule", {"use_for_shopping_cart": 1}, "name"):
			dontmanage.throw(dontmanage._("Set Tax Rule for shopping cart"), ShoppingCartSetupError)
...
def validate_cart_settings(doc=None, method=None):
	dontmanage.get_doc("E Commerce Settings", "E Commerce Settings").run_method("validate")


def get_shopping_cart_settings():
	return dontmanage.get_cached_doc("E Commerce Settings")
...
def check_shopping_cart_enabled():
	if not get_shopping_cart_settings().enabled:
		dontmanage.throw(_("You need to enable Shopping Cart"), ShoppingCartSetupError)
