...
from dontmanageerp.e_commerce.doctype.item_review.item_review import (
	UnverifiedReviewer,
	add_item_review,
	get_item_reviews,
)
...
class TestItemReview(unittest.TestCase):
	def setUp(self):
		...
		if not dontmanage.db.exists("Website Item", {"item_code": "Test Mobile Phone"}):
	...
	def test_add_and_get_item_reviews_from_customer(self):
		...
		# create user
		web_item = dontmanage.db.get_value("Website Item", {"item_code": "Test Mobile Phone"})
		...
		try:
			...
			review_name = dontmanage.db.get_value("Item Review", {"website_item": web_item})
		except Exception:
		...
		dontmanage.delete_doc("Item Review", review_name)
		customer.delete()
