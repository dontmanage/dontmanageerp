...
class UnverifiedReviewer(dontmanage.ValidationError):
	pass
...
def get_item_reviews(web_item, start=0, end=10, data=None):
	...
	if settings and settings.get("enable_reviews"):
		reviews_cache = dontmanage.cache().hget("item_reviews", web_item)
...
def get_queried_reviews(web_item, start=0, end=10, data=None):
	...
	data.reviews = dontmanage.db.get_all(
		"Item Review",
	...
	rating_data = dontmanage.db.get_all(
		"Item Review",
		filters={"website_item": web_item},
...
def set_reviews_in_cache(web_item, reviews_dict):
	dontmanage.cache().hset("item_reviews", web_item, reviews_dict)
...
def add_item_review(web_item, title, rating, comment=None):
	...
	if dontmanage.session.user == "Guest":
		...
		dontmanage.throw(_("You are not verified to write a review yet."), exc=UnverifiedReviewer)

	if not dontmanage.db.exists("Item Review", {"user": dontmanage.session.user, "website_item": web_item}):
		doc = dontmanage.get_doc(
			{
				...
				"item": dontmanage.db.get_value("Website Item", web_item, "item_code"),
				"review_title": title,
				"rating": rating,
				"comment": comment,
			}
...
def get_customer(silent=False):
	...
	if customer:
		return dontmanage.db.get_value("Customer", customer)
	elif silent:
		return None
	else:
