...
from dontmanageerp.e_commerce.doctype.e_commerce_settings.test_e_commerce_settings import (
...
from dontmanageerp.stock.doctype.item.item import DataValidationError
...
class TestWebsiteItem(unittest.TestCase):
	...
	def test_index_creation(self):
		...
		from dontmanageerp.e_commerce.doctype.website_item.website_item import on_doctype_update

		on_doctype_update()

		indices = dontmanage.db.sql("show index from `tabWebsite Item`", as_dict=1)
	...
	def test_publish_variant_and_template(self):
		...
		self.assertIsNone(dontmanage.db.exists("Website Item", {"item_code": variant.variant_of}))
		...
		try:
			template_web_item = dontmanage.get_doc("Website Item", {"item_code": variant.variant_of})
	...
	def test_impact_on_merging_items(self):
		...
		with self.assertRaises(DataValidationError):
			dontmanage.rename_doc("Item", "Test First Item", "Test Second Item", merge=True)
	...
	def test_website_item_breadcrumbs(self):
		...
		"""
		from dontmanageerp.setup.doctype.item_group.item_group import get_parent_item_groups
		...
		if not dontmanage.db.exists("Website Item", {"item_code": item_code}):
			...
			web_item = dontmanage.get_cached_doc("Website Item", {"item_code": item_code})

		dontmanage.db.set_value("Item Group", "_Test Item Group B - 1", "show_in_website", 1)
		dontmanage.db.set_value("Item Group", "_Test Item Group B", "show_in_website", 1)
	...
	def test_website_item_price_for_logged_in_user(self):
		...
		dontmanage.set_user("test_contact_customer@example.com")

		# check if price and slashed price is fetched correctly
		dontmanage.local.shopping_cart_settings = None
	...
	def test_website_item_stock_when_out_of_stock(self):
		...
		# tear down
		dontmanage.get_cached_doc("Website Item", {"item_code": "Test Mobile Phone"}).delete()
	...
	def test_website_item_stock_when_in_stock(self):
		...
		# unset warehouse
		dontmanage.db.set_value("Website Item", {"item_code": item_code}, "website_warehouse", "")
	...
	def test_recommended_item(self):
		...
		dontmanage.get_cached_doc("Item", "Test Mobile Phone 1").delete()
...
def make_web_item_price(**kwargs):
	...
	if not dontmanage.db.exists("Item Price", {"item_code": item_code}):
		...
		item_price = dontmanage.get_cached_doc("Item Price", {"item_code": item_code})
...
def make_web_pricing_rule(**kwargs):
	...
	if not dontmanage.db.exists("Pricing Rule", title):
		pricing_rule = dontmanage.get_doc(
			{
		...
		pricing_rule = dontmanage.get_doc("Pricing Rule", {"title": title})
...
def create_user_and_customer_if_not_exists(email, first_name=None):
	...
	contact = dontmanage.get_last_doc("Contact", filters={"email_id": email})
