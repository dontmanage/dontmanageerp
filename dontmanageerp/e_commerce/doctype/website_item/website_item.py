...
if TYPE_CHECKING:
	from dontmanageerp.stock.doctype.item.item import Item

import dontmanage
from dontmanage import _
from dontmanage.utils import cint, cstr, flt, random_string
...
from dontmanageerp.e_commerce.doctype.item_review.item_review import get_item_reviews
from dontmanageerp.e_commerce.redisearch_utils import (
	delete_item_from_index,
	insert_item_to_index,
	update_index_for_item,
)
from dontmanageerp.e_commerce.shopping_cart.cart import _set_price_list
from dontmanageerp.setup.doctype.item_group.item_group import (
	get_parent_item_groups,
	invalidate_cache_for,
)
from dontmanageerp.utilities.product import get_price
...
class WebsiteItem(WebsiteGenerator):
	...
	def autoname(self):
		...
		from dontmanage.model.naming import get_default_naming_series, make_autoname
	...
	def validate(self):
		...
		if not self.item_code:
			dontmanage.throw(_("Item Code is required"), title=_("Mandatory"))
		...
		if not self.get("__islocal"):
			wig = dontmanage.qb.DocType("Website Item Group")
			query = (
				dontmanage.qb.from_(wig)
	...
	def validate_duplicate_website_item(self):
		existing_web_item = dontmanage.db.exists("Website Item", {"item_code": self.item_code})
		if existing_web_item and existing_web_item != self.name:
			message = _("Website Item already exists against Item {0}").format(dontmanage.bold(self.item_code))
			dontmanage.throw(message, title=_("Already Published"))

	def publish_unpublish_desk_item(self, publish=True):
		if dontmanage.db.get_value("Item", self.item_code, "published_in_website") and publish:
			return  # if already published don't publish again
		dontmanage.db.set_value("Item", self.item_code, "published_in_website", publish)

	def make_route(self):
		"""Called from set_route in WebsiteGenerator."""
		if not self.route:
			return (
				cstr(dontmanage.db.get_value("Item Group", self.item_group, "route"))
				+ "/"
	...
	def update_template_item(self):
		...
		if self.variant_of:
			if self.published:
				# show template
				template_item = dontmanage.get_doc("Item", self.variant_of)
	...
	def validate_website_image(self):
		...
		if not file_doc:
			...
			dontmanage.msgprint(_("Website Image should be a public file or website URL"))

			self.website_image = None

	def make_thumbnail(self):
		"""Make a thumbnail of `website_image`"""
		if dontmanage.flags.in_import or dontmanage.flags.in_migrate:
			return

		import requests.exceptions

		db_website_image = dontmanage.db.get_value(self.doctype, self.name, "website_image")
		...
		if self.website_image and not self.thumbnail:
			...
			try:
				...
				dontmanage.msgprint(_("Warning: Invalid attachment {0}").format(self.website_image))
				self.website_image = None
	...
	def get_context(self, context):
		...
		self.attributes = dontmanage.get_all(
		...
		if dontmanage.db.exists(
			"Wishlist Item", {"item_code": self.item_code, "parent": dontmanage.session.user}
		):
			context.wished = True
	...
	def set_selected_attributes(self, variants, context, attribute_values_available):
		for variant in variants:
			variant.attributes = dontmanage.get_all(
				"Item Variant Attribute",
			...
			variant.attribute_map = dontmanage._dict(
	...
	def set_attribute_values(self, attributes, context, attribute_values_available):
		for attr in attributes:
			...
			if cint(dontmanage.db.get_value("Item Attribute", attr.attribute, "numeric_values")):
				for val in sorted(attribute_values_available.get(attr.attribute, []), key=flt):
					values.append(val)
			else:
				# get list of values defined (for sequence)
				for attr_value in dontmanage.db.get_all(
	...
	def set_metatags(self, context):
		context.metatags = dontmanage._dict({})

		safe_description = dontmanage.utils.to_markdown(self.description)

		context.metatags.url = dontmanage.utils.get_url() + "/" + context.route

		if context.website_image:
			if context.website_image.startswith("http"):
				url = context.website_image
			else:
				url = dontmanage.utils.get_url() + context.website_image
		...
		context.metatags["og:site_name"] = "DontManageErp"

	def set_shopping_cart_data(self, context):
		from dontmanageerp.e_commerce.shopping_cart.product_info import get_product_info_for_website
	...
	def copy_specification_from_item_group(self):
		...
		if self.item_group:
			for label, desc in dontmanage.db.get_values(
	...
	def get_tabs(self):
		...
		tab_values["tab_1_content"] = dontmanage.render_template(
	...
	def get_recommended_items(self, settings):
		ri = dontmanage.qb.DocType("Recommended Items")
		wi = dontmanage.qb.DocType("Website Item")

		query = (
			dontmanage.qb.from_(ri)
			.join(wi)
		...
		if settings.show_price:
			is_guest = dontmanage.session.user == "Guest"
...
def invalidate_cache_for_web_item(doc):
	...
	from dontmanageerp.stock.doctype.item.item import invalidate_item_variants_cache_for_website
...
def on_doctype_update():
	...
	dontmanage.db.add_index("Website Item", ["route(500)"])
...
def make_website_item(doc: "Item", save: bool = True) -> Union["WebsiteItem", List[str]]:
	...
	if dontmanage.db.exists("Website Item", {"item_code": doc.get("item_code")}):
		message = _("Website Item already exists against {0}").format(dontmanage.bold(doc.get("item_code")))
		dontmanage.throw(message, title=_("Already Published"))

	website_item = dontmanage.new_doc("Website Item")
	...
	if not dontmanage.flags.in_migrate and (doc.get("image") and not website_item.website_image):
