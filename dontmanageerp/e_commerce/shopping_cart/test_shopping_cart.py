...
from dontmanage.utils import add_months, cint, nowdate

from dontmanageerp.accounts.doctype.tax_rule.tax_rule import ConflictingTaxRule
from dontmanageerp.e_commerce.doctype.website_item.website_item import make_website_item
from dontmanageerp.e_commerce.shopping_cart.cart import (
	_get_cart_quotation,
	get_cart_quotation,
	get_party,
...
class TestShoppingCart(unittest.TestCase):
	...
	def setUp(self):
		...
		if not dontmanage.db.exists("Website Item", {"item_code": "_Test Item"}):
			make_website_item(dontmanage.get_cached_doc("Item", "_Test Item"))

		if not dontmanage.db.exists("Website Item", {"item_code": "_Test Item 2"}):
			make_website_item(dontmanage.get_cached_doc("Item", "_Test Item 2"))
	...
	def test_get_cart_new_user(self):
		...
		self.assertEqual(
			...
			dontmanage.db.get_value("Contact", dict(email_id="test_cart_user@example.com")),
		)
		self.assertEqual(quotation.contact_email, dontmanage.session.user)
	...
	def test_add_item_variant_without_web_item_to_cart(self):
		...
		dontmanage.render_template("templates/includes/cart/cart_items.html", cart)
	...
	def test_cart_without_checkout_and_draft_quotation(self):
		...
		quote_doctstatus = cint(dontmanage.db.get_value("Quotation", quote_name, "docstatus"))

		self.assertEqual(quote_doctstatus, 0)

		dontmanage.db.set_value("E Commerce Settings", None, "save_quotations_as_draft", 0)
		dontmanage.local.shopping_cart_settings = None
		update_cart("_Test Item", 1)
		quote_name = request_for_quotation()  # Request for Quote
		quote_doctstatus = cint(dontmanage.db.get_value("Quotation", quote_name, "docstatus"))
	...
	def create_tax_rule(self):
		tax_rule = dontmanage.get_test_records("Tax Rule")[0]
		try:
			dontmanage.get_doc(tax_rule).insert(ignore_if_duplicate=True)
		except (dontmanage.DuplicateEntryError, ConflictingTaxRule):
			pass

	def create_quotation(self):
		quotation = dontmanage.new_doc("Quotation")

		values = {
			"doctype": "Quotation",
			"quotation_to": "Customer",
			"order_type": "Shopping Cart",
			"party_name": get_party(dontmanage.session.user).name,
			"docstatus": 0,
			...
			"taxes": dontmanage.get_doc("Sales Taxes and Charges Template", "_Test Tax 1 - _TC").taxes,
			"company": "_Test Company",
		}
	...
	def enable_shopping_cart(self):
		settings = dontmanage.get_doc("E Commerce Settings", "E Commerce Settings")
	...
	def login_as_new_user(self):
		...
		dontmanage.set_user("test_cart_user@example.com")
	...
	def login_as_customer(
		...
		dontmanage.set_user(email)

	def clear_existing_quotations(self):
		quotations = dontmanage.get_all(
			"Quotation",
		...
		for quotation in quotations:
			dontmanage.delete_doc("Quotation", quotation, ignore_permissions=True, force=True)
