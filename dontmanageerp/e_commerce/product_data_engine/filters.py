...
import dontmanage
from dontmanage.utils import floor


class ProductFiltersBuilder:
	def __init__(self, item_group=None):
		if not item_group:
			self.doc = dontmanage.get_doc("E Commerce Settings")
		else:
			self.doc = dontmanage.get_doc("Item Group", item_group)
	...
	def get_field_filters(self):
		...
		for df in fields:
			...
			if df.fieldtype == "Link":
				if self.item_group:
					include_child = dontmanage.db.get_value("Item Group", self.item_group, "include_descendants")
					if include_child:
				...
				if dontmanage.db.get_single_value("E Commerce Settings", "hide_variants"):
					item_filters["variant_of"] = ["is", "not set"]

				# Get link field values attached to published items
				item_values = dontmanage.get_all(
					"Website Item",
					fields=[df.fieldname],
	...
	def get_filtered_link_doctype_records(self, field):
		...
		meta = dontmanage.get_meta(link_doctype, cached=True) if link_doctype else None
		if meta:
			filters = self.get_link_doctype_filters(meta)
			link_doctype_values = set(d.name for d in dontmanage.get_all(link_doctype, filters))
	...
	def get_attribute_filters(self):
		...
		for name, values in attribute_value_map.items():
			out.append(dontmanage._dict(name=name, item_attribute_values=values))
		return out
