...
from dontmanageerp.e_commerce.doctype.item_review.item_review import get_customer
from dontmanageerp.e_commerce.shopping_cart.product_info import get_product_info_for_website
from dontmanageerp.utilities.product import get_non_stock_item_status
...
class ProductQuery:
	...
	def __init__(self):
		self.settings = dontmanage.get_doc("E Commerce Settings")
	...
	def query_items(self, start=0):
		...
		# dontmanage does not accept two parameters for limit
		# https://dev.mysql.com/doc/refman/8.0/en/select.html#id4651989
		count_items = dontmanage.db.get_all(
			"Website Item",
	...
	def query_items_with_attributes(self, attributes, start=0):
		...
		for attribute, values in attributes.items():
			...
			item_code_list = dontmanage.db.get_all(
				"Item",
				fields=["item_code"],
				filters=[
	...
	def build_fields_filters(self, filters):
		...
		for field, values in filters.items():
			...
			meta = dontmanage.get_meta("Website Item", cached=True)
			df = meta.get_field(field)
			if df.fieldtype == "Table MultiSelect":
				child_doctype = df.options
				child_meta = dontmanage.get_meta(child_doctype, cached=True)
	...
	def build_item_group_filters(self, item_group):
		...
		from dontmanageerp.setup.doctype.item_group.item_group import get_child_groups_for_website
		...
		if dontmanage.db.get_value("Item Group", item_group, "include_descendants"):
	...
	def build_search_filters(self, search_term):
		...
		meta = dontmanage.get_meta("Website Item")
		meta_fields = set(meta.get_search_fields())

		# Join the meta fields and default fields set
		search_fields = default_fields.union(meta_fields)
		if dontmanage.db.count("Website Item", cache=True) > 50000:
	...
	def add_display_details(self, result, discount_list, cart_items):
		...
		for item in result:
			...
			if dontmanage.db.exists(
				"Wishlist Item", {"item_code": item.item_code, "parent": dontmanage.session.user}
			):
				item.wished = True
	...
	def get_stock_availability(self, item):
		...
		is_stock_item = dontmanage.get_cached_value("Item", item.item_code, "is_stock_item")
	...
	def filter_results_by_discount(self, fields, result):
		if fields and fields.get("discount"):
			discount_percent = dontmanage.utils.flt(fields["discount"][0])
			result = [
				row
				for row in result
