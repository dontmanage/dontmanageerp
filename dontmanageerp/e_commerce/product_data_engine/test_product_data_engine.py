...
from dontmanageerp.e_commerce.doctype.website_item.test_website_item import create_regular_web_item
from dontmanageerp.e_commerce.product_data_engine.filters import ProductFiltersBuilder
from dontmanageerp.e_commerce.product_data_engine.query import ProductQuery
...
class TestProductDataEngine(unittest.TestCase):
	...
	def test_change_product_ranking(self):
		...
		old_ranking = dontmanage.db.get_value("Website Item", {"item_code": item_code}, "ranking")

		# low rank, appears on second page
		self.assertEqual(old_ranking, 2)

		# set ranking as highest rank
		dontmanage.db.set_value("Website Item", {"item_code": item_code}, "ranking", 10)
		...
		# tear down
		dontmanage.db.set_value("Website Item", {"item_code": item_code}, "ranking", old_ranking)

	def test_product_list_field_filter_builder(self):
		"Test if field filters are fetched correctly."
		dontmanage.db.set_value("Item Group", "Raw Material", "show_in_website", 0)
		...
		dontmanage.db.set_value("Item Group", "Raw Material", "show_in_website", 1)
		field_filters = filter_engine.get_field_filters()
	...
	def test_product_list_discount_filter_builder(self):
		...
		from dontmanageerp.e_commerce.doctype.website_item.test_website_item import (
			make_web_item_price,
	...
	def test_product_list_with_api(self):
		...
		from dontmanageerp.e_commerce.api import get_product_filter_data
	...
	def test_custom_field_as_filter(self):
		...
		settings = dontmanage.get_doc("E Commerce Settings")
...
def create_variant_web_item():
	...
	if not dontmanage.db.exists("Item", "Test Web Item-L"):
		variant = create_variant("Test Web Item", {"Test Size": "Large"})
		variant.save()

	if not dontmanage.db.exists("Website Item", {"variant_of": "Test Web Item"}):
