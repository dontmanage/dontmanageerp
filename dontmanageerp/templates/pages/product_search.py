...
from dontmanageerp.e_commerce.shopping_cart.product_info import set_product_info_for_website
from dontmanageerp.setup.doctype.item_group.item_group import get_item_for_list_in_html
...
def get_product_data(search=None, start=0, limit=12):
	...
	return dontmanage.db.sql(query, {"search": search}, as_dict=1)  # nosemgrep
...
def product_search(query, limit=10, fuzzy_search=True):
	...
	search_results["results"] = sorted(
		search_results["results"], key=lambda k: dontmanage.utils.cint(k["ranking"]), reverse=True
	)
...
def get_category_suggestions(query):
	...
	if not is_redisearch_enabled():
		...
		categories = dontmanage.db.get_all(
			"Item Group",
	...
	ac = AutoCompleter(make_key(WEBSITE_ITEM_CATEGORY_AUTOCOMPLETE), conn=dontmanage.cache())
