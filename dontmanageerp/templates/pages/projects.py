...
def get_context(context):
	project_user = dontmanage.db.get_value(
		"Project User",
		{"parent": dontmanage.form_dict.project, "user": dontmanage.session.user},
		["user", "view_attachments"],
		as_dict=True,
	)
	if dontmanage.session.user != "Administrator" and (not project_user or dontmanage.session.user == "Guest"):
		raise dontmanage.PermissionError

	context.no_cache = 1
	context.show_sidebar = True
	project = dontmanage.get_doc("Project", dontmanage.form_dict.project)

	project.has_permission("read")

	project.tasks = get_tasks(
		project.name, start=0, item_status="open", search=dontmanage.form_dict.get("search")
	)

	project.timesheets = get_timesheets(project.name, start=0, search=dontmanage.form_dict.get("search"))
...
def get_task_html(project, start=0, item_status=None):
	return dontmanage.render_template(
		"dontmanageerp/templates/includes/projects/project_tasks.html",
		{
			"doc": {
				"name": project,
...
def get_timesheet_html(project, start=0):
	return dontmanage.render_template(
		"dontmanageerp/templates/includes/projects/project_timesheets.html",
