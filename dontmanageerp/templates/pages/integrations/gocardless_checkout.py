...
from dontmanage.utils import flt, get_url
...
def get_context(context):
	...
	if not (set(expected_keys) - set(dontmanage.form_dict.keys())):
		...
		context["header_img"] = dontmanage.db.get_value(
...
def check_mandate(data, reference_doctype, reference_docname):
	...
	payer = dontmanage.get_doc("Customer", data["payer_name"])

	if payer.customer_type == "Individual" and payer.customer_primary_contact is not None:
		primary_contact = dontmanage.get_doc("Contact", payer.customer_primary_contact)
		prefilled_customer = {
		...
		if primary_contact.email_id is not None:
			...
			prefilled_customer.update({"email": dontmanage.session.user})

	else:
		prefilled_customer = {"company_name": payer.name, "email": dontmanage.session.user}
	...
	try:
		redirect_flow = client.redirect_flows.create(
			params={
				...
				"session_token": dontmanage.session.user,
