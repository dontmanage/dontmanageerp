dontmanage.ready(function () {
	...
	var reload_items = function (item_status, item, $btn) {
		$.ajax({
			...
			dataType: "json",
			data: {
				cmd: "dontmanageerp.templates.pages.projects.get_" + item + "_html",
				project: "{{ doc.name }}",
	...
	var close_item = function (item, item_name) {
		...
		dontmanage.call({
			btn: this,
			type: "POST",
			method: "dontmanageerp.templates.pages.projects.set_" + item + "_status",
			args: args,
			callback: function (r) {
				if (r.exc) {
					if (r._server_messages) dontmanage.msgprint(r._server_messages);
				} else {
					$(this).remove();
				}
			},
