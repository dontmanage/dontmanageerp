class ItemConfigure {
	...
	show_range_input_if_applicable(fieldname) {
		...
		if (changed_value && changed_value.includes(' to ')) {
			...
			if (!numbers.some(n => isNaN(n))) {
				...
				const control = dontmanage.ui.form.make_control({
					df: {
						fieldtype: 'Int',
	...
	get_html_for_item_found({ filtered_items_count, filtered_items, exact_match, product_info, available_qty, settings }) {
		...
		let item_add_to_cart = one_item ? `
			...
			>
				<span class="mr-2">
					${dontmanage.utils.icon('assets', 'md')}
				</span>
				${__("Add to Cart")}
			</button>
		` : '';
	...
	get_next_attribute_and_values(selected_attributes) {
		return this.call('dontmanageerp.e_commerce.variant_selector.utils.get_next_attribute_and_values', {
	...
	get_attributes_and_values() {
		return this.call('dontmanageerp.e_commerce.variant_selector.utils.get_attributes_and_values', {
			item_code: this.item_code
		});
	}
