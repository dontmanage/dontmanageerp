...
dontmanage.ready(function() {
	...
	dontmanage.call({
		type: "POST",
		method: "dontmanageerp.e_commerce.shopping_cart.product_info.get_product_info_for_website",
		args: {
	...
	$("#item-add-to-cart button").on("click", function() {
		dontmanage.provide('dontmanageerp.shopping_cart');

		dontmanageerp.shopping_cart.update_cart({
			item_code: get_item_code(),
	...
	$("[itemscope] .item-view-attribute .form-control").on("change", function() {
		try {
			...
			if (!item_code) {
				dontmanage.msgprint(__("Cannot find a matching Item. Please select some other value for {0}.", [attribute]))
				throw e;
			}
		}
