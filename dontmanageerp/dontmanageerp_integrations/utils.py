	...
	def innerfn(fn):
		...
		if dontmanage.request and settings and settings.get(secret_key) and not dontmanage.flags.in_test:
			sig = base64.b64encode(
				hmac.new(
					settings.get(secret_key).encode("utf8"), dontmanage.request.data, hashlib.sha256
				).digest()
			)
...
def get_webhook_address(connector_name, method, exclude_uri=False, force_https=False):
	endpoint = f"dontmanageerp.dontmanageerp_integrations.connectors.{connector_name}.{method}"
