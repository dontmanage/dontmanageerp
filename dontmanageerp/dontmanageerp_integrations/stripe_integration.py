...
def create_stripe_subscription(gateway_controller, data):
	...
	stripe_settings = dontmanage.get_doc("Stripe Settings", gateway_controller)
	stripe_settings.data = dontmanage._dict(data)
	...
	try:
		...
		stripe_settings.payment_plans = dontmanage.get_doc(
...
def create_subscription_on_stripe(stripe_settings):
	...
	for payment_plan in stripe_settings.payment_plans:
		plan = dontmanage.db.get_value("Subscription Plan", payment_plan.plan, "product_price_id")
	...
	try:
		...
		if subscription.status == "active":
			...
			dontmanage.log_error(f"Stripe Subscription ID {subscription.id}: Payment failed")
	except Exception:
		stripe_settings.integration_request.db_set("status", "Failed", update_modified=False)
		stripe_settings.log_error("Unable to create Stripe subscription")
