...
def verify_request():
	...
	sig = base64.b64encode(
		hmac.new(
			woocommerce_settings.secret.encode("utf8"), dontmanage.request.data, hashlib.sha256
		).digest()
	)

	if (
		dontmanage.request.data
		and not sig == dontmanage.get_request_header("X-Wc-Webhook-Signature", "").encode()
	):
		dontmanage.throw(_("Unverified Webhook Data"))
	dontmanage.set_user(woocommerce_settings.creation_user)
...
def order(*args, **kwargs):
	try:
		...
		error_message = (
			dontmanage.get_traceback() + "\n\n Request Data: \n" + json.loads(dontmanage.request.data).__str__()
		)
		dontmanage.log_error("WooCommerce Error", error_message)
		raise


def _order(*args, **kwargs):
	woocommerce_settings = dontmanage.get_doc("Woocommerce Settings")
	if dontmanage.flags.woocomm_test_order_data:
		order = dontmanage.flags.woocomm_test_order_data
		event = "created"

	elif dontmanage.request and dontmanage.request.data:
		verify_request()
		try:
			order = json.loads(dontmanage.request.data)
		except ValueError:
			# woocommerce returns 'webhook_id=value' for the first request which is not JSON
			order = dontmanage.request.data
		event = dontmanage.get_request_header("X-Wc-Webhook-Event")

	else:
		return "success"

	if event == "created":
		sys_lang = dontmanage.get_single("System Settings").language or "en"
...
def link_customer_and_address(raw_billing_data, raw_shipping_data, customer_name):
	...
	customer_exists = dontmanage.get_value("Customer", {"woocommerce_email": customer_woo_com_email})
	if not customer_exists:
		# Create Customer
		customer = dontmanage.new_doc("Customer")
	else:
		# Edit Customer
		customer = dontmanage.get_doc("Customer", {"woocommerce_email": customer_woo_com_email})
	...
	if customer_exists:
		dontmanage.rename_doc("Customer", old_name, customer_name)
		for address_type in (
			"Billing",
			"Shipping",
		):
...
def create_address(raw_data, customer, address_type):
	...
	address.country = dontmanage.get_value("Country", {"code": raw_data.get("country", "IN").lower()})
...
def rename_address(address, customer):
	...
	dontmanage.rename_doc("Address", old_address_title, new_address_title)
...
def link_items(items_list, woocommerce_settings, sys_lang):
	for item_data in items_list:
		...
		if not dontmanage.db.get_value("Item", {"woocommerce_id": item_woo_com_id}, "name"):
			# Create Item
...
def create_sales_order(order, woocommerce_settings, customer_name, sys_lang):
	new_sales_order = dontmanage.new_doc("Sales Order")
	...
	new_sales_order.delivery_date = dontmanage.utils.add_days(created_date[0], delivery_after)
...
def set_items_in_sales_order(new_sales_order, woocommerce_settings, order, sys_lang):
	company_abbr = dontmanage.db.get_value("Company", woocommerce_settings.company, "abbr")

	default_warehouse = _("Stores - {0}", sys_lang).format(company_abbr)
	if not dontmanage.db.exists("Warehouse", default_warehouse) and not woocommerce_settings.warehouse:
		dontmanage.throw(_("Please set Warehouse in Woocommerce Settings"))

	for item in order.get("line_items"):
		woocomm_item_id = item.get("product_id")
		found_item = dontmanage.get_doc("Item", {"woocommerce_id": cstr(woocomm_item_id)})
