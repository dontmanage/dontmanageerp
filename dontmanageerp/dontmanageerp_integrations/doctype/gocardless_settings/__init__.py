...
def set_mandate_status(event):
	...
	for mandate in mandates:
		dontmanage.db.set_value("GoCardless Mandate", mandate, "disabled", disabled)


def authenticate_signature(r):
	"""Returns True if the received signature matches the generated signature"""
	received_signature = dontmanage.get_request_header("Webhook-Signature")
...
def get_webhook_keys():
	...
	return dontmanage.cache().get_value("gocardless_webhooks_secret", _get_webhook_keys)


def clear_cache():
	dontmanage.cache().delete_value("gocardless_webhooks_secret")
