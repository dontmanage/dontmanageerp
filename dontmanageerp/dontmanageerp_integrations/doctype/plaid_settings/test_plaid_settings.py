...
class TestPlaidSettings(unittest.TestCase):
	...
	def tearDown(self):
		for bt in dontmanage.get_all("Bank Transaction"):
			doc = dontmanage.get_doc("Bank Transaction", bt.name)
			doc.cancel()
			doc.delete()

		for doctype in ("Bank Account", "Bank Account Type", "Bank Account Subtype"):
			for d in dontmanage.get_all(doctype):
	...
	def test_add_account_type(self):
		...
		self.assertEqual(dontmanage.get_doc("Bank Account Type", "brokerage").name, "brokerage")
	...
	def test_new_transaction(self):
		...
		self.assertTrue(len(dontmanage.get_all("Bank Transaction")) == 1)
