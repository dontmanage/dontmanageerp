...
def get_plaid_configuration():
	if dontmanage.db.get_single_value("Plaid Settings", "enabled"):
		...
		return {
			...
			"client_name": dontmanage.local.site,
		}
...
def add_institution(token, response):
	...
	if not dontmanage.db.exists("Bank", response["institution"]["name"]):
		try:
			...
			dontmanage.log_error("Plaid Link Error")
	else:
		bank = dontmanage.get_doc("Bank", response["institution"]["name"])
...
def add_bank_accounts(response, bank, company):
	...
	if not parent_gl_account:
		dontmanage.throw(
			...
			).format(dontmanage.bold(_("Bank")), company)
		)
	...
	for account in response["accounts"]:
		...
		acc_subtype = dontmanage.db.get_value("Bank Account Subtype", account["subtype"])
		if not acc_subtype:
			add_account_subtype(account["subtype"])

		bank_account_name = "{} - {}".format(account["name"], bank["bank_name"])
		existing_bank_account = dontmanage.db.exists("Bank Account", bank_account_name)
		...
		if not existing_bank_account:
			...
			try:
				existing_account = dontmanage.get_doc("Bank Account", existing_bank_account)
...
def add_account_type(account_type):
	try:
		dontmanage.get_doc({"doctype": "Bank Account Type", "account_type": account_type}).insert()
	except Exception:
...
def add_account_subtype(account_subtype):
	try:
		dontmanage.get_doc({"doctype": "Bank Account Subtype", "account_subtype": account_subtype}).insert()
	except Exception:
		dontmanage.throw(dontmanage.get_traceback())
...
def sync_transactions(bank, bank_account):
	...
	last_transaction_date = dontmanage.db.get_value("Bank Account", bank_account, "last_integration_date")
	...
	try:
		...
		if result:
			last_transaction_date = dontmanage.db.get_value("Bank Transaction", result.pop(), "date")

			dontmanage.logger().info(
				f"Plaid added {len(result)} new Bank Transactions from '{bank_account}' between {start_date} and {end_date}"
			)

			dontmanage.db.set_value("Bank Account", bank_account, "last_integration_date", last_transaction_date)
	except Exception:
		dontmanage.log_error(dontmanage.get_traceback(), _("Plaid transactions sync error"))
...
def get_transactions(bank, bank_account=None, start_date=None, end_date=None):
	...
	if bank_account:
		related_bank = dontmanage.db.get_values(
			"Bank Account", bank_account, ["bank", "integration_id"], as_dict=True
		)
		access_token = dontmanage.db.get_value("Bank", related_bank[0].bank, "plaid_access_token")
		account_id = related_bank[0].integration_id
	else:
		access_token = dontmanage.db.get_value("Bank", bank, "plaid_access_token")
		account_id = None
...
def new_bank_transaction(transaction):
	...
	bank_account = dontmanage.db.get_value("Bank Account", dict(integration_id=transaction["account_id"]))
	...
	if not dontmanage.db.exists(
		...
		try:
			...
			dontmanage.throw(_("Bank transaction creation error"))

	return result


def automatic_synchronization():
	settings = dontmanage.get_doc("Plaid Settings", "Plaid Settings")
...
def enqueue_synchronization():
	plaid_accounts = dontmanage.get_all(
		"Bank Account", filters={"integration_id": ["!=", ""]}, fields=["name", "bank"]
	)

	for plaid_account in plaid_accounts:
		dontmanage.enqueue(
			"dontmanageerp.dontmanageerp_integrations.doctype.plaid_settings.plaid_settings.sync_transactions",
...
def get_company(bank_account_name):
	from dontmanage.defaults import get_user_default

	company_names = dontmanage.db.get_all("Company", pluck="name")
	if len(company_names) == 1:
		return company_names[0]
	if dontmanage.db.exists("Bank Account", bank_account_name):
		return dontmanage.db.get_value("Bank Account", bank_account_name, "company")
	company_default = get_user_default("Company")
	if company_default:
		return company_default
	dontmanage.throw(_("Could not detect the Company for updating Bank Accounts"))
...
def update_bank_account_ids(response):
	...
	bank = dontmanage.get_doc("Bank", institution_name).as_dict()
