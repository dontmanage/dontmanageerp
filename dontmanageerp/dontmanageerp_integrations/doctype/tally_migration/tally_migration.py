...
from dontmanage.utils.data import format_datetime
...
from dontmanageerp.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer import (
	unset_existing_data,
)
...
class TallyMigration(Document):
	...
	if TYPE_CHECKING:
		...
		dontmanageerp_company: DF.Data | None
	...
	def get_collection(self, data_file):
		...
		master_file = dontmanage.get_doc("File", {"file_url": data_file})
	...
	def set_account_defaults(self):
		self.default_cost_center, self.default_round_off_account = dontmanage.db.get_value(
			"Company", self.dontmanageerp_company, ["cost_center", "round_off_account"]
		)
		self.default_warehouse = dontmanage.db.get_value("Stock Settings", "Stock Settings", "default_warehouse")
	...
	def _process_master_data(self):
				...
				items.append(
					{
						...
						"item_defaults": [{"company": self.dontmanageerp_company}],
					}
				)
		...
		try:
			...
			self.dontmanageerp_company = company
	...
	def _import_master_data(self):
		def create_company_and_coa(coa_file_url):
			coa_file = dontmanage.get_doc("File", {"file_url": coa_file_url})
			dontmanage.local.flags.ignore_chart_of_accounts = True

			try:
				company = dontmanage.get_doc(
					{
						"doctype": "Company",
						"company_name": self.dontmanageerp_company,
						"default_currency": "INR",
						"enable_perpetual_inventory": 0,
					}
				).insert()
			except dontmanage.DuplicateEntryError:
				company = dontmanage.get_doc("Company", self.dontmanageerp_company)
				unset_existing_data(self.dontmanageerp_company)

			dontmanage.local.flags.ignore_chart_of_accounts = False
		...
		def create_parties_and_addresses(parties_file_url, addresses_file_url):
			parties_file = dontmanage.get_doc("File", {"file_url": parties_file_url})
			for party in json.loads(parties_file.get_content()):
				try:
					party_doc = dontmanage.get_doc(party)
					party_doc.insert()
				except Exception:
					self.log(party_doc)
			addresses_file = dontmanage.get_doc("File", {"file_url": addresses_file_url})
			for address in json.loads(addresses_file.get_content()):
				try:
					address_doc = dontmanage.get_doc(address)
					address_doc.insert(ignore_mandatory=True)
				except Exception:
					self.log(address_doc)

		def create_items_uoms(items_file_url, uoms_file_url):
			uoms_file = dontmanage.get_doc("File", {"file_url": uoms_file_url})
			for uom in json.loads(uoms_file.get_content()):
				if not dontmanage.db.exists(uom):
					try:
						uom_doc = dontmanage.get_doc(uom)
						uom_doc.insert()
					except Exception:
						self.log(uom_doc)

			items_file = dontmanage.get_doc("File", {"file_url": items_file_url})
			for item in json.loads(items_file.get_content()):
				try:
					item_doc = dontmanage.get_doc(item)
					item_doc.insert()
				except Exception:
					self.log(item_doc)
	...
	def _process_day_book_data(self):
		...
		def voucher_to_journal_entry(voucher):
			...
			for entry in ledger_entries:
				account = {
					"account": encode_company_abbr(entry.LEDGERNAME.string.strip(), self.dontmanageerp_company),
			...
			journal_entry = {
				...
				"company": self.dontmanageerp_company,
				"accounts": accounts,
			}
		...
		def voucher_to_invoice(voucher):
			if voucher.VOUCHERTYPENAME.string.strip() in ["Sales", "Credit Note"]:
				...
				account_name = encode_company_abbr(self.tally_debtors_account, self.dontmanageerp_company)
				price_list_field = "selling_price_list"
			elif voucher.VOUCHERTYPENAME.string.strip() in ["Purchase", "Debit Note"]:
				doctype = "Purchase Invoice"
				party_field = "supplier"
				account_field = "credit_to"
				account_name = encode_company_abbr(self.tally_creditors_account, self.dontmanageerp_company)
		...
		def get_voucher_taxes(voucher):
			...
			for entry in ledger_entries:
				if entry.ISPARTYLEDGER.string.strip() == "No":
					tax_account = encode_company_abbr(entry.LEDGERNAME.string.strip(), self.dontmanageerp_company)
					taxes.append(
						{
							"charge_type": "Actual",
		...
		def get_party(party):
			if dontmanage.db.exists({"doctype": "Supplier", "supplier_name": party}):
				return "Supplier", encode_company_abbr(self.tally_creditors_account, self.dontmanageerp_company)
			elif dontmanage.db.exists({"doctype": "Customer", "customer_name": party}):
				return "Customer", encode_company_abbr(self.tally_debtors_account, self.dontmanageerp_company)
	...
	def _import_day_book_data(self):
		...
		try:
			dontmanage.db.set_value(
				"Account",
				encode_company_abbr(self.tally_creditors_account, self.dontmanageerp_company),
				"account_type",
				"Payable",
			)
			dontmanage.db.set_value(
				"Account",
				encode_company_abbr(self.tally_debtors_account, self.dontmanageerp_company),
				"account_type",
				"Receivable",
			)
			dontmanage.db.set_value(
				"Company", self.dontmanageerp_company, "round_off_account", self.default_round_off_account
			)

			vouchers_file = dontmanage.get_doc("File", {"file_url": self.vouchers})
	...
	def _import_vouchers(self, start, total, is_last=False):
		...
		for index, voucher in enumerate(chunk, start=start):
			try:
				voucher_doc = dontmanage.get_doc(voucher)
				voucher_doc.insert()
				voucher_doc.submit()
		...
		if is_last:
			...
			self.save()
			dontmanage.db.set_value("Price List", "Tally Price List", "enabled", 0)
		dontmanage.flags.in_migrate = False

	@dontmanage.whitelist()
	def process_master_data(self):
		self.set_status("Processing Master Data")
		dontmanage.enqueue_doc(self.doctype, self.name, "_process_master_data", queue="long", timeout=3600)

	@dontmanage.whitelist()
	def import_master_data(self):
		self.set_status("Importing Master Data")
		dontmanage.enqueue_doc(self.doctype, self.name, "_import_master_data", queue="long", timeout=3600)

	@dontmanage.whitelist()
	def process_day_book_data(self):
		self.set_status("Processing Day Book Data")
		dontmanage.enqueue_doc(self.doctype, self.name, "_process_day_book_data", queue="long", timeout=3600)

	@dontmanage.whitelist()
	def import_day_book_data(self):
		self.set_status("Importing Day Book Data")
		dontmanage.enqueue_doc(self.doctype, self.name, "_import_day_book_data", queue="long", timeout=3600)

	def log(self, data=None):
		if isinstance(data, dontmanage.model.document.Document):
			if sys.exc_info()[1].__class__ != dontmanage.DuplicateEntryError:
				failed_import_log = json.loads(self.failed_import_log)
				doc = data.as_dict()
			...
			return dontmanage.log_error(title="Tally Migration Error", message=message)
