	...
	$.each(temp, function (key, value) {
		if (value === "" || value === null) {
			...
			$.each(value, function (k, v) {
				dontmanageerp.tally_migration.cleanDoc(v);
			});
		}
	});
	return temp;
...
dontmanageerp.tally_migration.get_html_rows = (logs, field) => {
	...
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='dontmanageerp.tally_migration.create_new_doc(${JSON.stringify(
					doc
				)})'>
					${__("Create Document")}
				</button>`;
			...
			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='dontmanageerp.tally_migration.unresolve(${JSON.stringify(
					doc
				)})'>
