...
dontmanage.provide("dontmanageerp.tally_migration");

dontmanage.ui.form.on("Tally Migration", {
	onload: function (frm) {
		let reload_status = true;
		dontmanage.realtime.on("tally_migration_progress_update", function (data) {
			if (reload_status) {
				dontmanage.model.with_doc(frm.doc.doctype, frm.doc.name, () => {
					frm.refresh_header();
				});
				reload_status = false;
			}
	...
	refresh: function (frm) {
		...
		dontmanageerp.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		dontmanageerp.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	dontmanageerp_company: function (frm) {
		dontmanage.db.exists("Company", frm.doc.dontmanageerp_company).then(exists => {
			if (exists) {
				dontmanage.msgprint(
					__("Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts", [frm.doc.dontmanageerp_company]),
				);
			}
		});
	},
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = dontmanageerp.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = dontmanageerp.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = dontmanageerp.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = dontmanageerp.tally_migration.fixed_errors_log;
...
dontmanageerp.tally_migration.getError = (traceback) => {
...
dontmanageerp.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function(key, value){
		if (value === "" || value === null){
			...
			dontmanageerp.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k,v) { dontmanageerp.tally_migration.cleanDoc(v); });
		}
	});
	return temp;
}

dontmanageerp.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = dontmanageerp.tally_migration.failed_import_log;
	let fixed_log = dontmanageerp.tally_migration.fixed_errors_log;

	let modified_fixed_log = fixed_log.filter(row => {
		if (!dontmanage.utils.deep_equal(dontmanageerp.tally_migration.cleanDoc(row.doc), document)) {
			return row
		}
	});
...
dontmanageerp.tally_migration.resolve = (document) => {
...
dontmanageerp.tally_migration.create_new_doc = (document) => {
	/* Mark as resolved and create new document */
	dontmanageerp.tally_migration.resolve(document);
	return dontmanage.call({
		type: "POST",
		method: 'dontmanageerp.dontmanageerp_integrations.doctype.tally_migration.tally_migration.new_doc',
		args: {
			document
		},
		freeze: true,
		callback: function(r) {
			if(!r.exc) {
				...
				dontmanage.get_doc(r.message.doctype, r.message.name).__run_link_triggers = true;
...
dontmanageerp.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let error_message = dontmanageerp.tally_migration.getError(traceback);
			index++;
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(dontmanageerp.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='dontmanageerp.tally_migration.create_new_doc(${JSON.stringify(doc)})'>
					${__("Create Document")}
				</button>`

			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='dontmanageerp.tally_migration.unresolve(${JSON.stringify(doc)})'>
