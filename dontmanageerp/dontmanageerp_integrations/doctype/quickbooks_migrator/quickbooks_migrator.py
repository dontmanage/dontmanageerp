...
def callback(*args, **kwargs):
	migrator = dontmanage.get_doc("QuickBooks Migrator")
	...
	dontmanage.respond_as_web_page("Quickbooks Authentication", html="<script>window.close()</script>")
...
class QuickBooksMigrator(Document):
	...
	def on_update(self):
		if self.company:
			# We need a Cost Center corresponding to the selected dontmanageerp Company
			self.default_cost_center = dontmanage.db.get_value("Company", self.company, "cost_center")
			company_warehouses = dontmanage.get_all("Warehouse", filters={"company": self.company, "is_group": 0})
	...
	@dontmanage.whitelist()
	def migrate(self):
		dontmanage.enqueue_doc("QuickBooks Migrator", "QuickBooks Migrator", "_migrate", queue="long")
	...
	def _migrate(self):
		try:
			...
			# QuickBooks data can have transactions that do not fall in existing fiscal years in DontManageErp
	...
	def _make_custom_quickbooks_id_field(self, doctype):
		if not dontmanage.get_meta(doctype).has_field("quickbooks_id"):
			dontmanage.get_doc(
				{
	...
	def _make_custom_company_field(self, doctype):
		if not dontmanage.get_meta(doctype).has_field("company"):
			dontmanage.get_doc(
				{
					"doctype": "Custom Field",
					"label": "Company",
	...
	def _create_fiscal_years(self):
		try:
			...
			oldest_fiscal_year = dontmanage.get_all(
			...
			while smallest_ledger_entry_date < oldest_fiscal_year.year_start_date:
				new_fiscal_year = dontmanage.get_doc({"doctype": "Fiscal Year"})
				...
				entries.append(
					{
						...
						"type": data[1]["value"],
						"id": data[1].get("id"),
						"credit": dontmanage.utils.flt(data[2]["value"]),
						"debit": dontmanage.utils.flt(data[3]["value"]),
					}
				)
			if row["type"] == "Section":
	...
	def _save_account(self, account):
		...
		# Map Quickbooks Account Types to DontManageErp root_accunts and and root_type
		try:
			if not dontmanage.db.exists(
	...
	def _save_customer(self, customer):
		try:
			if not dontmanage.db.exists(
				...
				try:
					receivable_account = dontmanage.get_all(
						"Account",
						filters={
				...
				erpcustomer = dontmanage.get_doc(
					{
						"doctype": "Customer",
		...
		try:
			if not dontmanage.db.exists(
				...
				if item["Type"] in ("Service", "Inventory"):
					...
					dontmanage.get_doc(item_dict).insert()
		except Exception as e:
			self._log_error(e, item)

	def _allow_fraction_in_unit(self):
		dontmanage.db.set_value("UOM", "Unit", "must_be_whole_number", 0)
	...
	def _save_vendor(self, vendor):
		try:
			if not dontmanage.db.exists(
				...
				erpsupplier = dontmanage.get_doc(
					{
						"doctype": "Supplier",
	...
	def _save_sales_invoice(self, invoice, quickbooks_id, is_return=False, is_pos=False):
		try:
			if not dontmanage.db.exists(
				...
				invoice_doc = dontmanage.get_doc(invoice_dict)
				invoice_doc.insert()
				invoice_doc.submit()
		except Exception as e:
	...
	def _save_invoice_as_journal_entry(self, invoice, quickbooks_id):
		try:
			...
			for line in self.general_ledger["Invoice"][invoice["Id"]]["lines"]:
				...
				if dontmanage.db.get_value("Account", line["account"], "account_type") == "Receivable":
					account_line["party_type"] = "Customer"
					account_line["party"] = dontmanage.get_all(
						"Customer",
			...
			for line in payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Invoice":
					...
					if dontmanage.db.exists(
						...
						sales_invoice = dontmanage.get_all(
							"Sales Invoice",
							filters={
								"quickbooks_id": si_quickbooks_id,
								"company": self.company,
					...
					if dontmanage.db.exists(
						...
						journal_entry = dontmanage.get_doc(
							"Journal Entry",
							{
								"quickbooks_id": si_quickbooks_id,
								"company": self.company,
			...
			for line in bill_payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Bill":
					...
					if dontmanage.db.exists(
						...
						purchase_invoice = dontmanage.get_all(
							"Purchase Invoice",
							filters={
								"quickbooks_id": pi_quickbooks_id,
								"company": self.company,
	...
	def _create_address(self, entity, doctype, address, address_type):
		try:
			if not dontmanage.db.exists({"doctype": "Address", "quickbooks_id": address["Id"]}):
				dontmanage.get_doc(
					{
						"doctype": "Address",
	...
	def _get_account_name_by_id(self, quickbooks_id):
		return dontmanage.get_all("Account", filters={"quickbooks_id": quickbooks_id, "company": self.company})[
			0
		]["name"]

	def _publish(self, *args, **kwargs):
		dontmanage.publish_realtime("quickbooks_progress_update", *args, **kwargs, user=self.modified_by)
