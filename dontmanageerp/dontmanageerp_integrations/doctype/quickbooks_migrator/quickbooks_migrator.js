...
dontmanage.ui.form.on('QuickBooks Migrator', {
	...
	onload: function(frm) {
		...
		var domain = dontmanage.urllib.get_base_url()
		var redirect_url = `${domain}/api/method/dontmanageerp.dontmanageerp_integrations.doctype.quickbooks_migrator.quickbooks_migrator.callback`
		...
		frm.trigger("set_indicator")
 		dontmanage.realtime.on("quickbooks_progress_update", function (data) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.access_token) {
			...
			// All we need now is a Company from dontmanageerp
