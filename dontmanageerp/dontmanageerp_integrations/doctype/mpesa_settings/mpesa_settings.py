...
from dontmanage.utils import call_hook_method, fmt_money, get_request_site_address

from dontmanageerp.dontmanageerp_integrations.doctype.mpesa_settings.mpesa_connector import MpesaConnector
from dontmanageerp.dontmanageerp_integrations.doctype.mpesa_settings.mpesa_custom_fields import (
	create_custom_pos_fields,
)
from dontmanageerp.dontmanageerp_integrations.utils import create_mode_of_payment
...
class MpesaSettings(Document):
	...
	def request_for_payment(self, **kwargs):
		args = dontmanage._dict(kwargs)
		request_amounts = self.split_request_amount_according_to_transaction_limit(args)

		for i, amount in enumerate(request_amounts):
			args.request_amount = amount
			if dontmanage.flags.in_test:
				from dontmanageerp.dontmanageerp_integrations.doctype.mpesa_settings.test_mpesa_settings import (
					get_payment_request_response_payload,
				)

				response = dontmanage._dict(get_payment_request_response_payload(amount))
			else:
				response = dontmanage._dict(generate_stk_push(**args))
	...
	def split_request_amount_according_to_transaction_limit(self, args):
		...
		if request_amount > self.transaction_limit:
			...
			request_amounts = []
			requests_to_be_made = dontmanage.utils.ceil(
	...
	def get_account_balance_info(self):
		...
		if dontmanage.flags.in_test:
			...
			response = dontmanage._dict(get_test_account_balance_response())
		else:
			response = dontmanage._dict(get_account_balance(payload))
	...
	def handle_api_response(self, global_id, request_dict, response):
		...
		if not dontmanage.db.exists("Integration Request", req_name):
			create_request_log(request_dict, "Host", "Mpesa", req_name, error)

		if error:
			dontmanage.throw(_(getattr(response, "errorMessage")), title=_("Transaction Error"))
...
def generate_stk_push(**kwargs):
	...
	try:
		callback_url = (
			...
			+ "/api/method/dontmanageerp.dontmanageerp_integrations.doctype.mpesa_settings.mpesa_settings.verify_transaction"
		)

		mpesa_settings = dontmanage.get_doc("Mpesa Settings", args.payment_gateway[6:])
		...
		dontmanage.log_error("Mpesa Express Transaction Error")
		dontmanage.throw(
...
def verify_transaction(**kwargs):
	...
	transaction_response = dontmanage._dict(kwargs["Body"]["stkCallback"])

	checkout_id = getattr(transaction_response, "CheckoutRequestID", "")
	if not isinstance(checkout_id, str):
		dontmanage.throw(_("Invalid Checkout Request ID"))

	integration_request = dontmanage.get_doc("Integration Request", checkout_id)
	transaction_data = dontmanage._dict(loads(integration_request.data))
	...
	if transaction_response["ResultCode"] == 0:
		if integration_request.reference_doctype and integration_request.reference_docname:
			try:
				...
				dontmanage.db.set_value("POS Invoice", pr.reference_name, "mpesa_receipt_number", mpesa_receipts)
				integration_request.handle_success(transaction_response)
			except Exception:
				integration_request.handle_failure(transaction_response)
				dontmanage.log_error("Mpesa: Failed to verify transaction")
...
def get_completed_integration_requests_info(reference_doctype, reference_docname, checkout_id):
	output_of_other_completed_requests = dontmanage.get_all(
		"Integration Request",
		filters={
			"name": ["!=", checkout_id],
	...
	for out in output_of_other_completed_requests:
		out = dontmanage._dict(loads(out))
...
def get_account_balance(request_payload):
	...
	try:
		mpesa_settings = dontmanage.get_doc("Mpesa Settings", request_payload.get("reference_docname"))
		env = "production" if not mpesa_settings.sandbox else "sandbox"
		connector = MpesaConnector(
			env=env,
			app_key=mpesa_settings.consumer_key,
		...
		callback_url = (
			get_request_site_address(True)
			+ "/api/method/dontmanageerp.dontmanageerp_integrations.doctype.mpesa_settings.mpesa_settings.process_balance_info"
		)
		...
		dontmanage.log_error("Mpesa: Failed to get account balance")
		dontmanage.throw(_("Please check your configuration and try again"), title=_("Error"))
...
def process_balance_info(**kwargs):
	...
	account_balance_response = dontmanage._dict(kwargs["Result"])

	conversation_id = getattr(account_balance_response, "ConversationID", "")
	if not isinstance(conversation_id, str):
		dontmanage.throw(_("Invalid Conversation ID"))

	request = dontmanage.get_doc("Integration Request", conversation_id)

	if request.status == "Completed":
		return

	transaction_data = dontmanage._dict(loads(request.data))
	...
	if account_balance_response["ResultCode"] == 0:
		try:
			...
			ref_doc = dontmanage.get_doc(transaction_data.reference_doctype, transaction_data.reference_docname)
