...
from dontmanageerp.dontmanageerp_integrations.doctype.mpesa_settings.mpesa_settings import (
...
class TestMpesaSettings(unittest.TestCase):
	...
	def tearDown(self):
		dontmanage.db.sql("delete from `tabMpesa Settings`")
		dontmanage.db.sql("delete from `tabIntegration Request` where integration_request_service = 'Mpesa'")

	def test_creation_of_payment_gateway(self):
		mode_of_payment = create_mode_of_payment("Mpesa-_Test", payment_type="Phone")
		self.assertTrue(dontmanage.db.exists("Payment Gateway Account", {"payment_gateway": "Mpesa-_Test"}))
	...
	def test_processing_of_account_balance(self):
		...
		integration_request = dontmanage.get_doc("Integration Request", "AG_20200927_00007cdb1f9fb6494315")
	...
	def test_processing_of_callback_payload(self):
		mpesa_account = dontmanage.db.get_value(
			"Payment Gateway Account", {"payment_gateway": "Mpesa-Payment"}, "payment_account"
		)
		dontmanage.db.set_value("Account", mpesa_account, "account_currency", "KES")
		dontmanage.db.set_value("Customer", "_Test Customer", "default_currency", "KES")
		...
		integration_req_ids = dontmanage.get_all(
			"Integration Request",
			filters={
		...
		integration_request = dontmanage.get_doc("Integration Request", integration_req_ids[0])
		...
		dontmanage.db.set_value("Customer", "_Test Customer", "default_currency", "")
		integration_request.delete()
		pr.reload()
		pr.cancel()
		pr.delete()
	...
	def test_processing_of_multiple_callback_payload(self):
		...
		dontmanage.db.set_value("Account", mpesa_account, "account_currency", "KES")
		dontmanage.db.set_value("Mpesa Settings", "Payment", "transaction_limit", "500")
		dontmanage.db.set_value("Customer", "_Test Customer", "default_currency", "KES")
		...
		mpesa_receipt_numbers = [dontmanage.utils.random_string(5) for d in integration_req_ids]
		...
		for i in range(len(integration_req_ids)):
			...
			integration_request = dontmanage.get_doc("Integration Request", integration_req_ids[i])
			self.assertEqual(integration_request.status, "Completed")
	...
	def test_processing_of_only_one_succes_callback_payload(self):
		...
		pr = pos_invoice.create_payment_request()
		new_integration_req_ids = dontmanage.get_all(
			"Integration Request",
			filters={
				"reference_doctype": pr.doctype,
				"reference_docname": pr.name,
...
def create_mpesa_settings(payment_gateway_name="Express"):
	if dontmanage.db.exists("Mpesa Settings", payment_gateway_name):
		return dontmanage.get_doc("Mpesa Settings", payment_gateway_name)
...
def get_payment_request_response_payload(Amount=500):
	...
	CheckoutRequestID = dontmanage.utils.random_string(10)
