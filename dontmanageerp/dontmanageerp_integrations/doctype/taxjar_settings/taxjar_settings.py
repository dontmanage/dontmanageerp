...
from dontmanageerp.dontmanageerp_integrations.taxjar_integration import get_client
...
class TaxJarSettings(Document):
	def on_update(self):
		...
		fields_already_exist = dontmanage.db.exists(
			"Custom Field",
			{"dt": ("in", ["Item", "Sales Invoice Item"]), "fieldname": "product_tax_category"},
		)
		fields_hidden = dontmanage.get_value(
		...
		if TAXJAR_CREATE_TRANSACTIONS or TAXJAR_CALCULATE_TAX or TAXJAR_SANDBOX_MODE:
			if not fields_already_exist:
				...
				make_custom_fields()
				add_permissions()
				dontmanage.enqueue("dontmanageerp.regional.united_states.setup.add_product_tax_categories", now=False)
	...
	def update_nexus_list(self):
		...
		new_nexus_list = [dontmanage._dict(address) for address in nexus]
...
def create_tax_categories(data):
	for d in data:
		if not dontmanage.db.exists("Product Tax Category", {"product_tax_code": d.get("product_tax_code")}):
			tax_category = dontmanage.new_doc("Product Tax Category")
