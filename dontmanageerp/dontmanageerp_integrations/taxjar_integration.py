...
from dontmanageerp import get_default_company, get_region
...
def get_client():
	taxjar_settings = dontmanage.get_single("TaxJar Settings")
...
def create_transaction(doc, method):
	TAXJAR_CREATE_TRANSACTIONS = dontmanage.db.get_single_value(
	...
	TAX_ACCOUNT_HEAD = dontmanage.db.get_single_value("TaxJar Settings", "tax_account_head")
	...
	tax_dict["transaction_date"] = dontmanage.utils.today()
	...
	try:
		...
		dontmanage.throw(_(sanitize_error_response(err)))
...
def get_tax_data(doc):
	SHIP_ACCOUNT_HEAD = dontmanage.db.get_single_value("TaxJar Settings", "shipping_account_head")

	from_address = get_company_address_details(doc)
	from_shipping_state = from_address.get("state")
	from_country_code = dontmanage.db.get_value("Country", from_address.country, "code")
	...
	to_country_code = dontmanage.db.get_value("Country", to_address.country, "code")
...
def get_state_code(address, location):
	if address is not None:
		...
		if state_code not in SUPPORTED_STATE_CODES:
			dontmanage.throw(_("Please enter a valid State in the {0} Address").format(location))
	else:
		dontmanage.throw(_("Please enter a valid State in the {0} Address").format(location))
...
def set_sales_tax(doc, method):
	TAX_ACCOUNT_HEAD = dontmanage.db.get_single_value("TaxJar Settings", "tax_account_head")
	TAXJAR_CALCULATE_TAX = dontmanage.db.get_single_value("TaxJar Settings", "taxjar_calculate_tax")
...
def check_for_nexus(doc, tax_dict):
	TAX_ACCOUNT_HEAD = dontmanage.db.get_single_value("TaxJar Settings", "tax_account_head")
	if not dontmanage.db.get_value("TaxJar Nexus", {"region_code": tax_dict["to_state"]}):
...
def check_sales_tax_exemption(doc):
	...
	sales_tax_exempted = (
		...
		or dontmanage.db.has_column("Customer", "exempt_from_sales_tax")
		and dontmanage.db.get_value("Customer", doc.customer, "exempt_from_sales_tax")
	)
...
def get_company_address_details(doc):
	...
	if not company_address:
		dontmanage.throw(_("Please set a default company address"))

	company_address = dontmanage.get_doc("Address", company_address)
...
def get_shipping_address_details(doc):
	...
	if doc.shipping_address_name:
		shipping_address = dontmanage.get_doc("Address", doc.shipping_address_name)
	elif doc.customer_address:
		shipping_address = dontmanage.get_doc("Address", doc.customer_address)
	else:
...
def get_iso_3166_2_state_code(address):
	...
	country_code = dontmanage.db.get_value("Country", address.get("country"), "code")
	...
	if len(state) <= 3:
		...
		dontmanage.throw(_(error_message))
	else:
		try:
