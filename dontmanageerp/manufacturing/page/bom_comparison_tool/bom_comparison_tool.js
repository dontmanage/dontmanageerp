dontmanage.pages["bom-comparison-tool"].on_page_load = function (wrapper) {
	...
	new dontmanageerp.BOMComparisonTool(page);
};

dontmanageerp.BOMComparisonTool = class BOMComparisonTool {
	constructor(page) {
		this.page = page;
		this.make_form();
	}
	...
	fetch_and_render() {
		...
		dontmanage
			.call("dontmanageerp.manufacturing.doctype.bom.bom.get_bom_diff", {
				bom1: name1,
				bom2: name2,
			})
			.then((r) => {
				let diff = r.message;
				dontmanage.model.with_doctype("BOM", () => {
	...
	render(doctype, name1, name2, diff) {
		let change_html = (title, doctype, changed) => {
			let values_changed = this.get_changed_values(doctype, changed)
				.map((change) => {
					...
					return `
						<tr>
							<td>${dontmanage.meta.get_label(doctype, fieldname)}</td>
							<td>${value1}</td>
							<td>${value2}</td>
						</tr>
					`;
		...
		let table_changes = Object.keys(row_changes_by_fieldname)
			.map((fieldname) => {
				...
				let df = dontmanage.meta.get_docfield(doctype, fieldname);
				...
				let html = changes
					.map((change) => {
						...
						<tr>
							${th}
							<td>${dontmanage.meta.get_label(child_doctype, fieldname)}</td>
							<td>${value1}</td>
							<td>${value2}</td>
						</tr>
					`;
		...
		let get_added_removed_html = (title, grouped_items) => {
			return Object.keys(grouped_items)
				.map((fieldname) => {
					...
					let df = dontmanage.meta.get_docfield(doctype, fieldname);
					let fields = dontmanage.meta.get_docfields(df.options).filter((df) => df.in_list_view);
