...
dontmanage.ui.form.on('BOM Update Tool', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__("View BOM Update Log"), () => {
			dontmanage.set_route("List", "BOM Update Log");
		});
	},
	...
	replace: (frm) => {
		if (frm.doc.current_bom && frm.doc.new_bom) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.bom_update_tool.bom_update_tool.enqueue_replace_bom",
				freeze: true,
				args: {
					boms: {
	...
	update_latest_price_in_all_boms: (frm) => {
		dontmanage.call({
			method: "dontmanageerp.manufacturing.doctype.bom_update_tool.bom_update_tool.enqueue_update_cost",
			freeze: true,
			callback: result => {
				if (result && result.message && !result.exc) {
					frm.events.confirm_job_start(frm, result.message);
	...
	confirm_job_start: (frm, log_data) => {
		let log_link = dontmanage.utils.get_form_link("BOM Update Log", log_data.name, true);
		dontmanage.msgprint({
