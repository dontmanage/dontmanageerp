...
dontmanage.ui.form.on('Blanket Order', {
	onload: function(frm) {
		...
		if (frm.doc.customer && frm.doc.docstatus === 1 && frm.doc.to_date > dontmanage.datetime.get_today()) {
			frm.add_custom_button(__("Sales Order"), function() {
				dontmanage.model.open_mapped_doc({
					method: "dontmanageerp.manufacturing.doctype.blanket_order.blanket_order.make_order",
					frm: frm,
					args: {
						doctype: 'Sales Order'
					}
	...
	tc_name: function (frm) {
		dontmanageerp.utils.get_terms(frm.doc.tc_name, frm.doc, function (r) {
			if (!r.exc) {
