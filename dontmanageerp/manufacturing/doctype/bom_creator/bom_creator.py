...
from dontmanageerp.manufacturing.doctype.bom.bom import get_bom_item_rate
...
class BOMCreator(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.manufacturing.doctype.bom_creator_item.bom_creator_item import BOMCreatorItem
	...
	def validate_items(self):
		for row in self.items:
			if row.is_expandable and row.item_code == self.item_code:
				dontmanage.throw(_("Item {0} cannot be added as a sub-assembly of itself").format(row.item_code))
	...
	def set_status_completed(self):
		...
		if not dontmanage.get_cached_value("BOM", {"bom_creator": self.name, "item": self.item_code}, "name"):
			has_completed = False
	...
	def validate_fields(self):
		...
		for field, label in fields.items():
			if not self.get(field):
				dontmanage.throw(_("Please set {0} in BOM Creator {1}").format(label, self.name))
	...
	def create_boms(self):
		...
		production_item_wise_rm.setdefault(
			(self.item_code, self.name), dontmanage._dict({"items": [], "bom_no": "", "fg_item_data": self})
		)
			...
			if row.is_expandable:
				if (row.item_code, row.name) not in production_item_wise_rm:
					production_item_wise_rm.setdefault(
						...
						dontmanage._dict({"items": [], "bom_no": "", "fg_item_data": row}),
					)

			if not row.fg_reference_id and production_item_wise_rm.get((row.fg_item, row.fg_reference_id)):
				dontmanage.throw(_("Please set Parent Row No for item {0}").format(row.fg_item))
		...
		try:
			...
			dontmanage.msgprint(_("BOMs created successfully"))
		except Exception:
			...
			dontmanage.msgprint(_("BOMs creation failed"))
	...
	def get_default_bom(self, item_code) -> str:
		return dontmanage.get_cached_value("Item", item_code, "default_bom")


@dontmanage.whitelist()
def get_children(doctype=None, parent=None, **kwargs):
	if isinstance(kwargs, str):
		kwargs = dontmanage.parse_json(kwargs)
	...
	return dontmanage.get_all("BOM Creator Item", fields=fields, filters=query_filters, order_by="idx")
...
def add_item(**kwargs):
	...
	doc = dontmanage.get_doc("BOM Creator", kwargs.parent)
...
def add_sub_assembly(**kwargs):
	...
	doc = dontmanage.get_doc("BOM Creator", kwargs.parent)
	bom_item = dontmanage.parse_json(kwargs.bom_item)
...
def delete_node(**kwargs):
	...
	if kwargs.docname:
		dontmanage.delete_doc("BOM Creator Item", kwargs.docname)

	for item in items:
		dontmanage.delete_doc("BOM Creator Item", item.name)
		if item.expandable:
...
def edit_qty(doctype, docname, qty, parent):
	dontmanage.db.set_value(doctype, docname, "qty", qty)
	doc = dontmanage.get_doc("BOM Creator", parent)
	doc.set_rate_for_items()
	doc.save()
