...
dontmanage.ui.form.on("BOM Creator", {
	...
	setup_bom_creator(frm) {
		...
		if (!frm.is_new()) {
			if (!dontmanage.bom_configurator || dontmanage.bom_configurator.bom_configurator !== frm.doc.name) {
	...
	make_new_entry(frm) {
		let dialog = new dontmanage.ui.Dialog({
			...
			primary_action: (values) => {
				...
				dontmanage.db.insert(values).then((doc) => {
	...
	set_root_item(frm) {
		if (frm.is_new() && frm.doc.items?.length) {
			dontmanage.model.set_value(frm.doc.items[0].doctype, frm.doc.items[0].name, "is_root", 1);
		}
	},
