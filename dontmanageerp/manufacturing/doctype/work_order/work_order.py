...
from dontmanageerp.manufacturing.doctype.manufacturing_settings.manufacturing_settings import (
...
class OverProductionError(dontmanage.ValidationError):
	pass


class CapacityError(dontmanage.ValidationError):
	pass


class StockOverProductionError(dontmanage.ValidationError):
	pass


class OperationTooLongError(dontmanage.ValidationError):
	pass


class ItemHasVariantError(dontmanage.ValidationError):
	pass


class SerialNoQtyError(dontmanage.ValidationError):
	pass
...
class WorkOrder(Document):
	...
	def validate_sales_order(self):
		if self.sales_order:
			...
			if len(so):
				...
				dontmanage.throw(_("Sales Order {0} is not valid").format(self.sales_order))

	def check_sales_order_on_hold_or_close(self):
		status = dontmanage.db.get_value("Sales Order", self.sales_order, "status")
		if status in ("Closed", "On Hold"):
			dontmanage.throw(_("Sales Order {0} is {1}").format(self.sales_order, status))

	def set_default_warehouse(self):
		if not self.wip_warehouse and not self.skip_transfer:
			self.wip_warehouse = dontmanage.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")
		if not self.fg_warehouse:
			self.fg_warehouse = dontmanage.db.get_single_value("Manufacturing Settings", "default_fg_warehouse")
	...
	def validate_work_order_against_so(self):
		# already ordered qty
		ordered_qty_against_so = dontmanage.db.sql(
		...
		allowance_percentage = flt(
			dontmanage.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_sales_order")
		)
	...
	def update_work_order_qty(self):
		...
		for purpose, fieldname in (
			...
			from dontmanageerp.selling.doctype.sales_order.sales_order import update_produced_qty_in_so_item
	...
	def get_transferred_or_manufactured_qty(self, purpose):
		table = dontmanage.qb.DocType("Stock Entry")
		query = dontmanage.qb.from_(table).where(
	...
	def update_production_plan_status(self):
		production_plan = dontmanage.get_doc("Production Plan", self.production_plan)
		produced_qty = 0
		if self.production_plan_item:
			total_qty = dontmanage.get_all(
				"Work Order",
	...
	def on_submit(self):
		if not self.wip_warehouse and not self.skip_transfer:
			dontmanage.throw(_("Work-in-Progress Warehouse is required before Submit"))
		if not self.fg_warehouse:
			dontmanage.throw(_("For Warehouse is required before Submit"))

		if self.production_plan and dontmanage.db.exists(
	...
	def create_batch_for_finished_good(self):
		...
		batch_auto_creation = dontmanage.get_cached_value("Item", self.production_item, "create_new_batch")
		if not batch_auto_creation:
			dontmanage.msgprint(
				_("Batch not created for item {} since it does not have a batch series.").format(
					dontmanage.bold(self.production_item)
				),
				alert=True,
				indicator="orange",
			)
	...
	def delete_auto_created_batch_and_serial_no(self):
		for row in dontmanage.get_all("Serial No", filters={"work_order": self.name}):
			dontmanage.delete_doc("Serial No", row.name)

		for row in dontmanage.get_all("Batch", filters={"reference_name": self.name}):
	...
	def set_operation_start_end_time(self, idx, row):
		...
		if row.planned_start_time == row.planned_end_time:
			dontmanage.throw(_("Capacity Planning Error, planned start time can not be same as end time"))

	def validate_cancel(self):
		if self.status == "Stopped":
			dontmanage.throw(_("Stopped Work Order cannot be cancelled, Unstop it first to cancel"))

		# Check whether any stock entry exists against this Work Order
		stock_entry = dontmanage.db.sql(
		...
		if stock_entry:
			dontmanage.throw(
				_("Cannot cancel because submitted Stock Entry {0} exists").format(
					dontmanage.utils.get_link_to_form("Stock Entry", stock_entry[0][0])
				)
			)
	...
	def update_ordered_qty(self):
		if self.production_plan and self.production_plan_item and not self.production_plan_sub_assembly_item:
			...
			doc = dontmanage.get_doc("Production Plan", self.production_plan)
			doc.set_status()
	...
	def update_work_order_qty_in_so(self):
		...
		if self.product_bundle_item:
			total_bundle_qty = dontmanage.db.sql(
				""" select sum(qty) from
				`tabProduct Bundle Item` where parent = %s""",
				(dontmanage.db.escape(self.product_bundle_item)),
			)[0][0]
	...
	def update_work_order_qty_in_combined_so(self):
		...
		prod_plan = dontmanage.get_doc("Production Plan", self.production_plan)
		item_reference = dontmanage.get_value(
	...
	def update_completed_qty_in_material_request(self):
		if self.material_request and self.material_request_item:
			dontmanage.get_doc("Material Request", self.material_request).update_completed_qty(
	...
	def set_work_order_operations(self):
		...
		self.set("operations", [])
		if not self.bom_no or not dontmanage.get_cached_value("BOM", self.bom_no, "with_operations"):
			return

		operations = []

		if self.use_multi_level_bom:
			bom_tree = dontmanage.get_doc("BOM", self.bom_no).get_tree_representation()
		...
		bom_qty = dontmanage.get_cached_value("BOM", self.bom_no, "quantity")
	...
	def get_holidays(self, workstation):
		holiday_list = dontmanage.db.get_value("Workstation", workstation, "holiday_list")
	...
	def delete_job_card(self):
		for d in dontmanage.get_all("Job Card", ["name"], {"work_order": self.name}):
			dontmanage.delete_doc("Job Card", d.name)

	def validate_production_item(self):
		if dontmanage.get_cached_value("Item", self.production_item, "has_variants"):
			dontmanage.throw(_("Work Order cannot be raised against a Item Template"), ItemHasVariantError)
	...
	def validate_qty(self):
		if not self.qty > 0:
			dontmanage.throw(_("Quantity to Manufacture must be greater than 0."))

		if (
			self.stock_uom
			and dontmanage.get_cached_value("UOM", self.stock_uom, "must_be_whole_number")
			...
			dontmanage.throw(
				_(
					...
					dontmanage.bold(_("Must be Whole Number")),
					dontmanage.bold(self.stock_uom),
				),
			)

		if self.production_plan and self.production_plan_item and not self.production_plan_sub_assembly_item:
			qty_dict = dontmanage.db.get_value(
	...
	def validate_operation_time(self):
		for d in self.operations:
			if not d.time_in_mins > 0:
				dontmanage.throw(_("Operation Time must be greater than 0 for Operation {0}").format(d.operation))
	...
	def update_transferred_qty_for_required_items(self):
		ste = dontmanage.qb.DocType("Stock Entry")
		ste_child = dontmanage.qb.DocType("Stock Entry Detail")

		query = (
			dontmanage.qb.from_(ste)
		...
		transferred_items = dontmanage._dict({d.original_item or d.item_code: d.qty for d in data})
	...
	def update_returned_qty(self):
		...
		data = query.run(as_dict=1) or []
		returned_dict = dontmanage._dict({d.original_item or d.item_code: d.qty for d in data})
...
def get_bom_operations(doctype, txt, searchfield, start, page_len, filters):
	...
	return dontmanage.get_all("BOM Operation", filters=filters, fields=["operation"], as_list=1)
...
def get_item_details(item, project=None, skip_bom_info=False, throw=True):
	...
	res["bom_no"] = dontmanage.db.get_value("BOM", filters=filters)

	if not res["bom_no"]:
		variant_of = dontmanage.db.get_value("Item", item, "variant_of")

		if variant_of:
			res["bom_no"] = dontmanage.db.get_value("BOM", filters={"item": variant_of, "is_default": 1})
...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None, use_multi_level_bom=None):
	if not dontmanage.has_permission("Work Order", "write"):
		dontmanage.throw(_("Not permitted"), dontmanage.PermissionError)

	item_details = get_item_details(item, project)

	wo_doc = dontmanage.new_doc("Work Order")
...
def add_variant_item(variant_items, wo_doc, bom_no, table_name="items"):
	...
	for item in variant_items:
		...
		bom_doc = dontmanage.get_cached_doc("BOM", bom_no)
...
def check_if_scrap_warehouse_mandatory(bom_no):
	...
	if bom_no:
		bom = dontmanage.get_doc("BOM", bom_no)
...
def set_work_order_ops(name):
	po = dontmanage.get_doc("Work Order", name)
...
def make_stock_entry(work_order_id, purpose, qty=None, target_warehouse=None):
	work_order = dontmanage.get_doc("Work Order", work_order_id)
	...
	if work_order.bom_no:
		stock_entry.inspection_required = dontmanage.db.get_value("BOM", work_order.bom_no, "inspection_required")
...
def get_default_warehouse():
	doc = dontmanage.get_cached_doc("Manufacturing Settings")
...
def stop_unstop(work_order, status):
	...
	pro_order = dontmanage.get_doc("Work Order", work_order)

	if pro_order.status == "Closed":
		dontmanage.throw(_("Closed Work Order can not be stopped or Re-opened"))

	pro_order.update_status(status)
	pro_order.update_planned_qty()
	dontmanage.msgprint(_("Work Order has been {0}").format(status))
...
def split_qty_based_on_batch_size(wo_doc, row, qty):
	if not cint(dontmanage.db.get_value("Operation", row.operation, "create_job_card_based_on_batch_size")):
	...
	if flt(row.get("qty")) > flt(row.get("pending_qty")):
		dontmanage.throw(
			_("For operation {0}: Quantity ({1}) can not be greter than pending quantity({2})").format(
				dontmanage.bold(row.get("operation")),
				dontmanage.bold(row.get("qty")),
				dontmanage.bold(row.get("pending_qty")),
			)
		)


def create_job_card(work_order, row, enable_capacity_planning=False, auto_create=False):
	doc = dontmanage.new_doc("Job Card")
	doc.update(
		{
	...
	if auto_create:
		...
		doc.insert()
		dontmanage.msgprint(_("Job card {0} created").format(get_link_to_form("Job Card", doc.name)), alert=True)
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
	...
	max_finished_goods_qty = dontmanage.db.get_value("Work Order", source_name, "qty")
	...
	def update_item_quantity(source, target, source_parent):
		...
		if qty:
			...
			target.uom = dontmanage.get_value("Item", source.item_code, "stock_uom")
...
def make_stock_return_entry(work_order):
	from dontmanageerp.stock.doctype.stock_entry.stock_entry import get_available_materials

	non_consumed_items = get_available_materials(work_order)
	if not non_consumed_items:
		return

	wo_doc = dontmanage.get_cached_doc("Work Order", work_order)
