...
import dontmanage
from dontmanage.tests.utils import DontManageTestCase, change_settings, timeout
from dontmanage.utils import add_days, add_months, add_to_date, cint, flt, now, today

from dontmanageerp.manufacturing.doctype.job_card.job_card import JobCardCancelError
from dontmanageerp.manufacturing.doctype.job_card.job_card import make_stock_entry as make_stock_entry_from_jc
...
class TestWorkOrder(DontManageTestCase):
	def setUp(self):
	...
	def check_planned_qty(self):
		...
		planned1 = dontmanage.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
		...
		# from stores to wip
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 4))
		...
		# from wip to fg
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 4))
		s.insert()
		s.submit()

		self.assertEqual(dontmanage.db.get_value("Work Order", wo_order.name, "produced_qty"), 4)

		planned2 = dontmanage.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
	...
	def test_over_production(self):
		...
		s = dontmanage.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 7))
		s.insert()
	...
	def test_reserved_qty_for_partial_completion(self):
		...
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		s.submit()
	...
	def test_production_item(self):
		...
		dontmanage.db.set_value("Item", "_Test FG Item", "end_of_life", "2000-1-1")

		self.assertRaises(dontmanage.ValidationError, wo_order.save)

		dontmanage.db.set_value("Item", "_Test FG Item", "end_of_life", None)
		dontmanage.db.set_value("Item", "_Test FG Item", "disabled", 1)

		self.assertRaises(dontmanage.ValidationError, wo_order.save)

		dontmanage.db.set_value("Item", "_Test FG Item", "disabled", 0)
	...
	def test_reserved_qty_for_production_on_stock_entry(self):
		...
		s = dontmanage.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 2))
		...
		s = dontmanage.get_doc(make_stock_entry(self.wo_order.name, "Manufacture", 2))
	...
	def test_backflush_qty_for_overpduction_manufacture(self):
		...
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)

		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)
		...
		s1 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 50))
		s1.submit()
	...
	def test_reserved_qty_for_stopped_production(self):
		...
		s = dontmanage.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_scrap_material_qty(self):
		...
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		for d in s.get("items"):
			d.s_warehouse = "Stores - _TC"
		s.insert()
		s.submit()

		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		s.insert()
		s.submit()

		wo_order_details = dontmanage.db.get_value(
	...
	def test_allow_overproduction(self):
		...
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 3))
		s.insert()
		self.assertRaises(StockOverProductionError, s.submit)
	...
	def test_work_order_with_non_stock_item(self):
		...
		if not dontmanage.db.get_value("Item Price", {"item_code": "_Test FG Non Stock Item"}):
			dontmanage.get_doc(
				{
					"doctype": "Item Price",
		...
		if not dontmanage.db.get_value("BOM", {"item": fg_item, "docstatus": 1}):
			bom = make_bom(
				item=fg_item,
				rate=1000,
		...
		se = dontmanage.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se.insert()
		se.submit()

		ste = dontmanage.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		ste.insert()
	...
	def test_job_card(self):
		stock_entries = []
		bom = dontmanage.get_doc("BOM", {"docstatus": 1, "with_operations": 1, "company": "_Test Company"})
		...
		ste = dontmanage.get_doc(make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		ste.submit()
	...
	def test_work_order_material_transferred_qty_with_process_loss(self):
		...
		for row in job_cards:
			...
			doc = dontmanage.get_doc("Job Card", row.name)
			for row in doc.scheduled_time_logs:
				doc.append(
					"time_logs",
					{
	...
	def test_work_order_with_non_transfer_item(self):
		...
		if not dontmanage.db.get_value("BOM", {"item": fg_item}):
			make_bom(item=fg_item, raw_materials=["_Test FG Item", "_Test FG Item 1"])

		wo = make_wo_order_test_record(production_item=fg_item)
		ste = dontmanage.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		ste.insert()
		ste.submit()
		self.assertEqual(len(ste.items), 1)
		ste1 = dontmanage.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
	...
	def test_operation_time_with_batch_size(self):
		...
		bom_name = dontmanage.db.get_value("BOM", {"item": fg_item, "is_active": 1, "with_operations": 1}, "name")
	...
	def test_batch_size_for_fg_item(self):
		...
		batches = dontmanage.get_all("Batch", filters={"reference_name": work_order.name})
		self.assertEqual(len(batches), 3)
		batches = [batch.name for batch in batches]

		ste1 = dontmanage.get_doc(make_stock_entry(work_order.name, "Manufacture", 10))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
		...
		remaining_batches = []
		ste1 = dontmanage.get_doc(make_stock_entry(work_order.name, "Manufacture", 20))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
	...
	def test_partial_material_consumption(self):
		...
		ste1 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		ste1.submit()
		ste_cancel_list.append(ste1)

		ste3 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Consumption for Manufacture", 2))
	...
	def test_extra_material_transfer(self):
		...
		ste3 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		...
		ste2 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
	...
	def test_make_stock_entry_for_customer_provided_item(self):
		...
		if not dontmanage.db.exists("BOM", {"item": finished_item}):
		...
		ste = dontmanage.get_doc(make_stock_entry(wo.name, purpose="Material Transfer for Manufacture"))
		ste.insert()
	...
	def test_valuation_rate_missing_on_make_stock_entry(self):
		...
		if not dontmanage.db.get_value("BOM", {"item": item_name}):
		...
		stock_entry = dontmanage.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture"))
		self.assertRaises(dontmanage.ValidationError, stock_entry.save)

	def test_wo_completion_with_pl_bom(self):
		from dontmanageerp.manufacturing.doctype.bom.test_bom import (
		...
		if not dontmanage.db.exists("BOM", bom_no):
		...
		se = dontmanage.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", qty))
		se.get("items")[0].s_warehouse = "Stores - _TC"
		se.insert()
		se.submit()

		se = dontmanage.get_doc(make_stock_entry(wo.name, "Manufacture", qty))
		se.insert()
		se.submit()
	...
	def test_close_work_order(self):
		...
		job_cards = dontmanage.db.get_value("Job Card", {"work_order": wo_order.name}, "name")

		if len(job_cards) == len(bom.operations):
			for jc in job_cards:
				job_card_doc = dontmanage.get_doc("Job Card", jc)
				job_card_doc.append(
					"time_logs",
	...
	def test_partial_manufacture_entries(self):
		...
		sm = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 100))
		...
		s = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 90))
		...
		s1 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		...
		s2 = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
	...
	def test_auto_batch_creation(self):
		...
		fg_item = dontmanage.generate_hash(length=20)
		child_item = dontmanage.generate_hash(length=20)

		bom_tree = {fg_item: {child_item: {}}}

		create_nested_bom(bom_tree, prefix="")

		item = dontmanage.get_doc("Item", fg_item)
		item.has_batch_no = 1
		...
		try:
			...
			for row in stock_entry.items:
				if row.item_code == fg_item:
					...
					sn_doc = dontmanage.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def test_manufacture_entry_mapped_idx_with_exploded_bom(self):
		...
		ste_manu = dontmanage.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
	...
	def test_work_order_multiple_material_transfer(self):
		...
		transfer_entry = dontmanage.get_doc(
		...
		final_transfer_entry = dontmanage.get_doc(  # transfer last RM with For Quantity = 0
	...
	def test_backflushed_batch_raw_materials_based_on_transferred_autosabb(self):
		...
		manufacture_ste_doc = dontmanage.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 4))
	...
	def test_non_consumed_material_return_against_work_order(self):
		...
		ste_doc = dontmanage.get_doc(make_stock_entry(wo_doc.name, "Material Transfer for Manufacture", 5))
		for row in ste_doc.items:
			row.qty += 2
			row.transfer_qty += 2
			test_stock_entry.make_stock_entry(
		...
		ste_doc = dontmanage.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 5))
		for row in ste_doc.items:
	...
	def test_job_card_extra_qty(self):
		...
		dontmanage.db.set_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order", 100)
	...
	def test_operating_cost_account(self):
		...
		if not dontmanage.db.exists("Account", operating_cost_account):
			dontmanage.get_doc(
				{
					"doctype": "Account",
		...
		dontmanage.db.set_value("Company", company, "default_operating_cost_account", operating_cost_account)
		...
		job_cards = dontmanage.get_all("Job Card", filters={"work_order": wo.name})
		for job_card in job_cards:
			job_card_doc = dontmanage.get_doc("Job Card", job_card.name)
		...
		se_doc = dontmanage.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		se_doc.save()
	...
	def test_op_cost_and_scrap_based_on_sub_assemblies(self):
		...
		dontmanage.db.set_single_value("Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies", 1)
		...
		dontmanage.db.set_single_value("Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies", 0)
	...
	def test_partial_material_consumption_with_batch(self):
		...
		stock_entry = dontmanage.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 10))
		stock_entry.submit()
		stock_entry.reload()
	...
	def test_disassemby_order(self):
		...
		stock_entry = dontmanage.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		for row in stock_entry.items:
		...
		stock_entry = dontmanage.get_doc(make_stock_entry(wo.name, "Disassemble", 1))
		stock_entry.save()
	...
	def test_components_qty_for_bom_based_manufacture_entry(self):
		dontmanage.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		dontmanage.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 1)
		...
		transfer_entry = dontmanage.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		transfer_entry.save()
		for row in transfer_entry.items:
			row.qty = 5

		self.assertRaises(dontmanage.ValidationError, transfer_entry.save)
		...
		manufacture_entry = dontmanage.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		...
		self.assertRaises(dontmanage.ValidationError, manufacture_entry.save)
		manufacture_entry.reload()
		manufacture_entry.submit()

		dontmanage.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 0)
...
def prepare_data_for_workstation_type_check():
	from dontmanageerp.manufacturing.doctype.operation.test_operation import make_operation
	from dontmanageerp.manufacturing.doctype.workstation.test_workstation import make_workstation
	from dontmanageerp.manufacturing.doctype.workstation_type.test_workstation_type import (
	...
	if not dontmanage.db.exists("BOM", {"item": fg_item.name}):
		bom_doc = make_bom(
			item=fg_item.name,
			source_warehouse="Stores - _TC",
		...
		for index, operation in enumerate(operations):
			if not dontmanage.db.exists("BOM Operation", {"parent": bom_doc.name, "operation": operation}):
				bom_doc.append(
					"operations",
					{
						"operation": operation,
...
def allow_overproduction(fieldname, percentage):
	doc = dontmanage.get_doc("Manufacturing Settings")
...
test_records = dontmanage.get_test_records("Work Order")
