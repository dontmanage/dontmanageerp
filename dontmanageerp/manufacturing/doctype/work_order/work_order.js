...
dontmanage.ui.form.on("Work Order", {
	setup: function(frm) {
		...
		frm.set_query("bom_no", function() {
			if (frm.doc.production_item) {
				return {
					query: "dontmanageerp.controllers.queries.bom",
					filters: {item: cstr(frm.doc.production_item)}
				};
			} else {
				dontmanage.msgprint(__("Please enter Production Item first"));
			}
		});
		...
		frm.set_query("operation", "required_items", function() {
			return {
				query: "dontmanageerp.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	onload: function(frm) {
		...
		if(frm.doc.__islocal) {
			...
			dontmanageerp.work_order.set_default_warehouse(frm);
		}
	},
	...
	refresh: function(frm) {
		...
		dontmanageerp.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	create_stock_return_entry: function(frm) {
		dontmanage.call({
			method: "dontmanageerp.manufacturing.doctype.work_order.work_order.make_stock_return_entry",
			args: {
				"work_order": frm.doc.name,
			},
			callback: function(r) {
				if(!r.exc) {
					let doc = dontmanage.model.sync(r.message);
					dontmanage.set_route("Form", doc[0].doctype, doc[0].name);
				}
			}
		});
	},
	...
	make_job_card: function(frm) {
		...
		const dialog = dontmanage.prompt({fieldname: 'operations', fieldtype: 'Table', label: __('Operations'),
			fields: [
				{
					fieldtype: 'Link',
					fieldname: 'operation',
			...
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.work_order.work_order.make_job_card",
				freeze: true,
				args: {
					work_order: frm.doc.name,
	...
	production_item: function(frm) {
		if (frm.doc.production_item) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function(r) {
					if(r.message) {
						...
						dontmanageerp.in_production_item_onchange = true;
						...
						dontmanageerp.in_production_item_onchange = false;
					}
				}
			});
		}
	...
	project: function(frm) {
		if(!dontmanageerp.in_production_item_onchange && !frm.doc.bom_no) {
	...
	set_sales_order: function(frm) {
		if(frm.doc.production_item) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.work_order.work_order.query_sales_order",
	...
	additional_operating_cost: function(frm) {
		dontmanageerp.work_order.calculate_cost(frm.doc);
		dontmanageerp.work_order.calculate_total_cost(frm);
	},
});

dontmanage.ui.form.on("Work Order Item", {
	source_warehouse: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if(!row.item_code) {
			dontmanage.throw(__("Please set the Item Code first"));
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (row.item_code) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company
				},
...
dontmanage.ui.form.on("Work Order Operation", {
...
dontmanageerp.work_order = {
	set_custom_buttons: function(frm) {
		var doc = frm.doc;

		if (doc.status !== "Closed") {
			frm.add_custom_button(__('Close'), function() {
				dontmanage.confirm(__("Once the Work Order is Closed. It can't be resumed."),
					() => {
						dontmanageerp.work_order.change_work_order_status(frm, "Closed");
					}
				);
			}, __("Status"));
		}
		...
		if (doc.docstatus === 1 && !in_list(["Closed", "Completed"], doc.status)) {
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					dontmanageerp.work_order.change_work_order_status(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					dontmanageerp.work_order.change_work_order_status(frm, "Resumed");
				}, __("Status"));
			}
			...
			if (show_start_btn) {
				...
				if (pending_to_transfer && frm.doc.status != 'Stopped') {
					...
					frm.add_custom_button(__('Create Pick List'), function() {
						dontmanageerp.work_order.create_pick_list(frm);
					});
					var start_btn = frm.add_custom_button(__('Start'), function() {
						dontmanageerp.work_order.make_se(frm, 'Material Transfer for Manufacture');
					});
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0 || frm.doc.skip_transfer) {
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(__('Material Consumption'), function() {
								...
								dontmanageerp.work_order.make_consumption_se(frm, backflush_raw_materials_based_on);
							});
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0) {
						if ((flt(doc.produced_qty) < flt(doc.material_transferred_for_manufacturing))) {
							...
							var finish_btn = frm.add_custom_button(__('Finish'), function() {
								dontmanageerp.work_order.make_se(frm, 'Manufacture');
							});
							...
							dontmanage.db.get_doc("Manufacturing Settings").then((doc) => {
	...
	calculate_cost: function(doc) {
		if (doc.operations){
			...
			for(var i=0;i<op.length;i++) {
				...
				dontmanage.model.set_value('Work Order Operation', op[i].name,
	...
	set_default_warehouse: function(frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				callback: function(r) {
					if (!r.exe) {
	...
	show_prompt_for_qty_input: function(frm, purpose) {
		...
		return new Promise((resolve, reject) => {
			dontmanage.prompt({
				...
				if (data.qty > max) {
					dontmanage.msgprint(__('Quantity must not be more than {0}', [max]));
					reject();
				}
	...
	make_se: function(frm, purpose) {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return dontmanage.xcall('dontmanageerp.manufacturing.doctype.work_order.work_order.make_stock_entry', {
					'work_order_id': frm.doc.name,
					'purpose': purpose,
					'qty': data.qty
				});
			}).then(stock_entry => {
				dontmanage.model.sync(stock_entry);
				dontmanage.set_route('Form', stock_entry.doctype, stock_entry.name);
			});
	...
	create_pick_list: function(frm, purpose='Material Transfer for Manufacture') {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return dontmanage.xcall('dontmanageerp.manufacturing.doctype.work_order.work_order.create_pick_list', {
					'source_name': frm.doc.name,
					'for_qty': data.qty
				});
			}).then(pick_list => {
				dontmanage.model.sync(pick_list);
				dontmanage.set_route('Form', pick_list.doctype, pick_list.name);
			});
	},
	...
	make_consumption_se: function(frm, backflush_raw_materials_based_on) {
		...
		dontmanage.call({
			method:"dontmanageerp.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				"work_order_id": frm.doc.name,
	...
	change_work_order_status: function(frm, status) {
		...
		dontmanage.call({
			method: `dontmanageerp.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
