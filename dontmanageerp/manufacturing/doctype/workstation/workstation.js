...
dontmanage.ui.form.on("Workstation", {
	...
	onload(frm) {
		if (frm.is_new()) {
			dontmanage.call({
				type: "GET",
				method: "dontmanageerp.manufacturing.doctype.workstation.workstation.get_default_holiday_list",
				callback: function (r) {
					if (!r.exe && r.message) {
...
class WorkstationDashboard {
	...
	prepapre_dashboard() {
		dontmanage.call({
			method: "dontmanageerp.manufacturing.doctype.workstation.workstation.get_job_cards",
			args: {
	...
	render_job_cards() {
		let template = dontmanage.render_template("workstation_job_card", {
			data: this.job_cards,
		});
	...
	toggle_job_card() {
		this.$wrapper.find(".collapse-indicator-job").on("click", (e) => {
			...
			if (
				...
				$(e.currentTarget).html(dontmanage.utils.icon("es-line-down", "sm", "mb-1"));
			else $(e.currentTarget).html(dontmanage.utils.icon("es-line-up", "sm", "mb-1"));
		});
	}
	...
	complete_job(job_card, qty_to_manufacture) {
		...
		dontmanage.prompt(
			...
			(data) => {
				if (data.qty <= 0) {
					dontmanage.throw(__("Quantity should be greater than 0"));
				}
	...
	get_current_time(data) {
		...
		data.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += this.get_seconds_diff(dontmanage.datetime.now_datetime(), d.from_time);
			}
		});
