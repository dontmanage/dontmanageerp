...
class WorkstationHolidayError(dontmanage.ValidationError):
	pass


class NotInWorkingHoursError(dontmanage.ValidationError):
	pass
...
class Workstation(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.manufacturing.doctype.workstation_working_hour.workstation_working_hour import (
	...
	def validate_working_hours(self, row):
		if not (row.start_time and row.end_time):
			dontmanage.throw(_("Row #{0}: Start Time and End Time are required").format(row.idx))

		if get_time(row.start_time) >= get_time(row.end_time):
			dontmanage.throw(_("Row #{0}: Start Time must be before End Time").format(row.idx))
	...
	def set_data_based_on_workstation_type(self):
		if self.workstation_type:
			...
			data = dontmanage.get_cached_value("Workstation Type", self.workstation_type, fields, as_dict=True)
	...
	def validate_overlap_for_operation_timings(self):
		...
		for d in self.get("working_hours"):
			existing = dontmanage.db.sql_list(
	...
	def validate_workstation_holiday(self, schedule_date, skip_holiday_list_check=False):
		if not skip_holiday_list_check and (
			...
			or cint(dontmanage.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"))
		):
...
def get_job_cards(workstation):
	if dontmanage.has_permission("Job Card", "read"):
		jc_data = dontmanage.get_all(
			"Job Card",
			fields=[
				"name",
				"production_item",
		...
		allow_excess_transfer = dontmanage.db.get_single_value(
...
def get_default_holiday_list():
	return dontmanage.get_cached_value(
		"Company", dontmanage.defaults.get_user_default("Company"), "default_holiday_list"
	)


def check_if_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	if from_datetime and to_datetime:
		if not dontmanage.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"):
			check_workstation_for_holiday(workstation, from_datetime, to_datetime)

		if not cint(dontmanage.db.get_value("Manufacturing Settings", None, "allow_overtime")):
...
def is_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	...
	workstation = dontmanage.get_doc("Workstation", workstation)
...
def get_workstations(**kwargs):
	...
	_workstation = dontmanage.qb.DocType("Workstation")

	query = (
		dontmanage.qb.from_(_workstation)
		.select(
			_workstation.name,
