...
from dontmanage.utils import add_to_date, flt, getdate, now_datetime, nowdate
...
from dontmanageerp.manufacturing.doctype.work_order.work_order import make_stock_entry as make_se_from_wo
...
class TestProductionPlan(DontManageTestCase):
	def setUp(self):
		for item in [
			...
			sr = dontmanage.db.get_value(
				"Stock Reconciliation Item", {"item_code": item, "docstatus": 1}, "parent"
			)
			if sr:
				sr_doc = dontmanage.get_doc("Stock Reconciliation", sr)
				sr_doc.cancel()
	...
	def test_production_plan_mr_creation(self):
		...
		material_requests = dontmanage.get_all(
		...
		for row in material_requests:
			mr_schedule_date = getdate(dontmanage.db.get_value("Material Request", row[0], "schedule_date"))
		...
		for name in material_requests:
			mr = dontmanage.get_doc("Material Request", name[0])
			if mr.docstatus != 0:
				mr.cancel()

		for name in work_orders:
			mr = dontmanage.delete_doc("Work Order", name[0])

		pln = dontmanage.get_doc("Production Plan", pln.name)
		pln.cancel()
	...
	def test_production_plan_start_date(self):
		...
		for wo in work_orders:
			dontmanage.delete_doc("Work Order", wo.name)
	...
	def test_production_plan_sales_orders(self):
		...
		pln = dontmanage.new_doc("Production Plan")
		...
		work_order = dontmanage.db.get_value(
			"Work Order",
			{"sales_order": sales_order, "production_plan": pln.name, "sales_order_item": sales_order_item},
			"name",
		)

		wo_doc = dontmanage.get_doc("Work Order", work_order)
		wo_doc.update({"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"})
		wo_doc.submit()

		so_wo_qty = dontmanage.db.get_value("Sales Order Item", sales_order_item, "work_order_qty")
	...
	def test_donot_allow_to_make_multiple_pp_against_same_so(self):
		...
		pln.get_so_items()
		self.assertRaises(dontmanage.ValidationError, pln.save)
	...
	def test_so_based_bill_of_material(self):
		...
		pln1 = dontmanage.new_doc("Production Plan")
		...
		pln2 = dontmanage.new_doc("Production Plan")
	...
	def test_production_plan_combine_items(self):
		...
		for plan_reference in pln.prod_plan_references:
			...
			so_wo_qty = dontmanage.db.get_value(
		...
		for so_item in so_items:
			so_wo_qty = dontmanage.db.get_value("Sales Order Item", so_item, "work_order_qty")
	...
	def test_production_plan_subassembly_default_supplier(self):
		...
		item_doc = dontmanage.get_doc("Item", "Test Motherboard")
	...
	def test_production_plan_for_subcontracting_po(self):
		from dontmanageerp.manufacturing.doctype.bom.test_bom import create_nested_bom
		from dontmanageerp.subcontracting.doctype.subcontracting_bom.test_subcontracting_bom import (
		...
		item_doc = dontmanage.get_doc("Item", fg_item)
		company = "_Test Company"
		...
		po = dontmanage.db.get_value("Purchase Order Item", {"production_plan": plan.name}, "parent")
		po_doc = dontmanage.get_doc("Purchase Order", po)
	...
	def test_production_plan_combine_subassembly(self):
		...
		dontmanage.db.set_value("BOM Item", parent_bom_2.items[0].name, "bom_no", subassembly_bom)
		...
		plan.append(
			...
			{  # Add Green-Car to Prod Plan
				...
				"item_code": "Green-Car",
				"bom_no": dontmanage.db.get_value("Item", "Green-Car", "default_bom"),
				"planned_qty": 1,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = dontmanage.db.get_value(
			"Material Request Item",
			{"production_plan": production_plan.name, "item_code": "CUST-0987"},
			"parent",
		)
		mr = dontmanage.get_doc("Material Request", material_request)
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		if not dontmanage.db.get_value("BOM", {"item": "Test BOM 3"}):
			make_bom(item="Test BOM 3", raw_materials=["Test RM BOM 1"], rm_qty=3)

		if not dontmanage.db.get_value("BOM", {"item": "Test BOM 2"}):
			make_bom(item="Test BOM 2", raw_materials=["Test BOM 3"], rm_qty=3)

		if not dontmanage.db.get_value("BOM", {"item": "Test BOM 1"}):
		...
		pln.append(
			...
			{
				"item_code": item_code,
				"bom_no": dontmanage.db.get_value("BOM", {"item": "Test BOM 1"}),
				"planned_qty": 3,
			},
		)
	...
	def test_get_sales_order_with_variant(self):
		...
		if not dontmanage.db.exists("Item", {"item_code": "PIV"}):
		...
		if not dontmanage.db.exists("BOM", {"item": "PIV"}):
			parent_bom = make_bom(item="PIV", raw_materials=[rm_item.item_code])
		else:
			parent_bom = dontmanage.get_doc("BOM", {"item": "PIV"})

		if not dontmanage.db.exists("Item", {"item_code": "PIV-RED"}):
			variant = create_variant("PIV", {"Colour": "Red"})
			variant.save()
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		else:
			variant = dontmanage.get_doc("Item", "PIV-RED")
		if not dontmanage.db.exists("BOM", {"item": "PIV-RED"}):
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		...
		so.cancel()
		dontmanage.delete_doc("Sales Order", so.name)
		variant_bom.cancel()
		dontmanage.delete_doc("BOM", variant_bom.name)
	...
	def test_multiple_work_order_for_production_plan_item(self):
		...
		def create_work_order(item, pln, qty):
			...
			for _key, item in items_data.items():
				...
				wo_doc = dontmanage.get_doc("Work Order", wo_name)
				wo_doc.update(
		...
		wo1 = dontmanage.get_doc("Work Order", wo_list[0])
		wo1.cancel()
		pln.reload()
		self.assertEqual(pln.po_items[0].ordered_qty, 2)

		# Cancel 2nd Work Order
		wo2 = dontmanage.get_doc("Work Order", wo_list[1])
		wo2.cancel()
		pln.reload()
	...
	def test_production_plan_pending_qty_with_sales_order(self):
		...
		se = dontmanage.get_doc(make_se_from_wo(wo.name, "Manufacture", 1))
		se.submit()
	...
	def test_qty_based_status(self):
		pp = dontmanage.new_doc("Production Plan")
		pp.po_items = [dontmanage._dict(planned_qty=5, produce_qty=4)]
		self.assertFalse(pp.all_items_completed())

		pp.po_items = [
			dontmanage._dict(planned_qty=5, produce_qty=10),
			dontmanage._dict(planned_qty=5, produce_qty=4),
		]
		self.assertFalse(pp.all_items_completed())

	def test_production_plan_planned_qty(self):
		# Case 1: When Planned Qty is non-integer and UOM is integer.
		from dontmanageerp.utilities.transaction_base import UOMMustBeIntegerError
	...
	def test_temporary_name_relinking(self):
		...
		for _ in range(10):
			po_item = pp.append(
				"po_items",
				{
					...
					"temporary_name": dontmanage.generate_hash(length=10),
				},
			)
		...
		pp.append("sub_assembly_items", {"production_plan_item": dontmanage.generate_hash(length=16)})
		pp._rename_temporary_references()
	...
	def test_produced_qty_for_multi_level_bom_item(self):
		...
		pln.make_work_order()
		work_orders = dontmanage.get_all("Work Order", filters={"production_plan": pln.name}, pluck="name")
		sa_wo = fg_wo = None
		...
		se = dontmanage.get_doc(make_se_from_wo(sa_wo, "Manufacture"))
		se.submit()

		se = dontmanage.get_doc(make_se_from_wo(fg_wo, "Manufacture"))
		se.submit()
	...
	def test_material_request_item_for_purchase_uom(self):
		...
		if not dontmanage.db.exists("UOM Conversion Detail", {"parent": bom_item, "uom": "Nos"}):
			doc = dontmanage.get_doc("Item", bom_item)
	...
	def test_resered_qty_for_production_plan_for_material_requests(self):
		...
		before_qty = flt(dontmanage.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))

		pln = create_production_plan(item_code="Test Production Item 1")

		bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
		after_qty = flt(dontmanage.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_resered_qty_for_production_plan_for_work_order(self):
		...
		work_orders = []
		for row in dontmanage.get_all("Work Order", filters={"production_plan": pln.name}, fields=["name"]):
			wo_doc = dontmanage.get_doc("Work Order", row.name)
		...
		if rm_work_order:
			s = dontmanage.get_doc(make_se_from_wo(rm_work_order.name, "Material Transfer for Manufacture", 1))
			s.submit()
			bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
			after_qty = flt(dontmanage.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_resered_qty_for_production_plan_for_material_requests_with_multi_UOM(self):
		...
		for row in pln.mr_items:
			...
			reserved_qty = flt(dontmanage.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
			self.assertEqual(reserved_qty - before_qty, 100.0)

		pln.submit_material_request = 1
		pln.make_work_order()

		for work_order in dontmanage.get_all(
			"Work Order",
			fields=["name"],
			filters={"production_plan": pln.name},
		):
			wo_doc = dontmanage.get_doc("Work Order", work_order.name)
			wo_doc.source_warehouse = "_Test Warehouse - _TC"
			wo_doc.wip_warehouse = "_Test Warehouse 1 - _TC"
			wo_doc.fg_warehouse = "_Test Warehouse - _TC"
			wo_doc.submit()

		reserved_qty_after_mr = flt(dontmanage.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_transfer_and_purchase_mrp_for_purchase_uom(self):
		...
		parent_bom = create_nested_bom(bom_tree, prefix="")
		if not dontmanage.db.exists("UOM Conversion Detail", {"parent": "Test RM Item INK", "uom": "Kg"}):
			doc = dontmanage.get_doc("Item", "Test RM Item INK")
			doc.purchase_uom = "Kg"
	...
	def test_reserve_sub_assembly_items(self):
		...
		for d in work_orders:
			wo_doc = dontmanage.get_doc("Work Order", d.name)
			...
			if d.production_item == "Frame Assembly":
				...
				se_doc = dontmanage.get_doc(make_se_from_wo(wo_doc.name, "Manufacture", 2))
				se_doc.submit()
	...
	if not args.get("sales_order"):
		pln.append(
			"po_items",
			{
				...
				"bom_no": dontmanage.db.get_value("Item", args.item_code, "default_bom"),
