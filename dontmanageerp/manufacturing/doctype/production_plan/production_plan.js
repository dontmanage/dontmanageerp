		...
		if (frm.doc.docstatus === 1) {
			...
			if (frm.doc.status !== "Completed") {
				frm.add_custom_button(__("Work Order Tree"), ()=> {
					dontmanage.set_route('Tree', 'Work Order', {production_plan: frm.doc.name});
				}, __('View'));
		...
		if (row.warehouse && row.item_code && frm.doc.company) {
			dontmanage.call({
				...
				callback: function(r) {
					...
					dontmanage.model.set_value(cdt, cdn, 'projected_qty', projected_qty);
					dontmanage.model.set_value(cdt, cdn, 'actual_qty', actual_qty);
				}
			})
		}
	}
