...
dontmanage.ui.form.on('Production Plan', {
	...
	setup_queries(frm) {
		frm.set_query("sales_order", "sales_orders", () => {
			return {
				query: "dontmanageerp.manufacturing.doctype.production_plan.production_plan.sales_order_query",
				filters: {
		...
		frm.set_query("bom_no", "po_items", (doc, cdt, cdn) => {
			...
			} else dontmanage.msgprint(__("Please enter Item first"));
		});
		...
		if (frm.doc.docstatus === 1) {
			...
			if (frm.doc.status !== "Completed") {
				frm.add_custom_button(__("Production Plan Summary"), ()=> {
					dontmanage.set_route('query-report', 'Production Plan Summary', {production_plan: frm.doc.name});
				}, __('View'));
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://dontmanageerp.com/docs/user/manual/en/stock/projected-quantity">
	...
	make_material_request(frm) {
		...
		dontmanage.confirm(__("Do you want to submit the material request"),
			function() {
	...
	get_items_for_mr(frm) {
		if (!frm.doc.for_warehouse) {
			...
			dontmanage.throw(__("Select the Warehouse"));
		}
	...
	get_items_for_material_requests(frm, warehouses) {
		dontmanage.call({
			method: "dontmanageerp.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
	...
	download_materials_required(frm) {
		...
		dontmanage.prompt(fields, (row) => {
			let get_template_url = 'dontmanageerp.manufacturing.doctype.production_plan.production_plan.download_raw_materials';
...
dontmanage.ui.form.on("Production Plan Item", {
	item_code(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.item_code) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
					item_code: row.item_code
				},
				callback: function(r) {
					for (let key in r.message) {
						dontmanage.model.set_value(cdt, cdn, key, r.message[key]);
					}
				}
			});
		}
...
dontmanage.ui.form.on("Material Request Plan Item", {
	warehouse(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.warehouse && row.item_code && frm.doc.company) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
	...
	material_request_type(frm, cdt, cdn) {
		...
		if (row.from_warehouse &&
			row.material_request_type !== "Material Transfer") {
				dontmanage.model.set_value(cdt, cdn, 'from_warehouse', '');
		}
	}
});

dontmanage.ui.form.on("Production Plan Sales Order", {
	sales_order(frm, cdt, cdn) {
		...
		if (row.sales_order) {
			frm.call({
				...
				callback(r) {
					dontmanage.call({
						method: "dontmanageerp.manufacturing.doctype.production_plan.production_plan.get_so_details",
						args: { sales_order },
						callback(r) {
							const {transaction_date, customer, grand_total} = r.message;
							dontmanage.model.set_value(cdt, cdn, 'sales_order_date', transaction_date);
							dontmanage.model.set_value(cdt, cdn, 'customer', customer);
							dontmanage.model.set_value(cdt, cdn, 'grand_total', grand_total);
						}
					});
				}
			});
...
dontmanage.ui.form.on("Production Plan Sub Assembly Item", {
	fg_warehouse(frm, cdt, cdn) {
		dontmanageerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "sub_assembly_items", "fg_warehouse");
	},
})

dontmanage.tour['Production Plan'] = [
	{
