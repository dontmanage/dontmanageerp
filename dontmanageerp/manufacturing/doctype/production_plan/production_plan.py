...
class ProductionPlan(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.manufacturing.doctype.production_plan_item.production_plan_item import (
			ProductionPlanItem,
		)
...
def get_sales_orders(self):
	...
	open_so_subquery2 = (
		...
		.where(
			...
			& (
				ExistsCriterion(
					dontmanage.qb.from_(bom).select(bom.name).where((bom.item == pi.item_code) & (bom.is_active == 1))
				)
			)
		)
	)
