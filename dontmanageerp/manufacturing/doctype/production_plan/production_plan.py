...
from dontmanageerp.manufacturing.doctype.bom.bom import get_children as get_bom_children
from dontmanageerp.manufacturing.doctype.bom.bom import validate_bom_no
from dontmanageerp.manufacturing.doctype.work_order.work_order import get_item_details
...
from dontmanageerp.stock.utils import get_or_make_bin
...
class ProductionPlan(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.manufacturing.doctype.material_request_plan_item.material_request_plan_item import (
			MaterialRequestPlanItem,
		)
		from dontmanageerp.manufacturing.doctype.production_plan_item.production_plan_item import ProductionPlanItem
		from dontmanageerp.manufacturing.doctype.production_plan_item_reference.production_plan_item_reference import (
			ProductionPlanItemReference,
		)
		from dontmanageerp.manufacturing.doctype.production_plan_material_request.production_plan_material_request import (
			ProductionPlanMaterialRequest,
		)
		from dontmanageerp.manufacturing.doctype.production_plan_material_request_warehouse.production_plan_material_request_warehouse import (
			ProductionPlanMaterialRequestWarehouse,
		)
		from dontmanageerp.manufacturing.doctype.production_plan_sales_order.production_plan_sales_order import (
			ProductionPlanSalesOrder,
		)
		from dontmanageerp.manufacturing.doctype.production_plan_sub_assembly_item.production_plan_sub_assembly_item import (
	...
	def validate_sales_orders(self, sales_order=None):
		...
		if not data and sales_orders:
			...
			dontmanage.throw(msg, title=title)
	...
	def validate_data(self):
		for d in self.get("po_items"):
			if not d.bom_no:
				dontmanage.throw(_("Please select BOM for Item in Row {0}").format(d.idx))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				dontmanage.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			dontmanage.msgprint(_("Sales orders are not available for production"))
	...
	def get_pending_material_requests(self):
		...
		mr = dontmanage.qb.DocType("Material Request")
		mr_item = dontmanage.qb.DocType("Material Request Item")

		pending_mr_query = (
			dontmanage.qb.from_(mr)
			.from_(mr_item)
	...
	def get_bom_item_condition(self):
		...
		has_bom = dontmanage.db.exists({"doctype": "BOM", "item": self.item_code, "docstatus": 1})

		if not has_bom:
			bom = dontmanage.qb.DocType("BOM")
			template_item = dontmanage.db.get_value("Item", self.item_code, ["variant_of"])
	...
	def get_so_items(self):
		...
		if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			dontmanage.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))

		so_list = self.get_so_mr_list("sales_order", "sales_orders")

		bom = dontmanage.qb.DocType("BOM")
		so_item = dontmanage.qb.DocType("Sales Order Item")

		items_subquery = dontmanage.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
		items_query = (
			dontmanage.qb.from_(so_item)
			.select(
				so_item.parent,
				so_item.item_code,
				so_item.warehouse,
		...
		if self.item_code and dontmanage.db.exists("Item", self.item_code):
		...
		pi = dontmanage.qb.DocType("Packed Item")
	...
	def get_mr_items(self):
		...
		if not self.get("material_requests") or not self.get_so_mr_list(
			...
			dontmanage.throw(_("Please fill the Material Requests table"), title=_("Material Requests Required"))
	...
	def get_so_wise_planned_qty(sales_orders):
		so_wise_planned_qty = dontmanage._dict()
	...
	def update_bin_qty(self):
		for d in self.mr_items:
			if d.warehouse:
				...
				bin = dontmanage.get_doc("Bin", bin_name, for_update=True)
	...
	def delete_draft_work_order(self):
		for d in dontmanage.get_all(
			...
			dontmanage.delete_doc("Work Order", d.name)
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			if not item_details["project"] and d.sales_order:
				item_details["project"] = dontmanage.get_cached_value("Sales Order", d.sales_order, "project")
	...
	def make_work_order(self):
		from dontmanageerp.manufacturing.doctype.work_order.work_order import get_default_warehouse
		...
		if not wo_list:
			dontmanage.msgprint(_("No Work Orders were created"))
	...
	def make_subcontracted_purchase_order(self, subcontracted_po, purchase_orders):
		...
		for supplier, po_list in subcontracted_po.items():
			po = dontmanage.new_doc("Purchase Order")
	...
	def show_list_created_message(self, doctype, doc_list=None):
		...
		dontmanage.flags.mute_messages = False
		if doc_list:
			doc_list = [get_link_to_form(doctype, p) for p in doc_list]
			msgprint(_("{0} created").format(comma_and(doc_list)))

	def create_work_order(self, item):
		from dontmanageerp.manufacturing.doctype.work_order.work_order import OverProductionError

		if flt(item.get("qty")) <= 0:
			return

		wo = dontmanage.new_doc("Work Order")
		wo.update(item)
	...
	def make_material_request(self):
		...
		for item in self.mr_items:
			item_doc = dontmanage.get_cached_doc("Item", item.item_code)
			...
			if key not in material_request_map:
				...
				material_request_map[key] = dontmanage.new_doc("Material Request")
			...
			material_request.append(
				"items",
				{
					...
					"project": dontmanage.db.get_value("Sales Order", item.sales_order, "project")
					if item.sales_order
					else None,
				},
			)
	...
	def get_sub_assembly_items(self, manufacturing_type=None):
		...
		for row in self.po_items:
			if self.skip_available_sub_assembly_item and not self.sub_assembly_warehouse:
				dontmanage.throw(_("Row #{0}: Please select the Sub Assembly Warehouse").format(row.idx))

			if not row.item_code:
				dontmanage.throw(_("Row #{0}: Please select Item Code in Assembly Items").format(row.idx))

			if not row.bom_no:
				dontmanage.throw(_("Row #{0}: Please select the BOM No in Assembly Items").format(row.idx))
		...
		if not sub_assembly_items_store and self.skip_available_sub_assembly_item:
			...
			dontmanage.msgprint(message, title=_("Note"))
	...
	def set_sub_assembly_items_based_on_level(self, row, bom_data, manufacturing_type=None):
		...
		is_group_warehouse = dontmanage.db.get_value("Warehouse", self.sub_assembly_warehouse, "is_group")
	...
	def set_default_supplier_for_subcontracting_order(self):
		...
		default_supplier = dontmanage._dict(
			dontmanage.get_all(
				"Item Default",
...
def get_exploded_items(item_details, company, bom_no, include_non_stock_items, planned_qty=1, doc=None):
	bei = dontmanage.qb.DocType("BOM Explosion Item")
	bom = dontmanage.qb.DocType("BOM")
	item = dontmanage.qb.DocType("Item")
	item_default = dontmanage.qb.DocType("Item Default")
	item_uom = dontmanage.qb.DocType("UOM Conversion Detail")

	data = (
		dontmanage.qb.from_(bei)
		.join(bom)
...
def get_subitems(
	...
	bom_item = dontmanage.qb.DocType("BOM Item")
	bom = dontmanage.qb.DocType("BOM")
	item = dontmanage.qb.DocType("Item")
	item_default = dontmanage.qb.DocType("Item Default")
	item_uom = dontmanage.qb.DocType("UOM Conversion Detail")
...
def get_material_request_items(
	...
	if row["purchase_uom"] != row["stock_uom"]:
		if not (row["conversion_factor"] or dontmanage.flags.show_qty_in_stock_uom):
			dontmanage.throw(
	...
	if dontmanage.db.get_value("UOM", row["purchase_uom"], "must_be_whole_number"):
		required_qty = ceil(required_qty)

	if include_safety_stock:
		required_qty += flt(row["safety_stock"])

	item_details = dontmanage.get_cached_value("Item", row.item_code, ["purchase_uom", "stock_uom"], as_dict=1)
...
def get_sales_orders(self):
	bom = dontmanage.qb.DocType("BOM")
	pi = dontmanage.qb.DocType("Packed Item")
	so = dontmanage.qb.DocType("Sales Order")
	so_item = dontmanage.qb.DocType("Sales Order Item")

	open_so_subquery1 = dontmanage.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
...
def get_bin_details(row, company, for_warehouse=None, all_warehouse=False):
	if isinstance(row, str):
		row = dontmanage._dict(json.loads(row))

	bin = dontmanage.qb.DocType("Bin")
	wh = dontmanage.qb.DocType("Warehouse")

	subquery = dontmanage.qb.from_(wh).select(wh.name).where(wh.company == company)
...
def get_warehouse_list(warehouses):
	...
	for row in warehouses:
		child_warehouses = dontmanage.db.get_descendants("Warehouse", row.get("warehouse"))
		if child_warehouses:
...
def get_items_for_material_requests(doc, warehouses=None, get_parent_warehouse_data=None):
	...
	if doc.get("sub_assembly_items"):
		for sa_row in doc.sub_assembly_items:
			sa_row = dontmanage._dict(sa_row)
	...
	so_item_details = dontmanage._dict()
	...
	for data in po_items:
		...
		if data.get("bom") or data.get("bom_no"):
			...
			if not planned_qty:
				dontmanage.throw(_("For row {0}: Enter Planned Qty").format(data.get("idx")))
			...
			item_master = dontmanage.get_doc("Item", data["item_code"]).as_dict()
			purchase_uom = item_master.purchase_uom or item_master.stock_uom
			conversion_factor = (
				get_uom_conversion_factor(item_master.name, purchase_uom) if item_master.purchase_uom else 1.0
			)

			item_details[item_master.name] = dontmanage._dict(
				{
		...
		for item_code, details in item_details.items():
			so_item_details.setdefault(sales_order, dontmanage._dict())
	...
	if not mr_items:
		to_enable = dontmanage.bold(_("Ignore Existing Projected Quantity"))
		warehouse = dontmanage.bold(doc.get("for_warehouse"))
		message = (
			_(
...
def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
	from dontmanageerp.stock.doctype.pick_list.pick_list import get_available_item_locations

	stock_uom, purchase_uom = dontmanage.db.get_value(
	...
	precision = dontmanage.get_precision("Material Request Plan Item", "quantity")
	if flt(required_qty, precision) > 0:
		required_qty = required_qty

		if dontmanage.db.get_value("UOM", purchase_uom, "must_be_whole_number"):
			required_qty = ceil(required_qty)
...
def get_sub_assembly_items(bom_no, bom_data, to_produce_qty, company, warehouse=None, indent=0):
	...
	for d in data:
		if d.expandable:
			parent_item_code = dontmanage.get_cached_value("BOM", bom_no, "item")
...
def get_reserved_qty_for_production_plan(item_code, warehouse):
	...
	table = dontmanage.qb.DocType("Production Plan")
	child = dontmanage.qb.DocType("Material Request Plan Item")
...
def get_non_completed_production_plans():
	table = dontmanage.qb.DocType("Production Plan")
	child = dontmanage.qb.DocType("Production Plan Item")
...
def get_raw_materials_of_sub_assembly_items(
	...
	bei = dontmanage.qb.DocType("BOM Item")
	bom = dontmanage.qb.DocType("BOM")
	item = dontmanage.qb.DocType("Item")
	item_default = dontmanage.qb.DocType("Item Default")
	item_uom = dontmanage.qb.DocType("UOM Conversion Detail")
...
def sales_order_query(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	dontmanage.has_permission("Production Plan", throw=True)

	if not filters:
		filters = {}

	so_table = dontmanage.qb.DocType("Sales Order")
	table = dontmanage.qb.DocType("Sales Order Item")

	query = (
		dontmanage.qb.from_(so_table)
		.join(table)
...
def get_reserved_qty_for_sub_assembly(item_code, warehouse):
	table = dontmanage.qb.DocType("Production Plan")
	child = dontmanage.qb.DocType("Production Plan Sub Assembly Item")
