...
class BOM(WebsiteGenerator):
	...
	def autoname(self):
		...
		if dontmanage.db.exists("BOM", name):
			conflicting_bom = dontmanage.get_doc("BOM", name)

			if conflicting_bom.item != self.item:
				msg = _("A BOM with name {0} already exists for item {1}.").format(
					dontmanage.bold(name), dontmanage.bold(conflicting_bom.item)
				)
	...
	def validate_bom_currency(self, item):
		if (
			item.get("bom_no")
			and dontmanage.db.get_value("BOM", item.get("bom_no"), "currency") != self.currency
		):
			dontmanage.throw(
...
def add_additional_cost(stock_entry, work_order):
	...
	expenses_included_in_valuation = dontmanage.get_cached_value(
