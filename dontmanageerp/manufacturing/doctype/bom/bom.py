...
from dontmanage.utils import cint, cstr, flt, today
...
from dontmanageerp.stock.doctype.item.item import get_item_details
from dontmanageerp.stock.get_item_details import get_conversion_factor, get_price_list_rate

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class BOMRecursionError(dontmanage.ValidationError):
	pass
...
class BOMTree:
	...
	def __create_tree(self):
		bom = dontmanage.get_cached_doc("BOM", self.name)
...
class BOM(WebsiteGenerator):
	...
	def autoname(self):
		...
		if dontmanage.db.exists("BOM", name):
			existing_boms = dontmanage.get_all(
	...
	def set_onload_for_muulti_level_bom(self):
		use_multi_level_bom = dontmanage.db.get_value(
			"Property Setter",
	...
	def validate(self):
		self.route = dontmanage.scrub(self.name).replace("_", "-")

		if not self.company:
			dontmanage.throw(_("Please select a Company first."), title=_("Mandatory"))
	...
	def set_default_uom(self):
		...
		item_wise_uom = dontmanage._dict(
			dontmanage.get_all(
				"Item",
	...
	def on_update(self):
		dontmanage.cache().hdel("bom_children", self.name)
	...
	def get_item_det(self, item_code):
		...
		if not item:
			dontmanage.throw(_("Item: {0} does not exist in the system").format(item_code))
	...
	def get_bom_material_detail(self, args=None):
		...
		if not args:
			args = dontmanage.form_dict.get("args")
	...
	def validate_bom_currency(self, item):
		if item.get("bom_no") and dontmanage.db.get_value("BOM", item.get("bom_no"), "currency") != self.currency:
			dontmanage.throw(
	...
	def get_rm_rate(self, arg):
		...
		if arg.get("scrap_items"):
			...
			if not dontmanage.db.get_value("Item", arg["item_code"], "is_customer_provided_item") and not arg.get(
				"sourced_by_supplier"
			):
	...
	def update_cost(self, update_parent=True, from_child_bom=False, update_hour_rate=True, save=True):
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = dontmanage.db.sql_list(
			...
			for bom in parent_boms:
				dontmanage.get_doc("BOM", bom).update_cost(from_child_bom=True)
		...
		if not from_child_bom:
			...
			dontmanage.msgprint(_(msg), alert=True)
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			from dontmanage.model.utils import set_default

			set_default(self, "item")
			item = dontmanage.get_doc("Item", self.item)
			if item.default_bom != self.name:
				dontmanage.db.set_value("Item", self.item, "default_bom", self.name)
		elif (
			not dontmanage.db.exists(dict(doctype="BOM", docstatus=1, item=self.item, is_default=1))
			and self.is_active
		):
			...
			item = dontmanage.get_doc("Item", self.item)
			if item.default_bom == self.name:
				dontmanage.db.set_value("Item", self.item, "default_bom", None)
	...
	def validate_main_item(self):
		...
		if not item:
			dontmanage.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = dontmanage.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name = ret[2]

		if not self.quantity:
			dontmanage.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == "Price List":
			price_list_currency = dontmanage.db.get_value("Price List", self.buying_price_list, "currency")
	...
	def validate_materials(self):
		...
		if not self.get("items"):
			dontmanage.throw(_("Raw Materials cannot be blank."))
		...
		for m in self.get("items"):
			...
			if flt(m.qty) <= 0:
				dontmanage.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
	...
	def check_recursion(self, bom_list=None):
		...
		child_items = dontmanage.get_all(
			"BOM Item",
	...
	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			children = dontmanage.cache().hget("bom_children", bom_no)
			if children is None:
				children = dontmanage.db.sql_list(
					"""SELECT `bom_no` FROM `tabBOM Item`
					WHERE `parent`=%s AND `bom_no`!='' AND `parenttype`='BOM'""",
					bom_no,
				)
				dontmanage.cache().hset("bom_children", bom_no, children)
			return children
	...
	def update_rate_and_time(self, row, update_hour_rate=False):
		if not row.hour_rate or update_hour_rate:
			hour_rate = flt(dontmanage.get_cached_value("Workstation", row.workstation, "hour_rate"))
	...
	def get_rm_rate_map(self) -> dict[str, float]:
		...
		for item in self.get("items"):
			if item.bom_no:
				...
				explosion_items = dontmanage.get_all(
					"BOM Explosion Item",
	...
	def company_currency(self):
		return dontmanageerp.get_company_currency(self.company)
	...
	def get_child_exploded_items(self, bom_no, stock_qty):
		...
		child_fb_items = dontmanage.db.sql(
			"""
			SELECT
				bom_item.item_code,
				bom_item.item_name,
	...
	def add_exploded_items(self, save=True):
		...
		if save:
			dontmanage.db.sql("""delete from `tabBOM Explosion Item` where parent=%s""", self.name)
	...
	def validate_bom_links(self):
		if not self.is_active:
			...
			if act_pbom and act_pbom[0][0]:
				dontmanage.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
	...
	def validate_operations(self):
		if self.with_operations and not self.get("operations") and self.docstatus == 1:
			dontmanage.throw(_("Operations cannot be left blank"))

		if self.with_operations:
			for d in self.operations:
				if not d.description:
					d.description = dontmanage.db.get_value("Operation", d.operation, "description")
	...
	def validate_scrap_items(self):
		must_be_whole_number = dontmanage.get_value("UOM", self.uom, "must_be_whole_number")

		if self.process_loss_percentage and self.process_loss_percentage > 100:
			dontmanage.throw(_("Process Loss Percentage cannot be greater than 100"))

		if self.process_loss_qty and must_be_whole_number and self.process_loss_qty % 1 != 0:
			msg = f"Item: {dontmanage.bold(self.item)} with Stock UOM: {dontmanage.bold(self.uom)} can't have fractional process loss qty as UOM {dontmanage.bold(self.uom)} is a whole Number."
			dontmanage.throw(msg, title=_("Invalid Process Loss Configuration"))
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == "Valuation Rate":
		...
		rate = (
			...
			or flt(dontmanage.db.get_value("Item", args["item_code"], "last_purchase_rate"))
		) * (args.get("conversion_factor") or 1)
	elif bom_doc.rm_cost_as_per == "Price List":
		if not bom_doc.buying_price_list:
			dontmanage.throw(_("Please select Price List"))
		bom_args = dontmanage._dict(
			{
				"doctype": "BOM",
...
def get_valuation_rate(data):
	...
	bin_table = dontmanage.qb.DocType("Bin")
	wh_table = dontmanage.qb.DocType("Warehouse")
	item_valuation = (
	...
	if not valuation_rate:
		valuation_rate = dontmanage.db.get_value("Item", item_code, "valuation_rate")
...
def get_bom_items_as_dict(
	...
	if cint(fetch_exploded):
		...
		items = dontmanage.db.sql(query, {"qty": qty, "bom": bom, "company": company}, as_dict=True)
	else:
		query = query.format(
			table="BOM Item",
			where_conditions="",
	...
	for item, item_details in item_dict.items():
		for d in [
			...
			company_in_record = dontmanage.db.get_value(d[0], item_details.get(d[1]), "company")
			if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
				item_dict[item][d[1]] = dontmanage.get_cached_value("Company", company, d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	if not bom.is_active:
		dontmanage.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not getattr(dontmanage.flags, "in_test", False):
			dontmanage.throw(_("BOM {0} must be submitted").format(bom_no))
	if item:
		...
		if (
			...
			or bom.item.lower() == cstr(dontmanage.db.get_value("Item", item, "variant_of")).lower()
		):
			rm_item_exists = True
		if not rm_item_exists:
			dontmanage.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))


@dontmanage.whitelist()
def get_children(parent=None, is_root=False, **filters):
	if not parent or parent == "BOM":
		dontmanage.msgprint(_("Please select a BOM"))
		return

	if parent:
		dontmanage.form_dict.parent = parent

	if dontmanage.form_dict.parent:
		bom_doc = dontmanage.get_cached_doc("BOM", dontmanage.form_dict.parent)
		dontmanage.has_permission("BOM", doc=bom_doc, throw=True)

		bom_items = dontmanage.get_all(
			"BOM Item",
			fields=["item_code", "bom_no as value", "stock_qty"],
			filters=[["parent", "=", dontmanage.form_dict.parent]],
			order_by="idx",
		)

		item_names = tuple(d.get("item_code") for d in bom_items)

		items = dontmanage.get_list(
			"Item",
		...
		for bom_item in bom_items:
			...
			bom_item.image = dontmanage.db.escape(bom_item.image)
...
def add_additional_cost(stock_entry, work_order):
	...
	company_account = dontmanage.db.get_value(
		"Company",
		work_order.company,
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account):
	bom = dontmanage.get_doc("BOM", work_order.bom_no)
	...
	non_stock_items = dontmanage.get_all(
		"Item",
		fields="name",
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None):
	from dontmanageerp.stock.doctype.stock_entry.stock_entry import get_operating_cost_per_unit
...
def get_bom_diff(bom1, bom2):
	from dontmanage.model import table_fields

	if bom1 == bom2:
		dontmanage.throw(
			_("BOM 1 {0} and BOM 2 {1} should not be same").format(dontmanage.bold(bom1), dontmanage.bold(bom2))
		)

	doc1 = dontmanage.get_doc("BOM", bom1)
	doc2 = dontmanage.get_doc("BOM", bom2)
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	meta = dontmanage.get_meta("Item", cached=True)
	...
	if filters and filters.get("item_code"):
		has_variants = dontmanage.get_cached_value("Item", filters.get("item_code"), "has_variants")
		if not has_variants:
...
def make_variant_bom(source_name, bom_no, item, variant_items, target_doc=None):
	from dontmanageerp.manufacturing.doctype.work_order.work_order import add_variant_item
