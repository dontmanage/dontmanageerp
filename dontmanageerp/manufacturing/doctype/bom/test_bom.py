...
from dontmanage.utils import cstr, flt

from dontmanageerp.controllers.tests.test_subcontracting_controller import (
	set_backflush_based_on,
)
from dontmanageerp.manufacturing.doctype.bom.bom import BOMRecursionError, item_query, make_variant_bom
from dontmanageerp.manufacturing.doctype.bom_update_log.test_bom_update_log import (
...
class TestBOM(DontManageTestCase):
	@timeout
	...
	def test_get_items_list(self):
		from dontmanageerp.manufacturing.doctype.bom.bom import get_bom_items
	...
	def test_default_bom(self):
		def _get_default_bom_in_item():
			return cstr(dontmanage.db.get_value("Item", "_Test FG Item 2", "default_bom"))

		bom = dontmanage.get_doc("BOM", {"item": "_Test FG Item 2", "is_default": 1})
	...
	def test_update_bom_cost_in_all_boms(self):
		...
		bom_rates = dontmanage.db.get_values(
			"BOM Item",
			{
	...
	def test_bom_cost(self):
		bom = dontmanage.copy_doc(test_records[2])
		bom.insert()
	...
	def test_bom_cost_multi_uom_multi_currency_based_on_price_list(self):
		dontmanage.db.set_value("Price List", "_Test Price List", "price_not_uom_dependent", 1)
	...
	def test_bom_cost_multi_uom_based_on_valuation_rate(self):
		...
		reset_item_valuation_rate(
			...
			warehouse_list=dontmanage.get_all("Warehouse", {"is_group": 0, "company": bom.company}, pluck="name"),
			qty=200,
			rate=200,
		)
	...
	def test_bom_cost_with_fg_based_operating_cost(self):
		bom = dontmanage.copy_doc(test_records[4])
		bom.insert()
	...
	def test_subcontractor_sourced_item(self):
		...
		if not dontmanage.db.exists("Item", "Test Extra Item 1"):
			make_item("Test Extra Item 1", {"is_stock_item": 1, "stock_uom": "Nos"})

		if not dontmanage.db.exists("Item", "Test Extra Item 2"):
			make_item("Test Extra Item 2", {"is_stock_item": 1, "stock_uom": "Nos"})

		if not dontmanage.db.exists("Item", "Test Extra Item 3"):
	...
	def test_bom_recursion_1st_level(self):
		...
		bom.item = item_code
		bom.append("items", dontmanage._dict(item_code=item_code))
		bom.save()
	...
	def test_bom_recursion_transitive(self):
		...
		bom1 = dontmanage.new_doc("BOM")
		bom1.item = item1
		bom1.append("items", dontmanage._dict(item_code=item2))
		bom1.save()

		bom2 = dontmanage.new_doc("BOM")
		bom2.item = item2
		bom2.append("items", dontmanage._dict(item_code=item1))
		bom2.save()
	...
	def test_bom_with_process_loss_item(self):
		...
		#  PL can't be > 100
		self.assertRaises(dontmanage.ValidationError, bom_doc.submit)
	...
	def test_exclude_exploded_items_from_bom(self):
		...
		new_bom = dontmanage.copy_doc(dontmanage.get_doc("BOM", bom_no))
	...
	def test_valid_transfer_defaults(self):
		bom_with_op = dontmanage.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "with_operations": 1, "is_active": 1}
		)
		bom = dontmanage.copy_doc(dontmanage.get_doc("BOM", bom_with_op), ignore_no_copy=False)
	...
	def test_bom_versioning(self):
		bom_tree = {dontmanage.generate_hash(length=10): {dontmanage.generate_hash(length=10): {}}}
		...
		amendment = dontmanage.copy_doc(bom)
		...
		version = dontmanage.copy_doc(amendment)
		version.docstatus = 0
	...
	def test_clear_inpection_quality(self):
		bom = dontmanage.copy_doc(test_records[2], ignore_no_copy=True)
		bom.docstatus = 0
		bom.is_default = 0
	...
	def test_bom_pricing_based_on_lpp(self):
		...
		parent = dontmanage.generate_hash(length=10)
		child = dontmanage.generate_hash(length=10)
	...
	def test_set_default_bom_for_item_having_single_bom(self):
		...
		bom.save()
		bom.submit()
		self.assertEqual(dontmanage.get_value("Item", fg_item.item_code, "default_bom"), bom.name)

		# Step 2: Uncheck is_active field
		bom.is_active = 0
		bom.save()
		bom.reload()
		self.assertIsNone(dontmanage.get_value("Item", fg_item.item_code, "default_bom"))
	...
	def test_bom_raw_materials_stock_uom(self):
		...
		dontmanage.db.set_value("Item", rm_item, "stock_uom", "Kg")
...
def create_nested_bom(tree, prefix="_Test bom "):
	...
	def create_items(bom_tree):
		for item_code, subtree in bom_tree.items():
			...
			if not dontmanage.db.exists("Item", bom_item_code):
				dontmanage.get_doc(
	...
	for item in order_of_creating_bom:
		...
		if child_items:
			...
			bom = dontmanage.get_doc(doctype="BOM", item=bom_item_code)
...
def reset_item_valuation_rate(item_code, warehouse_list=None, qty=None, rate=None):
	...
	if not warehouse_list:
		warehouse_list = dontmanage.db.sql_list(
			"""
...
def create_bom_with_process_loss_item(
	...
	bom_doc = dontmanage.new_doc("BOM")
...
def create_process_loss_bom_item(item_tuple):
	...
	if dontmanage.db.exists("Item", item_code) is None:
