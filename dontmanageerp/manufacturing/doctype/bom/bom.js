...
dontmanage.ui.form.on("BOM", {
	...
	validate: function(frm) {
		if (frm.doc.fg_based_operating_cost && frm.doc.with_operations) {
			dontmanage.throw({message: __("Please check either with operations or FG Based Operating Cost."), title: __("Mandatory")});
		}
	},
	...
	refresh(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus<2) {
			...
			frm.add_custom_button(__("Browse BOM"), function() {
				...
				dontmanage.set_route("Tree", "BOM");
			});
		}

		if (!frm.is_new() && !frm.doc.docstatus == 0) {
			frm.add_custom_button(__("New Version"), function() {
				let new_bom = dontmanage.model.copy_doc(frm.doc);
				dontmanage.set_route("Form", "BOM", new_bom.name);
			});
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.$wrapper.find(".variants-intro").on("click", () => {
				dontmanage.set_route("List", "Item", {"variant_of": frm.doc.item});
			});
		}
	},
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(frm, "Variant BOM", (frm, item, data, variant_items) => {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.bom.bom.make_variant_bom",
				args: {
	...
	setup_variant_prompt(frm, title, callback, skip_qty_field) {
			...
			fields.push({
				...
				fields: [
					...
					},
					{
						...
						get_query(data) {
							if (!data.item_code) {
								dontmanage.throw(__("Select template item"));
							}
		...
		let dialog = dontmanage.prompt(fields, data => {
			let item = data.item || frm.doc.item;
			let variant_items = data.items || [];

			variant_items.forEach(d => {
				if (!d.variant_item_code) {
					dontmanage.throw(__("Select variant item code for the template item {0}", [d.item_code]));
				}
			})
	...
	make_quality_inspection(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm
		})
	},
		...
		if (frm.doc.routing) {
			dontmanage.call({
				...
				callback(r) {
					if (!r.exc) {
						frm.refresh_fields();
						dontmanageerp.bom.calculate_op_cost(frm.doc);
						dontmanageerp.bom.calculate_total(frm.doc);
					}
				}
			});
		}
...
dontmanageerp.bom.BomController = class BomController extends dontmanageerp.TransactionController {
	conversion_rate(doc) {
...
extend_cscript(cur_frm.cscript, new dontmanageerp.bom.BomController({frm: cur_frm}));

cur_frm.cscript.hour_rate = function(doc) {
	dontmanageerp.bom.calculate_op_cost(doc);
	dontmanageerp.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return dontmanage.call({
			...
			callback: function(r) {
				...
				dontmanageerp.bom.calculate_rm_cost(doc);
				dontmanageerp.bom.calculate_scrap_materials_cost(doc);
				dontmanageerp.bom.calculate_total(doc);
			},
			freeze: true
		});
...
cur_frm.cscript.rate = function(doc, cdt, cdn) {
	...
	if (d.bom_no) {
		dontmanage.msgprint(__("You cannot change the rate if BOM is mentioned against any Item."));
...
dontmanageerp.bom.update_cost = function(doc) {
	dontmanageerp.bom.calculate_op_cost(doc);
	dontmanageerp.bom.calculate_rm_cost(doc);
	dontmanageerp.bom.calculate_scrap_materials_cost(doc);
	dontmanageerp.bom.calculate_total(doc);
...
dontmanageerp.bom.calculate_op_cost = function(doc) {
	...
	if(doc.with_operations) {
		doc.operations.forEach((item) => {
			...
			dontmanage.model.set_value('BOM Operation',item.name, {
...
// rm : raw material
dontmanageerp.bom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for(var i=0;i<rm.length;i++) {
		...
		dontmanage.model.set_value('BOM Item', rm[i].name, 'base_rate',
			flt(rm[i].rate) * flt(doc.conversion_rate));
		dontmanage.model.set_value('BOM Item', rm[i].name, 'amount', amount);
		dontmanage.model.set_value('BOM Item', rm[i].name, 'base_amount', base_amount);
		dontmanage.model.set_value('BOM Item', rm[i].name,
...
// sm : scrap material
dontmanageerp.bom.calculate_scrap_materials_cost = function(doc) {
	...
	for(var i=0;i<sm.length;i++) {
		...
		dontmanage.model.set_value('BOM Scrap Item',sm[i].name, 'base_rate', base_rate);
		dontmanage.model.set_value('BOM Scrap Item',sm[i].name, 'amount', amount);
		dontmanage.model.set_value('BOM Scrap Item',sm[i].name, 'base_amount', base_amount);
...
dontmanageerp.bom.calculate_total = function(doc) {
...
dontmanage.ui.form.on("BOM Operation", "operation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
dontmanage.ui.form.on("BOM Operation", "workstation", function(frm, cdt, cdn) {
	...
	dontmanage.call({
		...
		callback: function (data) {
			dontmanage.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			dontmanage.model.set_value(d.doctype, d.name, "hour_rate",
...
dontmanage.ui.form.on("BOM Item", {
...
dontmanage.ui.form.on("BOM Item", "qty", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

dontmanage.ui.form.on("BOM Item", "item_code", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	dontmanage.db.get_value('Item', {name: d.item_code}, 'allow_alternative_item', (r) => {
		d.allow_alternative_item = r.allow_alternative_item
	})
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

dontmanage.ui.form.on("BOM Item", "sourced_by_supplier", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
dontmanage.ui.form.on("BOM Item", "rate", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.sourced_by_supplier) {
		d.rate = 0;
		refresh_field("rate", d.name, d.parentfield);
...
dontmanage.ui.form.on("BOM Operation", "operations_remove", function(frm) {
	dontmanageerp.bom.calculate_op_cost(frm.doc);
	dontmanageerp.bom.calculate_total(frm.doc);
});

dontmanage.ui.form.on("BOM Item", "items_remove", function(frm) {
	dontmanageerp.bom.calculate_rm_cost(frm.doc);
	dontmanageerp.bom.calculate_total(frm.doc);
});
...
dontmanage.ui.form.on("BOM Scrap Item", {
