...
from dontmanage.utils.data import add_to_date, now, today

from dontmanageerp.manufacturing.doctype.job_card.job_card import (
...
from dontmanageerp.manufacturing.doctype.work_order.work_order import WorkOrder
...
class TestJobCard(DontManageTestCase):
	def setUp(self):
	...
	def test_job_card_operations(self):
		...
		if job_cards:
			...
			dontmanage.db.set_value("Job Card", job_card.name, "operation_row_number", job_card.operation_id)

			doc = dontmanage.get_doc("Job Card", job_card.name)
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = dontmanage.db.get_value(
			...
			completed_qty = dontmanage.db.get_value(
	...
	def test_job_card_overlap(self):
		...
		jc1 = dontmanage.get_last_doc("Job Card", {"work_order": self.work_order.name})
		jc2 = dontmanage.get_last_doc("Job Card", {"work_order": wo2.name})
	...
	def test_job_card_overlap_with_capacity(self):
		...
		workstation = make_workstation(workstation_name=random_string(5)).name
		dontmanage.db.set_value("Workstation", workstation, "production_capacity", 1)
		...
		dontmanage.db.set_value("Workstation", workstation, "production_capacity", 2)
		jc2.load_from_db()
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = dontmanage.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = dontmanage.get_doc("Job Card", job_card_name)
	...
	def test_job_card_excess_material_transfer(self):
		...
		job_card = dontmanage.get_last_doc("Job Card", {"work_order": self.work_order.name})
	...
	def test_job_card_excess_material_transfer_with_no_reference(self):
		...
		self.assertRaises(dontmanage.ValidationError, transfer_entry_1.insert)
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = dontmanage.db.get_value(
	...
	def test_corrective_costing(self):
		...
		corrective_action = dontmanage.get_doc(
			doctype="Operation", is_corrective_operation=1, name=dontmanage.generate_hash()
		).insert()
	...
	def test_job_card_statuses(self):
		...
		jc = dontmanage.new_doc("Job Card")
		jc.for_quantity = 2
	...
	def test_job_card_material_request_and_bom_details(self):
		from dontmanageerp.stock.doctype.material_request.material_request import make_stock_entry

		create_bom_with_multiple_operations()
		work_order = make_wo_with_transfer_against_jc()

		job_card_name = dontmanage.db.get_value("Job Card", {"work_order": work_order.name}, "name")
	...
	def test_job_card_proccess_qty_and_completed_qty(self):
		from dontmanageerp.manufacturing.doctype.routing.test_routing import (
			create_routing,
			setup_bom,
			setup_operations,
		)
		...
		first_job_card = dontmanage.get_all(
			"Job Card",
		...
		jc = dontmanage.get_doc("Job Card", first_job_card)
		...
		second_job_card = dontmanage.get_all(
			"Job Card",
		...
		jc2 = dontmanage.get_doc("Job Card", second_job_card)
		...
		self.assertRaises(dontmanage.ValidationError, jc2.save)
		...
		s = dontmanage.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 10))
		s.submit()
...
def create_bom_with_multiple_operations():
	...
	test_record = dontmanage.get_test_records("BOM")[2]
	bom_doc = dontmanage.get_doc(test_record)
