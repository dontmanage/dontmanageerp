...
from dontmanage.query_builder.functions import IfNull, Max, Min
...
from dontmanageerp.manufacturing.doctype.workstation_type.workstation_type import get_workstations
...
class OperationMismatchError(dontmanage.ValidationError):
	pass


class OperationSequenceError(dontmanage.ValidationError):
	pass


class JobCardCancelError(dontmanage.ValidationError):
	pass


class JobCardOverTransferError(dontmanage.ValidationError):
	pass
...
class JobCard(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.manufacturing.doctype.job_card_item.job_card_item import JobCardItem
		from dontmanageerp.manufacturing.doctype.job_card_operation.job_card_operation import JobCardOperation
		from dontmanageerp.manufacturing.doctype.job_card_scheduled_time.job_card_scheduled_time import (
			JobCardScheduledTime,
		)
		from dontmanageerp.manufacturing.doctype.job_card_scrap_item.job_card_scrap_item import (
			JobCardScrapItem,
		)
		from dontmanageerp.manufacturing.doctype.job_card_time_log.job_card_time_log import JobCardTimeLog
	...
	def onload(self):
		excess_transfer = dontmanage.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
		self.set_onload("job_card_excess_transfer", excess_transfer)
		self.set_onload("work_order_closed", self.is_work_order_closed())
		self.set_onload("has_stock_entry", self.has_stock_entry())

	def has_stock_entry(self):
		return dontmanage.db.exists("Stock Entry", {"job_card": self.name, "docstatus": ["!=", 2]})
	...
	def validate_job_card_qty(self):
		...
		wo_qty = flt(dontmanage.get_cached_value("Work Order", self.work_order, "qty"))

		completed_qty = flt(dontmanage.db.get_value("Work Order Operation", self.operation_id, "completed_qty"))
		...
		job_card_qty = dontmanage.get_all(
			"Job Card",
		...
		if job_card_qty and ((job_card_qty - completed_qty) > wo_qty):
			...
			dontmanage.throw(_(msg), title=_("Extra Job Card Quantity"))
		...
		if self.get("time_logs"):
			for d in self.get("time_logs"):
				if d.to_time and get_datetime(d.from_time) > get_datetime(d.to_time):
					dontmanage.throw(_("Row {0}: From time must be less than to time").format(d.idx))
	...
	def get_overlap_for(self, args, open_job_cards=None):
		...
		if self.workstation:
			production_capacity = (
				dontmanage.get_cached_value("Workstation", self.workstation, "production_capacity") or 1
			)
	...
	def get_time_logs(self, args, doctype, open_job_cards=None):
		jc = dontmanage.qb.DocType("Job Card")
		jctl = dontmanage.qb.DocType(doctype)
	...
	def get_open_job_cards(self, employee):
		jc = dontmanage.qb.DocType("Job Card")
		jctl = dontmanage.qb.DocType("Job Card Time Log")

		query = (
			dontmanage.qb.from_(jc)
			.left_join(jctl)
	...
	def schedule_time_logs(self, row):
		...
		while row.remaining_time_in_mins > 0:
			args = dontmanage._dict({"from_time": row.planned_start_time, "to_time": row.planned_end_time})
	...
	def check_workstation_time(self, row):
		workstation_doc = dontmanage.get_cached_doc("Workstation", self.workstation)
		if not workstation_doc.working_hours or cint(
			dontmanage.db.get_single_value("Manufacturing Settings", "allow_overtime")
		):
	...
	def get_required_items(self):
		...
		doc = dontmanage.get_doc("Work Order", self.get("work_order"))
			...
			if self.get("operation") == d.operation:
				self.append(
					"items",
					{
						...
						"uom": dontmanage.db.get_value("Item", d.item_code, "stock_uom"),
	...
	def validate_job_card(self):
		if self.work_order and dontmanage.get_cached_value("Work Order", self.work_order, "status") == "Stopped":
			dontmanage.throw(
	...
	def validate_produced_quantity(self, for_quantity, process_loss_qty, wo):
		...
		if wo.produced_qty > for_quantity + process_loss_qty:
			...
			).format(dontmanage.bold(_("Job Card")), dontmanage.bold(self.name), dontmanage.bold(self.production_item))

			second_part_msg = _(
				"Kindly cancel the Manufacturing Entries first against the work order {0}."
			).format(dontmanage.bold(get_link_to_form("Work Order", self.work_order)))
	...
	def update_work_order_data(self, for_quantity, process_loss_qty, time_in_mins, wo):
		workstation_hour_rate = dontmanage.get_value("Workstation", self.workstation, "hour_rate")
		jc = dontmanage.qb.DocType("Job Card")
		jctl = dontmanage.qb.DocType("Job Card Time Log")
	...
	def set_transferred_qty_in_job_card_item(self, ste_doc):
		def _get_job_card_items_transferred_qty(ste_doc):
			...
			if job_card_items:
				...
				sed = dontmanage.qb.DocType("Stock Entry Detail")

				query = (
					dontmanage.qb.from_(sed)
					.join(se)
				...
				job_card_items_transferred_qty = dontmanage._dict(query.run(as_list=True))
		...
		def _validate_over_transfer(row, transferred_qty):
			...
			required_qty = dontmanage.db.get_value("Job Card Item", row.job_card_item, "required_qty")
			...
			if is_excess:
				dontmanage.throw(
					_(
						...
						row.idx, dontmanage.bold(required_qty), dontmanage.bold(row.item_code), ste_doc.job_card
					),
		...
		allow_excess = dontmanage.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
		...
		for row in ste_doc.items:
			...
			dontmanage.db.set_value("Job Card Item", row.job_card_item, "transferred_qty", flt(transferred_qty))
	...
	def validate_operation_id(self):
		if (
			...
			and self.operation
			and self.work_order
			and dontmanage.get_cached_value("Work Order Operation", self.operation_row_number, "name")
			!= self.operation_id
		):
	...
	def validate_work_order(self):
		if self.is_work_order_closed():
			dontmanage.throw(_("You can't make any changes to Job Card since Work Order is closed."))

	def is_work_order_closed(self):
		if self.work_order:
			status = dontmanage.get_value("Work Order", self.work_order)
	...
	def update_status_in_workstation(self, status):
		...
		dontmanage.db.set_value("Workstation", self.workstation, "status", status)
...
def make_time_log(args):
	...
	doc = dontmanage.get_doc("Job Card", args.job_card_id)
...
def get_operations(doctype, txt, searchfield, start, page_len, filters):
	if not filters.get("work_order"):
		dontmanage.msgprint(_("Please select a Work Order first."))
		return []
...
def make_stock_entry(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		wo_allows_alternate_item = dontmanage.db.get_value(
			"Work Order", target.work_order, "allow_alternative_item"
		)
		for item in target.items:
			item.allow_alternative_item = int(
				wo_allows_alternate_item
				and dontmanage.get_cached_value("Item", item.item_code, "allow_alternative_item")
			)
