...
class JobCard(Document):
	...
	def onload(self):
		excess_transfer = dontmanage.db.get_single_value(
	...
	def validate_job_card_qty(self):
		...
		completed_qty = flt(
			dontmanage.db.get_value("Work Order Operation", self.operation_id, "completed_qty")
		)
	...
	def validate_job_card(self):
		if (
			self.work_order
			and dontmanage.get_cached_value("Work Order", self.work_order, "status") == "Stopped"
		):
			dontmanage.throw(
	...
	def validate_produced_quantity(self, for_quantity, process_loss_qty, wo):
		...
		if wo.produced_qty > for_quantity + process_loss_qty:
			first_part_msg = _(
				...
				dontmanage.bold(_("Job Card")), dontmanage.bold(self.name), dontmanage.bold(self.production_item)
			)
	...
	def set_transferred_qty(self, update_status=False):
		...
		qty = 0
		if self.work_order:
			doc = dontmanage.get_doc("Work Order", self.work_order)
