...
dontmanage.ui.form.on('Job Card', {
	setup: function(frm) {
		frm.set_query('operation', function() {
			return {
				query: 'dontmanageerp.manufacturing.doctype.job_card.job_card.get_operations',
				filters: {
	...
	refresh: function(frm) {
		dontmanage.flags.pause_job = 0;
		dontmanage.flags.resume_job = 0;
		...
		if (frm.doc.docstatus == 0 && !frm.is_new() &&
			...
			if (frm.doc.work_order) {
				dontmanage.db.get_value('Work Order', frm.doc.work_order, ['skip_transfer', 'status'], (result) => {
		...
		if (frm.doc.work_order) {
			dontmanage.db.get_value('Work Order', frm.doc.work_order,
	...
	make_corrective_job_card: function(frm, operation, for_operation) {
		dontmanage.call({
			method: 'dontmanageerp.manufacturing.doctype.job_card.job_card.make_corrective_job_card',
			args: {
	...
	operation: function(frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
					"work_order":frm.doc.work_order,
		...
		if (!frm.doc.started_time && !frm.doc.current_time) {
			frm.add_custom_button(__("Start Job"), () => {
				if ((frm.doc.employee && !frm.doc.employee.length) || !frm.doc.employee) {
					dontmanage.prompt({fieldtype: 'Table MultiSelect', label: __('Select Employees'),
			...
			frm.add_custom_button(__("Complete Job"), () => {
				...
				if (set_qty) {
					dontmanage.prompt({fieldtype: 'Float', label: __('Completed Quantity'),
	...
	start_job: function(frm, status, employee) {
		const args = {
			...
			start_time: dontmanage.datetime.now_datetime(),
			employees: employee,
			status: status
		};
	...
	complete_job: function(frm, status, completed_qty) {
		const args = {
			job_card_id: frm.doc.name,
			complete_time: dontmanage.datetime.now_datetime(),
			status: status,
	...
	make_time_log: function(frm, args) {
		...
		dontmanage.call({
			method: "dontmanageerp.manufacturing.doctype.job_card.job_card.make_time_log",
			args: {
				args: args
			},
			freeze: true,
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.started_time || frm.doc.current_time) {
			if (frm.doc.status == "On Hold") {
				...
				currentIncrement += moment(dontmanage.datetime.now_datetime()).diff(moment(frm.doc.started_time),"seconds");
				initialiseTimer();
			}
		}
	},
	...
	make_material_request: function(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry: function(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.manufacturing.doctype.job_card.job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true
		});
	},
...
dontmanage.ui.form.on('Job Card Time Log', {
