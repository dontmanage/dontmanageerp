...
from dontmanage.utils import cint, cstr, date_diff, today

from dontmanageerp.manufacturing.doctype.bom_update_log.bom_updation_utils import (
	get_leaf_boms,
...
class BOMMissingError(dontmanage.ValidationError):
	pass
...
class BOMUpdateLog(Document):
	...
	def validate_same_bom(self):
		if cstr(self.current_bom) == cstr(self.new_bom):
			dontmanage.throw(_("Current BOM and New BOM can not be same"))

	def validate_bom_items(self):
		current_bom_item = dontmanage.db.get_value("BOM", self.current_bom, "item")
		new_bom_item = dontmanage.db.get_value("BOM", self.new_bom, "item")

		if current_bom_item != new_bom_item:
			dontmanage.throw(_("The selected BOMs are not for the same item"))

	def validate_bom_cost_update_in_progress(self):
		"If another Cost Updation Log is still in progress, dont make new ones."

		wip_log = dontmanage.get_all(
			"BOM Update Log",
		...
		if wip_log and date_diff(today(), wip_log[0].modified) < 1:
			log_link = dontmanage.utils.get_link_to_form("BOM Update Log", wip_log[0].name)
			dontmanage.throw(
	...
	def on_submit(self):
		if self.update_type == "Replace BOM":
			...
			dontmanage.enqueue(
				method="dontmanageerp.manufacturing.doctype.bom_update_log.bom_update_log.run_replace_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
...
def queue_bom_cost_jobs(current_boms_list: list[str], update_doc: "BOMUpdateLog", current_level: int) -> None:
	...
	while current_boms_list:
		...
		dontmanage.enqueue(
			method="dontmanageerp.manufacturing.doctype.bom_update_log.bom_updation_utils.update_cost_in_level",
			doc=update_doc,
...
def resume_bom_cost_update_jobs():
	...
	in_progress_logs = dontmanage.db.get_all(
		"BOM Update Log",
	...
	for log in in_progress_logs:
		...
		bom_batches = dontmanage.db.get_all(
			"BOM Update Batch",
		...
		if status == "Completed":
			dontmanage.db.delete("BOM Update Batch", {"parent": log.name})

		if parent_boms:  # there is a next level to process
			process_boms_cost_level_wise(
				update_doc=dontmanage.get_doc("BOM Update Log", log.name), parent_boms=parent_boms
			)
