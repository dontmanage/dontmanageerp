...
if TYPE_CHECKING:
	from dontmanageerp.manufacturing.doctype.bom_update_log.bom_update_log import BOMUpdateLog
...
def replace_bom(boms: Dict, log_name: str) -> None:
	...
	dontmanage.cache().delete_key("bom_children")
	parent_boms = get_ancestor_boms(new_bom)

	for bom in parent_boms:
		bom_obj = dontmanage.get_doc("BOM", bom)
...
def update_cost_in_level(
	...
	try:
		status = dontmanage.db.get_value("BOM Update Log", doc.name, "status")
		if status == "Failed":
			return

		update_cost_in_boms(bom_list=bom_list)  # main updation logic

		bom_batch = dontmanage.qb.DocType("BOM Update Batch")
		(
			dontmanage.qb.update(bom_batch)
...
def get_ancestor_boms(new_bom: str, bom_list: Optional[List] = None) -> List:
	...
	for d in parents:
		if new_bom == d.parent:
			dontmanage.throw(_("BOM recursion: {0} cannot be child of {1}").format(new_bom, d.parent))
...
def get_bom_unit_cost(bom_name: str) -> float:
	...
	new_bom_unitcost = (
		dontmanage.qb.from_(bom).select(bom.total_cost / bom.quantity).where(bom.name == bom_name).run()
	)

	return dontmanage.utils.flt(new_bom_unitcost[0][0])
...
def update_cost_in_boms(bom_list: List[str]) -> None:
	...
	for index, bom in enumerate(bom_list):
		bom_doc = dontmanage.get_doc("BOM", bom, for_update=True)
		bom_doc.calculate_cost(save_updates=True, update_hour_rate=True)
		bom_doc.db_update()

		if (index % 50 == 0) and not dontmanage.flags.in_test:
			dontmanage.db.commit()  # nosemgrep
...
def set_values_in_log(log_name: str, values: Dict[str, Any], commit: bool = False) -> None:
	...
	bom_update_log = dontmanage.qb.DocType("BOM Update Log")
	query = dontmanage.qb.update(bom_update_log).where(bom_update_log.name == log_name)

	for key, value in values.items():
		query = query.set(key, value)
	query.run()

	if commit and not dontmanage.flags.in_test:
		dontmanage.db.commit()  # nosemgrep
