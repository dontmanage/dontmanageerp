...
dontmanage.ui.form.on("Plant Floor", {
	setup(frm) {
	...
	add_workstation(frm) {
		frm.add_custom_button(__("Create Workstation"), () => {
			var doc = dontmanage.model.get_new_doc("Workstation");
	...
	setup_queries(frm) {
		frm.set_query("warehouse", (doc) => {
			if (!doc.company) {
				dontmanage.throw(__("Please select Company first"));
			}
	...
	prepare_stock_dashboard(frm) {
		...
		dontmanage.visual_stock = new VisualStock({
			wrapper: wrapper,
			frm: frm,
		});
	},
...
class VisualStock {
	...
	prepare_filters() {
		...
		this.item_filter = dontmanage.ui.form.make_control({
			df: {
				fieldtype: "Link",
				fieldname: "item_code",
				placeholder: __("Item"),
		...
		this.item_group_filter = dontmanage.ui.form.make_control({
			df: {
				fieldtype: "Link",
	...
	async get_stock_summary(start, item_code, item_group) {
		let stock_summary = await dontmanage.call({
			method: "dontmanageerp.manufacturing.doctype.plant_floor.plant_floor.get_stock_summary",
			args: {
	...
	render_stock_summary() {
		let template = dontmanage.render_template("stock_summary_template", {
	...
	update_values() {
		if (!this.values.qty) {
			dontmanage.throw(__("Quantity is required"));
		}
