...
from dontmanageerp.manufacturing.doctype.job_card.job_card import OperationSequenceError
...
class TestRouting(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def tearDownClass(cls):
		dontmanage.db.sql("delete from tabBOM where item=%s", cls.item_code)
	...
	def test_sequence_id(self):
		...
		for data in dontmanage.get_all(
			...
			job_card_doc = dontmanage.get_doc("Job Card", data.name)
...
def create_routing(**args):
	...
	doc = dontmanage.new_doc("Routing")
	doc.update(args)
	...
	if not args.do_not_save:
		try:
			...
			doc = dontmanage.get_doc("Routing", args.routing_name)
...
def setup_bom(**args):
	...
	if not args.raw_materials:
		if not dontmanage.db.exists("Item", "Test Extra Item N-1"):
			make_item(
				"Test Extra Item N-1",
				{
					"is_stock_item": 1,
	...
	name = dontmanage.db.get_value("BOM", {"item": args.item_code}, "name")
	if not name:
		bom_doc = make_bom(
			item=args.item_code,
		...
		bom_doc = dontmanage.get_doc("BOM", name)
