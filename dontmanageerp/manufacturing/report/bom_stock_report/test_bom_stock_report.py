...
from dontmanageerp.manufacturing.report.bom_stock_report.bom_stock_report import (
...
class TestBomStockReport(DontManageTestCase):
	def setUp(self):
...
def get_expected_data(bom, warehouse, qty_to_produce, show_exploded_view=False):
	...
	for item in bom.get("exploded_items") if show_exploded_view else bom.get("items"):
		in_stock_qty = dontmanage.get_cached_value(
