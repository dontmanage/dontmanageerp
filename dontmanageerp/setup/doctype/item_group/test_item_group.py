...
test_records = dontmanage.get_test_records("Item Group")


class TestItem(unittest.TestCase):
	def test_basic_tree(self, records=None):
		min_lft = 1
		max_rgt = dontmanage.db.sql("select max(rgt) from `tabItem Group`")[0][0]

		if not records:
			records = test_records[2:]

		for item_group in records:
			lft, rgt, parent_item_group = dontmanage.db.get_value(
				"Item Group", item_group["item_group_name"], ["lft", "rgt", "parent_item_group"]
			)

			if parent_item_group:
				parent_lft, parent_rgt = dontmanage.db.get_value("Item Group", parent_item_group, ["lft", "rgt"])
			else:
				# root
	...
	def test_recursion(self):
		group_b = dontmanage.get_doc("Item Group", "_Test Item Group B")
	...
	def test_move_group_into_another(self):
		# before move
		old_lft, old_rgt = dontmanage.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])
		...
		# after move
		new_lft, new_rgt = dontmanage.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])
	...
	def print_tree(self):
		...
		print(json.dumps(dontmanage.db.sql("select name, lft, rgt from `tabItem Group` order by lft"), indent=1))

	def test_move_leaf_into_another_group(self):
		# before move
		old_lft, old_rgt = dontmanage.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])

		group_b_3 = dontmanage.get_doc("Item Group", "_Test Item Group B - 3")
	...
	def test_delete_leaf(self):
		# for checking later
		parent_item_group = dontmanage.db.get_value("Item Group", "_Test Item Group B - 3", "parent_item_group")
		dontmanage.db.get_value("Item Group", parent_item_group, "rgt")

		ancestors = get_ancestors_of("Item Group", "_Test Item Group B - 3")
		ancestors = dontmanage.db.sql(
		...
		dontmanage.delete_doc("Item Group", "_Test Item Group B - 3")
		...
		for item_group in ancestors:
			new_lft, new_rgt = dontmanage.db.get_value("Item Group", item_group.name, ["lft", "rgt"])
			self.assertEqual(new_rgt, item_group.rgt - 2)

		# insert it back
		dontmanage.copy_doc(test_records[6]).insert()

		self.test_basic_tree()

	def test_delete_group(self):
		# cannot delete group with child, but can delete leaf
		self.assertRaises(NestedSetChildExistsError, dontmanage.delete_doc, "Item Group", "_Test Item Group B")

	def test_merge_groups(self):
		dontmanage.rename_doc("Item Group", "_Test Item Group B", "_Test Item Group C", merge=True)
		records_to_test = test_records[2:]
		del records_to_test[1]
		self.test_basic_tree(records=records_to_test)

		# insert Group B back
		dontmanage.copy_doc(test_records[3]).insert()
		self.test_basic_tree()

		# move its children back
		for name in dontmanage.db.sql_list(
			"""select name from `tabItem Group`
			where parent_item_group='_Test Item Group C'"""
		):
			doc = dontmanage.get_doc("Item Group", name)
	...
	def test_merge_leaves(self):
		dontmanage.rename_doc("Item Group", "_Test Item Group B - 2", "_Test Item Group B - 1", merge=True)
		records_to_test = test_records[2:]
		del records_to_test[3]
		self.test_basic_tree(records=records_to_test)

		# insert Group B - 2back
		dontmanage.copy_doc(test_records[5]).insert()
		self.test_basic_tree()
