...
import dontmanage
from dontmanage import _, scrub, throw
...
from dontmanage.utils import cstr, getdate, today, validate_email_address
from dontmanage.utils.nestedset import NestedSet

from dontmanageerp.utilities.transaction_base import delete_events


class EmployeeUserDisabledError(dontmanage.ValidationError):
	pass


class InactiveEmployeeStatusError(dontmanage.ValidationError):
	pass
...
class Employee(NestedSet):
	...
	def validate(self):
		...
		if self.user_id:
			...
			existing_user_id = dontmanage.db.get_value("Employee", self.name, "user_id")
			if existing_user_id:
				user = dontmanage.get_doc("User", existing_user_id)
	...
	def validate_user_details(self):
		if self.user_id:
			data = dontmanage.db.get_value("User", self.user_id, ["enabled", "user_image"], as_dict=1)
	...
	def update_user_permissions(self):
		...
		employee_user_permission_exists = dontmanage.db.exists(
	...
	def update_user(self):
		...
		user = dontmanage.get_doc("User", self.user_id)
	...
	def set_preferred_email(self):
		preferred_email_field = dontmanage.scrub(self.prefered_contact_email)
	...
	def validate_status(self):
		if self.status == "Left":
			reports_to = dontmanage.db.get_all(
				"Employee",
			...
			if reports_to:
				link_to_employees = [
					dontmanage.utils.get_link_to_form("Employee", employee.name, label=employee.employee_name)
	...
	def validate_for_enabled_user_id(self, enabled):
		...
		if enabled is None:
			dontmanage.throw(_("User {0} does not exist").format(self.user_id))
		if enabled == 0:
			dontmanage.throw(_("User {0} is disabled").format(self.user_id), EmployeeUserDisabledError)
	...
	def validate_preferred_email(self):
		if self.prefered_contact_email and not self.get(scrub(self.prefered_contact_email)):
			dontmanage.msgprint(_("Please enter {0}").format(self.prefered_contact_email))
	...
	def reset_employee_emails_cache(self):
		...
		if cell_number != prev_number or self.get("user_id") != prev_doc.get("user_id"):
			dontmanage.cache().hdel("employees_with_number", cell_number)
			dontmanage.cache().hdel("employees_with_number", prev_number)


def validate_employee_role(doc, method=None, ignore_emp_check=False):
	# called via User hook
	if not ignore_emp_check:
		if dontmanage.db.get_value("Employee", {"user_id": doc.name}):
			return

	user_roles = [d.role for d in doc.get("roles")]
	if "Employee" in user_roles:
		dontmanage.msgprint(_("User {0}: Removed Employee role as there is no mapped employee.").format(doc.name))
...
def update_user_permissions(doc, method):
	...
	if "Employee" in [d.role for d in doc.get("roles")]:
		if not has_permission("User Permission", ptype="write", raise_exception=False):
			return
		employee = dontmanage.get_doc("Employee", {"user_id": doc.name})
...
def get_holiday_list_for_employee(employee, raise_exception=True):
	if employee:
		holiday_list, company = dontmanage.get_cached_value("Employee", employee, ["holiday_list", "company"])
	else:
		holiday_list = ""
...
def is_holiday(employee, date=None, raise_exception=True, only_non_weekly=False, with_description=False):
	...
	holidays = dontmanage.get_all("Holiday", fields=["description"], filters=filters, pluck="description")
...
def deactivate_sales_person(status=None, employee=None):
	if status == "Left":
		sales_person = dontmanage.db.get_value("Sales Person", {"Employee": employee})
		if sales_person:
			dontmanage.db.set_value("Sales Person", sales_person, "enabled", 0)


@dontmanage.whitelist()
def create_user(employee, user=None, email=None):
	emp = dontmanage.get_doc("Employee", employee)
...
def get_all_employee_emails(company):
	...
	employee_list = dontmanage.get_all(
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = dontmanage.db.get_value(
...
def get_children(doctype, parent=None, company=None, is_root=False, is_tree=False):
	...
	employees = dontmanage.get_list(doctype, fields=fields, filters=filters, order_by="name")

	for employee in employees:
		is_expandable = dontmanage.get_all(doctype, filters=[["reports_to", "=", employee.get("value")]])
...
def on_doctype_update():
	dontmanage.db.add_index("Employee", ["lft", "rgt"])
