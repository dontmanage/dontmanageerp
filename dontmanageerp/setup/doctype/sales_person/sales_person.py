...
from dontmanageerp import get_default_currency
...
class SalesPerson(NestedSet):
	...
	def get_email_id(self):
		if self.employee:
			user = dontmanage.db.get_value("Employee", self.employee, "user_id")
			if not user:
				dontmanage.throw(_("User ID not set for Employee {0}").format(self.employee))
			else:
				return dontmanage.db.get_value("User", user, "email") or user

	def validate_employee_id(self):
		if self.employee:
			sales_person = dontmanage.db.get_value("Sales Person", {"employee": self.employee})
...
def on_doctype_update():
	dontmanage.db.add_index("Sales Person", ["lft", "rgt"])


def get_timeline_data(doctype: str, name: str) -> dict[int, int]:
	def _fetch_activity(doctype: str, date_field: str):
		sales_team = dontmanage.qb.DocType("Sales Team")
		transaction = dontmanage.qb.DocType(doctype)

		return dict(
			dontmanage.qb.from_(transaction)
			.join(sales_team)
