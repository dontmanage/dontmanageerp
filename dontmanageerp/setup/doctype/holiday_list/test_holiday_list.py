...
from dontmanageerp.setup.doctype.holiday_list.holiday_list import local_country_name
...
class TestHolidayList(unittest.TestCase):
	def test_holiday_list(self):
		...
		fetched_holiday_list = dontmanage.get_value("Holiday List", holiday_list.name)
		self.assertEqual(holiday_list.name, fetched_holiday_list)

	def test_weekly_off(self):
		holiday_list = dontmanage.new_doc("Holiday List")
...
def make_holiday_list(
	...
	dontmanage.delete_doc_if_exists("Holiday List", name, force=1)
