...
from dontmanage.utils import formatdate, getdate, today
...
class HolidayList(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.setup.doctype.holiday.holiday import Holiday
	...
	def get_local_holidays(self):
		...
		for holiday_date, holiday_name in country_holidays(
			...
			language=dontmanage.local.lang,
		).items():
	...
	def validate_dupliacte_date(self):
		...
		for row in self.holidays:
			if row.holiday_date in unique_dates:
				dontmanage.throw(_("Holiday Date {0} added multiple times").format(dontmanage.bold(row.holiday_date)))
...
def is_holiday(holiday_list, date=None):
	...
	if holiday_list:
		return bool(
			dontmanage.db.exists("Holiday", {"parent": holiday_list, "holiday_date": date}, cache=True)
		)
	else:
		return False
...
def local_country_name(country_code: str) -> str:
	...
	return Locale.parse(dontmanage.local.lang, sep="-").territories.get(country_code, country_code)
