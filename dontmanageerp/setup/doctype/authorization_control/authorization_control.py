...
from dontmanage.utils import comma_or, cstr, flt, has_common
...
class AuthorizationControl(TransactionBase):
	...
	def get_appr_user_role(self, det, doctype_name, total, based_on, condition, master_name, company):
		...
		if det:
			...
			if not has_common(appr_roles, dontmanage.get_roles()) and not has_common(
				appr_users, [session["user"]]
			):
				dontmanage.msgprint(_("Not authroized since {0} exceeds limits").format(_(based_on)))
				dontmanage.throw(_("Can be approved by {0}").format(comma_or(appr_roles + appr_users)))
	...
	def validate_auth_rule(self, doctype_name, total, based_on, cond, company, master_name=""):
		...
		if based_on in ["Itemwise Discount", "Item Group wise Discount"]:
			add_cond1 += " and master_name = " + dontmanage.db.escape(cstr(master_name))
			itemwise_exists = dontmanage.db.sql(
	...
	def bifurcate_based_on_type(self, doctype_name, total, av_dis, based_on, doc_obj, val, company):
		...
		if val == 1:
			add_cond += " and system_user = {}".format(dontmanage.db.escape(session["user"]))
		elif val == 2:
			add_cond += " and system_role IN %s" % ("('" + "','".join(dontmanage.get_roles()) + "')")
		else:
		...
		if based_on == "Grand Total":
			...
			if doc_obj:
				...
				add_cond = " and master_name = {}".format(dontmanage.db.escape(customer))
	...
	def validate_approving_authority(self, doctype_name, company, total, doc_obj=""):
		if not dontmanage.db.count("Authorization Rule"):
			return
