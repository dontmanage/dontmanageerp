...
test_records = dontmanage.get_test_records("Currency Exchange")
...
def save_new_records(test_records):
	for record in test_records:
		...
		try:
			dontmanage.set_value(**kwargs)
		except dontmanage.DoesNotExistError:
			curr_exchange = dontmanage.new_doc(record.get("doctype"))
...
class TestCurrencyExchange(unittest.TestCase):
	...
	def tearDown(self):
		dontmanage.db.set_single_value("Accounts Settings", "allow_stale", 1)
		self.clear_cache()
	...
	def test_exchange_rate_via_exchangerate_host(self, mock_get):
		...
		settings = dontmanage.get_single("Currency Exchange Settings")
	...
	def test_exchange_rate_strict(self, mock_get):
		...
		dontmanage.db.set_single_value("Accounts Settings", "allow_stale", 0)
		dontmanage.db.set_single_value("Accounts Settings", "stale_days", 1)
