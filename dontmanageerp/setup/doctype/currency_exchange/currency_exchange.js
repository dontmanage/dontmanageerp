...
dontmanage.ui.form.on("Currency Exchange", {
	onload: function (frm) {
		if (frm.doc.__islocal) {
			frm.set_value("to_currency", dontmanage.defaults.get_global_default("currency"));
		}
	},
	...
	set_exchange_rate_label: function (frm) {
		if (frm.doc.from_currency && frm.doc.to_currency) {
			var default_label = __(dontmanage.meta.docfield_map[frm.doctype]["exchange_rate"].label);
