...
import dontmanage
import dontmanage.desk.notifications
from dateutil.relativedelta import relativedelta
from dontmanage import _
from dontmanage.core.doctype.user.user import STANDARD_USERS
...
from dontmanageerp.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.setup.doctype.email_digest_recipient.email_digest_recipient import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = dontmanage.db.get_value("Company", self.company, "default_currency")

	@dontmanage.whitelist()
	def get_users(self):
		"""get list of users"""
		user_list = dontmanage.db.sql(
			"""
		...
		dontmanage.response["user_list"] = user_list
	...
	def get_msg_html(self):
		...
		from dontmanageerp.setup.doctype.email_digest.quotes import get_random_quote
		...
		if self.get("purchase_orders_items_overdue"):
			...
			if not context.purchase_order_list:
				dontmanage.throw(_("No items to be received are overdue"))

		if not context:
			return None

		dontmanage.flags.ignore_account_permission = False

		# style
		return dontmanage.render_template(
			"dontmanageerp/setup/doctype/email_digest/templates/default.html", context, is_path=True
		)
	...
	def get_notifications(self):
		...
		notifications = dontmanage.desk.notifications.get_notifications()
	...
	def get_issue_list(self, user_id=None):
		...
		meta = dontmanage.get_meta("Issue")
		role_permissions = dontmanage.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = dontmanage.db.sql(
			"""select *
	...
	def get_project_list(self, user_id=None):
		...
		project_list = dontmanage.db.sql(
			"""select *
	...
	def set_accounting_cards(self, context):
		...
		for key in (
			...
			if self.get(key):
				...
				if card:
					card = dontmanage.safe_eval(card)

				else:
					card = dontmanage._dict(getattr(self, "get_" + key)())
	...
	def get_expenses_booked(self):
		...
		expense_account = dontmanage.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		...
		value, count, billed_value, delivered_value = dontmanage.db.sql(
	...
	def get_purchase_orders_items_overdue_list(self):
		...
		purchase_order_list = dontmanage.db.sql(sql_po, as_dict=True)
		purchase_order_items_overdue_list = dontmanage.db.sql(sql_poi, as_dict=True)
...
def send():
	...
	for ed in dontmanage.db.sql(
		"""select name from `tabEmail Digest`
			where enabled=1 and docstatus<2""",
		as_list=1,
	):
		ed_obj = dontmanage.get_doc("Email Digest", ed[0])
...
def get_digest_msg(name):
	return dontmanage.get_doc("Email Digest", name).get_msg_html()
