...
dontmanage.provide("dontmanageerp.company");

dontmanage.ui.form.on("Company", {
	onload: function (frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			dontmanage.db.get_value("Company", frm.doc.parent_company, "is_group", (r) => {
				if (!r.is_group) {
	...
	date_of_commencement: function (frm) {
		if (frm.doc.date_of_commencement < frm.doc.date_of_incorporation) {
			dontmanage.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			...
			if (dontmanage.perm.has_perm("Cost Center", 0, "read")) {
				frm.add_custom_button(
					__("Cost Centers"),
					function () {
						dontmanage.set_route("Tree", "Cost Center", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (dontmanage.perm.has_perm("Account", 0, "read")) {
				frm.add_custom_button(
					__("Chart of Accounts"),
					function () {
						dontmanage.set_route("Tree", "Account", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (dontmanage.perm.has_perm("Sales Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Sales Tax Template"),
					function () {
						dontmanage.set_route("List", "Sales Taxes and Charges Template", {
			...
			if (dontmanage.perm.has_perm("Purchase Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Purchase Tax Template"),
					function () {
						dontmanage.set_route("List", "Purchase Taxes and Charges Template", {
							company: frm.doc.name,
						});
					},
					__("View")
		...
		dontmanageerp.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	make_default_tax_template: function (frm) {
		frm.call({
			...
			freeze: true,
			callback: function () {
				dontmanage.msgprint(__("Default tax templates for sales, purchase and items are created."));
			},
		});
	},
	...
	delete_company_transactions: function (frm) {
		dontmanage.call({
			method: "dontmanageerp.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
...
dontmanageerp.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return dontmanage.call({
			method: "dontmanageerp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
dontmanageerp.company.setup_queries = function (frm) {
	$.each(
		[
...
dontmanageerp.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
