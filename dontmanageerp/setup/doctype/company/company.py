...
class Company(NestedSet):
	...
	def validate_abbr(self):
		...
		if dontmanage.db.sql("select abbr from tabCompany where name!=%s and abbr=%s", (self.name, self.abbr)):
	...
	def validate_advance_account_currency(self):
		if (
			...
			and dontmanage.get_cached_value("Account", self.default_advance_received_account, "account_currency")
			!= self.default_currency
		):
			dontmanage.throw(
				_("'{0}' should be in company currency {1}.").format(
					dontmanage.bold(_("Default Advance Received Account")), dontmanage.bold(self.default_currency)
				)
			)

		if (
			self.default_advance_paid_account
			and dontmanage.get_cached_value("Account", self.default_advance_paid_account, "account_currency")
			!= self.default_currency
		):
			dontmanage.throw(
				_("'{0}' should be in company currency {1}.").format(
					dontmanage.bold(_("Default Advance Paid Account")), dontmanage.bold(self.default_currency)
				)
			)
	...
	def validate_currency(self):
		if self.is_new():
			return
		self.previous_default_currency = dontmanage.get_cached_value("Company", self.name, "default_currency")
		if (
			self.default_currency
	...
	def create_default_warehouses(self):
		for wh_detail in [
			...
			if not dontmanage.db.exists("Warehouse", "{} - {}".format(wh_detail["warehouse_name"], self.abbr)):
	...
	def create_default_accounts(self):
		...
		self.db_set(
			...
			dontmanage.db.get_value("Account", {"company": self.name, "account_type": "Payable", "is_group": 0}),
		)
			...
			if (
				...
				dontmanage.throw(
					_("Set default {0} account for non stock items").format(
						dontmanage.bold(_("Provisional Account"))
					)
				)
	...
	def check_country_change(self):
		...
		if not self.is_new() and self.country != dontmanage.get_cached_value("Company", self.name, "country"):
			dontmanage.flags.country_change = True
	...
	def on_trash(self):
		...
		if not rec:
			...
			for doctype in ["Account", "Cost Center", "Budget", "Party Account"]:
				dontmanage.db.sql(f"delete from `tab{doctype}` where company = %s", self.name)
...
def update_company_current_month_sales(company):
	...
	results = dontmanage.db.sql(
		...
		WHERE
			...
			AND docstatus = 1
			AND company = {dontmanage.db.escape(company)}
		GROUP BY
			month_year
	""",
		as_dict=True,
...
def update_company_monthly_sales(company):
	...
	filter_str = f"company = {dontmanage.db.escape(company)} and status != 'Draft' and docstatus=1"
...
def update_transactions_annual_history(company, commit=False):
	...
	dontmanage.db.set_value("Company", company, "transactions_annual_history", json.dumps(transactions_history))
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	return dontmanage.db.sql(
		...
		from
			`tabCompany` comp
		where
			ifnull(parent_company, "")={dontmanage.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def create_transaction_deletion_request(company):
	from dontmanageerp.setup.doctype.transaction_deletion_record.transaction_deletion_record import (
