...
from dontmanageerp.setup.doctype.company.company import get_default_company_address
...
class TestCompany(unittest.TestCase):
	def test_coa_based_on_existing_company(self):
		...
		dontmanage.delete_doc("Company", "COA from Existing Company")
			...
			for template in templates:
				try:
					...
					for account_type in account_types:
						...
						has_matching_accounts = dontmanage.get_all("Account", filters)
					...
					dontmanage.delete_doc("Company", template)
	...
	def test_basic_tree(self, records=None):
		min_lft = 1
		max_rgt = dontmanage.db.sql("select max(rgt) from `tabCompany`")[0][0]

		if not records:
			records = test_records[2:]

		for company in records:
			lft, rgt, parent_company = dontmanage.db.get_value(
				"Company", company["company_name"], ["lft", "rgt", "parent_company"]
			)

			if parent_company:
				parent_lft, parent_rgt = dontmanage.db.get_value("Company", parent_company, ["lft", "rgt"])
			else:
				# root
	...
	def test_primary_address(self):
		...
		primary = dontmanage.copy_doc(secondary)
	...
	def test_change_parent_company(self):
		child_company = dontmanage.get_doc("Company", "_Test Company 5")
...
def create_child_company():
	child_company = dontmanage.db.exists("Company", "Test Company")
	if not child_company:
		child_company = dontmanage.get_doc(
			{
				"doctype": "Company",
		...
		child_company = dontmanage.get_doc("Company", child_company)

	return child_company.name


def create_test_lead_in_company(company):
	lead = dontmanage.db.exists("Lead", "Test Lead in new company")
	if not lead:
		...
		lead = dontmanage.get_doc("Lead", lead)
