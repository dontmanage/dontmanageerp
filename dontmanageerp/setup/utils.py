...
from dontmanage.utils import add_days, flt, get_datetime_str, nowdate
...
def get_exchange_rate(from_currency, to_currency, transaction_date=None, args=None):
	...
	currency_settings = dontmanage.get_doc("Accounts Settings").as_dict()
	...
	if dontmanage.get_cached_value("Currency Exchange Settings", "Currency Exchange Settings", "disabled"):
		return 0.00
	...
	try:
		...
		if not value:
			...
			settings = dontmanage.get_cached_doc("Currency Exchange Settings")
			req_params = {
		...
		dontmanage.log_error("Unable to fetch exchange rate")
		dontmanage.msgprint(
			_(
...
def _enable_all_roles_for_admin():
	...
	all_roles = set(dontmanage.db.get_values("Role", pluck="name"))
	admin_roles = set(
		dontmanage.db.get_values("Has Role", {"parent": "Administrator"}, fieldname="role", pluck="role")
	)
...
def set_defaults_for_tests():
	...
	dontmanage.db.set_single_value("Selling Settings", defaults)
	for key, value in defaults.items():
		dontmanage.db.set_default(key, value)
...
def welcome_email():
	site_name = get_default_company() or "DontManageErp"
