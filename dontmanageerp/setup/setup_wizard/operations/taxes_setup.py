...
def setup_taxes_and_charges(company_name: str, country: str):
	if not dontmanage.db.exists("Company", company_name):
		dontmanage.throw(_("Company {} does not exist yet. Taxes setup aborted.").format(company_name))
...
def from_detailed_data(company_name, data):
	...
	if (
		dontmanage.db.get_value("Company", company_name, "create_chart_of_accounts_based_on")
		== "Existing Company"
	):
		charts_company_name = dontmanage.db.get_value("Company", company_name, "existing_company")
	coa_name = dontmanage.db.get_value("Company", charts_company_name, "chart_of_accounts")
...
def update_regional_tax_settings(country, company):
	path = dontmanage.get_app_path("dontmanageerp", "regional", dontmanage.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = f"dontmanageerp.regional.{dontmanage.scrub(country)}.setup.update_regional_tax_settings"
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	if dontmanage.db.exists(doctype, {"title": template.get("title"), "company": company_name}):
		return
	...
	for tax_row in template.get("taxes"):
		...
		tax_row_defaults = {
			...
			"cost_center": dontmanage.db.get_value("Company", company_name, "cost_center"),
		}
	...
	doc = dontmanage.get_doc(template)
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = dontmanage.get_all(
		"Account",
	...
	if existing_accounts:
		return dontmanage.get_doc("Account", existing_accounts[0].name)
	...
	doc = dontmanage.get_doc(account)
	doc.flags.ignore_links = True
	doc.flags.ignore_validate = True
...
def get_or_create_tax_group(company_name, root_type):
	...
	tax_group_name = dontmanage.db.get_value(
		"Account",
	...
	root_account = dontmanage.get_all(
		"Account",
		{
			"is_group": 1,
			"root_type": root_type,
	...
	tax_group_account = dontmanage.get_doc(
		{
			"doctype": "Account",
			"company": company_name,
			"is_group": 1,
...
def make_tax_category(tax_category):
	...
	if not dontmanage.db.exists(doctype, tax_category["title"]):
		doc = dontmanage.get_doc(tax_category)
