...
from dontmanageerp.accounts.doctype.account.account import RootNotEditable
from dontmanageerp.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	response = dontmanage.read_file(os.path.join(base_path, "delivery_trip/dispatch_notification_template.html"))
	...
	from dontmanageerp.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def update_buying_defaults():
	buying_settings = dontmanage.get_doc("Buying Settings")
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(dontmanage.get_app_path("dontmanageerp", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
		if not dontmanage.db.exists("UOM", _(d.get("uom_name"))):
			dontmanage.get_doc(
				{
					"doctype": "UOM",
	...
	uom_conversions = json.loads(
		open(
			dontmanage.get_app_path("dontmanageerp", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
	for d in uom_conversions:
		if not dontmanage.db.exists("UOM Category", _(d.get("category"))):
			dontmanage.get_doc({"doctype": "UOM Category", "category_name": _(d.get("category"))}).db_insert()
...
def add_sale_stages():
	...
	for sales_stage in records:
		dontmanage.get_doc(sales_stage).db_insert()
...
def install_defaults(args=None):  # nosemgrep
	...
	dontmanage.db.set_value("Currency", args.get("currency"), "enabled", 1)
	dontmanage.db.set_single_value("Stock Settings", "email_footer_address", args.get("company_name"))
...
def update_stock_settings():
	stock_settings = dontmanage.get_doc("Stock Settings")
	stock_settings.item_naming_by = "Item Code"
	stock_settings.valuation_method = "FIFO"
	stock_settings.default_warehouse = dontmanage.db.get_value("Warehouse", {"warehouse_name": _("Stores")})
...
def create_bank_account(args):
	...
	if bank_account_group:
		bank_account = dontmanage.get_doc(
			{
				"doctype": "Account",
		...
		try:
			...
			dontmanage.throw(_("Bank account cannot be named as {0}").format(args.get("bank_account")))
