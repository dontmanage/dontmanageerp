...
import dontmanage
from dontmanage import _, bold, scrub
...
class NegativeStockError(dontmanage.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(dontmanage.ValidationError):
	pass
...
def validate_serial_no(sle):
	...
	for sn in get_serial_nos(sle.serial_no):
		...
		for row in get_stock_ledger_entries(args, ">"):
			voucher_type = dontmanage.bold(row.voucher_type)
			voucher_no = dontmanage.bold(get_link_to_form(row.voucher_type, row.voucher_no))
			vouchers.append(f"{voucher_type} {voucher_no}")

		if vouchers:
			serial_no = dontmanage.bold(sn)
			msg = (
			...
			dontmanage.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransaction)


def validate_cancellation(args):
	if args[0].get("is_cancelled"):
		repost_entry = dontmanage.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = dontmanage.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def set_as_cancel(voucher_type, voucher_no):
	dontmanage.db.sql(
		...
		(now(), dontmanage.session.user, voucher_type, voucher_no),
	)


def make_entry(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	args["doctype"] = "Stock Ledger Entry"
	sle = dontmanage.get_doc(args)
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if args.get(field) in [None, ""]:
			validation_msg = f"The field {dontmanage.unscrub(field)} is required for the reposting"
			dontmanage.throw(_(validation_msg))
...
def get_items_to_be_repost(voucher_type=None, voucher_no=None, doc=None, reposting_data=None):
	...
	if not items_to_be_repost and voucher_type and voucher_no:
		items_to_be_repost = dontmanage.db.get_all(
			"Stock Ledger Entry",
...
def get_distinct_item_warehouse(args=None, doc=None, reposting_data=None):
	...
	if doc and doc.distinct_item_and_warehouse:
		...
		distinct_item_warehouses = {
			dontmanage.safe_eval(k): dontmanage._dict(v) for k, v in distinct_item_warehouses.items()
		}
	else:
		for i, d in enumerate(args):
			distinct_item_warehouses.setdefault(
				(d.item_code, d.warehouse), dontmanage._dict({"reposting_status": False, "sle": d, "args_idx": i})
			)
...
def get_affected_transactions(doc, reposting_data=None) -> set[tuple[str, str]]:
	...
	transactions = dontmanage.parse_json(doc.affected_transactions)
...
class update_entries_after:
	...
	def __init__(
		...
		self.use_moving_avg_for_batch = dontmanage.db.get_single_value(
		...
		self.args = dontmanage._dict(args)
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.company = dontmanage.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.set_precision()
	...
	def set_precision(self):
		self.flt_precision = cint(dontmanage.db.get_default("float_precision")) or 2
		self.currency_precision = get_field_precision(
			dontmanage.get_meta("Stock Ledger Entry").get_field("stock_value")
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, dontmanage._dict())
	...
	def has_stock_reco_with_serial_batch(self, sle):
		if (
			...
			and dontmanage.db.get_value(sle.voucher_type, sle.voucher_no, "set_posting_time") == 1
		):
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or dontmanage._dict(
	...
	def get_serialized_values(self, sle):
		from dontmanageerp.stock.serial_batch_bundle import SerialNoValuation
	...
	def reset_actual_qty_for_stock_reco(self, sle):
		...
		if sle.serial_and_batch_bundle and dontmanage.get_cached_value("Item", sle.item_code, "has_serial_no"):
	...
	def calculate_valuation_for_serial_batch_bundle(self, sle):
		if not dontmanage.db.exists("Serial and Batch Bundle", sle.serial_and_batch_bundle):
			return

		if self.args.get("sle_id") and sle.actual_qty < 0:
			doc = dontmanage.db.get_value(
			...
			doc = dontmanage.get_doc("Serial and Batch Bundle", sle.serial_and_batch_bundle)
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = dontmanage.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if dontmanage.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from dontmanageerp.controllers.sales_and_purchase_return import (
				...
				# check in item table
				item_code, incoming_rate = dontmanage.db.get_value(
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no, voucher_detail_no):
		stock_entry = dontmanage.get_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = dontmanage.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
			dontmanage.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if dontmanage.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			if sle.voucher_type in ["Purchase Receipt", "Purchase Invoice"] and dontmanage.get_cached_value(
		...
		if dontmanage.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = dontmanage.get_doc(sle.voucher_type, sle.voucher_no)
	...
	def get_incoming_value_for_serial_nos(self, sle, serial_nos):
		...
		all_serial_nos = dontmanage.get_all(
		...
		for serial_no in invalid_serial_nos:
			incoming_rate = dontmanage.db.sql(
				"""
	...
	def update_batched_values(self, sle):
		...
		if actual_qty > 0:
			...
			new_sle.qty = new_sle.actual_qty
			new_sle.batch_nos = dontmanage._dict({new_sle.batch_no: new_sle})
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return dontmanage.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else dontmanage._dict()
		return sle
		...
		for warehouse, exceptions in self.exceptions.items():
			...
			if (
				...
				msg = _(
					...
					dontmanage.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
	...
	def update_bin(self):
		...
		for warehouse, data in self.data.items():
			...
			dontmanage.db.set_value("Bin", bin_name, updated_values, update_modified=True)
...
def get_previous_sle_of_current_voucher(args, operator="<", exclude_current_voucher=False):
	...
	sle = dontmanage.db.sql(  # nosemgrep
		f"""
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(dontmanage.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
		...
		"""
			...
			dontmanage.db.escape(serial_no),
			dontmanage.db.escape(f"{serial_no}\n%"),
			dontmanage.db.escape(f"%\n{serial_no}"),
			dontmanage.db.escape(f"%\n{serial_no}\n%"),
		)
...
def get_valuation_rate(
	...
	if last_valuation_rate := dontmanage.db.sql(  # nosemgrep
	...
	if not valuation_rate:
		...
		valuation_rate = dontmanage.db.get_value("Item", item_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = dontmanage.db.get_value(
	...
	if (
		...
		and cint(dontmanageerp.is_perpetual_inventory_enabled(company))
	):
		...
		dontmanage.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if (
		...
		and dontmanage.db.get_value("Stock Reconciliation Item", args.voucher_detail_no, "qty") > 0
	):
		return
	...
	if is_negative_with_precision(neg_sle):
		message = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
			...
			dontmanage.get_desk_link("Item", args.item_code),
			dontmanage.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			dontmanage.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
...
def get_future_sle_with_negative_qty(sle_args):
	return dontmanage.db.sql(  # nosemgrep
		"""
		select
...
def is_negative_stock_allowed(*, item_code: str | None = None) -> bool:
	if cint(dontmanage.db.get_single_value("Stock Settings", "allow_negative_stock", cache=True)):
		return True
	if item_code and cint(dontmanage.db.get_value("Item", item_code, "allow_negative_stock", cache=True)):
		return True
	return False
...
def get_incoming_rate_for_inter_company_transfer(sle) -> float:
	...
	reference_name = dontmanage.get_cached_value(sle.voucher_type + " Item", sle.voucher_detail_no, field)

	if reference_name:
		rate = dontmanage.get_cached_value(
			doctype,
			reference_name,
			"incoming_rate",
		)
