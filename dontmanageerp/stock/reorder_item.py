...
from dontmanage.utils import add_days, cint, flt, nowdate
...
def reorder_item():
	...
	if not (dontmanage.db.a_row_exists("Company") and dontmanage.db.a_row_exists("Fiscal Year")):
		return

	if cint(dontmanage.db.get_value("Stock Settings", None, "auto_indent")):
		return _reorder_item()


def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = dontmanage._dict(
		dontmanage.db.sql(
	...
	default_company = (
		dontmanageerp.get_default_company() or dontmanage.db.sql("""select name from tabCompany limit 1""")[0][0]
	)
...
def get_items_for_reorder() -> dict[str, list]:
	reorder_table = dontmanage.qb.DocType("Item Reorder")
	item_table = dontmanage.qb.DocType("Item")

	query = (
		dontmanage.qb.from_(reorder_table)
	...
	itemwise_reorder = dontmanage._dict({})
	for d in data:
...
def get_reorder_levels_for_variants(itemwise_reorder):
	...
	query = (
		dontmanage.qb.from_(item_table)
		.select(
			item_table.name,
			item_table.variant_of,
		)
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in dontmanage.db.sql(
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = dontmanage.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	def _log_exception(mr):
		if dontmanage.local.message_log:
			exceptions_list.extend(dontmanage.local.message_log)
			dontmanage.local.message_log = []
		else:
			exceptions_list.append(dontmanage.get_traceback(with_context=True))

		mr.log_error("Unable to create material request")

	company_wise_mr = dontmanage._dict({})
		...
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					...
					must_be_whole_number = dontmanage.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
	...
	if company_wise_mr:
		if getattr(dontmanage.local, "reorder_email_notify", None) is None:
			dontmanage.local.reorder_email_notify = cint(
				dontmanage.db.get_value("Stock Settings", None, "reorder_email_notify")
			)

		if dontmanage.local.reorder_email_notify:
...
def send_email_notification(company_wise_mr):
	...
	for company, mr_list in company_wise_mr.items():
		...
		msg = dontmanage.render_template("templates/emails/reorder_item.html", {"mr_list": mr_list})

		dontmanage.sendmail(recipients=email_list, subject=_("Auto Material Requests Generated"), message=msg)


def get_email_list(company):
	users = get_comapny_wise_users(company)
	user_table = dontmanage.qb.DocType("User")
	role_table = dontmanage.qb.DocType("Has Role")

	query = (
		dontmanage.qb.from_(user_table)
...
def get_comapny_wise_users(company):
	...
	if parent_company := dontmanage.db.get_value("Company", company, "parent_company"):
...
def notify_errors(exceptions_list):
	subject = _("[Important] [DontManageErp] Auto Reorder Errors")
	content = (
	...
	from dontmanage.email import sendmail_to_system_managers
