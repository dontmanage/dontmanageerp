...
from dontmanage.utils import add_days, add_months, cint, cstr, flt, getdate
...
from dontmanageerp.stock.doctype.batch.batch import get_batch_no
...
def set_valuation_rate(out, args):
	if dontmanage.db.exists("Product Bundle", args.item_code, cache=True):
		valuation_rate = 0.0
...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if args.get("batch_no") and item.name != dontmanage.get_cached_value(
...
def get_barcode_data(items_list):
	...
	for item in items_list:
		barcodes = dontmanage.db.get_all(
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = "dontmanageerp.stock.get_item_details.{0}".format(method)
	...
	if (
		company
		and cost_center
		and dontmanage.get_cached_value("Cost Center", cost_center, "company") != company
	):
		return None
		...
		if not args.get("price_list_currency"):
			...
			if meta.get_field("plc_conversion_rate"):
				args.plc_conversion_rate = flt(
					args.plc_conversion_rate,
					get_field_precision(meta.get_field("plc_conversion_rate"), dontmanage._dict({"fields": args})),
				)
...
def get_serial_nos_by_fifo(args, sales_order=None):
	if dontmanage.db.get_single_value("Stock Settings", "automatically_set_serial_nos_based_on_fifo"):
...
def get_bin_details_and_serial_nos(
	...
	if flt(stock_qty) > 0:
		if has_batch_no:
			args = dontmanage._dict({"item_code": item_code, "warehouse": warehouse, "stock_qty": stock_qty})
			serial_no = get_serial_no(args)
...
def get_serial_no(args, serial_nos=None, sales_order=None):
	...
	if args.get("warehouse") and args.get("stock_qty") and args.get("item_code"):
		has_serial_no = dontmanage.get_value("Item", {"item_code": args.item_code}, "has_serial_no")
...
def get_so_reservation_for_item(args):
	...
	if args.get("against_sales_order"):
		...
		sales_order = dontmanage.db.get_all(
			"Sales Invoice Item",
...
def get_reserved_qty_for_so(sales_order, item_code):
	reserved_qty = dontmanage.db.get_value(
		"Sales Order Item",
		filters={
			"parent": sales_order,
			"item_code": item_code,
