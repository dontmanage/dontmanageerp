...
from dontmanage.utils import add_days, add_months, cint, cstr, flt, getdate, parse_json

from dontmanageerp import get_company_currency
from dontmanageerp.accounts.doctype.pricing_rule.pricing_rule import (
...
from dontmanageerp.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from dontmanageerp.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from dontmanageerp.stock.doctype.price_list.price_list import get_price_list_details
...
def get_item_details(args, doc=None, for_validate=False, overwrite_warehouse=True):
	...
	item = dontmanage.get_cached_doc("Item", args.item_code)
...
def set_valuation_rate(out, args):
	if dontmanage.db.exists("Product Bundle", {"name": args.item_code, "disabled": 0}, cache=True):
		valuation_rate = 0.0
		bundled_items = dontmanage.get_doc("Product Bundle", args.item_code)
...
def get_item_code(barcode=None, serial_no=None):
	if barcode:
		item_code = dontmanage.db.get_value("Item Barcode", {"barcode": barcode}, fieldname=["parent"])
		if not item_code:
			dontmanage.throw(_("No Item with Barcode {0}").format(barcode))
	elif serial_no:
		item_code = dontmanage.db.get_value("Serial No", serial_no, "item_code")
		if not item_code:
			dontmanage.throw(_("No Item with Serial No {0}").format(serial_no))
...
def validate_item_details(args, item):
	...
	from dontmanageerp.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if not item:
		item = dontmanage.get_doc("Item", args.get("item_code"))

	if item.variant_of and not item.taxes and dontmanage.db.exists("Item Tax", {"parent": item.variant_of}):
	...
	if args.get("doctype") == "Material Request" and not args.get("material_request_type"):
		args["material_request_type"] = dontmanage.db.get_value(
	...
	if args.get("batch_no") and item.name != dontmanage.get_cached_value("Batch", args.get("batch_no"), "item"):
		args["batch_no"] = ""
	...
	if args.get("doctype") in purchase_doctypes and not dontmanage.db.get_single_value(
		"Buying Settings", "disable_last_purchase_rate"
	):
		from dontmanageerp.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
	...
	for d in [
		...
		if not out[d[1]]:
			out[d[1]] = dontmanage.get_cached_value("Company", args.company, d[2]) if d[2] else None
	...
	meta = dontmanage.get_meta(child_doctype)
...
def get_item_warehouse(item, args, overwrite_warehouse, defaults=None):
	...
	if overwrite_warehouse or not args.warehouse:
		...
		if not warehouse:
			...
			warehouse_exists = dontmanage.db.exists(
	...
	if not warehouse:
		default_warehouse = dontmanage.db.get_single_value("Stock Settings", "default_warehouse")
		if dontmanage.db.get_value("Warehouse", default_warehouse, "company") == args.company:
...
def get_barcode_data(items_list=None, item_code=None):
	...
	if not items_list and item_code:
		_dict_item_code = dontmanage._dict(
			{
				"item_code": item_code,
			}
		)

		items_list = [dontmanage._dict(_dict_item_code)]

	for item in items_list:
		barcodes = dontmanage.db.get_all("Item Barcode", filters={"parent": item.item_code}, fields="barcode")
...
def get_item_tax_info(company, tax_category, item_codes, item_rates=None, item_tax_templates=None):
	...
	for item_code in item_codes:
		...
		out[item_code[1]] = {}
		item = dontmanage.get_cached_doc("Item", item_code[0])
		args = {
			"company": company,
...
def get_item_tax_template(args, item, out):
	...
	if not item_tax_template:
		...
		while item_group and not item_tax_template:
			item_group_doc = dontmanage.get_cached_doc("Item Group", item_group)
...
def _get_item_tax_template(args, taxes, out=None, for_validate=False):
	...
	for tax in taxes:
		tax_company = dontmanage.get_cached_value("Item Tax Template", tax.item_tax_template, "company")
...
def get_item_tax_map(company, item_tax_template, as_json=True):
	...
	if item_tax_template:
		template = dontmanage.get_cached_doc("Item Tax Template", item_tax_template)
		for d in template.taxes:
			if dontmanage.get_cached_value("Account", d.tax_type, "company") == company:
...
def get_default_deferred_account(args, item, fieldname=None):
	if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
		return (
			...
			or dontmanage.get_cached_value("Company", args.company, "default_" + fieldname)
		)
	else:
		return None
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
	...
	if args.get("project"):
		cost_center = dontmanage.db.get_value("Project", args.get("project"), "cost_center", cache=True)
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"dontmanageerp.stock.get_item_details.{method}"
			data = dontmanage.get_attr(path)(args.get("item_code"), company)
	...
	if company and cost_center and dontmanage.get_cached_value("Cost Center", cost_center, "company") != company:
		return None

	if not cost_center and company:
		cost_center = dontmanage.get_cached_value("Company", company, "cost_center")
...
def get_price_list_rate(args, item_doc, out=None):
	...
	meta = dontmanage.get_meta(args.parenttype or args.doctype)
	...
	if meta.get_field("currency") or args.get("currency"):
		...
		# insert in database
		if price_list_rate is None or dontmanage.db.get_single_value(
		...
		if dontmanage.db.get_single_value("Buying Settings", "disable_last_purchase_rate"):
			return out
		...
		if (
			...
			from dontmanageerp.stock.doctype.item.item import get_last_purchase_details
...
def insert_item_price(args):
	...
	if dontmanage.db.get_value("Price List", args.price_list, "currency", cache=True) == args.currency and cint(
		dontmanage.db.get_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing")
	):
		if dontmanage.has_permission("Item Price", "write"):
			price_list_rate = (
			...
			if item_price and item_price.name:
				if item_price.price_list_rate != price_list_rate and dontmanage.db.get_single_value(
					"Stock Settings", "update_existing_price_list_rate"
				):
					dontmanage.db.set_value("Item Price", item_price.name, "price_list_rate", price_list_rate)
					dontmanage.msgprint(
...
def get_item_price(args, item_code, ignore_party=False, force_batch_no=False) -> list[dict]:
	...
	:param args: dict (or dontmanage._dict) with mandatory fields price_list, uom
	        optional fields transaction_date, customer, supplier
	:param item_code: str, Item Doctype field item_code
	"""

	ip = dontmanage.qb.DocType("Item Price")
	query = (
		dontmanage.qb.from_(ip)
		...
		.orderby(ip.valid_from, order=dontmanage.qb.desc)
		.orderby(IfNull(ip.batch_no, ""), order=dontmanage.qb.desc)
		.orderby(ip.uom, order=dontmanage.qb.desc)
	)
...
def check_packing_list(price_list_rate_name, desired_qty, item_code):
	...
	flag = True
	item_price = dontmanage.get_doc("Item Price", price_list_rate_name)
...
def validate_conversion_rate(args, meta):
	from dontmanageerp.controllers.accounts_controller import validate_conversion_rate

	company_currency = dontmanage.get_cached_value("Company", args.company, "default_currency")
	...
	args.conversion_rate = flt(
		args.conversion_rate,
		get_field_precision(meta.get_field("conversion_rate"), dontmanage._dict({"fields": args})),
	)

	if args.price_list:
		if not args.plc_conversion_rate and args.price_list_currency == dontmanage.db.get_value(
		...
		if not args.get("price_list_currency"):
			...
			if meta.get_field("plc_conversion_rate"):
				args.plc_conversion_rate = flt(
					args.plc_conversion_rate,
					get_field_precision(
						meta.get_field("plc_conversion_rate"), dontmanage._dict({"fields": args})
					),
				)
...
def get_party_item_code(args, item_doc, out):
	if args.transaction_type == "selling" and args.customer:
		...
		if customer_item_code:
			...
			customer_group = dontmanage.get_cached_value("Customer", args.customer, "customer_group")
...
def get_pos_profile_item_details(company, args, pos_profile=None, update_data=False):
	...
	if not dontmanage.flags.pos_profile and not pos_profile:
		pos_profile = dontmanage.flags.pos_profile = get_pos_profile(company, args.get("pos_profile"))
...
def get_pos_profile(company, pos_profile=None, user=None):
	if pos_profile:
		return dontmanage.get_cached_doc("POS Profile", pos_profile)

	if not user:
		user = dontmanage.session["user"]

	pf = dontmanage.qb.DocType("POS Profile")
	pfu = dontmanage.qb.DocType("POS Profile User")

	query = (
		dontmanage.qb.from_(pf)
		.left_join(pfu)
...
def get_conversion_factor(item_code, uom):
	variant_of = dontmanage.db.get_value("Item", item_code, "variant_of", cache=True)
	filters = {"parent": item_code, "uom": uom}

	if variant_of:
		filters["parent"] = ("in", (item_code, variant_of))
	conversion_factor = dontmanage.db.get_value("UOM Conversion Detail", filters, "conversion_factor")
	if not conversion_factor:
		stock_uom = dontmanage.db.get_value("Item", item_code, "stock_uom")
...
def get_projected_qty(item_code, warehouse):
	return {
		"projected_qty": dontmanage.db.get_value(
...
def get_batch_qty(batch_no, warehouse, item_code):
	from dontmanageerp.stock.doctype.batch import batch
...
def apply_price_list(args, as_doc=False, doc=None):
	...
	if "items" in args:
		...
		for item in item_list:
			args_copy = dontmanage._dict(args.copy())
...
def apply_price_list_on_item(args, doc=None):
	item_doc = dontmanage.db.get_value("Item", args.item_code, ["name", "variant_of"], as_dict=1)
...
def get_default_bom(item_code=None):
	def _get_bom(item):
		bom = dontmanage.get_all("BOM", dict(item=item, is_active=True, is_default=True, docstatus=1), limit=1)
	...
	template_item = dontmanage.db.get_value("Item", item_code, "variant_of")
...
def get_valuation_rate(item_code, company, warehouse=None):
	if dontmanage.get_cached_value("Warehouse", warehouse, "is_group"):
	...
	if item.get("is_stock_item"):
		...
		pi_item = dontmanage.qb.DocType("Purchase Invoice Item")
		valuation_rate = (
			dontmanage.qb.from_(pi_item)
...
def get_blanket_order_details(args):
	...
	if args.item_code:
		bo = dontmanage.qb.DocType("Blanket Order")
		bo_item = dontmanage.qb.DocType("Blanket Order Item")

		query = (
			dontmanage.qb.from_(bo)
			.from_(bo_item)
