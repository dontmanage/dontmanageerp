...
def get_bin(item_code, warehouse):
	...
	if not bin:
		...
		bin_obj = dontmanage.get_doc("Bin", bin)
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	use_moving_avg_for_batch = dontmanage.db.get_single_value("Stock Settings", "do_not_use_batchwise_valuation")
...
def get_valuation_method(item_code):
	...
	if not val_method:
		val_method = dontmanage.db.get_value("Stock Settings", None, "valuation_method", cache=True) or "FIFO"
	return val_method
		...
		if d.get("convertible"):
			...
			columns.insert(
				idx + 1,
				{
					...
					"fieldname": "{}_{}".format(d.get("fieldname"), dontmanage.scrub(include_uom)),
	...
	for row_idx, row in enumerate(result):
		...
		for key, value in data:
			...
			if not is_dict_obj:
				...
				new_key = f"{key}_{dontmanage.scrub(include_uom)}"
