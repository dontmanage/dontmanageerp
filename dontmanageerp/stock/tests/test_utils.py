...
class StockTestMixin:
	...
	def assertGLEs(self, doc, expected_gles, gle_filters=None, order_by=None):
		...
		actual_gles = dontmanage.get_all(
			"GL Entry",
			fields=["*"],
			filters=filters,
...
class TestStockUtilities(DontManageTestCase, StockTestMixin):
	def test_barcode_scanning(self):
		...
		batch = dontmanage.get_doc(doctype="Batch", item=batch_item.name).insert()
		...
		serial = dontmanage.get_doc(
			doctype="Serial No", item_code=serial_item.name, serial_no=dontmanage.generate_hash()
		).insert()
