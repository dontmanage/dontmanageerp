...
dontmanageerp.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	make() {
		...
		this.content = $(dontmanage.render_template('item_dashboard')).appendTo(this.parent);
		...
		this.content.on('click', '.btn-edit', function () {
			...
			dontmanage.db.get_value('Putaway Rule', {
				'item_code': item,
				'warehouse': warehouse,
				'company': company
			}, 'name', (r) => {
				dontmanage.set_route("Form", "Putaway Rule", r.name);
			});
		});
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					dontmanageerp.stock.move_item(item, null, warehouse, actual_qty, rate, function () {
						me.refresh();
					});
				} else {
					dontmanageerp.stock.move_item(item, warehouse, null, actual_qty, null, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(item, warehouse, entry_type) {
			dontmanage.model.with_doctype('Stock Entry', function () {
				var doc = dontmanage.model.get_new_doc('Stock Entry');
				if (entry_type) {
					doc.stock_entry_type = entry_type;
				}

				var row = dontmanage.model.add_child(doc, 'items');
				row.item_code = item;
	...
	render(data) {
		...
		if (context.data.length > 0) {
			...
			$(dontmanage.render_template(this.template, context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data(data, max_count, show_item) {
		...
		let can_write = 0;
		if (dontmanage.boot.user.can_write.indexOf("Stock Entry") >= 0) {
			can_write = 1;
		}
	...
	get_capacity_dashboard_data(data) {
		...
		let can_write = 0;
		if (dontmanage.boot.user.can_write.indexOf("Putaway Rule") >= 0) {
			can_write = 1;
		}
...
dontmanageerp.stock.move_item = function (item, source, target, actual_qty, rate, callback) {
	...
	dialog.set_primary_action(__('Create Stock Entry'), function () {
		if (source && (dialog.get_value("qty") == 0 || dialog.get_value("qty") > actual_qty)) {
			dontmanage.msgprint(__("Quantity must be greater than zero, and less or equal to {0}", [actual_qty]));
			return;
		}

		if (dialog.get_value("source") === dialog.get_value("target")) {
			dontmanage.msgprint(__("Source and target warehouse must be different"));
			return;
		}

		dontmanage.model.with_doctype('Stock Entry', function () {
			let doc = dontmanage.model.get_new_doc('Stock Entry');
			doc.from_warehouse = dialog.get_value('source');
			doc.to_warehouse = dialog.get_value('target');
			doc.stock_entry_type = doc.from_warehouse ? "Material Transfer" : "Material Receipt";
			let row = dontmanage.model.add_child(doc, 'items');
