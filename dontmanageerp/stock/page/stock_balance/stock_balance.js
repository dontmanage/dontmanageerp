dontmanage.pages['stock-balance'].on_page_load = function(wrapper) {
	...
	page.warehouse_field = page.add_field({
		...
		fieldtype:'Link',
		options:'Warehouse',
		default: dontmanage.route_options && dontmanage.route_options.warehouse,
		change: function() {
	...
	page.item_field = page.add_field({
		...
		fieldtype:'Link',
		options:'Item',
		default: dontmanage.route_options && dontmanage.route_options.item_code,
		change: function() {
			page.item_dashboard.start = 0;
			page.item_dashboard.refresh();
		}
	...
	page.item_group_field = page.add_field({
		...
		fieldtype:'Link',
		options:'Item Group',
		default: dontmanage.route_options && dontmanage.route_options.item_group,
		change: function() {
			page.item_dashboard.start = 0;
			page.item_dashboard.refresh();
		}
	...
	page.sort_selector = new dontmanage.ui.SortSelector({
	...
	dontmanage.require('item-dashboard.bundle.js', function() {
		page.item_dashboard = new dontmanageerp.stock.ItemDashboard({
			parent: page.main,
			page_length: 20,
		...
		var setup_click = function(doctype) {
			page.main.on('click', 'a[data-type="'+ doctype.toLowerCase() +'"]', function() {
				...
				if(field.get_value()===name) {
					dontmanage.set_route('Form', doctype, name)
				} else {
