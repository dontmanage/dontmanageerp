...
dontmanage.ui.form.on('Repost Item Valuation', {
	setup: function(frm) {
	...
	setup_realtime_progress: function(frm) {
		dontmanage.realtime.on('item_reposting_progress', data => {
	...
	execute_reposting(frm) {
		frm.add_custom_button(__("Start Reposting"), () => {
			dontmanage.call({
				method: 'dontmanageerp.stock.doctype.repost_item_valuation.repost_item_valuation.execute_repost_item_valuation',
				callback: function() {
					dontmanage.msgprint(__('Reposting has been started in the background.'));
				}
			});
		});
	},
