...
from dontmanageerp.accounts.utils import repost_gle_for_stock_vouchers
from dontmanageerp.controllers.stock_controller import create_item_wise_repost_entries
...
from dontmanageerp.stock.doctype.repost_item_valuation.repost_item_valuation import (
...
from dontmanageerp.stock.utils import PendingRepostingError


class TestRepostItemValuation(DontManageTestCase, StockTestMixin):
	def tearDown(self):
		dontmanage.flags.dont_execute_stock_reposts = False

	def test_repost_time_slot(self):
		repost_settings = dontmanage.get_doc("Stock Reposting Settings")
	...
	def test_clear_old_logs(self):
		...
		logs = dontmanage.get_all("Repost Item Valuation", filters={"status": "Skipped"})
	...
	def test_deduplication(self):
		...
		riv1 = dontmanage.get_doc(riv_args)
		...
		riv2 = dontmanage.get_doc(riv_args.update({"posting_date": "2021-01-03"}))
		...
		riv3 = dontmanage.get_doc(riv_args.update({"posting_date": "2021-01-01"}))
		...
		riv4 = dontmanage.get_doc(riv_args.update({"warehouse": "Stores - _TC"}))
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		riv = dontmanage.get_last_doc(
			"Repost Item Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
		self.assertRaises(dontmanage.ValidationError, riv.cancel)
	...
	def test_queue_progress_serialization(self):
		...
		self.assertEqual(
			[["a", "b"], ["c", "d"]],
			sorted(dontmanage.parse_json(dontmanage.as_json(set([("a", "b"), ("c", "d")])))),
		)

	def test_gl_repost_progress(self):
		from dontmanageerp.accounts import utils
		...
		doc = dontmanage.new_doc("Repost Item Valuation")
	...
	def test_duplicate_ple_on_repost(self):
		...
		ple_entries = dontmanage.db.get_list(
