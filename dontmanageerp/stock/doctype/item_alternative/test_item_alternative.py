...
class TestItemAlternative(DontManageTestCase):
	def setUp(self):
		super().setUp()
		make_items()
	...
	def test_alternative_item_for_subcontract_rm(self):
		...
		reserved_qty_for_sub_contract = dontmanage.db.get_value(
			"Bin",
		...
		doc = dontmanage.get_doc("Stock Entry", se.name)
		...
		doc.save()
		doc.submit()
		after_transfer_reserved_qty_for_sub_contract = dontmanage.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_sub_contract",
		)
		...
		scr = dontmanage.get_doc("Subcontracting Receipt", scr.name)
		status = False
	...
	def test_alternative_item_for_production_rm(self):
		...
		reserved_qty_for_production = dontmanage.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_production",
		)

		ste = dontmanage.get_doc(make_stock_entry(pro_order.name, "Material Transfer for Manufacture", 5))
		ste.insert()
		...
		ste.submit()
		reserved_qty_for_production_after_transfer = dontmanage.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_production",
		)
		...
		ste1 = dontmanage.get_doc(make_stock_entry(pro_order.name, "Manufacture", 5))
...
def make_items():
	...
	if dontmanage.db.exists("Item", "Test FG A RW 1"):
		doc = dontmanage.get_doc("Item", "Test FG A RW 1")
		doc.allow_alternative_item = 1
		doc.save()

	if dontmanage.db.exists("Item", "Test Finished Goods - A"):
		doc = dontmanage.get_doc("Item", "Test Finished Goods - A")
		doc.is_sub_contracted_item = 1
		doc.save()

	if not dontmanage.db.get_value("BOM", {"item": "Test Finished Goods - A", "docstatus": 1}):
		make_bom(item="Test Finished Goods - A", raw_materials=["Test FG A RW 1", "Test FG A RW 2"])

	if not dontmanage.db.get_value("Warehouse", {"warehouse_name": "Test Supplier Warehouse"}):
		dontmanage.get_doc(
			{
				"doctype": "Warehouse",
