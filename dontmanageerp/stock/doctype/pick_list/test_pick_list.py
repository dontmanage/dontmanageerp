...
from dontmanageerp.selling.doctype.sales_order.sales_order import create_pick_list
...
class TestPickList(DontManageTestCase):
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = dontmanage.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = dontmanage.get_doc("Item", {"item_name": "Batched Item"})
	...
	def test_pick_list_for_items_from_multiple_sales_orders(self):
		...
		sales_order = dontmanage.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
				"items": [
					{
						"item_code": item_code,
						"qty": 10,
						"delivery_date": dontmanage.utils.today(),
						"warehouse": "_Test Warehouse - _TC",
					}
				],
			}
	...
	def test_multiple_dn_creation(self):
		sales_order_1 = dontmanage.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
		...
		sales_order_2 = dontmanage.get_doc(
			{
				"doctype": "Sales Order",
		...
		for dn in dontmanage.get_all(
			"Delivery Note",
			filters={"pick_list": pick_list.name, "customer": "_Test Customer"},
			fields={"name"},
		):
			for dn_item in dontmanage.get_doc("Delivery Note", dn.name).get("items"):
		...
		pick_list_1 = dontmanage.get_doc(
			{
				"doctype": "Pick List",
				"company": "_Test Company",
				"purpose": "Delivery",
		...
		for dn in dontmanage.get_all("Delivery Note", filters={"pick_list": pick_list_1.name}, fields={"name"}):
			for dn_item in dontmanage.get_doc("Delivery Note", dn.name).get("items"):
	...
	def test_validate_picked_qty_with_manual_option(self):
		...
		self.assertRaises(dontmanage.ValidationError, pl.save)
	...
	def test_over_allowance_picking(self):
		...
		self.assertRaises(dontmanage.ValidationError, pl_doc.submit)

		dontmanage.db.set_single_value("Stock Settings", "over_picking_allowance", 50)

		pl_doc.reload()
		pl_doc.submit()

		dontmanage.db.set_single_value("Stock Settings", "over_picking_allowance", 0)

	def test_ignore_pricing_rule_in_pick_list(self):
		dontmanage.flags.print_stmt = False
		warehouse = "_Test Warehouse - _TC"
		item = make_item(
			properties={
				"is_stock_item": 1,
		...
		pricing_rule.save()
		dontmanage.flags.print_stmt = True
