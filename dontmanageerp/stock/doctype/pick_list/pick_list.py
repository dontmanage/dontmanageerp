...
from dontmanage.utils import ceil, cint, floor, flt, get_link_to_form
...
class PickList(Document):
	...
	def check_serial_no_status(self):
		...
		for row in self.get("locations"):
			...
			validated_serial_nos = dontmanage.get_all(
				"Serial No",
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.locations:
			if (
				...
				and dontmanage.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "docstatus")
				== 1
			):
	...
	def set_item_locations(self, save=False):
		...
		updated_locations = dontmanage._dict()
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			if not item.item_code:
				dontmanage.throw(f"Row #{item.idx}: Item Code is Mandatory")
			if not cint(
				dontmanage.get_cached_value("Item", item.item_code, "is_stock_item")
			) and not dontmanage.db.exists("Product Bundle", {"new_item_code": item.item_code, "disabled": 0}):
				continue
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			dontmanage.throw(_("Qty of Finished Goods Item should be greater than 0."))
	...
	def update_bundle_picked_qty(self):
		...
		for so_row, item_code in product_bundles.items():
			...
			already_picked = dontmanage.db.get_value(item_table, so_row, "picked_qty", for_update=True)
			dontmanage.db.set_value(
				item_table,
				so_row,
				"picked_qty",
	...
	def get_picked_items_details(self, items):
		...
		for item_data in items_data:
			...
			if key not in picked_items[item_data.item_code]:
				picked_items[item_data.item_code][key] = dontmanage._dict(
					{
						"picked_qty": 0,
						"serial_no": [],
	...
	def _get_pick_list_items(self, items):
		pi = dontmanage.qb.DocType("Pick List")
		pi_item = dontmanage.qb.DocType("Pick List Item")
		query = (
	...
	def _get_product_bundles(self) -> dict[str, str]:
		...
		for item in self.locations:
			if not item.product_bundle_item:
				continue
			product_bundles[item.product_bundle_item] = dontmanage.db.get_value(
				"Sales Order Item",
	...
	def _get_product_bundle_qty_map(self, bundles: list[str]) -> dict[str, dict[str, float]]:
		...
		for bundle_item_code in bundles:
			bundle = dontmanage.get_last_doc("Product Bundle", {"new_item_code": bundle_item_code, "disabled": 0})
	...
	def _compute_picked_qty_for_bundle(self, bundle_row, bundle_items) -> int:
		...
		precision = dontmanage.get_precision("Stock Ledger Entry", "qty_after_transaction")
...
def update_pick_list_status(pick_list):
	if pick_list:
		doc = dontmanage.get_doc("Pick List", pick_list)
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		dontmanage.throw(_("Add items in the Item Locations table"))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	precision = dontmanage.get_precision("Pick List Item", "qty")
...
def get_available_item_locations(
	...
	has_serial_no = dontmanage.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = dontmanage.get_cached_value("Item", item_code, "has_batch_no")
...
def create_delivery_note(source_name, target_doc=None):
	pick_list = dontmanage.get_doc("Pick List", source_name)
	...
	dontmanage.msgprint(_("Delivery Note(s) created for the Pick List"))
	return delivery_note


def create_dn_wo_so(pick_list):
	delivery_note = dontmanage.new_doc("Delivery Note")
...
def map_pl_locations(pick_list, item_mapper, delivery_note, sales_order=None):
	for location in pick_list.locations:
		...
		if location.sales_order_item:
			sales_order_item = dontmanage.get_doc("Sales Order Item", location.sales_order_item)
		else:
	...
	delivery_note.customer = dontmanage.get_value("Sales Order", sales_order, "customer")
...
def add_product_bundles_to_delivery_note(pick_list: "PickList", delivery_note, item_mapper) -> None:
	...
	for so_row, item_code in product_bundles.items():
		sales_order_item = dontmanage.get_doc("Sales Order Item", so_row)
...
def create_stock_entry(pick_list):
	pick_list = dontmanage.get_doc(json.loads(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return dontmanage.msgprint(_("Stock Entry has been already created against this Pick List"))
...
def get_item_details(item_code, uom=None):
	details = dontmanage.db.get_value("Item", item_code, ["stock_uom", "name"], as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
	cost_center = dontmanage.db.get_value("Project", delivery_note.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return dontmanage.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = dontmanage.get_doc("Work Order", pick_list.get("work_order"))
	...
	is_wip_warehouse_group = dontmanage.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = dontmanage.get_value(
