...
dontmanage.ui.form.on('Pick List', {
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			dontmanage.xcall('dontmanageerp.stock.doctype.pick_list.pick_list.target_document_exists', {
	...
	work_order: (frm) => {
		dontmanage.db.get_value('Work Order',
			frm.doc.work_order,
	...
	create_stock_entry: (frm) => {
		dontmanage.xcall('dontmanageerp.stock.doctype.pick_list.pick_list.create_stock_entry', {
			'pick_list': frm.doc,
		}).then(stock_entry => {
