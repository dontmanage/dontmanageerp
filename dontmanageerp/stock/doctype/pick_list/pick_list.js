...
dontmanage.ui.form.on("Pick List", {
	...
	set_item_locations: (frm, save) => {
		if (!(frm.doc.locations && frm.doc.locations.length)) {
			dontmanage.msgprint(__("Add items in the Item Locations table"));
		} else {
			dontmanage.call({
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			dontmanage
				.xcall("dontmanageerp.stock.doctype.pick_list.pick_list.target_document_exists", {
				...
				dontmanage.prompt(
					...
					(data) => {
						...
						dontmanageerp.utils.map_current_doc({
							method: "dontmanageerp.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery_note: (frm) => {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.pick_list.pick_list.create_delivery_note",
			frm: frm,
		});
	},
	create_stock_entry: (frm) => {
		dontmanage
			.xcall("dontmanageerp.stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
				dontmanage.model.sync(stock_entry);
				dontmanage.set_route("Form", "Stock Entry", stock_entry.name);
			});
	},
...
dontmanage.ui.form.on("Pick List Item", {
	item_code: (frm, cdt, cdn) => {
		let row = dontmanage.get_doc(cdt, cdn);
		if (row.item_code) {
			get_item_details(row.item_code).then((data) => {
				dontmanage.model.set_value(cdt, cdn, "uom", data.stock_uom);
				dontmanage.model.set_value(cdt, cdn, "stock_uom", data.stock_uom);
				dontmanage.model.set_value(cdt, cdn, "conversion_factor", 1);
			});
		}
	},
	...
	uom: (frm, cdt, cdn) => {
		...
		if (row.uom) {
			get_item_details(row.item_code, row.uom).then((data) => {
				dontmanage.model.set_value(cdt, cdn, "conversion_factor", data.conversion_factor);
			});
		}
	},
...
function get_item_details(item_code, uom = null) {
	if (item_code) {
		return dontmanage.xcall("dontmanageerp.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
		});
	}
