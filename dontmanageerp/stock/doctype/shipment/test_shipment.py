...
from dontmanageerp.stock.doctype.delivery_note.delivery_note import make_shipment


class TestShipment(DontManageTestCase):
...
def create_test_shipment(delivery_notes=None):
	...
	shipment = dontmanage.new_doc("Shipment")
...
def get_shipment_customer_contact(customer_name):
	...
	contacts = dontmanage.get_all("Contact", fields=["name"], filters={"name": customer_name})
	if len(contacts):
		return contacts[0]
	else:
...
def get_shipment_customer_address(customer_name):
	...
	customer_address = dontmanage.get_all("Address", fields=["name"], filters={"address_title": address_title})
...
def get_shipment_customer():
	...
	customer = dontmanage.get_all("Customer", fields=["name"], filters={"name": customer_name})
	if len(customer):
		return customer[0]
	else:
...
def get_shipment_company_address(company_name):
	...
	addresses = dontmanage.get_all("Address", fields=["name"], filters={"address_title": address_title})
	if len(addresses):
		return addresses[0]
	else:
...
def get_shipment_company():
	return dontmanage.get_doc("Company", "_Test Company")
...
def create_customer_contact(fname, lname):
	customer = dontmanage.new_doc("Contact")
...
def create_material_receipt(item, company):
	...
	stock = dontmanage.new_doc("Stock Entry")
