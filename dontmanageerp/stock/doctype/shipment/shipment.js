...
dontmanage.ui.form.on('Shipment', {
	...
	onload: function(frm) {
		frm.set_query("delivery_address_name", () => {
			let delivery_to = `delivery_${dontmanage.model.scrub(frm.doc.delivery_to_type)}`;
		...
		frm.set_query("pickup_address_name", () => {
			let pickup_from = `pickup_${dontmanage.model.scrub(frm.doc.pickup_from_type)}`;
	...
	set_pickup_company_address: function(frm) {
		dontmanage.db.get_value('Address', {
	...
	pickup_from_type: function(frm) {
		if (frm.doc.pickup_from_type == 'Company') {
			frm.set_value("pickup_company", dontmanage.defaults.get_default('company'));
	...
	delivery_to_type: function(frm) {
		if (frm.doc.delivery_to_type == 'Company') {
			frm.set_value("delivery_company", dontmanage.defaults.get_default('company'));
	...
	delivery_address_name: function(frm) {
		if (frm.doc.delivery_to_type == 'Company') {
			dontmanageerp.utils.get_address_display(frm, 'delivery_address_name', 'delivery_address', true);
		} else {
			dontmanageerp.utils.get_address_display(frm, 'delivery_address_name', 'delivery_address', false);
		}
	},
	pickup_address_name: function(frm) {
		if (frm.doc.pickup_from_type == 'Company') {
			dontmanageerp.utils.get_address_display(frm, 'pickup_address_name', 'pickup_address', true);
		} else {
			dontmanageerp.utils.get_address_display(frm, 'pickup_address_name', 'pickup_address', false);
		}
	},
	get_contact_display: function(frm, contact_name, contact_type) {
		dontmanage.call({
			method: "dontmanage.contacts.doctype.contact.contact.get_contact_details",
			...
			callback: function(r) {
				if (r.message) {
					if (!(r.message.contact_email && (r.message.contact_phone || r.message.contact_mobile))) {
						...
						dontmanage.throw(__("Email or Phone/Mobile of the Contact are mandatory to continue.")
	...
	pickup_contact_person: function(frm) {
		if (frm.doc.pickup_contact_person) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.shipment.shipment.get_company_contact",
			...
			if (frm.doc.pickup_from_type === 'Company') {
				dontmanage.call({
					method: "dontmanageerp.stock.doctype.shipment.shipment.get_company_contact",
					args: { user: dontmanage.session.user },
					callback: function({ message }) {
						const r = message;
						let contact_display = `${r.first_name} ${r.last_name}`;
						if (r.email) {
	...
	set_company_contact: function(frm, delivery_type) {
		dontmanage.db.get_value('User', { name: dontmanage.session.user }, ['full_name', 'last_name', 'email', 'phone', 'mobile_no'], (r) => {
			if (!(r.last_name && r.email && (r.phone || r.mobile_no))) {
				...
				dontmanage.throw(__("Last Name, Email or Phone/Mobile of the user are mandatory to continue.") + "</br>"
					+ __("Please first set Last Name, Email and Phone for the user")
					+ ` <a href="/app/user/${dontmanage.session.user}">${dontmanage.session.user}</a>`);
			}
		...
		frm.set_value('pickup_contact_person', dontmanage.session.user);
	},
	...
	set_address_name: function(frm, ref_doctype, ref_docname, delivery_type) {
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.shipment.shipment.get_address_name",
			args: {
	...
	set_contact_name: function(frm, ref_doctype, ref_docname, delivery_type) {
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.shipment.shipment.get_contact_name",
			args: {
				ref_doctype: ref_doctype,
				docname: ref_docname
			},
	...
	add_template: function(frm) {
		if (frm.doc.parcel_template) {
			dontmanage.model.with_doc("Shipment Parcel Template", frm.doc.parcel_template, () => {
				let parcel_template = dontmanage.model.get_doc("Shipment Parcel Template", frm.doc.parcel_template);
				let row = dontmanage.model.add_child(frm.doc, "Shipment Parcel", "shipment_parcel");
	...
	pickup_date: function(frm) {
		if (frm.doc.pickup_date < dontmanage.datetime.get_today()) {
			dontmanage.throw(__("Pickup Date cannot be before this day"));
		}
	},
...
dontmanage.ui.form.on('Shipment Delivery Note', {
	delivery_note: function(frm, cdt, cdn) {
		...
		if (row.delivery_note) {
			...
			if (validate_duplicate(frm, 'shipment_delivery_note', row.delivery_note, row_index)) {
				dontmanage.throw(__("You have entered a duplicate Delivery Note on Row") + ` ${row.idx}. ` + __("Please rectify and try again."));
			}
		}
	},
