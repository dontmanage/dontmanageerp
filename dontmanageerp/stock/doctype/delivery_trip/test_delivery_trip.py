...
from dontmanage.utils import add_days, flt, now_datetime, nowdate

import dontmanageerp
from dontmanageerp.stock.doctype.delivery_trip.delivery_trip import (
...
class TestDeliveryTrip(DontManageTestCase):
	def setUp(self):
		super().setUp()
	...
	def tearDown(self):
		dontmanage.db.sql("delete from `tabDriver`")
		dontmanage.db.sql("delete from `tabVehicle`")
		dontmanage.db.sql("delete from `tabEmail Template`")
		dontmanage.db.sql("delete from `tabDelivery Trip`")
...
def create_address(driver):
	if not dontmanage.db.exists("Address", {"address_title": "_Test Address for Driver"}):
		...
		dontmanage.db.set_value("Driver", driver.name, "address", address.name)

		return address

	return dontmanage.get_doc("Address", {"address_title": "_Test Address for Driver"})


def create_driver():
	if not dontmanage.db.exists("Driver", {"full_name": "Newton Scmander"}):
		driver = dontmanage.get_doc(
			{
				"doctype": "Driver",
	...
	return dontmanage.get_doc("Driver", {"full_name": "Newton Scmander"})


def create_delivery_notification():
	if not dontmanage.db.exists("Email Template", "Delivery Notification"):
		dispatch_template = dontmanage.get_doc(
			{
	...
	delivery_settings = dontmanage.get_single("Delivery Settings")
	delivery_settings.dispatch_template = "Delivery Notification"
	delivery_settings.save()


def create_vehicle():
	if not dontmanage.db.exists("Vehicle", "JB 007"):
...
def create_delivery_trip(driver, address, contact=None):
	...
	delivery_trip = dontmanage.get_doc(
		{
