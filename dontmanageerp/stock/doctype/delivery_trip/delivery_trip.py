...
class DeliveryTrip(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.delivery_stop.delivery_stop import DeliveryStop
	...
	def __init__(self, *args, **kwargs):
		...
		self.default_distance_uom = (
			dontmanage.db.get_single_value("Global Defaults", "default_distance_unit") or "Meter"
		)
		self.uom_conversion_factor = dontmanage.db.get_value(
			"UOM Conversion Factor", {"from_uom": "Meter", "to_uom": self.default_distance_uom}, "value"
		)

	def validate(self):
		if self._action == "submit" and not self.driver:
			dontmanage.throw(_("A driver must be set to submit."))
	...
	def validate_stop_addresses(self):
		for stop in self.delivery_stops:
			if not stop.customer_address:
				stop.customer_address = get_address_display(dontmanage.get_doc("Address", stop.address).as_dict())
	...
	def update_delivery_notes(self, delete=False):
		...
		for delivery_note in delivery_notes:
			note_doc = dontmanage.get_doc("Delivery Note", delivery_note)
		...
		dontmanage.msgprint(_("Delivery Notes {0} updated").format(", ".join(delivery_notes)))
		...
		for route in route_list:
			...
			if directions:
				...
				for leg in legs:
					...
					stop_delay = dontmanage.db.get_single_value("Delivery Settings", "stop_delay")
	...
	def form_route_list(self, optimize):
		...
		if not self.driver_address:
			dontmanage.throw(_("Cannot Calculate Arrival Time as Driver Address is Missing."))

		home_address = get_address_display(dontmanage.get_doc("Address", self.driver_address).as_dict())
	...
	def get_directions(self, route, optimize):
		...
		"""
		if not dontmanage.db.get_single_value("Google Settings", "api_key"):
			dontmanage.throw(_("Enter API key in Google Settings."))

		import googlemaps

		try:
			maps_client = googlemaps.Client(key=dontmanage.db.get_single_value("Google Settings", "api_key"))
...
def get_default_contact(out, name):
	contact_persons = dontmanage.db.sql(
		"""
			SELECT parent,
...
def get_default_address(out, name):
	shipping_addresses = dontmanage.db.sql(
		"""
			SELECT parent,
...
def get_contact_display(contact):
	contact_info = dontmanage.db.get_value(
...
def notify_customers(delivery_trip):
	delivery_trip = dontmanage.get_doc("Delivery Trip", delivery_trip)

	context = delivery_trip.as_dict()

	if delivery_trip.driver:
		context.update(dontmanage.db.get_value("Driver", delivery_trip.driver, "cell_number", as_dict=1))
	...
	for stop in delivery_trip.delivery_stops:
		...
		if contact_info and contact_info.email_id:
			...
			dispatch_template_name = dontmanage.db.get_single_value("Delivery Settings", "dispatch_template")
			dispatch_template = dontmanage.get_doc("Email Template", dispatch_template_name)

			dontmanage.sendmail(
				recipients=contact_info.email_id,
				subject=dispatch_template.subject,
				message=dontmanage.render_template(dispatch_template.response, context),
	...
	if email_recipients:
		dontmanage.msgprint(_("Email sent to {0}").format(", ".join(email_recipients)))
		delivery_trip.db_set("email_notification_sent", 1)
	else:
		dontmanage.msgprint(_("No contacts with email IDs found."))


def get_attachments(delivery_stop):
	if not (
		dontmanage.db.get_single_value("Delivery Settings", "send_with_attachment")
		and delivery_stop.delivery_note
	):
		return []

	dispatch_attachment = dontmanage.db.get_single_value("Delivery Settings", "dispatch_attachment")
	attachments = dontmanage.attach_print(
		"Delivery Note",
...
def get_driver_email(driver):
	employee = dontmanage.db.get_value("Driver", driver, "employee")
	email = dontmanage.db.get_value("Employee", employee, "prefered_email")
	return {"email": email}
