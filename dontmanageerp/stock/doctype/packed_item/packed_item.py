...
from dontmanageerp.stock.get_item_details import get_item_details, get_price_list_rate
...
def make_packing_list(doc):
	...
	set_price_from_children = dontmanage.db.get_single_value("Selling Settings", "editable_bundle_item_rates")
...
def is_product_bundle(item_code: str) -> bool:
	return bool(dontmanage.db.exists("Product Bundle", {"new_item_code": item_code, "disabled": 0}))
...
def get_product_bundle_items(item_code):
	...
	product_bundle_item = dontmanage.qb.DocType("Product Bundle Item")

	query = (
		dontmanage.qb.from_(product_bundle_item)
		.join(product_bundle)
...
def update_packed_item_stock_data(main_item_row, pi_row, packing_item, item_data, doc):
	...
	pi_row.use_serial_batch_fields = dontmanage.db.get_single_value("Stock Settings", "use_serial_batch_fields")
...
def update_packed_item_price_data(pi_row, item_data, doc):
	...
	item_doc = dontmanage.get_cached_doc("Item", pi_row.item_code)
...
def get_packed_item_bin_qty(item, warehouse):
	bin_data = dontmanage.db.get_values(
		"Bin",
...
def on_doctype_update():
	dontmanage.db.add_index("Packed Item", ["item_code", "warehouse"])
