...
import dontmanage
from dontmanage import _, _dict, bold
...
from dontmanageerp.stock.serial_batch_bundle import get_serial_nos as get_serial_nos_from_bundle


class SerialNoExistsInFutureTransactionError(dontmanage.ValidationError):
	pass


class BatchNegativeStockError(dontmanage.ValidationError):
	pass


class SerialNoDuplicateError(dontmanage.ValidationError):
	pass


class SerialNoWarehouseError(dontmanage.ValidationError):
	pass
...
class SerialandBatchBundle(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.serial_and_batch_entry.serial_and_batch_entry import (
			SerialandBatchEntry,
		)
	...
	def validate(self):
		...
		if not self.flags.ignore_validate_serial_batch or dontmanage.flags.in_test:
	...
	def allow_existing_serial_nos(self):
		...
		if dontmanage.db.get_single_value("Stock Settings", "allow_existing_serial_no"):
			return

		if self.voucher_type not in ["Purchase Receipt", "Purchase Invoice", "Stock Entry"]:
			return

		if self.voucher_type == "Stock Entry" and dontmanage.get_cached_value(
	...
	def set_batch_no(self):
		if self.has_serial_no and self.has_batch_no:
			...
			serial_no_batch = dontmanage._dict(
				dontmanage.get_all(
					"Serial No",
	...
	def validate_serial_nos_inventory(self):
		...
		available_serial_nos = get_available_serial_nos(dontmanage._dict(kwargs))
	...
	def throw_error_message(self, message, exception=dontmanage.ValidationError):
		dontmanage.throw(_(message), exception, title=_("Error"))
	...
	def get_valuation_rate_for_return_entry(self, return_against):
		valuation_details = dontmanage._dict(
			{
	...
	def validate_negative_batch(self, batch_no, available_qty):
		if available_qty < 0 and not self.is_stock_reco_for_valuation_adjustment(available_qty):
			...
			dontmanage.throw(_(msg), BatchNegativeStockError)
	...
	def is_stock_reco_for_valuation_adjustment(self, available_qty):
		if (
			...
			and abs(dontmanage.db.get_value("Stock Reconciliation Item", self.voucher_detail_no, "qty"))
			== abs(available_qty)
		):
			return True
	...
	def get_return_against(self, parent=None):
		...
		if (
			...
			voucher_details = dontmanage.db.get_value(
	...
	def set_incoming_rate_for_inward_transaction(self, row=None, save=False):
		...
		if self.voucher_type == "Subcontracting Receipt":
			if not self.voucher_detail_no:
				return
			elif dontmanage.db.exists("Subcontracting Receipt Supplied Item", self.voucher_detail_no):
				valuation_field = "rate"
		...
		if not rate and self.voucher_detail_no and self.voucher_no:
			rate = dontmanage.db.get_value(child_table, self.voucher_detail_no, valuation_field)
	...
	def set_serial_and_batch_values(self, parent, row, qty_field=None):
		...
		if parent.doctype in [
			...
			return_ref_field = dontmanage.scrub(parent.doctype) + "_item"
	...
	def validate_voucher_no(self):
		...
		if (
			self.docstatus == 1
			and self.voucher_no
			and not dontmanage.db.exists(self.voucher_type, self.voucher_no)
		):
		...
		if (
			self.docstatus == 1
			and dontmanage.get_cached_value(self.voucher_type, self.voucher_no, "docstatus") != 1
		):
	...
	def check_future_entries_exists(self):
		...
		parent = dontmanage.qb.DocType("Serial and Batch Bundle")
		child = dontmanage.qb.DocType("Serial and Batch Entry")
		...
		if future_entries:
			...
			dontmanage.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransactionError)
	...
	def validate_quantity(self, row, qty_field=None):
		...
		if abs(abs(flt(self.total_qty, precision)) - abs(flt(qty, precision))) > 0.01:
			total_qty = dontmanage.format_value(abs(flt(self.total_qty)), "Float", row)
			set_qty = dontmanage.format_value(abs(flt(row.get(qty_field))), "Float", row)
			self.throw_error_message(
	...
	def calculate_outgoing_rate(self):
		...
		if self.voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
			return dontmanage.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type in ["Sales Invoice", "Delivery Note"]:
			return not dontmanage.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type == "Stock Entry":
			return dontmanage.get_cached_value(self.voucher_type, self.voucher_no, "purpose") in [
				"Material Receipt"
			]
	...
	def validate_serial_batch_no(self, serial_batches):
		correct_batches = dontmanage._dict(
			dontmanage.get_all(
				"Serial No",
	...
	def validate_incorrect_serial_nos(self, serial_nos):
		incorrect_serial_nos = dontmanage.get_all(
			"Serial No",
	...
	def validate_incorrect_batch_nos(self, batch_nos):
		incorrect_batch_nos = dontmanage.get_all(
		...
		for d in data:
			if self.has_serial_no:
				...
				if d.serial_and_batch_bundle:
					...
					batches = dontmanage._dict({d.batch_no: d.stock_qty})
	...
	def get_orignal_document_data(self):
		...
		return dontmanage.get_all(child_doc, fields=fields, filters={"name": self.returned_against})
	...
	def delink_serial_and_batch_bundle(self):
		sles = dontmanage.get_all("Stock Ledger Entry", filters={"serial_and_batch_bundle": self.name})

		for sle in sles:
			dontmanage.db.set_value("Stock Ledger Entry", sle.name, "serial_and_batch_bundle", None)
	...
	def delink_refernce_from_voucher(self):
		...
		if (
			...
			and self.voucher_detail_no
			and not dontmanage.db.exists("Subcontracting Receipt Item", self.voucher_detail_no)
		):
			self.voucher_type = "Subcontracting Receipt Supplied"

		vouchers = dontmanage.get_all(
			self.child_table,
			fields=fields,
			filters={"docstatus": 0},
		...
		if not vouchers and self.voucher_type == "Delivery Note":
			dontmanage.db.set_value("Packed Item", self.voucher_detail_no, "serial_and_batch_bundle", None)
			return

		for voucher in vouchers:
			if voucher.get("current_serial_and_batch_bundle"):
				dontmanage.db.set_value(self.child_table, voucher.name, "current_serial_and_batch_bundle", None)
			elif voucher.get("rejected_serial_and_batch_bundle"):
				dontmanage.db.set_value(self.child_table, voucher.name, "rejected_serial_and_batch_bundle", None)

			dontmanage.db.set_value(self.child_table, voucher.name, "serial_and_batch_bundle", None)
	...
	def delink_reference_from_batch(self):
		...
		for batch in batches:
			dontmanage.db.set_value("Batch", batch.name, {"reference_name": None, "reference_doctype": None})

	def validate_serial_and_batch_data(self):
		if not self.voucher_no:
			dontmanage.throw(_("Voucher No is mandatory"))
	...
	def validate_batch_inventory(self):
		if (
			...
			and dontmanage.db.get_value(self.voucher_type, self.voucher_no, "docstatus") == 1
		):
			return
		...
		if (
			self.voucher_type == "Stock Reconciliation"
			and self.type_of_transaction == "Outward"
			and dontmanage.db.get_value("Stock Reconciliation Item", self.voucher_detail_no, "qty") > 0
		):
			return
	...
	def validate_voucher_no_docstatus(self):
		...
		if dontmanage.db.get_value(self.voucher_type, self.voucher_no, "docstatus") == 1:
	...
	def delete_serial_batch_entries(self):
		SBBE = dontmanage.qb.DocType("Serial and Batch Entry")

		dontmanage.qb.from_(SBBE).delete().where(SBBE.parent == self.name).run()
...
def get_serial_batch_from_csv(item_code, file_path):
	if "private" in file_path:
		file_path = dontmanage.get_site_path() + file_path
	else:
		file_path = dontmanage.get_site_path() + "/public" + file_path
...
def make_serial_nos(item_code, serial_nos):
	item = dontmanage.get_cached_value(
	...
	existing_serial_nos = dontmanage.get_all("Serial No", filters={"name": ("in", serial_nos)})
	...
	dontmanage.msgprint(_("Serial Nos are created successfully"), alert=True)


def make_batch_nos(item_code, batch_nos):
	item = dontmanage.get_cached_value("Item", item_code, ["description", "item_code"], as_dict=1)
	batch_nos = [d.get("batch_no") for d in batch_nos if d.get("batch_no")]

	existing_batches = dontmanage.get_all("Batch", filters={"name": ("in", batch_nos)})
	...
	for batch_no in batch_nos:
		if dontmanage.db.exists("Batch", batch_no):
			continue
	...
	dontmanage.db.bulk_insert("Batch", fields=fields, values=set(batch_nos_details))

	dontmanage.msgprint(_("Batch Nos are created successfully"), alert=True)
...
def get_reference_serial_and_batch_bundle(child_row):
	...
	if field:
		return dontmanage.get_cached_value(child_row.doctype, child_row.get(field), "serial_and_batch_bundle")


@dontmanage.whitelist()
def add_serial_batch_ledgers(entries, child_row, doc, warehouse, do_not_save=False) -> object:
	if isinstance(child_row, str):
		child_row = dontmanage._dict(parse_json(child_row))
	...
	if dontmanage.db.exists("Serial and Batch Bundle", bundle):
...
def create_serial_batch_no_ledgers(
	...
	if (
		...
		and dontmanage.get_cached_value("Item", child_row.item_code, "has_batch_no")
	):
	...
	if do_not_save:
		dontmanage.db.set_value(child_row.doctype, child_row.name, "serial_and_batch_bundle", doc.name)

	dontmanage.msgprint(_("Serial and Batch Bundle created"), alert=True)
...
def update_serial_batch_no_ledgers(bundle, entries, child_row, parent_doc, warehouse=None) -> object:
	doc = dontmanage.get_doc("Serial and Batch Bundle", bundle)
	...
	dontmanage.msgprint(_("Serial and Batch Bundle updated"), alert=True)
...
def update_serial_or_batch(bundle_id, serial_no=None, batch_no=None):
	if batch_no and not serial_no:
		if qty := dontmanage.db.get_value(
	...
	doc = dontmanage.get_cached_doc("Serial and Batch Bundle", bundle_id)
...
def get_serial_and_batch_ledger(**kwargs):
	...
	sle_table = dontmanage.qb.DocType("Stock Ledger Entry")
	serial_batch_table = dontmanage.qb.DocType("Serial and Batch Entry")

	query = (
		dontmanage.qb.from_(sle_table)
...
def get_reserved_serial_nos_for_pos(kwargs):
	from dontmanageerp.controllers.sales_and_purchase_return import get_returned_serial_nos
	from dontmanageerp.stock.doctype.serial_no.serial_no import get_serial_nos

	ignore_serial_nos = []
	pos_invoices = dontmanage.get_all(
		"POS Invoice",
		fields=[
...
def get_reserved_serial_nos_for_sre(kwargs) -> list:
	...
	sb_entry = dontmanage.qb.DocType("Serial and Batch Entry")
	query = (
		dontmanage.qb.from_(sre)
		.inner_join(sb_entry)
	...
	if ids:
		for d in get_serial_batch_ledgers(kwargs.item_code, docstatus=1, name=ids):
			...
			if key not in pos_batches:
				pos_batches[key] = dontmanage._dict(
					{
						"qty": d.qty,
...
def get_reserved_batches_for_sre(kwargs) -> dict:
	...
	reserved_batches_details = dontmanage._dict()
	if data:
		reserved_batches_details = dontmanage._dict(
			{(d.batch_no, d.warehouse): dontmanage._dict({"warehouse": d.warehouse, "qty": d.qty}) for d in data}
		)
...
def get_auto_batch_nos(kwargs):
	...
	picked_batches = dontmanage._dict()
...
def get_available_batches(kwargs):
	from dontmanageerp.stock.utils import get_combine_datetime

	stock_ledger_entry = dontmanage.qb.DocType("Stock Ledger Entry")
	batch_ledger = dontmanage.qb.DocType("Serial and Batch Entry")
	...
	if kwargs.based_on == "LIFO":
		query = query.orderby(batch_table.creation, order=dontmanage.qb.desc)
	elif kwargs.based_on == "Expiry":
...
def get_voucher_wise_serial_batch_from_bundle(**kwargs) -> dict[str, dict]:
	...
	for row in data:
		...
		if kwargs.get("get_subcontracted_item"):
			...
			subcontracted_item_code = dontmanage.get_cached_value(doctype, row.voucher_detail_no, field_name)
		...
		if key not in group_by_voucher:
			group_by_voucher.setdefault(
				key,
				dontmanage._dict({"serial_nos": [], "batch_nos": defaultdict(float), "item_row": row}),
			)
...
def get_picked_batches(kwargs) -> dict[str, dict]:
	...
	table = dontmanage.qb.DocType("Serial and Batch Bundle")
	child_table = dontmanage.qb.DocType("Serial and Batch Entry")
	pick_list_table = dontmanage.qb.DocType("Pick List")
	...
	for row in data:
		...
		if key not in picked_batches:
			picked_batches[key] = dontmanage._dict(
				{
					"qty": row.qty,
					"warehouse": row.warehouse,
				}
...
def get_ledgers_from_serial_batch_bundle(**kwargs) -> list[dontmanage._dict]:
	bundle_table = dontmanage.qb.DocType("Serial and Batch Bundle")
	serial_batch_table = dontmanage.qb.DocType("Serial and Batch Entry")

	query = (
		dontmanage.qb.from_(bundle_table)
		.inner_join(serial_batch_table)
...
def get_batch_no_from_serial_no(serial_no):
	return dontmanage.get_cached_value("Serial No", serial_no, "batch_no")


@dontmanage.whitelist()
def is_serial_batch_no_exists(item_code, type_of_transaction, serial_no=None, batch_no=None):
	if serial_no and not dontmanage.db.exists("Serial No", serial_no):
		if type_of_transaction != "Inward":
			dontmanage.throw(_("Serial No {0} does not exists").format(serial_no))

		make_serial_no(serial_no, item_code)

	if batch_no and not dontmanage.db.exists("Batch", batch_no):
		if type_of_transaction != "Inward":
			dontmanage.throw(_("Batch No {0} does not exists").format(batch_no))

		make_batch_no(batch_no, item_code)


def make_serial_no(serial_no, item_code):
	serial_no_doc = dontmanage.new_doc("Serial No")
...
def make_batch_no(batch_no, item_code):
	batch_doc = dontmanage.new_doc("Batch")
...
def is_duplicate_serial_no(bundle_id, serial_no):
	return dontmanage.db.exists("Serial and Batch Entry", {"parent": bundle_id, "serial_no": serial_no})
