...
from dontmanage.utils import flt, nowtime, today
...
class TestSerialandBatchBundle(DontManageTestCase):
		...
		bundle_doc = make_serial_batch_bundle(
			{
				...
				"posting_time": nowtime(),
				"qty": -10,
				"batches": dontmanage._dict({batch_id: 10}),
		...
		bundle_doc = make_serial_batch_bundle(
			{
				...
				"posting_time": nowtime(),
				"qty": -20,
				"batches": dontmanage._dict({batch_id: 20}),
				"type_of_transaction": "Outward",
				"do_not_submit": True,
			}
		)
	...
	def test_old_serial_no_valuation(self):
		...
		if not dontmanage.db.exists("Serial No", serial_no_id):
	...
	def test_batch_not_belong_to_serial_no(self):
		...
		self.assertRaises(dontmanage.exceptions.ValidationError, doc.save)
	...
	def test_auto_delete_draft_serial_and_batch_bundle(self):
		...
		ste.delete()
		self.assertFalse(dontmanage.db.exists("Serial and Batch Bundle", bundle_doc.name))
	...
	def test_serial_and_batch_bundle_company(self):
		...
		for serial_no in ["TT-SER-VAL-00001", "TT-SER-VAL-00002", "TT-SER-VAL-00003"]:
			entries.append(dontmanage._dict({"serial_no": serial_no, "qty": 1}))
	...
	def test_auto_cancel_serial_and_batch(self):
		...
		docstatus = dontmanage.db.get_value("Serial and Batch Bundle", bundle, "docstatus")
	...
	def test_batch_duplicate_entry(self):
		...
		self.assertTrue(dontmanage.db.exists("Batch", batch_id))
		use_batchwise_valuation = dontmanage.db.get_value("Batch", batch_id, "use_batchwise_valuation")
	...
	def test_serial_no_duplicate_entry(self):
		...
		self.assertTrue(dontmanage.db.exists("Serial No", serial_no_id))
	...
	def test_duplicate_serial_and_batch_bundle(self):
		...
		self.assertRaises(dontmanage.exceptions.ValidationError, pr2.save)
	...
	def test_auto_create_serial_and_batch_bundle_for_outward_for_batch_item(self):
		...
		if not dontmanage.db.exists("Batch", "ACSBBO-TACSB-00001"):
			dontmanage.get_doc(
				{
					"doctype": "Batch",
