...
dontmanage.ui.form.on('Serial and Batch Bundle', {
	setup(frm) {
	...
	validate_prompt_data(frm, prompt_data) {
		if (prompt_data.using_csv_file && !prompt_data.csv_file) {
			dontmanage.throw(__("Please attach CSV file"));
		}
		...
		if (frm.doc.has_serial_no) {
			frm.doc.entries.forEach(row => {
				if (Math.abs(row.qty) !== 1) {
					dontmanage.model.set_value(row.doctype, row.name, "qty", 1);
				}
			})
		}
	...
	set_queries(frm) {
		frm.set_query('item_code', () => {
			return {
				query: 'dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.item_query',
			};
		});
...
dontmanage.ui.form.on("Serial and Batch Entry", {
	entries_add(frm, cdt, cdn) {
		if (frm.doc.warehouse) {
			dontmanage.model.set_value(cdt, cdn, 'warehouse', frm.doc.warehouse);
		}
	},
})