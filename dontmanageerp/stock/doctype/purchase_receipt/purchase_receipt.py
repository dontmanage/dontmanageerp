...
class PurchaseReceipt(BuyingController):
		...
		for d in self.get("items"):
			if d.item_code in stock_items and flt(d.valuation_rate) and flt(d.qty):
				if warehouse_account.get(d.warehouse):
					stock_value_diff = dontmanage.db.get_value(
						"Stock Ledger Entry",
						{
					...
					if divisional_loss:
						...
						cost_center = d.cost_center or dontmanage.get_cached_value(
	...
	def update_assets(self, item, valuation_rate):
		...
		for asset in assets:
			dontmanage.db.set_value("Asset", asset.name, "gross_purchase_amount", flt(valuation_rate))
			dontmanage.db.set_value("Asset", asset.name, "purchase_receipt_amount", flt(valuation_rate))
...
def on_doctype_update():
	dontmanage.db.add_index("Purchase Receipt", ["supplier", "is_return", "return_against"])
