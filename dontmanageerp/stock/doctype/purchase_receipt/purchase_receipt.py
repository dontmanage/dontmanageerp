...
from dontmanage.utils import cint, flt, get_datetime, getdate, nowdate
...
from dontmanageerp.controllers.accounts_controller import merge_taxes
from dontmanageerp.controllers.buying_controller import BuyingController
from dontmanageerp.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
...
class PurchaseReceipt(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.purchase_receipt_item.purchase_receipt_item import PurchaseReceiptItem
	...
	def validate_provisional_expense_account(self):
		provisional_accounting_for_non_stock_items = cint(
			dontmanage.db.get_value("Company", self.company, "enable_provisional_accounting_for_non_stock_items")
		)
		...
		if dontmanage.db.get_value("Buying Settings", None, "po_required") == "Yes":
			for d in self.get("items"):
				if not d.purchase_order:
					dontmanage.throw(_("Purchase Order number required for Item {0}").format(d.item_code))

	def validate_items_quality_inspection(self):
		for item in self.get("items"):
			if item.quality_inspection:
				qi = dontmanage.db.get_value(
					"Quality Inspection",
				...
				if qi.reference_type != self.doctype or qi.reference_name != self.name:
					msg = f"""Row #{item.idx}: Please select a valid Quality Inspection with Reference Type
						{dontmanage.bold(self.doctype)} and Reference Name {dontmanage.bold(self.name)}."""
	...
	def get_po_qty_and_warehouse(self, po_detail):
		po_qty, po_warehouse = dontmanage.db.get_value("Purchase Order Item", po_detail, ["qty", "warehouse"])
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			dontmanage.throw(_("Purchase Invoice {0} is already submitted").format(self.submit_rv[0][0]))
	...
	def on_cancel(self):
		...
		if submitted:
			dontmanage.throw(_("Purchase Invoice {0} is already submitted").format(submitted[0][0]))
	...
	def make_item_gl_entries(self, gl_entries, warehouse_account=None):
		...
		def validate_account(account_type):
			dontmanage.throw(_("{0} account not found while submitting purchase receipt").format(account_type))
		...
		def make_divisional_loss_gl_entry(item, outgoing_amount):
			...
			if divisional_loss:
				...
				cost_center = item.cost_center or dontmanage.get_cached_value(
		...
		for d in self.get("items"):
			if (
				...
				if not (
					(dontmanageerp.is_perpetual_inventory_enabled(self.company) and d.item_code in stock_items)
	...
	def add_provisional_gl_entry(
		...
		if reverse:
			...
			amount = item_amount
			expense_account = dontmanage.db.get_value(
	...
	def update_billing_status(self, update_modified=True):
		...
		for pr in set(updated_pr):
			pr_doc = self if (pr == self.name) else dontmanage.get_doc("Purchase Receipt", pr)
			update_billing_percentage(pr_doc, update_modified=update_modified)

	def reserve_stock_for_sales_order(self):
		if (
			self.is_return
			or not dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation")
			or not dontmanage.db.get_single_value(
	...
	def enable_recalculate_rate_in_sles(self):
		rejected_warehouses = dontmanage.get_all(
		...
		(
			dontmanage.qb.update(sle_table)
...
def get_purchase_receipts_against_po_details(po_details):
	...
	purchase_receipt = dontmanage.qb.DocType("Purchase Receipt")
	purchase_receipt_item = dontmanage.qb.DocType("Purchase Receipt Item")

	query = (
		dontmanage.qb.from_(purchase_receipt)
...
def get_billed_amount_against_pr(pr_items):
	...
	purchase_invoice_item = dontmanage.qb.DocType("Purchase Invoice Item")

	query = (
		dontmanage.qb.from_(purchase_invoice_item)
...
def make_purchase_invoice(source_name, target_doc=None, args=None):
	from dontmanageerp.accounts.party import get_payment_terms_template

	doc = dontmanage.get_doc("Purchase Receipt", source_name)
	returned_qty_map = get_returned_qty_map(source_name)
	invoiced_qty_map = get_invoiced_qty_map(source_name)

	def set_missing_values(source, target):
		if len(target.get("items")) == 0:
			dontmanage.throw(_("All items have already been Invoiced/Returned"))

		doc = dontmanage.get_doc(target)
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if dontmanage.db.get_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice"):
...
def get_invoiced_qty_map(purchase_receipt):
	...
	for pr_detail, qty in dontmanage.db.sql(
...
def update_purchase_receipt_status(docname, status):
	pr = dontmanage.get_doc("Purchase Receipt", docname)
...
def get_item_account_wise_additional_cost(purchase_document):
	landed_cost_vouchers = dontmanage.get_all(
	...
	for lcv in landed_cost_vouchers:
		landed_cost_voucher_doc = dontmanage.get_doc("Landed Cost Voucher", lcv.parent)

		based_on_field = None
		# Use amount field for total item cost for manually cost distributed LCVs
		if landed_cost_voucher_doc.distribute_charges_based_on != "Distribute Manually":
			based_on_field = dontmanage.scrub(landed_cost_voucher_doc.distribute_charges_based_on)
...
def make_lcv(doctype, docname):
	landed_cost_voucher = dontmanage.new_doc("Landed Cost Voucher")

	details = dontmanage.db.get_value(doctype, docname, ["supplier", "company", "base_grand_total"], as_dict=1)
