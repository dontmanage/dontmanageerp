...
from dontmanage.utils import add_days, cint, cstr, flt, get_datetime, getdate, nowtime, today
...
from dontmanageerp.controllers.buying_controller import QtyMismatchError
...
from dontmanageerp.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice
...
class TestPurchaseReceipt(DontManageTestCase):
	def setUp(self):
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		sl_entry = dontmanage.db.get_all(
			"Stock Ledger Entry",
		...
		sl_entry_cancelled = dontmanage.db.get_all(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": pr.name},
			["actual_qty"],
			order_by="creation",
	...
	def test_make_purchase_invoice(self):
		from dontmanageerp.accounts.doctype.payment_entry.test_payment_entry import create_payment_term
		...
		template = dontmanage.db.get_value(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		)
		old_template_in_supplier = dontmanage.db.get_value("Supplier", "_Test Supplier", "payment_terms")
		dontmanage.db.set_value("Supplier", "_Test Supplier", "payment_terms", template)

		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(dontmanage.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(dontmanage.ValidationError, dontmanage.get_doc(pi).submit)
		...
		pi.delete()  # draft PI
		pr.cancel()
		dontmanage.db.set_value("Supplier", "_Test Supplier", "payment_terms", old_template_in_supplier)
		dontmanage.get_doc("Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice").delete()

	def test_purchase_receipt_no_gl_entry(self):
		from dontmanageerp.stock.doctype.stock_entry.test_stock_entry import make_stock_entry

		existing_bin_qty, existing_bin_stock_value = dontmanage.db.get_value(
			"Bin",
		...
		stock_value_difference = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
		...
		current_bin_stock_value = dontmanage.db.get_value(
	...
	def test_batched_serial_no_purchase(self):
		item = dontmanage.db.exists("Item", {"item_name": "Batched Serialized Item"})
		if not item:
			...
			item = dontmanage.get_doc("Item", {"item_name": "Batched Serialized Item"})

		pr = make_purchase_receipt(item_code=item.name, qty=5, rate=500)

		self.assertTrue(dontmanage.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))
	...
	def test_duplicate_serial_nos(self):
		...
		item = dontmanage.db.exists("Item", {"item_name": "Test Serialized Item 123"})
		if not item:
			...
			item = dontmanage.get_doc("Item", {"item_name": "Test Serialized Item 123"})

		# First make purchase receipt
		pr = make_purchase_receipt(item_code=item.name, qty=2, rate=500)
		pr.load_from_db()

		bundle_id = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(dontmanageerp.is_perpetual_inventory_enabled(pr.company)), 1)
	...
	def test_serial_no_warehouse(self):
		...
		self.assertEqual(
			dontmanage.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"), pr.get("items")[0].warehouse
		)

		pr.cancel()
		self.assertFalse(dontmanage.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"))

	def test_rejected_warehouse_filter(self):
		pr = dontmanage.copy_doc(test_records[0])
		...
		self.assertRaises(dontmanage.ValidationError, pr.save)
	...
	def test_rejected_serial_no(self):
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(
				dontmanage.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].warehouse
			)
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = dontmanage.db.get_value(
			"Stock Ledger Entry",
		...
		returned = dontmanage.get_doc("Purchase Receipt", return_pr.name)
		...
		buying_settings = dontmanage.get_single("Buying Settings")
	...
	def test_purchase_return_for_rejected_qty(self):
		from dontmanageerp.stock.doctype.warehouse.test_warehouse import get_warehouse
		...
		actual_qty = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
	...
	def test_closed_purchase_receipt(self):
		...
		self.assertEqual(dontmanage.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_pr_billing_status(self):
		...
		3. PO -> PR2.
		"""
		from dontmanageerp.buying.doctype.purchase_order.purchase_order import (
	...
	def test_serial_no_against_purchase_receipt(self):
		...
		if not dontmanage.db.exists("Serial No", serial_no[0]):
			dontmanage.get_doc(
		...
		voucher_no = dontmanage.db.get_value("Serial and Batch Bundle", bundle_id, "voucher_no")
	...
	def test_auto_asset_creation(self):
		...
		if not dontmanage.db.exists("Item", asset_item):
			asset_category = dontmanage.get_all("Asset Category")
		...
		assets = dontmanage.db.get_all("Asset", filters={"purchase_receipt": pr.name})

		self.assertEqual(len(assets), 3)

		location = dontmanage.db.get_value("Asset", assets[0].name, "location")
		self.assertEqual(location, "Test Location")

		pr.cancel()

	def test_purchase_return_with_submitted_asset(self):
		from dontmanageerp.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return

		pr = make_purchase_receipt(item_code="Test Asset Item", qty=1)

		asset = dontmanage.get_doc("Asset", {"purchase_receipt": pr.name})
		asset.available_for_use_date = dontmanage.utils.nowdate()
		...
		self.assertRaises(dontmanage.exceptions.ValidationError, pr_return.submit)
	...
	def test_purchase_receipt_cost_center(self):
		...
		if not dontmanage.db.exists("Location", "Test Location"):
			dontmanage.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()
	...
	def test_make_purchase_invoice_from_pr_for_returned_qty(self):
		...
		dontmanage.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 0)
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		"""
		from dontmanageerp.buying.doctype.purchase_order import purchase_order, test_purchase_order
	...
	def test_purchase_receipt_with_exchange_rate_difference(self):
		from dontmanageerp.accounts.doctype.purchase_invoice.purchase_invoice import (
		...
		from dontmanageerp.accounts.party import add_party_account
		...
		exchange_gain_loss_account = dontmanage.db.get_value("Company", pr.company, "exchange_gain_loss_account")

		# fetching the latest GL Entry with exchange gain and loss account account
		amount = dontmanage.db.get_value(
	...
	def test_backdated_transaction_for_internal_transfer(self):
		...
		dn_value = dontmanage.db.get_value(
			"Stock Ledger Entry",
		...
		pr_value = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_receipt(
		...
		stk_ledger = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_invoice(
		...
		si_value = dontmanage.db.get_value(
			"Stock Ledger Entry",
		...
		pi_value = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_disable_last_purchase_rate(self):
		from dontmanageerp.stock.get_item_details import get_item_details
		...
		dontmanage.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 1)
		...
		dontmanage.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 0)
	...
	def test_validate_received_qty_for_internal_pr(self):
		...
		self.assertRaises(dontmanage.ValidationError, pr.submit)

		# Step 5: Test Over Receipt Allowance
		dontmanage.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 50)
		...
		dontmanage.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 0)
	...
	def test_internal_pr_gl_entries(self):
		...
		stock_account_value = dontmanage.db.get_value(
			"GL Entry",
			{
		...
		stock_diff = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
	...
	def test_purchase_receipt_with_backdated_landed_cost_voucher(self):
		from dontmanageerp.controllers.sales_and_purchase_return import make_return_doc
		from dontmanageerp.stock.doctype.landed_cost_voucher.test_landed_cost_voucher import (
	...
	def test_purchase_receipt_provisional_accounting(self):
		...
		amount = dontmanage.db.get_value("GL Entry", {"docstatus": 1, "voucher_no": pr.name}, ["sum(debit)"])
	...
	def test_use_serial_batch_fields_for_serial_nos(self):
		...
		sbb_doc = dontmanage.get_doc("Serial and Batch Bundle", pr.items[0].serial_and_batch_bundle)
		...
		serial_no_status = dontmanage.db.get_value("Serial No", "SNU-TSFISI-000015", "status")
		...
		doc = dontmanage.get_doc("Serial and Batch Bundle", dn.items[0].serial_and_batch_bundle)
		for row in doc.entries:
			self.assertTrue(row.serial_no in new_serial_nos)

		for sn in new_serial_nos:
			serial_no_status = dontmanage.db.get_value("Serial No", sn, "status")
			self.assertTrue(serial_no_status != "Active")
	...
	def test_purchase_receipt_with_use_serial_batch_field_for_rejected_qty(self):
		...
		if not dontmanage.db.exists("Batch", batch_no):
			dontmanage.get_doc(
				{
					"doctype": "Batch",
					"batch_id": batch_no,
		...
		for row in inter_transfer_pr_return.items:
			...
			if row.item_code == serial_item_doc.name:
				...
				for sn in serial_nos:
					serial_no_details = dontmanage.db.get_value(
	...
	def test_purchase_receipt_bill_for_rejected_quantity_in_purchase_invoice(self):
		...
		return_pr.items[0].purchase_receipt_item = pr.items[0].name
		return_pr.submit()
		old_value = dontmanage.db.get_single_value(
		...
		dontmanage.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 1)
		pi = make_purchase_invoice(pr.name)
		pi.submit()
	...
	def test_zero_valuation_rate_for_batched_item(self):
		...
		sabb_doc = dontmanage.get_doc("Serial and Batch Bundle", se.items[0].serial_and_batch_bundle)
	...
	def test_valuation_taxes_lcv_repost_after_billing(self):
		...
		old_perpetual_inventory = dontmanageerp.is_perpetual_inventory_enabled("_Test Company")
		dontmanage.local.enable_perpetual_inventory["_Test Company"] = 1
		...
		dontmanage.local.enable_perpetual_inventory["_Test Company"] = old_perpetual_inventory
	...
	def test_manufacturing_and_expiry_date_for_batch(self):
		...
		batch_no = get_batch_from_bundle(pr.items[0].serial_and_batch_bundle)
		batch = dontmanage.get_doc("Batch", batch_no)
	...
	def test_tax_account_heads_on_lcv_and_item_repost(self):
		...
		company_doc = dontmanage.get_doc("Company", test_company)
	...
	def create_lcv(self, receipt_document_type, receipt_document, company, expense_account, charges=50):
		ref_doc = dontmanage.get_doc(receipt_document_type, receipt_document)

		lcv = dontmanage.new_doc("Landed Cost Voucher")
		lcv.company = company
	...
	def test_do_not_use_batchwise_valuation_rate(self):
		...
		valuation_rate = dontmanage.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
		self.assertEqual(valuation_rate, 100)

		doc = dontmanage.get_doc("Stock Settings")
	...
	def test_internal_transfer_for_batch_items_with_cancel_use_serial_batch_fields(self):
		...
		dontmanage.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		dontmanage.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 0)
		...
		inter_transfer_pr.cancel()
		inter_transfer_dn.cancel()
		dontmanage.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 1)
	...
	def test_sles_with_same_posting_datetime_and_creation(self):
		...
		sle = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{"voucher_type": sr.doctype, "voucher_no": sr.name, "item_code": sr.items[0].item_code},
			"name",
		)

		sle_doc = dontmanage.get_doc("Stock Ledger Entry", sle)
	...
	def test_purchase_receipt_return_valuation_without_use_serial_batch_field(self):
		...
		for row in return_entry.items:
			if row.item_code == batch_item:
				bundle_data = dontmanage.get_all(
	...
	def test_do_not_allow_to_inward_same_serial_no_multiple_times(self):
		...
		dontmanage.db.set_single_value("Stock Settings", "allow_existing_serial_no", 0)
		...
		status = dontmanage.db.get_value("Serial No", serial_no, "status")
		...
		self.assertRaises(dontmanage.exceptions.ValidationError, pr.submit)
	...
	def test_seral_no_return_validation(self):
		...
		sn_return.save()
		self.assertRaises(dontmanage.ValidationError, sn_return.submit)
	...
	def test_batch_no_return_validation(self):
		...
		batch_return.save()
		self.assertRaises(dontmanage.ValidationError, batch_return.submit)
...
def prepare_data_for_internal_transfer():
	from dontmanageerp.accounts.doctype.sales_invoice.test_sales_invoice import create_internal_supplier
	...
	if not dontmanage.db.get_value("Company", company, "unrealized_profit_loss_account"):
		account = "Unrealized Profit and Loss - TCP1"
		if not dontmanage.db.exists("Account", account):
			dontmanage.get_doc(
				{
					"doctype": "Account",
		...
		dontmanage.db.set_value("Company", company, "unrealized_profit_loss_account", account)
...
def get_gl_entries(voucher_type, voucher_no, skip_cancelled=False, as_dict=True):
	...
	gl_query = (
		...
		.orderby(gl.account, order=dontmanage.qb.desc)
	)
	if skip_cancelled:
...
def make_purchase_receipt(**args):
	...
	dontmanage.db.set_single_value("Buying Settings", "allow_multiple_items", 1)
	pr = dontmanage.new_doc("Purchase Receipt")
	args = dontmanage._dict(args)
	...
	uom = args.uom or dontmanage.db.get_value("Item", item_code, "stock_uom") or "_Test UOM"
	...
	pr.append(
		"items",
		{
			...
			"uom": uom,
			"cost_center": args.cost_center or dontmanage.get_cached_value("Company", pr.company, "cost_center"),
	...
	if args.get_multiple_items:
		...
		company_cost_center = dontmanage.get_cached_value("Company", pr.company, "cost_center")
...
test_records = dontmanage.get_test_records("Purchase Receipt")
