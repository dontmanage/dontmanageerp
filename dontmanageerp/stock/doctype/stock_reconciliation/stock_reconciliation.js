...
dontmanage.ui.form.on("Stock Reconciliation", {
	...
	set_fields_onload_for_line_item(frm) {
		if (frm.is_new() && frm.doc?.items && cint(dontmanage.user_defaults?.use_serial_batch_fields) === 1) {
	...
	get_items: function (frm) {
		...
		dontmanage.prompt(
			...
			function (data) {
				dontmanage.call({
					method: "dontmanageerp.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
					args: {
					...
					callback: function (r) {
						...
						r.message.forEach((row) => {
							...
							item.use_serial_batch_fields = cint(
								dontmanage.user_defaults?.use_serial_batch_fields
							);
						});
	...
	set_valuation_rate_and_qty: function (frm, cdt, cdn) {
		...
		if (d.item_code && d.warehouse) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
				...
				callback: function (r) {
					const row = dontmanage.model.get_doc(cdt, cdn);
					if (!frm.doc.scan_mode) {
						dontmanage.model.set_value(cdt, cdn, "qty", r.message.qty);
					}
					dontmanage.model.set_value(cdt, cdn, "valuation_rate", r.message.rate);
					dontmanage.model.set_value(cdt, cdn, "current_qty", r.message.qty);
					dontmanage.model.set_value(cdt, cdn, "current_valuation_rate", r.message.rate);
	...
	toggle_display_account_head: function (frm) {
		frm.toggle_display(
			...
			dontmanageerp.is_perpetual_inventory_enabled(frm.doc.company)
		);
	},
	...
	set_expense_account: function (frm) {
		if (frm.doc.company && dontmanageerp.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "dontmanageerp.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
dontmanage.ui.form.on("Stock Reconciliation Item", {
	warehouse: function (frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if (child.batch_no && !frm.doc.scan_mode) {
			dontmanage.model.set_value(child.cdt, child.cdn, "batch_no", "");
		}
	...
	item_code: function (frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if (child.batch_no && !frm.doc.scan_mode) {
			dontmanage.model.set_value(cdt, cdn, "batch_no", "");
		}
	...
	serial_no: function (frm, cdt, cdn) {
		...
		if (child.serial_no) {
			...
			dontmanage.model.set_value(cdt, cdn, "qty", serial_nos.length);
		}
	},
	...
	items_add: function (frm, cdt, cdn) {
		...
		if (!item.warehouse && frm.doc.set_warehouse) {
			dontmanage.model.set_value(cdt, cdn, "warehouse", frm.doc.set_warehouse);
		}

		if (item.docstatus === 0 && cint(dontmanage.user_defaults?.use_serial_batch_fields) === 1) {
			dontmanage.model.set_value(item.doctype, item.name, "use_serial_batch_fields", 1);
		}
	},
...
cur_frm.cscript = new dontmanageerp.stock.StockReconciliation({ frm: cur_frm });
