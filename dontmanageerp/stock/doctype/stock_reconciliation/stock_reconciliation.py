...
import dontmanage
from dontmanage import _, bold, msgprint
...
from dontmanageerp.stock.utils import get_stock_balance
...
class StockReconciliation(StockController):
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": dontmanage._dict({row.batch_no: row.qty}) if row.batch_no else None,
	...
	def update_stock_ledger(self):
		...
		if sl_entries:
			allow_negative_stock = cint(
				dontmanage.db.get_single_value("Stock Settings", "allow_negative_stock")
			)
	...
	def recalculate_current_qty(self, voucher_detail_no, sle_creation, add_new_sle=False):
		...
		for row in self.items:
			...
			if (
				add_new_sle
				and not dontmanage.db.get_value(
					"Stock Ledger Entry",
		...
		if sl_entries:
			...
			if not dontmanage.db.exists("Repost Item Valuation", {"voucher_no": self.name, "status": "Queued"}):
...
def get_stock_balance_for(
	...
	item_dict = dontmanage.get_cached_value(
