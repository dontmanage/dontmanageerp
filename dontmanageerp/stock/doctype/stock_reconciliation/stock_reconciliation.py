...
import dontmanage
from dontmanage import _, bold, json, msgprint
from dontmanage.query_builder.functions import CombineDatetime, Sum
from dontmanage.utils import add_to_date, cint, cstr, flt
...
from dontmanageerp.stock.doctype.batch.batch import get_available_batches, get_batch_qty
...
class OpeningEntryAccountError(dontmanage.ValidationError):
	pass


class EmptyStockReconciliationItemsError(dontmanage.ValidationError):
	pass
...
class StockReconciliation(StockController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.stock_reconciliation_item.stock_reconciliation_item import (
	...
	def validate(self):
		...
		if not self.expense_account:
			self.expense_account = dontmanage.get_cached_value(
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": dontmanage._dict({row.batch_no: row.current_qty}) if row.batch_no else None,
	...
	def set_current_serial_and_batch_bundle(self, voucher_detail_no=None, save=False) -> None:
		...
		for item in self.items:
			if not dontmanage.db.exists("Item", item.item_code):
				dontmanage.throw(_("Item {0} does not exist").format(item.item_code))
			...
			if not item.current_serial_and_batch_bundle:
				serial_and_batch_bundle = dontmanage.get_doc(
					{
			...
			if save:
				sle_creation = dontmanage.db.get_value(
	...
	def has_change_in_serial_batch(self, row) -> bool:
		...
		if diff:
			...
			dontmanage.delete_doc("Serial and Batch Bundle", bundle)
	...
	def set_new_serial_and_batch_bundle(self):
		for item in self.items:
			...
			if item.current_serial_and_batch_bundle and not item.serial_and_batch_bundle:
				current_doc = dontmanage.get_doc("Serial and Batch Bundle", item.current_serial_and_batch_bundle)

				item.qty = abs(current_doc.total_qty)
				item.valuation_rate = abs(current_doc.avg_rate)

				bundle_doc = dontmanage.copy_doc(current_doc)
				...
				bundle_doc = dontmanage.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def update_existing_serial_and_batch_bundle(self, item):
		batch_details = dontmanage.get_all(
			"Serial and Batch Entry",
		...
		if batch_details and len(batch_details) == 1:
			...
			dontmanage.db.set_value("Serial and Batch Entry", batch.name, update_values)
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			...
			for i, item in enumerate(self.items):
				item.idx = i + 1
			dontmanage.msgprint(_("Removed items with no change in quantity or value."))
	...
	def validate_data(self):
		...
		for row_num, row in enumerate(self.items):
			...
			# validate warehouse
			if not dontmanage.db.get_value("Warehouse", row.warehouse):
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = dontmanage.db.get_value(
						"Item Price",
					...
					if buying_rate:
						...
						row.valuation_rate = dontmanage.get_value("Item", row.item_code, "valuation_rate")
		...
		if self.validation_messages:
			...
			raise dontmanage.ValidationError(self.validation_messages)

	def validate_item(self, item_code, row):
		from dontmanageerp.stock.doctype.item.item import (
	...
	def update_stock_ledger(self):
			...
			if item.has_serial_no or item.has_batch_no:
				...
				if row.serial_and_batch_bundle:
					dontmanage.throw(
						...
						).format(row.idx, dontmanage.bold(row.item_code))
					)
		...
		if sl_entries:
			allow_negative_stock = cint(dontmanage.db.get_single_value("Stock Settings", "allow_negative_stock"))
			self.make_sl_entries(sl_entries, allow_negative_stock=allow_negative_stock)

	def make_adjustment_entry(self, row, sl_entries):
		from dontmanageerp.stock.stock_ledger import get_stock_value_difference
	...
	def update_valuation_rate_for_serial_nos(self, row, serial_nos):
		...
		for d in serial_nos:
			dontmanage.db.set_value("Serial No", d, "purchase_rate", valuation_rate)
		...
		data = dontmanage._dict(
			{
				...
				"actual_qty": 0,
				"company": self.company,
				"stock_uom": dontmanage.db.get_value("Item", row.item_code, "stock_uom"),
	...
	def validate_expense_account(self):
		...
		if not self.expense_account:
			dontmanage.throw(_("Please enter Expense Account"))
		elif self.purpose == "Opening Stock" or not dontmanage.db.sql(
			"""select name from `tabStock Ledger Entry` limit 1"""
		):
			if dontmanage.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				dontmanage.throw(
					_(
	...
	def set_zero_value_for_customer_provided_items(self):
		...
		for d in self.get("items"):
			is_customer_item = dontmanage.db.get_value("Item", d.item_code, "is_customer_provided_item")
	...
	def recalculate_current_qty(self, voucher_detail_no):
		from dontmanageerp.stock.stock_ledger import get_valuation_rate
	...
	def get_current_qty_for_serial_or_batch(self, row):
		doc = dontmanage.get_doc("Serial and Batch Bundle", row.current_serial_and_batch_bundle)
		current_qty = 0.0
...
def get_batch_qty_for_stock_reco(item_code, warehouse, batch_no, posting_date, posting_time, voucher_no):
	ledger = dontmanage.qb.DocType("Stock Ledger Entry")
...
def get_item_and_warehouses(item_code, warehouse):
	...
	if dontmanage.get_cached_value("Warehouse", warehouse, "is_group"):
		...
		for ch_warehouse in childrens:
			items.append(dontmanage._dict({"item_code": item_code, "warehouse": ch_warehouse}))
	else:
		items = [dontmanage._dict({"item_code": item_code, "warehouse": warehouse})]
...
def get_itemwise_batch(warehouse, posting_date, company, item_code=None):
	from dontmanageerp.stock.report.batch_wise_balance_history.batch_wise_balance_history import execute
...
def get_stock_balance_for(
	...
	dontmanage.has_permission("Stock Reconciliation", "write", throw=True)

	item_dict = dontmanage.get_cached_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
	...
	if not item_dict:
		...
		dontmanage.throw(msg, title=_("Missing"))
