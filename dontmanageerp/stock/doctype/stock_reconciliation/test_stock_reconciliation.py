...
# DontManageErp - web based ERP (http://dontmanageerp.com)
...
from dontmanage.utils import add_days, cstr, flt, nowdate, nowtime

from dontmanageerp.accounts.utils import get_stock_and_account_balance
...
from dontmanageerp.stock.stock_ledger import get_previous_sle, update_entries_after
from dontmanageerp.stock.tests.test_utils import StockTestMixin
from dontmanageerp.stock.utils import get_incoming_rate, get_stock_value_on, get_valuation_method


class TestStockReconciliation(DontManageTestCase, StockTestMixin):
	@classmethod
	def setUpClass(cls):
	...
	def tearDown(self):
		...
		dontmanage.flags.pop("dont_execute_stock_reposts", None)
	...
	def test_stock_reco_for_serialized_item(self):
		...
		sn_doc = dontmanage.get_doc("Serial and Batch Bundle", sr.items[0].serial_and_batch_bundle)
		...
		for d in to_delete_records:
			stock_doc = dontmanage.get_doc("Stock Reconciliation", d)
			stock_doc.cancel()
	...
	def test_stock_reco_for_serial_and_batch_item(self):
		...
		self.assertEqual(dontmanage.db.get_value("Serial No", serial_nos[0], "batch_no"), batch_no)

		sr.cancel()

		self.assertEqual(dontmanage.db.get_value("Serial No", serial_nos[0], "warehouse"), None)
	...
	def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
		...
		self.assertEqual(dontmanage.db.get_value("Serial No", reco_serial_no, "batch_no"), batch_no)
		self.assertTrue(dontmanage.db.get_value("Serial No", reco_serial_no, "warehouse"))

		# Check if Serial No from Stock Entry is Unlinked and Inactive
		self.assertFalse(dontmanage.db.get_value("Serial No", serial_no_2, "warehouse"))
	...
	def test_backdated_stock_reco_qty_reposting(self):
		...
		def assertBalance(doc, qty_after_transaction):
			sle_balance = dontmanage.db.get_value(
	...
	def test_backdated_stock_reco_future_negative_stock(self):
		...
		from dontmanageerp.stock.stock_ledger import NegativeStockError
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		dn_balance = dontmanage.db.get_value(
		...
		repost_exists = bool(
			dontmanage.db.exists("Repost Item Valuation", {"voucher_no": sr.name, "status": "Queued"})
		)
	...
	def test_intermediate_sr_bin_update(self):
		...
		old_bin_qty = dontmanage.db.get_value(
		...
		new_bin_qty = dontmanage.db.get_value(
			"Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty"
		)
	...
	def test_serial_no_cancellation(self):
		...
		active_sr_no = dontmanage.get_all(
	...
	def test_serial_no_creation_and_inactivation(self):
		...
		if not dontmanage.db.exists("Serial No", "SR-CREATED-SR-NO"):
			dontmanage.get_doc(
				{
					"doctype": "Serial No",
					"item_code": item_code,
	...
	def test_serial_no_batch_no_item(self):
		...
		sl_entry = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_stock_reco_entry(self):
		...
		self.assertFalse(dontmanage.db.exists("Repost Item Valuation", {"voucher_no": stock_reco.name}))
	...
	def test_make_stock_zero_for_serial_batch_item(self):
		...
		for item_code in [serial_item, batch_item, serial_batch_item]:
			...
			serial_batch_bundle = dontmanage.get_all(
				"Stock Ledger Entry",
	...
	def test_backdated_purchase_receipt_with_stock_reco(self):
		...
		active_serial_no = dontmanage.get_all("Serial No", filters={"status": "Active", "item_code": item_code})
	...
	def test_not_reconcile_all_batch(self):
		...
		for qty in [10, 20, 30]:
			...
			batches.append(dontmanage._dict({"batch_no": batch_no, "qty": qty}))
		...
		doc = dontmanage.get_doc("Serial and Batch Bundle", current_sabb)
		...
		for row in dontmanage.get_all("Repost Item Valuation", filters={"voucher_no": sr.name}):
			rdoc = dontmanage.get_doc("Repost Item Valuation", row.name)
			rdoc.cancel()
			rdoc.delete()
	...
	def test_not_reconcile_all_serial_nos(self):
		...
		doc = dontmanage.get_doc("Serial and Batch Bundle", sabb)
		for row in doc.entries:
	...
	def test_stock_reco_with_legacy_batch(self):
		...
		batch_doc = dontmanage.get_doc("Batch", batch_id)
...
def create_stock_reconciliation(**args):
	...
	sr = dontmanage.new_doc("Stock Reconciliation")
	...
	sr.expense_account = args.expense_account or (
		(
			dontmanage.get_cached_value("Company", sr.company, "stock_adjustment_account")
			or dontmanage.get_cached_value(
				"Account", {"account_type": "Stock Adjustment", "company": sr.company}, "name"
			)
		)
		if dontmanage.get_all("Stock Ledger Entry", {"company": sr.company})
		else dontmanage.get_cached_value("Account", {"account_type": "Temporary", "company": sr.company}, "name")
	)
	sr.cost_center = (
		args.cost_center
		or dontmanage.get_cached_value("Company", sr.company, "cost_center")
		or dontmanage.get_cached_value("Cost Center", filters={"is_group": 0, "company": sr.company})
	)
...
def set_valuation_method(item_code, valuation_method):
	...
	dontmanage.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in dontmanage.get_all(
