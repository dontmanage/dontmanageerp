...
class TestStockReconciliation(DontManageTestCase, StockTestMixin):
	...
	def test_stock_reco_for_merge_serialized_item(self):
		...
		sle_entries = dontmanage.get_all(
	...
	def test_stock_reco_for_serial_and_batch_item(self):
		...
		self.assertEqual(dontmanage.db.get_value("Serial No", serial_nos[0], "status"), "Inactive")
	...
	def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
		...
		self.assertEqual(dontmanage.db.get_value("Serial No", reco_serial_no, "status"), "Active")

		# Check if Serial No from Stock Entry is Unlinked and Inactive
		self.assertEqual(dontmanage.db.get_value("Serial No", serial_no_2, "batch_no"), None)
		self.assertEqual(dontmanage.db.get_value("Serial No", serial_no_2, "status"), "Inactive")
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		repost_exists = bool(dontmanage.db.exists("Repost Item Valuation", {"voucher_no": sr.name}))
	...
	def test_valid_batch(self):
		...
		self.assertRaises(dontmanage.ValidationError, sr.submit)
