...
class LandedCostVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.landed_cost_item.landed_cost_item import LandedCostItem
		from dontmanageerp.stock.doctype.landed_cost_purchase_receipt.landed_cost_purchase_receipt import (
		...
		for pr in self.get("purchase_receipts"):
			if pr.receipt_document_type and pr.receipt_document:
				...
				for d in pr_items:
					...
					item.cost_center = d.cost_center or dontmanageerp.get_default_cost_center(self.company)
			...
			if (
				...
				dontmanage.throw(
					_("Row {0}: {2} Item {1} does not exist in {2} {3}").format(
						d.idx,
						dontmanage.bold(d.purchase_receipt_item),
						d.receipt_document_type,
						dontmanage.bold(d.receipt_document),
					),
	...
	def check_mandatory(self):
		if not self.get("purchase_receipts"):
			dontmanage.throw(_("Please enter Receipt Document"))

	def validate_receipt_documents(self):
		receipt_documents = []

		for d in self.get("purchase_receipts"):
			docstatus = dontmanage.db.get_value(d.receipt_document_type, d.receipt_document, "docstatus")
			if docstatus != 1:
				msg = (
					f"Row {d.idx}: {d.receipt_document_type} {dontmanage.bold(d.receipt_document)} must be submitted"
				)
				dontmanage.throw(_(msg), title=_("Invalid Document"))

			if d.receipt_document_type == "Purchase Invoice":
				update_stock = dontmanage.db.get_value(d.receipt_document_type, d.receipt_document, "update_stock")
				if not update_stock:
					msg = _("Row {0}: Purchase Invoice {1} has no stock impact.").format(
						d.idx, dontmanage.bold(d.receipt_document)
					)
					msg += "<br>" + _(
						"Please create Landed Cost Vouchers against Invoices that have 'Update Stock' enabled."
					)
					dontmanage.throw(msg, title=_("Incorrect Invoice"))

			receipt_documents.append(d.receipt_document)

		for item in self.get("items"):
			if not item.receipt_document:
				dontmanage.throw(_("Item must be added using 'Get Items from Purchase Receipts' button"))
	...
	def set_applicable_charges_on_item(self):
		if self.get("taxes") and self.distribute_charges_based_on != "Distribute Manually":
			...
			total_charges = 0.0
			item_count = 0
			based_on_field = dontmanage.scrub(self.distribute_charges_based_on)
	...
	def validate_applicable_charges_for_item(self):
		...
		precision = get_field_precision(
			dontmanage.get_meta("Landed Cost Item").get_field("applicable_charges"),
	...
	def update_landed_cost(self):
		for d in self.get("purchase_receipts"):
			doc = dontmanage.get_doc(d.receipt_document_type, d.receipt_document)
...
def get_pr_items(purchase_receipt):
	...
	pr_item = dontmanage.qb.DocType(purchase_receipt.receipt_document_type + " Item")
	return (
		dontmanage.qb.from_(pr_item)
		.inner_join(item)
