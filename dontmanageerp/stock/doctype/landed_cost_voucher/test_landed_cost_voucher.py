...
from dontmanage.utils import add_days, add_to_date, flt, now, nowtime, today
...
from dontmanageerp.accounts.utils import update_gl_entries_after
from dontmanageerp.assets.doctype.asset.test_asset import create_asset_category, create_fixed_asset_item
...
class TestLandedCostVoucher(DontManageTestCase):
	def test_landed_cost_voucher(self):
		...
		last_sle = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_landed_cost_voucher_stock_impact(self):
		...
		pr1 = make_purchase_receipt(
			...
			qty=500,
			rate=80,
			posting_date=add_days(dontmanage.utils.nowdate(), -2),
		)
		...
		last_sle = dontmanage.db.get_value(  # SLE of second PR
			"Stock Ledger Entry",
			{
		...
		last_sle_after_landed_cost = dontmanage.db.get_value(  # SLE of second PR after LCV's effect
			"Stock Ledger Entry",
			{
				"voucher_type": pr2.doctype,
				"voucher_no": pr2.name,
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = dontmanage.db.get_value(
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		dontmanage.db.set_value("Item", "_Test Serialized Item", "serial_no_series", "SNJJ.###")
	...
	def test_asset_lcv(self):
		...
		self.assertEqual(dontmanage.db.get_value("Asset", assets[0].name, "gross_purchase_amount"), 50080)
	...
	def test_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		...
		if not dontmanage.db.exists("Batch", "BATCH-TLCVSNO-0001"):
		...
		for sn in serial_nos:
			sn_doc = dontmanage.get_doc("Serial No", sn)
			sn_doc.db_set(
				{
					"warehouse": warehouse,
					"status": "Active",
		...
		for row in pr.items:
			...
			self.assertEqual(
				row.valuation_rate,
				dontmanage.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "avg_rate"),
			)
	...
	def test_do_not_validate_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		...
		if not dontmanage.db.exists("Batch", "BATCH-TDVLCVSNO-0001"):
			batch_doc = dontmanage.get_doc(
				{
					"doctype": "Batch",
					"item": batch_item,
		...
		stock_ledger_entries = dontmanage.get_all("Stock Ledger Entry", filters={"voucher_no": pr.name})
		for sle in stock_ledger_entries:
			doc = dontmanage.get_doc("Stock Ledger Entry", sle.name)
		...
		stock_ledger_entries = dontmanage.get_all("Stock Ledger Entry", filters={"voucher_no": dn.name})
		for sle in stock_ledger_entries:
			doc = dontmanage.get_doc("Stock Ledger Entry", sle.name)
			if doc.item_code == sn_item:
				doc.db_set("serial_no", ", ".join(serial_nos))
...
def make_landed_cost_voucher(**args):
	...
	ref_doc = dontmanage.get_doc(args.receipt_document_type, args.receipt_document)
...
test_records = dontmanage.get_test_records("Landed Cost Voucher")
