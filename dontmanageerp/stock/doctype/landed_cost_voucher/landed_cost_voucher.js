...
dontmanageerp.stock.LandedCostVoucher = class LandedCostVoucher extends dontmanageerp.stock.StockController {
	setup() {
		...
		this.frm.fields_dict.purchase_receipts.grid.get_field('receipt_document').get_query =
			function (doc, cdt, cdn) {
				...
				if (!me.frm.doc.company) dontmanage.msgprint(__("Please enter company first"));
				return {
					filters: filters
				}
			};
	...
	refresh() {
		...
		if (this.frm.doc.company) {
			let company_currency = dontmanage.get_doc(":Company", this.frm.doc.company).default_currency;
	...
	get_items_from_purchase_receipts() {
		...
		if(!this.frm.doc.purchase_receipts.length) {
			dontmanage.msgprint(__("Please enter Purchase Receipt first"));
		} else {
...
cur_frm.script_manager.make(dontmanageerp.stock.LandedCostVoucher);
