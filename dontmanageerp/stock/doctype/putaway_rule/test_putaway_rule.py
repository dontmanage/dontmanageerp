...
class TestPutawayRule(DontManageTestCase):
	def setUp(self):
		if not dontmanage.db.exists("Item", "_Rice"):
			make_item(
				"_Rice", {"is_stock_item": 1, "has_batch_no": 1, "create_new_batch": 1, "stock_uom": "Kg"}
			)

		if not dontmanage.db.exists("Warehouse", {"warehouse_name": "Rack 1"}):
			create_warehouse("Rack 1")
		if not dontmanage.db.exists("Warehouse", {"warehouse_name": "Rack 2"}):
			create_warehouse("Rack 2")

		self.warehouse_1 = dontmanage.db.get_value("Warehouse", {"warehouse_name": "Rack 1"})
		self.warehouse_2 = dontmanage.db.get_value("Warehouse", {"warehouse_name": "Rack 2"})

		if not dontmanage.db.exists("UOM", "Bag"):
	...
	def test_putaway_rules_multi_uom(self):
		...
		item = dontmanage.get_doc("Item", "_Rice")
		if not dontmanage.db.get_value("UOM Conversion Detail", {"parent": "_Rice", "uom": "Bag"}):
	...
	def test_putaway_rules_multi_uom_whole_uom(self):
		...
		dontmanage.db.set_value("UOM", "Bag", "must_be_whole_number", 1)
	...
	def test_putaway_rule_on_stock_entry_material_transfer_batch_serial_item(self):
		...
		if not dontmanage.db.exists("Item", "Water Bottle"):
			make_item(
				"Water Bottle",
				{
					"is_stock_item": 1,
		...
		rivs = dontmanage.get_all("Repost Item Valuation", filters={"voucher_no": stock_entry.name})
		for row in rivs:
			riv_doc = dontmanage.get_doc("Repost Item Valuation", row.name)
			riv_doc.cancel()
			riv_doc.delete()
	...
	def test_warehouse_capacity_dashbord(self):
		from dontmanageerp.stock.dashboard.warehouse_capacity_dashboard import get_data
...
def create_putaway_rule(**args):
	...
	putaway = dontmanage.new_doc("Putaway Rule")
	...
	putaway.stock_uom = dontmanage.db.get_value("Item", putaway.item_code, "stock_uom")
