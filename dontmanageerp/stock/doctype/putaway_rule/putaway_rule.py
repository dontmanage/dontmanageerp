...
from dontmanage.utils import cint, cstr, floor, flt, nowdate
...
class PutawayRule(Document):
	...
	def validate_duplicate_rule(self):
		existing_rule = dontmanage.db.exists(
	...
	def validate_priority(self):
		if self.priority < 1:
			dontmanage.throw(_("Priority cannot be lesser than 1."), title=_("Invalid Priority"))

	def validate_warehouse_and_company(self):
		company = dontmanage.db.get_value("Warehouse", self.warehouse, "company")
		if company != self.company:
			dontmanage.throw(
				_("Warehouse {0} does not belong to Company {1}.").format(
					dontmanage.bold(self.warehouse), dontmanage.bold(self.company)
				),
				title=_("Invalid Warehouse"),
			)

	def validate_capacity(self):
		stock_uom = dontmanage.db.get_value("Item", self.item_code, "stock_uom")
		...
		if flt(self.stock_capacity) < flt(balance_qty):
			dontmanage.throw(
				...
				).format(self.item_code, dontmanage.bold(balance_qty), stock_uom),
				title=_("Insufficient Capacity"),
			)

		if not self.capacity:
			dontmanage.throw(_("Capacity must be greater than 0"), title=_("Invalid"))
...
def get_available_putaway_capacity(rule):
	stock_capacity, item_code, warehouse = dontmanage.db.get_value(
...
def apply_putaway_rule(doctype, items, company, sync=None, purpose=None):
	...
	for item in items:
		...
		uom_must_be_whole_number = dontmanage.db.get_value("UOM", item.uom, "must_be_whole_number")
	...
	if updated_table and _items_changed(items, updated_table, doctype):
		items[:] = updated_table
		dontmanage.msgprint(_("Applied putaway rules."), alert=True)
...
def _items_changed(old, new, doctype: str) -> bool:
	...
	old = [dontmanage._dict(item) if isinstance(item, dict) else item for item in old]
...
def show_unassigned_items_message(items_not_accomodated):
	...
	for entry in items_not_accomodated:
		item_link = dontmanage.utils.get_link_to_form("Item", entry[0])
		formatted_item_rows += """
			<td>{0}</td>
			<td>{1}</td>
		</tr>""".format(
			item_link, dontmanage.bold(entry[1])
		)
	...
	dontmanage.msgprint(msg, title=_("Insufficient Capacity"), is_minimizable=True, wide=True)
