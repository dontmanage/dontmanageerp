...
class ItemAttributeIncrementError(dontmanage.ValidationError):
	pass
...
class ItemAttribute(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.item_attribute_value.item_attribute_value import ItemAttributeValue
	...
	def set_enabled_disabled_in_items(self):
		...
		if not db_value or db_value.disabled != self.disabled:
			item_variant_table = dontmanage.qb.DocType("Item Variant Attribute")
			query = (
				dontmanage.qb.update(item_variant_table)
	...
	def validate_numeric(self):
		if self.numeric_values:
			...
			if self.from_range is None or self.to_range is None:
				dontmanage.throw(_("Please specify from/to range"))

			elif flt(self.from_range) >= flt(self.to_range):
				dontmanage.throw(_("From Range has to be less than To Range"))

			if not self.increment:
				dontmanage.throw(_("Increment cannot be 0"), ItemAttributeIncrementError)
		else:
	...
	def validate_duplication(self):
		...
		for d in self.item_attribute_values:
			...
			if d.abbr.lower() in map(str.lower, abbrs):
				dontmanage.throw(_("Abbreviation: {0} must appear only once").format(d.abbr.title()))
			abbrs.append(d.abbr)
