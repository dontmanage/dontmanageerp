	...
	set_from_warehouse: function (frm) {
		if (frm.doc.material_request_type == "Material Transfer" && frm.doc.set_from_warehouse) {
			frm.doc.items.forEach((d) => {
				dontmanage.model.set_value(d.doctype, d.name, "from_warehouse", frm.doc.set_from_warehouse);
			});
		}
	},
...
dontmanageerp.buying.MaterialRequestController = class MaterialRequestController extends (
	dontmanageerp.buying.BuyingController
) {
	tc_name() {
		this.get_terms();
	}
...
function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		dontmanageerp.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
