...
dontmanage.ui.form.on('Material Request', {
	setup: function(frm) {
	...
	onload: function(frm) {
		...
		dontmanageerp.utils.add_item(frm);
	...
	set_from_warehouse: function(frm) {
		if (frm.doc.material_request_type == "Material Transfer"
			...
			frm.doc.items.forEach(d => {
				dontmanage.model.set_value(d.doctype, d.name,
	...
	make_custom_buttons: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.status != 'Stopped') {
			let precision = dontmanage.defaults.get_default("float_precision");
	...
	update_status: function(frm, stop_status) {
		dontmanage.call({
			method: 'dontmanageerp.stock.doctype.material_request.material_request.update_status',
		...
		dontmanage.call({
			...
			args: {
				args: {
					...
					buying_price_list: dontmanage.defaults.get_default('buying_price_list'),
					currency: dontmanage.defaults.get_default('Currency'),
					name: frm.doc.name,
					qty: item.qty || 1,
			...
			callback: function(r) {
				...
				if(!r.exc) {
					...
					if (d.price_list_rate != r.message.price_list_rate) {
						...
						dontmanage.model.set_value(d.doctype, d.name, "rate", d.price_list_rate);
					}
					...
					callback: function(r) {
						if (!r.message) {
							...
							$.each(r.message, function(i, item) {
								var d = dontmanage.model.add_child(cur_frm.doc, "Material Request Item", "items");
		...
		dontmanage.prompt(
			{
				...
				get_query: () => {
					return{
						query: "dontmanageerp.stock.doctype.material_request.material_request.get_default_supplier_query",
			...
			(values) => {
				dontmanage.model.open_mapped_doc({
					method: "dontmanageerp.stock.doctype.material_request.material_request.make_purchase_order",
					frm: frm,
	...
	make_request_for_quotation: function(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.material_request.material_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true
		});
	},
	...
	make_supplier_quotation: function(frm) {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.material_request.material_request.make_supplier_quotation",
			frm: frm
		});
	},
	...
	make_in_transit_stock_entry(frm) {
		dontmanage.prompt(
			...
			(values) => {
				dontmanage.call({
					method: "dontmanageerp.stock.doctype.material_request.material_request.make_in_transit_stock_entry",
					args: {
	...
	create_pick_list: (frm) => {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.material_request.material_request.create_pick_list",
			frm: frm
		});
	},
	...
	raise_work_orders: function(frm) {
		dontmanage.call({
			method:"dontmanageerp.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
dontmanage.ui.form.on("Material Request Item", {
	qty: function (frm, doctype, name) {
		const item = locals[doctype][name];
		if (flt(item.qty) < flt(item.min_order_qty)) {
			dontmanage.msgprint(__("Warning: Material Requested Qty is less than Minimum Order Qty"));
		}
	...
	rate(frm, doctype, name) {
		...
		dontmanage.model.set_value(doctype, name, "amount", item.amount);
	...
	schedule_date: function(frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if(!frm.doc.schedule_date) {
				dontmanageerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
dontmanageerp.buying.MaterialRequestController = class MaterialRequestController extends dontmanageerp.buying.BuyingController {
	tc_name() {
		this.get_terms();
	}
	...
	qty(doc, cdt, cdn) {
		...
		dontmanage.model.set_value(cdt, cdn, "amount", row.amount);
		refresh_field("amount", row.name, row.parentfield);
	}
};

// for backward compatibility: combine new and previous states
extend_cscript(cur_frm.cscript, new dontmanageerp.buying.MaterialRequestController({frm: cur_frm}));

function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		dontmanageerp.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}
