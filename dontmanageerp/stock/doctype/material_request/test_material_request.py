...
from dontmanage.utils import flt, today

from dontmanageerp.stock.doctype.item.test_item import create_item
from dontmanageerp.stock.doctype.material_request.material_request import (
...
class TestMaterialRequest(DontManageTestCase):
	def test_make_purchase_order(self):
		mr = dontmanage.copy_doc(test_records[0]).insert()

		self.assertRaises(dontmanage.ValidationError, make_purchase_order, mr.name)

		mr = dontmanage.get_doc("Material Request", mr.name)
		mr.submit()
	...
	def test_make_supplier_quotation(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_supplier_quotation, mr.name)
	...
	def test_make_stock_entry(self):
		...
		self.assertRaises(dontmanage.ValidationError, make_stock_entry, mr.name)
	...
	def test_partial_make_stock_entry(self):
		from dontmanageerp.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry as _make_stock_entry
	...
	def test_cannot_stop_cancelled_material_request(self):
		mr = dontmanage.copy_doc(test_records[0])
		mr.insert()
		mr.submit()

		mr.load_from_db()
		mr.cancel()
		self.assertRaises(dontmanage.ValidationError, mr.update_status, "Stopped")
	...
	def test_cannot_submit_cancelled_mr(self):
		...
		mr.load_from_db()
		mr.cancel()
		self.assertRaises(dontmanage.ValidationError, mr.submit)
	...
	def test_cannot_change_cancelled_mr(self):
		...
		self.assertRaises(dontmanage.InvalidStatusError, mr.update_status, "Draft")
		self.assertRaises(dontmanage.InvalidStatusError, mr.update_status, "Stopped")
		self.assertRaises(dontmanage.InvalidStatusError, mr.update_status, "Ordered")
		self.assertRaises(dontmanage.InvalidStatusError, mr.update_status, "Issued")
		self.assertRaises(dontmanage.InvalidStatusError, mr.update_status, "Transferred")
		self.assertRaises(dontmanage.InvalidStatusError, mr.update_status, "Pending")
	...
	def test_cannot_delete_submitted_mr(self):
		...
		self.assertRaises(dontmanage.ValidationError, mr.delete)
	...
	def test_completed_qty_for_purchase(self):
		...
		po = dontmanage.copy_doc(po_doc)
		po.insert()
		po.load_from_db()
		mr.update_status("Stopped")
		self.assertRaises(dontmanage.InvalidStatusError, po.submit)
		po.db_set("docstatus", 1)
		self.assertRaises(dontmanage.InvalidStatusError, po.cancel)
	...
	def test_completed_qty_for_transfer(self):
		...
		# check for stopped status of Material Request
		se = dontmanage.copy_doc(se_doc)
		se.insert()
		mr.update_status("Stopped")
		self.assertRaises(dontmanage.InvalidStatusError, se.submit)
		...
		se.submit()
		mr.update_status("Stopped")
		self.assertRaises(dontmanage.InvalidStatusError, se.cancel)
	...
	def test_over_transfer_qty_allowance(self):
		mr = dontmanage.new_doc("Material Request")
		...
		dontmanage.db.set_single_value("Stock Settings", "mr_qty_allowance", 20)
		...
		se = dontmanage.copy_doc(se_doc)
		se.insert()
		self.assertRaises(dontmanage.ValidationError)
		se.items[0].qty = 12
		se.submit()
	...
	def test_incorrect_mapping_of_stock_entry(self):
		...
		# check for stopped status of Material Request
		se = dontmanage.copy_doc(se_doc)
		self.assertRaises(dontmanage.MappingMismatchError, se.insert)
	...
	def test_material_request_type_manufacture(self):
		mr = dontmanage.copy_doc(test_records[1]).insert()
		mr = dontmanage.get_doc("Material Request", mr.name)
		mr.submit()
		completed_qty = mr.items[0].ordered_qty
		requested_qty = dontmanage.db.sql(
		...
		po = dontmanage.get_doc("Work Order", prod_order[0])
		...
		new_requested_qty = dontmanage.db.sql(
			"""select indented_qty from `tabBin` where \
			item_code= %s and warehouse= %s """,
			(mr.items[0].item_code, mr.items[0].warehouse),
		)[0][0]
	...
	def test_requested_qty_multi_uom(self):
		...
		wo = dontmanage.get_doc("Work Order", work_order[0])
		wo.qty = 50
	...
	def test_multi_uom_for_purchase(self):
		...
		if not dontmanage.db.get_value("UOM Conversion Detail", {"parent": item.item_code, "uom": "Kg"}):
			item_doc = dontmanage.get_doc("Item", item.item_code)
	...
	def test_auto_email_users_with_company_user_permissions(self):
		from dontmanageerp.stock.reorder_item import get_email_list
		...
		comapnywise_mr_list = dontmanage._dict({})
...
def get_in_transit_warehouse(company):
	...
	in_transit_warehouse = dontmanage.db.exists("Warehouse", {"warehouse_type": "Transit", "company": company})
...
test_records = dontmanage.get_test_records("Material Request")
