...
from dontmanage.utils import cint, cstr, flt, get_link_to_form, getdate, new_line_sep, nowdate
...
from dontmanageerp.stock.stock_balance import get_indented_qty, update_bin_qty
...
class MaterialRequest(BuyingController):
	...
	def validate_qty_against_so(self):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = dontmanage.db.sql(
					"""select sum(qty)
				...
				actual_so_qty = dontmanage.db.sql(
	...
	def check_modified_date(self):
		mod_db = dontmanage.db.sql("""select modified from `tabMaterial Request` where name = %s""", self.name)
		date_diff = dontmanage.db.sql(f"""select TIMEDIFF('{mod_db[0][0]}', '{cstr(self.modified)}')""")

		if date_diff and date_diff[0][0]:
			dontmanage.throw(_("{0} {1} has been modified. Please refresh.").format(_(self.doctype), self.name))
	...
	def get_mr_items_ordered_qty(self, mr_items):
		...
		if self.material_request_type in ("Material Issue", "Material Transfer", "Customer Provided"):
			doctype = dontmanage.qb.DocType("Stock Entry Detail")
			qty_field = doctype.transfer_qty
		elif self.material_request_type == "Manufacture":
			doctype = dontmanage.qb.DocType("Work Order")
		...
		if doctype and qty_field:
			...
			mr_items_ordered_qty = dontmanage._dict(query.run())
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
		...
		mr_qty_allowance = dontmanage.db.get_single_value("Stock Settings", "mr_qty_allowance")
		...
		for d in self.get("items"):
			...
			if d.name in mr_items:
				...
				dontmanage.db.set_value(d.doctype, d.name, "ordered_qty", d.ordered_qty)
	...
	def update_requested_qty(self, mr_item_rows=None):
		...
		for d in self.get("items"):
			if (
				...
				and d.warehouse
				and dontmanage.db.get_value("Item", d.item_code, "is_stock_item") == 1
			):
	...
	def update_requested_qty_in_production_plan(self):
		...
		for production_plan in production_plans:
			doc = dontmanage.get_doc("Production Plan", production_plan)
			doc.set_status()
...
def update_status(name, status):
	material_request = dontmanage.get_doc("Material Request", name)
...
def make_purchase_order(source_name, target_doc=None, args=None):
	...
	def postprocess(source, target_doc):
		if dontmanage.flags.args and dontmanage.flags.args.default_supplier:
			# items only for given default supplier
			supplier_items = []
			for d in target_doc.items:
				default_supplier = get_item_defaults(d.item_code, target_doc.company).get("default_supplier")
				if dontmanage.flags.args.default_supplier == default_supplier:
...
def get_material_requests_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
	...
	if not supplier_items:
		dontmanage.throw(_("{0} is not the default supplier for any items.").format(supplier))

	material_requests = dontmanage.db.sql(
...
def get_default_supplier_query(doctype, txt, searchfield, start, page_len, filters):
	doc = dontmanage.get_doc("Material Request", filters.get("doc"))
	item_list = []
	for d in doc.items:
	...
	meta = dontmanage.get_meta("Supplier")
...
def raise_work_orders(material_request):
	...
	errors = []
	work_orders = []
	default_wip_warehouse = dontmanage.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")

	for d in mr.items:
		if (d.stock_qty - d.ordered_qty) > 0:
			if dontmanage.db.exists("BOM", {"item": d.item_code, "is_default": 1}):
		...
		if len(work_orders) > 1:
			msgprint(
				_("The following {0} were created: {1}").format(
					dontmanage.bold(_("Work Orders")), "<br>" + ", ".join(work_orders_list)
				)
			)
		else:
			msgprint(
				_("The {0} {1} created sucessfully").format(dontmanage.bold(_("Work Order")), work_orders_list[0])
			)
