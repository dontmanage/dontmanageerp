...
class FinishedGoodError(dontmanage.ValidationError):
	pass


class IncorrectValuationRateError(dontmanage.ValidationError):
	pass


class DuplicateEntryForWorkOrderError(dontmanage.ValidationError):
	pass


class OperationsNotCompleteError(dontmanage.ValidationError):
	pass


class MaxSampleAlreadyRetainedError(dontmanage.ValidationError):
	pass
...
class StockEntry(StockController):
	...
	def __init__(self, *args, **kwargs):
		...
		if self.purchase_order:
			self.subcontract_data = dontmanage._dict(
				{
	...
	def before_validate(self):
		from dontmanageerp.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate(self):
		self.pro_doc = dontmanage._dict()
		if self.work_order:
			self.pro_doc = dontmanage.get_doc("Work Order", self.work_order)
	...
	def validate_work_order_status(self):
		...
		if pro_doc.status == "Completed":
			dontmanage.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def validate_purpose(self):
		...
		if self.purpose not in valid_purposes:
			dontmanage.throw(_("Purpose must be one of {0}").format(comma_or(valid_purposes)))
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in dontmanage.get_all(
				...
				dontmanage.delete_doc("Stock Entry", d.name)

	def set_transfer_qty(self):
		for item in self.get("items"):
			if not flt(item.qty):
				dontmanage.throw(_("Row {0}: Qty is mandatory").format(item.idx), title=_("Zero quantity"))
			if not flt(item.conversion_factor):
				dontmanage.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
	...
	def update_cost_in_project(self):
		if self.work_order and not dontmanage.db.get_value(
		...
		if self.project:
			...
			additional_costs = dontmanage.db.sql(
			...
			dontmanage.db.set_value("Project", self.project, "total_consumed_material_cost", amount)
			...
			if flt(item.qty) and flt(item.qty) < 0:
				dontmanage.throw(
					_("Row {0}: The item {1}, quantity must be positive number").format(
						item.idx, dontmanage.bold(item.item_code)
					)
				)

			if item.item_code not in stock_items:
				dontmanage.throw(_("{0} is not a stock Item").format(item.item_code))
	...
	def validate_qty(self):
		...
		if self.purpose in manufacture_purpose and self.work_order:
			if not dontmanage.get_value("Work Order", self.work_order, "skip_transfer"):
				item_code = []
				for item in self.items:
					if cstr(item.t_warehouse) == "":
						req_items = dontmanage.get_all(
							"Work Order Item",
		...
		for item_code, qty_list in item_wise_qty.items():
			...
			if self.fg_completed_qty != total:
				dontmanage.throw(
					...
					).format(dontmanage.bold(item_code), dontmanage.bold(total), dontmanage.bold(self.fg_completed_qty))
				)

	def validate_difference_account(self):
		if not cint(dontmanageerp.is_perpetual_inventory_enabled(self.company)):
			return
		...
		for d in self.get("items"):
			if not d.expense_account:
				...
				self.is_opening == "Yes"
				and dontmanage.db.get_value("Account", d.expense_account, "report_type") == "Profit and Loss"
			):
				dontmanage.throw(
					_(
		...
		for d in self.get("items"):
			...
			if self.purpose in source_mandatory and not d.s_warehouse:
				if self.from_warehouse:
					...
					dontmanage.throw(_("Source warehouse is mandatory for row {0}").format(d.idx))

			if self.purpose in target_mandatory and not d.t_warehouse:
				if self.to_warehouse:
					d.t_warehouse = self.to_warehouse
				else:
					dontmanage.throw(_("Target warehouse is mandatory for row {0}").format(d.idx))
			...
			if cstr(d.s_warehouse) == cstr(d.t_warehouse) and self.purpose not in [
				...
				dontmanage.throw(_("Source and target warehouse cannot be same for row {0}").format(d.idx))

			if not (d.s_warehouse or d.t_warehouse):
				dontmanage.throw(_("Atleast one warehouse is mandatory"))
		...
		if self.purpose in (
			...
			if (
				...
				if not self.fg_completed_qty:
					dontmanage.throw(_("For Quantity (Manufactured Qty) is mandatory"))
	...
	def check_if_operations_completed(self):
		...
		prod_order = dontmanage.get_doc("Work Order", self.work_order)
		allowance_percentage = flt(
			dontmanage.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order")
		)
			...
			if total_completed_qty > flt(completed_qty):
				...
				dontmanage.throw(
					_(
						...
						d.idx,
						dontmanage.bold(d.operation),
						dontmanage.bold(total_completed_qty),
						work_order_link,
						job_card_link,
					),
	...
	def check_duplicate_entry_for_work_order(self):
		other_ste = [
			t[0]
			for t in dontmanage.db.get_values(
				"Stock Entry",
				{
		...
		if other_ste:
			production_item, qty = dontmanage.db.get_value(
				"Work Order", self.work_order, ["production_item", "qty"]
			)
			args = [*other_ste, production_item]
			fg_qty_already_entered = dontmanage.db.sql(
			...
			if (
				...
				dontmanage.throw(
					...
					+ _("Available quantity is {0}, you need {1}").format(
						dontmanage.bold(flt(d.actual_qty, d.precision("actual_qty"))), dontmanage.bold(d.transfer_qty)
					),
					NegativeStockError,
	...
	def validate_component_quantities(self):
		...
		if not dontmanage.db.get_single_value("Manufacturing Settings", "validate_components_quantities_per_bom"):
			return
	...
	def set_basic_rate(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=dontmanageerp.get_company_currency(self.company),
					company=self.company,
		...
		if items:
			...
			if len(items) > 1:
				...
				).format(", ".join(dontmanage.bold(item) for item in items))
			else:
				message = _(
					"Item rate has been updated to zero as Allow Zero Valuation Rate is checked for item {0}"
				).format(dontmanage.bold(items[0]))

			dontmanage.msgprint(message, alert=True)
				...
				if dontmanage.db.exists(
					...
					for item in self.items:
						if not item.is_finished_item and not item.is_scrap_item:
							label = dontmanage.get_meta(settings.doctype).get_label(
							...
							dontmanage.throw(
								_(
									...
									dontmanage.bold(_("Manufacture")),
									dontmanage.bold(_("Material Consumption for Manufacture")),
								)
							)
					...
					if dontmanage.db.exists(
						...
						dontmanage.throw(
							_("Only one {0} entry can be created against the Work Order {1}").format(
								dontmanage.bold(_("Manufacture")), dontmanage.bold(self.work_order)
							)
						)
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = dontmanage.get_cached_value(
	...
	def validate_subcontract_order(self):
		...
		backflush_raw_materials_based_on = dontmanage.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

		qty_allowance = flt(dontmanage.db.get_single_value("Buying Settings", "over_transfer_allowance"))
		...
		if backflush_raw_materials_based_on == "BOM":
			subcontract_order = dontmanage.get_doc(
			...
			for se_item in self.items:
				...
				if not required_qty:
					...
					if se_item.allow_alternative_item:
						original_item_code = dontmanage.get_value(
				...
				se = dontmanage.qb.DocType("Stock Entry")
				se_detail = dontmanage.qb.DocType("Stock Entry Detail")
				...
				if flt(total_supplied - total_returned, precision) > flt(total_allowed, precision):
					...
					order_rm_detail = dontmanage.db.get_value(
				...
				if not row.subcontracted_item:
					dontmanage.throw(
						_("Row {0}: Subcontracted Item is mandatory for the raw material {1}").format(
							row.idx, dontmanage.bold(row.item_code)
						)
					)
	...
	def validate_purchase_order(self):
		if self.purpose == "Send to Subcontractor" and self.get("purchase_order"):
			is_old_subcontracting_flow = dontmanage.db.get_value(
	...
	def get_finished_item(self):
		finished_item = None
		if self.work_order:
			finished_item = dontmanage.db.get_value("Work Order", self.work_order, "production_item")
		elif self.bom_no:
			finished_item = dontmanage.db.get_value("BOM", self.bom_no, "item")
	...
	def validate_finished_goods(self):
		...
		if self.work_order:
			wo_details = dontmanage.db.get_value("Work Order", self.work_order, ["production_item", "qty"])
			if wo_details:
	...
	def validate_serial_batch_bundle_type(self, serial_and_batch_bundle):
		if (
			dontmanage.db.get_value("Serial and Batch Bundle", serial_and_batch_bundle, "type_of_transaction")
			!= "Outward"
		):
			dontmanage.throw(
				_(
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			...
			if msg:
				dontmanage.throw(_(msg), title=title)

		if self.job_card:
			job_doc = dontmanage.get_doc("Job Card", self.job_card)
	...
	def get_item_details(self, args=None, for_update=False):
		...
		for company_field, field in {
			...
			if not ret.get(field):
				ret[field] = dontmanage.get_cached_value("Company", self.company, company_field)
		...
		if (
			...
			subcontract_items = dontmanage.get_all(
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = dontmanage.get_doc("Stock Entry", self.outgoing_stock_entry)

			if doc.per_transferred == 100:
				dontmanage.throw(_("Goods are already received against the outward entry {0}").format(doc.name))
	...
	def get_items_for_disassembly(self):
		...
		if not self.work_order:
			dontmanage.throw(_("The Work Order is mandatory for Disassembly Order"))

		items = self.get_items_from_manufacture_entry()

		s_warehouse = ""
		if self.work_order:
			s_warehouse = dontmanage.db.get_value("Work Order", self.work_order, "fg_warehouse")
	...
	def get_items(self):
		...
		if not self.posting_date or not self.posting_time:
			dontmanage.throw(_("Posting date and posting time is mandatory"))

		self.set_work_order_details()
		self.flags.backflush_based_on = dontmanage.db.get_single_value(
			"Manufacturing Settings", "backflush_raw_materials_based_on"
		)

		if self.bom_no:
			backflush_based_on = dontmanage.db.get_single_value(
				"Manufacturing Settings", "backflush_raw_materials_based_on"
			)
			...
			if self.purpose in [
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					and dontmanage.db.get_single_value("Manufacturing Settings", "material_consumption") == 1
				):
					self.get_unconsumed_raw_materials()

				else:
					if not self.fg_completed_qty:
						dontmanage.throw(_("Manufacturing Quantity is mandatory"))
					...
					if (
						...
						# Get Subcontract Order Supplied Items Details
						parent = dontmanage.qb.DocType(self.subcontract_data.order_doctype)
						child = dontmanage.qb.DocType(self.subcontract_data.order_supplied_items_field)
						...
						item_wh = dontmanage._dict(item_wh)
			...
			if self.work_order and self.purpose == "Manufacture":
				work_order = dontmanage.get_doc("Work Order", self.work_order)
	...
	def load_items_from_bom(self):
		if self.work_order:
			...
			item_code = dontmanage.db.get_value("BOM", self.bom_no, "item")
	...
	def get_bom_raw_materials(self, qty):
		...
		for item in item_dict.values():
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = dontmanage.db.get_value(
	...
	def get_bom_scrap_material(self, qty):
		...
		if (
			dontmanage.db.get_single_value("Manufacturing Settings", "set_op_cost_and_scrape_from_sub_assemblies")
			and self.work_order
	...
	def get_scrap_items_from_job_card(self):
		...
		job_card = dontmanage.qb.DocType("Job Card")
		job_card_scrap_item = dontmanage.qb.DocType("Job Card Scrap Item")

		scrap_items = (
			dontmanage.qb.from_(job_card)
			.select(
		...
		for row in scrap_items:
			...
			if cint(dontmanage.get_cached_value("UOM", row.stock_uom, "must_be_whole_number")):
				row.stock_qty = dontmanage.utils.ceil(row.stock_qty)
	...
	def get_unconsumed_raw_materials(self):
		wo = dontmanage.get_doc("Work Order", self.work_order)
		wo_items = dontmanage.get_all(
			"Work Order Item",
			filters={"parent": self.work_order},
	...
	def add_transfered_raw_materials_in_items(self) -> None:
		...
		wo_data = dontmanage.db.get_value(
			"Work Order",
			self.work_order,
		...
		for _key, row in available_materials.items():
			...
			if cint(dontmanage.get_cached_value("UOM", item.stock_uom, "must_be_whole_number")):
	...
	def update_item_in_stock_entry_detail(self, row, item, qty) -> None:
		...
		use_serial_batch_fields = dontmanage.db.get_single_value("Stock Settings", "use_serial_batch_fields")
	...
	def get_pending_raw_materials(self, backflush_based_on=None):
		...
		if not len(item_dict):
			dontmanage.msgprint(_("""All items have already been transferred for this Work Order."""))
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		"""
		item_dict, job_card_items = dontmanage._dict(), []
		work_order = dontmanage.get_doc("Work Order", self.work_order)
		...
		if not dontmanage.db.get_value("Warehouse", work_order.wip_warehouse, "is_group"):
	...
	def get_job_card_item_codes(self, job_card=None):
		...
		job_card_items = dontmanage.get_all(
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		...
		for d in item_dict:
			...
			stock_uom = item_row.get("stock_uom") or dontmanage.db.get_value("Item", d, "stock_uom")
	...
	def validate_with_material_request(self):
		for item in self.get("items"):
			...
			if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
				parent_se = dontmanage.get_value(
					"Stock Entry Detail",
					item.ste_detail,
			...
			if material_request:
				mreq_item = dontmanage.db.get_value(
				...
				if mreq_item.item_code != item.item_code:
					dontmanage.throw(
						...
						dontmanage.MappingMismatchError,
					)
	...
	def validate_batch(self):
		if self.purpose in [
			...
			for item in self.get("items"):
				if item.batch_no:
					disabled = dontmanage.db.get_value("Batch", item.batch_no, "disabled")
					if disabled == 0:
						expiry_date = dontmanage.db.get_value("Batch", item.batch_no, "expiry_date")
						if expiry_date:
	...
	def update_subcontract_order_supplied_items(self):
		if self.get(self.subcontract_data.order_field) and (
			...
			# Get Subcontract Order Supplied Items Details
			order_supplied_items = dontmanage.db.get_all(
				self.subcontract_data.order_supplied_items_field,
			...
			for row in order_supplied_items:
				...
				dontmanage.db.set_value(self.subcontract_data.order_supplied_items_field, row.name, item)
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			for d in self.items:
				...
				transferred_qty = dontmanage.get_all(
					"Stock Entry Detail",
			...
			for data, transferred_qty in stock_entries.items():
				cond += """ WHEN (parent = {} and name = {}) THEN {}
					""".format(
					dontmanage.db.escape(data[0]),
					dontmanage.db.escape(data[1]),
					transferred_qty,
				)
	...
	def set_material_request_transfer_status(self, status):
		...
		if self.outgoing_stock_entry:
			parent_se = dontmanage.get_value("Stock Entry", self.outgoing_stock_entry, "add_to_transit")
		...
		for item in self.items:
			...
			if self.purpose == "Material Transfer" and material_request not in material_requests:
				if self.outgoing_stock_entry and parent_se:
					material_request = dontmanage.get_value(
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from dontmanageerp.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from dontmanageerp.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def move_sample_to_retention_warehouse(company, items):
	...
	stock_entry = dontmanage.new_doc("Stock Entry")
...
def make_stock_in_entry(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		if not dontmanage.db.get_single_value("Stock Settings", "use_serial_batch_fields"):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = dontmanage.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
				warehouse = dontmanage.get_value(
...
def get_work_order_details(work_order, company):
	work_order = dontmanage.get_doc("Work Order", work_order)
...
def get_operating_cost_per_unit(work_order=None, bom_no=None):
	...
	if not operating_cost_per_unit and bom_no:
		bom = dontmanage.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_valuation_rate_for_finished_good_entry(work_order):
	work_order_qty = flt(
		dontmanage.get_cached_value("Work Order", work_order, "material_transferred_for_manufacturing")
	)

	field = "(SUM(total_outgoing_value) / %s) as valuation_rate" % (work_order_qty)

	stock_data = dontmanage.get_all(
		"Stock Entry",
		fields=field,
		filters={
			"docstatus": 1,
...
def get_uom_details(item_code, uom, qty):
	...
	if not conversion_factor:
		dontmanage.msgprint(_("UOM conversion factor required for UOM: {0} in Item: {1}").format(uom, item_code))
		ret = {"uom": ""}
	else:
		ret = {
...
def get_expired_batch_items():
	from dontmanageerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
...
def get_expired_batches():
	...
	expired_batches = dontmanage._dict()
	for row in data:
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no=None):
	...
	max_retain_qty = dontmanage.get_value("Item", item_code, "sample_quantity")
...
def get_supplied_items(
	...
	for row in dontmanage.get_all("Stock Entry", fields=fields, filters=filters):
		...
		if key not in supplied_item_details:
			supplied_item_details.setdefault(
				key, dontmanage._dict({"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
			)
...
def get_items_from_subcontract_order(source_name, target_doc=None):
	from dontmanageerp.controllers.subcontracting_controller import make_rm_stock_entry
...
def create_serial_and_batch_bundle(parent_doc, row, child, type_of_transaction=None):
	...
	if row.serial_nos and row.batches_to_be_consume:
		...
		precision = dontmanage.get_precision("Serial and Batch Entry", "qty")
		doc.has_batch_no = 1
		for batch_no, qty in row.batches_to_be_consume.items():
