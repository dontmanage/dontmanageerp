			...
			if self.fg_completed_qty != total:
				dontmanage.throw(
					_("The finished product {0} quantity {1} and For Quantity {2} cannot be different").format(
						dontmanage.bold(item_code), dontmanage.bold(total), dontmanage.bold(self.fg_completed_qty)
					)
				)
	...
	def check_if_operations_completed(self):
		...
		for d in prod_order.get("operations"):
			...
			if total_completed_qty > flt(completed_qty):
				...
				work_order_link = dontmanage.utils.get_link_to_form("Work Order", self.work_order)
				job_card_link = dontmanage.utils.get_link_to_form("Job Card", job_card)
				dontmanage.throw(
					_(
	...
	def validate_subcontract_order(self):
		...
		if backflush_raw_materials_based_on == "BOM":
			...
			for se_item in self.items:
				...
				if not required_qty:
					bom_no = dontmanage.db.get_value(
	...
	def get_item_details(self, args=None, for_update=False):
		...
		if self.purpose == "Material Issue":
			ret["expense_account"] = (
				...
				or dontmanage.get_cached_value("Company", self.company, "default_expense_account")
			)
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		for d in work_order.get("required_items"):
			...
			if d.include_item_in_manufacturing:
				...
				if d.source_warehouse and not dontmanage.db.get_value("Warehouse", d.source_warehouse, "is_group"):
					...
					if disabled == 0:
						...
						if expiry_date:
							if getdate(self.posting_date) > getdate(expiry_date):
								dontmanage.throw(_("Batch {0} of Item {1} has expired.").format(item.batch_no, item.item_code))
					else:
						dontmanage.throw(_("Batch {0} of Item {1} is disabled.").format(item.batch_no, item.item_code))
		...
		for item in self.items:
			...
			if self.purpose == "Material Transfer" and material_request not in material_requests:
				if self.outgoing_stock_entry and parent_se:
					material_request = dontmanage.get_value("Stock Entry Detail", item.ste_detail, "material_request")
