...
from dontmanage.utils import add_days, cstr, flt, get_time, getdate, nowtime, today
...
class TestStockEntry(DontManageTestCase):
	def tearDown(self):
	...
	def test_fifo(self):
		...
		self.assertEqual([[1, 10]], dontmanage.safe_eval(sle.stock_queue))
		...
		self.assertEqual([[-2, 10]], dontmanage.safe_eval(sle.stock_queue))

		# move stock to positive
		make_stock_entry(item_code=item_code, target=warehouse, qty=3, basic_rate=20)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]
		self.assertEqual([[1, 20]], dontmanage.safe_eval(sle.stock_queue))

		# incoming entry with diff rate
		make_stock_entry(item_code=item_code, target=warehouse, qty=1, basic_rate=30)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]

		self.assertEqual([[1, 20], [1, 30]], dontmanage.safe_eval(sle.stock_queue))

		dontmanage.db.set_default("allow_negative_stock", 0)
	...
	def test_auto_material_request_for_variant(self):
		...
		make_item_variant()
		template = dontmanage.get_doc("Item", "_Test Variant Item")
	...
	def _test_auto_material_request(
		...
		variant = dontmanage.get_doc("Item", item_code)

		projected_qty, actual_qty = dontmanage.db.get_value(
		...
		projected_qty = (
			dontmanage.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "projected_qty") or 0
		)
	...
	def test_repack_multiple_fg(self):
		...
		repack = dontmanage.copy_doc(test_records[3])
	...
	def test_repack_with_additional_costs(self):
		...
		expenses_included_in_valuation = dontmanage.get_value(
	...
	def test_serial_move(self):
		...
		se.insert()
		se.submit()
		self.assertTrue(dontmanage.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC")

		se.cancel()
		self.assertTrue(dontmanage.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC")
	...
	def test_warehouse_company_validation(self):
		dontmanage.db.get_value("Warehouse", "_Test Warehouse 2 - _TC1", "company")
		dontmanage.get_doc("User", "test2@example.com").add_roles(
			"Sales User", "Sales Manager", "Stock User", "Stock Manager"
		)
		dontmanage.set_user("test2@example.com")

		from dontmanageerp.stock.utils import InvalidWarehouseCompany

		st1 = dontmanage.copy_doc(test_records[0])
	...
	def test_warehouse_user(self):
		...
		st1.get("items")[0].t_warehouse = "_Test Warehouse 2 - _TC1"
		self.assertRaises(dontmanage.PermissionError, st1.insert)
	...
	def test_work_order(self):
		...
		bom_no, bom_operation_cost = dontmanage.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "is_default": 1, "docstatus": 1}, ["name", "operating_cost"]
		)

		work_order = dontmanage.new_doc("Work Order")
		work_order.update(
			{
				"company": "_Test Company",
	...
	def test_work_order_manufacture_with_material_consumption(self):
		...
		bom_no = dontmanage.db.get_value("BOM", {"item": "_Test FG Item", "is_default": 1, "docstatus": 1})
		...
		s = dontmanage.get_doc(_make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		...
		s = dontmanage.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		s.save()
		rm_cost = 0
		for d in s.get("items"):
			if d.s_warehouse:
	...
	def test_variant_work_order(self):
		bom_no = dontmanage.db.get_value("BOM", {"item": "_Test Variant Item", "is_default": 1, "docstatus": 1})
		...
		from dontmanageerp.manufacturing.doctype.work_order.work_order import make_stock_entry

		stock_entry = dontmanage.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		stock_entry.insert()
	...
	def test_quality_check_for_scrap_item(self):
		...
		bom_name = dontmanage.db.get_value("BOM Scrap Item", {"docstatus": 1}, "parent")
		production_item = dontmanage.db.get_value("BOM", bom_name, "item")
		...
		work_order.update(
			{
				...
				"bom_no": bom_name,
				"qty": 1.0,
				"stock_uom": dontmanage.db.get_value("Item", production_item, "stock_uom"),
				"skip_transfer": 1,
			}
		)
		...
		stock_entry = dontmanage.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		for row in stock_entry.items:
			...
			if row.is_scrap_item:
				...
				row.uom = dontmanage.db.get_value("Item", scrap_item, "stock_uom")
				row.stock_uom = dontmanage.db.get_value("Item", scrap_item, "stock_uom")
	...
	def test_quality_check(self):
		...
		repack.insert()
		self.assertRaises(dontmanage.ValidationError, repack.submit)
	...
	def test_zero_incoming_rate(self):
		...
		value_diff = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_gle_for_opening_stock_entry(self):
		...
		is_opening = dontmanage.db.get_value(
			"GL Entry",
	...
	def test_additional_cost_distribution_manufacture(self):
		se = dontmanage.get_doc(
			doctype="Stock Entry",
			purpose="Manufacture",
			additional_costs=[dontmanage._dict(base_amount=100)],
			items=[
				dontmanage._dict(item_code="RM", basic_amount=10),
				dontmanage._dict(item_code="FG", basic_amount=20, t_warehouse="X", is_finished_item=1),
				dontmanage._dict(item_code="scrap", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_additional_cost_distribution_non_manufacture(self):
		se = dontmanage.get_doc(
			...
			additional_costs=[dontmanage._dict(base_amount=100)],
			items=[
				dontmanage._dict(item_code="RECEIVED_1", basic_amount=20, t_warehouse="X"),
				dontmanage._dict(item_code="RECEIVED_2", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_independent_manufacture_entry(self):
		...
		se = dontmanage.get_doc(
			...
			items=[
				...
				dontmanage._dict(item_code="_Test FG Item", qty=4, t_warehouse="_Test Warehouse 1 - _TC"),
			],
		)
	...
	def test_multi_batch_value_diff(self):
		...
		receipt.append(
			"items", dontmanage.copy_doc(receipt.items[0], ignore_no_copy=False).update({"basic_rate": 20})
		)
		receipt.save()
		receipt.submit()
	...
	def test_transfer_qty_validation(self):
		...
		self.assertRaises(dontmanage.ValidationError, se.save)

	def test_mapped_stock_entry(self):
		"Check if rate and stock details are populated in mapped SE given warehouse."
		from dontmanageerp.stock.doctype.purchase_receipt.purchase_receipt import make_stock_entry
	...
	def test_reposting_for_depedent_warehouse(self):
		...
		repost_name = dontmanage.db.get_value(
			"Repost Item Valuation", {"voucher_no": receipt2.name, "docstatus": 1}, "name"
		)

		doc = dontmanage.get_doc("Repost Item Valuation", repost_name)
	...
	def test_batch_expiry(self):
		from dontmanageerp.controllers.stock_controller import BatchExpiredError
		from dontmanageerp.stock.doctype.batch.test_batch import make_new_batch
		...
		batch = make_new_batch(
			batch_id=dontmanage.generate_hash("", 5), item_code=item_doc.name, expiry_date=add_days(today(), -1)
		)
	...
	def test_serial_batch_bundle_type_of_transaction(self):
		...
		dontmanage.db.set_value("Serial and Batch Bundle", sbb, "type_of_transaction", "Inward")
		self.assertRaises(dontmanage.ValidationError, se.submit)
...
test_records = dontmanage.get_test_records("Stock Entry")


def initialize_records_for_future_negative_sle_test(
	item_code, batch_no, warehouses, opening_qty, posting_date
):
	from dontmanageerp.stock.doctype.batch.test_batch import TestBatch, make_new_batch
	from dontmanageerp.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
		create_stock_reconciliation,
	)
	from dontmanageerp.stock.doctype.warehouse.test_warehouse import create_warehouse
