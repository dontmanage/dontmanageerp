...
dontmanage.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		dontmanage.db.get_single_value('Stock Settings', 'disable_serial_no_and_batch_selector')
		.then((value) => {
			if (value) {
		...
		dontmanage.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					dontmanage.msgprint(__("Retention Stock Entry already created or Sample Quantity not provided"));
				}
			}
		});
	},
...
dontmanageerp.stock.StockEntry = class StockEntry extends dontmanageerp.stock.StockController {
	...
	clean_up() {
		...
		if(this.frm.doc.work_order &&
			...
			dontmanage.model.remove_from_locals("Work Order",
...
function check_should_not_attach_bom_items(bom_no) {
  return (
	bom_no === undefined ||
	(dontmanageerp.stock.bom && dontmanageerp.stock.bom.name === bom_no)
  );
}
