// Copyright (c) 2015, DontManage and Contributors // License: GNU General Public License v3. See license.txt
...
dontmanage.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		dontmanage.db.get_single_value("Stock Settings", "disable_serial_no_and_batch_selector").then((value) => {
			if (value) {
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new dontmanageerp.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "dontmanageerp.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function (frm) {
		...
		if (!frm.doc.docstatus) {
			...
			frm.add_custom_button(
				__("Material Request"),
				function () {
					dontmanage.model.with_doctype("Material Request", function () {
						var mr = dontmanage.model.get_new_doc("Material Request");
						...
						items.forEach(function (item) {
							var mr_item = dontmanage.model.add_child(mr, "items");
							...
							mr_item.required_date = dontmanage.datetime.nowdate();
						});
						dontmanage.set_route("Form", "Material Request", mr.name);
					});
				},
				__("Create")
			);
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					dontmanageerp.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					dontmanage.model.open_mapped_doc({
						method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(
					...
					function () {
						...
						dontmanage.set_route("List", "Stock Entry");
					},
					__("View")
				);
			}
		...
		if (frm.doc.docstatus === 0) {
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = dontmanageerp.utils.map_current_doc({
						method: "dontmanageerp.stock.doctype.material_request.material_request.make_stock_entry",
	...
	validate_purpose_consumption: function (frm) {
		dontmanage
			.call({
				method: "dontmanageerp.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
				if (cint(r.message) == 0 && frm.doc.purpose == "Material Consumption for Manufacture") {
					frm.set_value("purpose", "Manufacture");
					dontmanage.throw(__("Material Consumption is not set in Manufacturing Settings."));
				}
			});
	},
	...
	company: function (frm) {
		if (frm.doc.company) {
			var company_doc = dontmanage.get_doc(":Company", frm.doc.company);
	...
	make_retention_stock_entry: function (frm) {
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args: {
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			dontmanage.call({
				...
				callback: function (r) {
					dontmanage.model.set_value(cdt, cdn, "basic_rate", r.message || 0.0);
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			if (!frm.doc.company) dontmanage.throw(__("Company field is required"));
			dontmanage.call({
				method: "dontmanageerp.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						dontmanage.throw(__("BOM does not contain any stock item"));
					} else {
						dontmanageerp.utils.remove_empty_first_row(frm, "items");
						$.each(r.message, function (i, item) {
							let d = dontmanage.model.add_child(cur_frm.doc, "Stock Entry Detail", "items");
	...
	calculate_total_additional_costs: function (frm) {
		const total_additional_costs = dontmanage.utils.sum(
	...
	source_warehouse_address: function (frm) {
		dontmanageerp.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		dontmanageerp.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	set_transit_warehouse: function (frm) {
		if (
			...
			dontmanage.db.get_value(dt, dn, "default_in_transit_warehouse", (r) => {
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) dontmanageerp.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			dontmanageerp.utils.map_current_doc({
				method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
dontmanage.ui.form.on("Stock Entry Detail", {
	...
	s_warehouse(frm, cdt, cdn) {
		...
		if (item.s_warehouse) {
			dontmanage.model.set_value(cdt, cdn, "allow_zero_valuation_rate", 0);
		}
	},
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return dontmanage.call({
				method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						dontmanage.model.set_value(cdt, cdn, r.message);
					}
				},
			});
		}
				...
				callback: function (r) {
					if (r.message) {
						...
						$.each(r.message, function (k, v) {
							if (v) {
								dontmanage.model.set_value(cdt, cdn, k, v); // qty and it's subsequent fields weren't triggered
							}
						});
						refresh_field("items");
						...
						if (
							...
							!dontmanage.flags.hide_serial_batch_dialog &&
							!dontmanage.flags.dialog_set
						) {
...
var validate_sample_quantity = function (frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.validate_sample_quantity",
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
			...
			callback: (r) => {
				dontmanage.model.set_value(cdt, cdn, "sample_quantity", r.message);
			},
		});
	}
};
...
dontmanageerp.stock.StockEntry = class StockEntry extends dontmanageerp.stock.StockController {
	setup() {
		var me = this;
		...
		if (me.frm.doc.company && dontmanageerp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
			this.frm.add_fetch("company", "stock_adjustment_account", "expense_account");
		}

		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (dontmanageerp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		dontmanage.dynamic_link = { doc: this.frm.doc, fieldname: "supplier", doctype: "Supplier" };
		this.frm.set_query("supplier_address", dontmanageerp.queries.address_query);
	}
	...
	set_default_account(callback) {
		...
		if (this.frm.doc.company && dontmanageerp.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "dontmanageerp.accounts.utils.get_company_default",
				args: {
	...
	clean_up() {
		...
		if (
			...
			dontmanage.model.remove_from_locals("Work Order", this.frm.doc.work_order);
		}
	}
	...
	get_items() {
		...
		if (!this.frm.doc.fg_completed_qty || !this.frm.doc.bom_no)
			dontmanage.throw(__("BOM and Manufacturing Quantity are required"));
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						dontmanageerp.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return dontmanage.call({
			method: "dontmanageerp.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button() {
		if (dontmanage.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(
				__("Excise Invoice"),
				function () {
					var excise = dontmanage.model.make_new_doc_and_get_name("Journal Entry");
					excise = locals["Journal Entry"][excise];
					excise.voucher_type = "Excise Entry";
					dontmanage.set_route("Form", "Journal Entry", excise.name);
				},
				__("Create")
			);
	}
	...
	items_add(doc, cdt, cdn) {
		var row = dontmanage.get_doc(cdt, cdn);
	...
	items_on_form_rendered(doc, grid_row) {
		dontmanageerp.setup_serial_or_batch_no();
	}
	...
	toggle_related_fields(doc) {
		...
		if (dontmanageerp.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		dontmanageerp.utils.get_party_details(this.frm, null, null, null);
	}
};
...
dontmanageerp.stock.select_batch_and_serial_no = (frm, item) => {
	...
	dontmanage.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			item.has_serial_no = r.message.has_serial_no;
			item.has_batch_no = r.message.has_batch_no;
			item.type_of_transaction = item.s_warehouse ? "Outward" : "Inward";

			new dontmanageerp.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
...
dontmanageerp.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	dontmanage.db.get_doc("BOM", bom_no).then((bom) => {
		const { name, items } = bom;
		dontmanageerp.stock.bom = { name, items: {} };
		items.forEach((item) => {
			dontmanageerp.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (dontmanageerp.stock.bom && dontmanageerp.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new dontmanageerp.stock.StockEntry({ frm: cur_frm }));
