...
import dontmanage
from dontmanage import ValidationError, _
...
from dontmanageerp.stock.get_item_details import get_reserved_qty_for_so
...
class SerialNo(StockController):
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = dontmanage.db.get_value("Serial No", self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				dontmanage.throw(_("Item Code cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				dontmanage.throw(_("Warehouse cannot be changed for Serial No."), SerialNoCannotCannotChangeError)

	def validate_item(self):
		"""
		Validate whether serial no is required for this item
		"""
		item = dontmanage.get_cached_doc("Item", self.item_code)
	...
	def set_purchase_details(self, purchase_sle):
		if purchase_sle:
			...
			if purchase_sle.voucher_type in ("Purchase Receipt", "Purchase Invoice"):
				self.supplier, self.supplier_name = dontmanage.db.get_value(
	...
	def set_sales_details(self, delivery_sle):
		if delivery_sle:
			...
			if delivery_sle.voucher_type in ("Delivery Note", "Sales Invoice"):
				self.customer, self.customer_name = dontmanage.db.get_value(
	...
	if item_det.has_serial_no == 0:
		...
		if serial_nos:
			...
			for serial_no in serial_nos:
				if dontmanage.db.exists("Serial No", serial_no):
					...
					if cint(sle.actual_qty) > 0 and has_serial_no_exists(sr, sle):
						doc_name = dontmanage.bold(get_link_to_form(sr.purchase_document_type, sr.purchase_document_no))
						dontmanage.throw(
							_("Serial No {0} has already been received in the {1} #{2}").format(
								dontmanage.bold(serial_no), sr.purchase_document_type, doc_name
							),
					...
					if (
						...
						return_against = dontmanage.db.get_value(sle.voucher_type, sle.voucher_no, "return_against")
						if return_against and return_against != sr.delivery_document_no:
							dontmanage.throw(_("Serial no {0} has been already returned").format(sr.name))
					...
					if cint(sle.actual_qty) < 0:
						...
						if not sr.purchase_document_no:
							dontmanage.throw(_("Serial No {0} not in stock").format(serial_no), SerialNoNotExistsError)
						...
						if sle.voucher_type in ("Delivery Note", "Sales Invoice"):
							...
							if sr.sales_order:
								if sle.voucher_type == "Sales Invoice":
									...
									if not dontmanage.db.exists(
										...
										if not invoice or dontmanage.db.exists(
											"Sales Invoice Item",
							...
							if sle.voucher_type == "Sales Invoice":
								sales_order = dontmanage.db.get_value(
									"Sales Invoice Item",
									{"parent": sle.voucher_no, "item_code": sle.item_code},
									"sales_order",
								)
								...
								sales_order = dontmanage.get_value(
									"Delivery Note Item",
									{"parent": sle.voucher_no, "item_code": sle.item_code},
									"against_sales_order",
								)
								if sales_order and get_reserved_qty_for_so(sales_order, sle.item_code):
									...
									sales_invoice = dontmanage.get_value(
										"Delivery Note Item",
										{"parent": sle.voucher_no, "item_code": sle.item_code},
										"against_sales_invoice",
									)
...
def check_serial_no_validity_on_cancel(serial_no, sle):
	...
	sr_link = dontmanage.utils.get_link_to_form("Serial No", serial_no)
	doc_link = dontmanage.utils.get_link_to_form(sle.voucher_type, sle.voucher_no)
	...
	if sr and (actual_qty < 0 or is_stock_reco) and (sr.warehouse and sr.warehouse != sle.warehouse):
		...
		msg = _("Cannot cancel {0} {1} as Serial No {2} does not belong to the warehouse {3}").format(
			sle.voucher_type, doc_link, sr_link, dontmanage.bold(sle.warehouse)
		)
		...
		if sr.warehouse:
			...
			msg = _("Cannot cancel {0} {1} as Serial No {2} is active in warehouse {3}").format(
				sle.voucher_type, doc_link, sr_link, dontmanage.bold(sr.warehouse)
			)
			...
			msg = _("Cannot cancel {0} {1} as Serial No {2} does not belong to the company {3}").format(
				sle.voucher_type, doc_link, sr_link, dontmanage.bold(sle.company)
			)

	if msg:
		dontmanage.throw(msg, title=_("Cannot cancel"))
...
def validate_material_transfer_entry(sle_doc):
	...
	if (
		...
		and dontmanage.get_cached_value("Stock Entry", sle_doc.voucher_no, "purpose") == "Material Transfer"
	):
...
def validate_so_serial_no(sr, sales_order):
	if not sr.sales_order or sr.sales_order != sales_order:
		...
		dontmanage.throw(_("""{0} Serial No {1} cannot be delivered""").format(msg, sr.name))
...
def allow_serial_nos_with_different_item(sle_serial_no, sle):
	...
	if sle.voucher_type == "Stock Entry" and cint(sle.actual_qty) > 0:
		stock_entry = dontmanage.get_cached_doc("Stock Entry", sle.voucher_no)
...
def get_new_serial_number(series):
	...
	if dontmanage.db.exists("Serial No", sr_no):
...
def auto_make_serial_nos(args):
	...
	for serial_no in serial_nos:
		is_new = False
		if dontmanage.db.exists("Serial No", serial_no):
			sr = dontmanage.get_cached_doc("Serial No", serial_no)
		elif args.get("actual_qty", 0) > 0:
			sr = dontmanage.new_doc("Serial No")
			is_new = True
	...
	if len(form_links) == 1:
		dontmanage.msgprint(_("Serial No {0} Created").format(form_links[0]), singular_title)
		...
		dontmanage.msgprint(message, multiple_title)
		...
		for sle in stock_ledger_entries:
			if sle.voucher_detail_no == d.name:
				if (
					...
					dontmanage.db.set_value(d.doctype, d.name, "serial_no", sle.serial_no)
					...
					dontmanage.db.set_value(d.doctype, d.name, "rejected_serial_no", sle.serial_no)
...
def update_maintenance_status():
	...
	for serial_no in serial_nos:
		doc = dontmanage.get_doc("Serial No", serial_no[0])
		doc.set_maintenance_status()
		dontmanage.db.set_value("Serial No", doc.name, "maintenance_status", doc.maintenance_status)


def get_delivery_note_serial_no(item_code, qty, delivery_note):
	serial_nos = ""
	dn_serial_nos = dontmanage.db.sql_list(
...
def auto_fetch_serial_number(
	...
	filters = dontmanage._dict({"item_code": item_code, "warehouse": warehouse})
...
def get_delivered_serial_nos(serial_nos):
	...
	SerialNo = dontmanage.qb.DocType("Serial No")
	serial_nos = get_serial_nos(serial_nos)
	query = (
		dontmanage.qb.select(SerialNo.name)
		.from_(SerialNo)
...
def get_pos_reserved_serial_nos(filters):
	...
	POSInvoiceItem = dontmanage.qb.DocType("POS Invoice Item")
	query = (
		dontmanage.qb.from_(POSInvoice)
...
def fetch_serial_numbers(filters, qty, do_not_include=None):
	...
	serial_no = dontmanage.qb.DocType("Serial No")

	query = (
		dontmanage.qb.from_(serial_no)
