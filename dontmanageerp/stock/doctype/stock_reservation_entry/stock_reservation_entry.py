...
from dontmanage.utils import cint, flt, nowdate, nowtime

from dontmanageerp.stock.utils import get_or_make_bin, get_stock_balance
...
class StockReservationEntry(Document):
	...
	def validate_amended_doc(self) -> None:
		...
		if self.amended_from:
			msg = _("Cannot amend {0} {1}, please create a new one instead.").format(
				self.doctype, dontmanage.bold(self.amended_from)
			)
	...
	def validate_group_warehouse(self) -> None:
		...
		if dontmanage.get_cached_value("Warehouse", self.warehouse, "is_group"):
			msg = _("Stock cannot be reserved in group warehouse {0}.").format(dontmanage.bold(self.warehouse))
			dontmanage.throw(msg, title=_("Invalid Warehouse"))

	def validate_uom_is_integer(self) -> None:
		"""Validates `Reserved Qty` with Stock UOM."""

		if cint(dontmanage.db.get_value("UOM", self.stock_uom, "must_be_whole_number", cache=True)):
	...
	def auto_reserve_serial_and_batch(self, based_on: str | None = None) -> None:
		...
		if (
			...
			and cint(dontmanage.db.get_single_value("Stock Settings", "auto_reserve_serial_and_batch"))
		):
			from dontmanageerp.stock.doctype.batch.batch import get_available_batches
			from dontmanageerp.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
			from dontmanageerp.stock.serial_batch_bundle import get_serial_nos_batch
			...
			kwargs = dontmanage._dict(
				{
					...
					"based_on": based_on
					or dontmanage.db.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
				}
			)
			...
			if serial_nos:
				serial_no_wise_batch = dontmanage._dict({})
	...
	def validate_reservation_based_on_serial_and_batch(self) -> None:
		...
		if self.reservation_based_on == "Serial and Batch":
			allow_partial_reservation = dontmanage.db.get_single_value(
			...
			if self.has_serial_no:
				...
				if not available_serial_nos:
					msg = _("Stock not available for Item {0} in Warehouse {1}.").format(
						dontmanage.bold(self.item_code), dontmanage.bold(self.warehouse)
					)
					dontmanage.throw(msg)
				...
				if self.has_serial_no:
					...
					if key not in available_serial_nos:
						msg = _(
							...
							entry.idx,
							dontmanage.bold(entry.serial_no),
							dontmanage.bold(self.item_code),
							_("Batch {0} and Warehouse").format(dontmanage.bold(entry.batch_no))
							if self.has_batch_no
							else _("Warehouse"),
							dontmanage.bold(self.warehouse),
							dontmanage.bold(_("Stock Reservation Entry")),
						)
					...
					if entry.serial_no in selected_serial_nos:
						msg = _("Row #{0}: Serial No {1} is already selected.").format(
							entry.idx, dontmanage.bold(entry.serial_no)
						)
						dontmanage.throw(msg)
					else:
					...
					if cint(dontmanage.db.get_value("Batch", entry.batch_no, "disabled")):
						msg = _(
							"Row #{0}: Stock cannot be reserved for Item {1} against a disabled Batch {2}."
						).format(entry.idx, dontmanage.bold(self.item_code), dontmanage.bold(entry.batch_no))
						dontmanage.throw(msg)
					...
					if available_qty_to_reserve <= 0:
						msg = _(
							...
							entry.idx,
							dontmanage.bold(self.item_code),
							dontmanage.bold(entry.batch_no),
							dontmanage.bold(self.warehouse),
						)
						dontmanage.throw(msg)
					...
					if entry.qty > available_qty_to_reserve:
						if allow_partial_reservation:
							...
							msg = _(
								...
								entry.idx,
								dontmanage.bold(available_qty_to_reserve),
								dontmanage.bold(self.item_code),
								dontmanage.bold(entry.batch_no),
								dontmanage.bold(self.warehouse),
							)
					...
					if entry.batch_no in selected_batch_nos:
						msg = _("Row #{0}: Batch No {1} is already selected.").format(
							entry.idx, dontmanage.bold(entry.batch_no)
						)
						dontmanage.throw(msg)
					else:
	...
	def update_reserved_stock_in_bin(self) -> None:
		...
		bin_doc = dontmanage.get_cached_doc("Bin", bin_name)
	...
	def update_status(self, status: str | None = None, update_modified: bool = True) -> None:
		...
		dontmanage.db.set_value(self.doctype, self.name, "status", status, update_modified=update_modified)
	...
	def validate_with_allowed_qty(self, qty_to_be_reserved: float) -> None:
		...
		if self.voucher_type == "Sales Order":
			delivered_qty, conversion_factor = dontmanage.db.get_value(
		...
		if self.get("_action") != "submit" and self.voucher_type == "Sales Order" and allowed_qty <= 0:
			msg = _("Item {0} is already reserved/delivered against Sales Order {1}.").format(
				dontmanage.bold(self.item_code), dontmanage.bold(self.voucher_no)
			)

			if self.docstatus == 1:
				self.cancel()
				return dontmanage.msgprint(msg)
			else:
				dontmanage.throw(msg)
		...
		if qty_to_be_reserved > allowed_qty:
			...
			""".format(
				dontmanage.bold(allowed_qty),
				self.stock_uom,
				dontmanage.bold(self.item_code),
				self.voucher_type,
				dontmanage.bold(self.voucher_no),
				actual_qty,
...
def validate_stock_reservation_settings(voucher: object) -> None:
	...
	if not dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation"):
		msg = _("Please enable {0} in the {1}.").format(
			dontmanage.bold(_("Stock Reservation")),
			dontmanage.bold(_("Stock Settings")),
		)
		dontmanage.throw(msg)
...
def get_sre_reserved_serial_nos_details(
	...
	return dontmanage._dict(query.run())
...
def get_ssb_bundle_for_voucher(sre: dict) -> object:
	...
	if sb_entries:
		bundle = dontmanage.new_doc("Serial and Batch Bundle")
...
def create_stock_reservation_entries_for_so_items(
	...
	from dontmanageerp.selling.doctype.sales_order.sales_order import get_unreserved_qty

	if not from_voucher_type and (
		sales_order.get("_action") == "submit"
		and sales_order.set_warehouse
		and cint(dontmanage.get_cached_value("Warehouse", sales_order.set_warehouse, "is_group"))
	):
		return dontmanage.msgprint(
			_("Stock cannot be reserved in the group warehouse {0}.").format(
				dontmanage.bold(sales_order.set_warehouse)
			)
		)

	validate_stock_reservation_settings(sales_order)

	allow_partial_reservation = dontmanage.db.get_single_value("Stock Settings", "allow_partial_reservation")

	items = []
	if items_details:
		for item in items_details:
			so_item = dontmanage.get_doc("Sales Order Item", item.get("sales_order_item"))
	...
	for item in items if items_details else sales_order.get("items"):
		...
		is_stock_item, has_serial_no, has_batch_no = dontmanage.get_cached_value(
		...
		if dontmanage.get_cached_value("Warehouse", item.warehouse, "is_group"):
			dontmanage.msgprint(
				_("Row #{0}: Stock cannot be reserved in group warehouse {1}.").format(
					item.idx, dontmanage.bold(item.warehouse)
				),
				title=_("Stock Reservation"),
				indicator="yellow",
			)
		...
		if available_qty_to_reserve <= 0:
			dontmanage.msgprint(
				_("Row #{0}: Stock not available to reserve for the Item {1} in Warehouse {2}.").format(
					item.idx, dontmanage.bold(item.item_code), dontmanage.bold(item.warehouse)
				),
				title=_("Stock Reservation"),
				indicator="orange",
			)
		...
		if qty_to_be_reserved < unreserved_qty:
			if not from_voucher_type and (
				...
				msg = _("Row #{0}: Only {1} available to reserve for the Item {2}").format(
					item.idx,
					dontmanage.bold(str(qty_to_be_reserved / item.conversion_factor) + " " + item.uom),
					dontmanage.bold(item.item_code),
				)
				dontmanage.msgprint(msg, title=_("Stock Reservation"), indicator="orange")
			...
			if not allow_partial_reservation:
				if qty_to_be_reserved == flt(item.get("qty_to_reserve")):
					...
					dontmanage.msgprint(msg, title=_("Partial Stock Reservation"), indicator="yellow")

				continue

		sre = dontmanage.new_doc("Stock Reservation Entry")
		...
		if item.get("serial_and_batch_bundle"):
			sbb = dontmanage.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
			sre.reservation_based_on = "Serial and Batch"
	...
	if sre_count and notify:
		dontmanage.msgprint(_("Stock Reservation Entries Created"), alert=True, indicator="green")
...
def cancel_stock_reservation_entries(
	...
	if sre_list:
		for sre in sre_list:
			dontmanage.get_doc("Stock Reservation Entry", sre).cancel()

		if notify:
			msg = _("Stock Reservation Entries Cancelled")
			dontmanage.msgprint(msg, alert=True, indicator="red")
