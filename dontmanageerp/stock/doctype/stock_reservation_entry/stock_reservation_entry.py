				...
				if self.has_serial_no:
					...
					if key not in available_serial_nos:
						msg = _(
							...
							dontmanage.bold("Stock Reservation Entry"),
						)
					...
					if cint(dontmanage.db.get_value("Batch", entry.batch_no, "disabled")):
						msg = _(
							...
							entry.idx, dontmanage.bold(self.item_code), dontmanage.bold(entry.batch_no)
						)
						dontmanage.throw(msg)
...
def validate_stock_reservation_settings(voucher: object) -> None:
	...
	if not dontmanage.db.get_single_value("Stock Settings", "enable_stock_reservation"):
		msg = _("Please enable {0} in the {1}.").format(
			dontmanage.bold("Stock Reservation"), dontmanage.bold("Stock Settings")
		)
		dontmanage.throw(msg)
