...
from dontmanageerp.selling.doctype.sales_order.sales_order import create_pick_list, make_delivery_note
...
from dontmanageerp.stock.doctype.stock_entry.stock_entry import StockEntry
...
class TestStockReservationEntry(DontManageTestCase):
	def setUp(self) -> None:
	...
	def test_validate_stock_reservation_settings(self) -> None:
		...
		with change_settings("Stock Settings", {"enable_stock_reservation": 0}):
			self.assertRaises(dontmanage.ValidationError, validate_stock_reservation_settings, voucher)
	...
	def test_auto_reserve_serial_and_batch(self) -> None:
		...
		for item in dn.items:
			if item.serial_and_batch_bundle:
				...
				delivered_sb_entries = dontmanage.db.get_all(
		...
		for item in so.items:
			...
			if sre_details.reservation_based_on == "Serial and Batch":
				sb_entries = dontmanage.db.get_all(
					"Serial and Batch Entry",
	...
	def test_stock_reservation_from_pick_list(self) -> None:
		...
		for location in pl.locations:
			...
			if location.serial_and_batch_bundle:
				picked_sb_entries = dontmanage.db.get_all(
					"Serial and Batch Entry",
	...
	def test_stock_reservation_from_purchase_receipt(self) -> None:
		...
		from dontmanageerp.selling.doctype.sales_order.sales_order import make_material_request
		...
		for item in pr.items:
			sre, status, reserved_qty = dontmanage.db.get_value(
			...
			if item.serial_and_batch_bundle:
				sb_details = dontmanage.db.get_all(
					"Serial and Batch Entry",
					filters={"parent": item.serial_and_batch_bundle},
					fields=["serial_no", "batch_no", "qty"],
					as_list=True,
				)
				reserved_sb_details = dontmanage.db.get_all(
					"Serial and Batch Entry",
					filters={"parent": sre},
					fields=["serial_no", "batch_no", "qty"],
					as_list=True,
	...
	def test_consider_reserved_stock_while_cancelling_an_inward_transaction(self) -> None:
		...
		self.assertRaises(dontmanage.ValidationError, se.cancel)
...
def create_material_receipt(
	...
	se = dontmanage.new_doc("Stock Entry")
	se.purpose = "Material Receipt"
	se.company = "_Test Company"
	cost_center = dontmanage.get_value("Company", se.company, "cost_center")
	expense_account = dontmanage.get_value("Company", se.company, "stock_adjustment_account")
...
def cancel_all_stock_reservation_entries() -> None:
	sre_list = dontmanage.db.get_all("Stock Reservation Entry", filters={"docstatus": 1}, pluck="name")
...
def make_stock_reservation_entry(**args):
	doc = dontmanage.new_doc("Stock Reservation Entry")
