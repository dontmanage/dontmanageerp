...
class StockSettings(Document):
	...
	def validate(self):
		for key in [
			...
			dontmanage.db.set_default(key, self.get(key, ""))
	...
	def validate_use_batch_wise_valuation(self):
		...
		if self.valuation_method == "FIFO":
			dontmanage.throw(_("Cannot disable batch wise valuation for FIFO valuation method."))

		if dontmanage.get_all(
			"Item", filters={"valuation_method": "FIFO", "is_stock_item": 1, "has_batch_no": 1}, limit=1
		):
			dontmanage.throw(_("Can't disable batch wise valuation for items with FIFO valuation method."))

		if dontmanage.get_all("Batch", filters={"use_batchwise_valuation": 1}, limit=1):
			dontmanage.throw(_("Can't disable batch wise valuation for active batches."))

	def validate_warehouses(self):
		warehouse_fields = ["default_warehouse", "sample_retention_warehouse"]
		for field in warehouse_fields:
			if dontmanage.db.get_value("Warehouse", self.get(field), "is_group"):
				dontmanage.throw(
					_(
						"Group Warehouses cannot be used in transactions. Please change the value of {0}"
					).format(dontmanage.bold(self.meta.get_field(field).label)),
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) and not int(self.db_get("clean_description_html") or 0):
			# changed to text
			dontmanage.enqueue(
				"dontmanageerp.stock.doctype.stock_settings.stock_settings.clean_all_descriptions",
			...
			if self.allow_negative_stock and self.enable_stock_reservation:
				dontmanage.throw(
					_("As {0} is enabled, you can not enable {1}.").format(
						dontmanage.bold(_("Stock Reservation")), dontmanage.bold(_("Allow Negative Stock"))
					)
				)
		...
		if self.has_value_changed("enable_stock_reservation"):
			...
			if self.enable_stock_reservation:
				...
				if self.allow_negative_stock:
					dontmanage.throw(
						_("As {0} is enabled, you can not enable {1}.").format(
							dontmanage.bold(_("Allow Negative Stock")), dontmanage.bold(_("Stock Reservation"))
						)
					)

				else:
					# Don't allow if there are negative stock
					from dontmanage.query_builder.functions import Round

					precision = dontmanage.db.get_single_value("System Settings", "float_precision") or 3
			...
			# Enable -> Disable
				...
				has_reserved_stock = dontmanage.db.exists(
	...
	def make_property_setter_for_precision(doctypes):
		for doctype in doctypes:
			if property_name := dontmanage.db.exists(
				"Property Setter",
				{"doc_type": doctype, "field_name": "conversion_factor", "property": "precision"},
			):
				dontmanage.db.set_value("Property Setter", property_name, "value", 9)
				continue
...
def clean_all_descriptions():
	for item in dontmanage.get_all("Item", ["name", "description"]):
		if item.description:
			clean_description = clean_html(item.description)
		if item.description != clean_description:
			dontmanage.db.set_value("Item", item.name, "description", clean_description)
