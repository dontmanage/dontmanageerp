...
from dontmanage.utils import add_to_date, get_datetime, get_time_str, time_diff_in_hours
...
class StockRepostingSettings(Document):
	...
	def convert_to_item_wh_reposting(self):
		...
		for ledger in get_stock_ledgers(vouchers):
			...
			elif dontmanage.utils.getdate(item_warehouses.get(key)) > dontmanage.utils.getdate(ledger.posting_date):
				item_warehouses[key] = ledger.posting_date
		...
		for row in reposting_data:
			dontmanage.db.set_value("Repost Item Valuation", row.name, "status", "Skipped")

		self.db_set("item_based_reposting", 1)
		dontmanage.msgprint(_("Item Warehouse based reposting has been enabled."))
