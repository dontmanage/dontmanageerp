...
class ItemVariantSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.variant_field.variant_field import VariantField
	...
	def set_default_fields(self):
		self.fields = []
		fields = dontmanage.get_meta("Item").fields
		exclude_fields = {
			"naming_series",
			"item_code",
			"item_name",
	...
	def validate(self):
		for d in self.fields:
			if d.field_name in self.invalid_fields_for_copy_fields_in_variants:
				dontmanage.throw(_("Cannot set the field <b>{0}</b> for copying in variants").format(d.field_name))
