...
from dontmanageerp.controllers.accounts_controller import get_taxes_and_charges, merge_taxes
from dontmanageerp.controllers.selling_controller import SellingController
from dontmanageerp.stock.doctype.serial_no.serial_no import get_delivery_note_serial_no
...
class DeliveryNote(SellingController):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.delivery_note_item.delivery_note_item import DeliveryNoteItem
	...
	def before_print(self, settings=None):
		...
		item_meta = dontmanage.get_meta("Delivery Note Item")
		print_hide_fields = {
	...
	def so_required(self):
		...
		if dontmanage.db.get_single_value("Selling Settings", "so_required") == "Yes":
			for d in self.get("items"):
				if not d.against_sales_order:
					dontmanage.throw(_("Sales Order required for Item {0}").format(d.item_code))
	...
	def set_serial_and_batch_bundle_from_pick_list(self):
		...
		for item in self.items:
			...
			if item.pick_list_item and not item.serial_and_batch_bundle:
				...
				bundle_id = dontmanage.db.get_value("Serial and Batch Bundle", filters, "name")
			...
			if missing_label and missing_label != "No Label":
				errors.append(
					_("The field {0} in row {1} is not set").format(
						dontmanage.bold(_(missing_label)), dontmanage.bold(item.idx)
					)
				)

		if errors:
			dontmanage.throw("<br>".join(errors), title=error_title)
	...
	def update_stock_reservation_entries(self) -> None:
		...
		if self._action == "submit":
			for item in self.get("items"):
				...
				sre_list = dontmanage.db.get_all(
				...
				for sre in sre_list:
					...
					sre_doc = dontmanage.get_doc("Stock Reservation Entry", sre)
			...
			if not item.warehouse:
				...
				if item.warehouse not in reserved_warehouses:
					msg = _("Row #{0}: Stock is reserved for item {1} in warehouse {2}.").format(
						...
						dontmanage.bold(reserved_warehouses[0])
						if len(reserved_warehouses) == 1
						else _("{0} and {1}").format(
							dontmanage.bold(", ".join(reserved_warehouses[:-1])),
							dontmanage.bold(reserved_warehouses[-1]),
						),
					)
					dontmanage.throw(msg, title=_("Stock Reservation Warehouse Mismatch"))
	...
	def validate_packed_qty(self):
		...
		if dontmanage.db.exists("Packing Slip", {"docstatus": 1, "delivery_note": self.name}):
				...
				if (
					...
					dontmanage.throw(
						_("Row {0}: Packed Qty must be equal to {1} Qty.").format(
							item.idx, dontmanage.bold(item.doctype)
						)
					)
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			dontmanage.throw(_("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))

		submit_in = dontmanage.db.sql(
			"""select t1.name
		...
		if submit_in:
			dontmanage.throw(_("Installation Note {0} has already been submitted").format(submit_in[0][0]))
	...
	def cancel_packing_slips(self):
		...
		if res:
			for r in res:
				ps = dontmanage.get_doc("Packing Slip", r[0])
				ps.cancel()
			dontmanage.msgprint(_("Packing Slip(s) cancelled"))
	...
	def update_billing_status(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dn_doc = self if (dn == self.name) else dontmanage.get_doc("Delivery Note", dn)
	...
	def make_return_invoice(self):
		try:
			...
			credit_note_link = dontmanage.utils.get_link_to_form("Sales Invoice", return_invoice.name)

			dontmanage.msgprint(_("Credit Note {0} has been created automatically").format(credit_note_link))
		except Exception:
			dontmanage.throw(
				_(
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	si_item = dontmanage.qb.DocType("Sales Invoice Item").as_("si_item")
	sum_amount = Sum(si_item.amount).as_("amount")

	billed_against_so = (
		dontmanage.qb.from_(si_item)
		.select(sum_amount)
		.where(
	...
	dn = dontmanage.qb.DocType("Delivery Note").as_("dn")
	dn_item = dontmanage.qb.DocType("Delivery Note Item").as_("dn_item")
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_agianst_dn = dontmanage.db.sql(
...
def get_invoiced_qty_map(delivery_note):
	...
	for dn_detail, qty in dontmanage.db.sql(
...
def get_returned_qty_map(delivery_note):
	...
	returned_qty_map = dontmanage._dict(
		dontmanage.db.sql(
...
def make_sales_invoice(source_name, target_doc=None, args=None):
	doc = dontmanage.get_doc("Delivery Note", source_name)
	...
	def set_missing_values(source, target):
		...
		if len(target.get("items")) == 0:
			dontmanage.throw(_("All these items have already been Invoiced/Returned"))
		...
		source_name,
		{
			...
			"Delivery Note Item": {
				...
				"condition": lambda item: (
					not dontmanage.db.exists("Product Bundle", {"new_item_code": item.item_code, "disabled": 0})
					and flt(item.packed_qty) < flt(item.qty)
				),
			},
			"Packed Item": {
...
def make_shipment(source_name, target_doc=None):
	def postprocess(source, target):
		user = dontmanage.db.get_value(
			"User", dontmanage.session.user, ["email", "full_name", "phone", "mobile_no"], as_dict=1
		)
		...
		target.pickup_contact_person = dontmanage.session.user
...
def update_delivery_note_status(docname, status):
	dn = dontmanage.get_doc("Delivery Note", docname)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		if not target.get("items"):
			dontmanage.throw(_("All items have already been received"))
