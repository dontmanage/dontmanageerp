...
dontmanage.ui.form.on("Delivery Note", {
	...
	print_without_amount: function (frm) {
		dontmanageerp.stock.delivery_note.set_print_hide(frm.doc);
	},
	...
	refresh: function (frm) {
		if (
			...
			dontmanage.model.can_create("Sales Invoice")
		) {
		...
		if (
			...
			dontmanage.model.can_create("Purchase Receipt")
		) {
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						dontmanage.model.open_mapped_doc({
							method: "dontmanageerp.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
dontmanage.ui.form.on("Delivery Note Item", {
...
dontmanageerp.stock.DeliveryNoteController = class DeliveryNoteController extends (
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			dontmanage.model.can_read("Sales Order") &&
		...
		if (!doc.is_return && doc.status != "Closed") {
			if (doc.docstatus == 1 && dontmanage.model.can_create("Shipment")) {
				this.frm.add_custom_button(
					__("Shipment"),
					function () {
						me.make_shipment();
			...
			if (
				...
				doc.docstatus == 1 &&
				dontmanage.model.can_create("Installation Note")
			) {
				this.frm.add_custom_button(
			...
			if (doc.docstatus == 1 && doc.status != "Completed" && dontmanage.model.can_create("Delivery Trip")) {
				this.frm.add_custom_button(
					__("Delivery Trip"),
					function () {
			...
			if (
				...
				dontmanage.model.can_create("Packing Slip")
			) {
				this.frm.add_custom_button(
					__("Packing Slip"),
					function () {
	...
	make_shipment() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		dontmanage.model.open_mapped_doc({
			method: "dontmanageerp.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm,
		});
	}
	...
	update_status(status) {
		...
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new dontmanageerp.stock.DeliveryNoteController({ frm: cur_frm }));
...
dontmanage.ui.form.on("Delivery Note", {
	...
	unhide_account_head: function (frm) {
		...
		var aii_enabled = dontmanageerp.is_perpetual_inventory_enabled(frm.doc.company);
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	},
});

dontmanageerp.stock.delivery_note.set_print_hide = function (doc, cdt, cdn) {
	var dn_fields = dontmanage.meta.docfield_map["Delivery Note"];
	var dn_item_fields = dontmanage.meta.docfield_map["Delivery Note Item"];
