...
from dontmanage.utils import add_days, cstr, flt, getdate, nowdate, nowtime, today
...
from dontmanageerp.stock.doctype.delivery_note.delivery_note import (
	make_delivery_trip,
	make_sales_invoice,
)
...
class TestDeliveryNote(DontManageTestCase):
	def test_over_billing_against_dn(self):
		...
		self.assertRaises(dontmanage.ValidationError, dontmanage.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		dontmanage.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
		...
		sle = dontmanage.get_doc("Stock Ledger Entry", {"voucher_type": "Delivery Note", "voucher_no": dn.name})
	...
	def test_delivery_note_gl_entry_packing_item(self):
		dontmanage.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = dontmanage.get_doc("Delivery Note", dn1.name)
	...
	def test_sales_return_against_serial_batch_bundle(self):
		...
		for row in return_dn.items:
			total_qty = dontmanage.db.get_value(
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from dontmanageerp.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(dontmanage.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
	...
	def test_sales_order_reference_validation(self):
		...
		self.assertRaises(dontmanage.ValidationError, dn.save)
		dn.reload()
	...
	def test_delivery_note_bundle_with_batched_item(self):
		...
		reposting_entries = dontmanage.get_all("Repost Item Valuation", filters={"docstatus": 1})
		for entry in reposting_entries:
			doc = dontmanage.get_doc("Repost Item Valuation", entry.name)
			doc.cancel()
			doc.delete()

		dontmanage.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 1)

		dn.reload()
		dn.delete()

		bundle = dontmanage.db.get_value("Serial and Batch Bundle", {"voucher_detail_no": packed_name}, "name")
		self.assertFalse(bundle)

		dontmanage.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		dontmanage.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 0)
		...
		dontmanage.get_doc(
			{
				...
				"title": dontmanage.generate_hash(),
				"apply_on": "Item Code",
	...
	def test_internal_transfer_precision_gle(self):
		...
		self.assertFalse(dontmanage.db.exists("GL Entry", {"voucher_no": dn.name, "voucher_type": dn.doctype}))
	...
	def test_delivery_note_legacy_serial_no_valuation(self):
		...
		sle_data = dontmanage.get_all(
			"Stock Ledger Entry",
			filters={"voucher_no": se_doc.name, "voucher_type": "Stock Entry"},
		)[0]

		sle_doc = dontmanage.get_doc("Stock Ledger Entry", sle_data.name)
	...
	def test_partial_sales_return_batch_no_for_batched_item_in_dn(self):
		...
		returned_batch_no = get_batch_from_bundle(dn_return.items[0].serial_and_batch_bundle)
		self.assertEqual(batch_no, returned_batch_no)
		sabb_qty = dontmanage.db.get_value(
	...
	def test_same_posting_date_and_posting_time(self):
		...
		self.assertRaises(dontmanage.ValidationError, dn5.submit)
	...
	def test_warranty_expiry_date_for_serial_item(self):
		...
		for row in serial_nos:
			sn = dontmanage.get_doc("Serial No", row)
...
def create_delivery_note(**args):
	dn = dontmanage.new_doc("Delivery Note")
	...
	if not args.use_serial_batch_fields and (args.get("batch_no") or args.get("serial_no")):
		...
		if args.get("batches"):
			batches = dontmanage._dict(args.batches)
