...
def make_item(item_code=None, properties=None, uoms=None, barcode=None):
	if not item_code:
		item_code = dontmanage.generate_hash(length=16)

	if dontmanage.db.exists("Item", item_code):
		return dontmanage.get_doc("Item", item_code)
...
class TestItem(DontManageTestCase):
	def setUp(self):
		super().setUp()
		dontmanage.flags.attribute_values = None

	def get_item(self, idx):
		item_code = test_records[idx].get("item_code")
		if not dontmanage.db.exists("Item", item_code):
			item = dontmanage.copy_doc(test_records[idx])
			item.insert()
		else:
	...
	def test_get_item_details(self):
		...
		dontmanage.db.sql("""delete from `tabItem Price`""")
		dontmanage.db.sql("""delete from `tabBin`""")
		...
		currency = dontmanage.get_cached_value("Company", company, "default_currency")
	...
	def test_item_defaults(self):
		dontmanage.delete_doc_if_exists("Item", "Test Item With Defaults", force=1)
		make_item(
	...
	def test_item_default_validations(self):
		with self.assertRaises(dontmanage.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_item_attribute_change_after_variant(self):
		dontmanage.delete_doc_if_exists("Item", "_Test Variant Item-L", force=1)

		variant = create_variant("_Test Variant Item", {"Test Size": "Large"})
		variant.save()

		attribute = dontmanage.get_doc("Item Attribute", "Test Size")
	...
	def test_copy_fields_from_template_to_variants(self):
		dontmanage.delete_doc_if_exists("Item", "_Test Variant Item-XL", force=1)
		...
		if not dontmanage.db.get_value(
			...
			item_attribute = dontmanage.get_doc("Item Attribute", "Test Size")
		...
		variant = dontmanage.get_doc("Item", "_Test Variant Item-XL")
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in dontmanage.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
			dontmanage.delete_doc_if_exists("Item", d.name)

		dontmanage.delete_doc_if_exists("Item", "_Test Numeric Template Item")
		dontmanage.delete_doc_if_exists("Item Attribute", "Test Item Length")
	...
	def test_item_merging(self):
		old = create_item(dontmanage.generate_hash(length=20)).name
		new = create_item(dontmanage.generate_hash(length=20)).name
		...
		dontmanage.rename_doc("Item", old, new, merge=True)

		self.assertFalse(dontmanage.db.exists("Item", old))

		self.assertTrue(dontmanage.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"}))
		self.assertTrue(
			dontmanage.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			dontmanage.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.delete()
		dontmanage.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(dontmanage.db.exists("Item", "Test Item Bundle Item 1"))

	def test_uom_conversion_factor(self):
		if dontmanage.db.exists("Item", "Test Item UOM"):
			dontmanage.delete_doc("Item", "Test Item UOM")
	...
	def test_add_item_barcode(self):
		...
		self.assertRaises(dontmanage.UniqueValidationError, item_doc.save)
	...
	def test_index_creation(self):
		...
		indices = dontmanage.db.sql("show index from tabItem", as_dict=1)
	...
	def test_check_stock_uom_with_bin(self):
		...
		item = dontmanage.get_doc("Item", "_Test Item")
		item.stock_uom = "Gram"
		self.assertRaises(dontmanage.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from dontmanageerp.stock.stock_balance import update_bin_qty
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", dontmanage._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(dontmanage.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
	...
	def test_item_dashboard(self):
		from dontmanageerp.stock.dashboard.item_dashboard import get_data
	...
	def test_customer_codes_length(self):
		...
		for _row in range(3):
			item.append("customer_items", {"ref_code": dontmanage.generate_hash("", 120)})
		item.save()
	...
	def test_update_is_stock_item(self):
		...
		pb = dontmanage.new_doc("Product Bundle")
	...
	def test_serach_fields_for_item(self):
		from dontmanageerp.controllers.queries import item_query
	...
	def test_group_warehouse_for_reorder_item(self):
		...
		warehouse_doc = dontmanage.get_doc("Warehouse", warehouse)
		...
		self.assertRaises(dontmanage.ValidationError, item_doc.save)


def set_item_variant_settings(fields):
	doc = dontmanage.get_doc("Item Variant Settings")
	doc.set("fields", fields)
	doc.save()


def make_item_variant():
	if not dontmanage.db.exists("Item", "_Test Variant Item-S"):
...
test_records = dontmanage.get_test_records("Item")
