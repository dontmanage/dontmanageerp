...
dontmanage.ui.form.on("Item", {
	...
	onload: function(frm) {
		dontmanageerp.item.setup_queries(frm);
	...
	refresh: function(frm) {
		if (frm.doc.is_stock_item) {
			...
			frm.add_custom_button(__("Stock Projected Qty"), function() {
				...
				dontmanage.set_route("query-report", "Stock Projected Qty");
			}, __("View"));
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(__("Show Variants"), function() {
				dontmanage.set_route("List", "Item", {"variant_of": frm.doc.name});
			}, __("View"));

			frm.add_custom_button(__("Item Variant Settings"), function() {
				dontmanage.set_route("Form", "Item Variant Settings");
			}, __("View"));

			frm.add_custom_button(__("Variant Details Report"), function() {
				dontmanage.set_route("query-report", "Item Variant Details", {"item": frm.doc.name});
			}, __("View"));

			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					dontmanageerp.item.show_single_variant_dialog(frm);
				}, __('Create'));
				frm.add_custom_button(__("Multiple Variants"), function() {
					dontmanageerp.item.show_multiple_variants_dialog(frm);
				}, __('Create'));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					dontmanageerp.item.show_modal_for_manufacturers(frm);
				}, __('Create'));
			}
		...
		if (dontmanage.defaults.get_default("item_naming_by")!="Naming Series" || frm.doc.variant_of) {
		...
		dontmanageerp.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			dontmanageerp.item.make_dashboard(frm);
		}

		frm.add_custom_button(__('Duplicate'), function() {
			var new_item = dontmanage.model.copy_doc(frm.doc);
			...
			dontmanage.set_route('Form', 'Item', new_item.name);
		});
	...
	validate: function(frm){
		dontmanageerp.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function(frm) {
		...
		dontmanage.call({
			method: "dontmanageerp.stock.doctype.item.item.get_asset_naming_series",
			callback: function(r) {
	...
	page_name: dontmanage.utils.warn_page_name_change,
...
dontmanage.ui.form.on('Item Reorder', {
...
dontmanage.ui.form.on('Item Customer Detail', {
	customer_items_add: function(frm, cdt, cdn) {
		dontmanage.model.set_value(cdt, cdn, 'customer_group', "");
	},
	...
	customer_group: function(frm, cdt, cdn) {
		if(set_customer_group(frm, cdt, cdn)){
			dontmanage.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	}
});
...
var set_customer_group = function(frm, cdt, cdn) {
	...
	dontmanage.model.with_doc("Customer", row.customer_name, function() {
		var customer = dontmanage.model.get_doc("Customer", row.customer_name);
...
$.extend(dontmanageerp.item, {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			dontmanage.require('item-dashboard.bundle.js', function() {
				const section = frm.dashboard.add_section('', __("Stock Levels"));
				dontmanageerp.item.item_dashboard = new dontmanageerp.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: 'dontmanageerp.stock.dashboard.item_dashboard.get_data',
					template: 'item_dashboard_list'
				});
				dontmanageerp.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function(frm) {
		frm.add_custom_button(__("Add / Edit Prices"), function() {
			dontmanage.set_route("List", "Item Price", {"item_code": frm.doc.name});
		}, __("Actions"));
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			dontmanage.call({
				method: "dontmanageerp.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new dontmanage.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				dontmanage.call({
					method: "dontmanageerp.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find('.form-column'))
				.find('.dontmanage-control')).css('margin-bottom', '0px');
			...
			dontmanage.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						dontmanage.msgprint_dialog = dontmanage.msgprint(__("Item Variant {0} already exists with same attributes",
							[repl('<a href="/app/item/%(item_encoded)s" class="strong variant-click">%(item)s</a>', {
								item_encoded: encodeURIComponent(variant),
								item: variant
							})]
						));
						dontmanage.msgprint_dialog.hide_on_page_refresh = true;
						dontmanage.msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							d.hide();
						});
					} else {
						d.hide();
						dontmanage.call({
							method: "dontmanageerp.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					dontmanage.call({
						method: "dontmanageerp.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
dontmanage.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			dontmanage.call({
				method: "dontmanageerp.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
				...
				callback: function(r) {
					if (!r.exc && r.message) {
						dontmanage.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				}
			});
		}
...
dontmanage.tour['Item'] = [
	...
	},
	{
		...
		description: __("If you are maintaining stock of this Item in your Inventory, DontManageErp will make a stock ledger entry for each transaction of this item.")
	},
	{
...
function open_form(frm, doctype, child_doctype, parentfield) {
	dontmanage.model.with_doctype(doctype, () => {
		...
		dontmanage.run_serially([
			...
			() => {
				dontmanage.flags.ignore_company_party_validation = true;
				dontmanage.model.trigger("item_code", frm.doc.name, new_child_doc);
			}
		])
	});
}
