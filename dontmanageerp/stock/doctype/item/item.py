...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.item_variant_attribute.item_variant_attribute import (
	...
	def validate_item_tax_net_rate_range(self):
		for tax in self.get("taxes"):
			if flt(tax.maximum_net_rate) < flt(tax.minimum_net_rate):
				dontmanage.throw(_("Row #{0}: Maximum Net Rate cannot be greater than Minimum Net Rate"))
				...
				if (d.item_tax_template, d.tax_category) in check_list:
					dontmanage.throw(
						_("{0} entered twice {1} in Item Taxes").format(
							...
							"for tax category {0}".format(dontmanage.bold(d.tax_category)) if d.tax_category else "",
						)
					)
				else:
	...
	def validate_duplicate_product_bundles_before_merge(self, old_name, new_name):
		...
		old_bundle = dontmanage.get_value(
			"Product Bundle", filters={"new_item_code": old_name, "disabled": 0}
		)
		new_bundle = dontmanage.get_value(
...
def get_last_purchase_details(item_code, doc_name=None, conversion_rate=1.0):
	...
	last_purchase_order = dontmanage.db.sql(
		"""\
	...
	last_purchase_receipt = dontmanage.db.sql(
		"""\
