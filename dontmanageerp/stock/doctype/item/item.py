...
from dontmanageerp.setup.doctype.item_group.item_group import invalidate_cache_for
...
class Item(Document):
	...
	def update_website_item(self):
		...
		web_item = dontmanage.db.exists("Website Item", {"item_code": self.item_code})
		...
		if web_item:
			...
			web_item_doc = dontmanage.get_doc("Website Item", web_item)
		...
		for d in self.get("taxes"):
			if d.item_tax_template:
				if d.item_tax_template in check_list:
					dontmanage.throw(_("{0} entered twice in Item Tax").format(d.item_tax_template))
				else:
	...
	def validate_duplicate_product_bundles_before_merge(self, old_name, new_name):
		...
		old_bundle = dontmanage.get_value("Product Bundle", filters={"new_item_code": old_name})
		new_bundle = dontmanage.get_value("Product Bundle", filters={"new_item_code": new_name})
	...
	def validate_duplicate_website_item_before_merge(self, old_name, new_name):
		...
		msg = f"Please delete linked Website Item {dontmanage.bold(web_item_link)} before merging {old_name} into {new_name}"
		dontmanage.throw(_(msg), title=_("Cannot Merge"), exc=DataValidationError)
	...
	def validate_attributes_in_variants(self):
		...
		for docname, attr_list in not_included.items():
			link = "<a href='/app/Form/Item/{0}'>{0}</a>".format(dontmanage.bold(_(docname)))
...
def invalidate_item_variants_cache_for_website(doc):
	...
	from dontmanageerp.e_commerce.variant_selector.item_variants_cache import ItemVariantsCacheManager

	item_code = None
	is_web_item = doc.get("published_in_website") or doc.get("published")
	if doc.has_variants and is_web_item:
		item_code = doc.item_code
	elif doc.variant_of and dontmanage.db.get_value("Item", doc.variant_of, "published_in_website"):
