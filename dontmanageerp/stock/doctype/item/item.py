...
from dontmanage.query_builder.functions import Count, CurDate, UnixTimestamp
...
from dontmanageerp.controllers.item_variant import (
...
class DuplicateReorderRows(dontmanage.ValidationError):
	pass


class StockExistsForTemplate(dontmanage.ValidationError):
	pass


class InvalidBarcode(dontmanage.ValidationError):
	pass


class DataValidationError(dontmanage.ValidationError):
	pass
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from dontmanageerp.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from dontmanageerp.stock.doctype.item_default.item_default import ItemDefault
		from dontmanageerp.stock.doctype.item_reorder.item_reorder import ItemReorder
		from dontmanageerp.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from dontmanageerp.stock.doctype.item_tax.item_tax import ItemTax
		from dontmanageerp.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from dontmanageerp.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
	...
	def autoname(self):
		if dontmanage.db.get_default("item_naming_by") == "Naming Series":
			if self.variant_of:
				if not self.item_code:
					template_item_name = dontmanage.db.get_value("Item", self.variant_of, "item_name")
	...
	def after_insert(self):
		...
		if self.standard_rate:
			for default in self.item_defaults or [dontmanage._dict()]:
	...
	def validate(self):
		...
		if not self.is_new():
			self.old_item_group = dontmanage.db.get_value(self.doctype, self.name, "item_group")
	...
	def validate_description(self):
		...
		if cint(dontmanage.db.get_single_value("Stock Settings", "clean_description_html")):
			self.description = clean_html(self.description)

	def validate_customer_provided_part(self):
		if self.is_customer_provided_item:
			if self.is_purchase_item:
				dontmanage.throw(_('"Customer Provided Item" cannot be Purchase Item also'))
			if self.valuation_rate:
				dontmanage.throw(_('"Customer Provided Item" cannot have Valuation Rate'))
			self.default_material_request_type = "Customer Provided"

	def add_price(self, price_list=None):
		"""Add a new price"""
		if not price_list:
			price_list = dontmanage.db.get_single_value(
				"Selling Settings", "selling_price_list"
			) or dontmanage.db.get_value("Price List", _("Standard Selling"))
		if price_list:
			item_price = dontmanage.get_doc(
				{
					"doctype": "Item Price",
	...
	def set_opening_stock(self):
		...
		if not self.valuation_rate and not self.standard_rate and not self.is_customer_provided_item:
			dontmanage.throw(_("Valuation Rate is mandatory if Opening Stock entered"))
		...
		for default in self.item_defaults or [
			dontmanage._dict({"company": dontmanage.defaults.get_defaults().company})
		]:
			default_warehouse = default.default_warehouse or dontmanage.db.get_single_value(
				"Stock Settings", "default_warehouse"
			)
			if default_warehouse:
				warehouse_company = dontmanage.db.get_value("Warehouse", default_warehouse, "company")

			if not default_warehouse or warehouse_company != default.company:
				default_warehouse = dontmanage.db.get_value(
	...
	def validate_fixed_asset(self):
		if self.is_fixed_asset:
			if self.is_stock_item:
				dontmanage.throw(_("Fixed Asset Item must be a non-stock item."))

			if not self.asset_category:
				dontmanage.throw(_("Asset Category is mandatory for Fixed Asset item"))

			if self.stock_ledger_created():
				dontmanage.throw(_("Cannot be a fixed asset item as Stock Ledger is created."))

		if not self.is_fixed_asset:
			asset = dontmanage.db.get_all("Asset", filters={"item_code": self.name, "docstatus": 1}, limit=1)
			if asset:
				dontmanage.throw(
	...
	def validate_retain_sample(self):
		if self.retain_sample and not dontmanage.db.get_single_value(
			"Stock Settings", "sample_retention_warehouse"
		):
			dontmanage.throw(_("Please select Sample Retention Warehouse in Stock Settings first"))
	...
	def update_template_tables(self):
		template = dontmanage.get_cached_doc("Item", self.variant_of)
	...
	def validate_item_type(self):
		if self.has_serial_no == 1 and self.is_stock_item == 0 and not self.is_fixed_asset:
			dontmanage.throw(_("'Has Serial No' can not be 'Yes' for non-stock item"))
			...
			if series and "#" in series and "." not in series:
				dontmanage.throw(
					_("Invalid naming series (. missing) for {0}").format(
						dontmanage.bold(self.meta.get_field(field).label)
					)
				)

	def check_for_active_boms(self):
		if self.default_bom:
			bom_item = dontmanage.db.get_value("BOM", self.default_bom, "item")
				...
				if (d.item_tax_template, d.tax_category) in check_list:
					dontmanage.throw(
						_("{0} entered twice {1} in Item Taxes").format(
							dontmanage.bold(d.item_tax_template),
							f"for tax category {dontmanage.bold(d.tax_category)}" if d.tax_category else "",
						)
					)
				else:
	...
	def validate_barcode(self):
		...
		if len(self.barcodes) > 0:
			for item_barcode in self.barcodes:
				options = dontmanage.get_meta("Item Barcode").get_options("barcode_type").split("\n")
				if item_barcode.barcode:
					duplicate = dontmanage.db.sql(
					...
					if item_barcode.barcode_type:
						barcode_type = convert_dontmanageerp_to_barcodenumber(
	...
	def validate_warehouse_for_reorder(self):
		...
		_warehouse_before_save = dontmanage._dict()
		...
		for d in self.get("reorder_levels"):
			...
			if d.warehouse_reorder_level and not d.warehouse_reorder_qty:
				dontmanage.throw(_("Row #{0}: Please set reorder quantity").format(d.idx))
	...
	def on_trash(self):
		dontmanage.db.sql("""delete from tabBin where item_code=%s""", self.name)
		dontmanage.db.sql("delete from `tabItem Price` where item_code=%s", self.name)
		for variant_of in dontmanage.get_all("Item", filters={"variant_of": self.name}):
			dontmanage.delete_doc("Item", variant_of.name)

	def before_rename(self, old_name, new_name, merge=False):
		if self.item_name == old_name:
			dontmanage.db.set_value("Item", old_name, "item_name", new_name)
	...
	def after_rename(self, old_name, new_name, merge):
		...
		dontmanage.db.set_value("Item", new_name, "item_code", new_name)
	...
	def validate_duplicate_item_in_stock_reconciliation(self, old_name, new_name):
		...
		msg = _("The items {0} and {1} are present in the following {2} :").format(
			dontmanage.bold(old_name), dontmanage.bold(new_name), document
		)
		...
		).format(dontmanage.bold(old_name))

		dontmanage.throw(_(msg), title=_("Cannot Merge"), exc=DataValidationError)

	def validate_properties_before_merge(self, new_name):
		# Validate properties before merging
		if not dontmanage.db.exists("Item", new_name):
			dontmanage.throw(_("Item {0} does not exist").format(new_name))

		field_list = ["stock_uom", "is_stock_item", "has_serial_no", "has_batch_no"]
		new_properties = [cstr(d) for d in dontmanage.db.get_value("Item", new_name, field_list)]
		...
		if new_properties != [cstr(self.get(field)) for field in field_list]:
			...
			dontmanage.throw(msg, title=_("Cannot Merge"), exc=DataValidationError)

	def validate_duplicate_product_bundles_before_merge(self, old_name, new_name):
		"Block merge if both old and new items have product bundles."
		old_bundle = dontmanage.get_value("Product Bundle", filters={"new_item_code": old_name, "disabled": 0})
		new_bundle = dontmanage.get_value("Product Bundle", filters={"new_item_code": new_name, "disabled": 0})

		if old_bundle and new_bundle:
			bundle_link = get_link_to_form("Product Bundle", old_bundle)
			old_name, new_name = dontmanage.bold(old_name), dontmanage.bold(new_name)
	...
	def set_last_purchase_rate(self, new_name):
		...
		dontmanage.db.set_value("Item", new_name, "last_purchase_rate", last_purchase_rate)

	def recalculate_bin_qty(self, new_name):
		from dontmanageerp.stock.stock_balance import repost_stock

		existing_allow_negative_stock = dontmanage.db.get_value("Stock Settings", None, "allow_negative_stock")
		dontmanage.db.set_single_value("Stock Settings", "allow_negative_stock", 1)

		repost_stock_for_warehouses = dontmanage.get_all(
			"Stock Ledger Entry",
			"warehouse",
		...
		dontmanage.db.delete("Bin", {"item_code": new_name})

		for warehouse in repost_stock_for_warehouses:
			repost_stock(new_name, warehouse)

		dontmanage.db.set_single_value("Stock Settings", "allow_negative_stock", existing_allow_negative_stock)
	...
	def validate_item_defaults(self):
		...
		if len(companies) != len(self.item_defaults):
			dontmanage.throw(_("Cannot set multiple Item Defaults for a company."))
	...
	def update_defaults_from_item_group(self):
		...
		item_defaults = dontmanage.db.get_values(
			"Item Default",
		...
		if item_defaults:
			...
			defaults = dontmanage.defaults.get_defaults() or {}
	...
	def update_variants(self):
		if self.flags.dont_update_variants or dontmanage.db.get_single_value(
			"Item Variant Settings", "do_not_update_variants"
		):
			return
		if self.has_variants:
			variants = dontmanage.db.get_all("Item", fields=["item_code"], filters={"variant_of": self.name})
			if variants:
				if len(variants) <= 30:
					update_variants(variants, self, publish_progress=False)
					dontmanage.msgprint(_("Item Variants updated"))
				else:
					dontmanage.enqueue(
						"dontmanageerp.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
	...
	def validate_has_variants(self):
		if not self.has_variants and dontmanage.db.get_value("Item", self.name, "has_variants"):
			if dontmanage.db.exists("Item", {"variant_of": self.name}):
				dontmanage.throw(_("Item has variants."))
	...
	def validate_attributes_in_variants(self):
		...
		items = [item["name"] for item in dontmanage.get_all("Item", {"variant_of": self.name})]
		...
		item_attributes = dontmanage.get_all(
		...
		for docname, attr_list in not_included.items():
			link = f"<a href='/app/item/{docname}'>{dontmanage.bold(docname)}</a>"
		...
		dontmanage.throw(message, title=_("Variant Attribute Error"), is_minimizable=True, wide=True)
	...
	def validate_variant_based_on_change(self):
		if not self.is_new() and (
			self.variant_of or (self.has_variants and dontmanage.get_all("Item", {"variant_of": self.name}))
		):
			if self.variant_based_on != dontmanage.db.get_value("Item", self.name, "variant_based_on"):
				dontmanage.throw(_("Variant Based On cannot be changed"))

	def validate_uom(self):
		if not self.is_new():
			check_stock_uom_with_bin(self.name, self.stock_uom)
		if self.has_variants:
			for d in dontmanage.db.get_all("Item", filters={"variant_of": self.name}):
				check_stock_uom_with_bin(d.name, self.stock_uom)
		if self.variant_of:
			template_uom = dontmanage.db.get_value("Item", self.variant_of, "stock_uom")
	...
	def validate_attributes(self):
		...
		if self.variant_based_on == "Item Attribute":
			...
			if not self.attributes:
				dontmanage.throw(_("Attribute table is mandatory"))
	...
	def cant_change(self):
		...
		values = dontmanage.db.get_value("Item", self.name, restricted_fields, as_dict=True)
		if not values:
			return

		if not values.get("valuation_method") and self.get("valuation_method"):
			values["valuation_method"] = (
				dontmanage.db.get_single_value("Stock Settings", "valuation_method") or "FIFO"
			)
		...
		if linked_doc := self._get_linked_submitted_documents(changed_fields):
			changed_field_labels = [dontmanage.bold(self.meta.get_label(f)) for f in changed_fields]
			msg = _(
			...
			if linked_doc and isinstance(linked_doc, dict):
				...
				msg += _("Example of a linked document: {0}").format(
					dontmanage.get_desk_link(linked_doc.doctype, linked_doc.docname)
				)

			dontmanage.throw(msg, title=_("Linked with submitted documents"))
	...
	def _get_linked_submitted_documents(self, changed_fields: list[str]) -> dict[str, str] | None:
		...
		for doctype in linked_doctypes:
			...
			if doctype in ("Product Bundle", "BOM"):
				...
				if linked_doc := dontmanage.db.get_value(doctype, filters, fieldname, as_dict=True):
				...
				if linked_doc := dontmanage.db.get_value(
					"Stock Ledger Entry",
			...
			elif linked_doc := dontmanage.db.get_value(
				doctype,
				filters,
	...
	def validate_auto_reorder_enabled_in_stock_settings(self):
		if self.reorder_levels:
			enabled = dontmanage.db.get_single_value("Stock Settings", "auto_indent")
			if not enabled:
				dontmanage.msgprint(
...
def convert_dontmanageerp_to_barcodenumber(dontmanageerp_number, barcode):
	if dontmanageerp_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if dontmanageerp_number in convert:
		return convert[dontmanageerp_number]
...
def validate_end_of_life(item_code, end_of_life=None, disabled=None):
	if (not end_of_life) or (disabled is None):
		end_of_life, disabled = dontmanage.db.get_value("Item", item_code, ["end_of_life", "disabled"])
	...
	if disabled:
		dontmanage.throw(_("Item {0} is disabled").format(item_code))


def validate_is_stock_item(item_code, is_stock_item=None):
	if not is_stock_item:
		is_stock_item = dontmanage.db.get_value("Item", item_code, "is_stock_item")

	if is_stock_item != 1:
		dontmanage.throw(_("Item {0} is not a stock Item").format(item_code))


def validate_cancelled_item(item_code, docstatus=None):
	if docstatus is None:
		docstatus = dontmanage.db.get_value("Item", item_code, "docstatus")

	if docstatus == 2:
		dontmanage.throw(_("Item {0} is cancelled").format(item_code))
...
def get_purchase_voucher_details(doctype, item_code, document_name):
	parent_doc = dontmanage.qb.DocType(doctype)
	child_doc = dontmanage.qb.DocType(doctype + " Item")

	query = (
		dontmanage.qb.from_(parent_doc)
...
def check_stock_uom_with_bin(item, stock_uom):
	if stock_uom == dontmanage.db.get_value("Item", item, "stock_uom"):
		return

	ref_uom = dontmanage.db.get_value("Stock Ledger Entry", {"item_code": item}, "stock_uom")
	...
	dontmanage.db.sql("""update `tabBin` set stock_uom=%s where item_code=%s""", (stock_uom, item))


def get_item_defaults(item_code, company):
	item = dontmanage.get_cached_doc("Item", item_code)
	...
	for d in item.item_defaults:
		if d.company == company:
			if not d.get(fieldname):
				dontmanage.db.set_value(d.doctype, d.name, fieldname, value)
			return
...
def get_item_details(item_code, company=None):
	out = dontmanage._dict()
	if company:
		out = get_item_defaults(item_code, company) or dontmanage._dict()

	doc = dontmanage.get_cached_doc("Item", item_code)
...
def get_uom_conv_factor(uom, stock_uom):
	...
	intermediate_match = dontmanage.db.sql(
		"""
...
def get_item_attribute(parent, attribute_value=""):
	...
	if not dontmanage.has_permission("Item"):
		dontmanage.throw(_("No Permission"))
...
def update_variants(variants, template, publish_progress=True):
	...
	for count, d in enumerate(variants, start=1):
		variant = dontmanage.get_doc("Item", d)
		copy_attributes_to_variant(template, variant)
		variant.save()
		if publish_progress:
			dontmanage.publish_progress(count / total * 100, title=_("Updating Variants..."))
	...
	for item_default in item_defaults:
		for doctype, field in [
			...
			if item_default.get(field):
				company = dontmanage.db.get_value(doctype, item_default.get(field), "company", cache=True)
				if company and company != item_default.company:
					dontmanage.throw(
						_("Row #{}: {} {} doesn't belong to Company {}. Please select valid {}.").format(
							item_default.idx,
							doctype,
							dontmanage.bold(item_default.get(field)),
							dontmanage.bold(item_default.company),
							dontmanage.bold(dontmanage.unscrub(field)),
						),
...
def get_asset_naming_series():
	from dontmanageerp.assets.doctype.asset.asset import get_asset_naming_series
