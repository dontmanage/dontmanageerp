...
from dontmanageerp.stock.doctype.delivery_note.delivery_note import make_packing_slip
...
class TestPackingSlip(DontManageTestCase):
	def test_packing_slip(self):
		...
		for item in dn.items:
			if not dontmanage.db.exists("Product Bundle", {"new_item_code": item.item_code}):
		...
		self.assertRaises(dontmanage.exceptions.ValidationError, ps3.save)
		...
		dn.load_from_db()
		self.assertRaises(dontmanage.exceptions.ValidationError, dn.submit)
