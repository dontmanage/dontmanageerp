...
class PackingSlip(StatusUpdater):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.packing_slip_item.packing_slip_item import PackingSlipItem
	...
	def validate_delivery_note(self):
		...
		if cint(dontmanage.db.get_value("Delivery Note", self.delivery_note, "docstatus")) != 0:
			dontmanage.throw(
	...
	def validate_case_nos(self):
		...
		if cint(self.from_case_no) <= 0:
			dontmanage.throw(_("The 'From Package No.' field must neither be empty nor it's value less than 1."))
		elif not self.to_case_no:
			self.to_case_no = self.from_case_no
		elif cint(self.to_case_no) < cint(self.from_case_no):
			dontmanage.throw(_("'To Package No.' cannot be less than 'From Package No.'"))
		else:
			ps = dontmanage.qb.DocType("Packing Slip")
			res = (
	...
	def validate_items(self):
		for item in self.items:
			if item.qty <= 0:
				dontmanage.throw(_("Row {0}: Qty must be greater than 0.").format(item.idx))
			...
			remaining_qty = dontmanage.db.get_value(
			...
			if remaining_qty is None:
				...
				dontmanage.throw(
					_("Row {0}: Qty cannot be greater than {1} for the Item {2}.").format(
						item.idx, dontmanage.bold(remaining_qty), dontmanage.bold(item.item_code)
					)
				)
	...
	def set_missing_values(self):
		...
		for item in self.items:
			stock_uom, weight_per_unit, weight_uom = dontmanage.db.get_value(
