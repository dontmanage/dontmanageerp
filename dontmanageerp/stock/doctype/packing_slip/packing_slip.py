...
class PackingSlip(Document):
	...
	def validate_delivery_note(self):
		...
		if cint(dontmanage.db.get_value("Delivery Note", self.delivery_note, "docstatus")) != 0:
			dontmanage.throw(_("Delivery Note {0} must not be submitted").format(self.delivery_note))

	def validate_items_mandatory(self):
		rows = [d.item_code for d in self.get("items")]
		if not rows:
			dontmanage.msgprint(_("No Items to pack"), raise_exception=1)
	...
	def validate_case_nos(self):
		...
		if not cint(self.from_case_no):
			dontmanage.msgprint(_("Please specify a valid 'From Case No.'"), raise_exception=1)
		elif not self.to_case_no:
			self.to_case_no = self.from_case_no
		elif cint(self.from_case_no) > cint(self.to_case_no):
			dontmanage.msgprint(_("'To Case No.' cannot be less than 'From Case No.'"), raise_exception=1)
	...
	def get_details_for_packing(self):
		...
		* No. of Cases of this packing slip
		...
		custom_fields = ", ".join(
			...
			for d in dontmanage.get_meta("Delivery Note Item").get_custom_fields()
	...
	def update_item_details(self):
		...
		for d in self.get("items"):
			res = dontmanage.db.get_value("Item", d.item_code, ["weight_per_unit", "weight_uom"], as_dict=True)
	...
	def get_recommended_case_no(self):
		...
		note
		"""
		recommended_case_no = dontmanage.db.sql(
	...
	def get_items(self):
		...
		custom_fields = dontmanage.get_meta("Delivery Note Item").get_custom_fields()
