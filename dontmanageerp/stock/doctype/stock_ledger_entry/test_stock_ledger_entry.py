...
import dontmanage
from dontmanage.core.page.permission_manager.permission_manager import reset
...
from dontmanage.utils import add_days, add_to_date, flt, today

from dontmanageerp.accounts.doctype.gl_entry.gl_entry import rename_gle_sle_docs
...
from dontmanageerp.stock.doctype.stock_ledger_entry.stock_ledger_entry import BackDatedStockTransaction
...
class TestStockLedgerEntry(DontManageTestCase, StockTestMixin):
	def setUp(self):
		items = create_items()
		reset("Stock Entry")
	...
	def test_item_cost_reposting(self):
		...
		target_wh_sle = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": "_Test Item for Reposting",
				"warehouse": "Finished Goods - _TC",
		...
		finished_item_sle = dontmanage.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_purchase_return_valuation_reposting(self):
		...
		# check sle
		outgoing_rate, stock_value_difference = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_fifo_dependent_consumption(self):
		...
		for rate in rates[1:]:
			row = dontmanage.copy_doc(receipt.items[0], ignore_no_copy=False)
			row.basic_rate = rate
		...
		for _ in rates[1:]:
			row = dontmanage.copy_doc(transfer.items[0], ignore_no_copy=False)
	...
	def test_fifo_multi_item_repack_consumption(self):
		...
		for _ in rates[1:]:
			row = dontmanage.copy_doc(repack.items[0], ignore_no_copy=False)
	...
	def test_tie_breaking(self):
		from dontmanageerp.stock.doctype.repost_item_valuation.repost_item_valuation import repost_entries

		dontmanage.flags.dont_execute_stock_reposts = True
		self.addCleanup(dontmanage.flags.pop, "dont_execute_stock_reposts")
...
def create_repack_entry(**args):
	...
	repack = dontmanage.new_doc("Stock Entry")
...
def create_product_bundle_item(new_item_code, packed_items):
	if not dontmanage.db.exists("Product Bundle", new_item_code):
		item = dontmanage.new_doc("Product Bundle")
...
class TestDeferredNaming(DontManageTestCase):
	@classmethod
	def setUpClass(cls) -> None:
		super().setUpClass()
		cls.gle_autoname = dontmanage.get_meta("GL Entry").autoname
		cls.sle_autoname = dontmanage.get_meta("Stock Ledger Entry").autoname
	...
	def get_gle_sles(se):
		...
		gle = set(dontmanage.get_list("GL Entry", filters, pluck="name"))
		sle = set(dontmanage.get_list("Stock Ledger Entry", filters, pluck="name"))
		return gle, sle
