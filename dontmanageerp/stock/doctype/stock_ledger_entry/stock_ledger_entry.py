...
from dontmanage.core.doctype.role.role import get_users
from dontmanage.model.document import Document
from dontmanage.utils import add_days, cint, flt, formatdate, get_datetime, getdate

from dontmanageerp.accounts.utils import get_fiscal_year
from dontmanageerp.controllers.item_variant import ItemTemplateCannotHaveStock
from dontmanageerp.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
from dontmanageerp.stock.serial_batch_bundle import SerialBatchBundle
from dontmanageerp.stock.stock_ledger import get_previous_sle


class StockFreezeError(dontmanage.ValidationError):
	pass


class BackDatedStockTransaction(dontmanage.ValidationError):
	pass
...
class StockLedgerEntry(Document):
	...
	def autoname(self):
		...
		"""
		self.name = dontmanage.generate_hash(txt="", length=10)
	...
	def validate_inventory_dimension_negative_stock(self):
		...
		flt_precision = cint(dontmanage.db.get_default("float_precision")) or 2
		if sle:
	...
	def throw_validation_error(self, diff, dimensions):
		...
		msg = _(
			...
			abs(diff),
			dontmanage.get_desk_link("Item", self.item_code),
			dontmanage.get_desk_link("Warehouse", self.warehouse),
			dimension_msg,
			self.posting_date,
			self.posting_time,
			dontmanage.get_desk_link(self.voucher_type, self.voucher_no),
		)

		dontmanage.throw(msg, title=_("Inventory Dimension Negative Stock"))
	...
	def on_submit(self):
		...
		if dontmanage.flags.in_test and dontmanage.flags.ignore_serial_batch_bundle_validation:
			return
	...
	def validate_mandatory(self):
		...
		for k in mandatory:
			if not self.get(k):
				dontmanage.throw(_("{0} is required").format(self.meta.get_label(k)))

		if self.voucher_type != "Stock Reconciliation" and not self.actual_qty:
			dontmanage.throw(_("Actual Qty is mandatory"))

	def validate_serial_batch_no_bundle(self):
		if self.is_cancelled == 1:
			return

		item_detail = dontmanage.get_cached_value(
			"Item",
			self.item_code,
	...
	def throw_error_message(self, message, exception=dontmanage.ValidationError):
		dontmanage.throw(_(message), exception)

	def check_stock_frozen_date(self):
		stock_settings = dontmanage.get_cached_doc("Stock Settings")

		if stock_settings.stock_frozen_upto:
			if (
				getdate(self.posting_date) <= getdate(stock_settings.stock_frozen_upto)
				and stock_settings.stock_auth_role not in dontmanage.get_roles()
			):
				dontmanage.throw(
		...
		if stock_frozen_upto_days:
			...
			if older_than_x_days_ago and stock_settings.stock_auth_role not in dontmanage.get_roles():
				dontmanage.throw(
	...
	def validate_batch(self):
		if self.batch_no and self.voucher_type != "Stock Entry":
			...
			expiry_date = dontmanage.db.get_value("Batch", self.batch_no, "expiry_date")
			if expiry_date:
				if getdate(self.posting_date) > getdate(expiry_date):
					dontmanage.throw(_("Batch {0} of Item {1} has expired.").format(self.batch_no, self.item_code))

	def validate_and_set_fiscal_year(self):
		if not self.fiscal_year:
			self.fiscal_year = get_fiscal_year(self.posting_date, company=self.company)[0]
		else:
			from dontmanageerp.accounts.utils import validate_fiscal_year
	...
	def block_transactions_against_group_warehouse(self):
		from dontmanageerp.stock.utils import is_group_warehouse

		is_group_warehouse(self.warehouse)

	def validate_with_last_transaction_posting_time(self):
		authorized_role = dontmanage.db.get_single_value(
			"Stock Settings", "role_allowed_to_create_edit_back_dated_transactions"
		)
		if authorized_role:
			authorized_users = get_users(authorized_role)
			if authorized_users and dontmanage.session.user not in authorized_users:
				last_transaction_time = dontmanage.db.sql(
					"""
				...
				if last_transaction_time and get_datetime(cur_doc_posting_datetime) < get_datetime(
					...
					msg = _("Last Stock Transaction for item {0} under warehouse {1} was on {2}.").format(
						dontmanage.bold(self.item_code), dontmanage.bold(self.warehouse), dontmanage.bold(last_transaction_time)
					)

					msg += "<br><br>" + _(
						"You are not authorized to make/edit Stock Transactions for Item {0} under warehouse {1} before this time."
					).format(dontmanage.bold(self.item_code), dontmanage.bold(self.warehouse))

					msg += "<br><br>" + _("Please contact any of the following users to {} this transaction.")
					msg += "<br>" + "<br>".join(authorized_users)
					dontmanage.throw(msg, BackDatedStockTransaction, title=_("Backdated Stock Entry"))
...
def on_doctype_update():
	dontmanage.db.add_index("Stock Ledger Entry", ["voucher_no", "voucher_type"])
	dontmanage.db.add_index("Stock Ledger Entry", ["batch_no", "item_code", "warehouse"])
	dontmanage.db.add_index("Stock Ledger Entry", ["warehouse", "item_code"], "item_warehouse")
	dontmanage.db.add_index("Stock Ledger Entry", ["posting_datetime", "creation"])
