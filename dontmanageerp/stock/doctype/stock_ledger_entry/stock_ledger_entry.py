...
class InventoryDimensionNegativeStockError(dontmanage.ValidationError):
	pass
...
class StockLedgerEntry(Document):
	...
	def throw_validation_error(self, diff, dimension, dimension_value):
		msg = _(
			...
			dontmanage.bold(dimension),
			dontmanage.bold(dimension_value),
			self.posting_date,
			self.posting_time,
			...
			if authorized_users and dontmanage.session.user not in authorized_users:
				...
				if last_transaction_time and get_datetime(cur_doc_posting_datetime) < get_datetime(
					...
					msg = _("Last Stock Transaction for item {0} under warehouse {1} was on {2}.").format(
						...
						dontmanage.bold(last_transaction_time),
					)
