...
from dontmanage.utils import add_days, cint, formatdate, get_datetime, getdate
...
class StockLedgerEntry(Document):
	...
	def on_submit(self):
		...
		if not self.get("via_landed_cost_voucher"):
			from dontmanageerp.stock.doctype.serial_no.serial_no import process_serial_no
	...
	def calculate_batch_qty(self):
		if self.batch_no:
			...
			dontmanage.db.set_value("Batch", self.batch_no, "batch_qty", batch_qty)
	...
	def validate_item(self):
		...
		if item_det.is_stock_item != 1:
			dontmanage.throw(_("Item {0} must be a stock Item").format(self.item_code))
		...
		if item_det.has_batch_no == 1:
			...
			if not self.batch_no:
				dontmanage.throw(_("Batch number is mandatory for Item {0}").format(batch_item))
			elif not dontmanage.db.get_value("Batch", {"item": self.item_code, "name": self.batch_no}):
				dontmanage.throw(
			...
			dontmanage.throw(_("The Item {0} cannot have Batch").format(self.item_code))
