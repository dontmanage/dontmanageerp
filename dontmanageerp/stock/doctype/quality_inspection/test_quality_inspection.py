...
from dontmanageerp.controllers.stock_controller import (
...
# test_records = dontmanage.get_test_records('Quality Inspection')


class TestQualityInspection(DontManageTestCase):
	def setUp(self):
		super().setUp()
		create_item("_Test Item with QA")
		dontmanage.db.set_value("Item", "_Test Item with QA", "inspection_required_before_delivery", 1)
	...
	def test_qa_for_delivery(self):
		...
		dontmanage.db.set_value("Quality Inspection", qa.name, "status", "Accepted")
		dn.reload()
		dn.submit()
	...
	def test_make_quality_inspections_from_linked_document(self):
		...
		for qi in quality_inspections:
			dontmanage.delete_doc("Quality Inspection", qi)
		dn.delete()
	...
	def test_rejected_qi_validation(self):
		...
		dontmanage.db.set_single_value("Stock Settings", "action_if_quality_inspection_is_rejected", "Stop")
		se.reload()
		self.assertRaises(
			QualityInspectionRejectedError, se.submit
		)  # when blocked in Stock settings, block rejected QI

		dontmanage.db.set_single_value("Stock Settings", "action_if_quality_inspection_is_rejected", "Warn")
		se.reload()
	...
	def test_diff_number_format(self):
		self.assertEqual(dontmanage.db.get_default("number_format"), "#.###,##")  # sanity check
...
def create_quality_inspection(**args):
	...
	qa = dontmanage.new_doc("Quality Inspection")
	...
	qa.sample_size = 1
	qa.inspected_by = dontmanage.session.user
