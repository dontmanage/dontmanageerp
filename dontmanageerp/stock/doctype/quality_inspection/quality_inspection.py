...
from dontmanage.utils import cint, cstr, flt, get_link_to_form, get_number_format_info

from dontmanageerp.stock.doctype.quality_inspection_template.quality_inspection_template import (
...
class QualityInspection(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanageerp.stock.doctype.quality_inspection_reading.quality_inspection_reading import (
	...
	def validate(self):
		...
		if self.inspection_type == "In Process" and self.reference_type == "Job Card":
			item_qi_template = dontmanage.db.get_value("Item", self.item_code, "quality_inspection_template")
	...
	def set_child_row_reference(self):
		...
		child_row_references = dontmanage.get_all(
			doctype,
	...
	def distribute_child_row_reference(self, child_row_references):
		quality_inspections = dontmanage.get_all(
			"Quality Inspection",
			filters={
	...
	def validate_inspection_required(self):
		if self.reference_type in ["Purchase Receipt", "Purchase Invoice"] and not dontmanage.get_cached_value(
		...
		if self.reference_type in ["Delivery Note", "Sales Invoice"] and not dontmanage.get_cached_value(
	...
	def get_item_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = dontmanage.db.get_value(
	...
	def get_quality_inspection_template(self):
		template = ""
		if self.bom_no:
			template = dontmanage.db.get_value("BOM", self.bom_no, "quality_inspection_template")

		if not template:
			template = dontmanage.db.get_value("BOM", self.item_code, "quality_inspection_template")
	...
	def validate_readings_status_mandatory(self):
		for reading in self.readings:
			if not reading.status:
				dontmanage.throw(_("Row #{0}: Status is mandatory").format(reading.idx))
	...
	def update_qc_reference(self):
		...
		if self.reference_type == "Job Card":
			...
			if doctype and self.reference_name:
				child_doc = dontmanage.qb.DocType(doctype)

				query = (
					dontmanage.qb.update(child_doc)
	...
	def set_status_based_on_acceptance_formula(self, reading):
		...
		try:
			result = dontmanage.safe_eval(condition, None, data)
			reading.status = "Accepted" if result else "Rejected"
		except NameError as e:
			field = dontmanage.bold(e.args[0].split()[1])
			dontmanage.throw(
				_(
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if not from_doctype or not dontmanage.db.exists("DocType", from_doctype):
		return []
...
def make_quality_inspection(source_name, target_doc=None):
	def postprocess(source, doc):
		doc.inspected_by = dontmanage.session.user
