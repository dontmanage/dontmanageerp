...
dontmanage.ui.form.on('Quick Stock Balance', {
	...
	check_warehouse_and_date: (frm) => {
		dontmanage.msgprint(__('Please enter Warehouse and Date'));
		frm.doc.item = '';
		frm.refresh();
	},
	...
	item: (frm) => {
		dontmanage.flags.last_updated_element = 'item';
		frm.trigger('get_stock_and_item_details');
		frm.trigger('make_custom_stock_report_button');
	},

	item_barcode: (frm) => {
		dontmanage.flags.last_updated_element = 'item_barcode';
		frm.trigger('get_stock_and_item_details');
		frm.trigger('make_custom_stock_report_button');
	},
	...
	get_stock_and_item_details: (frm) => {
		if (!(frm.doc.warehouse && frm.doc.date)) {
			...
			if (dontmanage.flags.last_updated_element === 'item') {
			...
			dontmanage.call({
				method: 'dontmanageerp.stock.doctype.quick_stock_balance.quick_stock_balance.get_stock_item_details',
				args: filters,
				callback: (r) => {
					if (r.message) {
