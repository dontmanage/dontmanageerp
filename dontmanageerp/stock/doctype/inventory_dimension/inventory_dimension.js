...
dontmanage.ui.form.on('Inventory Dimension', {
	setup(frm) {
	...
	set_query_on_fields(frm) {
		...
		frm.set_query('document_type', () => {
			return {
				query: 'dontmanageerp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_documents',
			};
		});
	},
	...
	set_parent_fields(frm) {
		if (frm.doc.apply_to_all_doctypes) {
			...
			dontmanage.call({
				method: 'dontmanageerp.stock.doctype.inventory_dimension.inventory_dimension.get_parent_fields',
				args: {
	...
	delete_dimension(frm) {
		...
		dontmanage.confirm(__(msg, [frm.doc.name.bold()]), () => {
			dontmanage.call({
				method: 'dontmanageerp.stock.doctype.inventory_dimension.inventory_dimension.delete_dimension',
				args: {
					dimension: frm.doc.name
				},
				callback: function() {
					dontmanage.set_route('List', 'Inventory Dimension');
				}
			});
		});
	}
