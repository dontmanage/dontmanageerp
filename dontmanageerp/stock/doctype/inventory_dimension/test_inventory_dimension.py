...
from dontmanage.utils import nowdate, nowtime
...
from dontmanageerp.stock.doctype.stock_ledger_entry.stock_ledger_entry import InventoryDimensionNegativeStockError
from dontmanageerp.stock.doctype.warehouse.test_warehouse import create_warehouse


class TestInventoryDimension(DontManageTestCase):
	def setUp(self):
		prepare_test_data()
	...
	def test_delete_inventory_dimension(self):
		...
		custom_field = dontmanage.db.get_value(
	...
	def test_inventory_dimension(self):
		dontmanage.local.document_wise_inventory_dimensions = {}
		...
		sle_data = dontmanage.db.get_value(
		...
		sle_shelf = dontmanage.db.get_value("Stock Ledger Entry", {"voucher_no": outward.name}, "shelf")
	...
	def test_inventory_dimension_for_purchase_receipt_and_delivery_note(self):
		...
		sle_rack = dontmanage.db.get_value(
			"Stock Ledger Entry",
	...
	def test_check_standard_dimensions(self):
		...
		self.assertFalse(
			dontmanage.db.get_value("Custom Field", {"fieldname": "project", "dt": "Stock Ledger Entry"}, "name")
		)
	...
	def test_for_purchase_sales_and_stock_transaction(self):
		...
		if not dontmanage.db.exists("Store", "Rejected Store"):
			dontmanage.get_doc({"doctype": "Store", "store_name": "Rejected Store"}).insert(
	...
	def test_validate_negative_stock_for_inventory_dimension(self):
		dontmanage.local.inventory_dimensions = {}
...
def create_store_dimension():
	if not dontmanage.db.exists("DocType", "Store"):
		dontmanage.get_doc(
			{
				"doctype": "DocType",
				"name": "Store",
	...
	for store in ["Store 1", "Store 2"]:
		if not dontmanage.db.exists("Store", store):
			dontmanage.get_doc({"doctype": "Store", "store_name": store}).insert(ignore_permissions=True)


def prepare_test_data():
	if not dontmanage.db.exists("DocType", "Shelf"):
		dontmanage.get_doc(
			{
				"doctype": "DocType",
				"name": "Shelf",
	...
	for shelf in ["Shelf 1", "Shelf 2"]:
		if not dontmanage.db.exists("Shelf", shelf):
			dontmanage.get_doc({"doctype": "Shelf", "shelf_name": shelf}).insert(ignore_permissions=True)

	create_warehouse("Shelf Warehouse")

	if not dontmanage.db.exists("DocType", "Rack"):
		dontmanage.get_doc(
			{
				"doctype": "DocType",
				"name": "Rack",
	...
	for rack in ["Rack 1"]:
		if not dontmanage.db.exists("Rack", rack):
			dontmanage.get_doc({"doctype": "Rack", "rack_name": rack}).insert(ignore_permissions=True)

	create_warehouse("Rack Warehouse")

	if not dontmanage.db.exists("DocType", "Pallet"):
		dontmanage.get_doc(
			{
				"doctype": "DocType",
				"name": "Pallet",
	...
	if not dontmanage.db.exists("DocType", "Inv Site"):
		dontmanage.get_doc(
			{
				"doctype": "DocType",
				"name": "Inv Site",
	...
	for site in ["Site 1", "Site 2"]:
		if not dontmanage.db.exists("Inv Site", site):
			dontmanage.get_doc({"doctype": "Inv Site", "site_name": site}).insert(ignore_permissions=True)


def create_inventory_dimension(**args):
	args = dontmanage._dict(args)

	if dontmanage.db.exists("Inventory Dimension", args.dimension_name):
		return dontmanage.get_doc("Inventory Dimension", args.dimension_name)

	doc = dontmanage.new_doc("Inventory Dimension")
	doc.update(args)
...
def prepare_data_for_internal_transfer():
	...
	cost_center = dontmanage.db.get_value("Company", company, "cost_center") or dontmanage.db.get_value(
		"Cost Center", {"company": company}, "name"
	)

	expene_account = dontmanage.db.get_value(
		"Company", company, "stock_adjustment_account"
	) or dontmanage.db.get_value("Account", {"company": company, "account_type": "Expense Account"}, "name")
	...
	return dontmanage._dict(
		{
			...
			"store_warehouse": dontmanage.db.get_value(
