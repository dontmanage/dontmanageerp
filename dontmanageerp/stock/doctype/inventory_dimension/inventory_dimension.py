...
class DoNotChangeError(dontmanage.ValidationError):
	pass


class CanNotBeChildDoc(dontmanage.ValidationError):
	pass


class CanNotBeDefaultDimension(dontmanage.ValidationError):
	pass
...
class InventoryDimension(Document):
	...
	def onload(self):
		if not self.is_new() and dontmanage.db.has_column("Stock Ledger Entry", self.target_fieldname):
	...
	def do_not_update_document(self):
		...
		for field in dontmanage.get_meta("Inventory Dimension").fields:
			if field.fieldname not in allow_to_edit_fields and old_doc.get(field.fieldname) != self.get(
				...
				dontmanage.throw(_(msg), DoNotChangeError)
	...
	def delete_custom_fields(self):
		...
		for field in dontmanage.get_all("Custom Field", filters=filters):
			dontmanage.delete_doc("Custom Field", field.name)

		msg = f"Deleted custom fields related to the dimension {self.name}"
		dontmanage.msgprint(_(msg))
	...
	def validate_reference_document(self):
		if dontmanage.get_cached_value("DocType", self.reference_document, "istable") == 1:
			msg = f"The reference document {self.reference_document} can not be child table."
			dontmanage.throw(_(msg), CanNotBeChildDoc)
		...
		if self.reference_document in ["Batch", "Serial No", "Warehouse", "Item"]:
			...
			dontmanage.throw(_(msg), CanNotBeDefaultDimension)
...
def field_exists(doctype, fieldname) -> str or None:
	return dontmanage.db.get_value("DocField", {"parent": doctype, "fieldname": fieldname}, "name")
...
def get_evaluated_inventory_dimension(doc, sl_dict, parent_doc=None):
	...
	for row in dimensions:
		...
		if row.condition and dontmanage.safe_eval(row.condition, evals):
...
def get_document_wise_inventory_dimensions(doctype) -> dict:
	if not hasattr(dontmanage.local, "document_wise_inventory_dimensions"):
		dontmanage.local.document_wise_inventory_dimensions = {}

	if not dontmanage.local.document_wise_inventory_dimensions.get(doctype):
		dimensions = dontmanage.get_all(
			"Inventory Dimension",
			fields=[
				"name",
				"source_fieldname",
		...
		dontmanage.local.document_wise_inventory_dimensions[doctype] = dimensions

	return dontmanage.local.document_wise_inventory_dimensions[doctype]


@dontmanage.whitelist()
def get_inventory_dimensions():
	if not hasattr(dontmanage.local, "inventory_dimensions"):
		dontmanage.local.inventory_dimensions = {}

	if not dontmanage.local.inventory_dimensions:
		dimensions = dontmanage.get_all(
			"Inventory Dimension",
			fields=[
		...
		dontmanage.local.inventory_dimensions = dimensions

	return dontmanage.local.inventory_dimensions


@dontmanage.whitelist()
def delete_dimension(dimension):
	doc = dontmanage.get_doc("Inventory Dimension", dimension)
	doc.delete()


@dontmanage.whitelist()
def get_parent_fields(child_doctype, dimension_name):
	parent_doctypes = dontmanage.get_all("DocField", fields=["parent"], filters={"options": child_doctype})
