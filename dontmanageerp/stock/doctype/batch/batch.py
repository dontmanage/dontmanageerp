...
class Batch(Document):
	...
	def set_batchwise_valuation(self):
		if self.is_new():
			if dontmanage.db.get_single_value("Stock Settings", "do_not_use_batchwise_valuation"):
	...
	def set_expiry_date(self):
		...
		if not self.expiry_date and has_expiry_date and shelf_life_in_days:
			if (
				...
				self.manufacturing_date = dontmanage.db.get_value(
		...
		if has_expiry_date and not self.expiry_date:
			dontmanage.throw(
				msg=_("Please set {0} for Batched Item {1}, which is used to set {2} on Submit.").format(
					dontmanage.bold(_("Shelf Life in Days")),
					get_link_to_form("Item", self.item),
					dontmanage.bold(_("Batch Expiry Date")),
				),
...
def get_batches_by_oldest(item_code, warehouse):
	...
	batches_dates = [[batch, dontmanage.get_value("Batch", batch.batch_no, "expiry_date")] for batch in batches]
