...
dontmanage.ui.form.on('Batch', {
	...
	item: (frm) => {
		// dontmanage.db.get_value('Item', {name: frm.doc.item}, 'has_expiry_date', (r) => {
		// 	frm.toggle_reqd('expiry_date', r.has_expiry_date);
		// });
		dontmanage.db.get_value('Item', {name: frm.doc.item}, ['shelf_life_in_days', 'has_expiry_date'], (r) => {
			if (r.has_expiry_date && r.shelf_life_in_days) {
				// Calculate expiry date based on shelf_life_in_days
				frm.set_value('expiry_date', dontmanage.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days));
				...
				callback: (r) => {
					...
					rows.find('.btn-move').on('click', function() {
						...
						dontmanage.prompt(
							...
							(data) => {
								dontmanage.call({
									method: 'dontmanageerp.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry',
									args: {
									...
									callback: (r) => {
										dontmanage.show_alert(__('Stock Entry {0} created',
...
dontmanage.ui.form.on('Batch', 'manufacturing_date', function (frm){
	dontmanage.db.get_value('Item', {name: frm.doc.item}, ['shelf_life_in_days', 'has_expiry_date'], (r) => {
		if (r.has_expiry_date && r.shelf_life_in_days) {
			// Calculate expiry date based on shelf_life_in_days
			frm.set_value('expiry_date', dontmanage.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days));
