...
from dontmanage.utils.data import add_to_date, getdate
...
class TestBatch(DontManageTestCase):
	def test_item_has_batch_enabled(self):
		self.assertRaises(
			ValidationError,
			dontmanage.get_doc({"doctype": "Batch", "name": "_test Batch", "item": "_Test Item"}).save,
		)
	...
	def test_batch_stock_levels(self, batch_qty=100):
			...
			SerialBatchCreation(
				{
					...
					"batches": dontmanage._dict({batch_no: 20}),
		...
		batchwise_qty = dontmanage._dict({})
	...
	def test_delivery_note(self):
			...
			SerialBatchCreation(
				{
					...
					"batches": dontmanage._dict({batch_no: batch_qty}),
		...
		delivery_note = dontmanage.get_doc(
			dict(
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 5000,
				"avg_rate": 10,
				"batches": dontmanage._dict({batch_no: 5000}),
				"type_of_transaction": "Outward",
				"company": receipt.company,
			}
		)
	...
	def test_batch_split(self):
		...
		from dontmanageerp.stock.doctype.batch.batch import split_batch
	...
	def test_total_batch_qty(self):
		...
		existing_batch_qty = flt(dontmanage.db.get_value("Batch", "B100", "batch_qty"))
		stock_entry = self.make_new_batch_and_entry("ITEM-BATCH-3", "B100", "_Test Warehouse - _TC")

		current_batch_qty = flt(dontmanage.db.get_value("Batch", "B100", "batch_qty"))
	...
	def make_new_batch_and_entry(cls, item_name, batch_name, warehouse):
		...
		if not dontmanage.db.exists("Batch", batch_name):
			batch = dontmanage.get_doc(dict(doctype="Batch", item=item_name, batch_id=batch_name)).insert(
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 90,
				"avg_rate": 10,
				"batches": dontmanage._dict({batch_name: 90}),
				"type_of_transaction": "Inward",
				"company": "_Test Company",
			}
		).make_serial_and_batch_bundle()
	...
	def test_batch_name_with_naming_series(self):
		stock_settings = dontmanage.get_single("Stock Settings")
		use_naming_series = cint(stock_settings.use_naming_series)

		if not use_naming_series:
			dontmanage.set_value("Stock Settings", "Stock Settings", "use_naming_series", 1)
		...
		if not use_naming_series:
			dontmanage.set_value("Stock Settings", "Stock Settings", "use_naming_series", 0)

	def make_new_batch(self, item_name=None, batch_id=None, do_not_insert=0):
		batch = dontmanage.new_doc("Batch")
	...
	def test_batch_wise_item_price(self):
		if not dontmanage.db.get_value("Item", "_Test Batch Price Item"):
			dontmanage.get_doc(
				{
					"doctype": "Item",
					"is_stock_item": 1,
	...
	def test_basic_batch_wise_valuation(self, batch_qty=100):
		...
		for batch, qty in consumption_plan:
			...
			sle = dontmanage.get_last_doc("Stock Ledger Entry", {"is_cancelled": 0, "voucher_no": se.name})
...
def create_batch(item_code, rate, create_item_price_for_batch):
	...
	batch = dontmanage.db.get_value("Batch", {"item": item_code, "reference_name": pi.name})
...
def make_new_batch(**args):
	...
	if dontmanage.db.exists("Batch", args.batch_id):
		batch = dontmanage.get_doc("Batch", args.batch_id)
	else:
