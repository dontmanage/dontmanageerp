...
dontmanage.ui.form.on("Warehouse", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			...
			if ("Stock Balance" in dontmanage.boot.user.all_reports) {
		...
		if (
			...
			"General Ledger" in dontmanage.boot.user.all_reports
		) {
...
function convert_to_group_or_ledger(frm) {
	dontmanage.call({
		method: "dontmanageerp.stock.doctype.warehouse.warehouse.convert_to_group_or_ledger",
		args: {
