...
from dontmanageerp.stock.doctype.warehouse.warehouse import convert_to_group_or_ledger, get_children

test_records = dontmanage.get_test_records("Warehouse")


class TestWarehouse(DontManageTestCase):
	def setUp(self):
		super().setUp()
	...
	def test_parent_warehouse(self):
		parent_warehouse = dontmanage.get_doc("Warehouse", "_Test Warehouse Group - _TC")
		self.assertEqual(parent_warehouse.is_group, 1)

	def test_warehouse_hierarchy(self):
		p_warehouse = dontmanage.get_doc("Warehouse", "_Test Warehouse Group - _TC")

		child_warehouses = dontmanage.db.sql(
	...
	def test_naming(self):
		...
		wh = dontmanage.get_doc(doctype="Warehouse", warehouse_name=warehouse_name, company=company).insert()
	...
	def test_unlinking_warehouse_from_item_defaults(self):
		...
		for warehouse in warehouse_ids:
			dontmanage.delete_doc("Warehouse", warehouse)

		# Check Item existance
		for item in item_names:
			self.assertTrue(bool(dontmanage.db.exists("Item", item)), f"{item} doesn't exist")
	...
	def test_group_non_group_conversion(self):
		...
		warehouse = dontmanage.get_doc("Warehouse", create_warehouse("TestGroupConversion"))
		...
		# chid exists
		self.assertRaises(dontmanage.ValidationError, convert_to_group_or_ledger, warehouse.name)
		dontmanage.delete_doc("Warehouse", child)
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	warehouse_id = dontmanageerp.encode_company_abbr(warehouse_name, company)
	if not dontmanage.db.exists("Warehouse", warehouse_id):
		w = dontmanage.new_doc("Warehouse")
...
def get_warehouse(**args):
	...
	if dontmanage.db.exists("Warehouse", args.warehouse_name + " - " + args.abbr):
		return dontmanage.get_doc("Warehouse", args.warehouse_name + " - " + args.abbr)
	else:
		w = dontmanage.get_doc(
			{
...
def get_warehouse_account(warehouse_name, company, company_abbr=None):
	...
	if not dontmanage.db.exists("Account", warehouse_name + " - " + company_abbr):
...
def get_group_stock_account(company, company_abbr=None):
	group_stock_account = dontmanage.db.get_value(
