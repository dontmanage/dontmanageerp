...
from dontmanageerp.stock.doctype.stock_reconciliation.stock_reconciliation import get_stock_balance_for
...
def execute(filters=None):
	...
	batch_balance_dict = dontmanage._dict({})
...
def get_serial_batch_bundle_details(sl_entries, filters=None):
	...
	_bundle_details = dontmanage._dict({})
	batch_entries = dontmanage.get_all(
...
def get_serial_and_batch_bundles(filters):
	SBB = dontmanage.qb.DocType("Serial and Batch Bundle")
	SBE = dontmanage.qb.DocType("Serial and Batch Entry")

	query = (
		dontmanage.qb.from_(SBE)
		.inner_join(SBB)
...
def get_items(filters):
	...
	query = dontmanage.qb.from_(item).select(item.name)
	conditions = []
...
def get_item_details(items, sl_entries, include_uom):
	...
	if include_uom:
		ucd = dontmanage.qb.DocType("UOM Conversion Detail")
		query = (
			query.left_join(ucd)
...
def get_opening_balance_from_batch(filters, columns, sl_entries):
	...
	opening_data = dontmanage.get_all(
		"Stock Ledger Entry",
	...
	sabb_table = dontmanage.qb.DocType("Serial and Batch Entry")
	query = (
...
def get_opening_balance(filters, columns, sl_entries):
	...
	for sle in list(sl_entries):
		if (
			...
			and dontmanage.db.get_value("Stock Reconciliation", sle.voucher_no, "purpose") == "Opening Stock"
		):
			last_entry = sle
...
def get_warehouse_condition(warehouse):
	warehouse_details = dontmanage.db.get_value("Warehouse", warehouse, ["lft", "rgt"], as_dict=1)
...
def get_item_group_condition(item_group, item_table=None):
	item_group_details = dontmanage.db.get_value("Item Group", item_group, ["lft", "rgt"], as_dict=1)
	if item_group_details:
		if item_table:
			ig = dontmanage.qb.DocType("Item Group")
			return item_table.item_group.isin(
				dontmanage.qb.from_(ig)
				.select(ig.name)
				.where(
