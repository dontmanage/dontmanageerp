...
dontmanage.query_reports["Stock Ledger"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			options: "Batch",
			on_change() {
				const batch_no = dontmanage.query_report.get_filter_value("batch_no");
				if (batch_no) {
					dontmanage.query_report.set_filter_value("segregate_serial_batch_bundle", 1);
				} else {
					dontmanage.query_report.set_filter_value("segregate_serial_batch_bundle", 0);
				}
			},
		},
		{
...
dontmanageerp.utils.add_inventory_dimensions("Stock Ledger", 10);
