...
def execute(filters=None):
	...
	for parent_item in parent_items:
		...
		for child_item in required_items:
			child_item_balance = stock_balance.get(child_item.item_code, dontmanage._dict())
		...
		for warehouse, company in warehouse_company_map.items():
			...
			for child_item_detail in required_items:
				child_item_balance = stock_balance.get(child_item_detail.item_code, dontmanage._dict()).get(
					warehouse, dontmanage._dict()
				)
				child_row = {
					"indent": 1,
...
def get_items(filters):
	pb_details = dontmanage._dict()
	item_details = dontmanage._dict()

	item = dontmanage.qb.DocType("Item")
	pb = dontmanage.qb.DocType("Product Bundle")
	...
	if parent_items:
		item = dontmanage.qb.DocType("Item")
		pb = dontmanage.qb.DocType("Product Bundle")
		pbi = dontmanage.qb.DocType("Product Bundle Item")

		child_item_details = (
			dontmanage.qb.from_(pbi)
			.inner_join(pb)
...
def get_stock_balance(filters, items):
	...
	stock_balance = dontmanage._dict()
	for d in sle:
		stock_balance.setdefault(d.item_code, dontmanage._dict())[d.warehouse] = d
...
def get_stock_ledger_entries(filters, items):
	...
	sle2 = dontmanage.qb.DocType("Stock Ledger Entry")
	...
	if date := filters.get("date"):
		...
		dontmanage.throw(_("'Date' is required"))
