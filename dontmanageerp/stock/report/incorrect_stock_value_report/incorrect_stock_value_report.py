...
from dontmanage.query_builder.functions import Min, Timestamp
...
def execute(filters=None):
	if not dontmanageerp.is_perpetual_inventory_enabled(filters.company):
		dontmanage.throw(
...
def get_unsync_date(filters):
	...
	if not date:
		date = (dontmanage.qb.from_("Stock Ledger Entry").select(Min(Field("posting_date")))).run()
		date = date[0][0]
