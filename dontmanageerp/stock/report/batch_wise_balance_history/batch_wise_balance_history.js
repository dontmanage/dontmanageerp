...
dontmanage.query_reports["Batch-Wise Balance History"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			options: "Warehouse",
			get_query: function () {
				let warehouse_type = dontmanage.query_report.get_filter_value("warehouse_type");
		...
		},
		{
			...
			options: "Batch",
			get_query: function () {
				let item_code = dontmanage.query_report.get_filter_value("item_code");
				return {
					filters: {
						item: item_code,
					},
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (column.fieldname == "Batch" && data && !!data["Batch"]) {
			value = data["Batch"];
			column.link_onclick =
				"dontmanage.query_reports['Batch-Wise Balance History'].set_batch_route_to_stock_ledger(" +
				JSON.stringify(data) +
				")";
		}
