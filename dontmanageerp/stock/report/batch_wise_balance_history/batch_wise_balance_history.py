...
from dontmanage.utils import add_to_date, cint, flt, get_datetime, get_table_name, getdate
...
def get_item_warehouse_batch_map(filters, float_precision):
	...
	for d in sle:
		iwb_map.setdefault(d.item_code, {}).setdefault(d.warehouse, {}).setdefault(
			d.batch_no, dontmanage._dict({"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0})
		)
...
def get_item_details(filters):
	item_map = {}
	for d in (dontmanage.qb.from_("Item").select("name", "item_name", "description", "stock_uom")).run(as_dict=1):
