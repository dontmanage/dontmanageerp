...
from dontmanage.utils import cint, date_diff, flt, getdate
...
from dontmanageerp.stock.utils import add_additional_uom_columns, is_reposting_item_valuation_in_progress
...
def execute(filters: Optional[StockBalanceFilter] = None):
	...
	if filters.get("company"):
		...
		company_currency = dontmanage.db.get_single_value("Global Defaults", "default_currency")
...
def apply_conditions(query, filters):
	...
	if to_date := filters.get("to_date"):
		...
		dontmanage.throw(_("'To Date' is required"))
...
def get_item_warehouse_map(filters: StockBalanceFilter, sle: List[SLEntry]):
	...
	float_precision = cint(dontmanage.db.get_default("float_precision")) or 3
	...
	for d in sle:
		...
		if group_by_key not in iwb_map:
			iwb_map[group_by_key] = dontmanage._dict(
				{
					"opening_qty": 0.0,
					"opening_val": 0.0,
					"in_qty": 0.0,
		...
		if d.posting_date < from_date or (
			...
			and dontmanage.db.get_value("Stock Reconciliation", d.voucher_no, "purpose") == "Opening Stock"
		):
...
def get_items(filters: StockBalanceFilter) -> List[str]:
	...
	if item_code := filters.get("item_code"):
		...
		return dontmanage.get_all("Item", filters=item_filters, pluck="name", order_by=None)
...
def get_item_details(items: List[str], sle: List[SLEntry], filters: StockBalanceFilter):
	...
	if uom := filters.get("include_uom"):
		uom_conv_detail = dontmanage.qb.DocType("UOM Conversion Detail")
		query = (
...
def get_item_reorder_details(items):
	item_reorder_details = dontmanage._dict()

	if items:
		item_reorder_details = dontmanage.get_all(
			"Item Reorder",
