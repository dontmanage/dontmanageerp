...
from dontmanage.utils import add_days, cint, date_diff, flt, getdate
...
from dontmanageerp.stock.report.stock_ageing.stock_ageing import FIFOSlots, get_average_age
from dontmanageerp.stock.utils import add_additional_uom_columns
...
class StockBalanceReport:
	...
	def set_company_currency(self) -> None:
		if self.filters.get("company"):
			self.company_currency = dontmanageerp.get_company_currency(self.filters.get("company"))
		else:
			self.company_currency = dontmanage.db.get_single_value("Global Defaults", "default_currency")

	def run(self):
		self.float_precision = cint(dontmanage.db.get_default("float_precision")) or 3
	...
	def prepare_opening_data_from_closing_balance(self) -> None:
		self.opening_data = dontmanage._dict({})
		...
		res = dontmanage.get_doc("Closing Stock Balance", closing_balance[0].name).get_prepared_data()

		for entry in res.data:
			entry = dontmanage._dict(entry)
	...
	def get_item_warehouse_map(self):
		...
		_system_settings = dontmanage.get_cached_doc("System Settings")
	...
	def initialize_data(self, item_warehouse_map, group_by_key, entry):
		...
		item_warehouse_map[group_by_key] = dontmanage._dict(
			{
	...
	def get_itemwise_conversion_factor(self):
		...
		table = dontmanage.qb.DocType("UOM Conversion Detail")
		query = (
			dontmanage.qb.from_(table)
			.select(
	...
	def get_variant_values_for(self):
		...
		attribute_info = dontmanage.get_all(
	...
	def get_opening_vouchers(self):
		...
		sr = dontmanage.qb.DocType("Stock Reconciliation")
...
def get_variants_attributes() -> list[str]:
	...
	return dontmanage.get_all("Item Attribute", pluck="name")
