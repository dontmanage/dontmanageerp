...
def execute(filters=None):
	if not filters:
		filters = {}
	float_precision = dontmanage.db.get_default("float_precision")

	avg_daily_outgoing = 0
	diff = ((getdate(filters.get("to_date")) - getdate(filters.get("from_date"))).days) + 1
	if diff <= 0:
		dontmanage.throw(_("'From Date' must be after 'To Date'"))
...
def get_delivered_items(filters):
	parent = dontmanage.qb.DocType("Delivery Note")
	child = dontmanage.qb.DocType("Delivery Note Item")
	query = (
	...
	parent = dontmanage.qb.DocType("Sales Invoice")
	child = dontmanage.qb.DocType("Sales Invoice Item")
	query = (
		dontmanage.qb.from_(parent)
		.from_(child)
...
def get_filtered_query(filters, table, query):
	if filters.get("from_date") and filters.get("to_date"):
		...
		dontmanage.throw(_("From and To dates are required"))
