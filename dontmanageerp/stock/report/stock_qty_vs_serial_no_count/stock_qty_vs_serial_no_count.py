...
def validate_warehouse(filters):
	...
	if not dontmanage.db.exists("Warehouse", {"name": warehouse, "company": company}):
		dontmanage.throw(_("Warehouse: {0} does not belong to {1}").format(warehouse, company))
...
def get_data(warehouse):
	serial_item_list = dontmanage.get_all(
		"Item",
		filters={
			"has_serial_no": True,
		},
	...
	for item in serial_item_list:
		total_serial_no = dontmanage.db.count(
			"Serial No",
		...
		# dontmanage.db.get_value returns null if no record exist.
		if not actual_qty:
			actual_qty = 0
