...
def get_batchwise_data_from_stock_ledger(filters):
	batchwise_data = dontmanage._dict({})
...
def get_batchwise_data_from_serial_batch_bundle(batchwise_data, filters):
	table = dontmanage.qb.DocType("Stock Ledger Entry")
	ch_table = dontmanage.qb.DocType("Serial and Batch Entry")
	batch = dontmanage.qb.DocType("Batch")
...
def get_query_based_on_filters(query, batch, table, filters):
	...
	if filters.warehouse:
		lft, rgt = dontmanage.db.get_value("Warehouse", filters.warehouse, ["lft", "rgt"])
