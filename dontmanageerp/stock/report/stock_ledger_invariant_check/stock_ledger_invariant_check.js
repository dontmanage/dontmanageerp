...
dontmanage.query_reports['Stock Ledger Invariant Check'] = {
	'filters': [
		{
			'fieldname': 'item_code',
			'fieldtype': 'Link',
	...
	onload(report) {
		report.page.add_inner_button(__('Create Reposting Entry'), () => {
			...
			if (!selected_rows.length) {
				dontmanage.throw(__('Please select a row to create a Reposting Entry'));
			}
			else if (selected_rows.length > 1) {
				dontmanage.throw(__('Please select only one row to create a Reposting Entry'));
			}
			else {
				dontmanage.confirm(__(message), () => {
					dontmanage.call({
						method: 'dontmanageerp.stock.report.stock_ledger_invariant_check.stock_ledger_invariant_check.create_reposting_entries',
						args: {
							rows: selected_rows,
							item_code: dontmanage.query_report.get_filter_values().item_code,
							warehouse: dontmanage.query_report.get_filter_values().warehouse,
						}
					});
				});
			}
