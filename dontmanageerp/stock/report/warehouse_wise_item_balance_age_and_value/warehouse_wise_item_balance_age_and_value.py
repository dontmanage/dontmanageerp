...
from dontmanageerp.stock.report.stock_analytics.stock_analytics import (
	get_item_details,
	get_items,
	get_stock_ledger_entries,
)
from dontmanageerp.stock.report.stock_balance.stock_balance import filter_items_with_no_transactions
from dontmanageerp.stock.utils import is_reposting_item_valuation_in_progress
...
def execute(filters=None):
	...
	itemwise_brand = dontmanage._dict(get_itemwise_brand(items))
	# sum bal_qty by item
...
def get_itemwise_brand(items):
	return dontmanage.get_all("Item", filters={"name": ("in", items)}, fields=["name", "brand"], as_list=1)
...
def validate_filters(filters):
	if not (filters.get("item_code") or filters.get("warehouse")):
		sle_count = flt(dontmanage.qb.from_("Stock Ledger Entry").select(Count("name")).run()[0][0])
		if sle_count > 500000:
			dontmanage.throw(_("Please set filter based on Item or Warehouse"))
...
def get_warehouse_list(filters):
	...
	warehouse = dontmanage.qb.DocType("Warehouse")
	lft, rgt = dontmanage.db.get_value("Warehouse", filters.get("warehouse"), ["lft", "rgt"])

	return (
		dontmanage.qb.from_(warehouse)
		.select("name")
