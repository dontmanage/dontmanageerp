...
def get_data(item):
	...
	variant_results = dontmanage.db.get_all(
		"Item", fields=["name"], filters={"variant_of": ["=", item], "disabled": 0}
	)

	if not variant_results:
		dontmanage.msgprint(_("There aren't any item variants for the selected item"))
		return []
	else:
	...
	for item_dict in variant_dicts:
		...
		for attribute in attribute_list:
			...
			if attr_dict and attr_dict.get(attribute):
				item_dict[dontmanage.scrub(attribute)] = attr_val_map.get(name).get(attribute)
...
def get_columns(item):
	...
	for entry in item_doc.attributes:
		columns.append(
			{
				"fieldname": dontmanage.scrub(entry.attribute),
...
def get_open_sales_orders_count(variants_list):
	open_sales_orders = dontmanage.db.get_list(
		"Sales Order",
...
def get_stock_details_map(variant_list):
	stock_details = dontmanage.db.get_all(
		"Bin",
		fields=[
...
def get_attribute_values_map(variant_list):
	attribute_list = dontmanage.db.get_all(
		"Item Variant Attribute",
