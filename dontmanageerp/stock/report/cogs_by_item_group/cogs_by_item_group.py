...
from dontmanage.utils import date_diff

from dontmanageerp.accounts.report.general_ledger.general_ledger import get_gl_entries
...
def update_filters_with_account(filters: Filters) -> None:
	account = dontmanage.get_value("Company", filters.get("company"), "default_expense_account")
...
def get_row(name: str, value: float, is_bold: int, indent: int) -> Row:
	item_group = name
	if is_bold:
		item_group = dontmanage.bold(item_group)
	return dontmanage._dict(item_group=item_group, cogs_debit=value, indent=indent)
...
def get_item_groups_dict() -> ItemGroupsDict:
	item_groups_list = dontmanage.get_all("Item Group", fields=("name", "is_group", "lft", "rgt"))
	return {
