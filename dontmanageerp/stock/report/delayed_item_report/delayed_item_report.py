...
class DelayedItemReport:
	...
	def get_data(self, consolidated=False):
		...
		parent = dontmanage.qb.DocType(doctype)
		child = dontmanage.qb.DocType(f"{doctype} Item")

		query = (
			dontmanage.qb.from_(child)
			.from_(parent)
			.select(
				child.item_code,
				child.item_name,
	...
	def filter_transactions_data(self, consolidated=False):
		...
		if dontmanage.db.has_column(doctype, "parent"):
			fields.append("parent")

		for d in dontmanage.get_all(doctype, filters=filters, fields=fields):
