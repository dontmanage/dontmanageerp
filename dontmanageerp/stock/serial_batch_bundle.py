...
from dontmanage.query_builder.functions import CombineDatetime, Sum, Timestamp
from dontmanage.utils import add_days, cint, cstr, flt, get_link_to_form, now, nowtime, today
from pypika import Order

from dontmanageerp.stock.deprecated_serial_batch import (
	DeprecatedBatchNoValuation,
	DeprecatedSerialNoValuation,
)
from dontmanageerp.stock.valuation import round_off_if_near_zero
...
class SerialBatchBundle:
	...
	def set_item_details(self):
		...
		self.item_details = dontmanage.get_cached_value("Item", self.sle.item_code, fields, as_dict=1)
	...
	def is_material_transfer(self):
		...
		if (
			...
			and dontmanage.get_cached_value("Stock Entry", self.sle.voucher_no, "purpose") in allowed_types
		):
			return True

	def make_serial_batch_no_bundle_for_material_transfer(self):
		from dontmanageerp.controllers.stock_controller import make_bundle_for_material_transfer

		bundle = dontmanage.db.get_value(
	...
	def validate_actual_qty(self, sn_doc):
		...
		if not condition and self.sle.actual_qty:
			...
			dontmanage.throw(_(msg), title=_("Incorrect Type of Transaction"))
	...
	def validate_item(self):
		...
		if self.sle.actual_qty > 0:
			...
			if (
				...
				and not dontmanage.db.get_single_value("Stock Settings", "naming_series_prefix")
			):
	...
	def set_serial_and_batch_bundle(self, sn_doc):
		...
		if sn_doc.is_rejected:
			...
			dontmanage.db.set_value(self.child_doctype, self.sle.voucher_detail_no, values_to_update)
	...
	def validate_item_and_warehouse(self):
		if self.sle.serial_and_batch_bundle and not dontmanage.db.exists(
	...
	def delink_serial_and_batch_bundle(self):
		...
		dontmanage.db.set_value(self.child_doctype, self.sle.voucher_detail_no, update_values)
	...
	def post_process(self):
		...
		if self.sle.serial_and_batch_bundle:
			docstatus = dontmanage.get_cached_value(
	...
	def cancel_serial_and_batch_bundle(self):
		...
		doc = dontmanage.get_cached_doc("Serial and Batch Bundle", self.sle.serial_and_batch_bundle)
		if doc.docstatus == 1:
			doc.cancel()
	...
	def submit_serial_and_batch_bundle(self):
		doc = dontmanage.get_doc("Serial and Batch Bundle", self.sle.serial_and_batch_bundle)
	...
	def set_warehouse_and_status_in_serial_nos(self):
		from dontmanageerp.stock.doctype.serial_no.serial_no import get_serial_nos as get_parsed_serial_nos
		...
		if status == "Delivered":
			warranty_period = dontmanage.get_cached_value("Item", self.sle.item_code, "warranty_period")
			if warranty_period:
	...
	def update_batch_qty(self):
		...
		if not self.sle.serial_and_batch_bundle and self.sle.batch_no:
			batches = dontmanage._dict({self.sle.batch_no: self.sle.actual_qty})

		batches_qty = get_available_batches(
			dontmanage._dict({"item_code": self.item_code, "batch_no": list(batches.keys())})
		)

		for batch_no in batches:
			dontmanage.db.set_value("Batch", batch_no, "batch_qty", batches_qty.get(batch_no, 0))
...
def get_serial_nos(serial_and_batch_bundle, serial_nos=None):
	...
	serial_nos = dontmanage.get_all("Serial and Batch Entry", filters=filters, order_by="idx", pluck="serial_no")
...
def get_serial_or_batch_nos(bundle):
	...
	bundle_data = dontmanage.get_cached_value(
	...
	data = dontmanage.get_all("Serial and Batch Entry", fields=fields, filters={"parent": bundle})
...
class SerialNoValuation(DeprecatedSerialNoValuation):
	...
	def calculate_stock_value_change(self):
		if flt(self.sle.actual_qty) > 0:
			self.stock_value_change = dontmanage.get_cached_value(
	...
	def get_incoming_rate_from_bundle(self, serial_no) -> float:
		bundle = dontmanage.qb.DocType("Serial and Batch Bundle")
		bundle_child = dontmanage.qb.DocType("Serial and Batch Entry")
...
def is_rejected(voucher_type, voucher_detail_no, warehouse):
	if voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
		return warehouse == dontmanage.get_cached_value(
...
class SerialBatchCreation:
	...
	def set_item_details(self):
		...
		item_details = dontmanage.get_cached_value("Item", self.item_code, fields, as_dict=1)
	...
	def duplicate_package(self):
		...
		package = dontmanage.get_doc("Serial and Batch Bundle", id)
		new_package = dontmanage.copy_doc(package)
	...
	def make_serial_and_batch_bundle(self):
		doc = dontmanage.new_doc("Serial and Batch Bundle")
		...
		if doc.voucher_no and dontmanage.get_cached_value(doc.voucher_type, doc.voucher_no, "docstatus") == 2:
			doc.voucher_no = ""
	...
	def update_serial_and_batch_entries(self):
		doc = dontmanage.get_doc("Serial and Batch Bundle", self.serial_and_batch_bundle)
	...
	def validate_qty(self, doc):
		if doc.type_of_transaction == "Outward" and self.actual_qty and doc.total_qty:
			...
			if required_qty - total_qty > 0:
				...
				dontmanage.throw(msg, title=_("Insufficient Stock"))
		...
		kwargs = dontmanage._dict(
			{
				...
				"based_on": dontmanage.db.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
			}
		)
	...
	def set_auto_serial_batch_entries_for_inward(self):
		...
		if self.has_serial_no:
			...
			self.batches = dontmanage._dict({self.batch_no: abs(self.actual_qty)})

	def make_serial_no_if_not_exists(self):
		non_exists_serial_nos = []
		for row in self.serial_nos:
			if not dontmanage.db.exists("Serial No", row):
	...
	def create_batch(self):
		...
		if self.is_rejected:
			...
			if bundle:
				if batch_no := dontmanage.db.get_value("Serial and Batch Entry", {"parent": bundle}, "batch_no"):
					return batch_no
...
def get_serial_or_batch_items(items):
	serial_or_batch_items = dontmanage.get_all(
		"Item",
